...
{	
	...
	while(1)
	{
		...
		{
			sSetFBInvCtrlSts(cFBWait);
			if(gi_uwOPRelayOn == true && !subGetParaBatUnderSts() && !subGetParaBatPowerDownSts() && mChkDCDCLLCOn())
			{
				g_uwPVBoostWork = false;
				gi_uwGridRelayOn = false;
				gi_uwGridNRelayOn = false;
...
//			sSetDCDCCtrlSts(cDCDCWait);
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			{
				if(!g_wSolarSigPowerLoad && (subGetParaBatUnderSts() || subGetParaBatOpenSts() ||subGetParaBatPowerDownSts()))
				{
					if(uwLineOKDelay > 50)
					{
						uwLineOKDelay = 250;
				...
				if(uwLineOKDelay < 250)
				{
					...
					|| g_wSysLiBatActFlg || fBMSStopDischgFlg /*|| g_wBusRealVoltLowFlg*/)
				{
					if(g_wSolarSigPowerLoad && (g_uwRLineRms3timeAvgPeak > (g_uwPBusAvgVoltNew + cBusVoltReal100V)))
					{
						g_uwWorkMode = cStandbyMode;
			...
			else
			{
				...
				{
					//g_uwWorkMode = cStandbyMode;
					OSEventSend(cPrioPara, eParaToStandbyMode);//Soc点关闭电池
				}
...
void sFaultMode(void)
{
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			else if(uwFaultCodeTemp == cPVCircirtTempOver || uwFaultCodeTemp == cBatCircirtTempOver
				|| uwFaultCodeTemp == cInvCircirtTempOver || uwFaultCodeTemp == cInnelTempOver)
			{
				if(swGetEEOverTempRstEn() &&!fSolarBSTTempOver && !fConvertTempOver 
					&& !fInnelTempOver && !fInvTempOver)
				{
					uwFaultRestartFlg = true;
				}
				else
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			else
			{
				...
				if(	(!g_uwLoadOnSts || subGetParaBatPowerDownSts() || g_uwWorkMode == cFaultMode) 
				...
				)
				{
					...
					{
						hoSPSSDOn;
						if(uwShutDownChkCnt > 1000 || (g_uwPBusAvgVoltNew < 800))	//Wait 15s
						{
