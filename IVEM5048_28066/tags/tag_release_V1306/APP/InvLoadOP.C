...
void sPowerPriorityProc(void)
{
	...
	//if(g_dwInvChgLimit>cGridRateCurr)g_dwInvChgLimit=cGridRateCurr;
	...
	if(mChkOPRlyOn())
	{
		g_dwBatChgLimit = (INT32S)cMaxACInputCurr - (INT32S)g_uwROPCurr;  //cGridRateCurr
...
//	}
	...
	if(g_dwPVFeedLoadLimit > 2121) 		//15A
	{
		g_dwPVFeedLoadLimit = 2121;
	}
	...
	if(g_dwInvChgLimit > 2121) 
	{
		g_dwInvChgLimit = 2121;
	}
...
#else
	if(g_dwPVFeedLoadLimit > 4240) 		//30A
	{
		g_dwPVFeedLoadLimit = 4240;
	}
	else if(g_dwPVFeedLoadLimit < 20)
	{
		g_dwPVFeedLoadLimit = 20;
	...
	if(g_dwInvChgLimit > 4240) 
	{
		g_dwInvChgLimit = 4240;
	}
	else if(g_dwInvChgLimit < -g_dwPVFeedLoadLimit)
	{
		g_dwInvChgLimit = -g_dwPVFeedLoadLimit;
...
//		}
	#if mModel == cModel3024
		if(gi_wInvChgCurrLimit < 2121)
		{
			gi_wInvChgCurrLimit ++;
		}
		else
		{
			gi_wInvChgCurrLimit = 2121;
		}
	#else
		if(gi_wInvChgCurrLimit < 4240)
		{
			gi_wInvChgCurrLimit ++;
		}
		else
		{
			gi_wInvChgCurrLimit = 4240;
		}
	#endif
		...
		if(g_uwWorkMode == cBatteryMode)
		{
			if(g_wOPVoltFresh)
			{
				...
				if(++g_wBatAvgVoltCnt >= 8)
				{
					...
					if(wOPWattAvg > (wDerateWatt + 50))
					{
						g_wOPVoltCntl -= 10;
					}
					else if((wOPWattAvg + 50) < wDerateWatt)
					{
						g_wOPVoltCntl += 10;
					}
