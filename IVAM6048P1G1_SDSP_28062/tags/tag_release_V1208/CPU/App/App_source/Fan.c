...
void Fan_Task_20ms(void)
{
	// Fan_FanFaultCheck();  // 风扇检测电路还有问题  先屏蔽故障 @todo Ivan 240111
	Fan_FanControl();
...
static void Fan_FanControl(void)
{
	...
	if(1 == g_FanVar.ubFanFlag.bit.FanPwmOn)
	{
		...
		if(g_FanVar.uiFanOnStartDelay > 40)//cDelay20s_20ms
		{
	        // 简单软起一下
	...
	else
	{
		...
		else
		{
			...
			{
				if(++g_FanVar.uiFanOff1msCnt >= 2)//cDelay1s_20ms
				{
					g_FanVar.uiFanOff1msCnt = 0;
