...
//    if(MPPTGLOBALSCAN == strMpptCtrlData_Pv1.uiMpptSearchState)  uiDelyPeriod = 2;  //200ms
//	else  														 uiDelyPeriod = cDelay2s_100ms;
	
    //MPPT 1s扰动,全局扫描100ms扰动
	if(++uiTimeCnt >= cDelay2s_100ms /*|| MPPTGLOBALSCAN == strMpptCtrlData_Pv1.uiMpptSearchState*/)
	{
		//===============================================================
		//Pv Mppt
		//===============================================================
...
void Pv_MpptSearching(Struct_MpptCtrlData *pstrMpptControlData,Struct_GlobalScanData *pstrGlobalScanData,float *pPvVoltRef,float fPvRealVolt)
{
	...
	else if(MPPTGLOBALSCAN == pstrMpptControlData->uiMpptSearchState)
	{
		...
		else  if(SCANPREPARE == pstrGlobalScanData->uiGlobalScanState)
		{
		    ...
		    *pPvVoltRef = g_PvVar.fInputVoltforMppt - Set_PvVolt(4);  //从开路电压开始扫描
		    pstrGlobalScanData->uiPvDataAvgCalc = TRUE;
		...
		else  if(SCANNING == pstrGlobalScanData->uiGlobalScanState)
		{
			...
			*pPvVoltRef = *pPvVoltRef - Set_PvVolt(4);	//先设定4V，效果不好再考虑变步长 

			//占空比大于80%或PV电压低于90V停止扫描
		    if((g_PvVar.fPv1Pwm < 0.8) && (*pPvVoltRef > MpptMinVoltLimit)) 
		    { 
		        pstrGlobalScanData->uiPvDataAvgCalc = TRUE;
		    }
			else
