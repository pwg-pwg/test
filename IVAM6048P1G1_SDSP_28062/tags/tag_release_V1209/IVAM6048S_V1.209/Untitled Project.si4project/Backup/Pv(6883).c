...
void Pv_System_AppInit(void)
{
    ...
    //1.0表示的是400V 调制公式=1-Vi/(Vi+Vout) 700-850 只要调节150V 故限制0.2
    g_strPv1CurrLoop.integTopLimit = 0.85;  // 占空比先限制到50%
    ...
    //调制波的上下限，以700V（母线额定电压值）为定标
    //1.0表示的是700V 调制公式=1-Vi/(Vi+Vout) 700-850 只要调节150V 故限制0.2
...
void Pv_Pv1PwmOutput(float fLoopOut)
{
    ...
    UpDownLimit(g_PvVar.fPv1Pwm,g_strPv1CurrLoop.integTopLimit, 0);  //Boost占空比限制到0.8 即VBus(Max)=Vpv/0.2 = 120/0.2 = 600
...
void Pv_PvMpptSoftStart(void)
{
    ...
    {
        g_PvVar.fPv1MpptVoltRef = stValue.fAveScale.VoltPv1 * 0.85;
    }
    ...
    if(ePvRunState_MpptCtrl == g_PvVar.PvFlag.bit.PvRunSts)
    {
		...
		{
            g_PvVar.fPv2MpptVoltRef = stValue.fAveScale.VoltPv2 * 0.85;
        }
    }
    else
    {
	...
	if(++uiTimeCnt >= cDelay2s_100ms)//1s扰动
	{
		...
		if(FALSE == g_PvVar.PvFlag.bit.Pv1PowerDerate)
		{
			...
			else
			{
				...
				g_PvVar.fMppt1VoltRefSet = stValue.fAveScale.VoltPv1 * 0.85;
			}
		}
		...
		if(FALSE == g_PvVar.PvFlag.bit.Pv2PowerDerate)
		{
			...
			else
			{
				...
				g_PvVar.fMppt2VoltRefSet = stValue.fAveScale.VoltPv2 * 0.85;;
			}
		}
		else	//PV充足，顶母线后不作MPPT
		{
