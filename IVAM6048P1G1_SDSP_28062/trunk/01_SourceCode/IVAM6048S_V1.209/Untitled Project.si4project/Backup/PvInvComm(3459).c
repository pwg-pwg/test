...
void IntSci_Task_20ms(void)
{
    ...
    strSlvState.ubState3.bit.rsvd14 = hiFan2Clk ;
    strSlvState.ubState3.bit.rsvd15 = hiFan1Clk ;
    ...
    uiSlaveAnalogData[eS2MRealData_LLC1Temp] = ((int16)stAdcTemp.Ave.LLC_TX_Temp); 	// LLC变压器温度，*10倍
    uiSlaveAnalogData[eS2MRealData_LeadAcidTemp] = ((int16)stAdcTemp.Ave.Bat_Temp); // 外部电池温度，*10倍
    uiSlaveAnalogData[eS2MRealData_LLC3Temp] =  ((int16)stAdcTemp.Ave.Pv_Temp); 	// PV电感温度，*10倍
    uiSlaveAnalogData[eS2MRealData_LLC4Temp] = ((int16)stAdcTemp.Ave.LLC_Temp);     // LLC-IGBT散热器温度，*10倍
    uiSlaveAnalogData[eS2MRealData_HS1Temp] = ((int16)stAdcTemp.Ave.Inv_Temp); 		// INV电感温度，*10倍
    uiSlaveAnalogData[eS2MRealData_HS2Temp] = 0;
    uiSlaveAnalogData[eS2MRealData_InnelTemp] = 0;//((int16)stAdcTemp.Ave.InnelTemp);
    uiSlaveAnalogData[eS2MRealData_PvIsoVolt] = stValue.fAveReal.PvIosVolt * 10;
    uiSlaveAnalogData[eS2MRealData_PosBusVolt] = stValue.fAveReal.VPBus * 10;
	...
	switch( unIntSciFlag.bit.bExecuteStep )
	{
		...
		case eIntSciReceiving:			//接收中
		{
			if( IntSciRxRdyStatus == eEnable )
			{
				...
				uiIntSciFrameEndTimeCnt = 0;
				if((uiIntSciRxDataBuff[0] != 'T')&&((uiIntSciFrameStartTimeCnt < uiIntSciFrameStartMinTime)||(uiIntSciRxDataBuff[0] != uiIntSciSlaveAddr)))
				{
					uiIntSciFrameStartTimeCnt = 0;
					uiIntSciRxDataLengthCnt = 0;
				}
...
void IntSci_ReceiveDataCheck(void)
{
	...
	strMstState.ubState1.bit.bAfciCommEn = uiMasterAnalogData[eM2SRealData_AFCIEn];
    ...
    UpDownLimit(g_SystemInfo.fPvMaxPowerSetRef,2,0);//6000*2 = 12000
...
//		g_SystemInfo.fPv2MaxPowerSet = g_SystemInfo.fPvMaxPowerSetRef * 0.5;
        ...
        UpDownLimit(g_SystemInfo.fPv1MaxPowerSet,1,0);//6000
        UpDownLimit(g_SystemInfo.fPv2MaxPowerSet,1,0);//6000
	}
	else
	{	
