...
void Pv_GolbalScan(Struct_MpptCtrlData *pstrMpptControlData,Struct_GlobalScanData *pstrGlobalScanData);
...
//				if(strGlobalScanPv1.fMaxPower != 0)
//				{
//					strGlobalScanPv1.fDeltaPower = abs(g_PvVar.fInputPowerforMppt - strGlobalScanPv1.fMaxPower) / strGlobalScanPv1.fMaxPower;
//				}
				bTest[0]++;
				if(sSysPara[eSetDataID3] == 1)
				{
				    sSysPara[eSetDataID3] = 0;
					Pv_GolbalScan(&strMpptCtrlData_Pv1,&strGlobalScanPv1);
				}
...
//				if(strGlobalScanPv2.fMaxPower != 0)
//				{
//					strGlobalScanPv2.fDeltaPower = abs(g_PvVar.fInputPowerforMppt - strGlobalScanPv2.fMaxPower) / strGlobalScanPv2.fMaxPower;
//				}

				if(sSysPara[eSetDataID7] == 1)
				{
					sSysPara[eSetDataID7] = 0;
					Pv_GolbalScan(&strMpptCtrlData_Pv2,&strGlobalScanPv2);
				}
...
/******************************************************************************
Function Name		 : Pv_GolbalScan
Function Descriptions:
Parameter Name list  :
ReturnType           :
Refer documents      :单PV带载不启用这个功能？TODO
******************************************************************************/
void Pv_GolbalScan(Struct_MpptCtrlData *pstrMpptControlData,Struct_GlobalScanData *pstrGlobalScanData)
{
//	static Uint16 uiGlobalScanEnPre = 0;
	//加上非无电池模式作为条件	TODO
//    if(TRUE == uiSciAnalogDataBag[eM2SRealData_MultiPointEn])
//	if(sSysPara[eSetDataID3] == 1)
//	{
//	    sSysPara[eSetDataID3] = 0;
	    //30min，全局扫描的时候不累计时间
...
//				||(pstrGlobalScanData->fDeltaPower > 0.1))  //功率突变重启扫描,先屏蔽
    	    {
				...
				//理论上这里不需要清功率计算值，进这里的时候MPPT的功率计算已经完成 TODO
//				uiGlobalScanEnPre = 1;  //后面再加，针对扫描过程中突然关闭扫描功能做处理
    	    }
    	}
//    }
//	else
...
//		pstrGlobalScanData->uiGlobalScanState = SCANINITIAL;
		...
		//后面再加,
//		if(uiGlobalScanEnPre != uiSciAnalogDataBag[eM2SRealData_MultiPointEn])  
//		{
//		    uiGlobalScanEnPre = uiSciAnalogDataBag[eM2SRealData_MultiPointEn];
...
void Pv_MpptSearching(Struct_MpptCtrlData *pstrMpptControlData,Struct_GlobalScanData *pstrGlobalScanData,float *pPvVoltRef,float fPvRealVolt)
{
	...
	else if(MPPTGLOBALSCAN == pstrMpptControlData->uiMpptSearchState)
	{
		...
		else  if(SCANNING == pstrGlobalScanData->uiGlobalScanState)
		{
			...
			//占空比过大或PV电压低于90V停止扫描
		    if((g_PvVar.fPv1Pwm < fScanDutyUpLimit) && (*pPvVoltRef > MpptMinVoltLimit)) 
		    { 
		        pstrGlobalScanData->uiPvDataAvgCalc = TRUE;
		    }
			else
