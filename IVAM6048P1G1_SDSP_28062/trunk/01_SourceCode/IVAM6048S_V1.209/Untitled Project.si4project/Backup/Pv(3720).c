...
//	static int16 Pv1BusChkCnt = 0,Pv1BusLimitFlg = FALSE;
    ...
    if((MPPTGLOBALSCAN == strMpptCtrlData_Pv1.uiMpptSearchState) && (SCANPREPARE == strGlobalScanPv1.uiGlobalScanState))
	{
		 ...
		 g_strPv1VoltLoop.Out = 0;	
	     g_strPv1CurrLoop.Ref = cPvCurrRef0_5A;   //通过将PV电流限制到0.5A来获取开路电压，需要确认环路有没有给定突变的问题，如A->0.5->A
    }
	else
	{
...
void Pv_PvPowerAvgCalc(void)
{
	...
	{
	    //400ms
//	    uiAvgCalcStartCnt = 10;   //200ms
//		uiAvgCalcEndCnt = 17;     //7+1->160ms
//		fAvgCalDiv = 0.125;  	  //1/8
//		uiDataAvgCalcEnable = 1;
        //200ms
        uiAvgCalcStartCnt = 5;    //100ms
		uiAvgCalcEndCnt = 8;      //8+1->180ms
		fAvgCalDiv = 0.25;  	  //1/4
		uiDataAvgCalcEnable = 1;
//先改成1s看效果
...
//	static Uint16 uiGlobalScanEnPre = 0;
    ...
    if(MPPTGLOBALSCAN == strMpptCtrlData_Pv1.uiMpptSearchState)  uiDelyPeriod = 2;  //400ms
	else  														 uiDelyPeriod = cDelay2s_100ms;
...
//			}
			...
			//占空比大于70%或PV电压低于90V停止扫描
		    if((g_PvVar.fPv1Pwm < 0.7) && (*pPvVoltRef > MpptMinVoltLimit)) 
		    { 
		        pstrGlobalScanData->uiPvDataAvgCalc = TRUE;
		    }
			else
