...
void Fault_Initialize(void)
{
	...
	uimdlIdentifyFailValue = g_SysFault.ubFaultSys.bit.MdlIdentifyFail;
    g_SysFault.ubFaultSys.fault.fault1 = 0;
    g_SysFault.ubFaultSys.fault.fault2 = 0;
	g_SysFault.ubFaultSys.bit.MdlIdentifyFail = uimdlIdentifyFailValue;
...
static void Fault_InvOutShortCheck(void)
{
    ...
    {
        if(++s_ulClearLockFlagCnt >= MinuteBy20msBase(60) || !g_ComInfo.Com1.bit.KeyOn || TRUE == uiSciCommandDataBag[eComFaultClear])
        {
...
//                FaultCodeClear(eInvShort);
    ...
    {
        if(++s_ulLLClearLockFlagCnt >= MinuteBy1msBase(60) || !g_ComInfo.Com1.bit.KeyOn || TRUE == uiSciCommandDataBag[eComFaultClear])
        {
...
static void Fault_InvSoftFailCheck(void)
{
    ...
    {
        if(++s_ulInvSoftFaultClearLockFlagCnt >= MinuteBy20msBase(60) || !g_ComInfo.Com1.bit.KeyOn || TRUE == uiSciCommandDataBag[eComFaultClear])
        {
...
static void Fault_GridOverLoadCheck(void)
{
    ...
    {
        if(++s_ulGenClearLockFlagCnt >= MinuteBy20msBase(60) || !g_ComInfo.Com1.bit.KeyOn || TRUE == uiSciCommandDataBag[eComFaultClear])
        {
            s_uiGenLoadOverLockedFlag = FALSE;
...
static void Fault_GridOverCurrCheck(void)
{
    ...
    // third： Fault_Locked   clear
    if(TRUE == s_uiCurrOverLockedFlag)//过载故障锁死,延时60Min 或 松开开机键 或清除故障设置，将锁死标志清零
    {
        if(++s_ulClearLockFlagCnt >= MinuteBy20msBase(60) || !g_ComInfo.Com1.bit.KeyOn || TRUE == uiSciCommandDataBag[eComFaultClear])
        {
            s_uiCurrOverLockedFlag = FALSE;
            s_ulClearLockFlagCnt = 0;
            s_u16CntEnter = SecondBy20msBase(10); // 非清除故障设置，立即清除故障，无需恢复延时
        }
    }
}
...
static void Fault_BusOverVoltCheck(void)
{
    ...
    {
        if(++s_ulBusOvpClearLockFlagCnt >= MinuteBy20msBase(60) || !g_ComInfo.Com1.bit.KeyOn || TRUE == uiSciCommandDataBag[eComFaultClear])
        {
...
static void Fault_BatAbnormalCheck(void)
{
	...
	//------------- BAT异常快速检测 -------------//
	//ATE电池源没有并联充电负载的时候容易误报
	if(!g_SysFaultEn.bit.BatAbnormalChk || (uiSciCommandDataBag[eComATECommand] & eAteBatAbnChKDisEnBit))
	{
            ...
            || (g_SystemInfo.SysFlag.bit.PvWork) )
        {
            if(eDcDcMainState_Running == g_DcDcVar.DcDcFlag.bit.bMainSts)
            {
                ...
                else if((eBatState_DisCharge == uwBatState) && (fabs(stADC.fRealScale.IDcDc) < fBatAbnormalChkCurrValue))
                {
                    ...
                    // 20260607--BMS内部截至电压为48V时，此时可能会因为BUs纹波过大导致误触发电池异常，进而造成PV无法充电，所以先屏蔽。|
...
static void Fault_PBACTempOver(void)
{
    ...
    {
		if((FALSE == strSlvState.ubState2.bit.Fan1Err) && (FALSE == strSlvState.ubState2.bit.Fan2Err))
		{
    ...
    if(s_u16PvWeakChkCnt)
    {
        ...
        if(g_SysAlarm.ubAlarmSys.bit.PvWeak) // 触发PV功率不足告警
        {
            if(   (++s_u16PvWeakRecoverCnt >= MinuteBy20msBase(15))    // 延时15Min恢复
               ...
               || (TRUE == uiSciCommandDataBag[eComFaultClear])        // 故障清除
               )
            {
                s_u16PvWeakChkCnt = 0;
