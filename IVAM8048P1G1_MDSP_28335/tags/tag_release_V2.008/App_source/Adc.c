...
//    stHwGain.fVN2Pe = cNEVoltHwGain  ;
    ...
    stADC.fGain.CurrCtABack = stADC.fGain.CurrCtA;
...
{   // 无片选
    ...
    // stADC.iSample.VBat      = (AdcRegs.ADCRESULT0 >> 4);//todo
...
//        stValue.lAcc2.VLineAB = 0;
        ...
        //这里是CT电流有效值，CT功率计算的周期点，在此处更新，完成一个有效值/功率的切换 ---
        //CT电流采样当前的作用：①计算有效值电流显示  ②实时电流计算功率，功率用于控制，---- 都是工频周期的值作用于控制
        if(stADC.fGain.CurrCtA != stADC.fGain.CurrCtABack)
        {
            stADC.fGain.CurrCtA = stADC.fGain.CurrCtABack;
        }
...
//    stADC.fGain.CurrCtA    = 1.0 / stHwGain.fCTCurr * cKADConversion * g_GridManager.Rated.f32IOutInvt;
    stADC.fGain.CurrCtABack= 1.0 / stHwGain.fCTCurr * cKADConversion * g_GridManager.Rated.f32IOutInvt;
...
static void Adc_SamplePeriodPointCalc(void)
{
    ...
    if(0 != g_StrEcap.FreqFilt.Inv)
    {
        ...
        UpDownLimit(uiSamplePointTemp, cSamplePointUp35Hz, cSamplePointDown70Hz);
        stValue.ui20msPeriodSamplePoint = uiSamplePointTemp; // 20260124 42HZ电网充电时，会因为电池电流纹波较大，且固定50HZ计算平均值，导致显示的电池电流和真实的不服，所以此处改为跟随真实Inv频率
...
void Adc_VOutTransferRatioCal(void)
{
    ...
    if(eInv_OffGridMode == g_InvVar.InvFlag.bit.InvWorkMode)
    {
        ...
        if(++s_uwInvVoltAvgCnt >= 2)
        {
            ...
            Dnlimit(fInvVoltAvg, 60.0f);

            fOPVoltRatio = (g_GridManager.Rated.f32VOut * g_GridManager.Rated.f32VOut) / (fInvVoltAvg * fInvVoltAvg);
            UpDownLimit(fOPVoltRatio, 16.0f, 1.0f);  //（240*240)/(60*60)=16，负载越大电压跌落越多，会导致负载百分比无法触发2倍过载保护，保护时间过长。
...
void Adc_VoltFloDeal(void)
{
    ...
    // //----------- Inv-Curr------------//
    // fInvFloSum -= fInvFloSave[uiFloCnt];
    // fInvFloSave[uiFloCnt] = stValue.fRmsScale.IInvA;
    // fInvFloSum += fInvFloSave[uiFloCnt];
    // g_fInvCurrFilt = fInvFloSum * 0.3333f; // 1/3
}

/******************************************************************************
Function Name: Adc_PvCurrSumCalc
--------------------
Function Descriptions: 20ms Task
Parameter Name list: PV低压时大占空比和大电流都会导致温度升高，和PV高温一样也要降额升风机转速
ReturnType: 
Refer documents:
******************************************************************************/
void Adc_PvCurrSumCalc(void)
{
    static Uint16 uiPv1CurrSummyStart = 0, uiPv2CurrSummyStart = 0;
    static Uint16 uiPv1CurrAlarmCnt = 0, uiPv2CurrAlarmCnt = 0;
    static Uint16 uiPv1CurrAlarmFlag = 0, uiPv2CurrAlarmFlag = 0;

    //----------- Pv1------------//
    if(strSlvAnalog.fCurrPv1 > 20)       { uiPv1CurrSummyStart = TRUE;  }
    else if(strSlvAnalog.fCurrPv1 <= 19) { uiPv1CurrSummyStart = FALSE; }

    if(FALSE == uiPv1CurrAlarmFlag)
    {
        if(uiPv1CurrSummyStart)
        {
            if(uiPv1CurrAlarmCnt < MinuteBy20msBase(1))
            {
                uiPv1CurrAlarmCnt ++;
            }
            else
            {
                uiPv1CurrAlarmFlag = TRUE;
                uiPv1CurrAlarmCnt = MinuteBy20msBase(2);
            }
        }
        else if(uiPv1CurrAlarmCnt)
        {
            uiPv1CurrAlarmCnt --;
        }
    }
    else
    {
        if(uiPv1CurrSummyStart)
        {
            if(uiPv1CurrAlarmCnt < MinuteBy20msBase(2))
            {
                uiPv1CurrAlarmCnt ++;
            }
        }
        ...
        {
            if(uiPv1CurrAlarmCnt)
            {
                uiPv1CurrAlarmCnt --;
            }
            else
            {
                uiPv1CurrAlarmFlag = FALSE;
            }
        }
    }
    //----------- Pv2------------//
    if(strSlvAnalog.fCurrPv2 > 20)       { uiPv2CurrSummyStart = TRUE;  }
    else if(strSlvAnalog.fCurrPv2 <= 19) { uiPv2CurrSummyStart = FALSE; }

    if(FALSE == uiPv2CurrAlarmFlag)
    {
        if(uiPv2CurrSummyStart)
        {
            if(uiPv2CurrAlarmCnt < MinuteBy20msBase(1))
            {
                uiPv2CurrAlarmCnt ++;
            }
            else
            {
                uiPv2CurrAlarmFlag = TRUE;
                uiPv2CurrAlarmCnt = MinuteBy20msBase(2);
            }
        }
        else if(uiPv2CurrAlarmCnt)
        {
            uiPv2CurrAlarmCnt --;
        }
    }
    else
    {
        if(uiPv2CurrSummyStart)
        {
            if(uiPv2CurrAlarmCnt < MinuteBy20msBase(2))
            {
                uiPv2CurrAlarmCnt ++;
            }
        }
        ...
        {
            if(uiPv2CurrAlarmCnt)
            {
                uiPv2CurrAlarmCnt --;
            }
            else
            {
                uiPv2CurrAlarmFlag = FALSE;
            }
        }
    }
    //-------------------------------
    if(uiPv1CurrAlarmFlag || uiPv2CurrAlarmFlag)
    {
        g_FanVar.bit.FanSpeedUpByPvCurr = TRUE;
    }
    else
    {
        g_FanVar.bit.FanSpeedUpByPvCurr = FALSE;
    }
}
#endif // ADC_C
//===========================================================================
