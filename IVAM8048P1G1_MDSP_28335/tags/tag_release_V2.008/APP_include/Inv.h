...
#define cVInvOutUpLimit              ((float)(310.0f))
#define cVInvOutDnLimit              ((float)(80.0f))
#define cVInvRefUpLimit              ((float)(266.0f))
#define cVInvRefDnLimit              ((float)(88.0f))
//-----------------------------------------------------------------------------
...
typedef struct
{
    ...
    float32  fVInvOutUpLimit;      // 输出上限 V_out_max/cV_Inv_Rated
    // float32  fVInvOutDnLimit;      // 输出下限 V_out_max/cV_Inv_Rated
    float32  fVInvRefUpLimit;      // 离网给定上限
    float32  fVInvRefDnLimit;      // 离网给定下限
...
typedef struct
{
    ...
    float32 fRatedVoltDecratePCT;    // 离网降额系数 = Min[温度降额，电池电流降额，电池电压降额]

    float32 fBatCurrPCTSummyInPhase; // 该相电池电流百分比汇总
    float32 fPLoadPCTSummyInPhase;   // 该相负载百分比汇总
    float32 fPPvPCTSummyInPhase;     // 该相PV功率百分比汇总

    float32 fBatCurrPCTSummyInSys;   // 该系统电池电流百分比汇总
    float32 fPLoadPCTSummyInSys;     // 该系统负载百分比汇总
    float32 fPPvPCTSummyInSys;       // 该系统PV功率百分比汇总
	
	float32 fRmsCurr_Diff;          //并机电流差值
    float32 s_fUPMCurrRMSAMax;
    float32 s_fUPMCurrRMSAMin;

    // 平均电流环
    float32 fDroopInvCurraAveInteg;
    float32 fDroopInvCurraAveOut;

    float32 fDroopInvCurrAveKp;
    float32 fDroopInvCurrAveKi;

    float32 fDroopInvCurrIntegUpLimit;
    float32 fDroopInvCurrIntegDnLimit;
}ParaVarStr;
...
INV_PUBLIC float32 fInputVoltFeedForwardKp;
...
INV_PUBLIC void Inv_ParaInvCurrAvgLoop(void);   // 并机平均电流环路
//-----------------------------------------------------------------------------
#endif	// INV_H
//===========================================================================
// End of file.
