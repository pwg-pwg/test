...
static void Fault_BusUnderVoltFastCheck(void)
{
    ...
    {
        if(stADC.fRealScale.VBus < cVBusUnIsrLimit)  // 290V
        {
            s_u16ChkCnt ++;
            if(s_u16ChkCnt >= cCnt_5)
            {
    ...
    if(eBat_Normal == g_uiBatAbnormalFlg)
    {
            ...
            || (g_SystemInfo.SysFlag.bit.PvWork) )
        {
            if(eDcDcMainState_Running == g_DcDcVar.DcDcFlag.bit.bMainSts)
            {
                ...
                if((fabs(stValue.fAveScale.IDcDc) < fBatAbnormalChkCurrValue) && (uwCleanCnt1++ >= 38400))//SecondByIntBase(2);
                {
                    uwCleanCnt1 = 0;
                    if(uwCleanCnt2++ >= 15)
                    {
...
// stateTest[7] = (int16)((s_fBatVoltMax1) * 10);
// stateTest[5] = stateTest[4];
// stateTest[4] = stateTest[3];
// stateTest[3] = (int16)(mVBatRealCalc(stADC.fRealScale.VBat) * 10);                    
...
// if(stateTest[6] == 0)
// {
//     stateTest[3] = s_u16WaveMaxCnt1;
//     stateTest[4] = s_u16WaveMaxCnt2;
// }
                    s_u16WaveMaxCnt1 = 3;
                    s_u16WaveMaxCnt2 = 1;
                    s_u16BusMaxCnt3 = 3;//5;
