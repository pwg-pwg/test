...
void ECAP_Initialize(void)
{
	...
	Ecap_CalculateRelayAngle(&strPllToGrid, cGridRlyCloseDlyTime1, cGridRlyCloseDlyTime2, g_StrEcap.FreqFilt.Grid, 0, cRelayClose);
    Ecap_CalculateRelayAngle(&strPllToGrid, cGridRlyOpenDlyTime1, cGridRlyOpenDlyTime2, g_StrEcap.FreqFilt.Grid, 0, cRelayOpen);
    //----------------------------
    g_StrEcap.Freq.Inv = 50;
    g_StrEcap.FreqFilt.Inv = 50;
	Ecap_CalculateRelayAngle(&strPllToInv, cInvRlyCloseDlyTime1, cInvRlyCloseDlyTime2, g_StrEcap.FreqFilt.Inv, -2, cRelayClose);  //实测逆变点要前移
	Ecap_CalculateRelayAngle(&strPllToInv, cInvRlyOpenDlyTime1, cInvRlyOpenDlyTime2, g_StrEcap.FreqFilt.Inv, -2, cRelayOpen);  //实测逆变点要前移
    //----------------------------
    g_StrEcap.Freq.Gen = 50;
    g_StrEcap.FreqFilt.Gen = 50;
	Ecap_CalculateRelayAngle(&strPllToGen, cGridRlyCloseDlyTime1, cGridRlyCloseDlyTime2, g_StrEcap.FreqFilt.Gen, 0, cRelayClose); 
	Ecap_CalculateRelayAngle(&strPllToGen, cGridRlyOpenDlyTime1, cGridRlyOpenDlyTime2, g_StrEcap.FreqFilt.Gen, 0, cRelayOpen); 
    //----------------------------
    hoZeroSynIoTxOff;
    hoParaTxOff;
}
...
static void Ecap_PhaseFreqCalc(void)
{
    ...
    if(strPllToGrid.fFinalStepRad > cTmDivFreToStepRad(35) && strPllToGrid.fFinalStepRad < cTmDivFreToStepRad(70))   // 用这个判断的话没有电网的时候就
    {
        ...
        // [40hz, 65hz]
        // strPllToGrid.fRlyOnPoint1 = 270 - (cRlyCloseDlyTime2 * 360.0f * g_StrEcap.FreqFilt.Grid / 1000.0f);// 360 - ((RlyTime * 360 ) / 1000 / g_StrEcap.FreqFilt.Grid) - 90;
        // UpDownLimit(g_StrEcap.FreqFilt.Grid, 160.0f, 90.7f);
        // strPllToGrid.fRlyOnPoint2 = 270 - (cRlyCloseDlyTime1 * 360.0f * g_StrEcap.FreqFilt.Grid / 1000.0f);// 360 - ((RlyTime * 360 ) / 1000 / g_StrEcap.FreqFilt.Grid) - 90;
        // UpDownLimit(g_StrEcap.FreqFilt.Grid, 164.6f, 98.7f);
        // [35hz, 70hz]
...
*/
    ...
    // [35hz, 70hz]
    //--------- 逆变继电器吸合角度计算 ---------//
	Ecap_CalculateRelayAngle(&strPllToInv, cInvRlyCloseDlyTime1, cInvRlyCloseDlyTime2, g_StrEcap.FreqFilt.Inv, -2, cRelayClose); 

    //--------- 逆变继电器断开角度计算 ---------//
    Ecap_CalculateRelayAngle(&strPllToInv, cInvRlyOpenDlyTime1, cInvRlyOpenDlyTime2, g_StrEcap.FreqFilt.Inv, -2, cRelayOpen); 
}
/******************************************************************************
Function Name: Ecap_CalculateRelayAngle
-------------------------------------------
...
void Ecap_CalculateRelayAngle(Struct_PhaseLock *pRelayAngles, float fFlyTime, float fTotalTime, float fFreq, float fAngleOffset, Uint16 ActionType)
{
	...
	if(bMachineModel == eModelIVAM6048 || bMachineModel == eModelUnknown)
	{
	...
	{
        fRlyPoint1 = 90.0f - (fTotalTime * 0.36f * fFreq) + fAngleOffset;
        fRlyPoint2 = 90.0f - (fFlyTime * 0.36f * fFreq) + fAngleOffset;
	}

    if(uiFrameParallelEnable) // 并机增加适应范围
    {
        fRlyPoint1 += (-3);
        fRlyPoint2 += (3);
    }
...
ReturnType:             
Refer doctuments: 1/4 INT， 208us
******************************************************************************/
...
void Ecap_InvOcpCntCap6(void)
{
    ...
    // 1.硬件触发一次封波，其恢复时间约为15uS，所以要放在中断级检测产生的脉冲数量；
    // -- 20260104 因为中断时间不足而优化为1/4Isr中，同时硬件封波需要“与”上逆变驱动，所以单次封波肯定在52us。
    ...
    if(g_AdcFlag.bit.InvOneCycleDone) // 工频周期提取触发次数
    {
        ...
        if(uiInvOcpCntTemp2 > 73) // 73*208=15.184mS，单周期内持续低电平时间超过15mS，判定为异常，由于改为1/4中断中判断，所以改为。
        {
