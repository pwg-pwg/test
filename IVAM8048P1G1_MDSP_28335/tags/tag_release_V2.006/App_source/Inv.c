...
*/
    ...
    g_InvVar.fVInvOutLimit = 266.0f * cSqrtTwo * g_GridManager.Rated.f32VOutInvt;
...
//                            uwInvRlyDelayCnt = 0;
        ...
        case eInvMainState_OffGrid:
        {
            ...
            // 离网下不允许切旁路
                    ...
                    && (g_ComInfo.Com1.bit.InvForceOffAllow)
...
void Inv_OnGridControlLoop(void)
{
    ...
    else   // 软起后稳定运行环路
    {
        ...
        if(eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode)
        {
            if(fabs(stADC.fRealScale.VGridA) >= g_InvVar.fVInvOutLimit)
            {
        ...
        {
            if(fabs(stADC.fRealScale.VGenA) >= g_InvVar.fVInvOutLimit)
            {
                strOnGridCurrARegu.Ref = 0;
            }
        }
...
void Inv_OffGridRealVoltLoop(void)
{
    ...
    if(fabs(stADC.fRealScale.VInvA) >= g_InvVar.fVInvOutLimit)
...
void Inv_ParaDroopCtrl(void)
{
    ...
    if(uiFrameParallelEnable && (eInvMainState_OffGrid == g_InvVar.InvFlag.bit.bInvMainSts))
    {
        ...
        UpDownLimit(fDroopTemp, (50.0 * g_GridManager.Rated.f32VOutInvt), (-50.0 * g_GridManager.Rated.f32VOutInvt)); // 这里为定标值
        ...
        stateTest[4] = (int16)(fReal_Power(fInvReactivePowerReal));
        stateTest[5] = (int16)(fReal_Power(fInvReactivePowerFilt));
    }
    else
    {
...
static void Inv_DisChgCurrLimitByBatVolt(void)
{
    ...
    if(bMachineModel == eModelIVAM6048)
    {
    ...
    else if(bMachineModel == eModelIVAM8048)
    {
        // if(g_SystemInfo.fBatVoltAvgReal >= 48)      { fLoopLimitDerateScaleSet = 1.135f; }
        // else if(g_SystemInfo.fBatVoltAvgReal <= 47) { fLoopLimitDerateScaleSet = 1.0f;  }
    ...
    // 更新至电压环和电流环的限幅
    //------------------------------------
    strLoadVoltARealRegu.integTopLimit  = fLoopLimitDerateScale;//fLoopLimitDerateScaleSet;// (fInvIntegTopLimitTemp * fLoopLimitDerateScale);//
    strLoadVoltARealRegu.integDownLimit = -fLoopLimitDerateScale;//-fLoopLimitDerateScaleSet;// -(fInvIntegTopLimitTemp * fLoopLimitDerateScale);//

    strLoadCurrARealRegu.integTopLimit  = fLoopLimitDerateScale;//fLoopLimitDerateScaleSet;// (fInvIntegTopLimitTemp * fLoopLimitDerateScale);//
    strLoadCurrARealRegu.integDownLimit = -fLoopLimitDerateScale;//-fLoopLimitDerateScaleSet;// -(fInvIntegTopLimitTemp * fLoopLimitDerateScale);//
}
