...
#define LlcTemp_FanSMaxUp       660//720//90
#define LlcTemp_FanSMaxBack     620//720//90
// // LLC变压器温度控风扇
...
#define InnerTemp_FanSMaxUp     750//720//90
#define InnerTemp_FanSMaxBack   710//720//90
// 负载控制风扇转速
// 电池功率控风扇
...
void Fan_FanControl(void)
{
    static Uint16 uiFanSpeedMaxLimit = 55;
    static Uint16 uiFanSpeedCtrlCnt = 0;
    Uint16 uiFanSpeedCtrlTarget = 0;
    ...
    {
        uiFanSpeedCtrlTarget = g_FanVar.fPowerforFan * 0.01f;
    }
    else
    {
        uiFanSpeedCtrlTarget = g_FanVar.fTempforFan * 0.01f;
    }

    if(uiFanSpeedCtrlTarget < 30)  { uiFanSpeedCtrlTarget = 0; }
    // 20250629--关机转动风机降低BUS电压，减少关机时间
    if(uiSystermOffDecrateBusFlag) { uiFanSpeedCtrlTarget = 30; }

    //------ 静音模式/性能模式，区分风机最高转速 ------//
    if(TRUE == uiSciSetDataBag[eSetSilentModeEnable])
    {
        uiFanSpeedMaxLimit = 55;  // 转速上限为55%
    }
    else
    {
        if(   (strSlvAnalog.fLLCTemp > LlcTemp_FanSMaxUp)
           || (strSlvAnalog.fInnelTemp > InnerTemp_FanSMaxUp)
           || (g_FanVar.bit.FanSpeedUpByPvCurr) )
        {
            uiFanSpeedMaxLimit = 75;  // 达到调节转速的温度，转速上限为75%
        }
        else if(   (strSlvAnalog.fLLCTemp < LlcTemp_FanSMaxBack)
                && (strSlvAnalog.fInnelTemp < InnerTemp_FanSMaxBack)
                && (FALSE == g_FanVar.bit.FanSpeedUpByPvCurr) )
        {
            uiFanSpeedMaxLimit = 55;  // 未达调节转速的温度，转速上限为55%
        }
    }
    Uplimit(uiFanSpeedCtrlTarget, uiFanSpeedMaxLimit);

    //ate 测试风扇未接
    if((uiSciCommandDataBag[eComATECommand] & eAteFanCtrlEnBit))
    {
        g_FanVar.bit.FanEnableFlag = TRUE;
        if(uiFanSpeedCtrlTarget < 50)
        {
            g_FanVar.uiFanDutySet = 50;
        }
        else
        {
            g_FanVar.uiFanDutySet = uiFanSpeedCtrlTarget;
        }
        if(++uiAteFanTestTimeCnt > cDelay20s_20ms)//强制控制20s
        {
            uiSciCommandDataBag[eComATECommand] &= (~eAteFanCtrlEnBit & 0xFFFF);
        }
        uiFanSpeedCtrlCnt = 0;
    }
    else
    {
        uiAteFanTestTimeCnt = 0;
        //------ 延时使能 ------//
        if(uiFanSpeedCtrlTarget >= 30)//不管哪个达到起始转速就延时使能
        {
        ...
        //------ 延时使能 ------//
        // 从DSP调速较快，由主DSP控制转速目标值
        if(++uiFanSpeedCtrlCnt >= SecondBy20msBase(2))
        {
            if(g_FanVar.uiFanDutySet < (uiFanSpeedCtrlTarget - 2))//缓启
            {
                g_FanVar.uiFanDutySet ++;
            }
            else if(g_FanVar.uiFanDutySet > (uiFanSpeedCtrlTarget + 2))
            {
                g_FanVar.uiFanDutySet --;
            }
            else
    ...
    //
    Uplimit(g_FanVar.uiFanDutySet, 100);  // 转速小于10% 风扇转动不了
}
