...
static void Bus_SysBusVoltRefCalu(void)
{
    ...
    if((eNoBatt != g_ComInfo.Com1.bit.BatMode) && (FALSE == g_SysAlarm.ubAlarmDc.bit.BatOpen))
    {
        ...
        else
        {
            ...
            if(eBat_ChgAbnormal == g_uiBatAbnormalFlg && g_SystemInfo.SysFlag.bit.ChgBurstMode) // 电池异常保护的扰动退出机制，给定+5V，观察电流是否有变化
			{
				VBusRefSetByBat = VBusRefSetByBatPre + 5.0;
			}
			else
			{
    ...
    else
    {
        ...
        if(eBat_Normal != g_uiBatAbnormalFlg)
        {
            ...
            {
                if(g_SystemInfo.SysFlag.bit.ChgBurstMode)  //充电异常，BUS给定做间接变化，观察电池是否恢复。
                {
                    g_SystemInfo.fPvBusVoltSet = g_DcDcVar.fBusVoltCtrlRef + Set_BusVolt(5);
                }
                else
                {
