...
volatile Uint16 uiCanSyncDataIDBegin;					    // 同步信号起始地址
...
void MonitorModbus_CrcCheck(void)
{
	...
	{
		uiCrcCheckTemp = Sci_CrcCaculate( &uiSciRxDataBuff[0],uiReceiveDataLengthCnt - 2);
...
//			}
			...
			if(uiSciComID != eCom_Invalid)
			{
                ...
                if(     //02   直接基于起始地址去看个数
                        ((uiSciComID == eComUploadRealStatus) &&(Addr2 > (DSPFAULT_NUM>> 4)))
                     ...
                     || (  (uiSciComID == eComUploadSetData)
                         &&(  ( Addr1 < AnalogDataIDBegin ) || ( Addr3 > AnalogDataIDEnd && Addr3 < SetDataNormalIDBegin )//03 real
                           || ( Addr1 > AnalogDataIDEnd && Addr1 < SetDataNormalIDBegin )        || ( Addr3 > SetDataNormalIDEnd && Addr3 < CommandDataIDBegin)   //03 infor
                           || ( Addr1 > SetDataNormalIDEnd && Addr1 < CommandDataIDBegin )       || ( Addr3 > CommandDataIDEnd && Addr3 < CommandData_BMSIDBegin ) //03 set
                           || ( Addr1 > CommandDataIDEnd && Addr1 < CommandData_BMSIDBegin )     || ( Addr3 > CommandData_BMSIDEnd && Addr3 < AnalogInforDataIDBegin ) //03 set
                     ...
                     || (  (uiSciComID == eComWriteSetSingleData)
                         &&(  ( Addr1 > SetDataNormalIDEnd && Addr1 < CommandDataIDBegin )       || ( Addr1 > CommandDataIDEnd && Addr1 < CommandData_BMSIDBegin )
                           || ( Addr1 > CommandData_BMSIDEnd && Addr1 < AnalogInforDataIDBegin ) || ( Addr1 > AnalogInforDataIDEnd)
                           )  )
...
//                         &&(uiFrameParallelEnable) && (Slaver == uiSelfMstOrSlv) )  // 并机情况下从机不允许设置部分设置量
    ...
    switch(uiSciComID)
    {
        ...
        case eComWriteSetSingleData:
        {
            if(uiSciBufferDataID < CommandDataIDBegin)
            {
                ...
                GetDataRange((uiSciBufferDataID-SetDataNormalIDBegin), &stValRange);
				if(stValRange.uiMinVal > 32767)
                {
                    if(   (int16)uireceivedatatemp < (int16)stValRange.uiMinVal  //(int16)uiSetDataRange[uiSciBufferDataID - SetDataNormalIDBegin][eMinValue]
                       || (int16)uireceivedatatemp > (int16)stValRange.uiMaxVal)  //(int16)uiSetDataRange[uiSciBufferDataID - SetDataNormalIDBegin][eMaxValue] )
                    { //超出设定范围则用默认值
                        uiBuffLength = 1;
                    ...
                    {
                        if(((uiFrameParallelEnable) && (Master == uiSelfMstOrSlv) && (OutSeq_PhaseA == uiFrameParaPhaseSeq)) || (FALSE == uiFrameParallelEnable))   // 并机且为从机不接收
                        {
                            uiSciSetDataBag[uiSciBufferDataID - SetDataNormalIDBegin] = uireceivedatatemp;
                        }
                        else
                        {
                            // 从机或者L2L3主机都不接收部分数据 主机同步过来的数据
                            if(   (uiSciBufferDataID - SetDataNormalIDBegin < eSetFromMasterBeginID)
                               || (uiSciBufferDataID - SetDataNormalIDBegin > eSetFromMasterEndID) )
                            {
                                uiSciSetDataBag[uiSciBufferDataID - SetDataNormalIDBegin] = uireceivedatatemp;
                            }
                        }
                        uiCanSyncDataIDBegin = uiSciBufferDataID - SetDataNormalIDBegin;
                        g_ParaVar.ParaFlag.bit.SyncDataRenew = TRUE;
                    }
                }
                else	//此值当无符号数
                {
                    if(   uireceivedatatemp < stValRange.uiMinVal  //uiSetDataRange[uiSciBufferDataID - SetDataNormalIDBegin][eMinValue]
                       || uireceivedatatemp > stValRange.uiMaxVal)  //uiSetDataRange[uiSciBufferDataID - SetDataNormalIDBegin][eMaxValue] )
                    { //超出设定范围则用默认值
                        uiBuffLength = 1;
                        uiErrorCode = MbDataError;
                        MonitorModbus_RenewTxBuffer(eCommandInvalidID,0,3);
                ...
                // sSysPara[eSetDataID0] = uiSciSetDataBag[eSciFuncSelectSetID];
			}
            ...
            else if(uiSciBufferDataID < (CommandData_BMSIDBegin + eCommand_BMSIDEnd)&&(uiSciBufferDataID >= CommandData_BMSIDBegin))
            {
				if(((uiFrameParallelEnable) && (Master == uiSelfMstOrSlv) && (OutSeq_PhaseA == uiFrameParaPhaseSeq)) || (FALSE == uiFrameParallelEnable))   // 并机且为从机不接收
				{
					...
					uiCanSyncDataIDBegin = uiSciBufferDataID - CommandData_BMSIDBegin + eSetFromMasterEndID + 1;
					g_ParaVar.ParaFlag.bit.SyncDataRenew = TRUE;
				}
            }
		...
		case eComWriteSetMultipleData:
		{
			...
			Uint16 uiSciBufferDataIDTemp = uiSciBufferDataID;

			if(   uiSciBufferDataID < CommandDataIDBegin
			   && (uiSciBufferDataID + uiSciBufferDataLength) <= SetDataNormalIDEnd )
			{
                for(usLoop = uiSciBufferDataIDTemp; usLoop < (uiSciBufferDataID + uiSciBufferDataLength); usLoop++)
                {
                    //uiSciSetDataBag[usLoop] = (uiSciRxDataBuff[pSciBuffAddr]<<8) + uiSciRxDataBuff[pSciBuffAddr+1];
                    uireceivedatatemp = (uiSciRxDataBuff[pSciBuffAddr]<<8) + uiSciRxDataBuff[pSciBuffAddr+1];
                    //uireceivedatatemp = (uiSciRxDataBuff[4]<<8) + uiSciRxDataBuff[5];

                    // 不作错误码返回，监控无法立即显示设置失败；设置好后反而导致连续下发的设置量全部无法设置。-- 20250825
                    // if(   ((uiSciBufferDataIDTemp - SetDataNormalIDBegin) == eSetGridStandardCode && eInvMainState_Idle != g_InvVar.InvFlag.bit.bInvMainSts) // 电网法规只能在逆变待机下修改，否则设置错误
                    //    || ((uiSciBufferDataIDTemp - SetDataNormalIDBegin) == eSetCTRatio && eTurnOn == g_ComInfo.Com1.bit.SysTurnOn) )
                    // {
                    //     uiBuffLength = 1;
                    //     uiErrorCode = MbDataError;
                    //     MonitorModbus_RenewTxBuffer(eCommandInvalidID,0,3);
                    ...
                    //if(uiSetDataRange[uiSciBufferDataIDTemp - SetDataNormalIDBegin][eMinValue] > 32767)   //此值当有符号数
                    GetDataRange((uiSciBufferDataIDTemp-SetDataNormalIDBegin), &stValRange);
					if(stValRange.uiMinVal > 32767)
                    {
                        if(   (int16)uireceivedatatemp < (int16)stValRange.uiMinVal  //uiSetDataRange[uiSciBufferDataIDTemp - SetDataNormalIDBegin][eMinValue]
                           || (int16)uireceivedatatemp > (int16)stValRange.uiMaxVal  //uiSetDataRange[uiSciBufferDataIDTemp - SetDataNormalIDBegin][eMaxValue]
                          ) //超出设定范围则用默认值
                        {
                            uiBuffLength = 1;
                            uiErrorCode = MbDataError;
                    ...
                    {
                        if(   uireceivedatatemp < stValRange.uiMinVal  //uiSetDataRange[uiSciBufferDataIDTemp - SetDataNormalIDBegin][eMinValue]
                           || uireceivedatatemp > stValRange.uiMaxVal  //uiSetDataRange[uiSciBufferDataIDTemp - SetDataNormalIDBegin][eMaxValue]
                          ) //超出设定范围则用默认值
                        {
                            uiBuffLength = 1;
                            uiErrorCode = MbDataError;
                ...
                for(usLoop = uiSciBufferDataID; usLoop < (uiSciBufferDataID + uiSciBufferDataLength); usLoop++)
                {
                    ...
                    {
                        // 从机或者L2L3主机都不接收部分数据 主机同步过来的数据
                        if(   (usLoop - SetDataNormalIDBegin < eSetFromMasterBeginID)
                           || (usLoop - SetDataNormalIDBegin > eSetFromMasterEndID) )
                        {
                            uiSciSetDataBag[usLoop - SetDataNormalIDBegin] = (uiSciRxDataBuff[pSciBuffAddr]<<8) + uiSciRxDataBuff[pSciBuffAddr+1];
                        }
                    }
            ...
            else if(((uiSciBufferDataID + uiSciBufferDataLength) <= (CommandData_BMSIDBegin + (Uint16)eCommand_BMSIDEnd) )&&(uiSciBufferDataID >= CommandData_BMSIDBegin))
            {
				...
				// MCU每13s才发一次BMS数据  而且一次性发26个数据 用10功能码
				if((uiFrameParallelEnable) && (Master == uiSelfMstOrSlv) && (OutSeq_PhaseA == uiFrameParaPhaseSeq))
				{
					if(FALSE == g_ParaVar.ParaFlag.bit.SyncDataRenew) // 等uiSciSetDataBag数据同步完成
					{
						uiCanSyncDataIDBegin = eSetFromMasterEndID + 1;
						g_ParaVar.ParaFlag.bit.SyncDataRenew = TRUE;
					}
				}
            }
    ...
    if(unFlagSci.bit.bToCmdDealFinish == FALSE)
    {
        switch(uiUpLoadDataType)
        {
            case eSentAnalogDataID:
            {
                ...
                uiUpLoadDataID = uiUpLoadDataFirstAddr - AnalogDataIDBegin;
                break;
            }
            ...
            case eSentSetDataID:
            {
                ...
                else if(uiUpLoadDataFirstAddr >= AnalogInforDataIDBegin)//上传infor 的模拟量   0xF800
                {
                    ...
                    //TODO 通讯握手完成判定 -- add in 20250927
                    if(FALSE == g_SystemInfo.SysFlag.bit.HandshakeFinish)
                    {
                        // if(uiUpLoadDataID >= AnalogInforDataIDBegin)
                        // {
                            g_SystemInfo.SysFlag.bit.HandshakeFinish = TRUE;
                        // }
                    }
                }
