...
void PowerDerating_BatTmpDeratedCalc(void)
{
    ...
    // 1、根据LLC-HS温度降电流
    //-------------------------------
    fBatDerateCurrRate = (float32)(cBatDischgCurrMax - LLCHSTempBatCurrStop) / (float32)(LLCHSTempDerateStop - LLCHSTempDerateStart);  // (190-130)/(100-95)
    ...
    {
        strBatPowerDerate.fLLCTmpOrder = cBatDischgCurrMax - (strSlvAnalog.fLLCTemp - LLCHSTempDerateStart) * fBatDerateCurrRate;   // 12A
    }
    else  // 这个要区分充电 还是放电
    {
        strBatPowerDerate.fLLCTmpOrder = cBatDischgCurrMax;
    }

    //-------------------------------
    // 2、根据内部温度降Bat电流    25A
    //-------------------------------
    fBatDerateCurrRate = (float32)(cBatDischgCurrMax - InnerTempBatCurrStop) / (float32)(InnerTempDerateStop - InnerTempDerateStart);  // (135-110)/(87-82) = 8.4
    if(fBatDerateCurrRate <= 0)
    {
        fBatDerateCurrRate = 0;
    }
    ...
    {
        strBatPowerDerate.fInnerTmpOrder = cBatDischgCurrMax - (strSlvAnalog.fInnelTemp - InnerTempDerateStart) * fBatDerateCurrRate;   // 25A
    }
    else  // 这个要区分充电 还是放电
    {
        strBatPowerDerate.fInnerTmpOrder = cBatDischgCurrMax;
    }
...
//            strPvPowerDerate.fTmpSumOrder = 1.0834; // 单路不限定时，PV最大功率为6.5KW
        ...
        {
            strPvPowerDerate.fTmpSumOrder = strPvPowerDerate.fTmpSumOrder * DeratePowerRatio;
        }
    }
...
void PowerDerating_OffGridInvVoltDerCalc(void)
{
    ...
    if(eInv_OffGridMode == g_InvVar.InvFlag.bit.InvWorkMode)
    {
        ...
        {
            // strDeratedPercent.fInvVoltlimitOrder = (strDeratedPercent.fInvVoltlimitOrder * 0.875f + fVoltDerateOrderTemp1 * 0.125f); // 100mS 调整一次，从230V调整到180V，需要4.1S，硬件反馈还是波动很大
            ...
            {
                // if(strDeratedPercent.fInvVoltlimitOrder > fVoltDerateOrderTemp1) { strDeratedPercent.fInvVoltlimitOrder -= cOutVoltDelta; }
                // else                                                             { strDeratedPercent.fInvVoltlimitOrder += cOutVoltDelta; }
        ...
        // fVoltDerateOrderTemp1 = fVoltDerateValue;
...
//         Uplimit(fVoltDerateOrderTemp1, (1 + strDeratedPercent.fBattVoltDerCtrlLoopOut));
        ...
        {
        	// strDeratedPercent.fBattCurrDerCtrlLoopRef = Set_BatCurr(cBatDischgCurrMax);
            ...
            {
                strDeratedPercent.fBattCurrDerCtrlLoopRef = Set_BatCurr(((float)strBMSLogicInfo.iBMS1MaxDisChgCurr * 0.155f));
            }
            else // 其它情况下，用设置的电池最大放电电流进行保护
            {
                strDeratedPercent.fBattCurrDerCtrlLoopRef = Set_BatCurr(((float)uiSciSetDataBag[eSetBat1MaxDisChargeCurrValue] * 0.155f));
            }
            // Dnlimit(strDeratedPercent.fBattCurrDerCtrlLoopRef, Set_BatCurr(10)); // 10A
            UpDownLimit(strDeratedPercent.fBattCurrDerCtrlLoopRef, Set_BatCurr(cBatDischgCurrMax), Set_BatCurr(10)); // [135A, 10A]
	        ...
	        strDeratedPercent.fBattCurrDerCtrlLoopIng += strDeratedPercent.fBattCurrDerCtrlLoopErr * 0.04f;
