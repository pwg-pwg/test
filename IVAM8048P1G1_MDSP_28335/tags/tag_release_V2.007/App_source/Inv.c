...
float32 fInvVoltFeedForwardKp = 0.96f;
...
void Inv_OffGridRealVoltLoop(void)
{
    ...
    if(fabs(stADC.fRealScale.VInvA) >= g_InvVar.fVInvOutUpLimit)
    { strLoadVoltARmsRegu.integrator *= 0.6f; strLoadVoltARmsRegu.Out *= 0.6f; }
...
void Inv_ParaDroopCtrl(void)
{
    ...
    if(uiFrameParallelEnable && (eInvMainState_OffGrid == g_InvVar.InvFlag.bit.bInvMainSts))
    {
        ...
        //====================================================================
		if(bMachineModel == eModel_IVAM6048)
    	{
    ...
    if(eNoBatt != g_ComInfo.Com1.bit.BatMode)
    {
            ...
            )
        {
            ...
            if(   (eBat_Normal == g_uiBatAbnormalFlg)
               && (eDcDcMainState_Running == g_DcDcVar.DcDcFlag.bit.bMainSts)
               && (!strBMSLogicInfo.uiBMS1BatStopChargeFlag)
               )
            {
                //4、对电池充电的话,由于电池损耗,电池无法达到要求的充电电流,需要将电池实时有效值电流作为反馈叠加上
                g_SystemInfo.PowerBalance.PvMaxPower += (g_DcDcVar.fBatMaxChrCurrSet * stValue.fAveScale.VBat * 1.04f);
...
//                    strPVBatChgPowerLimitLoop.integTopLimit = Set_Power(60);//0.05f;// 300W
//                    strPVBatChgPowerLimitLoop.integDownLimit = (-Set_Power(300));//-0.05f;// -300W

					if(eInv_OffGridMode == g_InvVar.InvFlag.bit.InvWorkMode)
                    {
                        strPVBatChgPowerLimitLoop.integTopLimit = Set_Power(300);//0.05f;// 300W
                    	strPVBatChgPowerLimitLoop.integDownLimit = (-Set_Power(400));//-0.05f;// -300W
                    }
					else
					{
					     strPVBatChgPowerLimitLoop.integTopLimit = Set_Power(300);//0.05f;// 300W
                    	strPVBatChgPowerLimitLoop.integDownLimit = 0;//-0.05f;// -300W
					}
                }
