...
enum
{
    ...
    eIntSciSending,         // 1
    eIntSciSendFinish,      // 2
    eIntSciWaitReceive,     // 3
    eIntSciReceiving,       // 4
    eIntSciReceiveFinish,   // 5
    eIntSciCmdIdentify,     // 6
    eIntSciCmdDeal,         // 7
...
//    eIntSci_Idle=0,                // 空闲
//    eIntSci_SetCmd,                // 发送指令
//    eIntSci_WaitSend,              // 等待发送完成
//    eIntSci_WaitRecv,              // 等待接收
//    eIntSci_Modbus_ParseData,      // 解析数据
//    eIntSci_Modbus_ParseSuccess,   // 解析成功
//    eIntSci_RecvError,             // 接收错误(超时或者数据解析错误)
//    eIntSci_End
//
//};
...
enum
{
    ...
    eIntComUploadRealData,            //2 上传实时模拟量
    eIntComWriteSetSingleData,        //4 下达单个设置量  查询单个设置量
    eIntComWriteSetMultipleData,      //5 下达多个设置量
    eIntCom_Invalid,                   //6 指令无效
    eIntCom_SlaveAddressMismatch,      //7 从机地址不匹配
    eIntCom_End
};
...
enum
{
    ...
    eIntSciSentSetDataID,           //请求设置量   这个没用@todo Ivan 240710
...
//     eM2SRealData_State1,             //  04 主DSP状态1
//     eM2SRealData_State2,             //  05 主DSP状态2
//     eM2SRealData_State3,             //  06 主DSP状态3
//     eM2SRealData_State4,             //  07 主DSP状态4
//     eM2SRealData_BatVolt,            //  08
//     eM2SRealData_BatCurr,            //  09
//     eM2SRealData_BatPower,           //  10
//     eM2SRealData_BatPower_Ref,       //  11
//     eM2SRealData_Bat1VoltUplimit,    //  12 电池恒充电压点
//     eM2SRealData_LlcOnOff,           //  13 LLC开关状态
...
//     eM2SRealData_SetIsoResLimit,     //  20 Iso检测阈值设置 unit:kΩ
//     eM2SRealData_SetId,              //  21  发送设置量地址 和 设置量数量  最多4个
//     eM2SRealData_SetData1,           //  22  第一个数据
//     eM2SRealData_SetData2,           //  23  第二个数据
//     eM2SRealData_AFCIEn,   			 //  24  AFCI使能位
//     // eM2SRealData25,                  //  25  第四个数据
//     eM2SRealData_MPPTMultiPointEn,   //  25  MPPT阴影扫描/MPPT多峰扫描使能位
//     eM2SRealDataEnd                  // 26
// };
...
enum
{
    ...
    eM2SRealData_SetData1,           //  22 第一个数据
    eM2SRealData_SetData2,           //  23 第二个数据
    eM2SRealData_FctionEn,   		 //  24 功能设置：bit0 = AFCI通讯使能位；bit1=AFCI自检使能；bit2=Afci清除电弧故障；bit3=MPPT阴影扫描/MPPT多峰扫描使能位；bit4 = 清除故障
    eM2SRealData_Model,              //  25 机型
    eEM2SRealData_PeriodSample,      //  26 采样均值计算周期随真实Inv频率变化
    eM2SRealDataEnd                  //
};
...
enum//接收
{
    ...
    eS2MRealData_LLCTXTemp,    //  05 LLC1Temp  LLC变压器温度
    eS2MRealData_LeadAcidTemp, //  06 LeadAcidTemp  外部电池温度
    eS2MRealData_PvTemp,       //  07 LLC3Temp  Pv电感温度
    eS2MRealData_LLCTemp,      //  08 LLC4Temp  LLC散热器温度
    eS2MRealData_InvTemp,      //  09 HS1Temp  Inv电感温度
...
typedef union
{
    ...
    struct
    {
        ...
        Uint16      bIntSciRxOverTime       : 1;        // 接收超时
        Uint16      bIntSciSetDataRenew     : 1;        // 设置量更新
...
typedef union
{
    ...
    struct
    {
        Uint16  bInvStateMachine        :3;     // 逆变工作模式
        ...
        Uint16  PvIsoCheckFlag          :1;     // Pv Iso检测开始标志	未使用
...
typedef union
{
    ...
    struct
    {
        ...
        Uint16  bFanLockCheckByFG       :1;     // 0=风机反馈信号是RD，只有高低电平，低电平为堵转故障；1=风机反馈信号是FG，可以通过反算真实转速。
...
typedef union
{
    ...
    struct
    {
        ...
        Uint16  Fan1Err            :1;     // 08  风机1堵转
        Uint16  Fan2Err            :1;     // 09  风机2堵转
...
typedef union
{
    ...
    struct
    {
        Uint16  PBACTempOver 	:1;     // 00
        Uint16  rsvd01 		 	:1;     // 01
        Uint16  BusVoltOver  	:1;     // 02  Bus过压 与主DSP重复的
        Uint16  PvTempOver   	:1;     // 03  PV过温保护

        Uint16  Pv1Reverse   	:1;     // 04  PV1反接
        Uint16  Pv2Reverse   	:1;     // 05  PV2反接
...
typedef struct
{
    float32 fVoltPv1;       //  01 Pv1电压  unit :1.0
    float32 fCurrPv1;       //  02 Pv1电流  unit :1.0
    float32 fVoltPv2;       //  03 Pv2电压  unit :1.0
    float32 fCurrPv2;       //  04 Pv2电流  unit :1.0
    float32 fLLCTXTemp;     //  05 fLLCTXTemp unit :1.0
    float32 fBatTemp;       //  06 fBatTemp unit :1.0
    float32 fPvTemp;        //  07 fPvTemp unit :1.0
    float32 fLLCTemp;       //  08 fLLCTemp unit :10
    float32 fInvTemp;       //  09 fInvTemp   unit :1.0
    float32 fHS2Temp;       //  10 HS2Temp   unit :1.0
    float32 fInnelTemp;     //  11 InnelTemp unit :10
    float32 fPvIsoVolt;     //  12 PvIso电压
    float32 fPosBusVolt;    //  13 从DSP正母线电压
    float32 fNegBusVolt;    //  14 从DSP负母线电压
    float32 fGfciCurr;      //  15 Gfci电流
    float32 fCtACurr;       //  16 CtACurr电流
    float32 fCtBCurr;       //  17 CtBCurr电流
    float32 fCtCCurr;       //  18 CtCCurr电流
    float32 fIsoChkRes;     //  19 绝缘阻抗
...
// Public Variables
//-----------------------------------------------------------------------------
PvInvComm_PUBLIC Uint16 uiIntSciTxDataBuff[SEND_DATA_LENGTH];
...
PvInvComm_PUBLIC Uint16 uiMachineInfo[eS2MMachineInfoEnd];  // 从DSP发送给主DSP的机型信息

PvInvComm_PUBLIC UnionFlagIntSci  unIntSciFlag;  // 定义变量取出280025发来的数据
PvInvComm_PUBLIC MasterStateStr   strMstState;   // 主机的状态标志
PvInvComm_PUBLIC MasterFuctionStr strMstFuction; // 主机的功能标志
PvInvComm_PUBLIC SlaveStateStr    strSlvState;   // 接收到从DSP的状态标志
PvInvComm_PUBLIC SlaveAnalogStr   strSlvAnalog;  // 接收到从DSP的数据
