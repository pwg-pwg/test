...
float32 fInvCurrFeedForwardKp = 0.2f;
float32 fInputVoltFeedForwardKp = 0.8f;
float32 fInvVoltFeedForwardKp = 0.97f;
...
static void Inv_CtrlBatChgPowerLimit(void);
static void Inv_CtrlBatDisChgPowerLimit(void);
...
void Inv_ParaInvCurrAvgLoop(void);
/******************************************************************************
...
void Inv_Initialize(void)
{
    ...
    // ------------------- 电池充电电流相关 ------------------- //
    strPVBatChgPowerLimitLoop.Kp = 0.8;//1.2;  // 展明原来系数是0.02;建豪改AcCouple改为1.2
    ...
    strLoadVoltARmsRegu.integTopLimit = 0.3;//0.25;  // 额定值的+-30%
    strLoadVoltARmsRegu.integDownLimit = -0.3;//-0.25;
    ...
    strLoadVoltARealRegu.Kp = 2.6;//3.0;// 4.5;//
    ...
    strLoadVoltARealRegu.integTopLimit = 1.6f;	//20250604 采用削顶降压的方式，避免DC侧电流过大，触发硬件保护，硬件逆变过流点为60A，软件限流点作41A。
    strLoadVoltARealRegu.integDownLimit = -1.6f;//20250605 根据电池电压调整软件限流点，低于47V时采用41A，高于48V时采用46A。
    ...
    // 离网电流瞬时值环
    strLoadCurrARealRegu.Kp = 0.26;//0.1;//0.05;
    strLoadCurrARealRegu.Ki = 0;
    strLoadCurrARealRegu.integrator = 0;
    strLoadCurrARealRegu.integTopLimit = 1.6f;	// 电流环输出得到调制波 即电压值 1.6*120V = 192V
    strLoadCurrARealRegu.integDownLimit = -1.6f;
    ...
    fInvCurrFeedForwardKp = 0.2f;// 负载电流前馈
    fInputVoltFeedForwardKp = 0.8f;
    fInvVoltFeedForwardKp = 0.97f;
    // 离网有效值给定
...
*/
    ...
    g_InvVar.fVInvOutUpLimit = cVInvOutUpLimit * cSqrtTwo * g_GridManager.Rated.f32VOutInvt;
    // g_InvVar.fVInvOutDnLimit = cVInvOutDnLimit * cSqrtTwo * g_GridManager.Rated.f32VOutInvt;
    g_InvVar.fVInvRefUpLimit = cVInvRefUpLimit * cSqrtTwo * g_GridManager.Rated.f32VOutInvt;
    g_InvVar.fVInvRefDnLimit = cVInvRefDnLimit * cSqrtTwo * g_GridManager.Rated.f32VOutInvt;
...
*/
    ...
    // 并机电流均流环 Pi 参数
    g_ParaVar.fDroopInvCurrAveKp = 0.02;
    g_ParaVar.fDroopInvCurrAveKi = 0.001;
    // 并机无功均流环 A
    g_ParaVar.fDroopInvCurraAveInteg = 0;
    g_ParaVar.fDroopInvCurraAveOut   = 0;
    // 并机无功均流环 B
    // g_ParaVar.fDroopInvCurraAveInteg = 0;
    // g_ParaVar.fDroopInvCurraAveOut   = 0;
    // 并机无功均流环 C
    ...
    // g_ParaVar.fDroopQcAveOut   = 0;
    // 并机无功均流环输出限制  输出结果是电压
    g_ParaVar.fDroopInvCurrIntegUpLimit = 0.0454545;  // 10%   110V定标的话就是10V
    // g_ParaVar.fDroopInvCurrIntegDnLimit = -0.0454545;

    // 电池电压转换放电电流峰值
    fLoopLimitDerateScale = 1.0;
    fLoopLimitDerateScaleSet = (cIInv_Offgrid_Hardware_Limit * g_GridManager.Rated.f32IOutInvt);

    strDeratedPercent.fInvVoltlimitOrder = 1.0f;
    // uiSciSetDataBag[eSetRsvd346] = 2600;
    // uiSciSetDataBag[eSetRsvd347] = 5000;

    // uiSciSetDataBag[eSetRsvd362] = 20;
    // uiSciSetDataBag[eSetRsvd363] = 10;
...
void Inv_Task_20ms(void)
{
    ...
    Inv_CtrlBatChgPowerLimit();//电池充电功率限制,作用在逆变环路上
    Inv_CtrlBatDisChgPowerLimit();//电池放电功率限制,作用在逆变环路上
...
void Inv_SubStateMachine(void)
{
    ...
    static Uint16 s_uwSInvOfflineOpDelayCnt = 0;
    static Uint16 s_uwInvRlyInterlaceDelayCnt = 0;
    // ==================== 故障检测和系统状态判断 ====================
    ...
    if(    (   (FALSE == g_InvVar.InvLogicJump.bit.bInvFaultFlag)
        ...
        || (unRxSysStaFlag2.bit.bUpmBatCutOff && uiFrameParallelEnable)  // 并机时，电池低压关AC，保证接收动作时间一致
        || (g_ParaVar.ParaFlag.bit.MdlFastOff)  						 // 并机下其他模块不允许开机
        || (g_ComInfo.Com1.bit.BatAbnTurnToStandby)                      // 电池异常，无其它源的情况下转待机
        || (g_SysAlarm.ubAlarmSys.bit.PvWeak)                            // PV功率不足，关闭逆变输出
        )
    {
    ...
    // ==================== 空闲状态 ====================
        case eInvMainState_Idle:
        {
            ...
            if(   (eTurnOn == g_ComInfo.Com1.bit.InvOnOff)             // 系统状态机下发开机命令
               ...
               && (FALSE == g_ParaVar.ParaFlag.bit.MdlFastOff)         // 母线电压大于350V
               && (FALSE == g_ComInfo.Com1.bit.BatAbnTurnToStandby)    // 非电池异常恢复时无外部源
               )
            {
        ...
        case eInvMainState_SoftStart:
        {
            ...
            if(   (eGridOnline == g_SystemInfo.Source.bit.GridSource)
               || (eGenOnline == g_SystemInfo.Source.bit.GenSource) ) // 1.0 无输出转旁路 // 电池低压保护有PV，上市电会强制解除电池低压保护，然后电池+PV先输出，而非旁路输出。
            {
                if(eGridOnline == g_SystemInfo.Source.bit.GridSource)
                {
            ...
            if(   (eRelayStateClose <= g_RelayVar.CtrlSta.bit.GridRlySta)
               && (    (TRUE == g_SysFault.Flag.bit.GridFault)
            ...
            if(FALSE == g_SystemInfo.SysFlag.bit.InvWorkStart)
            {
                ...
                {
                    if(FALSE == uiFrameParallelEnable ||(++s_uwInvRlyInterlaceDelayCnt > (40 * uiSelfIDAdrr)))
                    {
					    s_uwInvRlyInterlaceDelayCnt = 0;
                        ...
                        && (   (   eRelayStateClose == g_RelayVar.CtrlSta.bit.GridRlySta 
                                && stValue.fRmsReal.VGridA > 140)
                            || (   eRelayStateClose == g_RelayVar.CtrlSta.bit.GenRlySta
                                && stValue.fRmsReal.VGenA > 140)
            ...
            //     g_InvVar.InvFlag.bit.InvNormalLogicStart = FALSE; 
            // }
            g_InvVar.InvFlag.bit.DrivePowerSetupFinish = TRUE; //跳过驱动建立，直接跑逻辑
            g_InvVar.InvFlag.bit.InvNormalLogicStart = TRUE;
            // 建立了驱动电压之后再跑正常的判断逻辑
            if(g_InvVar.InvFlag.bit.InvNormalLogicStart)
            {
                ...
                if(eInv_NoOutputMode == g_InvVar.InvFlag.bit.InvWorkMode)
                {
                    ...
                    if(s_uwSInvOfflineOpDelayCnt > 0)
                    {
                        s_uwSInvOfflineOpDelayCnt--;
                        break;
                    }
...
//                        g_RelayVar.CtrlSta.bit.GenNRlyOn = FALSE;
                ...
                else if(eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode)
                {
                    ...
                    if(FALSE == g_SystemInfo.SysFlag.bit.AcSoftStart)  // 直流起机 母线有电压 逆变电压是从0开始增加到网侧电压
                    {
                        ...
                        {
                            if(++g_InvVar.uwOnGridCloseInvRlyCnt > (cDelay2s_1ms + 40 * uiSelfIDAdrr))   //2s_h
                            {
                    ...
                    {
                        if(    (eRelayStateClose == g_RelayVar.CtrlSta.bit.GridRlySta)
                            && (stValue.fAveReal.VBus >= (stValue.fRmsReal.VOutA * cSqrtTwo - 60.0f))  // 母线电压在AC峰值电压的60V以内
                           )
                        {
                            if(++g_InvVar.uwOnGridCloseInvRlyCnt > (cDelay100ms_1ms+40 * uiSelfIDAdrr))   //100ms_h
                            {
                                g_RelayVar.RlyReady.bit.InvRlyReadyOn = TRUE; // 并机加同步动作
                                if(g_RelayVar.CtrlSta.bit.RlyOnReadyFinish)
                                {
                ...
                else if(eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode)
                {
                    ...
                    if(FALSE == g_SystemInfo.SysFlag.bit.AcSoftStart)  // 直流起机 母线有电压 逆变电压是从0开始增加到网侧电压
                    {
                        ...
                        {
                            if(++g_InvVar.uwOnGenCloseInvRlyCnt > (cDelay2s_1ms + 40 * uiSelfIDAdrr))   //2s_h
                            {
                                g_RelayVar.RlyReady.bit.InvRlyReadyOn = TRUE;
                                if(g_RelayVar.CtrlSta.bit.RlyOnReadyFinish)
                                {
                    ...
                    {
                        if(    (eRelayStateClose == g_RelayVar.CtrlSta.bit.GenRlySta)
                            && (stValue.fAveReal.VBus >= (stValue.fRmsReal.VOutA * cSqrtTwo - 60.0f))  // 母线电压在AC电压的40V以上
                           )
                        {
                            if(++g_InvVar.uwOnGenCloseInvRlyCnt > (cDelay100ms_1ms + 40 * uiSelfIDAdrr))   //100ms_h
                            {
                                g_RelayVar.RlyReady.bit.InvRlyReadyOn = TRUE; // 并机加同步动作
                                if(g_RelayVar.CtrlSta.bit.RlyOnReadyFinish)
                                {
...
//                            uwInvRlyDelayCnt = 0;
                    ...
                    else
                    {
                        ...
                        {
                            if(++s_uwSInvOfflineOpDelayCnt > 100)
                            {
                                g_InvVar.InvFlag.bit.InvWorkMode = eInv_NoOutputMode;
                                s_uwSInvOfflineOpDelayCnt = 6;
                            }
                        }
                    }
                }
...
//                    if(g_SystemInfo.SysFlag.bit.SmartLoadRlyOnFlag)
        ...
        case eInvMainState_OnGrid:
        {
            ...
            //并网电池Lose，会断开逆变继电器，如果在断开之前进入锂电激活(考虑修改激活周期计数的逻辑？)，无法转到旁路，电池无法充电
            if(   (eNoBatt == g_ComInfo.Com1.bit.BatMode || eBatLose == g_SystemInfo.Source.bit.BatSource)
               && (eActiveByNull == g_ComInfo.Com1.bit.LiActiveSource || eRelayStateOpen == g_RelayVar.CtrlSta.bit.InvRlySta) )
            {
                ...
                )
            {
                ...
                {
                    g_InvVar.InvFlag.bit.InvOnToOffGridFlag = TRUE;
                }
...
//                if(g_SystemInfo.SysFlag.bit.SmartLoadRlyOnFlag)
        ...
        case eInvMainState_OnGen:
        {
            ...
            if(    (g_SysFault.Flag.bit.GenFault)     // 油机故障
                || (!g_ComInfo.Com1.bit.GenConnectEn) // 油机接入条件不满足
                || (FALSE == unRxSysStaFlag.bit.bUPMTurnToOnGen && unRxSysStaFlag.bit.bUPMTurnToOffGrid && uiFrameParallelEnable) )
            {
                g_RelayVar.RlyReady.bit.GenRlyReadyOn = FALSE;
                // g_RelayVar.RlyReady.bit.GenNRlyReadyOn = FALSE;
                ...
                g_InvVar.InvFlag.bit.InvOnToOffGridFlag = TRUE;  // 发电机切离网会震荡 直接封波处理
            }
...
void Inv_OnGridPowerGiven(void)
{
      ...
      )
    {
        ...
        if(eBat_Normal != g_uiBatAbnormalFlg)//电池异常要保证电网可以去稳母线
        {
        ...
        else
        {
            ...
            {
                UpDownLimit(g_InvVar.fPInvTotalRef, 1.0, -1.0/*-0.001*/);//为了PV馈电踢掉PV能稳母线
            }
        }
...
*/
    ...
    else
    {
        ...
        strOnGridPaRegu.Ref = 0;//作用在中断
        strOnGridQaRegu.Ref = 0;
    }
}
...
void Inv_OnGridPowerSoftStart(void)
{
    ...
    fPowerStep_TurnOn = fPowerStep_TurnOn * 0.01f * 0.001f;//0.01是倍率，0.001是1ms//5W
       ...
       )   
    {
        ...
        else//正常并网运行时的功率变化
        {
            ...
            fPowerStep_PChange = (float)uiSciSetDataBag[eSetPRampRate] * 0.01f * 0.001f;//0.01是倍率，0.001是1ms//5W  //*(float)uiSciSetDataBag[eSetACOutputRatedP]*0.001;//0.001是1ms
            fPowerStep_QChange = (float)uiSciSetDataBag[eSetQRampRate] * 0.01f * 0.001f;//0.01是倍率，0.001是1ms//5W  //*(float)uiSciSetDataBag[eSetACOutputRatedP]*0.001;
...
void Inv_OnGridDeltaBusRefCtrl(void)
{
    if(eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode || eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode)
    {
        if((eBat_Normal == g_uiBatAbnormalFlg) && (eDcDcMainState_Running == g_DcDcVar.DcDcFlag.bit.bMainSts))
        {
...
//    UpDownLimit(g_InvVar.fInvBusVoltRef,cInvBusSetMax,cInvBusSetMin);
	...
	{
        strBusVoltUpLmtRegu.Ref = g_InvVar.fInvBusVoltRef;  //原来2V不行,LLC不是严格按照9倍
	}
	else
	{
    ...
    // else
    // {
    //     strBusVoltUpLmtRegu.Ref = g_InvVar.fInvBusVoltRef + Set_BusVolt(10);//原来2V不行,LLC不是严格按照9倍
    //     // strBusVoltUpLmtRegu.Ref += strOnGridDeltaUpBusRef.Out;//当电池充电还未达到充电限制时需要抬高母线高压环,限制逆变环抽能量

    // //逆变母线低压环
    //     //不管电池优先还是负载优先以及TOU模式,逆变环路那边都有抽能量的基础功率环;所以在满足基础功率环后,母线低压环触发值以电池放电为界限
    //     strBusVoltDownLmtRegu.Ref =	g_InvVar.fInvBusVoltRef - Set_BusVolt(10);//原来2V不行,LLC不是严格按照9倍
    //     // strBusVoltDownLmtRegu.Ref -= strOnGridDeltaDownBusRef.Out;
    // }
...
void Inv_OnGridBusVoltLoop(void)
{
    ...
    if(eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode || eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode)
    {
        ...
        if((eBat_Normal != g_uiBatAbnormalFlg) || (eNoBatt == g_ComInfo.Com1.bit.BatMode) || (eActiveByAc == g_ComInfo.Com1.bit.LiActiveSource))
        {
			...
			if(eBat_Normal != g_uiBatAbnormalFlg || eNoBatt == g_ComInfo.Com1.bit.BatMode) 
			{
				//1ms
				//strBusVoltDownLmtRegu.Kp = 0.25;//20;
				//strBusVoltDownLmtRegu.Ki = 0.002;//0.005;
				//5ms
				strBusVoltDownLmtRegu.Kp = 1;//20;	
				strBusVoltDownLmtRegu.Ki = 0.005;//0.005; 

                strBusVoltUpLmtRegu.Ref = 1.325f; // 530V
			}
        }
        else
        {
        ...
        //if(eNoBatt != g_ComInfo.Com1.bit.BatMode)
        {
            ...
            UpDownLimit(strBusVoltUpLmtRegu.integrator, 0.2, -2.0);
        ...
        // {
            ...
            if(eActiveByAc == g_ComInfo.Com1.bit.LiActiveSource)
            {
                strBusVoltDownLmtRegu.integrator = 0;
...
{
    // 功率外环输出作为电流内环的给定
    static Uint16 uiInvForbidCnt = 0;
    ...
    else   // 软起后稳定运行环路
    {
        if(   (uiInvBatOverProtFlag)
           || (g_uiBatAbnormalFlg != eBat_Normal && uiInvForbidCnt < OneMsByIntBase(5))
           )
        {
            if(strOnGridPaRegu.Out < 0)
            {
                strOnGridPaRegu.Out = strOnGridPaRegu.Out * 0.75f;
                strOnGridPaRegu.integrator = strOnGridPaRegu.integrator * 0.75f;
            }

            if(strOnGridPaRegu.Ref < 0)
            {
                strOnGridPaRegu.Ref = strOnGridPaRegu.Ref * 0.75f;
                g_InvVar.fPInvSoftStart = g_InvVar.fPInvSoftStart * 0.75f;
                fPinvLoopOutputRef = fPinvLoopOutputRef * 0.75f;
                strBusVoltUpLmtRegu.Out = strBusVoltUpLmtRegu.Out * 0.75f;
                strBusVoltUpLmtRegu.integrator = strBusVoltUpLmtRegu.integrator * 0.75f;
                strBusVoltDownLmtRegu.Out = strBusVoltDownLmtRegu.Out * 0.75f;
                strBusVoltDownLmtRegu.integrator = strBusVoltDownLmtRegu.integrator * 0.75f;
                strPVInvCtrlBatChgPRegu.Out = strPVInvCtrlBatChgPRegu.Out * 0.75f;
                strPVInvCtrlBatChgPRegu.integrator = strPVInvCtrlBatChgPRegu.integrator * 0.75f;
                strInvCtrlBatDisChgPRegu.Out = strInvCtrlBatDisChgPRegu.Out * 0.75f;
                strInvCtrlBatDisChgPRegu.integrator = strInvCtrlBatDisChgPRegu.integrator * 0.75f;
                g_InvVar.fRefluxLoopOut = g_InvVar.fRefluxLoopOut * 0.75f;
                g_InvVar.fRefluxLoopIng = g_InvVar.fRefluxLoopIng * 0.75f;
                g_InvVar.fGridOutLoopOut = g_InvVar.fGridOutLoopOut * 0.75f;
                g_InvVar.fGridOutLoopIng = g_InvVar.fGridOutLoopIng * 0.75f;
            }

            if(g_uiBatAbnormalFlg != eBat_Normal) { uiInvForbidCnt ++; }
        }
        else if(g_uiBatAbnormalFlg == eBat_Normal)
        {
            uiInvForbidCnt = 0;
        }
        ...
        else
        {
            ...
            // 市电电压以为cos定向  。当Id为0时 Iq>0 发感性无功 即电流滞后电压90度 所以这里的电流给定要改为+
            //strOnGridCurrARegu.Ref = strOnGridIdRef.fphaseA * strPllToInv.fCosA + strOnGridIqRef.fphaseA * strPllToInv.fSinA + strInvDcCurrLoopOut.fphaseA; // TODO 并机时需要改用主机下发的INV-V-DC调节量
            // strOnGridCurrBRegu.Ref = strOnGridIdRef.fphaseB * strPllToInv.fCosB + strOnGridIqRef.fphaseB * strPllToInv.fSinB + strInvDcCurrLoopOut.fphaseB;
        }
        UpDownLimit(strOnGridCurrARegu.Ref, fOnGridCurrARefLimit, (-fOnGridCurrARefLimit)); // 硬件逆变电流保护为60A峰值，限定电流环给定55A
		// 规避并网切离网瞬间，继电器未断开时导致的输出电压过高 -- Yzl add in 20260201
        if(eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode)
        {
            if(fabs(stADC.fRealScale.VGridA) >= g_InvVar.fVInvOutUpLimit)
            {
                if(++uiInputOverLimitCnt >= 5)
                {
                    strOnGridCurrARegu.Ref = 0;
                    uiInputOverLimitCnt = 0;
                }
            }
            else
            {
        ...
        {
            if(fabs(stADC.fRealScale.VGenA) >= g_InvVar.fVInvOutUpLimit)
            {
                if(++uiInputOverLimitCnt >= 5)
                {
                    strOnGridCurrARegu.Ref = 0;
...
//            g_InvVar.fCtrlLoopOutA = strOnGridCurrARegu.Out + stADC.fRealScale.VGridA;
            ...
            {
                g_InvVar.fCtrlLoopOutA = strOnGridCurrARegu.Out + (stADC.fRealScale.VGridA  * fInputVoltFeedForwardKp);
            }
            else
            {
                g_InvVar.fCtrlLoopOutA = strOnGridCurrARegu.Out + stADC.fRealScale.VGridA;
        ...
        {
            g_InvVar.fCtrlLoopOutA = strOnGridCurrARegu.Out + (stADC.fRealScale.VGenA * fInputVoltFeedForwardKp);
...
void Inv_OffGridRealVoltLoop(void)
{
    ...
    {
        strLoadCurrFeedWard.fphaseA = strInvCurrAvg.fphaseA * fInvCurrFeedForwardKp;//stValue.fRmsScale.IInvA * fInvCurrFeedForwardKp;//
    }

    if(eInv_OffGridMode != g_InvVar.InvFlag.bit.InvWorkMode)
    { strLoadCurrFeedWard.fphaseA = 0; }
	
    // 规避离网卸载瞬间，输出电压过高 -- Yzl add in 20260201
    if(fabs(stADC.fRealScale.VInvA) >= (g_InvVar.fVInvRefUpLimit + 7.07 * g_GridManager.Rated.f32VOutInvt))//5 * 1.414
    {
        strLoadVoltARmsRegu.integrator *= 0.6f;
        strLoadVoltARmsRegu.Out        *= 0.6f;
    }
    // VInv_real_ref = (Rms-Out + 电压给定前馈 + 平均电感电流前馈) *  1.414 * cosθ + 直流分量(直流分量只在单机下调节)//  + 正负BUS电压偏差补偿量
...
//    strStaticRealVoltOut.phaseA = strStaticRealVoltErrFilter.phaseA * strLoadVoltARealRegu.Kp * strPllToInv.fCosA;
    ...
    UpDownLimit(strLoadCurrARealRegu.Ref, strLoadVoltARealRegu.integTopLimit, strLoadVoltARealRegu.integDownLimit);
...
*/  
    {
        g_InvVar.fCtrlLoopOutA = strLoadCurrARealRegu.Out + (stADC.fRealScale.VInvA * fInvVoltFeedForwardKp);// strLoadVoltARealRegu.Ref;
...
void Inv_InvCtrlDataCalu(void)
{
    ...
    strInvCurrSum.fphaseA += strInvCurrSave[uiInvCurrCnt].fphaseA;
    strInvCurrAvg.fphaseA = strInvCurrSum.fphaseA * 0.0208333f;//1/48≈0.0208333
    /*
...
//	else if(222 == uiSciSetDataBag[eSetRsvd348])  // 现在先用这两个环路已足够
    {
        ...
        {
            // if(666 == uiSciSetDataBag[eSetRsvd348])
            // {strLoadVoltRmsRealRef.fphaseA = strLoadVoltARmsRegu.Ref + g_ParaVar.fDroopDeltaVolt + fRxFromSysAnalog_OutRmsAVolt + g_ParaVar.fDroopInvCurraAveOut;}
            // else
            {strLoadVoltRmsRealRef.fphaseA = strLoadVoltARmsRegu.Ref + g_ParaVar.fDroopDeltaVolt + fRxFromSysAnalog_OutRmsAVolt;}

// stateTest[0] = (int16)(fRxFromSysAnalog_OutAvgQa * 100 * g_GridManager.Rated.f32IOut);
// stateTest[1] = (int16)(stValue.fRmsScale.IInvA * 100 * g_GridManager.Rated.f32IOut);
// stateTest[2] = (int16)(g_ParaVar.fDroopInvCurraAveOut * 100 * g_GridManager.Rated.f32IOut);
// stateTest[3] = (int16)(g_ParaVar.fDroopInvCurraAveOut * 100 * g_GridManager.Rated.f32VOut);
// stateTest[4] = (int16)(strLoadVoltARmsRegu.Ref * 10 * g_GridManager.Rated.f32VOut);
// stateTest[5] = (int16)(g_ParaVar.fDroopDeltaVolt * 10 * g_GridManager.Rated.f32IOut);
// stateTest[6] = (int16)(fRxFromSysAnalog_OutRmsAVolt * 100 * g_GridManager.Rated.f32IOut);
// stateTest[7] = (int16)(strLoadVoltRmsRealRef.fphaseA * 10 * g_GridManager.Rated.f32IOut);

        }
        UpDownLimit(strLoadVoltRmsRealRef.fphaseA, g_InvVar.fVInvRefUpLimit, g_InvVar.fVInvRefDnLimit); // 加入限幅，规避过载时下垂导致的电压冲高
...
//        // Uq = -sin * UAlpha + cos * Ubeta
        ...
        fInvReactivePowerFilt += (fInvReactivePowerReal - fInvReactivePowerFilt) * cTmFilterFreq(10); //10
        // fInvReactivePowerFilt += (fInvReactivePowerReal - fInvReactivePowerFilt) * cTmFilterFreq(uiSciSetDataBag[eSetRsvd349]); //10
    // }
//     else  // 裂相和2/3相分开计算
//     {
// #if(SOGI_DQCalu == 1)
...
void Inv_ParaDroopCtrl(void)
{
    ...
    if(uiFrameParallelEnable && (eInvMainState_OffGrid == g_InvVar.InvFlag.bit.bInvMainSts))
    {
        ...
        //====================================================================
        fDroopTemp = fActivePower_Droop_Factor * (0 - fInvActivePowerReal);
        ...
        //====================================================================
		if(bMachineModel == eModel_IVAM6048 || bMachineModel == eModel_Unknown)
    	{
        	fDroopTemp = fReactivePower_Droop_Factor * (0 - fInvReactivePowerReal);
		}
		else if(bMachineModel == eModel_IVAM8048)
		{
        	fDroopTemp = fReactivePower_Droop_Factor * (0 - fInvReactivePowerFilt);
		}
...
{	
    ...
    if(g_ParaVar.ParaFlag.bit.RxSysCtrlDataRenew)
    {
            ...
            && (uiSysOffGridRunNum >= 2) )	//&& TRUE == unRxSysStaFlag.bit.bUPMOffLineMultiNum )	// 两台以上并机运行才执行
        {
            ...
            // }
            fQaAveReguErr = fRxFromSysAnalog_OutAvgQa - stBasicPower.InvQa.fBasicWaveScale;
            UpDownLimit(fQaAveReguErr, 0.25f, -0.25f);  // Set_Power(1000)

            if(abs(fQaAveReguErr) <= 0.01f)  // Set_Power(40)
            {
            ...
            else if(abs(fQaAveReguErr) >= 0.025f) // Set_Power(100)
            {
                uiQaErrClrCnt = 0;
...
/******************************************************************************
Function Name: Inv_ParaInvCurrAvgLoop
Function Descriptions:并机平均电流环路
Parameter Name list:
ReturnType:
Refer doctuments: 1ms()
******************************************************************************/
void Inv_ParaInvCurrAvgLoop(void)
{
    // static Uint16 uiIaErrClrCnt = 0;
    // static Uint16 uiQbErrClrCnt = 0;
    // static Uint16 uiQcErrClrCnt = 0;

    float32 fIaAveReguErr;
    // float32 fQbAveReguErr;
    // float32 fQcAveReguErr;
    ...
    if(g_ParaVar.ParaFlag.bit.RxSysCtrlDataRenew)
    {
        ...
        {
            // Phase A
            fIaAveReguErr = fRxFromSysAnalog_OutAvgQa - stValue.fRmsScale.IInvA;//g_fInvCurrFilt;
            // UpDownLimit(fIaAveReguErr, 0.6f, -0.6f);  // Set_Power(1000)

            // if(abs(fIaAveReguErr) <= 0.01f)  // Set_Power(40)
            // {
            //     if(++uiIaErrClrCnt > SecondBy1msBase(2))
            //     {
            //         uiIaErrClrCnt = SecondBy1msBase(2);
            //         g_ParaVar.ParaFlag.bit.QaAveErrClr = TRUE;
            //     }
            // }
            // else if(abs(fIaAveReguErr) >= 0.025f) // Set_Power(100)
            // {
            //     uiIaErrClrCnt = 0;
            //     g_ParaVar.ParaFlag.bit.QaAveErrClr = FALSE;
            // }
            ...
            // if(g_ParaVar.ParaFlag.bit.QaAveErrClr)	// 功率差小于40W不调节
            // {
            //     fIaAveReguErr = 0;	// 环流波形偶尔会有抖动，效果不是很好
            // }

            g_ParaVar.fDroopInvCurraAveOut = g_ParaVar.fDroopInvCurraAveInteg + fIaAveReguErr * g_ParaVar.fDroopInvCurrAveKp;
            g_ParaVar.fDroopInvCurraAveInteg += fIaAveReguErr * g_ParaVar.fDroopInvCurrAveKi;
            UpDownLimit(g_ParaVar.fDroopInvCurraAveOut,   g_ParaVar.fDroopInvCurrIntegUpLimit, -g_ParaVar.fDroopInvCurrIntegUpLimit);
            UpDownLimit(g_ParaVar.fDroopInvCurraAveInteg, g_ParaVar.fDroopInvCurrIntegUpLimit, -g_ParaVar.fDroopInvCurrIntegUpLimit);
// stateTest[5] ++;
        }
        else
        {
...
// stateTest[0] = (int16)(g_ParaVar.fDroopInvCurrAveKp * 1000);
// stateTest[1] = (int16)(g_ParaVar.fDroopInvCurrAveKi * 1000);
// stateTest[2] = (int16)(fIaAveReguErr * 1000);
// stateTest[3] = (int16)(g_ParaVar.fDroopInvCurraAveInteg * 1000);
// stateTest[4] = (int16)(g_ParaVar.fDroopInvCurraAveOut * 1000);
// stateTest[0] = (int16)(fRxFromSysAnalog_OutAvgQa * 100 * g_GridManager.Rated.f32IOut);
// stateTest[1] = (int16)(stValue.fRmsScale.IInvA * 100 * g_GridManager.Rated.f32IOut);
// stateTest[2] = (int16)(g_ParaVar.fDroopInvCurraAveOut * 100 * g_GridManager.Rated.f32IOut);
// stateTest[3] = (int16)(g_ParaVar.fDroopInvCurraAveInteg * 100 * g_GridManager.Rated.f32IOut);
// stateTest[4] = (int16)(fIaAveReguErr * 100 * g_GridManager.Rated.f32IOut);
// stateTest[5] = (int16)(strLoadVoltARmsRegu.Ref * 100 * g_GridManager.Rated.f32IOut);
    }
// stateTest[6] = (int16)(uiFrameAnalogData_CurrRmsA(1) * 0.1 * g_GridManager.Rated.f32IOut);
// stateTest[7] = (int16)(uiFrameAnalogData_CurrRmsA(2) * 0.1 * g_GridManager.Rated.f32IOut);
// stateTest[8] = (int16)(uiFrameAnalogData_CurrRmsA(3) * 0.1 * g_GridManager.Rated.f32IOut);
// stateTest[9] = (int16)(fRxFromSysAnalog_OutAvgQa * 100 * g_GridManager.Rated.f32IOut);
}
...
//    strBusVoltUpLmtRegu.Kp = ((float)uiSciSetDataBag[eSetRsvd5]) * 0.001;  // 100
//    strBusVoltUpLmtRegu.Ki = ((float)uiSciSetDataBag[eSetRsvd6]) * 0.0001; // 50
//	
...
//	strPNBUSRmsReguVoltOnGrid.integDownLimit = -strPNBUSRmsReguVoltOnGrid.integTopLimit;
    ...
    {
        // 2k调节1Hz  设置多少功率调多少频率
        fActivePower_Droop_Factor = Set_Power(2000);
        // fActivePower_Droop_Factor = Set_Power(uiSciSetDataBag[eSetRsvd346]) * 1;

        // 4k调节30V 设置多少功率调多少V
        fReactivePower_Droop_Factor = Set_Power(4000) * 30 * g_GridManager.Rated.f32VOutInvt;
        // fReactivePower_Droop_Factor = Set_Power(uiSciSetDataBag[eSetRsvd347]) * 30 * g_GridManager.Rated.f32VOutInvt;
    }
    else if(bMachineModel == eModel_IVAM8048)
    {
        // 2.6k调节1Hz  设置多少功率调多少频率
        fActivePower_Droop_Factor = Set_Power(2600);
        // fActivePower_Droop_Factor = Set_Power(uiSciSetDataBag[eSetRsvd346]) * 1;

        // 5k调节30V 设置多少功率调多少V
        fReactivePower_Droop_Factor = Set_Power(5000) * 30 * g_GridManager.Rated.f32VOutInvt;
        // fReactivePower_Droop_Factor = Set_Power(uiSciSetDataBag[eSetRsvd347]) * 30 * g_GridManager.Rated.f32VOutInvt;
    }
    ...
    // g_ParaVar.fDroopInvCurrAveKp = ((float)uiSciSetDataBag[eSetRsvd362]) * 0.001;  // 100
    // g_ParaVar.fDroopInvCurrAveKi = ((float)uiSciSetDataBag[eSetRsvd363]) * 0.0001; // 50
}
...
//        UpDownLimit(g_InvVar.fRefluxLoopErr,0.1,-0.1);  // 这个限制会导致防逆流功率设置从大变小得时候每次只有0.1*0.1的变化
        g_InvVar.fRefluxLoopIng += g_InvVar.fRefluxLoopErr * 0.36f;
        g_InvVar.fRefluxLoopOut = g_InvVar.fRefluxLoopErr * 0.5f + g_InvVar.fRefluxLoopIng;
...
//                   || ((Set_Power(temp) < 0.005f) /*&& (fGridPower <0 &&  fGridPower >-0.005f)*/))
//                {
//                    SetPower = Set_Power(temp);
//                    SoutPower = (stPower.SOut.PhaseA + stPower.PSmartLoad.PhaseA);
//                    bTest1++;
//                	g_InvVar.fRefluxLoopOut = 0;
//                	g_InvVar.fRefluxLoopIng = 0;
//                }
//            }
    ...
    else if(eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode)
    {
		...
		{
			fGenPower = fRxFromSysAnalog_GridAvgPa;
		}
        g_ComInfo.fRefluxPowerRef = 0;
        g_InvVar.fRefluxLoopErr = 0 - fGenPower;
//        UpDownLimit(g_InvVar.fRefluxLoopErr, 0.1, -0.1);
        g_InvVar.fRefluxLoopIng += g_InvVar.fRefluxLoopErr * 0.36f;//fluxLimitKi;
        g_InvVar.fRefluxLoopOut = g_InvVar.fRefluxLoopErr * 0.5f/*fluxLimitKp*/ + g_InvVar.fRefluxLoopIng;
...
static void Inv_GridShavePowerLimit(void)
{
       ...
       && (eDcDcMainState_Running == g_DcDcVar.DcDcFlag.bit.bMainSts) )
    {
        if(    (g_ComInfo.Com1.bit.UserTimeEn)
            ...
            && (eECOModeDischarge != g_ComInfo.iEcoBatChgDisChgEn)
...
//        }//20251018--实际测试中用PI会出现反应过慢且波动的问题。
        ...
        UpDownLimit(g_InvVar.fGridInPowerLoopIng, 0.1f, -2);  //0.5f,
    ...
    if(eNoBatt != g_ComInfo.Com1.bit.BatMode)
    {
            ...
            )
        {
            ...
            if(eRelayStateClose == g_RelayVar.CtrlSta.bit.InvRlySta)
            {
                ...
                g_SystemInfo.PowerBalance.PvMaxPower += (g_ComInfo.fRefluxPowerRef * 1.04f + fPOutTotalTemp * 1.06f);  //1.086f
                //交流侧最大限制在6K, 这个要算上损耗 按最大1.2倍 加上0.1的损耗
                UpDownLimit(g_SystemInfo.PowerBalance.PvMaxPower, 1.1f, 0);//Set_Power(6000)
            }
            // 3、直流侧电池功率-----有电池 且 电池充电正常 且 无锂电禁充
            if(   (eBat_Normal == g_uiBatAbnormalFlg)
               && (eDcDcMainState_Running == g_DcDcVar.DcDcFlag.bit.bMainSts)
//               && (!strBMSLogicInfo.uiBMS1BatStopChargeFlag)  //暂时屏蔽，进版时要关注
               )
            {
                //4、对电池充电的话,由于电池损耗,电池无法达到要求的充电电流,需要将电池实时有效值电流作为反馈叠加上
                g_SystemInfo.PowerBalance.PvMaxPower += (g_DcDcVar.fBatMaxChrCurrSet * stValue.fAveScale.VBat * 1.07f);

                //解决并网PV充足市电会出一部分功率的问题(PV功率不足积分会饱和)
                //充电补偿环对带载也有补偿，所以这里判断的是市电功率
				//理论上如果是市电带载工况，PV功率充足去加积分没有问题，PV功率不足去加积分也没有问题
				if(    fabs(strPVBatChgPowerLimitLoop.Err) < Set_Power(100)
					&& (   (eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode && stPower.fPGridTotal > Set_Power(30)/* && stPower.fPGridTotal < Set_Power(500)*/)
                         ||(eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode && stPower.fPGenTotal > Set_Power(30) /* && stPower.fPGenTotal < Set_Power(500)*/)
                       )
                   )
                {
                    strPVBatChgPowerLimitLoop.integrator += Set_Power(0.5);
                }
                ...
                {
				    strPVBatChgPowerLimitLoop.integTopLimit = Set_Power(350);      //0.05f;// 300W
                	strPVBatChgPowerLimitLoop.integDownLimit = (-Set_Power(350));  //0;//-0.05f;// -300W
                }
            ...
            else // 电池开路
            {
                ...
                strPVBatChgPowerLimitLoop.Out = 0;
//                // if( (eBat_Normal != g_uiBatAbnormalFlg)
//                if(    (eBat_Normal != g_uiBatAbnormalFlg)//(!BatAbnormalFlg)
...
//                }
    ...
    else if((TRUE == (uiSciCommandDataBag[eComATECommand] & eAtePvCcEnBit)) && (0 != uiSciSetDataBag[eSetMaxPVInputPower]))
    {
        g_SystemInfo.PowerBalance.PvMaxPower = Set_Power(uiSciSetDataBag[eSetMaxPVInputPower]);
    }
...
/******************************************************************************
Function Name: Inv_CtrlBatChgPowerLimit
-------------------
Function Descriptions:
并网时：PV存在时,PV对电池充电,多余并网,通过电池充电参数来控逆变环路,从而达到满足电池充电参数(电池最大充电电流、电池最大充电电压)。
...
static void Inv_CtrlBatChgPowerLimit(void)
{
    ...
    if(   ((eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode) || (eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode))
       && (g_DcDcVar.DcDcFlag.bit.bMainSts == eDcDcMainState_Running) && (eBat_Normal == g_uiBatAbnormalFlg) )//(FALSE == BatAbnormalFlg) )//有电池且LLC软启成功;并网/油机
    {
...
/******************************************************************************
Function Name: Inv_CtrlBatDisChgPowerLimit
-------------------
...
//  float fPOutTotalGrid = 0;//市电端负载功率
    if(    ((eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode) || (eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode))
        ...
        && (eBat_Normal == g_uiBatAbnormalFlg) )//(FALSE == BatAbnormalFlg) ) //有PV有电池且LLC软启成功
    {
...
//		UpDownLimit(strInvCtrlBatDisChgPRegu.integTopLimit,1,0);
        ...
        if(fabs(strInvCtrlBatDisChgPRegu.Err) < 0.005f) { strInvCtrlBatDisChgPRegu.Err = 0; } //大于0.6A才调节,防止一直调节抑制PV出功率;积分也做双边调节(PV不追或者PV追了又掉)，原来1.2A会导致停止市电充电后还有一定充电电流，此处改小。
...
static void Inv_DisChgCurrLimitByBatVolt(void)
{
    ...
    if(bMachineModel == eModel_IVAM6048 || bMachineModel == eModel_Unknown)
    {
        ...
        if(g_SystemInfo.fBatVoltAvgReal >= 52)      { fLoopLimitDerateScaleSet = (cIInv_Offgrid_Hardware_Limit  * g_GridManager.Rated.f32IOutInvt); }
        else if(g_SystemInfo.fBatVoltAvgReal <= 51) { fLoopLimitDerateScaleSet = (cIInv_Offgrid_Hardware_Limit2  * g_GridManager.Rated.f32IOutInvt); }
    ...
    else if(bMachineModel == eModel_IVAM8048)
    {
        // if(g_SystemInfo.fBatVoltAvgReal >= 48)      { fLoopLimitDerateScaleSet = 1.135f; }
        // else if(g_SystemInfo.fBatVoltAvgReal <= 47) { fLoopLimitDerateScaleSet = 1.0f;  }
        if(g_SystemInfo.fBatVoltAvgReal >= 48)      { fLoopLimitDerateScaleSet = (cIInv_Offgrid_Hardware_Limit  * g_GridManager.Rated.f32IOutInvt); }
        else if(g_SystemInfo.fBatVoltAvgReal <= 47) { fLoopLimitDerateScaleSet = (cIInv_Offgrid_Hardware_Limit2  * g_GridManager.Rated.f32IOutInvt); }
    }
    ...
    UpDownLimit(fLoopLimitDerateScale, 1.85f, 1.0f);

    //------------------------------------
    // 更新至电压环和电流环的限幅
    //------------------------------------
    strLoadVoltARealRegu.integTopLimit  = fLoopLimitDerateScale;
    strLoadVoltARealRegu.integDownLimit = -fLoopLimitDerateScale;

    strLoadCurrARealRegu.integTopLimit  = fLoopLimitDerateScale;
    strLoadCurrARealRegu.integDownLimit = -fLoopLimitDerateScale;
}
