...
// 程序版本和日期放到这里
#define cDSPVersion     ((Uint16)2008)   //((Uint16)2319)   // 单机并机合并程序 整合机型
//版本说明
...
static void System_GenConnectCheck(void)
{
    ...
    static float fRmsRealVGenABack = 0;
    static float fRmsRealVGridABack = 0;
    ...
    // //①若signal本身是0，没必要进入下面了 ②已经在市电的状态下（备份市电电压，或者标志也行) ③特殊接法：油机接入到电网端口上
    if((0 == g_SystemInfo.SysFlag.bit.GenStartSignal)
    		||(eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode)
			||(uiSciSetDataBag[eSetGenConnectToGridPortEnable] && eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode)
		)
    {
        s_uiOffGridSteadyWorkCnt = 0;
        // VGridABack只在市电端口电压正常时才跟随更新，低压时冻结.(经过测试发现，电网/油机电压低于60V时还没切出市电模式，这样会更新Back值导致无法进入下一个判断)
        if(stValue.fRmsReal.VGenA >= 60.0f)
        {
            fRmsRealVGenABack = stValue.fRmsReal.VGenA;
        }
        if(stValue.fRmsReal.VGridA >= 60.0f)
        {
            fRmsRealVGridABack = stValue.fRmsReal.VGridA;
        }

    }
    //非市电条件，满足原先进入过的条件，则去清除
    else if(   (g_SystemInfo.SysFlag.bit.GenStartSignal)
       && (((eInv_OnGenMode != g_InvVar.InvFlag.bit.InvWorkMode)&& (((stValue.fRmsReal.VGenA < 60) && (fRmsRealVGenABack > 60.0f))))
			   ||
			   /* 新增逻辑：油机接市电端口，VGenA采不到油机电压改用市电端口电压 */
		((eInv_OnGridMode != g_InvVar.InvFlag.bit.InvWorkMode)&&(uiSciSetDataBag[eSetGenConnectToGridPortEnable]
			    && (stValue.fRmsReal.VGridA < 60)  && (fRmsRealVGridABack > 60.0f))))    // 市电端口此时也低压 // Back值曾经>60V
	  )
    {
