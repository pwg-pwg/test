...
static void Gen_ReConnectCheck(void)
{
    ...
    if(eSysStandByStatus == g_SystemVar.SysRunFlag.bit.bSystemMainSts)
    {
        if(FALSE == g_SysFault.ubFaultGen.bit.GenOVP)
        {
            ...
            //    )
            if(stValue.fRmsReal.VGenA > g_GenConnect.f32VGenMaxLimitBack && g_InvVar.InvFlag.bit.bPllLockGenOKFlag) // 20260128--锁相未完成，计算的电压有效值在临界点附近容易波动，导致无法正常接入
            {
                s_u16OVPCntEnter++;
        ...
        if(FALSE == g_SysFault.ubFaultGen.bit.GenUVP && g_InvVar.InvFlag.bit.bPllLockGenOKFlag) // 20260128--锁相未完成，计算的电压有效值在临界点附近容易波动，导致无法正常接入
        {
...
static void Gen_OverVolt1Check(void)
{
    ...
    if(FALSE == g_SysFault.ubFaultGen.bit.GenOVP)
    {
        ...
        //   )
        if(stValue.fRmsReal.VGenA > g_GenVolt.f32VGenMax1Limit && g_InvVar.InvFlag.bit.bPllLockGenOKFlag) // 20260128--锁相未完成，计算的电压有效值在临界点附近容易波动，导致无法正常接入
        {
            s_u16CntEnter++;
...
static void Gen_UnderVolt1Check(void)
{
    ...
    {    
        ...
        //    )
        if(stValue.fRmsReal.VGenA < g_GenVolt.f32VGenMin1Limit && g_InvVar.InvFlag.bit.bPllLockGenOKFlag) // 20260128--锁相未完成，计算的电压有效值在临界点附近容易波动，导致无法正常接入
        {
            s_u16CntEnter++;
...
static void Gen_GenRecoverChk(void)
{
    ...
    {
		if( (stValue.fRmsReal.VGenA > g_GenVolt.f32VGenMin1Limit) && (stValue.fRmsReal.VGenA < g_GenVolt.f32VGenMax1Limit) //使用电网电压的da值进行判断 
...
/******************************************************************************
Function Name: 电网过载保护
------------------------------------------------------------------------------
Function Descriptions: 故障发生后关整机
Parameter Name list:
ReturnType:
Refer doctuments: 20ms()
******************************************************************************/
void Gen_GenOverLoadCheck(void)
{
 // 在Grid里一起判断了
}
