...
static float32 Bat_BatChgByVoltPerCalc(float32 fBatEocVolt, Uint16 uiType);
static void Bat_ForceChargeInEOD(void);
/******************************************************************************
...
void Bat_Initialize(void)
{
   ...
   fChrgProtCurrLimit       = fModelParam[eChrgProtCurr];
    //----------------------------
...
static void Bat_BmsDataUpdate(void)
{
    ...
    // 并机要均分BMS下发的最大充放电电流
    // 并机从机数据都是0
    if(uiSciCommand_BMSDataBag[eBMS1ConnectFlg] == FALSE || eLithiumSOC != uiSciSetDataBag[eSetBatMode])  // 如果是电压模式就算是连上了也不使用电池数据
    {
        strBMSLogicInfo.uiBMS1Connect           = FALSE;
        strBMSLogicInfo.uiBMS1ForceCharge       = FALSE;
        strBMSLogicInfo.uiBMS1BatStopChargeFlag = FALSE;
...
void Bat_BatForceChargeJudge(void)
{
    ...
    if(eSystemInitAllSuccess == g_SystemVar.SysRunFlag.bit.bSystemInitState)
    {
        if((eLithiumSOC == g_ComInfo.Com1.bit.BatMode) && (strBMSLogicInfo.uiBMS1DataAllowUse))
        {
			...
			// 强充点要比关机点高2%
		    if(uiSciSetDataBag[eSetOnGridBatAutoStartChargeSOC] <= (uiSciSetDataBag[eSetBatLowShutDownSOC] + 2))
		    {
			    uiSciSetDataBag[eSetOnGridBatAutoStartChargeSOC] = uiSciSetDataBag[eSetBatLowShutDownSOC] + 2;
		    }
            ...
            Uplimit(uiSciSetDataBag[eSetOnGenBatExitAutoChargeSOC], 100);

            // 1、确定电池强充的启动点和退出点
            if(uiSciSetDataBag[eSetGenForceRun] || strBMSLogicInfo.uiBMS1ForceCharge) // 客户设置强充 或 BMS发起强充
            {
            ...
            else
            {
                if(    (uiSciSetDataBag[eSetECO_TimeOfUse])
                    ...
                    && (g_SystemInfo.SysFlag.bit.EcoGenCharTemp)
                    )
                {
                    if(1 == uiSciSetDataBag[eSetGenConnectToGridPortEnable]) // 区分油机接入市电端口还是油机端口
                    {
        ...
        else if(eNoBatt != g_ComInfo.Com1.bit.BatMode)
        {
            ...
            // 强充点要比关机点高2V
            if(uiSciSetDataBag[eSetOnGridBat1AutoStartChargeVolt] <= (uiSciSetDataBag[eSetBat1LowShutDownVolt] + 20))
            {
                uiSciSetDataBag[eSetOnGridBat1AutoStartChargeVolt] = uiSciSetDataBag[eSetBat1LowShutDownVolt] + 20;
            }
            // 强充点要大于等于40V
            ...
            // 强充点要比关机点高2V
			if(uiSciSetDataBag[eSetOnGenBat1AutoStartChargeVolt] <= (uiSciSetDataBag[eSetBat1LowShutDownVolt] + 20))
			{
			    uiSciSetDataBag[eSetOnGenBat1AutoStartChargeVolt] = uiSciSetDataBag[eSetBat1LowShutDownVolt] + 20;
			}
            // 强充点要大于等于40V
...
//    int16 iEcoModeStopSocLimitTemp;
    ...
    {
        strBatPowerDerate.fBatDisBmsOrder = cBatDischgCurrMax;
    }
...
//            }
        ...
        else
        {
            ...
            {
                uiBatLowCnt2 = 0;
                strBatPowerDerate.fBatDisVoltOrder = cBatDischgCurrMax;
                uiBatFirstLowFlg = 0;
            }
        }
    }
...
//            }
        ...
        else
        {
            ...
            {
                strBatPowerDerate.fBatDisSocOrder = cBatDischgCurrMax;
            }
        }
    }
    else
    {
        strBatPowerDerate.fBatDisSocOrder = cBatDischgCurrMax;  // 额定 todo Ivan 240430
    }
    ...
    // 电池最大放电电流限制 -- 负数就变成充电了
    fBatDisChrCurrOrder = strBatPowerDerate.fBatDisCmdOrder;     // 读取设置的放电电流
    if(fBatDisChrCurrOrder > strBatPowerDerate.fBatDisVoltOrder) // 电压模式限定放电电流
    {
        fBatDisChrCurrOrder = strBatPowerDerate.fBatDisVoltOrder;
    }
    if(fBatDisChrCurrOrder > strBatPowerDerate.fBatDisSocOrder) // SOC模式限定放电电流
    {
        fBatDisChrCurrOrder = strBatPowerDerate.fBatDisSocOrder;
    }
    if(fBatDisChrCurrOrder > strBatPowerDerate.fBatDisBmsOrder) // BMS限定放电电流
    {
        fBatDisChrCurrOrder = strBatPowerDerate.fBatDisBmsOrder;
    }
    if(fBatDisChrCurrOrder > strBatPowerDerate.fBatDisTmpSumOrder) // 过温限定放电电流
    {
    ...
    // 1、根据指令限制最大充电电流
    strBatPowerDerate.fBatChrCmdOrder = (((float)uiSciSetDataBag[eSetBat1MaxChargeCurrValue]) * 0.1f); // 23 电池1最大充电电流限幅值

    //低压关AC时，强制充电
    if(g_ComInfo.Com1.bit.ForceChargeInEOD)
    {
        if(strBMSLogicInfo.uiBMS1Connect)
        {
            strBatPowerDerate.fBatChrCmdOrder = ((float)strBMSLogicInfo.iBMS1BatMaxChgCurrent * 0.1f);
        }
        else
        {
			UpDownLimit(strBatPowerDerate.fBatChrCmdOrder, 30.0f, 10.0f);
        }
    }
    // 电池高压限充电 或 浮充停止充电时，充电功率不能超过保护限定
    ...
    if(stValue.fAveReal.VBat < g_ComInfo.fBat1VoltUpLimit && g_ComInfo.Com1.bit.BatCharDis == FALSE)
    {
        ...
        {
            strBatPowerDerate.fBatChrCmdOrder = fReal_BatCurrLimit(stValue.fAveReal.VBat) + 0.5f;//0.5是取四舍五入
        }
    }
    else
    {
    ...
    {
        strBatPowerDerate.fBatChrBmsOrder = ((float)strBMSLogicInfo.iBMS1BatMaxChgCurrent * 0.1f);
    }
    else
    {
    ...
    // 2、根据电池高压限制充电
    if(FALSE == strBMSLogicInfo.uiBMS1DataAllowUse) // BMS连上的话不用电压作为限流的判断
    {
        if(g_ComInfo.Com1.bit.UserTimeEn)
        {
            if(eECOModeCharge == g_ComInfo.iEcoBatChgDisChgEn)
            {
                ...
                {
                    fBatChrLimitVolt = ((float)uiSciSetDataBag[eSetMicro_OpenBatVolt] - 5) * 0.1f;
                }
                else
                {
                ...
                uiBatChgPerVolt = Bat_BatChgByVoltPerCalc(fBatChrLimitVolt, eInEco);
        ...
        else
        {
            ...
            uiBatChgPerVolt = Bat_BatChgByVoltPerCalc(fBatChrLimitVolt, eInNormal);
            strBatPowerDerate.fBatChrVoltOrder = strBatPowerDerate.fBatChrCmdOrder * ((float32)uiBatChgPerVolt) * 0.01f;
        }
    }
    else
    ...
    if(strBMSLogicInfo.uiBMS1DataAllowUse)
    {
        ...
        if(g_ComInfo.Com1.bit.UserTimeEn)
        {
            if(eECOModeCharge == g_ComInfo.iEcoBatChgDisChgEn)
            {
                ...
                strBatPowerDerate.fBatChrSocOrder = strBatPowerDerate.fBatChrCmdOrder * (float)uiBatChgPerSoc * 0.01f;
            }
            else if(eECOModeDischarge == g_ComInfo.iEcoBatChgDisChgEn)
            {
...
//        if(fChrgProtCurrLimit < 135)   fChrgProtCurrLimit = fChrgProtCurrLimit +  1.0;
    ...
    if(g_ComInfo.Com1.bit.BatCharDis == FALSE)
    {
        if(fChrgProtCurrLimit < fModelParam[eChrgProtCurr])
        {
...
static float32 Bat_BatDisChgByVoltPerCalc(float32 fBatEodVolt)
{
    float32 iBat1DisChgPerVolt = 100;
...
static float32 Bat_BatChgByVoltPerCalc(float32 fBatEocVolt, Uint16 uiType)
{
    ...
    {
        if(stValue.fAveReal.VBat <= (fBatEocVolt - 0.5f))  // 达到Volt(Set)-0.5V以下，充电功率百分比为100
        {
            fBatChgPerVolt = 100;
        }
        else if(stValue.fAveReal.VBat < fBatEocVolt)  // 达到Volt(Set)-0.5和 Volt(Set)之间，做曲线
        {
...
/******************************************************************************
Function Name: Bat_ForceChargeInEOD
--------------------
Function Descriptions: 防止电池电压过低，在禁止充电下也会强制充电
Parameter Name list:
ReturnType:
Refer doctuments:
//-----------------------------------------------------------------------
// 强制保电：--20260429
// 1. 并电网，电池电压 or SOC < EOD, 开启AC强制保电，按照设定的充电电流进行，充电电流最小为10A。
// 2. 并油机，电池电压 or SOC < EOD, 禁止AC强制保电。
// 3. 电池充电电流为0，离网+PV，电池电压 or SOC < EOD，开启PV强制保电，充电电流最小为20A。
// 4. BMS发起强充标志
//-----------------------------------------------------------------------
******************************************************************************/
static void Bat_ForceChargeInEOD(void)
{
    static Uint16 uiForceChargeVoltCnt = 0;
	static Uint16 uiForceChargeSocCnt = 0;
	static Uint16 uiForceChargeByVolt = 0;
	static Uint16 uiForceChargeBySoc = 0;

	if(uiSciCommandDataBag[eComATECommand] & eAteForceChargeDisEnBit)
	{
		g_ComInfo.Com1.bit.ForceChargeInEOD = 0;
		return;
	}

    if(    (eBatNormal == g_SystemInfo.Source.bit.BatSource) // 电池非开路 且 不进入激活逻辑
        && (eNoBatt != g_ComInfo.Com1.bit.BatMode)
        && (    (eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode)           // 电网正常，电网强制保电
            ||  (  (eInvMainState_Idle == g_InvVar.InvFlag.bit.bInvMainSts)     // 电网异常，油机异常，PV强制保电
                && (eGridOffline == g_SystemInfo.Source.bit.GridSource && eGenOffline == g_SystemInfo.Source.bit.GenSource && g_SystemInfo.SysFlag.bit.PvWork)
                && (0 == uiSciSetDataBag[eSetBat1MaxChargeCurrValue])
                )
    ...
    {
        //--- 强制保电启动 ---//
        if(0 == g_ComInfo.Com1.bit.ForceChargeInEOD) 
        {
            if(g_SystemInfo.fBatVoltAvgReal <= (g_ComInfo.fBat1VoltShunDown))  
            {
                uiForceChargeByVolt = 1;
            }

			if(strBMSLogicInfo.uiBMS1DataAllowUse && (strBMSLogicInfo.uiBMS1BatSOC <= (g_ComInfo.uwBat1SocShunDown * 10)))  
			{
			    uiForceChargeBySoc = 1;
			}
			
			// 1. BMS发起强充
			// 2. 强制保电启动: Volt <= EOD点
			// 3. 强制保电启动: SOC <= EOD点
            if(strBMSLogicInfo.uiBMS1ForceCharge || uiForceChargeByVolt || uiForceChargeBySoc)          

            {
                g_ComInfo.Com1.bit.ForceChargeInEOD = 1;
                uiForceChargeVoltCnt = 0;
				uiForceChargeSocCnt = 0;
            }
        }
        else
        {
            if(uiForceChargeByVolt)
            {   
                // 强制保电计时: Volt >= EOD点 + 0.8V 
                if(g_SystemInfo.fBatVoltAvgReal >= (g_ComInfo.fBat1VoltShunDown + 0.8))       
                {
                 	if(++uiForceChargeVoltCnt > HoursBy100msBase(1))
                	{
                    	uiForceChargeVoltCnt = HoursBy100msBase(1);
                	}
                }

				// 1. Volt强制保电退出: Volt >= (EOD点 + 2V) & 2Min
				// 2. Volt强制保电退出: Volt >= (EOD点 + 0.8V) & 10Min
				if(    ( g_SystemInfo.fBatVoltAvgReal >= (g_ComInfo.fBat1VoltShunDown + 2) && uiForceChargeVoltCnt >= MinuteBy100msBase(2))
                    || ( g_SystemInfo.fBatVoltAvgReal >= (g_ComInfo.fBat1VoltShunDown + 0.8) && uiForceChargeVoltCnt >= MinuteBy100msBase(10)) )
				{
				    uiForceChargeByVolt = 0;
					uiForceChargeVoltCnt = 0;
				}
            }

			if(uiForceChargeBySoc)
			{
			    // 强制保电计时: SOC >= EOD点 + 1%
			    if(strBMSLogicInfo.uiBMS1DataAllowUse && (strBMSLogicInfo.uiBMS1BatSOC >= (g_ComInfo.uwBat1SocShunDown * 10 + 10)))  
			    {
					if(++uiForceChargeSocCnt > HoursBy100msBase(1))
            		{
                		uiForceChargeSocCnt = HoursBy100msBase(1);
            		}
			    }

                // 1. Soc强制保电退出: SOC >= EOD点 + 2% & 2Min
                // 2. Soc强制保电退出: SOC >= EOD点 + 1% & 10Min
				if(strBMSLogicInfo.uiBMS1DataAllowUse)
				{
					if(    ( strBMSLogicInfo.uiBMS1BatSOC >= (g_ComInfo.uwBat1SocShunDown * 10 + 20) && uiForceChargeSocCnt >= MinuteBy100msBase(2))
                        || ( strBMSLogicInfo.uiBMS1BatSOC >= (g_ComInfo.uwBat1SocShunDown * 10 + 10) && uiForceChargeSocCnt >= MinuteBy100msBase(10)) )
					{
					     uiForceChargeBySoc = 0;
					     uiForceChargeSocCnt = 0;
					}
				}
				else
            ...
            //--- 强制保电退出 ---//
            if(    (FALSE == strBMSLogicInfo.uiBMS1ForceCharge) 
			    && (FALSE == uiForceChargeByVolt)
			    && (FALSE == uiForceChargeBySoc)
              )
            {
                g_ComInfo.Com1.bit.ForceChargeInEOD = 0;
            }
