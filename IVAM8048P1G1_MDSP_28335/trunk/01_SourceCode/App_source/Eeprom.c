...
//                        if(uiSetDataRange[uiE2promBufferDataID][eMinValue] > 32767)   //此值当有符号数
						GetDataRange(uiE2promBufferDataID, &stValRange);
						if(stValRange.uiMinVal> 32767)
                        {
                            if(   (int16)strE2promBag.unActualData[uiE2promBufferDataID] < (int16)stValRange.uiMinVal
                               || (int16)strE2promBag.unActualData[uiE2promBufferDataID] > (int16)stValRange.uiMaxVal
                              ) //超出设定范围则用默认值
                            {
                                SciProtocol_SetDataSet(uiE2promBufferDataID, stValRange.uiDefaultVal);
                            }
                            else
                            {
                        ...
                        {
                            if(   strE2promBag.unActualData[uiE2promBufferDataID] < stValRange.uiMinVal
                               || strE2promBag.unActualData[uiE2promBufferDataID] > stValRange.uiMaxVal
                              ) //超出设定范围则用默认值
                            {
                                SciProtocol_SetDataSet(uiE2promBufferDataID, stValRange.uiDefaultVal);
                            }
		...
		if(1 == uiSciCommandDataBag[eComFactoryReset])  //监控恢复出厂设置
 		{
			...
			//需要根据Model区分的默认值
			SetSpecialParamsDefaultValue(&uiSciSetDataBag[eSetMachineID]);
