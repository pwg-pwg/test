...
static void Llc_CtrlLoop(void)   // 软起时间太长 移到任务里执行
{
	...
	static Uint16 uwLlcStep1DelayCnt = 0;
    int32 uwLlcOpenSoftTemp = 0;
    Uint16 uwLlcOpenSoftTimeTemp = 0;
    static float  fLlcSoftScale = 1.0f;
	static Uint16 uwLlcPwmSoftStep1Time = cDelay400ms_20ms;
	static Uint16 uwSoftFastAdjusted = FALSE;   // 本次软启动快速调整是否完成
	Uint16 uwScaledInt;
    float  fDeltaRatio; 
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
				...
				//第一阶段可能超时,需要判断母线电压是否正常,异常可能是母线短路，再继续软起可能炸LLC管
                if(LLC_SOFTSTART1 == g_DcDcVar.uwLlcSoftStartStep)
			    {
				    uwLlcStep1DelayCnt++;
				    if(stValue.fAveReal.VBus <  120.0f)
				    {
					    if(uwLlcStep1DelayCnt > 3)
					    {
						    g_SysFault.ubFaultDc.bit.LlcSoftStartFail = TRUE;
						    g_DcDcVar.DcDcFlag.bit.LlcSoftStartFinish = FALSE;
						    uwLlcStep1DelayCnt = 0;
					    }
				    } 
				    else if(uwLlcStep1DelayCnt > 5)
				    {
            ...
            if(LLC_SOFTSTART1 == g_DcDcVar.uwLlcSoftStartStep)
            {
                ...
                {
                    if(g_ComInfo.Com1.bit.LLCSoftFast && !uwSoftFastAdjusted)
                    {
	                    if(stValue.fAveReal.VBus > (stValue.fAveReal.VBat + (30 * fLlcSoftScale)))
	                    {
	                       fDeltaRatio = (stValue.fAveReal.VBus - stValue.fAveReal.VBat) / 30.0f;
	                       uwScaledInt = (Uint16)(fDeltaRatio * 2.0f) + 1;        
						   fLlcSoftScale = (float)(uwScaledInt) * 0.5f;
						   
						   if(fLlcSoftScale < 1.0f)
						   {
						       fLlcSoftScale = 1.0f;
						   }
	                    }
						uwSoftFastAdjusted = TRUE;
                    }

                    if(bMachineModel == eModel_IVAM6048 || bMachineModel == eModel_Unknown)
                    {
                    	uwLlcPwmSoftStep1Time = cDelay200ms_20ms;
                    }
					else if(bMachineModel == eModel_IVAM8048)
					{
                    	if(g_DcDcVar.uwLlcPwmValue < cLLCDuty002)  uwLlcPwmSoftStep1Time = cDelay400ms_20ms;
						else									   uwLlcPwmSoftStep1Time = cDelay200ms_20ms;					    
					}
					
                    if(++uwLlcPwmSoftStartCnt >= uwLlcPwmSoftStep1Time) // 2s cycle increase 1, about 6.3s up to 62(3%)
                    {
                        uwLlcPwmSoftStartCnt = 0;
                        if(stValue.fAveReal.VBus < (stValue.fAveReal.VBat + (30 * fLlcSoftScale)))
                        {
            ...
            else if(LLC_SOFTSTART2 == g_DcDcVar.uwLlcSoftStartStep)
            {
                if(g_ComInfo.Com1.bit.LiActLLCWork) // 电池激活限定充电电流
                {
                    ...
                    else
                    {
                        ...
                        UpDownLimit(g_DcDcVar.uwLlcPwmValue, cLLCDuty040, cLLCDuty003);
                        uwLlcPwmSoftStartCnt2 = 0;
                    }
                }
                else
