...
INT16U 	uwSinAmpLimitMax =13000;
INT16S gi_wInvSinAmpLimit =11000;	//

extern INT16U	wBatAvgVoltNew;
extern INT16U wParaMode;

extern INT16S wOPSharePower;
extern INT32S dwOPSharePowerSum;
...
void	sInverterModuleInit(void)
{
	...
	#else
	wRInverterVoltAdj = 2048;                      
	wInverterPVoltShort= cVac40v;//cVac50v;
	wInverterPCurrShort= cIac10A;
...
void sRVoltRegu(INT16U wRTrackVolt, INT16U wRKVoltrack1, INT16U wRKVoltrack2)
{
	...
	temp=(INT16S)wRKVoltrack1*wInvVoltError-(INT16S)wRKVoltrack2*wInvVoltError0;
	
	wRNewSinAmpTemp=wRNewSinAmpTemp+temp;

	wInvVoltError0 = wInvVoltError;

	if(wOPSharePowerCnt > 0 && (swGetEepromOutputMode() != cOP_Single))
	{
		OS_ENTER_CRITICAL();
		wOPSharePower = dwOPSharePowerSum / wOPSharePowerCnt /320;
		OS_EXIT_CRITICAL();
	}
	else
	{
...
//	if(wOPSharePower > 1000)
//	{
//		wOPSharePower = 1000;
//	}
//	else if(wOPSharePower < -1000)
//	{
//		wOPSharePower = -1000;
//	}
//
//	wInvVoltErrorSum += wOPSharePower;

	if(wParaMode == cSlaveMode)
	{
		if(wOPSharePower > 160)
		{
			wOPSharePower = 160;
		}
		else if(wOPSharePower < -160)
		{
			wOPSharePower = -160;
		}
	}
	else
	{
		...
		else if(wOPSharePower < -80)
		{
			wOPSharePower = -80;
		}		
	}

	temp = wOPSharePower >> 2; 

	wRNewSinAmpTemp=wRNewSinAmpTemp+temp;
	
//	sRAmpLimit(13000,1);		//250*32*1.414=11312,	13000= 287V*32*1.414
//	sRAmpLimit(6787,1);			//6787= 150V*32*1.414
	sRAmpLimit(uwSinAmpLimitMax,1);
...
//	wNewStepResidueTemp=((INT32U)wInverterPeriodCntNew*factor)-((INT32U)wInverterStepTemp*pointer);
	...
	wPointerMaxTemp = (INT16U)(((INT32U)((INT32U)wInverterPeriodCntNew * factor * 10 / cPWMCntlPeriod) + 5) /10);
...
INT8U	sbRInverterVoltLowChk(INT8U bFilter)
{
	static INT8U	bRInterterVoltLowChkCnt = 0;
	 ...
	 if(sbGetInvVoltSoftFinishSts()==true && bPVMode == cBatteryMode \
	 	&& bLowPowerflag == false &&bBatToLineModeflag == false)	  
	{
...
INT8U	sRInverterNegPowChk(INT16S wActivePow,INT16S limit1,INT16S limit2,INT8U bFilter)
{
	...
	if(hoOPRLY != true || (swGetEepromOutputMode() == cOP_Single)|| bPVMode != cBatteryMode)
	{
		bRNegPowChkCnt = 0;
		return(false);
	}
	
	if((wActivePow<limit1)&&(!fInv.RShort))
	{
		bRNegPowChkCnt=0;
		return(true);
	}
	
	if((wActivePow<limit2)&&(!fInv.RShort))
	{
		bRNegPowChkCnt++;
