...
INT16U uwParaOverCurrProtectCNT = 0;
INT16S s_wCnt;
INT16S gi_wBatVolt8CAvg; 
INT32S s_dwBusSum;
INT16S gi_wBus8CAvg =0;
INT16U wRCountsHalfPeriodTest =192;
INT16U HoParaTxCnt =0;

INT16S gi_wPLLPointer = 0;
INT16S gi_wPLLCtrlPointer = 0;//根据相位每个继电器断开时间点不同
...
INT16U ProtectFlag2 = 0;////原边硬件过流保护将驱动限小
INT16U ProtectFlag1 = 0;//并机过流保护将驱动限小

INT16S uwInvVoltLimtHight;
extern INT16S HardProtectOffSetCnt;
INT16S HardProtectOffSetCntTest=40;
extern INT16S wHardProtectOffAdjCnt;
...
/************************************************************************************
Name:HardOverCurrInterrupt
Input vars:void
Output vars:void
Internal vars:
Funciont:硬件过流保护信号使用捕获口进行触发
************************************************************************************/
...
interrupt void SCIBReceiveInterrupt(void)//485、232
{
	sSciRxISR(1);
	// To recieve more interrupts from this PIE group, acknowledge this interrupt 
	PieCtrlRegs.PIEACK.all = PIEACK_GROUP9;
...
INT32S NTotalPwm;
INT32S PTotalPwm;
INT32S NTotalPwmInterFace;
INT32S PTotalPwmInterFace;
extern INT16S gi_wPwmOffSetCnt;
INT32S TotalPwmPre =0;
INT32S TotalPwm=0;
INT32S dwInvVolt2SumTempTestIntface;
INT32S dwInvVolt2SumTempTest;
INT32S dwInvVolt2SumTempTotal;
INT32S dwInvVolt2SumTempTotalre=0;
INT16S gi_InvVolt2SumOffSetCnt =0;
INT16S TouchCnt;
extern INT8U g_SCITouchFlag;

extern INT16U uwP3PhaseLineNLossFlag;//

extern void mLineLedOff();
extern void mLineLedOn();
...
{	
	...
	wOPCurrReal = (INT16S)(((INT32S)wOPCurrSample * wOPCurrRatio)>>11);//0.1A//wOPCurrRatio = 660
	...
	if((swBatVoltSolpeSum > cBatVoltSolpeSumHighLever) && (wBatVoltReal  > g_wBatVoltSlopeCal))//б�ʱ���
	{
	...
	if(wBatVoltReal > wBatAvgVoltNew+cBat1v5)//ѹ���
	{
...
//变压器NTC温度采样累计值
	...
	dwInvVolt2SumTempTest += (INT32S)wInvVoltReal;
	...
	if(++gi_wPLLPointer >= gi_wPLLPointerMax)
	{
		...
		dwInvVolt2SumTempTestIntface =dwInvVolt2SumTempTest;
		dwInvVolt2SumTempTotal =dwInvVolt2SumTempTest -10400;
		
		dwInvVolt2SumTempTotalre =(dwInvVolt2SumTempTotalre*7+dwInvVolt2SumTempTotal)>>3;
		gi_InvVolt2SumOffSetCnt=(INT16S)(dwInvVolt2SumTempTotalre>>5);
		if(gi_InvVolt2SumOffSetCnt>90) gi_InvVolt2SumOffSetCnt =90;
		if(gi_InvVolt2SumOffSetCnt<-90) gi_InvVolt2SumOffSetCnt =-90;
		
		dwInvVolt2SumTempTest =0;
		
		NTotalPwmInterFace =NTotalPwm;
 		PTotalPwmInterFace =PTotalPwm;
		TotalPwm=NTotalPwm+PTotalPwm;
		TotalPwmPre =(TotalPwmPre*3+TotalPwm)>>2;
		gi_wPwmOffSetCnt=(INT16S)(TotalPwmPre>>6);
		if(gi_wPwmOffSetCnt>80) gi_wPwmOffSetCnt =80;
		if(gi_wPwmOffSetCnt<-80) gi_wPwmOffSetCnt =-80;
		NTotalPwm =0;
		PTotalPwm =0;
	}
...
//参与计算不同相位的断开点
	...
	else if(mEepromOutputMode() == cOP_2P2)
	{
		...
		//wCtrlSinpointer = wRSinPointer + wRCountsHalfPeriodTest;
	}
	else
	{
...
//	}
	...
	wTestModeSample  = 4095;
		...
		wBusVoltSample   = 0;            //1V = 7.6561CNT
		wTxtTempSample   = ADCRESULT14;//变压器NTC
		wInvTempSample   = ADCRESULT15;//散热器NTC
		wTestModeSample  = ADCRESULT10;
		//wL1InvVoltSample1  = ADCRESULT4-2048;	         //1V = 4.3977CNT
		//wL2OPCurrSample1 = ADCRESULT2-2048-wInvCurrSampleBias;	  //1A = 12.784CNT
	...
	{
//硬件过流保护后，将驱动减小，经过wInvOffPoint时间后，再次检测
		if(LiHardProtectCNT>=3)//硬件过流
		{
			if((wInvOffPoint == 0)&&(wContinueTrig == 0))
			{

				if(wCtrlSinpointer<gi_wPLLPointThreeSix)//正半?�出现�???
				{
					HardProtectOffSetCnt =HardProtectOffSetCntTest;
				}
				else if(wCtrlSinpointer>gi_wPLLPointThreeSix)
				{
					HardProtectOffSetCnt =- HardProtectOffSetCntTest;
				}

				wHardProtectOffAdjCnt = 5;
				wHardProtectDelreatOutVolt =2;
				
				wInvOffPoint =100;
				wContinueTrig =1;
				ubHardProtectFlag =true;
				if(ubHardProtectCnt >= 300)//电池模式下连续触发300次进行报故障    
				{
					ubHardProtectCnt = 0;
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>20))//wContinueTrigCNT 20次累加的不触发过流保护，再次触发过流保护，起到延长保护时间作用
 			{
				wContinueTrigCNT =30; 
				wContinueTrig =0;
			}
...
//			EDIS;
			...
			if((wCtrlSinpointer == 0)||(wCtrlSinpointer == gi_wPLLPointThreeSix)\
				||(wCtrlSinpointer == (gi_wPLLPointThreeSix+1)))//硬件触发过流后保护到过零点后的10个点
			{
...
//副边过流保护倒计时
		if(wInvOverCurrenProtectCnt>0)
		{
			...
			if(((wCtrlSinpointer == 0)||(wCtrlSinpointer == gi_wPLLPointThreeSix)\
				||(wCtrlSinpointer == (gi_wPLLPointThreeSix+1)))&&(mEepromOutputMode() > cOP_Single))//并机模式关断到过零点后10个点
			{
				if(wInvOverCurrenProtectCnt > 10)
				{
		...
		if(uwParaOverCurrProtectCNT >= 3)//并机过流信号
		{
			ProtectFlag1 = 1;
		}
		else if(uwParaOverCurrProtectCNT == 0)
		{
			ProtectFlag1 = 0;
		}
	}
	...
	if((bPVMode == cBatteryMode )||((bPVMode == cStandbyMode )&&(wFBControlStatus == cFBInverterControl)))
	{
		...
		if (abs(wInvLCurrSample1) > wInvOverCurrentLimit*g_wInvOverCurrentFactor)//55*31 副边软件过流保护
		{
			//mDisInvPwmOut();		//20210907 Test
			wInvOverCurrenProtectCnt =100;//10;
			if(wInvOverCurrentLimit != g_wInvOverCurrRated/2)//40->g_wInvOverCurrRated/2
			{
				if(++wInvOverCurrentCnt >= 50)   //过流时间50个中断周期：50/2*52us=1.3ms
				{
					mLineLedOn();
	...
	else if(bPVMode == cLineMode )
	{
		...
		if(wBatVoltReal >(mEEBatVoltOverShut()-swGetBatteryPcs()*cBat0v5))//当电池电压>(最大设定电压-电池结束*0.5A)过压保护
		{
			ubBatVoltOveCnt ++;
			if(wChgPwmLimit > 5)
			{
	...
	if(((wInvOffPoint > 0) || (wInvOverCurrenProtectCnt > 0)) && (HoParaTx == 1) && (mEepromOutputMode() > cOP_Single))
	{//原边过流保护信号||副边过流保护信号||并机过流信号拉低;并机通讯板发送和接收信号都会拉低
		HoParaTxOn;//硬件过流保护;信号拉低
	}
	else if((wInvOffPoint == 0) && (wInvOverCurrenProtectCnt == 0))//原边、副边不过流拉高
	{//单台机器的拉高信号，(并机通讯板发送影响接收，接收不影响发送)当还有其中一台拉低信号发送，接收到信号都为拉低；当三台并机均拉高时候接收到的信号为高电平
		if(HoParaTx==0)
		{
			HoparaTxOff;
		}
	...
	//ma9x InvCurr 15A  1A = 25.5680CNT
	HardProtectChk();
...
//		}
	...
	#else
	sRLinePeakVoltChk(gi_wRLineVoltRealQuadratic,cLine55VLowLoss,cLine165VHighLoss,20,120);//
	#endif
...
//	static INT16U wLowPowerCnt1 = 0;
	...
	if(bPVMode == cBatteryMode )
	{
		if(LiHardProtectCNT>=3)//硬件过流保护
		{
			if((wInvOffPoint == 0)&&(wContinueTrig==0))
			{
				if(wCtrlSinpointer<gi_wPLLPointThreeSix)
				{
					HardProtectOffSetCnt =HardProtectOffSetCntTest;
				}
				else if(wCtrlSinpointer>gi_wPLLPointThreeSix)
...
//				EDIS;
	...
	if((bPVMode == cBatteryMode )||((bPVMode == cStandbyMode )&&(wFBControlStatus == cFBInverterControl)))//副边过流保护信号
	{
		if (abs(wInvLCurrSample2) > wInvOverCurrentLimit*g_wInvOverCurrentFactor)//55*31     George 141219 25->44 for 3KVA  44->g_wInvOverCurrentFactor
		{
//			mDisInvPwmOut();
			wInvOverCurrenProtectCnt =100;//10;
			if(wInvOverCurrentLimit != g_wInvOverCurrRated/2)//40->g_wInvOverCurrRated/2
			{
				if(++wInvOverCurrentCnt >= 50)//5)   //过流时间50个中断周期：50/2*52us=1.3ms
				{
					mLineLedOn();
					wInvOverCurrentCnt = 0;
					g_uwInvOverCurCnt++;
	...
	if((bPVMode == cBatteryMode)||((bPVMode == cStandbyMode )&&(wFBControlStatus == cFBInverterControl)))
	{
		 if(abs(wInvVoltReal)  >= uwInvVoltLimtHight)  //if(abs(wInvVoltReal)  >= cVac198v) //140*1.414 =198V      //280*1.414=395.92v     cVac406v   Rms 287V   //     cVac380v
		{
...
//		if((abs(wInvVoltReal) >=  gi_wInvOverValue)  && (bBatToLineModeflag == false) && mEepromOutputMode() == cOP_Single)//254.6*1.414=360v    360/8=45V   //amy20121214     
		if((abs(wInvVoltReal) >=  gi_wInvOverValue)  && (bBatToLineModeflag == false))//240*1.414=339V	 360/8=45V	 //amy20121214	   
		{
			if(wRNewSinAmp >= gi_wInvSinAmpLimit)
			{
			 	wRSinAmp -= 30;
				wRNewSinAmp -= 30; 
//				wInvVoltAdjCnt++;
...
//				{
//					wInvVoltAdjCnt = 0;
//					wRSinAmp = gi_wInvSinAmpLimit-500;//10407;
//					wRNewSinAmp = gi_wInvSinAmpLimit-500;//10407;
//				}
			}
		}
//		else
...
void sLineWaveChk(INT16U wCmpPoint)
{
	...
	else 
	{
		...
		if(((wLineVoltReal > wLineWaveVoltHighLoss) || (wLineVoltReal < wLineWaveVoltLowLoss))
		      &&(abs(wOPCurrReal) < 680) && s_ubCTSaturationChkCnt < 15)    // (5000/140*1.414=50.5)*2.5=126	避免瞬投RCD载，误报波形
		{
			wLineLossTemp = true;
		}
		else
	...
	if((wLineWaveLossFlag == true) || (wLineWaveLossFlag1 == true)||(uwP3PhaseLineNLossFlag == true))
	{
...
void  swGetLineQuadraticSum(INT16S wRLineVoltRealTemp )
{
	...
	//wRLineVoltRealTemp=wRLineVoltRealTemp+50;//5V?�压?��?
	if(++wSinPointerCnt >= 2)	//
	{
		wSinPointerCnt=0;
...
void HardProtectChk(void)
{
	...
	if((liparaRx==0)&&(mEepromOutputMode() > cOP_Single))//并机硬件过流保护信号
	{
		if(++uwParaOverCurrProtectCNT>4)
		{
			uwParaOverCurrProtectCNT =4;
		}
	}
	else
	{
		if(uwParaOverCurrProtectCNT > 0)
		{
			uwParaOverCurrProtectCNT--;
		}
	}
