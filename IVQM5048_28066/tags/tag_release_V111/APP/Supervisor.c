...
INT16S   wOpShareCurrSampleBias = 0;
...
INT16S g_wInvCurrLimitRated3Div4 ;
INT16S g_wToBatteryFlag =1;	
INT16S wLoadPF =100;
INT16S wLoadPFpre =100;
INT16U uwFaultToStandbyCnt =0;
INT16U g_wHWOverCurrFaultCnt =0;
INT16U bBatToLineModeflag =false;
...
INT16U	g_uwStartupWithoutBattery = false;//Ê≤°ÊúâÁî®Âà∞
...
extern INT16U uwLineToBatModeCnt;//3/6ÂàÜÈíüÂÜÖÂàáÊç¢Ê¨°Êï∞
extern INT8U  bLineToBatModeFlag;
void sModeSwitchTask(void)//20ms
{
	TASK_EVENT	event;
	...
	while(1)
	{
		...
		if(event&(1<<eModeSwitchTimer))
		{
				...
				|| bPVMode==cLineMode || bPVMode==cBypassMode) 	
			{
				...
				&& (((strParaExistInfo.BIT.uw3PExistOK & 0x30) == 0x30)||((strParaExistInfo.BIT.uw3PExistOK & 0x0E) == 0x0E))))
				{
				...
				if(bPVMode != bSwitchWorkMode)
				{
					...
					if(bSwitchWorkMode == cStandbyMode)
					{
						...
						&& (bPVMode == cBatteryMode || bPVMode == cBypassMode))
						{
					...
					else
					{
						...
						if((wModeSwitchToLineCnt+30) > wModeSwitchToLineDelay)
						{
							bBatToLineModeflag =true;
						}
			...
			if(bPVMode==cLineMode || bPVMode==cBypassMode)
			{
				if(swGetEEpromVer() == cEEPROM_VER_2)
				{
					if(swGetEEPowerKeyMode() == cPKM_OP_OFF)
					{
						...
						{
							g_uwOPRlyWaitOnFlg = true;//OSEventSend(cPrioSuper, eSuperOPRlyOn);
						}
					}
					else
					{
			...
			else if(bPVMode==cBatteryMode)
			{
				if(sbGetLoadOnCmd())
				{
					if(mOPRLYChk() == cOPRLYOff && mInvSoftOKFlg == true && sbGetInverterPLStatus() == true)
					{
						g_uwOPRlyWaitOnFlg = true;//OSEventSend(cPrioSuper, eSuperOPRlyOn);
					}
				}
...
void	sPowerOnMode(void)
{
	...
	while(1)
	{
		...
		if(event&(1<<eSuperTimer))
		{
			...
			wSuperTemp = sbWaitSystemSteady(3);//Á≠âÂæÖ3SÂà§Êñ≠ÊúâÊó†ÊïÖÈöú; 
			if(wSuperTemp == false)
			{
				swFaultInfoCollect();//Êî∂ÈõÜÊïÖÈöú‰ø°ÊÅØ
				sOffPWM();
		    	bPVMode=cFaultMode;
				return;
			}
			
			wSuperTemp = swCTBiasChk();//ÈááÊ†∑ÂÅèÁΩÆ
			if(wSuperTemp == false)
			{
				swFaultInfoCollect();
				sOffPWM();
...
//	hoSFTNRLYOff;
	hoOPRLYOff;
	AvrAllOff;
	if(swGetEepromOutputMode() != cOP_Single && hoSFTRLY)
	{
		hoSFTRLYOff;
		hoSFTSCROn;
...
void	sBatteryMode(void)
{
	...
	g_wInvCurrLimitRated3Div4 =g_wInvCurrLimitRated*8/10;
	g_wToBatteryFlag =1;	
	...
	while(1)
	{
		...
		if(event&(1<<eSuperOPRlyOn))
		{
			if(swGetEepromOutputMode() == cOP_Single)
			{
				...
				while(OSMaskEventPend(0x7FFF) == 0);//ÂΩìËææÂà∞ÁªßÁîµÂô®Èó≠Âêà‰ªªÂä°ÁÇπÊó∂ÔºåË∑≥Âá∫Âæ™ÁéØ
				sOSTimerStart();
				hoOPRLYOn;
			}
			else if(mOPRLYChk() == cOPRLYOff && ((mInvSoftOKFlg == true && sbGetInverterPLStatus() == true)))
			{
		...
		if(event&(1<<eSuperToLine))
		{
			...
			if((g_wVAType == cb3500VAModel)||(swGetEepromOutputMode() == cOP_Single))
			{
				...
				while(OSMaskEventPend(0x7FFF) == 0);
				hoSFTRLYOn;
				sSetRlyPointer(RLY_DELAY,cSFTRlyDelayTime+10,eSuperSelectPoint,cPrioSuper);// Ëß£ÂÜ≥ÂúÜÁéØÂèòÂéãÂô®ÂÜ≤ÂáªÁîµÊµ?
				while(OSMaskEventPend(0x7FFF) == 0);
...
//			if( wInvOverCurrentLimit != g_wInvOverCurrRated)
//			{
//				OS_ENTER_CRITICAL();				
...
//			{
//				wLoadPF=(INT16S)((INT32S)swGetLoadPowerWatt()*100/swGetLoadPowerVA());
//				wLoadPFpre=(wLoadPFpre*3+wLoadPF)>>2;
//			}
//			else
//			{
//				wLoadPF =100;
...
//			
//			{	
//				if((swGetLoadPowerPercent()>=60)&&(wLoadPFpre>50))
//				{
//					if(wInvCrrentLimit < g_wInvCurrLimitRated)
//					{
//						wInvCrrentLimit ++;
//					}
//						
//				}
//				else if((swGetLoadPowerPercent()<40)||(wLoadPFpre<40))
//				{
//					if(wInvCrrentLimit > g_wInvCurrLimitRated3Div4)
//					{
//						wInvCrrentLimit --;
//					}
//				}													
//				if(mEepromOutputMode() == cOP_2P2)// ÁªÑÂèåÁÅ´Á∫øÈúÄË¶ÅÊîπ‰∏§‰∏™‰∏çÂêåÁöÑÈôêÊµÅÂÄ?
//				{
...
//				}
//				if(g_wToBatteryFlag ==1)
//				{
//					g_wToBatteryFlag =0;
//					wInvCrrentLimit = g_wInvCurrLimitRated3Div4;
//				}
//			}
//			#else
			...
			{
				OS_ENTER_CRITICAL();
				wInvCrrentLimit = g_wInvCurrLimitRated;//cInvCurrLimit;5048=80
				wInvOverCurrentLimit = g_wInvOverCurrRated;//cInvOverCurr;5048=88
				OS_EXIT_CRITICAL();

				if(mEepromOutputMode() == cOP_2P2)//
				{
					wInvCrrentLimit = g_wInvCurrLimitRated+3;
				}
				else if(mEepromOutputMode() == cOP_2P1)
...
//	HoparaTxOff;
	while(1)
	{
		...
		if(event&(1<<eSuperOPRlyOn))
		{
			...
			{
				if((g_wVAType == cb3500VAModel)||(swGetEepromOutputMode() == cOP_Single))// 3.5K Ê≤°ÊúâSCR
				{					
					sOSTimerStop();
					sSetRlyPointer(RLY_CROSSZERO,cOPRlySpeedUpOnDelayTime,eSuperSelectPoint,cPrioSuper);
					while(OSMaskEventPend(0x7FFF) == 0);
...
//			#endif
		...
		if(event&(1<<eSuperToBat))//Â∏ÇÁîµÂàáÁîµÊ±†
		{
			if(bLineToBatModeFlag == false)
			{
				bLineToBatModeFlag = true;
				uwLineToBatModeCnt++;
			}
			...
			if(fLoad.LoadOnCmd == true)
			{
				...
				AvrAllOff;
				sOSTimerStop();
				if(swGetEepromOutputMode() != cOP_Single)//ÈùûÂçïÊú∫Ê®°Âºè‰∏ãÂàáÊç¢
				{//ÂÖàÂÖ≥LÁªßÁîµÂô®ÂºÄSCRÔºåÁî®SCRÊâõÂÖ≥Êñ≠Êó∂ÂÄôÁöÑÁîµÊµÅÔºåÂÜçÂÖ≥NÁªßÁîµÂô®
//					hoSFTRLYOff;
//					hoSFTSCROn;
//					sSetRlyPointer(RLY_DELAY,cDelay10ms,eSuperSelectPoint,cPrioSuper);
//					while(OSMaskEventPend(0x7FFF) == 0);
...
//					if(wParaMode ==cMasterMode)
//					{
//						sSetRlyPointer(RLY_DELAY,5,eSuperSelectPoint,cPrioSuper);// delau 0.5ms
//						while(OSMaskEventPend(0x7FFF) == 0);
//					}
					sSetRlyPointer(RLY_ZEROPOINT,0,eSuperSelectPoint,cPrioSuper);					
					while(OSMaskEventPend(0x7FFF) == 0);
				...
				else//ÂçïÊú∫Ê®°ÂºèÂàáÊç¢
				{
					hoSFTRLYOff;
					hoSFTNRLYOff;//ÂÖ≥Êñ≠Â∏ÇÁîµ
					{
						sSetRlyPointer(RLY_DELAY,cDelay3ms,eSuperSelectPoint,cPrioSuper);//ÈÄâÊã©ÁªßÁîµÂô®ÂàáÊç¢ÁÇπ
						while(OSMaskEventPend(0x7FFF) == 0);
						
						if(gi_wPLLCtrlPointer > gi_wPLLPointThreeSix)// ÈÄâÊã©(1/2)Âë®ÊúüÈ°∂ÁÇπ
						{
							sSetRlyPointer(RLY_POINT,gi_wPLLPoint1Div4,eSuperSelectPoint,cPrioSuper);
							while(OSMaskEventPend(0x7FFF) == 0);
						}
						...
						if((true == sbGetRLineFreqLossStatus()) && (swGetLineFreqNew() > cFreq63Hz)&& (swGetRLineVoltNew() > cVac140v))//lory 160105
						{
	  ...
	  {				
			...
			if((fBMSForceChgFlg ==0)&&((swGetSccOkFlag() == true && sbGetEepromChargerPriority() == cChargeSolarFrist)
			...
			if(!uwACChgerOn || sbGetRLineLossStatus() == true)
			{
				g_uwAPL10SChgStartCnt = 0;
				wAPL10SChgStartFlag = false;		
...
//	hoSFTNRLYOff;
	...
	else if(swGetFaultCode() == cOverCurHardProtFault)
	{
	...
	while(1)
	{
		...
		if(event&(1<<eSuperTimer))
		{
			...
			else if(wFaultCodeTemp == cRInvOverCurrent)
			{
				g_uwOvldRstToLineDelay = 0;
			...
			else if(wFaultCodeTemp == cOverCurHardProtFault)
			{
...
INT16U swCTBiasChk(void)
{
	...
	OS_ENTER_CRITICAL();
	wTemp1 = wOpCurrSampleBias;//ËæìÂá∫ÈááÊ†∑ÁîµÊµÅÂÅèÁΩÆ
	wTemp3 = wLowOpCurrSampleBias;
	wTemp2 = wInvCurrSampleBias;//ÂÆûÊó∂ÈÄÜÂèòLÁîµÊµÅ
	wTemp4 = wOpShareCurrSampleBias;//ÂùáÊµÅÈááÊ†∑
	wOpCurrSampleBias = 0;
	wInvCurrSampleBias = 0;
	wLowOpCurrSampleBias =0;
	wOpShareCurrSampleBias = 0;
	...
	while(1)
	{
		...
		if(event&(1<<eSuperTimer))              
		{
			...
			if(wTempCnt ++ > wSteadyTime)
			{
				...
				|| abs(swGetInvCurrSampleBiasTemp()) > 100
				|| abs(swGetOpShareCurrSampleBiasTemp()) > 100)			//1A = 24cnt     120
				{
				...
				else
				{
					...
					wOpShareCurrSampleBias = swGetOpShareCurrSampleBiasTemp();
					OS_EXIT_CRITICAL();
					return(true);
				}
			}
	...
	while(1)
	{
		...
		if(event&(1<<eSuperTimer))   //20ms
	    {
	    	if((sbGetInvVoltSoftFinishSts() != true)&&(++wDelayToLineCnt<1000)&&(InvVoltRelayAction ==0)\
				&&(wBatAvgVoltNew >=swGetBatUnderVolt())&&(InvVoltSoftFalseFlag == 0))//20S‰ª•ÂêéÊú™ËΩØËµ∑ÊàêÂäü‰πü‰ºöÂàáÊç¢Â∏ÇÁî?
			{
				b2Cnt = 0;
	...
	while(1)
	{
		...
		if(event & (1<<eSuperTimer))	//20ms
		{
			if(swGetEepromOutputMode() == cOP_Single)
			{
				...
				if(swGetRInverterVolt() < cVac100v)
				{
					if(++bCnt >= 5)
					{
						bCnt = 0;
...
INT8U  sbTestModeChk(INT8U b20ms)
{
	...
	while(bTemp)
	{
		...
		if(event & (1 << eSuperTimer))
		{
			// ÊµãËØïÊ®°ÂºèÈúÄÊé•ÂÖ•75Œ©ÁîµÈòª,ÊâÄ‰ª•ÈááÊ†∑ÂÄº‰∏∫75/(5100+75)=59(-1%=-40,+0.5%=20)
...
void swFaultInfoCollect(void)
{
	...
	&& ((sdwGetWarningCode() & cBatOpen) != cBatOpen))
	{
		...
		if(mOPRLYChk()==cOPRLYOn)//ËæìÂá∫ÁªßÁîµÂô®
		{
...
INT16S swWorkModeChk(void)
{
	...
	if(swGetEepromOutputMode() > cOP_Parallel)//‰∏âÁõ∏
	{
	...
	else
	{
		...
		{
			// ÁîµÊ±†ÁÆ°ÁêÜ‰ºòÂåñ,ÂàáÊç¢Ëá≥ÁîµÊ±†‰ºòÂÖàÊ®°Âºè
			// EEPROMËæìÂá∫‰ºòÂÖàÊ®°Âºè‰∏çÂÅö‰øÆÊîπ,Èò≤Ê≠¢Êú™‰øùÂ≠òÊñ≠ÁîµÂØºËá¥Ê®°ÂºèËÆæÁΩÆÊîπÂèò
			if(swGetBattMgtFlag())
			{
...
void sWorkModeInfoUpdate(void)
{
	...
	{
		g_uniuwOSFSBUMainRlyOnInfo.BIT.uwBatWeak = true;
	}
	else
	{
		g_uniuwOSFSBUMainRlyOnInfo.BIT.uwBatWeak = false;
	}
	...
	g_uniuwOSFSBUMainRlyOnInfo.BIT.uwSCCOK = g_uwSysSCCOK;//swGetSccOkFlag();
...
INT8U sbGetFaultToStandbyCnt(void)
{
	return(uwFaultToStandbyCnt);
}

void sSetFaultToStandbyCnt(INT8U bValue)
{
	
	uwFaultToStandbyCnt = bValue;
