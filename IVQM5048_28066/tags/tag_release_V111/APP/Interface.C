...
INT16U g_uwDischgCurr = 0;			// ƒÊ±‰∆˜µÁ≥ÿ∑≈µÁµÁ¡˜
INT16S wBat1Ref;
INT16S wBat2Ref;
INT16S wBat1Real;
INT16S wBat2Real;
INT16U wSCCReflash = false;			// SCCπÃº˛…˝º∂◊¥Ã¨±Í÷æ
INT16U wSCCReflashFinish = false;	// SCCπÃº˛…˝º∂ÕÍ≥…±Í÷æ
...
INT16U wBatVerifyFlg = false;	// =1µÁ≥ÿµÁ—π–£◊º ±,PWMøÿ÷∆∆˜Õ£÷π≥‰µÁ
...
INT8U g_bDischgVoltSet = cBat52v;	// 11.5-14.5V/pcsø……Ë÷√
INT8U g_bDischgCurrSet = cIdc50A;	// 0-100Aø……Ë÷√
...
const INT8U cbCommLenTbl[] =
{
	...
	8,				//sSCCReflashCommand		"\r"
	8,				//sSCCReflashCommand		"\r" 
	8,              //sBOOTLOADIAPCommand 		"IVPM10K\r",
	5,				//sTESTCommand				"\r"		 
	6,				//sCHECKCommand				"\r"		
	2,				//sQ3Command				"",
	6,				//sQSCIRCommand				"\r"
	...
	6,				//sQPIWSCommand 	"\r",
	8,				//sQMCHGCRCommand	"\r",
	9,				//sQMUCHGCRCommand	"\r",
	6,				//sQPIHFCommand 	"\r",
	6,				//sQPICFCommand 	"\r",
	10,				//sMNCHGCCommand	 "*\r",//*4  
	10,				//sMUCHGCCommand	"*\r",
	4,				//sF50Command		"\r",
	...
	4,				//sF60Command		"\r",		--60
	5,				//sVCommand			"Vr",//*3
	8,				//sBATLOWCommand	"*\r",//Reserved
	7,				//sBATUNCommand		"*\r",//Reserved
	9,				//sPBCVCommand		"*\r",//*4
	9,				//sPCVVCommand		"*\r",//*4
	5,				//sQCVVCommand		"\r",
	9,				//sPBFTCommand		"*\r",//*4
	...
	11,				//sPUPSTYPECommand	"*\r",//*2
	9,				//sQUPSTYPECommand	"\r",  
	7,        		//sPLCDVCommand    	"*\r",
	6,        		//sQLCDVCommand    	"\r",
	8,				//sPBVTUCommand		"*\r",//*2
	6,				//sQBVTUCommand		"\r",
	...
	7,				//sSetEqEnaCommand		"*\r",
	9,				//sSetEqTimeCommand		"*\r",
	9,				//sSetEqPeriodCommand	"*\r",
	11,				//sSetEqVoltCommand		"*\r",
	10,				//sSetEqOutTimeCommand	"*\r",
	6,				//sQBEqICommand			"\r",
	7,				//sSetEqActiveCommand	"*\r",
	10,				//sSetELTimeCommand		"*\",

	7,				//sSetSearchEnaCommand	"*\r",//"ENF2B*\r",
	6,				//sQENF2Command			"\r",//"QENF2\r",
	115,			//sQ1CommandAck			"*\r",	
	59,				//sQCHGSSCommandAck 	"*\r",
	5,				//sChgCntlSwitch		CHG1
	2,				//sQDCommand			"QD", 
...
const INT8U cbCommTbl[cCOMM_NUM][10] =
{
	...
	"PFAULT\r",		//sQFAULTCommand	Has  CRC
	...
	"PENF2\r",		//sQENF2Command
	"(PV*\r",		//sQ1CommandAck
	"(*\r",			//sQCHGSSCommandAck
	"CHG*\r",		//sChgCntlSwitch
...
pFunc1 GetDataSubArray[100] =
{
	...
	swGetInvCurrSampleBiasTemp,	swGetOpCurrSampleBiasTemp,	swGetInvStep,			//70-72
...
INT8U ucSciIdleCnt[2] = {0,0};	// 1=50ms,∑¿÷πModBus–≠“Èµ⁄“ª÷° ˝æ›Ω” ’¥ÌŒÛµº÷¬∫Û–¯¡¨–¯Ω” ’ ß∞‹
...
void sSciTask(void)
{
	...
	sInverterModuleInit();
	sGetIDSeriesForPar();
	gi_wInvOverValue = (INT16S)((((INT32S)swGetEEOutputVolt() + cVac15v) * 181) >> 7);
	sSccInit();
	sIntSccInit();
	sExtSccInit();
	sBMSParaInit();
	...
	while(1)
	{
		...
		if(event&(1<<eInterfaceTimer))
		{
			...
			// ºÃ–¯∑¢ÀÕPWM≥‰µÁ∆˜÷ÿ∆Ù÷∏¡Ó°¢∫ÕµÁ≥ÿµÁ—π–£◊º÷∏¡Ó
			...
			if((cDisable==sbGetEepromBMSSetEn())&&((cEnable == swGetEEMpptChg()) || (true == sbGetDetectMppt())))
			{
				...
				if(++ucGetSccDataCnt >= 20)	// 20*50ms=1s
				{
					...
					{
						sSccSyncCheck();	// ºÏ≤È «∑Ò”–≤Œ ˝∏ƒ±‰–Ë“™Õ¨≤Ω
...
void sSciRxDebugCommand(INT8U sciid)
{
	...
	while(1)
	{
		...
		{
			// ≥¨π˝100ms√ª”– ˝æ›‘ÚΩ” ’«Â¡„( µº ¬˙◊„15msº¥3.5◊÷∑˚ ±º‰º¥ø…)
...
extern INT32S NTotalPwmInterFace;
extern INT32S PTotalPwmInterFace;
extern INT16S gi_wPwmOffSetCnt;
extern INT32S dwInvVolt2SumTempTestIntface;
...
extern INT16S wOpShareCurrSampleBiasTemp;
...
INT16S TestEEPROM = 0;
INT16S TestEEPROMSave1Cnt = 0;
INT16S TestEEPROMSave2Cnt = 0;
INT16S TestEEPROMFlag = 0;
void sQ1Command(INT8U sciid)
{
    INT8U	bStrLen;
	INT8U *pDataBuf;
...
//	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 
	...
	wTemp =wVmUpLimit/10;	
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(abs(wTemp),pDataBuf);	
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
		
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(abs(wTxRatio),pDataBuf);	
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
...
//	}
	...
	NTotalPwmInterFace=(-NTotalPwmInterFace)/100;
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(NTotalPwmInterFace,pDataBuf);   
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	bUserDataBuf0[sciid][bStrLen++]='P'; 
	PTotalPwmInterFace=PTotalPwmInterFace/100;
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(PTotalPwmInterFace,pDataBuf);   
	bStrLen += 4;
	
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
	
	if(gi_InvVolt2SumOffSetCnt<0)//gi_wPwmOffSetCnt
	{
		wTemp =-gi_InvVolt2SumOffSetCnt;
		bUserDataBuf0[sciid][bStrLen++]='-';
	}
	else
	{
	...
	if(dwInvVolt2SumTempTestIntface<0)
	{
		wTemp =-dwInvVolt2SumTempTestIntface/10;
		bUserDataBuf0[sciid][bStrLen++]='-';
	}
	else
	{
...
////EEPROM Save1ËÆ°Êï∞
//	bUserDataBuf0[sciid][bStrLen++]='C';//error
//	bUserDataBuf0[sciid][bStrLen++]='1';
//	bUserDataBuf0[sciid][bStrLen++]=':';
//	wTemp = (INT16S)TestEEPROMSave1Cnt;
//	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
...
////EEPROM Save2ËÆ°Êï∞
//	bUserDataBuf0[sciid][bStrLen++]='C';//error
//	bUserDataBuf0[sciid][bStrLen++]='2';
//	bUserDataBuf0[sciid][bStrLen++]=':';
//	wTemp = (INT16S)TestEEPROMSave2Cnt;
//	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
//	sbNumToXX(wTemp,pDataBuf);   
//	bStrLen += 2;
//	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 
//
////EEPROMÂ§ç‰ΩçÈóÆÈ¢ò
//	bUserDataBuf0[sciid][bStrLen++]='E';//error
//	bUserDataBuf0[sciid][bStrLen++]=':';
//	wTemp = (INT16S)TestEEPROM;
//	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
//	sbNumToXX(wTemp,pDataBuf);   
//	bStrLen += 2;
//	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 
...
//Êï£ÁÉ≠Âô®Ê∏©Â∫¶
	...
	bUserDataBuf0[sciid][bStrLen++]=':';
	wTemp = (INT16S)swGetTempDegreeInv();
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
...
//ÂèòÂéãÂô®Ê∏©Â∫¶
	...
	bUserDataBuf0[sciid][bStrLen++]='X';
	bUserDataBuf0[sciid][bStrLen++]=':';
	wTemp = (INT16S)swGetTempDegreeTxt();
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXX(wTemp,pDataBuf);   
	bStrLen += 3;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 
...
//ÊïÖÈöúÁ†Å
	...
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 
//È£éÊâáËΩ¨ÈÄü
	bUserDataBuf0[sciid][bStrLen++]='F';//Fan degree
	bUserDataBuf0[sciid][bStrLen++]=':';
...
extern INT16U wRCountsHalfPeriodTest;
...
INT16U wPLLPointSel;
INT16U wPointSetFlag;
extern INT16S OPloopFlag;
extern INT16S wRLoadCurrCntlTest;
extern INT16S g_wInvOverCurrRated;
extern INT16S HardProtectOffSetCntTest;
extern INT16S wLoadPFpre;
extern INT16S g_wHardPortOPVoltCntl;
...
void	sTestCommand(INT8U sciid)
{
	...
	{ 
		...
		else if((bCommandBuffer[sciid][3] == 'K') && (bCommandBuffer[sciid][4] == 'I'))
		{
			...
			if(wTemp >= 20 && wTemp <= 1500)
			{
				sSetRKi(wTemp);
				//TestVlotErr =wTemp; //Test g_wInvCurrLimitRated
				sACK(sciid);
			}
			else
			{
		...
		else if((bCommandBuffer[sciid][3] == 'K') && (bCommandBuffer[sciid][4] == 'L'))
		{
			wTemp =(bCommandBuffer[sciid][5] - '0') * 1000 + (bCommandBuffer[sciid][6] - '0')*100
				      + (bCommandBuffer[sciid][7] - '0') * 10 + (bCommandBuffer[sciid][8] - '0'); 
		
			if(wTemp >= 180 && wTemp <= 204)
			{
				//sSetRKi(wTemp);
				wRCountsHalfPeriodTest =wTemp;
				sACK(sciid);
			}
			else
			{
		...
		else if((bCommandBuffer[sciid][3] == 'T') && (bCommandBuffer[sciid][4] == 'P'))
		{
			wTemp =(bCommandBuffer[sciid][5] - '0') * 100 + (bCommandBuffer[sciid][6] - '0')*10
				      + (bCommandBuffer[sciid][7] - '0');
			if(wTemp >= 1 && wTemp <= 383)
			{
				//sSetRKi(wTemp);
				wPLLPointSel =wTemp; //Test OPloopFlag
				wPointSetFlag =1;
				sACK(sciid);
			}
			else
		...
		else if((bCommandBuffer[sciid][3] == 'C') && (bCommandBuffer[sciid][4] == 'F'))
		{
			wTemp =(bCommandBuffer[sciid][5] - '0')*10+(bCommandBuffer[sciid][6] - '0') ;
			if(wTemp >= 20 && wTemp <= 99)
			{
				wRLoadCurrCntlTest =wTemp; //Test 
				sACK(sciid);
			}
			else
			{
		...
		else if((bCommandBuffer[sciid][3] == 'C') && (bCommandBuffer[sciid][4] == 'L'))
		{
			...
			if(wTemp >= 20 && wTemp <= 99)
			{
				g_wInvOverCurrRated =wTemp; //Test 
				sACK(sciid);
			}
			else
			{
		...
		else if((bCommandBuffer[sciid][3] == 'O') && (bCommandBuffer[sciid][4] == 'F'))
		{
			wTemp =(bCommandBuffer[sciid][6] - '0')*10+(bCommandBuffer[sciid][7] - '0') ;
			if(wTemp >= 10 && wTemp <= 99)
			{
				if(bCommandBuffer[sciid][5] == '-')
				{
					wTemp=-wTemp;
				}
				HardProtectOffSetCntTest =wTemp; //Test 
				sACK(sciid);
			}
			else
			{
		...
		else if((bCommandBuffer[sciid][3] == 'O') && (bCommandBuffer[sciid][4] == 'P'))
		{
			wTemp =(bCommandBuffer[sciid][5] - '0') ;
			if(wTemp >= 0 && wTemp <= 2)
			{
				OPloopFlag =wTemp; //Test extern 
				sACK(sciid);
			}
			else
			{
			...
			&&(bCommandBuffer[sciid][5] == 'A') && (bCommandBuffer[sciid][6] == 'N'))
		{
		...
		else if((bCommandBuffer[sciid][3] == 'L') && (bCommandBuffer[sciid][4] == 'C')
		&& (bCommandBuffer[sciid][5] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXXX(wInvCrrentLimit,&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][5] = ' ';
			sbNumToXXXX(wLoadPFpre,&bUserDataBuf0[sciid][6]);	
			bUserDataBuf0[sciid][10] = ' ';
			sbNumToXXXX(g_wHardPortOPVoltCntl,&bUserDataBuf0[sciid][11]);	
			bUserDataBuf0[sciid][15] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],16);
		}
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'M')&& (bCommandBuffer[sciid][5] == 'S'))
		{
			...
			{
				g_ubBMSForceChgTestFlg = (bCommandBuffer[sciid][8] - '0');//Âº∫ÂÜ?
				sACK(sciid);
			}
			else if ((bCommandBuffer[sciid][6] == 'S')&&(bCommandBuffer[sciid][7] == 'C'))
			{
				g_ubBMSStopChgTestFlg=(bCommandBuffer[sciid][8] - '0');//Á¶ÅÊ≠¢ÂÖÖ
				sACK(sciid);
			}
			else if ((bCommandBuffer[sciid][6] == 'S')&&(bCommandBuffer[sciid][7] == 'D'))
			{
				g_ubBMSStopDischgTestFlg=(bCommandBuffer[sciid][8] - '0');//Á¶ÅÊ≠¢ÂÖÖ
				sACK(sciid);
			}
			else
			{
		...
		else if((bCommandBuffer[sciid][3] == 'L') && (bCommandBuffer[sciid][4] == 'E')
		&& (bCommandBuffer[sciid][5] == 'D'))
		{
			wTemp = bCommandBuffer[sciid][7]-'0';
			...
			else if(bCommandBuffer[sciid][6] == 'C')
			{
				if(wTemp == 0)
				{
					hoLEDCOff;
			...
			else if(bCommandBuffer[sciid][6] == 'D')
			{
				if(wTemp == 0)
				{
					hoLEDLINEOff;
		...
		else if((bCommandBuffer[sciid][3] == 'S') && (bCommandBuffer[sciid][4] == 'W'))
		{
			if(bCommandBuffer[sciid][5] == 'A')
			{
				wTemp =  hiSwitchESCChk() + '0';
				bUserDataBuf0[sciid][bStrLen++]='A';
				bUserDataBuf0[sciid][bStrLen++]=':';
				bUserDataBuf0[sciid][bStrLen++]=wTemp;
				bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;	
				sSciWrite(sciid,bUserDataBuf0[sciid],bStrLen);
			}
			else if(bCommandBuffer[sciid][5] == 'B')
			{
				wTemp =  hiSwitchUpChk() + '0';
				bUserDataBuf0[sciid][bStrLen++]='B';
				bUserDataBuf0[sciid][bStrLen++]=':';
				bUserDataBuf0[sciid][bStrLen++]=wTemp;
				bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;	
				sSciWrite(sciid,bUserDataBuf0[sciid],bStrLen);
			}
			else if(bCommandBuffer[sciid][5] == 'C')
			{
				wTemp =  hiSwitchDOWNChk() + '0';
				bUserDataBuf0[sciid][bStrLen++]='C';
				bUserDataBuf0[sciid][bStrLen++]=':';
				bUserDataBuf0[sciid][bStrLen++]=wTemp;
				bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;	
				sSciWrite(sciid,bUserDataBuf0[sciid],bStrLen);
			}		
			else if(bCommandBuffer[sciid][5] == 'D')
			{
				wTemp =  hiSwitchENTChk() + '0';
				bUserDataBuf0[sciid][bStrLen++]='D';
				bUserDataBuf0[sciid][bStrLen++]=':';
				bUserDataBuf0[sciid][bStrLen++]=wTemp;
				bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;	
		...
		else if((bCommandBuffer[sciid][3] == 'W') && (bCommandBuffer[sciid][4] == 'H'))
		{
			wTemp = bCommandBuffer[sciid][5] - '0';
			if(wTemp == 1)
			{
...
void sQGSCommand(INT8U sciid)		// SCC PWM
{
	...
	//wIntSccHsTemp = swASCIIToNum(4, 0, &bCommandBuffer[sciid][26]);	// Œﬁ”√
	...
	if(wBatVerifyFlg == true)	// ÁîµÊ±†ÁîµÂéãÊ†°ÂáÜÊó∂ÁΩÆ‰ΩçÊ†áÂøó,‰∏ÄÂàÜÈíüÂêéËá™Âä®Ê∏ÖÈô§
	{
	...
	// 9-13:ÁîµÊ±†ÁîµÂéã(0.1V)
	sNumToASCII(swGetBatAvgVoltNew(), 3, 1, &bUserDataBuf0[sciid][wSciLength]);
	wSciLength += 5;

	bUserDataBuf0[sciid][wSciLength++] = CHAR_SPACE;
	
	// 15-18:ÂÖÖÁîµÁîµÊµÅËÆæÁΩÆ(0.1A)(PS:ÊúÄÂ§ßÂÖÅËÆ∏50A,Êú™Ë∂ÖËøáÊó∂+ÁîµÊ±†ÊîæÁîµÁîµÊµÅ)
	...
	// 32-33:ÂÖÖÁîµÁä∂ÊÄÅ10=no charge, 11=CC, 12=CV, 13=Float
...
INT16U cal_crc_half(INT8U far *pin, INT8U len)
{
	...
	while(len--!=0) 
	{
		da=((INT8U)(crc>>8))>>4; /* ‘›¥ÊCRCµƒ∏ﬂÀƒŒª */

		crc<<=4; /* CRC”““∆4Œª£¨œ‡±”⁄»?RCµƒµÕ12Œª£©*/

		crc^=crc_ta[da^(*ptr>>4)]; /* CRCµƒ∏ﬂ4Œª∫Õ±æ◊÷Ω⁄µƒ«∞∞Î◊÷Ω⁄œ‡º”∫Û≤È±Ìº∆À„CRC£¨»ª∫Ûº”…œ…œ“ª¥ŒCRCµƒ”‡ ˝ */

		da=((INT8U)(crc>>8))>>4; /* ‘›¥ÊCRCµƒ∏ﬂ4Œª */

		crc<<=4; /* CRC”““∆4Œª£¨ œ‡µ±”⁄CRCµƒµÕ12Œª£© */

		crc^=crc_ta[da^(*ptr&0x0f)]; /* CRCµƒ∏ﬂ4Œª∫Õ±æ◊÷Ω⁄µƒ∫Û∞Î◊÷Ω⁄œ‡º”∫Û≤È±Ìº∆À„CRC£¨»ª∫Û‘Ÿº”…œ…œ“ª¥ŒCRCµƒ”‡ ˝ */
...
void sPvChgCurrRangeChk(void)
{
	...
	else
	{
		if(swGetBatteryPcs() == cbBatt48VType)
		{
			pDefaultTableTemp = sGet48V120DefaultTable2();
		}
		else if(swGetBatteryPcs() == cbBatt24VType)
		{
			pDefaultTableTemp = sGet24V120DefaultTable2();
		}
		else
		{
...
* Description	 : ÈÖçÂêà‰∏≤Âè£Âä©Êâã‰ΩøÁî®ÔºåËß¶ÂèëÂêéÊçïÊçâ‰ø°Âè∑
* Input 		 : INT8U b_FlagBitÔºöËß¶Âèë‰ø°Âè∑ÁΩÆ‰ΩçÔºå=true\false
* Output		 : None
* Date			 : 2021.02.28
* Author		 : CM
********************************************************************************/
