...
INT16U swWriteEESetting(INT16U WrAddr, INT16U WrData)
{
	...
	switch(WrAddr)
	{
		...
		case E2_OutputVolt:
			...
			#else
			if((WrData == 1200)||(WrData == 1100)||(WrData == 1270))
			{
				if(swGetEEOutputVolt() != WrData)
				{
					sSetEEOutputVolt(WrData);
	...
	switch(WrAddr)
	{
		...
		case ATE_InvVoltAdj1:
			...
			if((swGetEEInvVoltAdj() == 2048) && (wTemp1 >= -200) && (wTemp1 <= 200) \
				&& (wTemp2 >= -200) && (wTemp2 <= 200))//æ ¡å‡†å€¼ä¸ŽEEPROMè¾“å‡ºç”µåŽ‹å€¼ä¸è¶…è¿‡20V
			{
		...
		break;
		case ATE_LineVoltAdj1://è¾“å…¥æµ‹é‡ç”µåŽ‹
		...
		break;
		case ATE_LineVoltAdj2://ç›´æŽ¥è°ƒæ ¡å‡†å€¼
			if((-8 <= (INT16S)WrData) && ((INT16S)WrData <= 8))
			{
		...
		break;
		case ATE_InvVoltBias:
			if((-8 <= (INT16S)WrData) && ((INT16S)WrData <= 8))
			{
					sSetInvVoltBias(swGetInvVoltBias()+(INT16S)WrData);
					OS_ENTER_CRITICAL();
...
void sRealTimeDataUpdate(void)
{
	...
	else
	{
		if(g_uwDischgCurr>1)
		{
			...
			uRealTimeData.Stru.wBattWatt = ((INT32S)uRealTimeData.Stru.wBattVolt * g_uwDischgCurr) / 100;
		}
		else
		{
			uRealTimeData.Stru.wBattCurr = g_wSCCACChgCurr;
			uRealTimeData.Stru.wBattWatt = (INT16U)sdwGetBatWatt();
		}
	...
	uRealTimeData.Stru.wRInvWatt = sdwGetInvWatt();	// ÊÐµçµçÑ¹*Äæ±äµçÁ÷
	uRealTimeData.Stru.wRInvVA = sdwGetInvVA();		// Äæ±äµçÑ¹*Êä³öµçÁ÷
	uRealTimeData.Stru.wROpVolt = swGetRInverterVolt();	// ´úÌæ
	uRealTimeData.Stru.wROpCurr = swGetROPCurrentNew();
	uRealTimeData.Stru.wROpFreq = 0;
	uRealTimeData.Stru.wROpDVI = swGetInvDCVolt();
	uRealTimeData.Stru.wROpWatt = sdwGetOPWatt();	// ÊÐµçµçÑ¹*Êä³öµçÁ÷
	uRealTimeData.Stru.wROpVA = sdwGetOPVA();		// ÊÐµçµçÑ¹*Êä³öµçÁ÷
	uRealTimeData.Stru.wRLineVolt = swGetRLineVolt();
	uRealTimeData.Stru.wRLineCurr = swGetOPShareCurr();	// ´úÌæ
