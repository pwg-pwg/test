...
INT16S wOutputVoltBasic =cVac220v;
...
void sSetConfigMode(void)
{
	...
	bOvtmpRestartIDTemp = bOvtmpRestartID;
 	...
 	bOuputVoltID = (INT16U)(swGetEEOutputVolt()-wOutputVoltBasic+cVac5v)/100;// +5V 兼容127V
...
//							if(bOPModeIDTemp == OP_MODE_PAL_ID)
//							{
//								if(bDownFlag)
//								{
//									bOPModeIDTemp=OP_MODE_2P1_ID;
//									wAdjustValue ++;
//								}
//								else
//								{
...
void	sConfigDisplay(void)
{
  ...
  {    
    ...
    else 
    {
      ...
      else if(bMenuIndex ==  CF_OP_SOURCE_ID)//设置项18:输出源;Utility first\PV first\Battery first
      {
      ...
      else if(bMenuIndex ==  CF_MAX_CHG_CUR_ID)//设置项10:最大充电电流;
      {
	...
	else if(bMenuIndex ==  CF_AC_RANGE_ID)//设置项03:市电输入范围;APL\UPS
	{
	...
	else if(bMenuIndex ==  CF_POWER_SAVE_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_BAT_TYPE_ID)//设置项05:电池类型;USE\FLD\AGM\LIB
	{
	...
	else if(bMenuIndex ==  CF_CHG_STAGE_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_CHG_CV_TIMER_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_OVLD_RESTART_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_OVTMP_RESTART_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_OP_VOLT_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_OP_FREQ_ID)//设置项:
	{
	...
	else if(bMenuIndex == CF_MAX_SOLAR_CHG_CUR_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_MAX_AC_CHG_CUR_ID)//设置项06:最大市电充电电流;
	{
	...
	else if(bMenuIndex ==  CF_BAT_VOLT_ID)//设置项19:当设置18电池优先时,电池电压低于此设置值输出切换为市电输出
	{
	...
	else if(bMenuIndex ==  CF_BAT2_VOLT_ID)//设置项20:当设置18电池优先时,电池电压高于此设置值输出源从市电切换回电池
	{
	...
	else if(bMenuIndex ==  CF_CHG_SOURCE_ID)//设置项09:充电优先级;PV first/PV and Utility/PV Only
	{
	...
	else if(bMenuIndex ==  CF_BEEP_ONOFF_ID)//设置项15:蜂鸣器使能;
	{
	...
	else if(bMenuIndex ==  CF_BMS_SET_ID)//设置项16:BMS波特率设置;2400/9600
	{
	...
	else if(bMenuIndex ==  CF_BACKLIGNT_ONOFF_ID)//设置项17:背光灯使能;
	{
	...
	else if(bMenuIndex ==  CF_MIPOFF_ALARM_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_OVLD_Bypass_ID)//设置项:过载旁路使能;
	{
	...
	else if(bMenuIndex ==  CF_FAULT_RECORD_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_DATA_LOG_POP_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_BAT_CHG_CV_ID)//设置项07:CV电压设置
	{
	...
	else if(bMenuIndex ==  CF_BAT_CHG_FLTV_ID)//设置项08:浮充电压设置
	{
	...
	else if(bMenuIndex ==  CF_BAT_UNDERV_ID)//设置项11:电池低压关机点电压
	{
	...
	else if(bMenuIndex ==  CF_EQ_ENA_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_EQ_VOLT)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_EQ_TIME)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_EQ_OUTTIME)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_EQ_InterVal)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_EQ_Active)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_POWER_KEY_MODE_ID)//设置项37:OFF,关断开关，停止输出;ON，当有市电输入充电时，关断开关仍有输出
	{
	...
	else if(bMenuIndex ==  CF_OP_MODE_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_BM_ACTIVE_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_BM_BAT_VOLT_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_BM_INTERVAL_ID)//设置项:
	{
	...
	else if(bMenuIndex ==  CF_BM_TIMEOUT_ID)//设置项:
	{
...
void  sSaveConfiguration(void) 
{
	...
	wVlaueTemp = (INT16U)(bOuputVoltID*100+wOutputVoltBasic);
	if(wVlaueTemp ==cVac130v)
	{
		wVlaueTemp =cVac127v;
	}
    if(wVlaueTemp != swGetEEOutputVolt())
    {
        if(wVlaueTemp == 2200 || wVlaueTemp == 2300 || wVlaueTemp == 2400)
        {
            ...
            gi_wInvOverValue = (INT16S)((((INT32S)swGetEEOutputVolt() + cVac20v) * 181) >> 7);	
            bSaveFlag = 1;
        }
		else  if((wVlaueTemp == cVac120v)||(wVlaueTemp == cVac110v)||(wVlaueTemp == cVac127v))
        {
            sSetEEOutputVolt(wVlaueTemp);
            wInverterVoltHigh = swGetEEOutputVolt() + cVac20v;
            wInverterVoltLow = cVac90v;
...
INT8U sbDisplayWarning(void)
{
	...
	if(ubWarnCodeDisPlayFlag)
	{
		if(wTemp)
		{
			...
			if(wTemp & c3PhaseNLoss)
			{
				ubWarnCodeTab[ubdisWarnCnt++] = cWarn3PhaseNLossFault;
			}
		}
