...
void    sParaShareCurChk(void)
{
	...
	if (swGetFBControlStatus() == cFBInverterControl && hoOPRLY == 1 && (swGetEepromOutputMode() >= cOP_Parallel))//并机或3相
	{
		//share current >= 5A
...
//					wTemp = swGetEEOutputVolt();
//				}
				if(bBatToLineModeflag ==true)
				{
					wTemp = swGetRLineVolt();
				}
				else
				{
