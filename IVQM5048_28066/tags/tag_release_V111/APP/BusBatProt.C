...
INT16U  wPvFANPWMTemp=0;
INT16U  wPvFANPWMTempPre=0;
INT16U  wPvFANPWMTempNew=0;
INT16U wPvFANPWMTempSet =0;
extern INT16U wFanComCtrl2;
extern INT16U wFanSpeedCtrl2;
...
extern INT16U uwFaultToStandbyCnt;
...
void sBusBatProtectTask(void)
{
	...
	while(1)
	{
		...
		{	
			...
			sSetTempDegreeInv(swInvTempCal(swGetInvTempAvgSample()));//ÈÄÜÂèòÂô®Êï£ÁÉ≠Âô®NTCÊ∏©Â∫¶ÂÄº
			sSetTempDegreeBat(swBatTempCal(swGetBatTempAvgSample()));
			
			if(false == bSetForceTemp)
			{
				sSetTempDegreeTxt(swTxtTempCal(swGetTxtTempAvgSample()));//ÂèòÂéãÂô®NTCÊ∏©Â∫¶ÂÄº
			}
			...
			sTemperatureInvChk(50);//Êï£ÁÉ≠Âô®Ê∏©Â∫¶Ê£ÄÊµã
			sTemperatureBatChk(50);
			sTemperatureTxtChk(50);//ÂèòÂéãÂô®Ê∏©Â∫¶Ê£ÄÊµã
			...
			else if(bBusBatProtTimerCnt == 10)
			{
				...
				sLineVolDegJudg();
				sFanSpeedControl();//È£éÊâáÈááÁî®Áªü‰∏ÄÊéßÂà∂
//				sPVFanControl();
				sControlParameterSeting();//ÈöêËóèÁïåÈù¢ËÆæÁΩÆ
			}
			sDryRelayAction();
			sBattManagementTime();
...
/*******************************************************************************
* Function Name:sPVFanControl
* Description:MPPT partial fan control;∏˘æ›¥´ ‰≤ªÕ¨µƒµµŒª£®∞Ÿ∑÷±»£©øÿ÷∆£¨“ª¥Œ‘ˆºı1%
* Input:Null
* Output:Null
* Note date: 2022.1.6
********************************************************************************/
void sFanSpeedControl(void)	//Fan Control
{
	if(false == wSciForeceFanCtrl)
	{
		...
		{
			wFANPWMTemp = cFanClosed;//cFanLowerSpeed;
			wFANPWMTempPre = cFanClosed;//cFanLowerSpeed;
			wFANPWMTempNew = cFanClosed;//cFanLowerSpeed;
			wFANPWMTempSet = cFanClosed;//cFanLowerSpeed;
			g_wACFanOnFlag =0;
			bFan1status = true;
			bFan2status = true;
		}
		else
		{
			...
			else if(FanStatus.HeavyLoad || FanStatus.DCIHigh \
	...
	//				g_wACFanOnFlag =1;
			...
			else if(FanStatus.HalfLoad ||(bMpptHsTempInfo >= 2)\
				|| (sbGetTxTempStatus() >= 1) || (sbGetInvTempStatus() >= 1))//∞Î‘ÿ£¨MPPTµµŒª2°¢±‰—π∆˜µµŒª1°¢ƒÊ±‰∆˜µµŒª1£¨ÀŸ∂»£∫»´ÀŸ
			{
				//wFANPWMTempNew = cFanHalfSpeed;
				//wFANPWMTempNew = cFanFullSpeed;//CC20210410
				wFANPWMTempSet = cFanFullSpeed;
				bFan1status = true;
				bFan2status = true;
			}
			else if(FanStatus.FanOnLoad || (bMpptHsTempInfo >= 1) \
				|| (sbGetTxTempStatus() >= 1) || (sbGetInvTempStatus() >= 1))//–°‘ÿ£¨MPPTµµŒª1°¢±‰—π∆˜µµŒª1°¢ƒÊ±‰∆˜µµŒª1£¨ÀŸ∂»£∫∞ÎÀŸ
			{
				wFANPWMTempSet = cFanHalfSpeed;
				bFan1status = true;
				bFan2status = true;
			}
			else//ø’‘ÿ«Èøˆœ¬∑Á…»Õ£÷π◊™∂Ø
			{
				#ifdef FANSLEEP
					//wFANPWMTempNew = cFanClosed;
					wFANPWMTempSet = cFanClosed;
		...
		if((sbGetTxTempStatus() >= 1)||FanStatus.HalfLoad)// Ωª¡˜∑Á…»ø™πÿøÿ÷∆£¨±‰—π∆˜Œ¬∂»µ≤ÀŸ1/∞Î‘ÿ
		{
			g_wACFanOnFlag = 1;
		}
		else
		...
		if(wFanComCtrl)//¥Æø⁄√¸¡Óøÿ÷∆∑Á…»◊™ÀŸ
		{
			g_wACFanOnFlag =1;
			wFANPWMTempNew =wFanSpeedCtrl;
		}
	...
	{
		if(hoACFAN==0)// CC 20210621 ÊéßÂà∂‰∫§ÊµÅÈ£éÊâáÁªßÁîµÂô?
		{
			sOSTimerStop();
	...
	if(wFANPWMTempPre > wFANPWMTempNew)//µ±«∞∑Á…»◊™ÀŸ>…Ë÷√∑Á…»◊™ÀŸ
	{
		wFANPWMTempPre -= 1;
		wFANPWMTemp = wFANPWMTempPre;
	}
	else if(wFANPWMTempPre < wFANPWMTempNew)//µ±«∞∑Á…»◊™ÀŸ<…Ë÷√∑Á…»◊™ÀŸ
	{
		if(bFan1status == true && bFan2status == true)
		{
			if(mFan1OnChk())
	...
	if(wFanControlStatus == cFanWaitSts)//∑Á…»«˝∂Øø™πÿ…Ë÷√
	{
	...
	if(wFANPWMTemp >= 100)
	{
		...
		EPwm7Regs.CMPA.half.CMPA = 4499;
	}
	else
	{
		EPwm4Regs.CMPA.half.CMPA = wFANPWMTemp * 45;
		EPwm7Regs.CMPA.half.CMPA = wFANPWMTemp * 45;;
	}
...
void sPVFanControl(void)
{	
	 if(bMpptHsTempInfo >= 3)
	{
		wPvFANPWMTempSet = cFanFullSpeed;
	}
	 else if(bMpptHsTempInfo >= 2)
	{
		wPvFANPWMTempSet = cFanHalfSpeed;
	}
	 else if(bMpptHsTempInfo >= 1)
	{
		wPvFANPWMTempSet = cFanLowerSpeed;
	}
	 else
 	{
 		wPvFANPWMTempSet = 0;
	 ...
	 wPvFANPWMTempNew = wPvFANPWMTempSet;

	if(wFanComCtrl2)// ÈÄöËÆØÊéßÂà∂È£éÊâá
	{		
		wPvFANPWMTempNew = wFanSpeedCtrl2;
	}

	if(wPvFANPWMTempPre > wPvFANPWMTempNew)
	{	
		{
			wPvFANPWMTempPre -= 1;
			wPvFANPWMTemp = wPvFANPWMTempPre;
		}
	}
	else if(wPvFANPWMTempPre < wPvFANPWMTempNew)
	{
...
//			wPvFANPWMTempPre += 10;
	...
	if(wPvFANPWMTemp >= 100)
	{
		EPwm7Regs.CMPA.half.CMPA = 4499;
	}
	else
	{
		EPwm7Regs.CMPA.half.CMPA = wPvFANPWMTemp * 45;
	}
}
...
void 	sBatWeakChk(INT8U bFilter)
{
	...
	//if((!g_uwBatWeakFlg)&&(sbGetBatLow() == true))// ÁîµÊ±†ÁîµÂéãbatlowbackÂíåbatlow‰πãÂâçÔºå‰∏ÄÁõ¥Êä•‰ΩéÂéã‰∏çÂàáÂ∏ÇÁîµÈóÆÈ¢ò
	//{
	//	g_uwBatWeakFlg = 1;
	//}
...
void sLineModeInvOverCurProtCnt(void)
{
	...
	if(bPVMode!=cLineMode)
	{
		wSetInvOverCurrentLineModeCnt = 4;//ÂàöÂàáÂà∞Â∏ÇÁîµÊ®°ÂºèÁöÑÊó∂ÂÄôÔºåÂë®ÊúüÊîπ‰∏∫4
		uwTranformCnt = 0;
	}
	else
	{
...
void sACSPSControl(void)
{
		...
		&& (0 == sbGetRLineWaveLoss()))
	{
		...
		{
			if(++wACspsCtrlMSCnt>20)// 10S Â∏ÇÁîµÊ≠£Â∏∏Âª∂Êó∂10SÊâçÈó≠Âê?
			{
				wACspsCtrlMSCnt =0;
				bACSpsCtrlOffFlag =0;
				hoACSPS_ON;
		...
		if( ++wACspsCtrlSecCnt>620)//5 minute ‰ª•ÂêéÊñ≠ÂºÄÁªßÁîµÂô?
		{
			wACspsCtrlSecCnt =0;
			if(!hoACSPS)
			{
...
void sBatParaUpdate(void)
{
	...
	*/
	...
	else//ÁîµÊ±†Á±ªÂûãÔºöcUseType„ÄÅcLiBType
	{
...
//	}
	...
	sFaultCodeClr(&uwFaultToStandbyCnt,3,&s_FaultToStandbyCnt);
...
//					wRKv = 45;            //Q7  0.18*128 
//					wRKi = 320;            //Q7  8*128// 
				wRKv = wRKvBase -0;           //Q7  0.18*128 
				wRKi = wRKiBase -0;           //Q7  8*128// 
			}
...
//				wRKi = 320;             //Q7  8*128// 
			wRKv = wRKvBase -0;           //Q7  0.18*128 
			wRKi = wRKiBase -15;           //Q7  8*128//             //Q7  8*128// 
			}
			else if(swGetEEKpKiParameter()==2)
			{
//					wRKv = 45;            //Q7  0.18*128 
//					wRKi = 300;             //Q7  8*128// 
				wRKv = wRKvBase -3;           //Q7  0.18*128 
			wRKi = wRKiBase -0;           //Q7  8*128// 
			}
...
//					wRKv = 37;             //Q7  0.18*128 
//					wRKi = 285;            //Q7  8*128// 
				wRKv = wRKvBase -5;           //Q7  0.18*128 
				wRKi = wRKiBase -20;           //Q7  8*128// 
			}
			else
			{
//					wRKv = 45;            //Q7  0.18*128 
//					wRKi = 320;            //Q7  8*128// 
	...
	if(wChgParaMeterSet != swGetEEChgParameter())
	{
		...
		{
			wKCurrentForwardAdj =10; 
		}
		else if(swGetEEChgParameter()==2)
		{
			wKCurrentForwardAdj =25; 
		}
		else
		{
			wKCurrentForwardAdj =0; 
