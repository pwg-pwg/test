...
INT8U  bLineToBatModeFlag = true;
INT16U uwLineToBatModeCnt = 0;//3/6分钟内切换次数
INT16U uwP3PhaseLineNLossFlag = false;//3相市电输入缺N相
...
void	sGridTask(void)
{
	...
	while(1)
	{
		...
		{ 	
			...
			{
				uwP3PhaseLineNLossFlag = false;
				uwLineToBatModeCnt = 0;
			}
		}		
	}
}
...
void sCalMaxCurAccept(void)
{
	...
	wMaxVmUpLimit =(INT16S)bMaxACChgCur*80;//
}
...
void	sSYNFreqCal(INT16U wSYNPeriodNew)
{
	if(wSYNPeriodNew != 0)
	{
		wSYNFreq = cdwINTERNAL_BUS / wSYNPeriodNew;//wSYNPeriodNew缩小了100倍；计算出来的频率放大100倍
	}
	else
	{
		wSYNFreq = 0;
...
void sLineVRmsCompensation(void)
{
 ...
 /*******************************************************************************
* Function Name  : sLineNLossChk
* Description	 :三相电N相缺失检测，N相丢失，继电器会来回切换，检测2分钟内，市电切换电池次数达到三次，
				   判断市电N相丢失，30分钟恢复或者掉完市电后2分钟恢复
* Input 		 : void
* Output		 : void
* Date			 : 2021.0630
* Author		 : CM
...
void sLineNLossChk(void)
{
	static INT16U uWLineLoss2minSCnt = 0;//
	static INT16U uwLineToBatModeCntTemp = 0;
	static INT32U dwLineNLossRecover = 0;//20min恢复


	if((uwLineToBatModeCnt >= 3) && (uwP3PhaseLineNLossFlag == false))
	{
		uwP3PhaseLineNLossFlag = true;
		dwLineNLossRecover = 180000;//30min恢复

		fLine.PhaseLoss = 0;
		sSetWarningCode(c3PhaseNLoss);
	}

	if(++uWLineLoss2minSCnt > 12000)//2min内检测一次，市电电压波动
	{
		uWLineLoss2minSCnt = 0;

		 if(uwLineToBatModeCnt <= uwLineToBatModeCntTemp)
		{
			if(uwLineToBatModeCnt > 0)
			{
				uwLineToBatModeCnt--;
			}
		}
		
		uwLineToBatModeCntTemp = uwLineToBatModeCnt;
	}

	if((--dwLineNLossRecover == 0)||((sbGetLineLossStatus() == true)&&(swGetRLineVolt() < cVac60v)))
	{
		uwP3PhaseLineNLossFlag = false;
		sClrWarningCode(c3PhaseNLoss);
	}
}
