...
INT16U TestCNTProtect =0;
INT16U TestCNTProtect2 =0;
...
Function:
	1.INV Control
	2.àÃ––ADíÒò”£¨ÅKå¢“ªÇÄƒÊ◊É÷‹∆⁄É»∏˜ÇÄíÒò”¿€º”÷µ±£¥ÊÀÕ≥ˆ°£
	3.ÆîwSinPointer£Ω=0ïr£¨∞lÀÕ ¬º˛PRIO_INVERTERMODULE,EVENTID_SIN_ZERO
...
//			{
//				wInvOffPoint =0;// ÂÖ≥Èó≠150ÁÇπ
//				wContinueTrig =0;
//				wOverVoltOffFlag =0;
//			}
//		}
	
		if(LiHardProtectCNT>=3)//Á°¨‰ª∂Ëß¶Âèë‰øùÊä§ 
		{
			if((wInvOffPoint ==0)&&(wContinueTrig==0))
			{
				wInvOffSinpointer=wCtrlSinpointer;

				if(wCtrlSinpointer<gi_wPLLPointThreeSix)//Ê≠£ÂçäÂë®Âá∫Áé∞‰øùÊä§
				{
				...
				wInvOffPoint =100;// ÂÖ≥Èó≠50ÁÇπ wInvOffPoint =150;// ÂÖ≥Èó≠150ÁÇπ
				wContinueTrig =1;
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>20))//if((wInvOffTimes ==0)&&(++wContinueTrigCNT>30))//Èò≤Ê≠¢ËøûÁª≠Ëß¶Âèë
			{
				wContinueTrigCNT =30;
				wContinueTrig =0;
			}
		...
		if(wInvOffTimes >0)
		{
			if((wInvOffsetpointer ==50)&&((wInvOffSinpointer == wCtrlSinpointer)||(wInvOffSinpointer == (wCtrlSinpointer+1))))
			{
				...
				wInvOffsetpointer =0;
				TestCNTProtect++;
				wInvOffSinpointer +=5;// ÂÖ≥Èó≠ËßíÂ∫¶ËΩØËµ∑
...
//				EDIS;
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>30))//if((wInvOffTimes ==0)&&(++wContinueTrigCNT>30))//Âª∂Êó∂30‰∏™ÁÇπ Èò≤Ê≠¢ËøûÁª≠Ëß¶Âèë
			{
				wContinueTrigCNT =30;
				wContinueTrig =0;
			}
...
void sLineWaveChk(INT16U wCmpPoint)
{
	...
	if(1)//‰∏çËøõË°åÊ≥¢ÂΩ¢Ê£ÄÊµã //20210111Chandler
	{
	   wLineLossTemp = false;
	}
	else 
	{
		...
		if(((wLineVoltReal > wLineWaveVoltHighLoss) || (wLineVoltReal < wLineWaveVoltLowLoss))
		      &&(abs(wOPCurrReal) < 680) && s_ubCTSaturationChkCnt < 15)    // (5000/140*1.414=50.5)*2.5=126	  ±‹√‚À≤Õ∂RCD‘ÿ£¨ŒÛ±®≤®–ŒLÂoss
		{
			wLineLossTemp = true;
		}
		else
...
void  swGetLineQuadraticSum(INT16S wRLineVoltRealTemp )
{
	...
	//wRLineVoltRealTemp=wRLineVoltRealTemp+50;//5VÁîµÂéãÂÅèÁΩÆ
...
void HardProtectChk(void)
{
	if((LiHardProtect==0)||((liparaTx==0)&&(mEepromOutputMode() > cOP_Single)))//Á°¨‰ª∂Ëß¶Âèë‰øùÊä§
	{
