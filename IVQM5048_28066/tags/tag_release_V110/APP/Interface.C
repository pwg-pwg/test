...
void	sTestCommand(INT8U sciid)
{
	...
	{ 
		...
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'M')&& (bCommandBuffer[sciid][5] == 'S'))
		{
			...
			{
				g_ubBMSForceChgTestFlg=(bCommandBuffer[sciid][8] - '0');// 1ÊòØÂº∫ÂÜ≤Áîµ
				sACK(sciid);
			}
			else if ((bCommandBuffer[sciid][6] == 'S')&&(bCommandBuffer[sciid][7] == 'C'))
			{
				g_ubBMSStopChgTestFlg=(bCommandBuffer[sciid][8] - '0');// 1ÊòØÁ¶ÅÊ≠¢ÂÖÖÁîµ
				sACK(sciid);
			}
			else if ((bCommandBuffer[sciid][6] == 'S')&&(bCommandBuffer[sciid][7] == 'D'))
			{
				g_ubBMSStopDischgTestFlg=(bCommandBuffer[sciid][8] - '0');// 1ÊòØÁ¶ÅÊ≠¢ÂÖÖÁîµ
				sACK(sciid);
			}
			else
			{
...
void sQGSCommand(INT8U sciid)		// SCC PWM
{
	...
	if(wBatVerifyFlg == true)	// µÁ≥ÿµÁ—π–£◊º ±÷√Œª±Í÷æ,“ª∑÷÷”∫Û◊‘∂Ø«Â≥˝
	{
	...
	// 15-18:≥‰µÁµÁ¡˜…Ë÷√(0.1A)(PS:◊Ó¥Û‘ –Ì50A,Œ¥≥¨π˝ ±+µÁ≥ÿ∑≈µÁµÁ¡˜)
	...
	// 32-33:≥‰µÁ◊¥Ã¨10=no charge, 11=CC, 12=CV, 13=Float
