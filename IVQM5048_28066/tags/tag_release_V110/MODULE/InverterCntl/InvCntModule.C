...
//		if(dwRVoltLimit < -16000) dwRVoltLimit = -16000;                 //500V max   
	...
	if((OPloopFlag)&&(mEepromOutputMode() == cOP_Single)||(mGetEEKpKiParameter ==5))//CC DeLETE
	{
...
void sFBINVPFCInitial(void)
{
		...
		|| (g_wVAType == cb10KVAModel))
	{
		...
		wRKv = 48;//45;//35;//48;//23;//10;   //48;             //Q7  0.18*128 
		wRKi = 320;//250;//300;//274;   //250;           //Q7  8*128//  
		wMovePoint = 0;
		wDeltaK=0;
		wCCKi = 64;            //2*32
		wChgKp = 2000;
...
void sFBINVController2(void)
{
	...
	else
	{
		...
		*  电流环每1个点一次
		***************************************************************************/
		wLineVoltCntlQ5=(INT16S)((INT32S)gi_wLineVoltQ5_0+(INT32S)(abs(wLineVoltCntlQ5+200))>>1);// 200是电压采样偏置补偿
