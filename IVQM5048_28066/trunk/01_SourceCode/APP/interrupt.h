...
#define	cFwVersion		110	// ÕýÊ½°æ±¾³ÌÐòV301
//#define	cFwVersion		2003	// ÔÚV200°æ±¾ÉÏµÚ3´ÎÐÞÕý°æ±¾
...
/***********************************************************************
 eeprom struct
***********************************************************************/
struct EepromStruct1//æ ¡å‡†å€¼ã€åç½®ã€SNç 
{
	INT16S  wEEBatteryVoltAdjb1;	//0
	INT16U  wEEBatteryVoltAdjA;		//1
	INT16U  wEepromRInvDcMid;		//2
	INT16U  wEepromRCurrMid;   		//3    
	INT16U  wEepromRSearchMid;    	//4
	INT16U  wSerial1;             	//5
	INT16U  wSerial2;            	//6
	INT16U  wSerial3;             	//7
	INT16U  wSerial4;   			//8
	INT16S  wEEBatVoltAdj ;			//9
	INT16S  dwEEBatVoltBias;  		//10
	INT8U   bUPSType;  				//11
	INT8U   bAxpertKSType; 			//12 
	
	INT16U  wInvVoltAdj;			//13
	INT16U  dwEEBatVoltBiasH;		//14
	INT16U  wEEBatVoltBiasVersion;	//15
	INT16S  wEEIDLength;			//16
	INT16U  wSerial5;				//17
	INT16U  wEELineVoltAdj;			//18
	INT16U  wEEInvCurrAdj;			//19
	INT16U  wEEOPCurrAdj;			//20
	INT16U  wEEShareCurrAdj;		//21
	INT16U  wEELCDType;    			//22
	INT16U  wPVCharger;				//23
	INT16U  wEEUSID;				//24
	
	INT16U  wEEpromVer;				//25	
	INT16S  wInvVoltBias;			//26
	INT16U  wKpKiParameter;			//27
	INT16U  wChgParameter;			//28
	
	INT16U wFlag;					//29
	INT16U wEECheckSum1;			//30
};

struct EepromStruct2//å……æ”¾ç”µå‚æ•°
{
