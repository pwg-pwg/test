...
INT16U	wInitLineVoltHLoss;
INT16U	wInitLineVoltLLoss;
INT16U	wInitLineVoltHBack;
INT16U	wInitLineVoltLBack;
INT16U  wAPLInitLineVoltLLoss;  	
INT16U 	wAPLInitLineVoltLBack;	
...
//	}
	if(sbGetEepromModeSelect() == 0)   //APL
	{
		wLineVoltLLoss =wAPLInitLineVoltLLoss;// cVac90v;  
		wLineVoltLBack = wAPLInitLineVoltLBack;//cVac100v;			
	}
	else
	{
		//wLineVoltLLoss = cVac170v;
		//wLineVoltLBack = cVac180v;
		wLineVoltLLoss = wInitLineVoltLLoss;//cVac170v;	
		wLineVoltLBack = wInitLineVoltLBack;//cVac180v;		
	}
...
void	sLineModuleInit(void)
{
    if(sbGetEepromModeSelect() == 0)   //APL
    {
		wLineVoltLLoss =wAPLInitLineVoltLLoss;
		wLineVoltLBack = wAPLInitLineVoltLBack;		
    }
	else
	{
		//wLineVoltLLoss = cVac170v;
		//wLineVoltLBack = cVac180v;
		wLineVoltLLoss = wInitLineVoltLLoss;//cVac90v;  
		wLineVoltLBack = wInitLineVoltLBack;//cVac95v;		
	}
	//wLineVoltHLoss = cVac280v;       
	//wLineVoltHBack = cVac270v;     
	wLineVoltHLoss = wInitLineVoltHLoss;       
	wLineVoltHBack = wInitLineVoltHBack;   
...
void	sLineModuleUpdate(void)
{
	...
	if((!fLine.RVoltLoss)&&(!fLine.FreqLoss)&&(!fLine.RWavLoss)&&(!fLine.RVoltLongTimeLoss)\
		&&(!fLine.OrderFault)&&(!fLine.VoltFastChkLoss)&&(!fLine.PhaseLoss)&&(!fLine.P3PhaseNLoss))
	{
		OS_ENTER_CRITICAL();
