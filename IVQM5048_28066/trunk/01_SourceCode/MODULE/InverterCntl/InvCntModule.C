...
extern INT16U ProtectFlag2;//原边过流保护信号
extern INT16U ProtectFlag1;//并机过流信号
...
INT16S wDCVoltSet = 0;
INT16S OPloopFlag =0;
INT16S wRLoadCurrCntlTest =64;
extern INT16S HardProtectOffSetCnt;
extern INT16S wInvOverCurrenProtectCnt;
extern INT16U wInvOffFlg;
extern INT32S NTotalPwm;
extern INT32S PTotalPwm;
INT16S gi_wPwmOffSetCnt =0;
...
//
	...
	wRVref = (INT16S)(((INT32S)wRSinAmp * pSinTab[wCtrlSinpointer] >> 14));
      //voltage loop
	wRInvVoltError = wRVref + gi_wInvVoltBias - wRInvVoltCntl;//HardProtectOffSetCnt   -gi_wPwmOffSetCnt gi_InvVolt2SumOffSetCnt
...
//	wRInvVoltErrorPre = wRInvVoltError;
	...
	if(wVoltLoopTemp < -wInvCrrentLimit<<5) wVoltLoopTemp = -wInvCrrentLimit<<5;//【54】wVoltLoopTemp

//	wRLoadCurrCntl=(INT16S)(((INT32S)wRLoadCurrCntl*wRLoadCurrCntlTest)>>6);
	wRVBeforeSaturation=wVoltLoopTemp+wRLoadCurrCntl;//wRKv = 10;  

	if(hoOPRLY == 1 && mEepromOutputMode() != cOP_Single)//并机
	{
	...
	if(wRVBeforeSaturation > wInvCrrentLimit<<5) wRVBeforeSaturation = wInvCrrentLimit<<5; // 【30】wRVBeforeSaturation      //5000/230*3=65A max   250
	...
	dwRK1 = (dwRK1*133 + dwRK1_1*123)>>8;//【47】dwRK1
...
//		if(dwRVoltLimit < -16000) dwRVoltLimit = -16000;                 //500V max   
	...
	if((mEepromOutputMode() == cOP_Single)&&(mGetEEKpKiParameter ==5))//开环
	{
		wRLoopOutput=(INT16S)(((INT32S)wRVref * wKpwm) >> 12);
	}
	else
	{
		wRLoopOutput=(INT16S)((dwRVoltLimit * wKpwm) >> 12);//�ջ�
	}
...
*/
	...
	if(wR_PWM >= 0)    
	{
		...
		if((ProtectFlag2)||(wInvOverCurrenProtectCnt > 0)||(ProtectFlag1))//(ProtectFlag ==1)
		{
...
//				EDIS;
				wR_PWMTemp =(INT16S)((INT32S)wPwmPeriod*3 >>6);	//5;限流打最小占空比
			}
			else
			{
//				wInvOffFlg =0;
...
//			ProtectFlagCnt++;
	...
	{
		wR_PWMNegtive = -wR_PWM;
		NTotalPwm +=wR_PWM;//wR_PWMNegtive;
...
void sFBINVPFCInitial(void)
{
		...
		|| (g_wVAType == cb10KVAModel))
	{
		...
		wFBKCurrentForward = 6;//5;//3;//4;  //Breeze 2016.1.29
  		wRKvBase = wRKv = 48;//45;//35;//48;//23;//10;   //48;             //Q7  0.18*128 
		wRKiBase = wRKi = 320;//250;//300;//274;   //250;           //Q7  8*128//  
		wMovePoint = 0;
		wDeltaK=0;
		wCCKi = 64;            //2*32
		wChgKp = 2000;
		...
		wRRepetCtrlUp = 800;//1600;//640;   //Q5 20*32 //Q5 62.5*32
		wRRepetCtrUnder = -800;//-1600;//-640;
...
//			{
//				if((wR_Is_P>=0)&&(wR_Is_P<50))  // 5A
//				{
//		             wKCurrentForward = 150;
//				}
//				else if((wR_Is_P>=50)&&(wR_Is_P<100))  //10A
//				{
//		             wKCurrentForward = 130;
//				}
//				else if((wR_Is_P>=100)&&(wR_Is_P<150))  //15A
//				{
//		             wKCurrentForward = 110;
//				}
//				else if(wR_Is_P>=200)  // 20A
//				{
//		             wKCurrentForward = 80;
//				}
//			}
...
//			wKCurrentForward =wFB_CurPI_KpTest;
			...
			dwTemp =dwTemp+1700;
//			dwTemp =dwTemp+wLineVoltRealOffSet;
			dwTemp = dwTemp * wPwmPeriod /g_wBatChgBusAvg;
			dwTemp = (dwTemp*14)>>4;
//			dwTemp = (dwTemp*wFB_CurMOTest)>>wFB_CurDNTest;
			//dwTemp = (dwTemp)>>2;
			//dwTemp =dwTemp+300;
			TestLineSet =dwTemp;
