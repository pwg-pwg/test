...
INT32S  g_dwGenPowerWattCversion;// >= 0 --> spend of Gen;   < 0 --> feed to Gen
...
//  g_dwBatACChgLimit = (g_dwBatACChgLimit * 136) >> 7; //0.94 efficiency
    ...
    {
        g_dwPVFeedLoadLimit = g_wInvDeratPowerLimit;//馈电功率计算
    }
...
void sInvPowerCal(void)
{
    ...
    g_wRLineWatt = g_dwROPWatt - g_dwRInvWatt;//市电功率 = 输出功率-逆变功率
    if(!sGetSmartOutputRlyOn())
    {
        g_dwSmartLoadPowerConversion = 0;//若智能负载没开，智能负载功率为0
    }
    else
    {
        g_dwSmartLoadPowerConversion = g_dwSmartOPWatt;//智能负载功率
        g_wRLineWatt += g_dwSmartOPWatt;//市电功率 = 输出功率+智能负载功率-逆变功率
    }
...
//    g_wTLineWatt = g_dwTOPWatt - g_dwTInvWatt;
    ...
    // if(g_uwLineModeJoinInWay == cLineModeJoinInByGrid)
    // {
    //     g_dwLinePowerWattCversion = g_wRLineWatt;
    // }
    // else if(g_uwLineModeJoinInWay == cLineModeJoinInByGen)
    // {
    //     g_dwGenPowerWattCversion = g_wRLineWatt;
    // }
//    g_dwLinePowerWattCversion = g_wRLineWatt + g_wSLineWatt + g_wTLineWatt;

    // if(g_dwLinePowerWattCversion < 0)        //不允许出现馈电
    // {
    //     g_GridFeedPower = g_dwLinePowerWattCversion;//记录馈电功率（负值）
    //     g_dwLinePowerWattCversion = 0;//强制归零市电功率
    // }
    // else if(g_dwLinePowerWattCversion < 80)  //防止侦测误差//低功率视为误差
    // {
    //     g_dwLinePowerWattCversion = 0;
    // }

    // if(g_dwGenPowerWattCversion < 0)        //不允许出现馈电
    // {
    //     g_GridFeedPower = g_dwGenPowerWattCversion;//记录馈电功率（负值）
    //     g_dwGenPowerWattCversion = 0;//强制归零市电功率
    // }
    // else if(g_dwGenPowerWattCversion < 80)  //防止侦测误差//低功率视为误差
    // {
    //     g_dwGenPowerWattCversion = 0;
    // }
    if(mChkGridRlyOn() && g_uwLineModeJoinInWay == cLineModeJoinInByGrid)//市电功率
    {
        g_dwLinePowerWattCversion = g_wRLineWatt;
        g_dwGenPowerWattCversion = 0;
        if(g_dwLinePowerWattCversion < 0)        //不允许出现馈电
    ...
    else if(sGetGenRlyOn() && g_uwLineModeJoinInWay == cLineModeJoinInByGen)//发电机功率
    {
        g_dwLinePowerWattCversion = 0;
        g_dwGenPowerWattCversion = g_wRLineWatt;
        if(g_dwGenPowerWattCversion < 0)        //不允许出现馈电
        {
            g_GridFeedPower = g_dwGenPowerWattCversion;//记录馈电功率（负值）
            g_dwGenPowerWattCversion = 0;//强制归零发电功率
        }
        else if(g_dwGenPowerWattCversion < 80)  //防止侦测误差//低功率视为误差
        {
            g_dwGenPowerWattCversion = 0;
    ...
    if(g_uwWorkMode == cLineMode)
    {
        ...
        {  
            ...
            if(g_uwSolarLoss)//grid -> load
            {
                ...
                if(mChkGridRlyOn() && g_uwLineModeJoinInWay == cLineModeJoinInByGrid)//市电接入
                {
                    //g_dwLinePowerWattCversion = g_dwLoadPowerConversion + g_dwSmartLoadPowerConversion;//电网功率=负载功率 + 智能负载功率
                    g_PowerTest = 1;
                    g_dwLinePowerWattCversion = g_dwLoadPowerConversion;//电网功率=负载功率
                    if(sGetSmartOutputRlyOn())
                    {
                        g_PowerTest = 2;
                        g_dwLinePowerWattCversion += g_dwSmartLoadPowerConversion;//电网功率=负载功率 + 智能负载功率
                    }
                }
                else if(sGetGenRlyOn() && g_uwLineModeJoinInWay == cLineModeJoinInByGen)//发电机接入
                {
                    g_PowerTest = 3;
                    g_dwGenPowerWattCversion = g_dwLoadPowerConversion;//发电机功率=负载功率
                }
            }
            else//PV + Grid -> load
            {
                ...
                {
                    g_PowerTest = 4;
                    g_dwLoadPowerConversion = g_dwLinePowerWattCversion + g_uwPVPowerConversion;//负载功率 = 市电功率 + PV功率
                    if(sGetSmartOutputRlyOn())
                    {
                        g_PowerTest = 5;
                        g_dwLoadPowerConversion -= g_dwSmartLoadPowerConversion;//负载功率 = 市电功率 + PV功率 - 智能负载功率
                    }
                }
                else if(sGetGenRlyOn() && g_uwLineModeJoinInWay == cLineModeJoinInByGen)
                {
                    g_PowerTest = 6;
                    g_dwLoadPowerConversion = g_dwGenPowerWattCversion + g_uwPVPowerConversion;   //负载功率 = 发电机功率 + PV功率               
                }
            }
        }
        else//有电池
        {
            ...
            if(g_uwSolarLoss)//batt + grid -> load
            {
                ...
                if(abs(g_dwInvWatt) < 100)//电池放电到SOC点之后，停止，但是功率折算出一定的数值导致电池变成充电，避免小功率出问题   20230629
                {
                    ...
                    if(mChkGridRlyOn() && g_uwLineModeJoinInWay == cLineModeJoinInByGrid)
                    {
                        ...
                        {
                            g_PowerTest = 8;
                            g_dwLinePowerWattCversion += g_dwSmartLoadPowerConversion;
                        }
                    }
                    else if(sGetGenRlyOn() && g_uwLineModeJoinInWay == cLineModeJoinInByGen)
                    {
                        g_PowerTest = 9;
                        g_dwGenPowerWattCversion = g_dwLoadPowerConversion;//逆变小功率，判定为电池未输出功率，电网功率=负载功率                
                    }
                }
                else
                {
                   // g_PowerTest = 10;
                    ...
                    if(mChkGridRlyOn() && g_uwLineModeJoinInWay == cLineModeJoinInByGrid)
                    {
                        ...
                        {
                            g_PowerTest = 11;
                            g_dwLoadPowerConversion -= g_dwSmartLoadPowerConversion;
                        }
                    }
                    else if(sGetGenRlyOn() && g_uwLineModeJoinInWay == cLineModeJoinInByGen)
                    {
                        g_PowerTest = 12;
                        g_dwLoadPowerConversion = g_dwGenPowerWattCversion + g_dwInvWatt;
                    }
                }
            }
            else// if(!g_uwSolarLoss)//pv + batt + grid -> load
            {
                ...
                //仅是逆变输出下，PV和逆变的功率相差接近，PV等于逆变功率，规避电池小功率输出
                if(g_dwInvWatt > 0 && (labs((INT32S)g_wPVPowerCal - g_dwInvWatt) < 100))
                {
                ...
                //避免电池充满时，存在小功率放电，导致与能流不一致
                if(g_uniSystemSts.Bit.uwBatDirection  == 0)//0：不充不放   1：充电    //2： 放电
                {
                    g_wBatPowerConversion = 0;
                }
                else if(g_uniSystemSts.Bit.uwBatDirection  == 1)
                {
                    if(g_wBatPowerConversion < 0)  g_wBatPowerConversion = 0;
                }
                else if(g_uniSystemSts.Bit.uwBatDirection  == 2)
                {
                    if(g_wBatPowerConversion > 0)  g_wBatPowerConversion = 0;
                }
                ...
                {
                    g_PowerTest = 15;
                    g_dwLoadPowerConversion = ((INT32S)g_uwPVPowerConversion - g_wBatPowerConversion) + g_dwGenPowerWattCversion;
                }

                //INV整流但市电功率小于80W时(处理为0)，这部分能量要从电池充电中处理掉，防止输出出现负功率
                if(g_dwInvWatt < 0 && g_dwLinePowerWattCversion == 0 && g_dwGenPowerWattCversion == 0)
                {
                    g_wBatPowerConversion = g_uwPVPowerConversion;
                    g_dwLoadPowerConversion = 0;
                    g_dwSmartLoadPowerConversion = 0;
                }
            }
        }
    }
...
//        g_wTLineWatt = 0;
        ...
        g_dwLoadPowerConversion = g_dwOPWatt;
        if(subGetBatOpenSts())//PV -> load   单PV模式下，二路不会有输出的
        {
            g_PowerTest = 16;
            g_wBatPowerConversion = 0;
            ...
            // {
            //     g_PowerCnt = 27;
            //     g_uwPVPowerConversion += g_dwSmartLoadPowerConversion;
            // }
        }
        else//PV + bat -> load
        {
            ...
            {
                g_PowerTest = 18;
                g_wBatPowerConversion -= g_dwSmartLoadPowerConversion;
            }
        }
    }
...
//        g_wTLineWatt = 0;
    ...
    else if(g_uwWorkMode == cBypassMode)//grid -> load
    {
        ...
        {
            g_PowerTest = 22;
            g_dwGenPowerWattCversion = g_dwLoadPowerConversion;
        }
...
//        g_wTLineWatt = 0;
    ...
    if(g_dwLoadPowerConversion < 0 )  g_dwLoadPowerConversion = 0;
    if(g_dwSmartLoadPowerConversion < 0 )  g_dwSmartLoadPowerConversion = 0;
}
//-----------------------------------------------------------
//-----------------------------------------------------------
