...
//    sInitHRCAP();
    ...
    PieVectTable.EPWM3_INT = &OSTimeBase;
...
//  PieVectTable.EPWM8_TZINT = &Epwm8_tzInterrupt;
    ...
    //Init system functions Data
//   System_Initialize();

    // Enable EPWM TZINTn in the PIE: Group 2 for EPWM8_TZINT
//    PieCtrlRegs.PIEIER2.bit.INTx8 = 1;

    // Enable EPWM INTn in the PIE: Group 3 interrupt 1-6
    PieCtrlRegs.PIEIER3.bit.INTx1 = 1;      //inverter
    PieCtrlRegs.PIEIER3.bit.INTx2 = 1;      //MPPT
    PieCtrlRegs.PIEIER3.bit.INTx3 = 1;      //time basse
         
    //Enable PIE group 4 interrupt 1,2,3 for eCAP1,2,3
    PieCtrlRegs.PIEIER4.bit.INTx1 = 1;     
    PieCtrlRegs.PIEIER4.bit.INTx2 = 1;      
    PieCtrlRegs.PIEIER4.bit.INTx3 = 1;
     

    //Enable PIE group 9 interrupt 3,4for scib;
    //interrupt 6 for E-CAN1   
    PieCtrlRegs.PIEIER9.bit.INTx3 = 1;    
    PieCtrlRegs.PIEIER9.bit.INTx4 = 1;  
//  PieCtrlRegs.PIEIER9.bit.INTx6 = 1;  // CAN可以不开
...
void sRamCheck(void)
{
    ...
    // RAMH0 : origin = 0x00D000, length = 0x001000
    // RAML1 : origin = 0x00A000, length = 0x003000
    //--------------------------------
...
void sInitAdc(void)
{
    ...
    EALLOW;
    AdcRegs.ADCCTL1.bit.ADCBGPWD  = 1;      // Power ADC BG         //--带隙缓冲器电路上电(内部参考电压)
    AdcRegs.ADCCTL1.bit.ADCREFPWD = 1;      // Power reference      //--基准缓冲器电路上电
    AdcRegs.ADCCTL1.bit.ADCPWDN   = 1;      // Power ADC            //--模拟核心内所有模拟电路上电(带隙和基准电路除外)
    AdcRegs.ADCCTL1.bit.ADCENABLE = 1;      // Enable ADC           //--ADC启用
//  AdcRegs.ADCCTL1.bit.ADCREFSEL = 0;      // Select interal BG    //--内部带隙用于基准生成(即0~4096对应0~3.3V)
    AdcRegs.ADCCTL1.bit.ADCREFSEL = 1;      // Select interal BG    //--外部基准电压3V
    EDIS;
    ...
    AdcRegs.ADCCTL2.bit.CLKDIV2EN = 1;//ADC clock = CPU clock/2     //--ADC时钟预分频
    AdcRegs.ADCCTL2.bit.CLKDIV4EN = 0;                              //--Fadc = Fcpu/2 = 45Mhz 
    // AdcRegs.ADCCTL2.bit.CLKDIV4EN = 1;
    AdcRegs.ADCCTL2.bit.ADCNONOVERLAP = 1;                          //--不允许采样重叠
    EDIS;
...
//  EDIS;
    ...
    //--选择AD通道
    AdcRegs.ADCSOC0CTL.bit.CHSEL = 0;       //--ADCSOC选择ADCINA0通道
...
void sInitGpio(void)
{
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO0       = 0;    // Enable pullup on GPIO0       //INVPWM_NU
    GpioCtrlRegs.GPAMUX1.bit.GPIO0      = 1;    // GPIO0 = PWM1A

    GpioCtrlRegs.GPAPUD.bit.GPIO1       = 0;    // Enable pullup on GPIO1       //INVPWM_ND
    GpioCtrlRegs.GPAMUX1.bit.GPIO1      = 1;    // GPIO1 = PWM1B

    GpioCtrlRegs.GPAPUD.bit.GPIO2       = 0;    // Enable pullup on GPIO2       //INVPWM_LU
    GpioCtrlRegs.GPAMUX1.bit.GPIO2      = 1;    // GPIO2 = PWM2A

    GpioCtrlRegs.GPAPUD.bit.GPIO3       = 0;    // Enable pullup on GPIO3       //INVPWM_LD
    GpioCtrlRegs.GPAMUX1.bit.GPIO3      = 1;    // GPIO3 = PWM2B

    GpioCtrlRegs.GPAPUD.bit.GPIO4       = 0;    // Enable pullup on             //TestIO1
    GpioDataRegs.GPACLEAR.bit.GPIO4     = 1;    // Load output latch to Low
    GpioCtrlRegs.GPADIR.bit.GPIO4       = 1;    // GPIO4 = Iutput
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO6       = 0;    // Enable pullup on             //FAN.SPEED
    GpioCtrlRegs.GPAMUX1.bit.GPIO6      = 1;    // GPIO6 = PWM4A

    GpioCtrlRegs.GPAPUD.bit.GPIO7       = 0;    // Enable pullup on GPIO7       //M.SW_ON
    GpioCtrlRegs.GPAMUX1.bit.GPIO7      = 0;    // GPIO7 = I/O
    GpioCtrlRegs.GPADIR.bit.GPIO7       = 0;    // GPIO7 = Iutput

    GpioCtrlRegs.GPAPUD.bit.GPIO8       = 0;    // Enable pullup on GPIO8       //PV.BOOST.PWM
    GpioCtrlRegs.GPAMUX1.bit.GPIO8      = 1;    // GPIO8 = PWM5A

    GpioCtrlRegs.GPAPUD.bit.GPIO9       = 0;   // Enable pullup on GPIO9            //FAN.FAULT
    GpioCtrlRegs.GPAMUX1.bit.GPIO9      = 0;  // GPIO9 = I/O
    GpioCtrlRegs.GPADIR.bit.GPIO9       = 0;   // GPIO9 = Iutput

    GpioCtrlRegs.GPAPUD.bit.GPIO10      = 0;    // Enable pullup on GPIO10       //M.LLC.LPWM1
    GpioCtrlRegs.GPAMUX1.bit.GPIO10     = 1;    // GPIO0 = PWM6A

    GpioCtrlRegs.GPAPUD.bit.GPIO11      = 0;    // Enable pullup on GPIO11       //M.LLC.LPWM2
    GpioCtrlRegs.GPAMUX1.bit.GPIO11     = 1;    // GPIO0 = PWM6B

    GpioCtrlRegs.GPAPUD.bit.GPIO12      = 0;    // Enable pullup on GPIO12       //M.ISO+.RLY
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO13      = 0;    // Enable pullup on GPIO13       //M.ISO-.RLY
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO15      = 0;    // Enable pullup on GPIO15      //SYN.RXD
    GpioCtrlRegs.GPAQSEL1.bit.GPIO15    = 0;    // Synch to SYSCLOUT
    GpioCtrlRegs.GPAMUX1.bit.GPIO15     = 1;    // GPIO15 = ECAP2

    //TZ信号时系统保护信号用来切断PWM信号，保护主电路，
    //所以要设置为异步模式asynchronous，这样当这个信号一进来的时候就马上有效，
    //不用等待与系统时钟的同步才去采样这个信号。
    GpioCtrlRegs.GPAPUD.bit.GPIO16      = 1;    // Enable pullup on GPIO16      //BB_OCP
    GpioCtrlRegs.GPAMUX2.bit.GPIO16     = 0;    // GPIO16 = I/O
    GpioCtrlRegs.GPADIR.bit.GPIO16      = 0;    // GPIO16 = INPUT

    GpioCtrlRegs.GPAPUD.bit.GPIO17      = 1;    // Enable pullup on GPIO17      //M.LLC_OCP
    GpioCtrlRegs.GPAMUX2.bit.GPIO17     = 0;    // GPIO17 = I/O
    GpioCtrlRegs.GPADIR.bit.GPIO17      = 0;    // GPIO17 = INPUT

    GpioCtrlRegs.GPAPUD.bit.GPIO18      = 1;    // Enable pullup on GPIO18      //M.PV_OCP
    GpioCtrlRegs.GPAMUX2.bit.GPIO18     = 0;    // GPIO18 = I/O
    GpioCtrlRegs.GPADIR.bit.GPIO18      = 0;    // GPIO18 = INPUT

    GpioCtrlRegs.GPAPUD.bit.GPIO19      = 0;    // Enable pullup on GPIO19     //GRIDN.RLY
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO20      = 1;    // Disable pullup on GPIO20    //GEN.SCR
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO21      = 1;    // Disable pullup on GPIO21    //GRID.SCR
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO25      = 0;    // Enable pullup on GPIO25      //AD.SWITCH2
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO26      = 0;    // Enable pullup on GPIO26      //GEN.Zero
    GpioCtrlRegs.GPAQSEL2.bit.GPIO26    = 1;    // Synch to SYSCLOUT
    GpioCtrlRegs.GPAMUX2.bit.GPIO26     = 1;    // GPIO26 = ECAP3

    GpioCtrlRegs.GPAPUD.bit.GPIO27      = 0;    // Enable pullup on GPIO27      //M.OP.L_RLY
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO28      = 1;    // Enable pullup on GPIO28      //M.LLC_OCP_RESET
    ...
    GpioCtrlRegs.GPAPUD.bit.GPIO29      = 1;    // Enable pullup on GPIO29      //M.BAT_OVP
    ...
    GpioCtrlRegs.GPBPUD.bit.GPIO34      = 0;    // Enable pullup on GPIO34      //GPIO34 /GPIO37 BOOT
    GpioCtrlRegs.GPBMUX1.bit.GPIO34     = 0;    // GPIO34 = I/O
    GpioCtrlRegs.GPBDIR.bit.GPIO34      = 0;    // GPIO34 = Intput


    //GPIO35~38 仅作为烧录引脚(线路图悬空，这里配置为上拉，输出低电平)
    //GPBPUD     0->上拉，增加输出驱动能力 1->禁止上拉
    //GPBCLEAR   0->忽略                     1->将相应的输出数据锁存器置低。
    //GPBMUX1    0-> 通用IO                  1->外部接口扩展
    GpioCtrlRegs.GPBPUD.bit.GPIO35      = 0;    // Enable pullup on GPIO35      //M.TDI
    ...
    GpioCtrlRegs.GPBPUD.bit.GPIO36      = 0;    // Enable pullup on GPIO36      //M.TMS
    GpioDataRegs.GPBCLEAR.bit.GPIO36    = 1;    // Load output latch to low
    GpioCtrlRegs.GPBMUX1.bit.GPIO36     = 0;    // GPIO36 = I/O
    GpioCtrlRegs.GPBDIR.bit.GPIO36      = 0;    // GPIO36 = INTPUT


    GpioCtrlRegs.GPBPUD.bit.GPIO37      = 0;    // Enable pullup on GPIO37      //M.TDO
    ...
    GpioCtrlRegs.GPBPUD.bit.GPIO38      = 0;    // Enable pullup on GPIO38      //M.TCK
    ...
    GpioCtrlRegs.GPBPUD.bit.GPIO39      = 1;    // Enable pullup on GPIO39     //M.BAT.SPS_EN
    ...
    GpioCtrlRegs.GPBMUX1.bit.GPIO40     = 1;    // GPIO40 = PWM7A            //M.BOOST.PWM

    GpioCtrlRegs.GPBPUD.bit.GPIO41      = 0;    // Enable pullup on
    GpioCtrlRegs.GPBMUX1.bit.GPIO41     = 1;    // GPIO41 = PWM7B            //M.BUCK.PWM

    GpioCtrlRegs.GPBPUD.bit.GPIO42      = 0;    // Enable pullup on
    GpioCtrlRegs.GPBMUX1.bit.GPIO42     = 1;    // GPIO42 = PWM8A            //M.LLC.HPWM1

    GpioCtrlRegs.GPBPUD.bit.GPIO43      = 0;    // Enable pullup on
    GpioCtrlRegs.GPBMUX1.bit.GPIO43     = 1;    // GPIO43 = PWM8B            //M.LLC.HPWM2

    GpioCtrlRegs.GPBPUD.bit.GPIO44      = 0;    // Enable pullup on             //M.GRID.SPS.EN
    GpioDataRegs.GPBCLEAR.bit.GPIO44    = 1;    // Load output latch to Low
    GpioCtrlRegs.GPBDIR.bit.GPIO44      = 0;    // GPIO44 = Intput
    GpioCtrlRegs.GPBMUX1.bit.GPIO44     = 0;    // GPIO44 = I/O

    GpioCtrlRegs.GPBPUD.bit.GPIO50      = 1;    // Enable pullup on GPIO50      //M.INV_OCP
    GpioCtrlRegs.GPBMUX2.bit.GPIO50     = 0;    // GPIO50 = I/O
    GpioCtrlRegs.GPBDIR.bit.GPIO50      = 0;    // GPIO50 = Intput

    GpioCtrlRegs.GPBPUD.bit.GPIO51      = 0;    // Enable pullup on GPIO51      //M.LED1
    GpioDataRegs.GPBSET.bit.GPIO51      = 1;    // Load output latch to high
    GpioCtrlRegs.GPBMUX2.bit.GPIO51     = 0;    // GPIO51 = I/O
    GpioCtrlRegs.GPBDIR.bit.GPIO51      = 1;    // GPIO51 = Output

    GpioCtrlRegs.GPBPUD.bit.GPIO52      = 1;    // Enable pullup on GPIO52      //M.BUS_OVP
    GpioCtrlRegs.GPBMUX2.bit.GPIO52     = 0;    // GPIO52 = I/O
    GpioCtrlRegs.GPBDIR.bit.GPIO52      = 0;    // GPIO52 = Intput

    GpioCtrlRegs.GPBPUD.bit.GPIO53      = 0;    // Enable pullup on GPIO53      //AD.SWITCH1
    ...
    GpioCtrlRegs.GPBPUD.bit.GPIO54      = 0;    // Enable pullup on GPIO54      //GEN.L RLY
    ...
    GpioCtrlRegs.GPBPUD.bit.GPIO56      = 1;    // Enable pullup on GPIO56      //M.12V_UVP
    GpioCtrlRegs.GPBMUX2.bit.GPIO56     = 0;    // GPIO56 = I/O
    GpioCtrlRegs.GPBDIR.bit.GPIO56      = 0;    // GPIO56 = Intput

    GpioCtrlRegs.GPBPUD.bit.GPIO57      = 0;    // Enable pullup on GPIO57       //INV.L.RLY
    ...
    GpioCtrlRegs.GPBPUD.bit.GPIO58      = 0;    // Enable pullup on GPIO58      //AD.SWITCH0
...
//  EPwm1Regs.TBCTL.bit.SYNCOSEL    = TB_SYNC_DISABLE;      // Disable EPWMxSYNCO signal
    EPwm1Regs.TBCTL.bit.SYNCOSEL    = TB_CTR_ZERO;          // Disable EPWMxSYNCO signal //PWM1/2进行时钟同步
...
//  EPwm1Regs.AQCTLA.bit.ZRO        = AQ_CLEAR;             //0x0555
//  EPwm1Regs.AQCTLA.bit.PRD        = AQ_CLEAR;
//  EPwm1Regs.AQCTLA.bit.CAU        = AQ_CLEAR;
//  EPwm1Regs.AQCTLA.bit.CAD        = AQ_CLEAR;
//  EPwm1Regs.AQCTLA.bit.CBU        = AQ_CLEAR;
//  EPwm1Regs.AQCTLA.bit.CBD        = AQ_CLEAR;
    EPwm1Regs.AQCTLA.all            = 0x0555;               //EPWM1A always low,modify it when  control start //0x0555=0101 0101 0101  CBD=CBU=CAD=CAU=PRD=ZRO => low
    ...
    EPwm1Regs.DBFED                 = 153;                  //153_1.7us//333; //333_3.7us
    EPwm1Regs.DBRED                 = 153;                  //153_1.7us//333; //333_3.7us
    
    EPwm1Regs.ETPS.bit.INTCNT       = 0;
    EPwm1Regs.ETPS.bit.INTPRD       = ET_1ST;               //first event trigger
...
//  EPwm1Regs.ETPS.bit.SOCBPRD      = ET_1ST;
 ...
 //ePWM2 
    ...
    EPwm2Regs.DBFED                 = 153;                  // 153_1.7us//333;  //333_3.7us
    EPwm2Regs.DBRED                 = 153;                  // 153_1.7us//333;  //333_3.7us
...
//  EPwm2Regs.ETPS.bit.SOCBPRD      = ET_1ST;
    ...
    //ePWM3             //定时器
    EPwm3Regs.TBPRD = cOSTimeBase; //ePWM6B   Period = 5200 TBCLK counts
    ...
    EPwm3Regs.CMPB                  = 44999;        //49999->44999
...
//  EPwm6Regs.TZFRC.bit.OST     = 1;
    ...
    EPwm6Regs.TBPRD                 = cDCDCConvertPeriod;
    ...
    EPwm6Regs.TBCTL.bit.PRDLD       = TB_SHADOW;             //loaded from shadow register when TBCTR is equal to zero
    EPwm6Regs.TBCTL.bit.SYNCOSEL    = TB_SYNC_DISABLE;       // Disable EPWMxSYNCO signal

    EPwm6Regs.CMPCTL.bit.SHDWAMODE  = CC_SHADOW;
    EPwm6Regs.CMPCTL.bit.LOADAMODE  = CC_CTR_PRD;//CC_CTR_ZERO;修改装载模式，处理LLC关机封波阶段，低压侧末端波形仅输出半周期的现象
    EPwm6Regs.CMPCTL.bit.SHDWBMODE  = CC_SHADOW;
    EPwm6Regs.CMPCTL.bit.LOADBMODE  = CC_CTR_ZERO;//CC_CTR_PRD;
...
//  EPwm5Regs.ETSEL.bit.INTSEL  = ET_CTR_PRD;       //Enable event time-base counter equal to period.
    ...
    //ePWM8  LLC
    EALLOW;
    EPwm8Regs.TZCTL.bit.TZA     = TZ_FORCE_LO;         //do nothing
    EPwm8Regs.TZCTL.bit.TZB     = TZ_FORCE_LO;         //do nothing
...
void sInitECAP(void)
{
    //ECAP1  GRID_ZERO 只用到ECAP的CAP1，即只需要一次捕获事件就触发中断
    ECap1Regs.ECEINT.all = 0x0000;             // Disable all capture interrupts
    ECap1Regs.ECCLR.all = 0xFFFF;              // Clear all CAP interrupt flags    
    ECap1Regs.ECCTL1.bit.CAPLDEN = 0;          // 禁止在捕获事件中加载 CAP1-4 寄存器的值 
    ECap1Regs.ECCTL2.bit.TSCTRSTOP = 0;        // 时间戳（TSCTR）计数器停止
    ECap1Regs.ECCTL2.bit.CAP_APWM = 0;         // eCAP mode     //0->eCAP  1->APWM
    ECap1Regs.ECCTL2.bit.CONT_ONESHT = 1;      // 单次捕获
    ECap1Regs.ECCTL2.bit.SYNCO_SEL = 3;        // 禁用同步输出信号（仅作为输入捕获，不需要APWM同步输出）
    ECap1Regs.ECCTL2.bit.SYNCI_EN = 0;         // 禁用同步输入选项
    ECap1Regs.ECCTL2.bit.STOP_WRAP = 0;        // 捕获1次后停止
    ECap1Regs.ECCTL1.bit.CAP1POL = 1;          // 下降沿捕获
    ECap1Regs.ECCTL1.bit.CTRRST1 = 0;          // 单次模式下，在CAP1的捕获事件发生后产生停止信号    
    ECap1Regs.ECCTL1.bit.PRESCALE = 0;         // 输入信号分频（输入信号频率过高时需要）
    ...
    ECap1Regs.ECCTL2.bit.REARM = 1;            // 复位MOD4计数器，重新捕获（单次模式下当MOD4计数到STOP_WRAP会停止捕获）
    ECap1Regs.ECCTL1.bit.CAPLDEN = 1;          // 使能在捕获事件时将计数器（CSCTR）加载到CAP1~4
    ...
    ECap3Regs.ECCTL2.bit.REARM = 1;            // 复位MOD4计数器，重新捕获（单次模式下当MOD4计数到STOP_WRAP会停止捕获）
    ECap3Regs.ECCTL1.bit.CAPLDEN = 1;          // 使能在捕获事件时将计数器（CSCTR）加载到CAP1~4
    ECap3Regs.ECEINT.bit.CEVT1 = 1;            // 捕获事件1中断使能
...
{
    EALLOW;
    HRCap4Regs.HCCTL.bit.SOFTRESET = 1;         // 清除 HCCOUNTER、所有捕获寄存器和 IFR 寄存器位
    HRCap4Regs.HCCTL.bit.HCCAPCLKSEL = 0;       // HCCAPCLK = SYSCLKOUT
    HRCap4Regs.HCCTL.bit.FALLINTE = 1;          // Enable Falling Edge Capture Event Interrupt
    HRCap4Regs.HCCTL.bit.OVFINTE = 1;    
