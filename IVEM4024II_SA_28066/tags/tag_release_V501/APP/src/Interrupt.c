...
//Line
INT16S  g_wRGeneratorWaveChkCnt = 0;
INT16S  g_wRGeneratorVoltMax = 0;
INT16S  g_wRGeneratorVoltMin = 0;
INT16U  g_uwRLineWaveLossCnt = 0;
INT16U  g_uwRLineWaveOKCnt = 0;
INT16U  g_uwRLineWaveLossFlag = true;

//Gen
INT16S  g_wRGenerator1WaveChkCnt = 0;
INT16S  g_wRGenerator1VoltMax = 0;
INT16S  g_wRGenerator1VoltMin = 0;
...
//    swGetLineQuadraticSum(g_wRLineVoltReal);
    if(!(g_uwLineStatus & (cBitLineStatusVoltLoss | cBitLineStatusFreqLoss)))
    {
        ...
        {
            sLineWaveChk(g_wRLineVoltReal,g_wSinValueR,g_uwRLineRms3timeAvgPeak,g_wROPCurrSample, \
                                        &g_uwRLineWaveLossCnt, &g_uwRLineWaveOKCnt, &g_uwRLineWaveLossFlag,\
                                        &g_wRGeneratorWaveChkCnt, &g_wRGeneratorVoltMax, &g_wRGeneratorVoltMin);
...
//        swGetGenQuadraticSum(g_wRGenVoltReal);
        ...
        {
            sGenWaveChk(g_wRGenVoltReal,g_wSinValueR,g_uwRLineRms3timeAvgPeak,g_wROPCurrSample, \
                                        &g_uwRGenWaveLossCnt, &g_uwRGenWaveOKCnt, &g_uwRGenWaveLossFlag,\
                                        &g_wRGenerator1WaveChkCnt, &g_wRGenerator1VoltMax, &g_wRGenerator1VoltMin);
            sGenWavLossStsUpdate(g_uwRGenWaveLossFlag, 0, 0);
...
//    //PV硬件过流   拉弧时容易触发PV硬件过流故障，暂时屏蔽
    //软件PV过流点为35A持续超过520us，硬件过流点45A持续超过260us，仅能加长滤波时间改为8*52us = 416us,待定
    if(!liPvIOVER)
    {
...
//    || uniWarningCode.BIT.uwAFCIComFail
    || (!(g_uwAFCIStatus & (1 << eAFCIState_Work)) && swGetEEAFCIEn())) //AFCI功能使能后出现通信故障关PV,或者AFCI自检未完成
    {
	...
	//当出现ARC故障后，又出现AFCI通信故障，该如何处理？
	if(g_uwAFCIStatus & (1 << eAFCIState_Arcfault) /*&& g_uwWorkMode != cFaultMode*/ && g_uwFaultCode != cARCFault)
	{
		g_uwFaultCode = cARCFault;
        OSISREventSend(cPrioSuper, eSuperFault);
        sFaultRecord(cARCFault, 1, g_uwFaultCode);
	}
	if(g_uwAFCIStatus & (1 << eAFCIState_Comfault) /*&& g_uwWorkMode != cFaultMode*/ && g_uwFaultCode != cAFCIComFault)
	{
		g_uwFaultCode = cAFCIComFault;
        OSISREventSend(cPrioSuper, eSuperFault);
        sFaultRecord(cAFCIComFault, 1, g_uwFaultCode);
	}	
    //--------------------------------------------------------------------
    //              母线电压调制比计算
    //--------------------------------------------------------------------
...
interrupt void LineZeroCrossInterrupt(void)
{
    ...
    //------ 未使用 ------//
     ...
     g_wRGenerator1WaveChkCnt = 0;
     g_wRGenerator1VoltMax = 0;
     g_wRGenerator1VoltMin = 0;
    //-------------------//
