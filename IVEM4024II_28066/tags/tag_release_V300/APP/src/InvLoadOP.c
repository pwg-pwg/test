...
//      }
    ...
    if(g_wAgingMode)
    {
        ...
        g_wDCDCBusVoltRef = g_wBusVoltRef + cBusVoltReal10V;
...
//  g_dwBatACChgLimit = (g_dwBatACChgLimit * 136) >> 7; //0.94 efficiency
    ...
    else if(mChkOPRlyOn() && ((swGetEEOPPriority() != cOutputUtilityFrist) || subGetParaBatOpenSts()))
    {
        ...
        if(dwPowerTemp > g_dwSolarLimitInvPower)
        {
            dwPowerTemp = g_dwSolarLimitInvPower;
        }
...
void sSystemWorkSts(void)
{
    ...
    else if(g_uwWorkMode == cLineMode)
    {
        ...
        {
            if(   suwGetBoost1CtrlSts() != cBoostWait
               && (g_dwInvWattAvg > 0 || suwGetDCDCCtrlSts() != cDCDCWork || (swGetEEChgPriority() == cChargeSolarOnly && !g_ucBatDanger) )
               )
