			...
			if(subGetParaBatOpenSts() || uwLLCWorkFlg || uwForceGridSoftBusFlag == 1)//无电池 或 (市电建立BUS经过buck启动中间BUS后，LLC工作，再关闭DCDC)
			{
				...
				if(g_uwPBusAvgVoltNew >= uwBusSoftPoint)
				{
					if(--uwBusVoltOKDelay == 0)
					{
						...
						sRelayPoweLift(); //YXF
						//sRlyDelayProc(RLY_DELAY, cDelay0ms7, eSuperSelectPoint, cPrioSuper);
						//sRlyDelayProc(RLY_ZEROPOINT, cDelay0ms9, eSuperSelectPoint, cPrioSuper);
		...
		if(g_uwSuperEvent & (1 << eSuperToLine))
		{
			...
			if(g_LineModeJoinInWay == cLineModeJoinInByGrid)
			{
				...
				else
				{
					...
					sRelayPoweLift(); //YXF
					//sRlyDelayProc(RLY_DELAY, cDelay0ms7, eSuperSelectPoint, cPrioSuper);
					//sRlyDelayProc(RLY_ZEROPOINT, cDelay0ms9, eSuperSelectPoint, cPrioSuper);有问题：sRelayPoweLift动作后延时9ms后才能让继电器动作，继电器电源时间实际只有11ms。低于10ms继电器可能无法正常闭合
