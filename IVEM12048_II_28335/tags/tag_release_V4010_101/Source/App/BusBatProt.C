...
void sDischgCurOverProc(void)
{
	...
	INT16U uwLineToBatCur1;
	INT16U uwLineToBatCur2;
	...
	if(fBMSConnectFlg)
	{
		...
		uwBatToLineCur2 = (INT16U)((mBMSBatMaxDischgCurr * 15) / 10);
		uwLineToBatCur1 = (INT16U)((mBMSBatMaxDischgCurr * 9) / 10);
		uwLineToBatCur2 = (INT16U)((mBMSBatMaxDischgCurr * 14) / 10);
	}
	else
	{
		uwBatToLineCur1 = swGetEEBatDisChgCurrMax();              
		uwBatToLineCur2 = (INT16U)((swGetEEBatDisChgCurrMax() * 15) / 10);
		uwLineToBatCur1 = (INT16U)((swGetEEBatDisChgCurrMax() * 9)  / 10);
		uwLineToBatCur2 = (INT16U)((swGetEEBatDisChgCurrMax() * 14) / 10);
	}
	...
	if(    g_uwWorkMode == cBatteryMode
		...
		&& (g_wDCDCCurrAvg < 0) )
	{
		if(-g_wDCDCCurrAvg > uwBatToLineCur2)
		{
			s_uwBatToLineCnt1++;
			s_uwBatToLineCnt2++;
		}
		else if(-g_wDCDCCurrAvg > uwBatToLineCur1)
		{
			s_uwBatToLineCnt1++;
		}
		else
		...
		if(s_uwBatToLineCnt1 > 400 || s_uwBatToLineCnt2 > 50)   //一级保护8S,二级保护1s
		{
			s_uwBatToLineCnt1 = 0;
			s_uwBatToLineCnt2 = 0;
			g_uwBMSDischgOverCurFlg = true;
			// g_uwBackToBatModeFlg = false;
		}
	}
	...
	{
		if((!g_uwSolar1Loss || !g_uwSolar2Loss) /*&& swGetEEParallelEn() == cOP_Single*/)
		{
			uwBatCurr = (INT16U)((g_dwOPWattFilter + g_dwSmartOPWattFilter) * 100 / g_uwBatVoltAvg);
			if(uwBatCurr < uwLineToBatCur2) 
			{
				if((g_uwSolarPower1Avg + g_uwSolarPower2Avg) < (g_dwOPWattFilter + g_dwSmartOPWattFilter))
				{
					uwBatCurr = (INT16U)(((g_dwOPWattFilter + g_dwSmartOPWattFilter) - (g_uwSolarPower1Avg + g_uwSolarPower2Avg)) * 100 / g_uwBatVoltAvg);
				}
				else
				{
					uwBatCurr = 0;
		...
		if(uwBatCurr < uwLineToBatCur1)
		{
			if(++s_uwLineToBatCnt >= 30000)  //10min
			{
				s_uwLineToBatCnt = 0;
				// g_uwBackToBatModeFlg = true;
				g_uwBMSDischgOverCurFlg = false;
			}
		}
		else
