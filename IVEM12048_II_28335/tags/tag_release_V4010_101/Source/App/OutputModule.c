...
INT16U	g_uwSmartOPFreq;

INT32S	g_dwSmartOPWatt;
INT32S	g_dwRSmartOPWatt;
INT32S	g_dwSSmartOPWatt;
INT32S	g_dwTSmartOPWatt;
INT32S	g_dwSmartOPRealWatt;

INT32S	g_dwRSmartOPWattFilter = 0;
INT32S	g_dwSSmartOPWattFilter = 0;
INT32S	g_dwTSmartOPWattFilter = 0;
INT32S	g_dwSmartOPWattFilter = 0;

INT16U	g_uwRSmartOPVolt;
INT16U	g_uwRSmartOPCurr;
INT32S	g_dwSmartOPVA;
INT32S	g_dwRSmartOPVA;

INT32S dwRSmartOPWattRes = 0;
INT32S dwRSmartOPWattTemp = 0;
...
INT16S	g_wSOpCurrPercent;
INT16S	g_wSOpCurrPercentTemp;
...
void sSmartOPWattAdj(INT32S dwRSmartOPWatt, INT32S dwSSmartOPWatt, INT32S dwTSmartOPWatt)
{
	...
	g_dwRSmartOPWatt = (g_dwRSmartOPWatt * g_wOPVoltTransferRatio) >> 10;
...
void sOPPercentCal(void)
{
	...
	// 计算市电电流百分比
	//-----------------------------------------------
	if(mChkGridRlyOn() || sGetGenRlyOn())//市电带载 或 发电机带载
	{
		if(g_dwInvWatt > 0)// if(g_uwRInvCurr > 0)
		{
			g_wGridCurrPercent = (INT16S)(((INT32S)g_uwROPCurr + (INT32S)g_uwRSmartOPCurr - (INT32S)g_uwRInvCurr) * 100 / cGridRateCurr);
		}
		else
		{
			g_wGridCurrPercent = (INT16S)((INT32S)(g_uwROPCurr + g_uwRSmartOPCurr) * 100 / cGridRateCurr);
		}
	...
	if(g_uwRSmartOPCurr)
	{
		g_wSOpCurrPercent = (INT16S)(((INT32S)g_uwRSmartOPCurr * 100) / cGridRateCurr);
	}
	else
	{
		g_wSOpCurrPercent = 0;
	}
	g_wSOpCurrPercentTemp = g_wSOpCurrPercent;

	//-----------------------------------------------
	g_wOPWattPercent = (INT16S)((g_dwOPWatt+g_dwSmartOPWatt) * 100 / g_dwOPMaxPower);
	g_wROPWattPercent = (INT16S)((g_dwROPWatt+g_dwRSmartOPWatt) * 100 / g_dwOPPhaseMaxPower);
...
//	g_wTOPVAPercent = (INT16S)(g_dwTOPVA * 100 / g_dwOPPhaseMaxPower);
	...
	//     { 
	// 		if(g_wOPPercent < g_wGenCurrPercent)
	// 		{
	// 			g_wOPPercent = g_wGenCurrPercent;
	// 		}
	// 	}
	// 	else 
	// 	{ 		
	...
	if(g_wOPPercent < g_wSOpCurrPercent) { g_wOPPercent = g_wSOpCurrPercent; }
...
//	if(g_uwMachineType == cOffGrid3LoadBat)
	{
		...
		else if(g_uwWorkMode == cLineMode || g_uwWorkMode == cBypassMode)
		{
			...
			else
			{
				...
				g_wROPLineRatioPercent = (INT32S)g_wROPLineRatioPercent * swGetEEOutputVolt() / g_uwRLineVolt;
			}
...
void sOverLoadChk(void)
{
    ...
    if((g_uwWorkMode != cBypassMode) && (g_uwWorkMode != cLineMode))
    {
        //离网105%过载10.5S保护，150%过载5.5S保护，200%过载200mS快速保护
    ...
    {   //市电120%过载5s保护，200%过载200mS快速保护
        // 电网和油机都需要过载保护
        // if(g_LineModeJoinInWay == cLineModeJoinInByGrid)
        {
            if(g_uwOverLoadLevel1Cnt >= 20 || g_uwOverLoadLevel2Cnt >= 550 || g_uwOverLoadLevel3Cnt >= 1050)  //20230705  增加市电过载逻辑
            {
                g_uwFaultCode = cOverLoad;
                g_uwOverLoadLevel1Cnt = 0;
                g_uwOverLoadLevel2Cnt = 0;
        ...
        if(g_uwSOPCurr >= 50)
        {
            if(g_uwSOPShareCurr > (g_uwSOPCurr * 5))
            {
                if(++s_uwShareCurrChkCnt >= 10)
                {
                    ...
                    g_uwFaultCode = cOPCurrCircuitFault;
                    OSEventSend(cPrioPara, eParaToFaultMode);
				    sFaultRecord(cOPCurrCircuitFault, g_uwSOPShareCurr, g_uwFaultCode);
                }
            }
            else
            {
		...
		if(g_uwTOPCurr >= 50)
		{
			if(g_uwTOPShareCurr > (g_uwTOPCurr * 5))
			{
				...
				if(s_uwShareCurrChkCnt >= 10)
				{
					...
					g_uwFaultCode = cOPCurrCircuitFault;
					OSEventSend(cPrioPara, eParaToFaultMode);
                    sFaultRecord(cOPCurrCircuitFault, g_uwTOPShareCurr, g_uwFaultCode);
				}
			}
			else	
			{
