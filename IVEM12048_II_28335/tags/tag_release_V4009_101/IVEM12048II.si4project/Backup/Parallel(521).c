...
INT16U  g_uwSystemOverloadNumPre = 0;
...
//							}
				...
				else if(g_uwCommand == cToFault)
				{
					...
					else if(g_uwSourceComEve == cMasterSend)
					{
						...
						else if(g_strCanReceiveFrame.Data0 == cOverLoad)
						{
						    g_uwSystemOverloadNumPre = g_strCanReceiveFrame.Data1;
                            g_uwFaultCode = (g_strCanReceiveFrame.Data0);
                            OSEventSend(cPrioSuper, eSuperFault);
                            if(g_uwWorkMode != cFaultMode)
                            {
...
INT16U swChkOverLoadStatus(void)
{
	...
	static INT16U s_uwOverloadCnt = 20;
	static INT16U s_wSystemOverloadNumPre = 0;
...
//				}
	...
	*/
	...
	{
		if(g_uwSystemOverloadNumPre < wSystemOverloadNum 
			&& wSystemOverloadNum != 0)    //过载机器增加 
		{
	        g_uwFaultCode = (cOverLoad);
	        sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh,cNullData,g_strLocalID.BIT.wLocalIDLow,\
	                        cToFault,cMasterSend,cNullData,\
	                        cOverLoad,wSystemOverloadNum,cNullData);
	        sCan2Write9(&g_strCanTxCom2); 
	        OSEventSend(cPrioSuper, eSuperFault);
//			if(--s_uwOverloadCnt == 0) //延时处理
//			{
//				if(g_strLocalInvStatus.BIT.wOverLoadFail == 0)
//				{
//					s_uwOverloadCnt = 20;
//					g_uwFaultCode = (cOverLoad);
//					OSEventSend(cPrioSuper, eSuperFault);
...
//	if(wSystemOverloadNum && uwExcessLoadAvg == 0x02 /*&& !g_uwOverLoadOnecetime*/)
//	{
////        g_uwFaultCode = (cOverLoad);
//        sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh,cNullData,g_strLocalID.BIT.wLocalIDLow,\
//                        cToFault,cMasterSend,cNullData,\
...
//	}
	...
	g_uwSystemOverloadNumPre = wSystemOverloadNum;
	//==========================================================
	if(wOverLoadNum > 0)
	{
