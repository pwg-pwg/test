...
INT16U swRoot(INT32U dwNumber)
{
	...
	if(dwNumber == 0) { return (0); }

	if(dwNumber <= 4194304) 		{ dwSquareRoot = dwNumber / 1024 + 63;    }
	else if (dwNumber <= 134217728) { dwSquareRoot = dwNumber / 4096 + 255;   }
	else 							{ dwSquareRoot = dwNumber / 16384 + 1023; }
...
{
	if((wCompareData > wHighRange) || (wCompareData < wLowRange)) { (*pbCounter) ++;  }
	else 														  { (*pbCounter) = 0; }

	if((*pbCounter) >= bFilter) { (*pbCounter) = 0; return(true); }
	else 						{ return(false); }
}
...
{
	if((wCompareData > wHighRange) || (wCompareData < wLowRange)) { (*pbCounter) = 0; }
	else 														  { (*pbCounter) ++;  }
...
{
	if(wCompareData > wHighLever) { (*pbCounter) ++;  }
	else 						  { (*pbCounter) = 0; }
...
INT8U swOverLevelChk(INT16S wCompareData, INT16S wHighLever, INT8U bFilter, INT16U *pbCounter)
{
	if(wCompareData > wHighLever) { (*pbCounter) ++;  }
	else 						  { (*pbCounter) = 0; }
...
{
	if(wCompareData < wLowLever) { (*pbCounter) ++;  }
	else 						 { (*pbCounter) = 0; }
...
INT8U swUnderLevelChk(INT16S wCompareData, INT16S wLowLever, INT8U bFilter, INT16U *pbCounter)
{
	if(wCompareData < wLowLever) { (*pbCounter) ++;  }
	else 						 { (*pbCounter) = 0; }
