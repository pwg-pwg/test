		...
		if(event&(1<<eInvLoadOPSinZero))
		{
			...
			if(swGetRInvHWOverCurrPCnt() > 10)
			{
				if(   (suwGetFBInvCtrlSts() == cFBInvCtrlVolt || suwGetFBInvCtrlSts() == cFBInvCtrlBus)
				   && g_uwWorkMode != cPowerOnMode )
				{
					// if(++wInvHardOCPPoriodCnt >= 2) // 20250116--出现逆变硬件过流连续两个周期(每个周期超过10个中断周�???)，触发硬件过流保护�?
					// {
			...
			if(sRInverterNegPowChk((INT16S)g_dwRInvWatt, -(cNegPowerLevel1), -(cNegPowerLevel2), 3)) // ���������ж���ֵΪ����ʵ�33.3%��22.2%��
			{
...
//	{
					...
					|| ((g_uwWorkMode == cLineMode || g_uwWorkMode == cBypassMode) && g_LineModeJoinInWay == cLineModeJoinInByGrid) ) )
		{
            ...
            g_uwInvTraceSource = cLineModeJoinInByGrid;
			    ...
			    && g_uniInputStatus2.BIT.uwGenAbnormal != 2 && g_uwWorkMode != cPowerOnMode )//锁相发电�???
		{
			wInvPeriodLast = swGetSinePeriodCntNew();
...
//	}
    ...
    //------------- 智能端口系列：跟踪源改变，需要额�???500mS以判定锁相是否完�??? --------------------//
    if(g_uwWorkMode == cBatteryMode)
    {
...
//no.2 电池最大电�???*电压作为功率，折算到逆变电流,结合降额因素以及电网最大流�???50A，得到g_dwInvChgLimit 0~500(50A)，属于逆变侧流�???
	...
	else if(mChkOPRlyOn() && ((swGetEEOPPriority() != cOutputUtilityFirst) || subGetParaBatOpenSts()))
	{
		...
		dwPowerTemp = (g_dwOPWattFilter + g_dwSmartOPWattFilter) * 108; // 20250124--补充智能负载带载，折算PV并网功率。效�???92.6%，转化为1.08�???
...
//no.3 PV和Bat优先供负载，需要逆变输出功率：当前负载功�???/电网电压，得到逆变输出的电�???  g_dwPVFeedLoadLimit
...
//no.5  并网馈电环路最大值限幅软�??? gi_wInvFeedCurrLimit = (INT16S)g_dwPVFeedLoadLimit;
		...
		if(g_dwInvChgLimit < gi_wInvChgCurrLimit)
		{
			...
			//if(!g_wAgingMode) // 20250610--实测老化时，由于g_dwInvChgLimit为负值，加入以下幅值会导致并网电压外环的输出限幅都为负数，在电网电压波动时就会出现发散波动，导致逆变过流�?
			//{ gi_wInvChgCurrLimit = g_dwInvChgLimit;} // 20250328--此处修改避免：市电充电时，充�??240A下限�??10A充电电流，再放开�??240A，充电电流会立即满足，应力可能很大�?
		}
		else
		{
			gi_wBatChgInvLimit = gi_wInvChgCurrLimit;
		}
//no.6  并网充电当前电流值取�???              gi_wBatChgInvLimit = g_dwInvChgLimit;
	}
	else
	{
...
void sEnergyInfoUpdate(void)
{
	if(!g_uwSolarLoss && !g_wSolarPowerWeak && subGetBatChrgEnable()) // PV正常 �?? 电池允许充电
	...
	if(   g_uwLoadOnSts 												// 开�??
	   ...
	   && (   swGetEEOPPriority() != cOutputUtilityFirst 				// SBU �?? SUB
	       || subGetParaBatOpenSts()					 				// 电池开�??
		   || (!subGetBatChrgEnable() && !subGetBatDischrgEnable()) )	// 电池禁止充电 �?? 禁止放电
		)
	{
...
#define DrtProcWay  0  // 降额方式: 0=增减控制; 1=PI控制
INT16S OpVoltDrt = 0;
...
INT16S sOutputVoltDrtProc(INT16S wOutputVoltRated)
{
    ...
    if(g_uwParaMode == cMasterMode)
    {
        if(g_uwWorkMode == cBatteryMode)
        {
            ...
            {
                #if(cBattCurrDerate == 1)
                // g_dwBatAvgCurrSum += g_uwBatVoltAvg; // 放大10�??
                g_dwBatAvgCurrSum += g_wDCDCCurrAvg; // 放大10�??
                #endif
                ...
                if(++g_wBatAvgVoltCnt >= 4) //4 = 80mS //8 = 160mS
                {
                    ...
                    #if(DrtProcWay == 0)
                    {
                        ...
                        //20241216--方案1：实测采用此方法，电池电流会240A±3A波动，满载下输出电压又波动，但不会超2V，稳定性差，为了确保输出不降额采用方案1.
                        if(0 == g_wSolarSigPowerLoad) // 仅电池下逆变时，限定电池放电电流240A
                        {
                        ...
                        // 2. 过载降额
                        // 3. 过温降额
                        // 4. 仅单路PV带载，输出降额处理功�?? -- add by 20250212
                        //------------------------------------
                        if(    (wDerateWatt == -1)
                    ...
                    #else
                    {
                        //----------------- ②Limit Ibat to 240A --------------------//
                        //When the battery Current is lower than the rated battery curr, reduce the output power
                        if(0 == g_wSolarSigPowerLoad) // 仅电池下逆变时，限定电池放电电流240A//20241216--方案2：实测采用此方法，电池电流稳定控制不超过240A，满载下输出电压会跌落到226V但稳定，为了确保输出不降额采用方�??
                        {
...
// 加入BUS快速低压标志，规避主机因为BUS快速低压，先进入了g_uwMasterWorkMode和g_uwWorkMode进入待机态，导致误判�??? -- add in 20250225
void sInputOutputRvsChk(INT8U bFilter)
{
    ...
    if(    !mChkOPRlyOn()
        ...
        && g_uwWorkMode != cFaultMode && !g_wBusRealVoltLowFlg )  //避免并机的时候报反接, 避免主机因为BUS快速低压退出导致误�???
    {
        if(sbOverLevelChk(g_uwROPVoltAvg, 1000, bFilter, &s_uwChkCnt)) // 输出加入Y电容，可以将异常180Vrms电压改为110Vdc电压，有效值小�???10Vrms�???
        {
    ...
    //------ 智能负载端口反接检�??? ------//
    // 20241208 �???54V电池，满足智能负载条件，输出后再接入发电机，会存在智能负载开启和发电机共通的问题�???
    if(    swGetEESmartPortType() == cOutPort && !mChkSmart_Port_Rly_On()
        && (   (!gi_wParallelEnable) // 单机是全模式下检�???
            || (g_uwMasterWorkMode != cLineMode && g_uwMasterWorkMode != cBypassMode && g_uwMasterWorkMode != cBatteryMode)
            )
...
void sInvPowerCal(void)
{
    ...
    if(g_uwAcInputPowerConversion < 0)          //不允许出现馈�???
    {
    ...
    if(g_uwWorkMode == cLineMode)
    {
        ...
        else//有电�???
        {
            ...
            {
                g_uwPVPowerConversion = 0;
                //rule2:LMF  20230707  电池出功率小�???100W处理�???0(售后问题�???40W左右)
                if(abs(g_dwInvWatt) < 100)//电池放电到SOC点之后，停止，但是功率折算出一定的数值导致电池变成充电，避免小功率出问题   20230629  
                {
                    g_wBatPowerConversion = 0;
                    g_dwLoadPowerConversion = g_dwOPWatt; 
                    g_dwSmartOPPowerConversion = g_dwSmartOPWatt;
                    g_uwAcInputPowerConversion = g_dwLoadPowerConversion + g_dwSmartOPPowerConversion;//逆变小功率，判定为电池未输出功率，电网功�???=负载功率
                }
                else
                {
                    g_wBatPowerConversion = (INT16S)((INT32S)g_uwPVPowerConversion - g_dwInvWatt);//电池=逆变功率，不考虑空载损�?
            ...
            {
                //rule2:LMF  20230707  电池功率小于100W处理�???0(售后问题�???40W左右)
                ...
                //防止刚卸载时出现比较大的馈电功率全叠加在负载�???(实测由于PV功率下降比较慢，会维�???7s左右)
                ...
                //INV整流但市电功率小�???80W�???(处理�???0)，这部分能量要从电池充电中处理掉，防止输出出现负功率
