...
INT16S gi_wFeedChgPowerLimit = 133;//136;
...
void sDCDCControl(INT16U *uwDCDCCtrlSts)
{
	...
	if(++g_wLoopCnt >= 8)
	{
		...
		else if(g_uwFBInvCtrlSts != cFBInvCtrlBus)
		{
			...
			{
				if(g_wLoadCurrAvg >= (g_wLoadCurrAvgBack + 100))
				{ dwLoadCurrForwardTemp = ((INT32S)g_wLoadCurrAvg * 8 * 26 >> 8); } // 基于V4006基础上添加前馈，加速给定增加.  --20250416 采用26还是可能触发LLC过流保护。
				else if(dwLoadCurrForwardTemp >= 2) { dwLoadCurrForwardTemp -= 2; }
