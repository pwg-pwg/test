...
INT16U	g_uwConvertVoltAvg;//LLC高压侧电压
...
INT8U	g_fBatVoltDanger = 0;
INT8U	g_fBatSocDanger = 0;
...
extern INT16U g_uwBMSDischgOverCurFlg;

void sBatteryModuleInit(void)
{
	g_wBatUnderVolt = swGetEEBatUnderVolt();
	g_wBatUnderBackVolt = swGetEEBatUnderBackVolt();
	g_wBatLowVolt = g_wBatUnderVolt + (cBatVoltReal0V5*cBatSerialPcs);//电池低压告警阈值 为 低压保护阈值提高（0.5V*电池节数）
	g_wBatLowBackVolt = g_wBatLowVolt + (cBatVoltReal0V5*cBatSerialPcs);//电池低压告警恢复阈值 为 电池低压告警阈值提高（0.5V*电池节数）
...
void sConvertVoltAvgAdj(INT16U uwBatVolt)//LLC高压侧电压
{
...
void sBatUnderChk(INT16U uwFilter)
{
	...
	if(g_uwBatType == cBatVoltType)	
	{
		...
		else
		{
			...
			if(g_wBatPowerDownTemp < (cBatVoltReal10V5 * cBatSerialPcs))
			{ 
				// g_wBatPowerDownTemp = ((cBatVoltReal10V5 * cBatSerialPcs) + (cBatVoltReal0V5 * cBatSerialPcs));
				g_wBatPowerDownTemp = (cBatVoltReal11V * cBatSerialPcs);
			}
			//恢复值为电池低压的恢复点-2V
	...
	else
	{
		...
		if(!g_fBatOpen)
		{
			if(!g_fBatSocDanger && fBMSConnectFlg)
			{
				if(sbUnderLevelChk(mBMSBatSOC, g_wBatUnderSoc, 3000, &s_uwBatDangerChkCnt))
				{
					g_fBatSocDanger = 1;
				}
			}
			...
			{
				if(   sbOverLevelChk(mBMSBatSOC, (g_wBatUnderSoc + 1), 30000, &s_uwBatDangerChkCnt)  //维持10min
				   || g_wBatChgStage == cBatChgFloatStage
				   || g_fBMSSOCFull )
				{
					s_uwBatVoltUnderChkCnt = 0;
	...
	if(g_fBatVoltDanger || g_fBatSocDanger)
	{
	    g_fBatDanger = 1;
	}
	else
...
void sBatWeakChk(INT16U uwFilter)
{
	...
	if(g_uwBatType == cBatVoltType)
	{
		...
		else
		{
			...
			if(   !g_fBatUnder
			   && (   sbOverLevelChk(g_uwBatVoltAvg, wBatWeakBackVolt, uwFilter, &s_uwBatWeakChkCnt)
			       || g_wBatChgStage == cBatChgFloatStage 
	...
	else
	{
		...
		{
			if(    !g_fBatUnder
				&& (   sbOverLevelChk(mBMSBatSOC, g_wBatWeakBackSoc, uwFilter, &s_uwBatWeakChkCnt)
			        || g_wBatChgStage == cBatChgFloatStage 
			        || g_fBMSSOCFull
			        || s_uwBatVoltOverCnt >= 500 )
			   )
	...
	if(!g_fBatWeak)
	{
		if(g_fBatSocWeak || g_fBatVoltWeak /*|| g_uwBMSDischgOverCurFlg*/)
		{
			g_fBatWeak = true;
...
void sBatOpenChk(INT16U uwBatOpenLevel, INT16U uwFilter)
{
	...
	if(!g_fBatOpen)
	{
		if(sbUnderLevelChk(g_uwBatVoltAvg, uwBatOpenLevel, uwFilter, &s_uwBatChkCnt))//34V
		{
			g_fBatOpen = 1;
		}
	}
	...
	{
		if(sbOverLevelChk(g_uwBatVoltAvg, uwBatOpenLevel + (cBatVoltReal1V*cBatSerialPcs), uwFilter, &s_uwBatChkCnt))//38V
		{
			g_fBatOpen = 0;
		}
	}
...
INT8U subGetBatOpenSts(void) //电池开路
{
	return g_fBatOpen;
}
