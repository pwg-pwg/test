...
//            if(sbUnderLevelChk(g_uwSolarVolt1Avg, (g_uwSolarHighMPPTPoint - 50),uwFilter, &s_uwPV1BypassChkCnt))
            if(sbUnderLevelChk(g_uwSolarVolt1Avg, 4000,uwFilter, &s_uwPV1BypassChkCnt))
            {
...
//            if(sbOverLevelChk(g_uwSolarVolt1Avg, g_uwSolarHighMPPTPoint, uwFilter, &s_uwPV1BypassChkCnt))
            if(sbOverLevelChk(g_uwSolarVolt1Avg, 3950, uwFilter, &s_uwPV1BypassChkCnt))
            {
...
void sSolarLimitCurrUpdate(INT16U uwBoost1Sts, INT16U uwBoost2Sts)
{
	...
	if(--wDelayCnt == 0)
	{
		...
		if(wSolarLimitCurrMin > 1760)//PV旁路限制PV功率无意义，由DCDC/输出限制
		{
			wSolarLimitCurrMin = 1760;//15A
			//if(g_uwSolar1BypassFlag) wSolarLimitCurrMin = 1760;//15A
	...
	//PV功率 大于500W时 0.05A/20ms   -- 400m/1A
...
void sSolarPowerAbnormalChk(void)
{
	...
	if(suwGetBoost1CtrlSts() >= cBoostMppt)
	{
	    ...
	    if(   (g_uwSolarCurr1Avg > g_wPv1CurrLimit+200)
			//&&(g_wR_Boost1_PWM == 1)
			&&((INT32S)g_uwSolarPower1Avg*10 > (g_dwTotalPower*10+5000))
		)//多1A且PWM达到极限
	    {
		    if(++s_uwSolar1PowerAbnormalChkCnt > 600)//10s
		    {
