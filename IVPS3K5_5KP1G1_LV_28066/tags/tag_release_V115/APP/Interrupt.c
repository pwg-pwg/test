...
//		}
	...
	else if(bPVMode == cLineMode )
	{
		...
		if(wBatVoltReal >(mEEBatVoltOverShut()-swGetBatteryPcs()*cBat0v7))//当电池电压>(最大设定电压-电池结束*0.7A)过压保护,60.2V
		{
			ubBatVoltOveCnt ++;
			if(wChgPwmLimit > 5)
			{
		...
		if((abs(wInvLCurrSample1) > g_wInvLCurrLineLevel2)//40*25=960 
			...
			||(wBatAvgVoltNew > mEEBatVoltOverShut())||(ubBatVoltOveCnt > 5)\
...
//	static INT16U wInvVoltAdjCnt = 0;
	...
	if((bPVMode == cBatteryMode)||((bPVMode == cStandbyMode )&&(wFBControlStatus == cFBInverterControl)))
	{
		if(abs(wInvVoltReal)  >= cVac198v) //140*1.414 =198V if(abs(wInvVoltReal)  >= cVac396v)        //280*1.414=395.92v     cVac406v   Rms 287V   //amy20121213     cVac380v
		{
...
INT32S sdwInvWattCal(void)
{
	...
	OS_ENTER_CRITICAL();
	dwRInvWattRealSum = dwRInvWattRealSumTemp;//=市电电压*逆变电流；wLineVoltReal * wInvLCurrReal
...
void HardProtectChk(void)
{
	if((LiHardProtect==0)||((liparaTx==0)&&(mEepromOutputMode() > cOP_Single)))//纭欢瑙﹀彂淇濇姢
	{
//		LiHardProtectCNT++;
