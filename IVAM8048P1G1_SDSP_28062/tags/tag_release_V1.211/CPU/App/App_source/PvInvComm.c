...
#define MDSP_TO_SDSP_FREQ_SCALE        (float)0.78125f  // MDSP到SDSP 的频率系数=0.78125=15K/19.2K
...
void IntSci_Task_20ms(void)
{
	...
	strSlvState.ubState3.bit.MdlIdentifyFail = g_SysFault.ubFault.bit.MdlIdentifyFail;	
...
void IntSci_CommandDeal(void)
{
	...
	switch( uiIntSciComID )
	{
		...
		case eIntComUploadRealData:   // 现在用这个
		{
			...
			//首次接收主机发过来的机型信息，和自身的做对比(升级从DSP后也在这里做对比)
			if(!unIntSciFlag.bit.bOnOffToggle)
			{
			    if(bMachineModel != uiMasterAnalogData[eM2SRealData_Model])
			    {
...
//		g_SystemInfo.fPv2MaxPowerSet = g_SystemInfo.fPvMaxPowerSetRef * 0.5;
	...
	if(0 != uiMasterAnalogData[eEM2SRealData_PeriodSample])
	{
	    stValue.ui20msPeriodSamplePoint = (int16)((float)uiMasterAnalogData[eEM2SRealData_PeriodSample] * MDSP_TO_SDSP_FREQ_SCALE);
	}
