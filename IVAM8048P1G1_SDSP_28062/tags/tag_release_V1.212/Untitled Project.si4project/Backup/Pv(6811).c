...
#define  MPPTGRADEB		 2
#define  MPPTGRADEC		 3
#define  MPPTGLOBALSCAN	 4

#define  SCANINITIAL	 0
#define  SCANPREPARE	 1
#define  SCANNING	     2
#define  SCANFINISH 	 3
...
Struct_GlobalScanData strGlobalScanPv1;
Struct_GlobalScanData strGlobalScanPv2;
...
void Pv_MpptSearching(Struct_MpptCtrlData *pstrMpptControlData,Struct_GlobalScanData *pstrGlobalScanData,float *pPvVoltRef,float fPvRealVolt);
void Pv_GolbalScanCheck(Struct_MpptCtrlData *pstrMpptControlData,Struct_GlobalScanData *pstrGlobalScanData,float *pPvVoltRef);
...
void Pv_System_AppInit(void)
{
    ...
    //总pv功率环
    g_strPvMaxPowerLoop.Kp = 0.3;      //0.12;
    g_strPvMaxPowerLoop.Ki = 0.0001;   //0.00004
    ...
    // Pv1 Power Loop
    g_strPv1PowerLoop.Kp = 0.3;     //0.12;
    g_strPv1PowerLoop.Ki = 0.0001;  //0.00004;
...
//	static int16 Pv1BusChkCnt = 0,Pv1BusLimitFlg = FALSE;
    ...
    UpDownLimit(fPv1CurrLoopRefTemp,cPvCurrMax,0);  // about 26A Set_PvCurr(26)
    
    //=======================================================================
    // PV1 Ctrl Pv Curr Loop
    //=======================================================================
    if(TRUE == strMstFuction.ubFuction.bit.bATEPv1CurrContrl)
    {
         g_strPv1VoltLoop.integrator = 0; 		  //清积分，根据主DSP下发的功率控电流
		 g_strPv1VoltLoop.Out = 0;	
	     g_strPv1CurrLoop.Ref = g_PvVar.fATEPv1CurrRef;
    }
    else  if((MPPTGLOBALSCAN == strMpptCtrlData_Pv1.uiMpptSearchState) && (SCANPREPARE == strGlobalScanPv1.uiGlobalScanState))
	{
	     g_strPv1VoltLoop.integrator = 0;         //清积分，避免从开路电压开始扫描时有占空比突然增加又减小
		 g_strPv1VoltLoop.Out = 0;	
	     g_strPv1CurrLoop.Ref = cPvCurrRef0_5A;   //通过将PV电流限制到0.5A来获取开路电压
    }
	else
	{
...
//        if((Pv2BusLimitFlg == TRUE)&&(stADC.fRealScale.VBus - g_strPv2BusVoltLoop.Ref > 0.125))  // 0.0375*800 Set_BusVolt(40)
	...
	else  // 独立跑 则需要重新跑电压环
	{
		...
		UpDownLimit(fPv2CurrLoopRefTemp,cPvCurrMax,0);   // about 26A Set_PvCurr(26)
	
		//=======================================================================
		// PV2 Ctrl Pv Curr Loop
		//=======================================================================
	    if(TRUE == strMstFuction.ubFuction.bit.bATEPv2CurrContrl)
	    {
	         g_strPv2VoltLoop.integrator = 0; 		  //清积分，根据主DSP下发的功率控电流
			 g_strPv2VoltLoop.Out = 0;	
		     g_strPv2CurrLoop.Ref = g_PvVar.fATEPv2CurrRef;
	    }
		else if((MPPTGLOBALSCAN == strMpptCtrlData_Pv2.uiMpptSearchState) && (SCANPREPARE == strGlobalScanPv2.uiGlobalScanState))
		{
		     g_strPv2VoltLoop.integrator = 0;         //清积分，避免从开路电压开始扫描时有占空比突然增加又减小
			 g_strPv2VoltLoop.Out = 0;	
		     g_strPv2CurrLoop.Ref = cPvCurrRef0_5A;   //通过将PV电流限制到0.5A来获取开路电压
	    }
		else
		{
...
void Pv_PvPowerSoftStart(void)
{
    ...
    if(ePvRunState_MpptCtrl == g_PvVar.PvFlag.bit.PvRunSts && g_PvVar.PvFlag.bit.Pv1Enable)         // max = 13.6us real = 13.6us
    {
        if(   (fabs(g_strPv1PowerLoop.Ref - g_PvVar.fPv1CtrlPowerSet) <= cPvPowerSoftRate*2)
			||(g_strPv1PowerLoop.Ref > g_PvVar.fPv1CtrlPowerSet)) //考虑要不要做个差值，比如充电电流补偿那部分  todo
        {
        ...
        else
        {
            ...
            {
                g_strPv1PowerLoop.Ref += cPvPowerSoftRate;//2.6w/5ms;6500w/10s
            }
            else
            {
...
void Pv_PvMpptSoftStart(void)
{
    float32  fPv1VoltStep,fPv1VoltDiff;  
    float32  fPv2VoltStep,fPv2VoltDiff; 
    ...
    if(ePvRunState_MpptCtrl == g_PvVar.PvFlag.bit.PvRunSts)
	{
		...
		{
		    if((MPPTGLOBALSCAN == strMpptCtrlData_Pv1.uiMpptSearchState) && (SCANFINISH == strGlobalScanPv1.uiGlobalScanState))
		    {
		        fPv1VoltStep = 0.5;
			    fPv1VoltDiff = 1; 
		    }
			...
			if(   (fabs(g_PvVar.fPv1MpptVoltRef - g_PvVar.fMppt1VoltRefSet) < Set_PvVolt(fPv1VoltDiff))
			    ||((MPPTGLOBALSCAN == strMpptCtrlData_Pv1.uiMpptSearchState) && (SCANNING == strGlobalScanPv1.uiGlobalScanState))
			  )
			{
			...
			else
			{
				...
				{
				    g_PvVar.fPv1MpptVoltRef += Set_PvVolt(fPv1VoltStep);
				}
				else
				{
				    g_PvVar.fPv1MpptVoltRef -= Set_PvVolt(fPv1VoltStep);
    ...
    if(ePvRunState_MpptCtrl == g_PvVar.PvFlag.bit.PvRunSts)
    {
		...
		{
		    if((MPPTGLOBALSCAN == strMpptCtrlData_Pv2.uiMpptSearchState) && (SCANFINISH == strGlobalScanPv2.uiGlobalScanState))
		    {
		        fPv1VoltStep = 0.5;
			    fPv1VoltDiff = 1; 
		    }
			...
			if(   (fabs(g_PvVar.fPv2MpptVoltRef - g_PvVar.fMppt2VoltRefSet) < Set_PvVolt(fPv2VoltDiff))
				||((MPPTGLOBALSCAN == strMpptCtrlData_Pv2.uiMpptSearchState) && (SCANNING == strGlobalScanPv2.uiGlobalScanState))
		    )
			{
			...
			else
			{
				...
				{
				    g_PvVar.fPv2MpptVoltRef += Set_PvVolt(fPv2VoltStep);
				}
				else
				{
				    g_PvVar.fPv2MpptVoltRef -= Set_PvVolt(fPv2VoltStep);
...
/******************************************************************************
Function Name       : ATE_PvCurrSoftStart
Function Descriptions:
Parameter Name list :
ReturnType          :
Refer doctuments    :
...
{
	//只在ATE的时候使用
    if((TRUE == strMstFuction.ubFuction.bit.bATEPv1CurrContrl) && (ePvRunState_MpptCtrl == g_PvVar.PvFlag.bit.PvRunSts))
	{
		g_PvVar.fATEPv1CurrRefSet = g_strPv1PowerLoop.Ref / stADC.fRealScale.VoltPv1;  
		if( (TRUE == g_PvVar.PvFlag.bit.Pv1MpptEn) && (FALSE == g_PvVar.PvFlag.bit.Pv1PowerDerate) )
		{
			if(fabs(g_PvVar.fATEPv1CurrRef - g_PvVar.fATEPv1CurrRefSet) < Set_PvCurr(0.04))
			{
				g_PvVar.fATEPv1CurrRef = g_PvVar.fATEPv1CurrRefSet;
			}
			else
			{
				if(g_PvVar.fATEPv1CurrRef < g_PvVar.fATEPv1CurrRefSet)
				{
				    g_PvVar.fATEPv1CurrRef += Set_PvCurr(0.02);  //20A需要5s左右
				}
				else
				{
				    g_PvVar.fATEPv1CurrRef -= Set_PvCurr(0.02);
				}

			}
			UpDownLimit(g_PvVar.fATEPv1CurrRef,g_strPv1CurrLoop.integTopLimit,0); //PV1电流给定范围
		}
    }
    else
    {
        g_PvVar.fATEPv1CurrRef = 0; 
    }


    if((TRUE == strMstFuction.ubFuction.bit.bATEPv1CurrContrl) && (ePvRunState_MpptCtrl == g_PvVar.PvFlag.bit.PvRunSts))
    {
    	g_PvVar.fATEPv2CurrRefSet = g_strPv2PowerLoop.Ref / stADC.fRealScale.VoltPv2;  
		if( (TRUE == g_PvVar.PvFlag.bit.Pv2MpptEn) && (FALSE == g_PvVar.PvFlag.bit.Pv2PowerDerate))
		{
			if(fabs(g_PvVar.fATEPv2CurrRef - g_PvVar.fATEPv2CurrRefSet) < Set_PvCurr(0.04))
			{
				g_PvVar.fATEPv2CurrRef = g_PvVar.fATEPv2CurrRefSet;
			}
			else
			{
				if(g_PvVar.fATEPv2CurrRef < g_PvVar.fATEPv2CurrRefSet)
				{
				    g_PvVar.fATEPv2CurrRef += Set_PvCurr(0.02);
				}
				else
				{
				    g_PvVar.fATEPv2CurrRef -= Set_PvCurr(0.02);
			...
			UpDownLimit(g_PvVar.fATEPv2CurrRef,g_strPv2CurrLoop.integTopLimit,0); //限制PV2快速扫描范围
		}
    }
    else
    {
        g_PvVar.fATEPv2CurrRef = 0;
    }
}
...
void Pv_PvPowerAvgCalc(void)
{
	...
	static Uint16 uiAvgCalcStartCnt,uiAvgCalcEndCnt;
	static float  fAvgCalDiv;

//	if(TRUE == strMpptCtrlData_Pv1.uiMpptPvInputDataAvgCalc || TRUE == strMpptCtrlData_Pv2.uiMpptPvInputDataAvgCalc)
//	{
////	    uiAvgCalcStartCnt = 41;   //820ms
////		uiAvgCalcEndCnt = 80;	  //80+1->1620ms
////		fAvgCalDiv = 0.025;		  //1/40
////		uiDataAvgCalcEnable = 1;
//		uiAvgCalcStartCnt = 21;   //80ms        //测试先改成1s
//		uiAvgCalcEndCnt = 40;	  //7+1->160ms
//		fAvgCalDiv = 0.05;		  //1/20
//	}
//	else if(TRUE == strGlobalScanPv1.uiPvDataAvgCalc || TRUE == strGlobalScanPv2.uiPvDataAvgCalc)
//	{
//        //200ms
////      uiAvgCalcStartCnt = 4;    //80ms
////		uiAvgCalcEndCnt = 7;      //80ms
////		fAvgCalDiv = 0.25;  	  //1/4
////		uiDataAvgCalcEnable = 1;
////空闲至少要保留40ms，不然可能会有计算值未清0的问题
////      //300ms
//	    uiAvgCalcStartCnt = 6;    //120ms
//		uiAvgCalcEndCnt = 12;     //140ms
//		fAvgCalDiv = 0.142857;    //1/7
//
////先改成1s看效果
////		uiAvgCalcStartCnt = 21;	  //80ms
////		uiAvgCalcEndCnt = 40;	  //7+1->160ms
////		fAvgCalDiv = 0.05;		  //1/20
////		uiDataAvgCalcEnable = 1;
//	}
	
	if( TRUE == strMpptCtrlData_Pv1.uiMpptPvInputDataAvgCalc || TRUE == strGlobalScanPv1.uiPvDataAvgCalc )
	{
		...
		if(++uiPv1PowerCalcCnt > uiAvgCalcStartCnt)  //21	//400ms
		{
			...
			if(uiPv1PowerCalcCnt > uiAvgCalcEndCnt)	//40 //400ms
			{
				strInputPowerAvgForPv.fPv1 = fPv1PowerTemp * fAvgCalDiv;  //0.05;
				strInputVoltAvgForPv.fPv1  = fPv1VoltTemp  * fAvgCalDiv;  //0.05;
				strMpptCtrlData_Pv1.uiMpptPvInputDataAvgCalc = FALSE;
				strGlobalScanPv1.uiPvDataAvgCalc = FALSE;
				fPv1PowerTemp     = 0;  //避免扫描的时候剩余时间不够未清0
				fPv1VoltTemp      = 0;
				uiPv1PowerCalcCnt = 0;
			}
		}
	...
	if( TRUE == strMpptCtrlData_Pv2.uiMpptPvInputDataAvgCalc || TRUE == strGlobalScanPv2.uiPvDataAvgCalc )
	{
		...
		if(++uiPv2PowerCalcCnt > uiAvgCalcStartCnt)  //21	//400ms
		{
			fPv2PowerTemp += stPower.fPv2Scale;
			fPv2VoltTemp  += stValue.fAveScale.VoltPv2;
			if(uiPv2PowerCalcCnt > uiAvgCalcEndCnt)	//40  //400ms
			{
				strInputPowerAvgForPv.fPv2 = fPv2PowerTemp * fAvgCalDiv;  //0.05;;
				strInputVoltAvgForPv.fPv2  = fPv2VoltTemp  * fAvgCalDiv;  //0.05;
				strMpptCtrlData_Pv2.uiMpptPvInputDataAvgCalc = FALSE;
				strGlobalScanPv2.uiPvDataAvgCalc = FALSE;
				fPv2PowerTemp     = 0;
				fPv2VoltTemp      = 0;
				uiPv2PowerCalcCnt = 0;
			}
...
//	static Uint16 uiGlobalScanEnPre = 0;
//	static Uint16 uiDelyPeriod = 3;
	static Uint16 uiPv1MpptTimeCnt = 0;
	static Uint16 uiPv2MpptTimeCnt = 0;
	Uint16 uiPv1MPPTPeriod = 10;
	Uint16 uiPv2MPPTPeriod = 10;

	//===============================================================
	//Pv Mppt/Scan Period 
	//===============================================================

	if(MPPTGLOBALSCAN == strMpptCtrlData_Pv1.uiMpptSearchState)
	{
		if(SCANFINISH == strGlobalScanPv1.uiGlobalScanState)
		{ 
			uiPv1MPPTPeriod = 40; //4s
		}
		else
		{
			uiPv1MPPTPeriod = 3; //300ms
		}
    }
	else
	{ 
		uiPv1MPPTPeriod = 10;	 //1s
	} 

	if(MPPTGLOBALSCAN == strMpptCtrlData_Pv2.uiMpptSearchState) 
	{
		if(SCANFINISH == strGlobalScanPv2.uiGlobalScanState) 
		{
			uiPv2MPPTPeriod = 40;
		}
		else	 											 
	...
	//Pv Mppt
	//===============================================================
	if(++uiPv1MpptTimeCnt >= uiPv1MPPTPeriod)
	{
	    uiPv1MpptTimeCnt = 0;
...
//				 if((TRUE == strMstFuction.ubFuction.bit.bMPPTMultiPointEn) && (strGlobalScanPv1.fMaxPower != 0))
//				{
//					strGlobalScanPv1.fDeltaPower = abs(g_PvVar.fInputPowerforMppt - strGlobalScanPv1.fMaxPower) / strGlobalScanPv1.fMaxPower;
//				}
				bTest[0]++;
				//目前是扫描过程中如果使能->不使能立即生效，要不要等扫描完再停止？TODO
				Pv_GolbalScanCheck(&strMpptCtrlData_Pv1,&strGlobalScanPv1,&g_PvVar.fMppt1VoltRefSet);
				Pv_MpptSearching(&strMpptCtrlData_Pv1,&strGlobalScanPv1,&g_PvVar.fMppt1VoltRefSet,(stValue.fAveScale.VoltPv1 - Set_PvVolt(4)));//往左边扰动4V开始追
			}
			else
			{
				strMpptCtrlData_Pv1.uiMpptSearchState = MPPTINITIAL;
				strGlobalScanPv1.uiGlobalScanState = SCANINITIAL;
		...
		else	//PV充足，顶母线后不作MPPT
		{
			strMpptCtrlData_Pv1.uiMpptSearchState = MPPTINITIAL;
			strGlobalScanPv1.uiGlobalScanState = SCANINITIAL;
			strGlobalScanPv1.uiTimeScanCnt = 0;  //顶母线说明不需要扫描，清0计数器
//			strGlobalScanPv1.fMaxPower = 0;      //重启扫描需要
		}
	}

	if(++uiPv2MpptTimeCnt >= uiPv2MPPTPeriod)
	{
		uiPv2MpptTimeCnt = 0;
...
//				if((TRUE == strMstFuction.ubFuction.bit.bMPPTMultiPointEn) && (strGlobalScanPv2.fMaxPower != 0))
//				{
//					strGlobalScanPv2.fDeltaPower = abs(g_PvVar.fInputPowerforMppt - strGlobalScanPv2.fMaxPower) / strGlobalScanPv2.fMaxPower;
//				}
				Pv_GolbalScanCheck(&strMpptCtrlData_Pv2,&strGlobalScanPv2,&g_PvVar.fMppt2VoltRefSet);
				Pv_MpptSearching(&strMpptCtrlData_Pv2,&strGlobalScanPv2,&g_PvVar.fMppt2VoltRefSet,(stValue.fAveScale.VoltPv2 - Set_PvVolt(4)));
			}
			else
			{
				strMpptCtrlData_Pv2.uiMpptSearchState = MPPTINITIAL;
				strGlobalScanPv2.uiGlobalScanState = SCANINITIAL;
		...
		else	//PV充足，顶母线后不作MPPT
		{
			strMpptCtrlData_Pv2.uiMpptSearchState = MPPTINITIAL;
			strGlobalScanPv2.uiGlobalScanState = SCANINITIAL;
			strGlobalScanPv2.uiTimeScanCnt = 0;
//			strGlobalScanPv2.fMaxPower = 0;      //重启扫描需要			
		}
	}
		//===============================================================
		//Inv Mppt
...
/******************************************************************************
Function Name		 : Pv_GolbalScanCheck
Function Descriptions:
Parameter Name list  :
ReturnType           :
Refer documents      :
...
void Pv_GolbalScanCheck(Struct_MpptCtrlData *pstrMpptControlData,Struct_GlobalScanData *pstrGlobalScanData,float *pPvVoltRef)
{
    if(TRUE == strMstFuction.ubFuction.bit.bMPPTMultiPointEn)
    {
	    //30min，全局扫描的时候不累计时间
    	if(pstrMpptControlData->uiMpptSearchState != MPPTGLOBALSCAN)
    	{
    	    if(   (++pstrGlobalScanData->uiTimeScanCnt >= cDelay20s_2s)   //cDelay30min_2s
				/*||(pstrGlobalScanData->fDeltaPower > 0.1)*/)  //功率突变超过10%重启扫描，屏蔽。启用需要处理PV功率充足时记录的最大功率比实际需要功率大的问题
    	    {
    	    	pstrGlobalScanData->uiTimeScanCnt = 0;
				pstrGlobalScanData->fDeltaPower = 0;
				pstrGlobalScanData->fMaxPower = 0;    		
    			pstrGlobalScanData->fBestVoltage = 0;
				pstrGlobalScanData->uiScanInterruption = 0;
				pstrGlobalScanData->uiGlobalScanState = SCANINITIAL;
    	    	pstrMpptControlData->uiMpptSearchState = MPPTGLOBALSCAN;
    	    }
    	}
    }
	else
    {
		...
		if(MPPTGLOBALSCAN == pstrMpptControlData->uiMpptSearchState)
		{
			if((SCANNING == pstrGlobalScanData->uiGlobalScanState) &&(pstrGlobalScanData->fBestVoltage != 0))
			{
			    //扫描过程中中断，进到SCANNING直接给定最大功率点，4s后从下面的初始化结束扫描
				pstrGlobalScanData->uiScanInterruption = 1;
			}
			else
			{
				pstrGlobalScanData->fMaxPower = 0;    //清空，才不会计算扫描后的功率变化
			    pstrGlobalScanData->uiGlobalScanState = SCANINITIAL;
...
void Pv_MpptSearching(Struct_MpptCtrlData *pstrMpptControlData,Struct_GlobalScanData *pstrGlobalScanData,float *pPvVoltRef,float fPvRealVolt)
{
	...
	else if(MPPTGLOBALSCAN == pstrMpptControlData->uiMpptSearchState)
	{
	    if(SCANINITIAL == pstrGlobalScanData->uiGlobalScanState)
		{		    
		    //--考虑加长时间，给电流一个缓慢减小的过程 TODO
			pstrGlobalScanData->uiPvDataAvgCalc = TRUE;
			pstrGlobalScanData->uiGlobalScanState = SCANPREPARE;
		}
		else  if(SCANPREPARE == pstrGlobalScanData->uiGlobalScanState)
		{
		    if(TRUE == pstrGlobalScanData->uiPvDataAvgCalc)  //测试是否没计算完就进来了
		    {
		        bTest[1]++;
		    }
		    OpenCircuitVolt = g_PvVar.fInputVoltforMppt;  //测试用，记录开路电压
		    if(g_PvVar.fInputVoltforMppt < Set_PvVolt(300))
		    {
		        fScanDutyUpLimit = 0.8f;
		    }
			else
			{ 
			    fScanDutyUpLimit = 0.7f; 
			}
		    *pPvVoltRef = g_PvVar.fInputVoltforMppt - Set_PvVolt(6);  //从开路电压开始扫描
		    pstrGlobalScanData->uiPvDataAvgCalc = TRUE;
		    pstrGlobalScanData->uiGlobalScanState = SCANNING;			 
		}
		else  if(SCANNING == pstrGlobalScanData->uiGlobalScanState)
		{
		    if(TRUE == pstrGlobalScanData->uiPvDataAvgCalc)  //测试是否没计算完就进来了
		    {
		        bTest[2]++;
		    ...
		    if(g_PvVar.fInputPowerforMppt > pstrGlobalScanData->fMaxPower)
		    {
		        pstrGlobalScanData->fMaxPower = g_PvVar.fInputPowerforMppt;
				pstrGlobalScanData->fBestVoltage = *pPvVoltRef; 	
		    }

			if(pstrGlobalScanData->fMaxPower < 0.2) // about小于1kw 0.27-> 0.0702
			{
				*pPvVoltRef = *pPvVoltRef - Set_PvVolt(6);	//先设定4V，效果不好再考虑变步长
			}
			else
			{
				*pPvVoltRef = *pPvVoltRef - Set_PvVolt(4);	
			}

			//占空比过大或PV电压低于90V停止扫描
		    if((g_PvVar.fPv1Pwm < fScanDutyUpLimit) && (*pPvVoltRef > MpptMinVoltLimit) && (0 == pstrGlobalScanData->uiScanInterruption)) 
		    { 
		        pstrGlobalScanData->uiPvDataAvgCalc = TRUE;
		    }
			else
			{
				if(pstrGlobalScanData->uiScanInterruption)
				{
				    pstrGlobalScanData->uiScanInterruption = 0;
				   	pstrGlobalScanData->fMaxPower = 0;
				}
			    *pPvVoltRef = pstrGlobalScanData->fBestVoltage;
				pstrGlobalScanData->uiGlobalScanState = SCANFINISH;
			}
		}
		else  if(SCANFINISH == pstrGlobalScanData->uiGlobalScanState)
		{
		    //--PV电压给定赋值为最大功率点后，等PV电压回到最大功率点再开始MPPT
		    //--考虑加长时间，给电压一个缓慢回到给定电压的过程 TODO 需要测应力？
		    pstrMpptControlData->uiMpptSearchState = MPPTINITIAL;
			pstrGlobalScanData->uiGlobalScanState = SCANINITIAL;
		}
	}
