...
// Mppt电压范围(90-425V),最大MPPT点由CV点定
#define MpptMaxVoltLimit	1.0625f	// 425V 1.0625 * cVPvRated
#define MpptMinVoltLimit	0.225f	//  90V 0.225  * cVPvRated
...
typedef struct
{
	...
	float32 fATEPv1CurrRef;		// Pv1控制Pv1电流给定（用于Pv电流环）
	float32 fATEPv2CurrRef;		// Pv1控制Pv1电流给定
...
typedef struct
{
	Uint16 uiTimeScanCnt;
	Uint16 uiPvDataAvgCalc;
	Uint16 uiScanInterruption;
	Uint16 uiGlobalScanState;			

    float fDeltaPower;          //功率改变比例
	float fMaxPower;    		//扫描过程中的最大功率
    float fBestVoltage;			//扫描过程中最大功率对应的PV电压
}Struct_GlobalScanData;
...
Pv_PUBLIC Struct_GlobalScanData strGlobalScanPv1;
Pv_PUBLIC Struct_GlobalScanData strGlobalScanPv2;
