...
//	static int16 Pv1BusChkCnt = 0,Pv1BusLimitFlg = FALSE;
    ...
    //=======================================================================
    if(TRUE == strMstFuction.ubFuction.bit.bATEPvCurrContrl)
    {
         g_strPv1VoltLoop.integrator = 0; 		  //清积分，根据主DSP下发的功率控电流
		 g_strPv1VoltLoop.Out = 0;	
	     g_strPv1CurrLoop.Ref = g_PvVar.fATEPv1CurrRef;
		 UpDownLimit(g_strPv1CurrLoop.Ref,cPvCurrMax,0); 
    }
...
//    	}
    ...
    //PV曲线切换（PV电压小->大），会导致工作点跑到左侧，工作在短路电流段
	if(uiPv1OverCurrFlag)
	{
	    uiPv1OverCurrFlag = FALSE;
		g_strPv1VoltLoop.integrator *= 0.75f;
		g_strPv1CurrLoop.integrator *= 0.75f;
	}
...
//        if((Pv2BusLimitFlg == TRUE)&&(stADC.fRealScale.VBus - g_strPv2BusVoltLoop.Ref > 0.125))  // 0.0375*800 Set_BusVolt(40)
	...
	else  // 独立跑 则需要重新跑电压环
	{
	    ...
	    if(TRUE == strMstFuction.ubFuction.bit.bATEPvCurrContrl)
	    {
			 ...
			 UpDownLimit(g_strPv2CurrLoop.Ref,cPvCurrMax,0);
	    }
...
//		if((Pv2BusLimitFlg == TRUE)&&(stADC.fRealScale.VBus - g_strPv2BusVoltLoop.Ref > 0.125))	// about 15V Set_BusVolt(15)
	    ...
	    //PV曲线切换（PV电压小->大），会导致工作点跑到左侧，工作在短路电流段	
		if(uiPv2OverCurrFlag)
		{
	  	    uiPv2OverCurrFlag = FALSE;
			g_strPv2VoltLoop.integrator *= 0.75f;
			g_strPv2CurrLoop.integrator *= 0.75f;
		}
...
void Pv_PvPowerSoftStart(void)
{
    ...
    if(ePvRunState_MpptCtrl == g_PvVar.PvFlag.bit.PvRunSts && g_PvVar.PvFlag.bit.Pv1Enable)         // max = 13.6us real = 13.6us
    {
        if(   (fabs(g_strPv1PowerLoop.Ref - g_PvVar.fPv1CtrlPowerSet) <= cPvPowerSoftRate*2)
			||(  (g_strPv1PowerLoop.Ref > g_PvVar.fPv1CtrlPowerSet) 
			   &&(FALSE == strMstFuction.ubFuction.bit.bATEPvCurrContrl) ) 
		  ) 
        {
            g_strPv1PowerLoop.Ref = g_PvVar.fPv1CtrlPowerSet;
	...
	//软起由功率完成
	if((TRUE == strMstFuction.ubFuction.bit.bATEPvCurrContrl) && (stADC.fRealScale.VoltPv1 != 0))
	{
		g_PvVar.fATEPv1CurrRef = g_strPv1PowerLoop.Ref / stADC.fRealScale.VoltPv1; 
		UpDownLimit(g_PvVar.fATEPv1CurrRef,cPvCurrMax,0); //PV1电流给定范围
	}
	else
	{
	...
	//软起由功率完成
	if((TRUE == strMstFuction.ubFuction.bit.bATEPvCurrContrl) && (stADC.fRealScale.VoltPv2 != 0))
	{
		g_PvVar.fATEPv2CurrRef = g_strPv2PowerLoop.Ref / stADC.fRealScale.VoltPv2; 
		UpDownLimit(g_PvVar.fATEPv2CurrRef,cPvCurrMax,0); //PV2电流给定范围
	}
	else
	{
...
void Pv_PvPowerAvgCalc(void)
{
	static Uint16 uiPv1PowerCalcCnt,uiPv2PowerCalcCnt;
//	static Uint16 uiPv1PowerCalcCnt_Inv,uiPv1PowerCalcCnt_Bat;
	static float  fPv1PowerTemp,fPv2PowerTemp;
//	static float  fPvPowerForInvTemp,fPvPowerForBatTemp;
	static float  fPv1VoltTemp,fPv2VoltTemp;
//	static float  fPvVoltForInvTemp,fPvVoltForBatTemp;
...
void Pv_GolbalScanCheck(Struct_MpptCtrlData *pstrMpptControlData,Struct_GlobalScanData *pstrGlobalScanData,float *pPvVoltRef)
{
    if(TRUE == strMstFuction.ubFuction.bit.bMPPTMultiPointEn)
    {
    	...
    	{
    	    if(   (++pstrGlobalScanData->uiTimeScanCnt >= cDelay30min_1s) 
...
void Pv_IsoCheckDeal(void)
{
    ...
    if( ePvMainState_PvIsoCheck == g_PvVar.PvFlag.bit.PvMainSts)
    {
        if(ePvIsoCheckState_NoCheck == g_PvVar.PvFlag.bit.PvIsoCheckState)
        {
			...
			//所有状态复位，防止ISO检测中途系统复位，ISO检测状态机未复位
