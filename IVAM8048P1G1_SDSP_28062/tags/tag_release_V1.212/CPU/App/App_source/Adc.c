...
void Adc_System_PeripheralInit(void)
{
	...
	#define		ADC_SAMPLE_WINDOUW  		8		// Sample window is N+1 cycles long (N + 1 clock cycles).
	...
	AdcRegs.ADCSOC6CTL.bit.CHSEL	= 5;	//set SOC6	  ADCINA5  PV2电感电流		    / ADCINB5  PV1电感电流 
	AdcRegs.ADCSOC8CTL.bit.CHSEL	= 3;	//set SOC8	  ADCINA3  PV2电压 		 	/ ADCINB3  PV电感温度检测
...
//	  stADC.iSample.Model		= AdcResult.ADCRESULT1;// - stAdc OffSet.IGfci;
	  ...
	  stADC.iSample.CurrPv2     = AdcResult.ADCRESULT6 - stAdcOffSet.CurrPv2;
	  stADC.iSample.CurrPv1     = AdcResult.ADCRESULT7 - stAdcOffSet.CurrPv1;
	  
	  stADC.iSample.VoltPv2 	= AdcResult.ADCRESULT8;
	  stAdcTemp.Sample.Pv_Temp	= AdcResult.ADCRESULT9;
...
//    stADC.fRealScale.Model       = ((float32)stADC.iSample.Model * stADC.fGain.Model);
	...
	//校准后可能为负数，空载PV稳不住母线
	if(stADC.fRealScale.CurrPv1 < 0)  stADC.fRealScale.CurrPv1 = 0;
	if(stADC.fRealScale.CurrPv2 < 0)  stADC.fRealScale.CurrPv2 = 0;
