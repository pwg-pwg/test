...
*/
    ...
    // ========== 第一步：先复位串口模块，清空原有配置 ==========
    ScibRegs.SCICTL1.all = 0x0001;  // 禁用TX/RX，置位软件复位（SWRESET=0）
    DELAY_US(100);                  // 等待复位完成（100μs足够）
    
    // ========== 第二步：基础串口配置 ==========
    ScibRegs.SCICCR.all = 0x0007;    // 8位数据位、1位停止位、无奇偶校验、空闲线模式
    // 配置波特率（必须在复位后、释放复位前配置）
	ScibRegs.SCIHBAUD				 = INTSCI_HBAUD_HBAUD;//0x00;		//设置波特率38400,低速外设37.5MHz
    ScibRegs.SCILBAUD				= INTSCI_HBAUD_LBAUD;//0xC2;	   //38400~=LSPCLK/[(BRR+1)*8]=15MHz/[(0XC2+1)*8]
    
    // ========== 第三步：中断配置（明确禁用不需要的中断） ==========
    ScibRegs.SCICTL2.all = 0x0000;    // 先清空所有中断使能
    ScibRegs.SCICTL2.bit.TXINTENA = 0; // 禁用发送中断（符合你的需求）
    ScibRegs.SCICTL2.bit.RXBKINTENA = 0; // 禁用接收断点中断（你的需求）
    
    // ========== 第四步：FIFO配置（完整启用FIFO，提升接收效率） ==========
    ScibRegs.SCIFFTX.all = 0xC040;   // 0xC040=开启TX FIFO + FIFO复位 + 触发级别64
    ScibRegs.SCIFFRX.all = 0x204F;   // 开启RX FIFO + 触发级别15 + 清空RX FIFO
    ScibRegs.SCIFFCT.all = 0x00;     // 禁用波特率自动检测（符合你的需求）
    
    // ========== 第五步：优先级配置 ==========
    ScibRegs.SCIPRI.bit.SOFT = 0x0;  // 调试挂起时立即停止
    ScibRegs.SCIPRI.bit.FREE = 0x1;  // 仿真挂起时继续运行（原代码注释错误，FREE=1是继续运行）
    
    // ========== 第六步：释放复位，使能TX/RX ==========
    ScibRegs.SCICTL1.all = 0x0023;    // SWRESET=1（释放复位） + TX/RX使能（bit0=1, bit1=1）
    DELAY_US(1000);                  // 等待模块就绪（1ms，硬件稳定）
