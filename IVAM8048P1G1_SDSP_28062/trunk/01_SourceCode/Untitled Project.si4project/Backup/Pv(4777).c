    ...
    if(ePvRunState_MpptCtrl == g_PvVar.PvFlag.bit.PvRunSts)
	{
		  ...
		  )
		{
			if(   (fabs(g_PvVar.fPv1MpptVoltRef - g_PvVar.fMppt1VoltRefSet) < Set_PvVolt(0.4))
			    ||((MPPTGLOBALSCAN == strMpptCtrlData_Pv1.uiMpptSearchState) && (strGlobalScanPv1.uiGlobalScanState > SCANPREPARE))
			  )
			{
    ...
    if(ePvRunState_MpptCtrl == g_PvVar.PvFlag.bit.PvRunSts)
    {
		if( TRUE == g_PvVar.PvFlag.bit.Pv2MpptEn && FALSE == g_PvVar.PvFlag.bit.Pv2PowerDerate)
		{
			if(   (fabs(g_PvVar.fPv2MpptVoltRef - g_PvVar.fMppt2VoltRefSet) < Set_PvVolt(0.4))
				||((MPPTGLOBALSCAN == strMpptCtrlData_Pv2.uiMpptSearchState) && (strGlobalScanPv2.uiGlobalScanState > SCANPREPARE))
		    )
			{
...
//	static Uint16 uiDelyPeriod = 3;
	...
	if(MPPTGLOBALSCAN == strMpptCtrlData_Pv1.uiMpptSearchState)  uiPv1MPPTPeriod = 10;   //300ms
	else  														 uiPv1MPPTPeriod = 10;  //1s

	if(MPPTGLOBALSCAN == strMpptCtrlData_Pv2.uiMpptSearchState)  uiPv2MPPTPeriod = 10;   //300ms
	else  														 uiPv2MPPTPeriod = 10;  //1s
