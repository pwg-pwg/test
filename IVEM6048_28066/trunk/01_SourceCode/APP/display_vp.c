...
STRDisplayPage g_strDisplayPage={0,0,0,0,0};
...
extern INT8U  ubInverterWarningLEDFlash;
//extern INT8U  ubBMSWarningLEDFlash;
...
extern INT8U g_ubBatEqDateReach;
...
//void sSettingPageFaultRecord(INT8U* pubOprtStep, INT16S* pwChangeValue);
...
void sSettingPageSetRecover(INT8U* pubOprtStep, INT16S* pwChangeValue);
void sSettingPageFaultRecord(INT8U* pubOprtStep, INT16S* pwChangeValue);
void sSettingPageBatEq(INT8U* pubOprtStep, INT16S* pwChangeValue);
void sSettingPageBatEqVolt(INT8U* pubOprtStep, INT16S* pwChangeValue);
void sSettingPageBatEqTime(INT8U* pubOprtStep, INT16S* pwChangeValue);
void sSettingPageBatEqTimeout(INT8U* pubOprtStep, INT16S* pwChangeValue);
void sSettingPageBatEqInterval(INT8U* pubOprtStep, INT16S* pwChangeValue);
void sSettingPageBatEqActImd(INT8U* pubOprtStep, INT16S* pwChangeValue);
...
//	char2,	char0,	charF,		charP,		charG,		sSettingPageFeedPowerEn,
    ...
    char2,  char9,  charR,      charS,      charT,      sSettingPageSetRecover,
    char3,  char0,  charF,      charE,      charN,      sSettingPageFaultRecord,
    char3,  char1,  charE,      charQ,      cCharSpace, sSettingPageBatEq,
	char3,  char2,  charE,      charQ,      charV, 		sSettingPageBatEqVolt,
	char3,  char3,  charE,      charQ,      charT, 		sSettingPageBatEqTime,
	char3,  char4,  charE,      charQ,      charO, 		sSettingPageBatEqTimeout,
	char3,  char5,  charE,      charQ,      charI, 		sSettingPageBatEqInterval,
	char3,  char6,  charE,      charA,      charI, 		sSettingPageBatEqActImd
};
...
void sDisplayTask(void)
{
	...
	while(1)
	{
		...
		if(event&(1<<eDisplayTimer))	 //100ms
		{
			...
			//
			if(swGetEESetRecoverEn() && mLCDPage ==  false)//退出该设置界面后生效
			{
...
void sBuzzerProc(void)
{
	if(swGetEEBuzzEn())
	{
		...
		else if(  (uniWarningCode.BIT.uwBatLow && g_uwWorkMode != cStandbyMode)
				|| uniWarningCode.BIT.uwBmsDisconnect
		)
		{
...
void sLEDDisplay(void)
{
	...
	static INT16S wBMSUpdateFlashCnt = 0;
	...
	if(bStartBMSUpdateFlag)
	{
		...
		if(++wBMSUpdateFlashCnt >= 5)
		{
			wBMSUpdateFlashCnt = 0;
		}
	}
	...
	else
	{
		...
		if(	  (g_uwWorkMode == cFaultMode)
			||(ubInverterWarningLEDFlash  && (wLEDDispalyCnt > 4))
		)
		{
			hoFAULTLEDOn;
		}
...
{ 
    ...
    sSendBitToVram(cBIT_IVEM5048, DISPLAY_ON, cALWAYS_LIGHT);
    #else
    sSendBitToVram(cBIT_IVEM6048, DISPLAY_ON, cALWAYS_LIGHT);
    #endif

	if((++wCnt >=5) || (wStartUpDelayCnt == 2))
	{
		wCnt = 0;
		sDisplayUpdateData();
		sDisplayUpdate();	
...
//	}
	...
	else if(swGetIOTState() == 0)
	{
		sSendBitToVram(cBIT_WIFI, DISPLAY_OFF, cALWAYS_LIGHT);
	}
	else if(swGetIOTState() == 1 || swGetIOTState() == 2 || swGetIOTState() == 4 || swGetIOTState() == 5)
	{
		sSendBitToVram(cBIT_WIFI, DISPLAY_ON, cFLASH_TIME_1S);
	}

	if(mRealTimePageNum != RT_BATSOC_DATE && mRealTimePageNum != RT_BATSOC_TIME)
	{
		if(!g_uwFaultCode && !ubInverterWarningLEDFlash)
		{
			if(g_uniSysChgStatus2.BIT.uwEqStatue == eEqState_Eqing && g_wBatChgStage == cBatChgEqStage)   		//正式进入EQ阶段：EQ常亮
			{
				sViewFigModule(cFIG_TYPE_7, FIG4_7SEG, charE, cALWAYS_LIGHT);
				sViewFigModule(cFIG_TYPE_7, FIG5_7SEG, charQ, cALWAYS_LIGHT);
			}
			else if(g_uniSysChgStatus2.BIT.uwEqStatue == eEqState_EqWaiting && g_wBatChgStage == cBatChgFloatStage) 	//EQ阶段准备阶段：EQ闪烁
			{
				sViewFigModule(cFIG_TYPE_7, FIG4_7SEG, charE, cFLASH_TIME_1S);
				sViewFigModule(cFIG_TYPE_7, FIG5_7SEG, charQ, cFLASH_TIME_1S);
			}
			else
			{
				sViewFigModule(cFIG_TYPE_7, FIG4_7SEG, charE, 0XFF);
				sViewFigModule(cFIG_TYPE_7, FIG5_7SEG, charQ, 0XFF);
			}
		}
...
void sViewBatCap(void)
{
	...
	else if(!uniWarningCode.BIT.uwBatOpen)
	{
		...
		if(uniWarningCode.BIT.uwBatLow  && g_uwBatType == cBatVoltType)
		{
			mSetVramBit(cBIT_BAT_FRM , cFLASH_TIME_1S);    
		}
		else 
...
void sDisplayFaultWarning(void)
{
	...
	do
	{
				...
				if(swGetEEParallelEn() == cOP_Parallel)
				{
					if(g_uwParaMode == cMasterMode)
					{
						if(g_ubBatEqState == eEqState_Eqing)
						{
							...
							sFigSpecialDisplayLetter(charH, charS, cFLASH_TIME_1S);
						}
						else
						{
							mSetVramBit(cBIT_MIDNUM_FRM, cALWAYS_LIGHT);
					...
					{
						if(g_uniSysChgStatus2.BIT.uwEqStatue == eEqState_Eqing)
						{
							mSetVramBit(cBIT_MIDNUM_FRM, cALWAYS_LIGHT);
							sFigSpecialDisplayLetter(charS, charL, cFLASH_TIME_1S);
						}
						else
						{
							mSetVramBit(cBIT_MIDNUM_FRM, cALWAYS_LIGHT);
		...
		{
			if(uniWarningCode.BIT.uwBatLow)
			{
		...
		else if(ubChkCnt == FW_BatFail)
		{
			if(g_uniSysChgStatus2.BIT.uwEqStatue == eEqState_EqFail)
			{
				s_ubCurrentIndex = FW_BatFail;
				s_ubIndexRollCnt++;
				ubWarningCode = WR_BatEqFail_Code;
			}
			else
			{
				ubChkCnt++;
...
INT8U sbDisplayWarning(void)
{
    ...
    else if(uniWarningCode.BIT.uwBatLow) 
    {
...
INT8U subSettingValueOprt(INT8U* pubOprtStep, INT16S* pwChangeValue, INT16S* pwSettingValue, INT16S* pwSettingValueTemp,
									INT8U* pubDisplayTime, INT16S wValueHLimit,	INT16S wValueLLimit)
{
	if((*pubOprtStep) == cSetStepInit)
	{
		*pubOprtStep = cSetStepOne;
		*pubDisplayTime = cALWAYS_LIGHT;
		*pwSettingValueTemp = *pwSettingValue;
		*pwChangeValue = 0;
	}
	else 
	{
		...
		{
			*pubDisplayTime = cFLASH_TIME_1S;
			if((*pubOprtStep) > cSetStepTwo)
			{
				*pubOprtStep = cSetStepOne;
				if((*pwChangeValue) != 0)
				{
					*pwSettingValue = *pwSettingValueTemp;
					*pwChangeValue = 0;
					return true;
				}
			}
	...
	*pwSettingValueTemp = *pwSettingValue + (*pwChangeValue);
	if(*pwSettingValueTemp < wValueLLimit)
	{
		*pwSettingValueTemp = wValueHLimit;
		(*pwChangeValue) = (*pwSettingValueTemp - *pwSettingValue);
	}
	else if(*pwSettingValueTemp > wValueHLimit)
	{
		*pwSettingValueTemp = wValueLLimit;
		(*pwChangeValue) = (*pwSettingValueTemp - *pwSettingValue);
	}
	return false;
}
...
void sSettingPageMaxChgCurr(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, cSettingMaxChgCurr, 10))
	{
		sSetEEBatChgCurrMax(wSettingValue*10);
		fUserSettingChange = true;		
	}
    mSetVramBit(cBIT_CONFIG_FRM, cFLASH_TIME_1S);	
	sSendBitToVram(cBIT_OP_UNIT_CURR, DISPLAY_ON, cALWAYS_LIGHT);
...
void sSettingPageMaxACChgCurr(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, cSettingMaxACChgCurr, 10))
	{
...
void sSettingPageOPPriority(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 2, 0))
	{
...
void sSettingPageChgPriority(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 3, 1))
	{
...
void sSettingPageOverLoadBps(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 1, 0))
	{
...
void sSettingPageFaultRecord(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
    ...
    wSettingValue = swGetEEFaultRecordEn();

    if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 1, 0))
    {
        sSetEEFaultRecordEn(wSettingValue);
        fUserSettingChange = true;
    }
    mSetVramBit(cBIT_CONFIG_FRM, cFLASH_TIME_1S);
    sFigGroupDisplayLetter(FIG6_7SEG , 3, ubDisplayBuff[s_wSettingValueTemp], ubDisplayTime);
...
void sSettingPageBatteryType(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	INT8U ubDisplayBuff[5][3] = {{charA,charG,charM},
								 {charF,charL,charD},
								 {charU,charS,charE},
								 {charL,charI,charB},
								 {charP,charY,charL}};
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 4, 0))
	{
...
void sSettingPageBatUnderVolt(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(g_uwBatType == cBatVoltType)
	{
		...
		if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, \
							&ubDisplayTime, (cBatVoltReal13V5*cBatSerialPcs), (cBatVoltReal10V5*cBatSerialPcs)))
		{
	...
	else
	{
		...
		wSettingValue = swGetEEBatUnderSoc();
				...
				if(*pubOprtStep > cSetStepTwo)
				{
					*pubOprtStep = cSetStepOne;
					if(*pwChangeValue != 0)
					{
						...
						sSetEEBatUnderSoc(wSettingValue); 
		...
		{
			s_wSettingValueTemp = 90;
			(*pwChangeValue) = (s_wSettingValueTemp - wSettingValue)/5;
		}
		else if(s_wSettingValueTemp > 90)
		{
	...
	if(swGetEEBatteryType() == cBatType_USER || swGetEEBatteryType() == cBatType_LIB || swGetEEBatteryType() == cBatType_Pylon)
    {
        mSetVramBit(cBIT_CONFIG_FRM, cFLASH_TIME_1S);
    }
    else
...
void sSettingPageBatCVVolt(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(swGetEEBatteryType() == cBatType_AGM || swGetEEBatteryType() == cBatType_FLD || g_uwBatType == cBatSocType)
	{
		*pwChangeValue = 0;
	}

	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, \
							&ubDisplayTime, (cBatVoltReal15V*cBatSerialPcs), (cBatVoltReal12V*cBatSerialPcs)))
	{
		sSetEEBatCVVolt(wSettingValue);
		fUserSettingChange = true;
		g_uwParameterSetChange = true;
	}
	if(swGetEEBatteryType() == cBatType_USER)
	{
	    mSetVramBit(cBIT_CONFIG_FRM, cFLASH_TIME_1S);
	}
	else
...
void sSettingPageBatFloatVolt(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(fBMSConnectFlg)
	{
		wSettingValue = mBMSBatFloatVolt;
	}
	else
	{
...
void sSettingPageBatWeakVolt(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(g_uwBatType == cBatVoltType)
	{
		...
		if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, \
								&ubDisplayTime, (cBatVoltReal13V5*cBatSerialPcs), (cBatVoltReal11V*cBatSerialPcs)))
		{
	...
	{
		wSettingValue = swGetEEBatWeakSoc();
			...
			else
			{
				...
				if(*pubOprtStep > cSetStepTwo)
				{
					*pubOprtStep = cSetStepOne;
					...
					{
						wSettingValue = s_wSettingValueTemp;
						sSetEEBatWeakSoc(wSettingValue); 
		...
		s_wSettingValueTemp = wSettingValue + (*pwChangeValue)*5;
		if(s_wSettingValueTemp < 5) 
		{
			s_wSettingValueTemp = 95;
			(*pwChangeValue) = (s_wSettingValueTemp - wSettingValue)/5;
		}
		else if(s_wSettingValueTemp > 95)
		{
			s_wSettingValueTemp = 5;
			(*pwChangeValue) = (s_wSettingValueTemp - wSettingValue)/5;
		}
...
void sSettingPageBatWeakBackVolt(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(g_uwBatType == cBatVoltType)
	{
		...
		if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, \
								&ubDisplayTime, ((cBatVoltReal15V*cBatSerialPcs)+1), (cBatVoltReal12V*cBatSerialPcs)))
		{
	...
	{
		wSettingValue = swGetEEBatWeakBackSoc();
			...
			else
			{
				...
				if(*pubOprtStep > cSetStepTwo)
				{
					*pubOprtStep = cSetStepOne;
					...
					{
						wSettingValue = s_wSettingValueTemp;
						sSetEEBatWeakBackSoc(wSettingValue); 
...
void sSettingPageTimeYear(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 99, cEnergySaveStartYear))
	{
...
void sSettingPageTimeMonth(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 12, 1))
	{
...
void sSettingPageTimeDay(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 31, 1))
	{
...
void sSettingPageTimeHour(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 23, 0))
	{
...
void sSettingPageTimeMin(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 59, 0))
	{
...
void sSettingPageSetRecover(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
    ...
    wSettingValue = swGetEESetRecoverEn();

    if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 1, 0))
    {
        sSetEESetRecoverEn(wSettingValue);
        fUserSettingChange = true;
    }
	mSetVramBit(cBIT_CONFIG_FRM, cFLASH_TIME_1S);
    sFigGroupDisplayLetter(FIG6_7SEG , 3, ubDisplayBuff[s_wSettingValueTemp], ubDisplayTime);
...
void sSettingPageBatEq(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(swGetEEBatteryType() == cBatType_AGM || swGetEEBatteryType() == cBatType_LIB || swGetEEBatteryType() == cBatType_Pylon || g_uwBatType == cBatSocType)
	{
		*pwChangeValue = 0;
		mSetVramBit(cBIT_CONFIG_FRM, cALWAYS_LIGHT); 
	}
...
void sSettingPageBatEqVolt(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	wSettingValue = swGetEEBatEqVolt();
	...
	s_wSettingValueTemp = wSettingValue + (*pwChangeValue);
	if(s_wSettingValueTemp < 480)
	{
		s_wSettingValueTemp = 610;
		(*pwChangeValue) = s_wSettingValueTemp - wSettingValue;
	}
	else if(s_wSettingValueTemp > 610)
	{
		s_wSettingValueTemp = 480;
		(*pwChangeValue) = s_wSettingValueTemp - wSettingValue;
	}
	//sSendBitToVram(cBIT_OP_BAT_INDICATE, DISPLAY_ON, cALWAYS_LIGHT);	
	sSendBitToVram(cBIT_OP_UNIT_VOL, DISPLAY_ON, cALWAYS_LIGHT);
...
void sSettingPageBatEqTime(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	wSettingValue = swGetEEBatEqTime();
	...
	if(s_wSettingValueTemp < 5)
	{
		s_wSettingValueTemp = 900;
		(*pwChangeValue) = (s_wSettingValueTemp - wSettingValue)/5;
	}
	else if(s_wSettingValueTemp > 900)
	{
		s_wSettingValueTemp = 5;
		(*pwChangeValue) = (s_wSettingValueTemp - wSettingValue)/5;
	}
...
void sSettingPageBatEqTimeout(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	wSettingValue = swGetEEBatEqTimeout();
		...
		else
		{
			...
			if(*pubOprtStep > cSetStepTwo)
			{
				*pubOprtStep = cSetStepOne;
				...
				{
					wSettingValue = s_wSettingValueTemp;
					sSetEEBatEqTimeout(wSettingValue);
					fUserSettingChange = true;
					g_uwParameterSetChange = true;
					*pwChangeValue = 0;
				}
...
void sSettingPageBatEqInterval(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	wSettingValue = swGetEEBatEqInterval();
		...
		else
		{
			...
			if(*pubOprtStep > cSetStepTwo)
			{
				*pubOprtStep = cSetStepOne;
				...
				{
					wSettingValue = s_wSettingValueTemp;
					sSetEEBatEqInterval(wSettingValue);
					fUserSettingChange = true;
					g_uwParameterSetChange = true;
					*pwChangeValue = 0;
				}
...
void sSettingPageBatEqActImd(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	wSettingValue = swGetEEBatEqActImd();
	if(*pubOprtStep == cSetStepInit)
	{
		*pubOprtStep = cSetStepOne;
		ubDisplayTime = cALWAYS_LIGHT;
		...
		else
		{
			...
			if(*pubOprtStep > cSetStepTwo)
			{
				*pubOprtStep = cSetStepOne;
				...
				{
					wSettingValue = s_wSettingValueTemp;
					sSetEEBatEqActImd(wSettingValue);
					fUserSettingChange = true;
					g_uwParameterSetChange = true;
					*pwChangeValue = 0;
				}
