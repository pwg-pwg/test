...
//struct STRFaulRecordStrut g_strFaultRecord;
...
//			#endif
        ...
        case E2_EEFactory:
            if(WrData <= 1)
            {
                if(swGetEESetRecoverEn() != WrData)
                {
                    sSetEESetRecoverEn(WrData);
//                    OSEventSend(cPrioInterface, eI2CEEpromUserSave);
                }
            }
            else
...
void sRealTimeDataUpdate(void)
{
	...
	uRealTimeData.Stru.wRLineCurr = g_uwRGridCurr;
