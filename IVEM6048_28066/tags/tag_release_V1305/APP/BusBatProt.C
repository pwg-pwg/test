...
#define cFanControlPercentum			650//700		//70% * 10
...
#define cLLC_MosTjProtectPoint			1125//1200	//120C
#define cOtherTjProtectPoint			1313//1400	//140C		//cLLC_IGBTKt\cINV_IGBTKt\cPV_BOOST_DiodeKt
#define cInnelTempProtectPoint			700		//70C
#define cLLC_TXTempProtectPoint			1350//1440	//144C
//Temprature Derat
#define cLLC_MosTjDeratPoint			1050//1125	//112.5C
#define cOtherTjDeratPoint				1225//1312	//131.2C	//cLLC_IGBTKt\cINV_IGBTKt\cPV_BOOST_DiodeKt
...
//#define cNTCTempDelta5					50
//#define cNTCTempDelta10					100
//#define cNTCTempDelta28					220//280
#define cNTCTempDelta					220
...
#define cOtherTjStarTempPoint			613		//61.3C
#define cOtherTjEndTempPoint			1313	//131.25C
#define cOtherTjTempRange				613		//61.3C

#define cLLC_TXStarTempPoint			630		//63.0C
#define cLLC_TXEndTempPoint				1313	//131.25C
#define cLLC_TXTempRange				630		//63.0C
...
#define cLLC_MosTjProtectPoint			1125	//1200	//120C
#define cOtherTjProtectPoint			1400	//1400	//140C		//cLLC_IGBTKt\cINV_IGBTKt\cPV_BOOST_DiodeKt
#define cInnelTempProtectPoint			700		//70C
#define cLLC_TXTempProtectPoint			1440	//1440	//144C
//Temprature Derat
#define cLLC_MosTjDeratPoint			900///1050	//1125	//112.5C
#define cOtherTjDeratPoint				1130//1312	//1312	//131.2C	//cLLC_IGBTKt\cINV_IGBTKt\cPV_BOOST_DiodeKt
#define cInnelTempDeratPoint			g_wInnelOTBackPoint	//60C
#define cLLC_TXTempDeratPoint			1350//1350	//1350C


#define cLLC_MosTjDeratPercent			444//1333	//13.33%
#define cOtherTjDeratPercent			370//3.7% 1136	//11.36%
#define cInnelTempDeratPercent			1000	//10.00%
#define cLLC_TXTempDeratPercent			1111	//11.11%
...
// add in 20231206
#define cSolarBSTOTPoint 		650
#define cSolarBSTOTBackPoint 	550
#define cInvOTPoint 			750
#define cInvOTBackPoint	 		600
#define cConvertLOTPoint 		700
#define cConvertLOTBackPoint 	550
#define cConvertHOTPoint 		850
#define cConvertHOTBackPoint 	700
#define cInnelOTPoint 			700
#define cInnelOTBackPoint 		600
...
extern INT16U g_uwSolar1BypassFlag;
...
INT16S g_wSolarBSTOTPoint 		= cSolarBSTOTPoint;
INT16S g_wSolarBSTOTBackPoint 	= cSolarBSTOTBackPoint;
INT16S g_wInvOTPoint 			= cInvOTPoint;
INT16S g_wInvOTBackPoint	 	= cInvOTBackPoint;
INT16S g_wConvertLOTPoint 		= cConvertLOTPoint;
INT16S g_wConvertLOTBackPoint 	= cConvertLOTBackPoint;
INT16S g_wConvertHOTPoint 		= cConvertHOTPoint;
INT16S g_wConvertHOTBackPoint 	= cConvertHOTBackPoint;
INT16S g_wInnelOTPoint 			= cInnelOTPoint;
INT16S g_wInnelOTBackPoint 		= cInnelOTBackPoint;
INT16S g_wLLC_TXOTPoint 		= cLLC_TXOTPoint;
INT16S g_wLLC_TXOTBackPoint 	= cLLC_TXOTBackPoint;
...
INT8U  g_ubBatEqState     = 0;//工作状态
INT8U  g_ubBatEqDateReach = 0;
INT8U  g_ubBatEqEnable    = 0;
...
const INT16U wNTCTempSampleTab47K[126]=		//47K	3.3V//0C to 125C
{
	...
	while(1)
	{
		...
		if(event&(1<<eBusBatProtTimer))			
		{
			...
			else
			{
				...
				if(
				        subGetBatLowSts()
				    || (subGetBatWeakSts()/* &&  g_uwWorkMode == cBatteryMode  && subGetPalLineLossStatus() == false */&& swGetEEOPPriority() == cOutputSolarBatUtility)
				    || subGetParaBatOpenSts()       //可不添加
				    || subGetParaBatUnderSts()      //可不添加
				    || subGetParaBatPowerDownSts()  //可不添加
				)
				{
				    hoDRYOUTRLYOn;
				}
				...
				{
				    if(  (!subGetBatWeakSts() && swGetEEOPPriority() == cOutputSolarBatUtility)
...
//				if(subGetParaBatOpenSts() || subGetParaBatUnderSts() ||subGetBatWeakSts() || subGetParaBatPowerDownSts() || subGetBatLowSts())
//				{
//					hoDRYOUTRLYOn;
//				}
//				else
...
//				}
			...
			//g_wSolarBSTTemp = swSolarBSTTempCal(suwSolarBSTTempAvgCal());//PV Boost的温度与"逆变温度"同一个信号
			g_wSolarBSTTemp = g_wInvTemp;
//			g_wInnelTemp = swInnelTempCal(suwInnelTempAvgCal());		//未采"内部环温"
...
void sOverTempParaInit(void)
{
	...
	g_wSolarBSTOTPoint 		= cSolarBSTOTPoint;
	g_wSolarBSTOTBackPoint 	= cSolarBSTOTBackPoint;
	g_wInvOTPoint 			= cInvOTPoint;
	g_wInvOTBackPoint	 	= cInvOTBackPoint;
	g_wConvertLOTPoint 		= cConvertLOTPoint;
	g_wConvertLOTBackPoint 	= cConvertLOTBackPoint;
	g_wConvertHOTPoint 		= cConvertHOTPoint;
	g_wConvertHOTBackPoint 	= cConvertHOTBackPoint;
	g_wInnelOTPoint 		= cInnelOTPoint;
	g_wInnelOTBackPoint 	= cInnelOTBackPoint;
	g_wLLC_TXOTPoint 		= cLLC_TXOTPoint;
	g_wLLC_TXOTBackPoint 	= cLLC_TXOTBackPoint;
}
...
INT16S wTempDeratePerMos = 100;
INT16S wTempDeratePerOther = 100;
INT16S wTempDeratePerTXT = 100;

INT16S wOPVoltMinTmepMos = 2300;
INT16S wOPVoltMinTmepOther = 2300;
INT16S wOPVoltMinTmepTXT = 2300;
void sOverTempDerating(void)
{
	...
	if(++s_wTjSumCnt > 50)//1s
	{
        ...
        wTempDeratePerOther = wTempratureDeratePerTemp;
        wOPVoltMinTmepOther = wOPVoltDeratTemp;
		...
		if(wTemp2 > cLLC_MosTjProtectPoint)
		{
			wTempratureDeratePerTemp = 0;
			wOPVoltDeratTemp = wOPVoltMinTmep;
            wTempDeratePerMos = wTempratureDeratePerTemp;
            wOPVoltMinTmepMos = wOPVoltDeratTemp;
		}
		else if(wTemp2 > cLLC_MosTjDeratPoint)
		{
			...
			wTempDeratePerMos = wTempratureDeratePerTemp2;
            wOPVoltMinTmepMos = wOPVoltDeratTemp2;
...
//		Test_InnelTjDeratePerTemp = wTempratureDeratePerTemp2;
		...
		if(wTemp2 > cLLC_TXTempProtectPoint)
		{
			wTempratureDeratePerTemp = 0;
			wOPVoltDeratTemp = wOPVoltMinTmep;
            wTempDeratePerTXT = wTempratureDeratePerTemp;
            wOPVoltMinTmepTXT = wOPVoltDeratTemp;
		}
		else if(wTemp2 > cLLC_TXTempDeratPoint)
		{
            ...
            wTempDeratePerTXT = wTempratureDeratePerTemp2;
            wOPVoltMinTmepTXT = wOPVoltDeratTemp2;
...
void sNTCOverTempSlopeProcess(void)
{
	...
	g_wSolarTemp15PointSlopeSum = g_wInvTemp15PointSlopeSum;
...
INT16S swGetPV_BOOST_DiodeTj(INT16S swPVNumber)
{
	...
	return(swComponentTjCal(cPV_BOOST_DiodeKt,uwSolarCurrAvgTemp,g_wSolarBSTTemp,cNTCTempDelta,100));
}
...
void sSolarPowerOverLoadChk(INT8U bFilter)
{
	...
	if(g_wSolarPowerDeratFlag == false)
	{
		...
		if(   g_uwSolar1BypassFlag
		   && sbOverLevelChk((INT16U)((INT16S)g_uwSolarPower1Avg + (INT16S)g_uwSolarPower2Avg),(INT16U)(g_dwTotalPower+200),bFilter,&s_uwSolarPowerOverLoadChkCnt))
		{
			g_wSolarPowerDeratFlag = true;
//			if(g_wPVPowerOverLoadCurrLimit > g_wDCDCCurrAvg + 5)
//			{
...
//			}
	...
	{
		if(   g_uwSolar1BypassFlag == false
		   || sbUnderLevelChk((INT16U)((INT16S)g_uwSolarPower1Avg + (INT16S)g_uwSolarPower2Avg),(INT16U)(g_dwTotalPower-100),bFilter,&s_uwSolarPowerOverLoadChkCnt))
		{
