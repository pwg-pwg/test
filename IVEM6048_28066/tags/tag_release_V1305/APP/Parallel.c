...
//			}
		...
		if(g_uwSlaveExsitFlg[wCnt] == 1)
		{
			...
			if(uwMasterReleaseChk == true)
			{
				...
				else
				{
					...
					{
					    //工作模式不一致： 工作模式优先级：离网 > 市电 > 旁路 > 故障/启机
	...
	//非主机，且第二ID不为0：第一从机作为第二ID？
	if(uwSlaveFirstExsitCnt != 0 && uwRsvMasterAddr == 0)
	{
		uwRsvMasterAddr = uwSlaveFirstExsitCnt;//为啥用第二小ID作为第二ID
	}
	//主机发生改变：三相A比如，单相本身低于它时
	if(wHighPriorityCnt != 0)
	{
	...
	else if(uwRsvMasterAddr != 0)//第二ID接入
	{
...
//				OS_EXIT_CRITICAL();
	...
	if(g_uwParaEvent & (1 << eParaToFaultMode))
	{
		if(g_uwParaMode == cMasterMode)
		{
			...
			OSEventSend(cPrioSuper, eSuperFault); //在传递时已触发
...
INT16U swChkOPRly(void)
{
	...
	for(wCnt = 0; wCnt < g_uwParaNumMaxSize; wCnt++)
	{
		if(g_uwSlaveExsitFlg[wCnt] == 1)
		{
			...
			//锁相完成  且
			//主机离网 ： 要求从机离网 且 输出电压与本机压差 小于3V
			//主机并网/旁路，要求从机也并网或者并网或者旁路
			if(    g_strInvParaStatus.BIT.wPhaseLockOK == 1
				&& (    (g_uwWorkMode == cBatteryMode  && g_strLocalBatVoltWorkModeTemp.BIT.uwWorkMode == cBatteryMode && abs((INT16S)g_strLocalInvVoltLoadPercentTemp.BIT.uwInvVolt - (INT16S)(g_uwRInvVolt/10)) < 3)
				     || (   (g_uwWorkMode == cLineMode || g_uwWorkMode == cBypassMode)
				         && (g_strLocalBatVoltWorkModeTemp.BIT.uwWorkMode == cLineMode || g_strLocalBatVoltWorkModeTemp.BIT.uwWorkMode == cBypassMode)
				         )
				    )
		    )
			{
			    //先进入模式再闭合继电器
			    //若输出闭合 或者 等待闭合 -- 置位输出继电器闭合状态
			...
			//记录等待开机的数量(非待机)
			if(    wCnt > 0
			    && g_strInvParaStatus.BIT.wInvOnRlyOn
	...
	if(gi_wParallelEnable > cOP_Parallel)
	{
		...
		//已开机 且 存在输出等待输出数  且   三相均存在开机
		else if(wParaOPWaitOnCnt > 0 && uwOPModeOPRlySts == 0x1C)	//1 1100
		{
		    //延时输出闭合  -- 等待机架后再统一下发输出继电器闭合的事件
			g_uwOPWaitOnChkCnt++;
	...
	*/
	...
	if(wOverLoadNum > 0)
	{
		if(wParaNum == wOverLoadNum)
		{
			...
			sCanWrite9(&g_strCanTxCom);
			OSEventSend(cPrioSuper, eSuperFault);
//            sFaultRecord(cOverLoad, 255, g_uwFaultCode);
		}
		else
		{
...
INT8U sbParameterSetChk(void)
{
	...
	for(wCnt = 0; wCnt < cParaMaxSize; wCnt++)
	{
		...
		{			
			...
			else if(((g_uwSystemData[wCnt][cParamerterSet1] & cParameterSet1Mask) != (g_uwSystemData[0][cParamerterSet1] & cParameterSet1Mask))
