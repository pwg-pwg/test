...
extern INT16U g_uwEqTimeout1;
extern INT16U g_uwEqTimeout2;
extern INT16S g_wBatVoltRef;
...
//        if(event & (1 << eEepromSaveFaultRecordBackup))
//        {
//            if(!g_wSPSSDProcFlg)
//            {
...
//	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;							//R-4
	...
	g_ubUserDataBuf0[sciid][wSciLength++] = '0' + hoDRYOUTRLY;
	...
	wSciLength += 2;
	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(g_uwEepromWRFlg, 3, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
    wSciLength += 3;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(g_uwEepromRDFlg, 3, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
    wSciLength += 3;

	g_ubUserDataBuf0[sciid][wSciLength++] = 'T';
	g_ubUserDataBuf0[sciid][wSciLength++] = '0' + g_wBatChgStage;
	g_ubUserDataBuf0[sciid][wSciLength++] = '0' + g_ubBatEqState;
	g_ubUserDataBuf0[sciid][wSciLength++] = '0' + g_ubBatEqDateReach;
	g_ubUserDataBuf0[sciid][wSciLength++] = '0' + g_wBatChgerOn;


	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	g_ubUserDataBuf0[sciid][wSciLength++] = 'D';
	sNumToASCII((INT16U)(swGetEEBatEqLastTime() >> 9), 2, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 2;
	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;

	sNumToASCII((INT16U)((swGetEEBatEqLastTime() >> 5) & 0X000F), 2, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 2;
	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;

	sNumToASCII((INT16U)(swGetEEBatEqLastTime() & 0X001F), 2, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 2;
	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	
	sNumToASCII((INT16U)g_wBatVoltRef, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 4;
...
extern INT16S wTempDeratePerMos;
extern INT16S wTempDeratePerOther;
extern INT16S wTempDeratePerTXT;

extern INT16S wOPVoltMinTmepMos;
extern INT16S wOPVoltMinTmepOther;
extern INT16S wOPVoltMinTmepTXT;
extern INT16S wPVCurrLimitTest;
extern INT16S gi_wChgCurrLimitTest;
extern INT16S gi_wDischgCurrLimitTest;

extern INT16S gi_wInvChgCurrLimit;
extern INT16S gi_wInvFeedCurrLimit;
extern INT16S g_wGridLoadPowerPercent;
void sQ4Command(INT8U sciid)
{
    INT16S wSciLength = 0;
    ...
    sNumToASCII(g_wSolarInvDeratePer, 3, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//PV降额百分比
    wSciLength += 3;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(g_wOPVoltDereByTemp, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//输出降额电压
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    //1+4*3=13
    g_ubUserDataBuf0[sciid][wSciLength++] = 'D';
    sNumToASCII(wTempDeratePerMos, 3, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//mos降额百分比
    wSciLength += 3;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(wTempDeratePerOther, 3, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//其他降额百分比
    wSciLength += 3;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(wTempDeratePerTXT, 3, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//TXT降额百分比
    wSciLength += 3;

    g_ubUserDataBuf0[sciid][wSciLength++] = 'V';//Current
    sNumToASCII(wOPVoltMinTmepMos, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//MOS降压值
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(wOPVoltMinTmepOther,4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//Other降压
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(wOPVoltMinTmepTXT, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//TXT降压
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;

    g_ubUserDataBuf0[sciid][wSciLength++] = 'L';
    sNumToASCII(wPVCurrLimitTest, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//PV限流
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(gi_wChgCurrLimitTest, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//DCDC充电限流
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;

    sNumToASCII(gi_wDischgCurrLimitTest, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//DCDC放电限流
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(gi_wInvChgCurrLimit, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//并网充电电流限制
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;

    sNumToASCII(gi_wInvFeedCurrLimit, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//并网馈电电流限制
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;

    g_ubUserDataBuf0[sciid][wSciLength++] = 'C';//Tj
    sNumToASCII(g_wGridLoadPowerPercent, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);              //PV电流
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(abs(g_wPDCDCCurrAvg), 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);           //DCDC电流平均
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(g_wChgCurrAvg, 2, 1, &g_ubUserDataBuf0[sciid][wSciLength]);                  //充电电流
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII(abs(g_wDCDCCurrAvg), 2, 1, &g_ubUserDataBuf0[sciid][wSciLength]);             //放电电流
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;

    g_ubUserDataBuf0[sciid][wSciLength++] = 'P';
    sNumToASCII(g_uwRInvVolt, 3, 1, &g_ubUserDataBuf0[sciid][wSciLength]);                    //逆变电压
    wSciLength += 5;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;                                      //输出功率
    sNumToASCII((INT16U)g_dwOPVA, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
    wSciLength += 4;
    g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
    sNumToASCII((INT16U)g_uwSolarPower1Avg, 5, 0, &g_ubUserDataBuf0[sciid][wSciLength]);      //PV功率
    wSciLength += 5;
