...
INT16S  g_wGridLoadPowerPercent;
...
INT16U g_uwOverLoadGridLevel4Cnt = 0;
...
void sOPPercentCal(void)
{
	...
	INT32S dwOPWatt = 0;
	INT32S dwROPWatt = 0;
//	INT32S dwInvWattTemp = 0;
/*
    //Y = 容量   X = 电压           1(170V,6000W)   2(90V,3000W)
	//两点式    (Y-y1)/(y2-y1)
	//      ----------------  Y = (300X - 3000)/8
	//      (X-x1)/(x2-x1)
	 ...
	 else if(g_uwRLineVolt < 900)
	 {
	     g_dwGridLoadMaxPower = cInvWattMax>>1;
	 }
	 else
	 {
	     g_dwGridLoadMaxPower =(((INT32S)g_uwRLineVolt * 30 - 3000)>>3);       //市电电压倍率10
	 }


	 if(mChkGridRlyOn() && mChkOPRlyOn())
	 {
//	     if(g_dwRInvWatt < -200)     dwInvWattTemp = -200;
//	     else                        dwInvWattTemp = g_dwRInvWatt;

//	     g_wGridLoadPowerPercent = ((g_dwOPVA - g_dwRInvWatt)*100)/g_dwGridLoadMaxPower;

	     //当逆变功率小于0时，此时会通过市电最大电流限制充电功率
	     //当逆变功率大于0时，PV、Bat出功，相当于给市电扩容，故减去该逆变功率计数百分比
	     if(g_dwRInvWatt > 0)
	     {
	         g_wGridLoadPowerPercent = ((g_dwOPVA - g_dwRInvWatt)*100)/g_dwGridLoadMaxPower;
	     }
	     ...
	     {
	         g_wGridLoadPowerPercent = (g_dwOPVA*100)/g_dwGridLoadMaxPower;
	     }
	 }
	 else
	 {
	     g_wGridLoadPowerPercent = 0;
	 }
...
*/
	if(	mChkGridRlyOn())//市电带载
	{
	    if(g_uwRInvCurr > 0)
	    {
//	        g_wGridCurrPercent = (INT16S)(((INT32S)g_uwROPCurr - g_uwRInvCurr) * 100 / cGridRateCurr);
            g_wGridCurrPercent = (INT16S)(((INT32S)g_uwROPCurr - g_uwRInvCurr) * 100 / cMaxACInputCurr);
	    }
	    else
	    {
//	        g_wGridCurrPercent = (INT16S)((INT32S)(g_uwROPCurr) * 100 / cGridRateCurr);
	        g_wGridCurrPercent = (INT16S)((INT32S)(g_uwROPCurr) * 100 / cMaxACInputCurr);
	    }
	}
	else
	{
...
//    if(g_uwWorkMode == cBypassMode || g_uwWorkMode == cLineMode)
//    {
//        if(g_wOPPercent < g_wGridLoadPowerPercent)
//        {
//            g_wOPPercent = g_wGridLoadPowerPercent;
//        }
//    }
...
//	}
    //------------------------------------------
    //   过载转旁路的检测
    //  只有离网转并网  ，并网转离网需要清除标志，将市电电压条件下的输出功率按照离网输出电压转化
    //------------------------------------------
...
void sOverLoadChk(void)
{
	if(g_wOPPercent > 200)
	{
		...
		if(g_uwOverLoadGridLevel4Cnt < 7000)  g_uwOverLoadGridLevel4Cnt++;
	...
	else
	{
		...
		else
		{
			...
			if(g_uwOverLoadGridLevel4Cnt > 60)  g_uwOverLoadGridLevel4Cnt = 60;
		...
		if(g_uwOverLoadGridLevel4Cnt > 0)   g_uwOverLoadGridLevel4Cnt--;
	}
	...
	{
		if( g_uwOverLoadLevel1Cnt >= 20 || g_uwOverLoadLevel2Cnt >= 550 || g_uwOverLoadLevel3Cnt >= 750 || g_uwOverLoadGridLevel4Cnt > 1050)
		{
			g_uwFaultCode = cOverLoad;
			g_uwOverLoadLevel1Cnt = 0;
			g_uwOverLoadLevel2Cnt = 0;
			g_uwOverLoadLevel3Cnt = 0;
			g_uwOverLoadLevel4Cnt = 0;
			g_uwOverLoadGridLevel4Cnt = 0;
			OSEventSend(cPrioSuper, eSuperFault);
            sFaultRecord(cOverLoad, g_wOPPercent, g_uwFaultCode);
		}
		if(g_uwOverLoadLevel4Cnt > 6000)        g_uwOverLoadLevel4Cnt = 6000;
		if(g_uwOverLoadGridLevel4Cnt > 6000)    g_uwOverLoadGridLevel4Cnt = 6000;
	}
