...
INT16S gi_wliBBIOverSample;
INT16S gi_wliLLCIOverSample;
INT16S gi_hiButtonSWUpSample;
				  ...
				  INT16S *pwGeneratorWaveChkCnt, INT16S *pwGeneratorVoltMax, INT16S *pwGeneratorVoltMin)
{
	...
	{
		if( (abs(wLinvVolt) < 300 || abs(wLinvVolt) > 2200) && abs(wOPCurr) < cOPCurr25A )
		{	
		...
		if( ((*pwGeneratorVoltMax) - (*pwGeneratorVoltMin) > 200) || abs(wOPCurr) > cOPCurr5A )
		{
...
//--------------------------------------
	...
	else
	{
		...
		gi_udwButtonSwUpSampleSumTemp += gi_hiButtonSWUpSample;
		gi_uwButtonSwUpSampleSumCntTemp ++;
...
// hoCHGLEDTOGGLE;//单板10.67us
	...
	gi_wliBBIOverSample		=	ADCRESULT14;
	gi_wliLLCIOverSample	=	ADCRESULT15;
...
//			gi_uwInnelTempSampleSumCntTemp++;
	...
	else
	{
		...
		gi_hiButtonSWUpSample	=	ADCRESULT9;			//SW.UP
	...
	EPwm1Regs.ETCLR.all |= 0x000C; // TODO 合并指令

/*TODO gi_wRInvCurrSample = ((gi_wRInvCurr1Sample + gi_wRInvCurr2Sample) >> 1) + gi_wRInvCurrSampleBiasSet;
	   gi_wRInvCurrSample = (INT16S)(((INT32S)gi_wRInvCurrSample * uEepromCfg1.EepromStructCfg1.wEEDSPRInvCurrAdj) >> 11);
  合并指令-->*/ gi_wRInvCurrSample = (INT16S)(((INT32S)(((gi_wRInvCurr1Sample + gi_wRInvCurr2Sample) >> 1) + gi_wRInvCurrSampleBiasSet) * uEepromCfg1.EepromStructCfg1.wEEDSPRInvCurrAdj) >> 11);
	
/*TODO gi_wROPCurrSample = ((gi_wROPCurr1Sample + gi_wROPCurr2Sample) >> 1);
	   gi_wROPCurrSample = (INT16S)(((INT32S)gi_wROPCurrSample * uEepromCfg1.EepromStructCfg1.wEEDSPROPCurrAdj) >> 11);
  合并指令-->*/ gi_wROPCurrSample = (INT16S)(((INT32S)((gi_wROPCurr1Sample + gi_wROPCurr2Sample) >> 1) * uEepromCfg1.EepromStructCfg1.wEEDSPROPCurrAdj) >> 11);
	
/*TODO gi_wROPShareCurrSample = (INT16S)(((INT32S)gi_wROPShareCurrSample * uEepromCfg1.EepromStructCfg1.wEEDSPROPShareCurrAdj) >> 11);
	gi_wROPShareCurrSample = -gi_wROPShareCurrSample;		//HW is opposite to OP current sample
  合并指令-->*/ gi_wROPShareCurrSample = -(INT16S)(((INT32S)gi_wROPShareCurrSample * uEepromCfg1.EepromStructCfg1.wEEDSPROPShareCurrAdj) >> 11);		//HW is opposite to OP current sample

/*TODO gi_wRInvVoltSample = gi_wRInvVoltSample + gi_wRInvVoltSampleBiasSet;
	   gi_wRInvVoltSample = (INT16S)(((INT32S)gi_wRInvVoltSample * uEepromCfg1.EepromStructCfg1.wEEDSPRInvVoltAdj) >> 11);
  合并指令-->*/ gi_wRInvVoltSample = (INT16S)(((INT32S)(gi_wRInvVoltSample + gi_wRInvVoltSampleBiasSet) * uEepromCfg1.EepromStructCfg1.wEEDSPRInvVoltAdj) >> 11);
...
/*TODO gi_wBatVoltReal = mBatVoltReal10(gi_wBatVoltSample);
	   gi_wBatVoltReal = (INT16S)(((INT32S)gi_wBatVoltReal * uEepromCfg1.EepromStructCfg1.wEEDSPBatAdj + uEepromCfg1.EepromStructCfg1.dwEEDSPBatAdjBias) >> 11) / 10;
  合并指令-->*/ gi_wBatVoltReal = (INT16S)(((INT32S)(mBatVoltReal10(gi_wBatVoltSample)) * uEepromCfg1.EepromStructCfg1.wEEDSPBatAdj + uEepromCfg1.EepromStructCfg1.dwEEDSPBatAdjBias) >> 11) / 10;
	if(gi_wBatVoltReal < 0) gi_wBatVoltReal = 0;

	//FIXME--屏蔽，减小中断时间。
	// gi_wBBIOverVoltReal 	= mIoVoltReal(gi_wliBBIOverSample);		 //无用,屏蔽
	// gi_wliLLCIOverVoltReal 	= mIoVoltReal(gi_wliLLCIOverSample); //只有后续一处判定使用，转入调用函数中，避免影响逆变发波
...
// hoLINELEDTOGGLE; //电池逆变锁相市电测试到 10.6uS
	...
	EPwm2Regs.ETCLR.all |= 0x000C; // TODO 合并指令
	...
	if(g_uwWorkMode == cBatteryMode)
	{
		if(g_fBatFastLow && gi_BatFastLowCutOffInv)	//------ 延迟52us关逆变驱动 ------//
		{
			if(g_uwSolarLoss) // 电池电压快速跌落，单电池关逆变驱动，有PV不关驱动
			{
				...
				if(subGetPalLineLossStatus()) { OSISREventSend(cPrioSuper, eSuperToStandby); }
			}
...
INT16U suwButtonSwitchUpState(void)
{
	if(mIoVoltReal(gi_hiButtonSWUpSample) > 150)
	{
		return true;
	}
	else
...
INT16U suwBBIOverState(void)
{
	if(gi_wBBIOverVoltReal > 150)
	{
		return true;
	}
	else
...
INT16U suwliLLCIOverState(void)
{
	if(mIoVoltReal(gi_wliLLCIOverSample) > 150)
	{
		return true;
	}
	else
