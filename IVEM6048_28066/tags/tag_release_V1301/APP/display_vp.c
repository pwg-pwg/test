...
void sViewBatCap(void)
{
	...
	else if(!uniWarningCode.BIT.uwBatOpen)
	{
		...
		if(uniWarningCode.BIT.uwBatLow  && g_uwBatType == cBatVoltType)//xxx
		{
			mSetVramBit(cBIT_BAT_FRM , cFLASH_TIME_1S);    
		}
		else 
...
void sSettingPageBatteryType(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 3, 0))
	{
				...
				if(*pubOprtStep > cSetStepTwo)
				{
					*pubOprtStep = cSetStepOne;
					if(*pwChangeValue != 0)
					{
						...
						if( swGetEEBatUnderVolt()  > swGetEEBatWeakSoc())
						{
