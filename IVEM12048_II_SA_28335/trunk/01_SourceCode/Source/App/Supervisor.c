...
extern INT16U g_uwArcFaultClearFlag;
			...
			if(subGetParaBatOpenSts() || uwLLCWorkFlg || uwForceGridSoftBusFlag == 1)//无电池 或 (市电建立BUS经过buck启动中间BUS后，LLC工作，再关闭DCDC)
			{
				...
				if(g_uwPBusAvgVoltNew >= uwBusSoftPoint)
				{
					if(--uwBusVoltOKDelay == 0)
					{
						...
						sRlyDelayProc(RLY_ZEROPOINT, cDelay0ms9, eSuperSelectPoint, cPrioSuper);
						sRelayPoweLift(); //YXF
...
g_ModeTest[5]=14;
    ...
    //				}
    //			}
            else if(uwFaultCodeTemp == cARCFault)   //电弧故障下不立即重新启动
            {
                //AFCI故障后自动重启或手动重启
                if((g_uwAFCIStatus & (1 << eAFCIState_ArcfaultClear)) || swGetEEClearARCFault())  //ARC故障失能后需要手动清除49故障才能退出故障模式
                {
                    uwFaultRestartFlg = true;
                }
                else
            ...
            else if(uwFaultCodeTemp == cAFCIComFault)
            {
                if(!(g_uwAFCIStatus & (1 << eAFCIState_Comfault)))
                {
                    uwFaultRestartFlg = true;
                }
                else
            ...
            {
                if(++uwFaultRestartCnt > 500 || uwFaultCodeTemp == cARCFault || uwFaultCodeTemp == cAFCIComFault)
                {
                    //5MIN自动清除电弧故障后由DSP写0
                    if(uwFaultCodeTemp == cARCFault)
                    {
...
g_ModeTest[5]=17;
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			{
				if(   (!g_uwINVLoadOnSts || subGetParaBatPowerDownSts() || (g_uwWorkMode == cFaultMode && g_wOverTempFaultRestartFlg && g_uwFaultCode != cFanLock  && g_uwFaultCode != cARCFault))
