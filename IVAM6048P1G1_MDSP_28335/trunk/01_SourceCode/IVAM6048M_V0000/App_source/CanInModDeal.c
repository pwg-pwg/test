...
void CanInModDeal_ParaFrameInit(void)
{
    ...
    // 并机的基础信息
    //-------------------------------------
    bSelfFrameFlagData_ParaVersion      = 1;                        // 并机通讯版本
	...
	// bSelfFrameFlagData_ParaVersion：
	//0= V2003版本内频率渐变的方案，在频率偏差较大时出现三相120°角度差不对的问题，还有环流问题；
	//1= 修改了并离网锁相时频率突变的方案
}
...
void CanPara_HeartBeatCheck(void)
{
    ...
    // Uint16 uiFrameGridAbnormalCnt = 0;
    // Uint16 uiFrameGenAbnormalCnt = 0;
...
*/
            ...
            // if(FALSE == bFrameFlagData_UpmGridNormal(uiFrameIDTemp)) { uiFrameGridAbnormalCnt ++; }
            // if(FALSE == bFrameFlagData_UpmGenNormal(uiFrameIDTemp))  { uiFrameGenAbnormalCnt ++; }
        }
        ...
        // if(uiFrameGridAbnormalCnt) { g_SysFault.Flag.bit.ParaGridAbnormal = 1; } // 由于三相将各自寻点吸合断开，所以不采用此方案
        // else { g_SysFault.Flag.bit.ParaGridAbnormal = 0; }

        // if(uiFrameGenAbnormalCnt) { g_SysFault.Flag.bit.ParaGenAbnormal = 1; }
        // else { g_SysFault.Flag.bit.ParaGenAbnormal = 0; }
        //==============================================================
        // 通信异常汇总
        //==============================================================
            ...
            && (FALSE == bSelfFrameFlagData_MdlMaster) )
    {
        ...
        switch(uiCanFrameManageState)
        {
            ...
            case eCanFrame_WaitMasterData:
            {
                ...
                // s_uiSlaveSendDelayTime = 3 * (uiSelfFrameCanIDPrio);
                // if((uiSelfFrameCanID > uiMasterFrameID) && (uiSelfFrameCanIDPrio > 0))
                // {
                //     s_uiSlaveSendDelayTime = 3 * (uiSelfFrameCanIDPrio - 1);
                // }
