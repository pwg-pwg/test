...
// 程序版本和日期放到这里
#define cDSPVersion     ((Uint16)2105)    //-004   // 单机并机合并程序 整合机型
// #define cDSPVersion     ((Uint16)20041)   //-004   // 单机并机合并程序 整合机型
//版本说明
//第一个数字代表硬件版本——————1:代表第1代硬件版本(变比不同,硬件版本直接写死)		2:代表第2,3代硬件版本
//第二个数字代表应用场景——————1:代表特殊版本	2:代表正式发布	3:代表调试场景(3:Ivan；4:Ming；5:JIANHAO)
//第三四个数字代表版本——————0-99个版本

#define cVerDate        ((Uint16)60318)//20251209
...
void System_CtDirectionChk(void);
//-----------------------------------------------------------------------------
...
void System_Task_100ms(void)
{
    ...
    //-------------------------------
    g_SysFaultEn.all = uiSciSetDataBag[eSetRsvd361];
...
static void System_ModuleOnOffDeal(void)
{
    ...
    // 电池激活只允许：单机 或 （并机的A相主机允许AC、PV激活） 或 (并机的非A相主机只允许PV激活)
    ...
    else if(   (FALSE == g_ComInfo.Com1.bit.LiActiveEn)  // 未使能激活标志 或 非无电池类型
            || (eNoBatt == g_ComInfo.Com1.bit.BatMode)
            || (eActiveByNull != g_ComInfo.Com1.bit.LiActiveSource && eBatNormal == g_SystemInfo.Source.bit.BatSource && eBatteryNotActivated == g_ComInfo.Com1.bit.LiActiveStatus) )
    {
        g_ComInfo.Com1.bit.LiActiveSource = eActiveByNull;
    ...
    else if(eActivateStatusConfirmed == g_ComInfo.Com1.bit.LiActiveStatus)  // 激活完电池电容还有电压，需要加判断
    {
        ...
        // if((++uiBatVoltDownCnt >= SecondBy5msBase(45)) || (eBatLose == g_SystemInfo.Source.bit.BatSource))
    ...
    if(eSysAppRunStatus == g_SystemVar.SysRunFlag.bit.bSystemMainSts)//这里如果离网运行没问题看下把DCDCONOff改成LLCONOff
    {
        ...
        else
        {
            if(eTurnOff == g_ComInfo.Com1.bit.DcDcOnOff)
            {
                ...
                else if((!g_ComInfo.Com1.bit.LiActFailByBatopen) && (eBatNormal == g_SystemInfo.Source.bit.BatSource))
                {
        ...
        // 增加电池低压或SOC低关Ac
        //-------------------------------
        if(g_InvVar.InvLogicJump.bit.bBatLowOffFlag || (g_SysAlarm.ubAlarmSys.bit.EPOAlarm && eNoBatt != g_ComInfo.Com1.bit.BatMode)) // 开机键不按下INV不运行，可运行PV和LLC
        {
            g_ComInfo.Com1.bit.InvOnOff = eTurnOff;
    ...
    if(0 == uiSciSetDataBag[eSetGenConnectToGridPortEnable])
    {
            ...
            )
        {
                ...
                )
            {
                ...
                {
                    s_uiSignalExitCnt = 0;
                    if((eLithiumSOC == g_ComInfo.Com1.bit.BatMode) && (strBMSLogicInfo.uiBMS1DataAllowUse))//(strBMSLogicInfo.uiBMS1Connect))
                    {
...
static void System_SBUModeProCess(void)
{
    ...
    {
        g_ComInfo.Com1.bit.InvForceOffAllow = 1;
    }
    else
    {
        g_ComInfo.Com1.bit.InvForceOffAllow = 0;
    }
}
...
/******************************************************************************
Function Name: System_CtDirectionChk
--------------------
Function Descriptions: 自动检测CT方向
Parameter Name list: 开启后检测是否为防逆流To CT 和并网模式，满足后主动增加馈电100W，持续200mS，记录电网功率和CT功率，比对馈电前和馈电后的功率变化，连续三次，得出CT方向是否接反。
ReturnType: 如何确保逆变一定进入馈电？？？
Refer documents: 100mS Task
******************************************************************************/
// stPower.PGrid.PhaseA 是取电为正，馈电为负；显示是一致的。
// stPower.PGridCt.PhaseA 是取电为负，馈电为正；显示是取反的。
//  工况                  逆变状态         CT方向正确
// ①充电 + 有家庭负载 --> 充电变馈电 --> 显示CT功率减小；
// ②充电 + 无家庭负载 --> 充电变馈电 --> 显示CT功率减小；
// ③不充不馈 + 有家庭负载 --> 0功率变馈电 --> 显示CT功率减小；
// ④不充不馈 + 无家庭负载 --> 0功率变馈电 --> 显示CT功率减小；
// ⑤馈电 + 有家庭负载 --> 一直馈电 --> 显示CT功率减小 或 馈电量增加；
// ⑥馈电 + 无家庭负载 --> 一直馈电 --> 显示CT功率减小 或 馈电量增加；
void System_CtDirectionChk(void)
{
    Uint16 uiCTDirChkEn = 0;
    static Uint16 uiCTDirChkStep = 0, uiCTDirChkCnt = 0;

    if(   (TRUE == (uiSciCommandDataBag[eComATECommand] & 0x0004))
       && (eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode)
       )
    { uiCTDirChkEn = 1; }
    else
    ...
    if(uiCTDirChkEn)
    {
        ...
        {
            uiCTDirChkCnt ++;
            g_ComInfo.uiCtDircCheckEn = TRUE;
        }
        else
        {
            uiCTDirChkCnt = 0;
            g_ComInfo.uiCtDircCheckEn = FALSE;
        }
