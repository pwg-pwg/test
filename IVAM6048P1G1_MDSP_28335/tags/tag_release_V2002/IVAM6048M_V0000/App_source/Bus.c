...
static void Bus_SysBusVoltRefCalu(void)
{
    ...
    if((eNoBatt != g_ComInfo.Com1.bit.BatMode) && (FALSE == g_SysAlarm.ubAlarmDc.bit.BatOpen))
    {
        ...
        if(FALSE == BatAbnormalFlg)//未触发电池异常
        {
    ...
    if(ePvOnline == g_SystemInfo.Source.bit.Pv1 || ePvOnline == g_SystemInfo.Source.bit.Pv2)
    {
        ...
        VBusRefSetByPvFilter += ((VBusRefSetByPv - VBusRefSetByPvFilter) * 0.0125);
    ...
    //离网/油机,若触发电池充电充不进去导致母线高,则PV母线给定不加20V
    if((TRUE == BatAbnormalFlg) && ((eInv_OnGridMode != g_InvVar.InvFlag.bit.InvWorkMode) || (1 == uiSciSetDataBag[eSetGenConnectToGridPortEnable])))//离网
    //	if(TRUE == BatAbnormalFlg)
    {
    ...
    if((1000 == strBMSLogicInfo.uiBMS1BatSOC) && ((eInv_OnGridMode != g_InvVar.InvFlag.bit.InvWorkMode) || (1 == uiSciSetDataBag[eSetGenConnectToGridPortEnable])))//离网/PV给电池充电
    {
        g_SystemInfo.fPvBusVoltSet = g_DcDcVar.fBusVoltCtrlRef;//SOC已充满,按照浮充电压点(电池包传过来)
    }

    UpDownLimit(g_SystemInfo.fPvBusVoltSet, cPvBusSetMax, cPvBusSetMin);  // 限制在360和550之间
    g_SystemInfo.uwPvBusVoltSet = F2Q5(fReal_BusVolt(g_SystemInfo.fPvBusVoltSet));
