...
#define		PLL_OutputToBypass_KP 				0.0002//0.0005  
#define		PLL_OutputToBypass_KI 				0.0001//0.00004  
...
void CanInModDeal_ParaFrameInit(void)
{
    ...
    g_SystemInfo.fPllOut2BypassKp = 0.0002;
    g_SystemInfo.fPllOut2BypassKi = 0.0001;
}
...
inline void CanPara_MasterDataSend(void)
{
    ...
    //==================================================
    //              数据发送  这一段赋值用了2.65us 可以优化
    //==================================================
//    if( uiFrameParallelEnable )
//    {
//        strSysToSysSendBuffer.data11 = uiMasterFlag_Sys2UpmFlag;    // 充当系统板下发系统数据
//        strSysToSysSendBuffer.data12 = (int16)(fMasterAnalogData_GridAvgPa * 10000);
//        strSysToSysSendBuffer.data13 = (int16)(fMasterAnalogData_GridAvgPb * 10000);
//        strSysToSysSendBuffer.data14 = (int16)(fMasterAnalogData_GridAvgPc * 10000);
//
//        if(fMasterAnalogData_Omega >= 0)
//        {
//            strSysToSysSendBuffer.data21 = (int16)(fMasterAnalogData_Omega * cKParaDeltaStepRadTx + 0.5);
//        }
//        else
//        {
//            strSysToSysSendBuffer.data21 = (int16)(fMasterAnalogData_Omega * cKParaDeltaStepRadTx - 0.5);
//        }
//        strSysToSysSendBuffer.data22 = (int16)(fMasterAnalogData_OutRmsAVolt * 10000);
//        strSysToSysSendBuffer.data23 = (int16)(fMasterAnalogData_OutRmsBVolt * 10000);
//        strSysToSysSendBuffer.data24 = (int16)(fMasterAnalogData_OutRmsCVolt * 10000);
//
//        strSysToSysSendBuffer.data31 = (int16)(fMasterAnalogData_OutAvgQa * 10000);
//        strSysToSysSendBuffer.data32 = (int16)(fMasterAnalogData_OutAvgQb * 10000);
//        strSysToSysSendBuffer.data33 = (int16)(fMasterAnalogData_OutAvgQc * 10000);
//        strSysToSysSendBuffer.data34 = 0;
//
//        strSysToSysSendBuffer.data41 = uiMasterSetDataID;
//        strSysToSysSendBuffer.data42 = uiMasterSetDataVaule;
//        strSysToSysSendBuffer.data43 = 0;
//        strSysToSysSendBuffer.data44 = 0;
//    }
...
//    strSysToSysSendBuffer.data34 = 0;
    ...
    //  把接口数据压入CAN物理层
    //======================================================
    ParaCan_SlaverCanTxData(uiSelfFrameCanID, 0, FRAME_20MS_TYPE, 12);
...
//    bSelfFrameFlagData_ToConstVolt  = strBatChgState.unFlag.bit.bToContVolt;
    ...
    else  if(FALSE == strBatChgState.unFlag.bit.bToFloatVolt)
    {
    ...
    uiSelfFrameAnalogData_CurrRmsA = 0;
    uiSelfFrameAnalogData_CurrRmsB = 0;
    uiSelfFrameAnalogData_CurrRmsC = 0;
    // 离网降压百分比
    uiSelfFrameAnalogData_DeratPerc = (int16)(strDeratedPercent.fInvVoltlimitOrder * 1000);
    // 电池放电电流
    ...
    uiFrameAnalogData_DeratPerc(uiSelfFrameCanID)   = uiSelfFrameAnalogData_DeratPerc;
...
void CanPara_SelfFrameDataCopyWhenIDChange(void)
{
    ...
    uiDataDealTemp                                   = uiFrameAnalogData_DeratPerc(uiSelfFrameCanID);
    uiFrameAnalogData_DeratPerc(uiSelfFrameCanID)     = uiSelfFrameAnalogData_DeratPerc;
    uiFrameAnalogData_DeratPerc(uiSelfFrameCanIDPre)  = uiDataDealTemp;
...
void CanPara_DataSummary(void)
{
    ...
    Uint16 s_uiUPMBatCurrPercSummary = 0;
    ...
    for(uiFrameIDTemp = 0; uiFrameIDTemp < Frame_Max_Num; uiFrameIDTemp ++)
    {
		   ...
		   && (uiFrameParaPhaseSeq == bFrameFlagData_MdlPhaseSeq(uiFrameIDTemp)) ) //机架通信异常恢复后才开始统计其信息 否则在一台机器并机线断开并且EPO 在此接入系统会导致输出判断为EPO故障 断输出
        {
            ...
            if(eInvMainState_OffGrid == bFrameFlagData_InvState(uiFrameIDTemp))
            {
                ...
                if(s_uiUPMDeratPercMin > (int16)(uiFrameAnalogData_DeratPerc(uiFrameIDTemp))) // 提取各模块最小降额系数
                {
                    s_uiUPMDeratPercMin = (int16)(uiFrameAnalogData_DeratPerc(uiFrameIDTemp));
                }

                s_uiUPMBatCurrPercSummary += (int16)(uiFrameAnalogData_BatDisCurr(uiFrameIDTemp)); // 汇总系统电池放电电流
            }
            //Step2
...
//            if(bFrameFlagData_ToConstVolt(uiFrameIDTemp))  { s_uiUPMToConstVoltCnt ++; }
    ...
    uiSysMdlTurnOnNum    = s_uiInvMdlTurnOnCnt;

    fMasterAnalogData_OutDeratVal = (float)((int16)s_uiUPMDeratPercMin) * 0.001f;
    /* 限幅 */
    Dnlimit(fMasterAnalogData_OutDeratVal, ((float32)180.0f * g_GridManager.Rated.f32VOutInvt));
    g_ParaVar.fBatCurrSummyInPhase = s_uiUPMBatCurrPercSummary;
    //-----------------------------------------
    // 整个同相系统的模拟量信息
    //-----------------------------------------
    ...
    uiSysTxAnalogData_OutAvgQa  = ((int16)iSysReactivePowerAvgA);
    uiSysTxAnalogData_OutAvgQb  = ((int16)iSysReactivePowerAvgB);
    uiSysTxAnalogData_OutAvgQc  = ((int16)iSysReactivePowerAvgC);
    fMasterAnalogData_OutAvgQa = (float)((int16)iSysReactivePowerAvgA) * 0.0001; 
    ...
    if(ARPToCTMode == uiSciSetDataBag[eSetWorkMode]) // CT功率正是馈电 流入电网
    {
        fMasterAnalogData_GridAvgPa = (float)((int16)iSysCTPowerAvgA) * 0.001;
    ...
    {
        fMasterAnalogData_GridAvgPa = (float)((int16)iSysCTPowerAvgA) * 0.0001;
...
//        else
    ...
    if((s_uiSmartOutEnableCnt >= uiSysRunningNum) && (uiSysRunningNum > 0))
    {
    ...
    // stateTest[1] = s_uiUPMBatCutOff;
//    bMasterFlag_UpmBatCutOff = bMFrameFlag_UpmBatCutOff;
    bMasterFlag_UpmBatCutOff = bMFrameFlagData_UpmCutOff(OutSeq_PhaseA) ;

    bMasterFlag_UpmBatChgMode = bMFrameFlag_UpmBatChgMode;
...
void CanPara_MaterDataSum(void)
{
    ...
    float  fMframeUpmBatCurrSum = 0;
    Uint16 uiMframeUpmBatCutOffNum = 0;
    static Uint16  uiMframeUpmBatCutOffDelay = 0;
...
//                }
            ...
            fMframeUpmBatCurrSum += uiMFrameData_BatDisCurr(uiMFrameIDTemp);
...
//       || (uiMFrameUpmChgModeCnt[eBatManage_DisCharge] == ePhaseSum && uiFrameParaOutType == Flag_ThreeParaFrame) )
    ...
    g_ParaVar.fBatCurrSummyInSys = fMframeUpmBatCurrSum;
    //----------------------------------
	// 机架主机自己保存
	// uiMFrameFlag_Sys2UpmFlag  = uiMasterFlag_Sys2UpmFlag;
	// uiMFrameFlag_Sys2UpmFlag2 = uiMasterFlag_Sys2UpmFlag2;
    ...
    if(uiFrameParallelEnable)
    {
        ...
        for(uiParallelHeartBeatCntTemp = 0; uiParallelHeartBeatCntTemp < Frame_Max_Num; uiParallelHeartBeatCntTemp ++)
        {
            if(uiParallelHeartBeatCntTemp != uiSelfFrameCanID)                  //非本机架的通讯异常判断
            {
                ...
                if(uiParaCanOkCnt[uiParallelHeartBeatCntTemp] > 100)
                {
                    uiFrameData_ComOK[uiParallelHeartBeatCntTemp]   = 1;
                    uiParaCanOkCnt[uiParallelHeartBeatCntTemp]      = 120;
                }
            }
        }
        ...
        {
            uiCanComFaultDelayCnt = 0;
        }
//        if(uiCanComFaultDelayCnt < SecondBy1msBase(60))
        if(uiCanComFaultDelayCnt < SecondBy1msBase(5))
        {
            uiCanComFaultDelayCnt ++;  //上电延时
        }
        else
        {
//            if(uiFrameOnlineNum < 2)
            if(uiFrameOnlineNum < 2 || uiFrameOnlineNum < uiFrameOnlineMaxNum)
            {
...
void CanPara_FrameNumChangeCheck(void)
{
    ...
    {
        if(uiSelfSystemFrameNumOutEnCnt < FrameNumSteadyTime) uiSelfSystemFrameNumOutEnCnt ++; //判断出机架数量改变后，延时180ms判断当前系统内机架数量是否满足最小机架数量
        else
        {
...
//                    strPllValue.Flag.bit.ParaMstSendDataRad = FALSE;
            ...
            && (FALSE == bSelfFrameFlagData_MdlMaster) )
    {
        ...
        switch(uiCanFrameManageState)
        {
            ...
            {
                // 根据ID先确定延时 延时1ms
                // 从机发送3包数据接近0.8ms 时基是208us  计数4次就是0.832ms
                if(uiSelfFrameCanID < uiMasterFrameID)  // 从机ID小于主机
                {
                    s_uiSlaveSendDelayTime = 4 * uiSelfFrameCanID;
                }
                else if((uiSelfFrameCanID > uiMasterFrameID) && (0 != uiSelfFrameCanID))// 从机ID大于主机
                {
                    s_uiSlaveSendDelayTime = 4 * (uiSelfFrameCanID - 1);
                }
...
void CanPara_ComDataDeal(void)
{
    ...
    if(uiFrameParallelEnable)
    {
        ...
        {
            if(uiMasterDataDelayCnt < OnsMsBy1msBase(30))
            {
            ...
            if(   (uiMasterDataDelayCnt >= OnsMsBy1msBase(30))  // 主机延时达到后发送
               ...
               || (TRUE == uiPhaseAMasterDataHadReceive) )      // 组三相的时候调用
            {
                CanPara_SelfFrameDataRenew();
...
void CanPara_FreqReguLoop(void)
{
    ...
    if(    (uiSysComOkNum == uiSysGridNormalNum)
        ...
        && (   (   (FALSE == g_SysFault.Flag.bit.GridFault)
                && (   (0 == uiSciSetDataBag[eSetGenConnectToGridPortEnable]) // 电网正常 且 (未使能油机接入市电端口 或 满足油机接入市电端口条件)
...
//				g_ParaVar.ParaFlag.bit.bRsvd15 = 1;
...
//			g_ParaVar.ParaFlag.bit.bRsvd15 = 0;
			...
			&& eGenPortMode_Generator == g_ComInfo.Com1.bit.GenModeSet )	// 油机正常，输出跟踪油机
	{
		...
		if(fabs(sin(fDeltaRad)) < 0.17) // < 5° 0.087
		{
			if(++g_ParaVar.uiPCCPllToGridOKCnt > SecondBy20msBase(2))// 实际是20mS调用一次 //if(++g_ParaVar.uiPCCPllToGridOKCnt > SecondBy1msBase(2))
			{
				g_ParaVar.uiPCCPllToGridOKCnt = SecondBy20msBase(2);// 实际是20mS调用一次 //g_ParaVar.uiPCCPllToGridOKCnt = SecondBy1msBase(2);
		...
		{
            if(++g_ParaVar.uiPCCPllToGridBackCnt > SecondBy20msBase(2))
            {
                g_ParaVar.uiPCCPllToGridBackCnt = SecondBy20msBase(2);// 实际是20mS调用一次 //g_ParaVar.uiPCCPllToGridOKCnt = SecondBy1msBase(2);
			    g_ParaVar.ParaFlag.bit.bPCCPllToGenOKFlag = 0;
            }
...
//		else
	...
	// 这里得到的其实是detla w
	strPllToSys.fFinalStepRad = strPllToSys.fFreStepRad - strPllOfSys.fFreStepRad + strPllToSys.fPhaseStepRad;	// Wg - W0 + DeltaW
...
void CanPara_RmsVoltReguLoop(void)
{
        ...
        )
    {
        ...
        // }
        if(fRxFromSysAnalog_OutDerarVal <= 0.99f)
        {

            fOutputVoltRmsASet = fRxFromSysAnalog_OutDerarVal * g_GridManager.Rated.f32VOut; // TODO,此处需要判定是否已经处于降额中，如果正在降额，则不跟踪幅值，立即切换。
        }
        else
        {
    ...
    else
    {
        ...
        fOutputVoltRmsASet = g_GridManager.Rated.f32VOut * fRxFromSysAnalog_OutDerarVal;    //g_ParaVar.fRatedVoltDecratePer;
        uiPCCAmpToGridOKCnt = 0;
        g_ParaVar.ParaFlag.bit.bPCCAmpToGridOKFlag = 0;
        g_ParaVar.ParaFlag.bit.bPCCAmpToGenOKFlag = 0;
    }
...
void CanPara_SysDataRenew(void)
{
    if(bSelfFrameFlagData_MdlMaster)
    {
        ...
        fRxFromSysAnalog_OutDerarVal = fMasterAnalogData_OutDeratVal;
    }
    else
    {
