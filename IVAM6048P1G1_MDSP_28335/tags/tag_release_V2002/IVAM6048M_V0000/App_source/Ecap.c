...
void ECAP_Initialize(void)
{
    ...
    strPllToInv.fRlyOnPoint1 = 270 - (cRlyCloseDlyTime2 * 0.36f * g_StrEcap.FreqFilt.Inv) - 2;// 实测逆变点要前移// 360 - ((RlyTime * 360 ) / 1000 / g_StrEcap.FreqFilt.Gen) - 90;
    UpDownLimit(strPllToInv.fRlyOnPoint1, 174.0f, 76.5f);
    strPllToInv.fRlyOnPoint2 = 270 - (cRlyCloseDlyTime1 * 0.36f * g_StrEcap.FreqFilt.Inv) - 2;// 实测逆变点要前移// 360 - ((RlyTime * 360 ) / 1000 / g_StrEcap.FreqFilt.Gen) - 90;
...
*/
    ...
    // [35hz, 70hz]
    //--------- 逆变继电器吸合角度计算 ---------//
    strPllToInv.fRlyOnPoint1 = 270 - (cRlyCloseDlyTime2 * 0.36f * g_StrEcap.FreqFilt.Inv) - 2;// 实测逆变点要前移// 360 - ((RlyTime * 360 ) / 1000 / g_StrEcap.FreqFilt.Inv) - 90;
    UpDownLimit(strPllToInv.fRlyOnPoint1, 174.0f, 76.5f);
    strPllToInv.fRlyOnPoint2 = 270 - (cRlyCloseDlyTime1 * 0.36f * g_StrEcap.FreqFilt.Inv) - 2;// 实测逆变点要前移// 360 - ((RlyTime * 360 ) / 1000 / g_StrEcap.FreqFilt.Inv) - 90;
    UpDownLimit(strPllToInv.fRlyOnPoint2, 178.0f, 85.0f);
    //--------- 逆变继电器断开角度计算 ---------//
    // strPllToInv.fRlyOffPoint1 = 270 - (cRlyOpenDlyTime2 * 0.36f * g_StrEcap.FreqFilt.Inv);// 360 - ((RlyTime * 360 ) / 1000 / g_StrEcap.FreqFilt.Grid) - 90;
    // UpDownLimit(strPllToInv.fRlyOffPoint1, 245.0f, 219.0f);
    // strPllToInv.fRlyOffPoint2 = 270 - (cRlyOpenDlyTime1 * 0.36f * g_StrEcap.FreqFilt.Inv);// 360 - ((RlyTime * 360 ) / 1000 / g_StrEcap.FreqFilt.Grid) - 90;
...
{
    Uint16 uiEcapTemp = 0;
    static Uint16 uiInvOcpCntTemp = 0;

    // 硬件触发一次封波，恢复时间约为20uS，所以要放在中断级
