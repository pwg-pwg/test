...
static void Fault_BatAbnormalCheck(void);       // 检测电池充放电断开电池检测
...
void Fault_ModuleFaultSumry(void)
{
    ...
    // Grid Module
    if(g_SysFault.ubFaultAc.fault.fault1 || g_SysFault.Flag.bit.ParaGridLose)
    {
    ...
    // Gen Module
    if((eGenPortMode_Generator == g_ComInfo.Com1.bit.GenModeSet) && (g_SysFault.ubFaultGen.all || g_SysFault.Flag.bit.ParaGenLose))
    {
...
//  if((g_InvVar.InvFlag.bit.InvPwmEnable)&&((EPwm1Regs.TZFLG.bit.OST)||(EPwm2Regs.TZFLG.bit.OST)||(EPwm3Regs.TZFLG.bit.OST)||(EPwm4Regs.TZFLG.bit.OST)))
    ...
    {
        if((FALSE == g_SysFault.ubFaultAc.bit.InvTz))//&&(++s_u16InvTzCnt >= 5))    // 单次触发后立即保护
        {
            s_u16InvTzCnt = 0;
...
static void Fault_INVLLShortCheck(void)
{
    ...
    fPhaseErr = fabs(strPllToInv.fFinalRad - strPllToPCC.fFinalRad);


    if(FALSE == g_SysFault.ubFaultAc.bit.LLShortFault)
    {
        if((fPhaseErr > cRad_45Angle) && (stValue.fRmsReal.IInvA >= 18.0f))   //f32IoutA
        {
            if(++s_u16CntEnter >= cCnt_8)
            {
                s_u16CntEnter = 0;
    ...
    {
        if(++s_ulLLShortClearLockCnt >= MinuteBy20msBase(30)) //每隔30Min将Lock计数器清零
        {
    ...
    {
        if(++s_ulLLClearLockFlagCnt >= MinuteBy20msBase(60) || !g_ComInfo.Com1.bit.KeyOn)
        {
...
static void Fault_InvRmsOcpCheck(void)
{
    ...
    if(FALSE == g_SysFault.ubFaultAc.bit.AcRmsOCP)
    {
        if(   (stValue.fRmsScale.IInvA >= cInvCurrOvLimit)
           || (stPower.SInv.PhaseA >= cPowerOvLimit)
        ...
        if(eInvMainState_SoftStart == g_InvVar.InvFlag.bit.bInvMainSts && g_InvVar.InvFlag.bit.InvNormalLogicStart)
        {
            if(  (   eInv_OffGridMode == g_InvVar.InvFlag.bit.InvWorkMode
              ...
              || (  eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode
                 && (  fabs(stValue.fRmsReal.VInvA - (stValue.fRmsReal.VGridA * g_InvVar.fOnGridVoltSoftFactor)) > 20
              ...
              || (   eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode
                  && (  fabs(stValue.fRmsReal.VInvA - (stValue.fRmsReal.VGenA * g_InvVar.fOnGridVoltSoftFactor)) > 20
                  /*|| fabs(stValue.fRmsReal.VInvB - stValue.fRmsReal.VGenB * g_InvVar.fOnGridVoltSoftFactor) > 20*/)
                 )
              )
            {
                if(++s_uiInvSoftFailCnt > cDelay200ms_20ms)  //
                {
...
//	}
    ...
    if(FALSE == g_SysFault.ubFaultAc.bit.LoadOver110)
    {
        ...
        {
            if(   (stPower.SInv.PhaseA > fLoadOver1Point)                                           // 逆变 视在功率过载
               || ((stValue.fRmsScale.IInvA > fLoadOver1Point) && (stPower.PInv.PhaseA > 0.02f))    // 逆变 电流过载 -- 需要规定放电，避免充电触发过载
               || (fLoadTemp > fLoadOver4Point)        // 输出+智能负载 视在功率过载
               || (fCurrTemp > fLoadOver4Point)        // 输出+智能负载 电流过载
               )
            {
        ...
        {
            if(   (stPower.SInv.PhaseA > fLoadOver2Point)                                           // 逆变 视在功率过载
               || ((stValue.fRmsScale.IInvA > fLoadOver2Point) && (stPower.PInv.PhaseA > 0.02f))    // 逆变 电流过载 -- 需要规定放电，避免充电触发过载
               || (fLoadTemp > fLoadOver2Point)        // 输出+智能负载 视在功率过载
        ...
        {
            if(   (stPower.SInv.PhaseA > fLoadOver3Point)                                           // 逆变 视在功率过载
               || ((stValue.fRmsScale.IInvA > fLoadOver3Point) && (stPower.PInv.PhaseA > 0.02f))    // 逆变 电流过载 -- 需要规定放电，避免充电触发过载
               || (fLoadTemp > fLoadOver3Point)        // 输出+智能负载 视在功率过载
...
static void Fault_GridOverLoadCheck(void)
{
      ...
      )
    {
        ...
        if(uiSciSetDataBag[eSetGenInputRatePower] > cPowerRated) { fCurrTemp = (1.1f * ((float32)uiSciSetDataBag[eSetGenInputRatePower] * cKPowerRated)); }
        else { fCurrTemp = 1.1f; }
        //------ 油机过载判定 ------//
        if(FALSE == g_SysFault.ubFaultGen.bit.GenOverLoad)
        {
            if(  (stPower.SGen.PhaseA > fCurrTemp)   // 发电机只做一段过载 而且时间要快
    ...
    else
    {
        ...
        {
            if(  (stPower.SGrid.PhaseA > 1.25)   // 继电器规格书最大电流是40A，限定在38A=6KW/220*1.3933倍  软件写为1.25倍
...
// fCurrTemp = (float32)(int16)uiSciCommandDataBag[eComRsvd17]; // TEST
...
static void Fault_BusUnderVoltFastCheck(void)
{
    ...
    {
        if(stADC.fRealScale.VBus < cVBusUnIsrLimit)  // 325V
        {
            s_u16ChkCnt++;
            if(s_u16ChkCnt >= cCnt_5)
            {
...
//    static Uint16 s_u16BusUvpChkExtiCnt = 0;
    ...
    if(FALSE == g_SystemInfo.SysFlag.bit.BusVoltLowChk)
    {
        ...
        if(eInv_NoOutputMode != g_InvVar.InvFlag.bit.InvWorkMode && (eActiveByAc != g_ComInfo.Com1.bit.LiActiveSource))
        {
            g_SystemInfo.SysFlag.bit.BusVoltLowChk = TRUE;
        }
...
//    }
    ...
    //  Llc硬件过流
    //------------------------------------
    if((!liLlcCurrOver) || (g_SysFault.ubFaultDc.bit.LlcTz == TRUE))
    {
...
//      FaultCodeSet(eBatHardOC);
    ...
    // 电池电流过流保护，封PV驱动
    //------------------------------------
    if(   (g_SysAlarm.ubAlarmDc.bit.BatCurrHiOffPV)
       || (stADC.fReal.VBus > 550.0f)
       || (stADC.fReal.VBat > 60.5f) )
    {
        hoFVRTTRIPOn;
    }
...
//        }
        // if( (TRUE == g_SysFault.Flag.bit.GridFault) && (eGenOnline != g_SystemInfo.Source.bit.GenSource) ) //离网下，SOC和电压条件不满足，恢复输出
        if(   (   (g_SysFault.Flag.bit.GridFault)
               || (1 == uiSciSetDataBag[eSetGenConnectToGridPortEnable] && !g_ComInfo.Com1.bit.GenConnectToGridEn) //(电网故障 || 不满足油机接入电网端口条件 || SBU模式) 
               || (1 == uiSciSetDataBag[eSetInvForceOffGrid] && g_ComInfo.unSBUModeEn.bit.bSBUEn)
               )
...
//---------------------------------------------------------------------------
// #define BatAbnormalCheckEn  1
static void Fault_BatAbnormalCheck(void)
{
#ifdef BatAbnormalCheckEn
    ...
    static Uint16 s_u16BusMaxCnt3 = 0,s_u16BusMinCnt3 = 0;//当前值比最大值大
//	static Uint16 s_u16Cnt2 = 0;
    static float32 s_fBatVoltMax1 = 0,s_fBatVoltMin1 = 0,s_fBatVoltMax2 = 0,s_fBatVoltMin2 = 0;
    static Uint16 BatAbnormalFlg1 = 0,BatAbnormalFlg2 = 0;

    static float32 s_fBatVoltMaxBack = 0,s_fBatVoltMinBack = 0;
    static Uint16 s_uiBatNormalFloatingCnt = 0;
    float32 s_fBatTemp = 0;

    //待机下
    if(FALSE == BatAbnormalFlg1)
    {
        //电池建立母线,初始值
            ...
            || (g_SystemInfo.SysFlag.bit.PvWork) )
        {
            ...
            {
                //断电池检测,把母线冲高或者拉低，没有充电电流和放电电流
                //1、可能采样会有滞后,可能会有检测不到的时候
                if((s_fBatVoltMax1 + 0.5) < (mVBatRealCalc(stADC.fRealScale.VBat)))//比最大值大0.1V
                {stateTest[0] ++;hoTestToggle;
                ...
                {
                    s_fBatVoltMin1 = mVBatRealCalc(stADC.fRealScale.VBat);//求出最小值
                    s_u16WaveMinCnt1++;
                }
                //-----------------------------//
                if(s_u16WaveMaxCnt1 > 0)
                {
                    if(mVBatRealCalc(stADC.fRealScale.VBat) < (s_fBatVoltMax1 - 0.5))
                    {stateTest[4] ++;
                        // if(++s_uiBatNormalFloatingCnt > 3)
                        // {
                            // s_uiBatNormalFloatingCnt = 0;
                            s_u16WaveMaxCnt1 = 0;
                        // }
                    }
                    else if(mVBatRealCalc(stADC.fRealScale.VBat) < s_fBatVoltMax1)
                    {stateTest[5] ++;
                        if(++s_uiBatNormalFloatingCnt > 10)
                        {
                            s_uiBatNormalFloatingCnt = 0;
                            s_u16WaveMaxCnt1 = 0;
                        }
...
stateTest[1] = (int16)(s_fBatVoltMax1 * 10);
stateTest[3] = (int16)(s_fBatVoltMin1 * 10);
                ...
                //3、以BUS作为最后一道防线
                //BUS大于最高电池浮充电压*9倍
                s_fBatTemp = (g_ComInfo.fBat1VoltUpLimit * cTransformerRatio) + 10;
                Uplimit(s_fBatTemp, 550); // 限定最高550V
                if(stADC.fRealScale.VBus > Set_BusVolt(s_fBatTemp)) // 均/浮充点后再冲高10V
                {
                    s_u16BusMaxCnt3++;
                }
                //BUS小于46V*9倍
                if(stADC.fRealScale.VBus < Set_BusVolt(414))
                {
                    s_u16BusMinCnt3++;
                }

                //电池充电断开在这时间反复来回波动或者一直升
                //电池放电断开在这时间反复来回波动或者一直降
                if(    (s_u16WaveMaxCnt1 >= 2) || (s_u16WaveMinCnt1 >= 2)
                    || (s_u16WaveMaxCnt2 >= 1) || (s_u16WaveMinCnt2 >= 1)
                    || (s_u16BusMaxCnt3  >= 5) || (s_u16BusMinCnt3  >= 5)
                    )
                {stateTest[6] ++;
                    s_u16WaveMaxCnt1 = 3;
                    s_u16WaveMinCnt1 = 3;
                    ...
                    s_u16BusMaxCnt3 = 5;
                    s_u16BusMinCnt3 = 5;
                    BatAbnormalFlg1 = TRUE;//电池异常/电池Mos脱开
                }
            }
        }
    }
    ...
    {
        //电池正常充放功率需要更新电池电压点
        //模拟锂电池,电池充放电,断开电池,电池电压拉低或者升高;电池没电流,那么会一直维持住当前触发到的母线电压,BatAbnormalFlg1 = TRUE;
        //模拟铅酸电池,电池充放电,电池电压拉低或者升高;电池有电流,这个时候可能会母线电压慢慢抬高到57.6V,同时电流限制为0,BatAbnormalFlg1 = TRUE又FALSE来回,直到电池电流0;
        if(    (fabs(mIDcDcRealCalc(stADC.fRealScale.IDcDc)) >= 10.0)
...
//电池没断开,但是把电池冲高了,有充电电流
//电池充电,充不进去将母线电压拱高,停止电池电压的母线给定随动(电池能钳位住端口)
//	if(stPower.fPBatReal < -250)
//	{
//		//有PV
...
//		{
//			if(FALSE == BatAbnormalFlg2)//母线电压大于PV母线给定且电池电压大于恒定充电电压0.2V
//			{
//				if( (stADC.fRealScale.VBus > g_SystemInfo.fPvBusVoltSet)
//				  &&((mVBatRealCalc(stADC.fRealScale.VBat) - g_ComInfo.fBat1VoltUpLimit) > 0.1)
//				  )
//				{
//					if(s_u16Cnt2++ >=  2)
//					{
//						s_u16Cnt2 = 2;
//						BatAbnormalFlg2 = TRUE;
//					}
//				}
//			}
//		}
...
//		{
//			if(FALSE == BatAbnormalFlg2)//母线电压大于逆变母线给定且电池电压大于恒定充电电压0.2V
//			{
//				if( (stADC.fRealScale.VBus > g_InvVar.fInvBusVoltSet)
//				  &&((mVBatRealCalc(stADC.fRealScale.VBat) - g_ComInfo.fBat1VoltUpLimit) > 0.1)
//				  )
//				{
//					if(s_u16Cnt2++ >=  2)
...
//		}
//	}
//	else if(stPower.fPBatReal > 500)//放电才退出
//	{
//		BatAbnormalFlg2 = FALSE;
//		s_u16Cnt2 = 0;
//	}

    if((TRUE == BatAbnormalFlg1) || (TRUE ==  BatAbnormalFlg2))
    {stateTest[7] ++;
        BatAbnormalFlg = TRUE;
    }
    else
    {stateTest[8] ++;
        BatAbnormalFlg = FALSE;
    }
#endif
    ...
    //------------- BAT快速过压检测 -------------//
    if(stADC.fRealScale.VBat > 1.2)//60.0
    {
...
static void Fault_BatOverCurrCheck(void)
{
    ...
    if(FALSE == g_SysFault.ubFaultDc.bit.BatOCP)
    {
        ...
        //
        //     if(stValue.fBatChargeCurrAveReal > ((float32)uiTemp * 0.1f))
        //     {
        //         if(++s_u16OcpCntEnter4 >= SecondBy20msBase(30))
        //         {
        //             s_u16OcpCntEnter4 = 0;
        //             g_SysFault.ubFaultDc.bit.BatOCP = TRUE;
        //             //  FaultCodeSet(eBatSoftOC);
...
static void Fault_InOutReverseCheck(void)
{
    ...
    if(FALSE == g_SysFault.ubFaultSys.bit.InOutReverse)
    {
        ...
        {
            if(stValue.fRmsReal.VOutA > cVOutOver)   // about 60V
            {
...
static void Fault_SmartPortReverseCheck(void)
{
    ...
    if( (eGenPortMode_SmartLoad == g_ComInfo.Com1.bit.GenModeSet) && (FALSE == g_SysFault.ubFaultSys.bit.GenSmartPortReverse) )
    {
        if(eInvMainState_Idle == g_InvVar.InvFlag.bit.bInvMainSts)
        {
            if(stValue.fRmsReal.VGenA > cVOutOver)   // about 100V
            {
                if(++s_uwRChkCnt >= cDelay3s_20ms)
                {
                    s_uwRChkCnt = 0;
...
static void Fault_TempOverDerateCheck(void)
{
    ...
    {
        // if(   (uiSlaveAnalogData[eS2MRealData_LLCTemp] >= LLCHSTempDerateStart * 10)
        //    || (stTempValue.LLCMax >= LLCHSTempDerateStart) )
        if(uiSlaveAnalogData[eS2MRealData_LLCTemp] >= LLCHSTempDerateStart)
        {
    ...
    else
    {
        // if(   (uiSlaveAnalogData[eS2MRealData_LLCTemp] < LLCHSTempDerateStart * 10)
        //    && (stTempValue.LLCMax < LLCHSTempDerateStart) )
        if(uiSlaveAnalogData[eS2MRealData_LLCTemp] < (LLCHSTempDerateStart - 20))
        {
            if(++s_uChkCnt1 >= cDelay5s_20ms)
            {
                s_uChkCnt1 = 0;
...
/*
	            ...
	            {
	                ++s_uwLNReverseCnt;
	                if(s_uwLNReverseCnt >= cDelay1s_1ms)  // 添加跟踪时间
	                {
	                    s_uwLNReverseCnt = 0;
	            ...
	            {
	                s_uwLNReverseCnt = 0;
	                if(++s_uwLNNormalCnt >= cDelay100ms_1ms) // cDelay100ms_1ms  cDelay100ms_1ms
	                {
	                    s_uwLNNormalCnt = 0;
...
/*
                      ...
                      && FALSE == g_SysFault.Flag.bit.GenFault)
                {
                  // fPhaseErr = fabs(strPllToInv.fFinalRad - strPllToEcap.fFinalRad);  // Ecap的角度是主机发波的角度
                    fPhaseErr = fabs(strPllToInv.fFinalRad - strPllOfSys.fFinalRad);  // Ecap的角度是主机发波的角度
                }
	            ...
	            if(fabs(fPhaseErr - cRad_PI) < cRad_15Angle)
	            {
	                if(++s_uwGenLNReverseCnt >= cDelay1s_1ms)
	                {
	                    s_uwGenLNReverseCnt = 0;
	            ...
	            {
	                s_uwGenLNReverseCnt = 0;
	                if(++s_uwGenLNNormalCnt > cDelay100ms_1ms)
	                {
	                    s_uwGenLNNormalCnt = 0;
...
//		        if(++stateTest[9] > 6000)  stateTest[9] = 0;
/*
...
//                }
				...
				else if(fabs(fPhaseErr2 - cRad_180) < cRad_15Angle)  // LN接反
                {
                    s_uwBCReverseCnt = 0;
                    s_uwLNNormalCnt = 0;
                    if(++s_uwLNReverseCnt >= cDelay1s_1ms)  // 40ms判断cDelay40ms_1ms
                    {
                        s_uwLNReverseCnt = 0;
                        g_SysFault.ubFaultSys.bit.ParaLNReverse = TRUE;
                        g_ParaVar.ParaFlag.bit.GridLNCheckFinish = TRUE;
...
//    static Uint16 s_u16Cnt3 = 0;
    ...
    {
        if(liInvCurrOver && g_StrEcap.uiInvOCPCntByOneCycle == 0)
        {
            s_u16Cnt2++;
...
//              //  FaultCodeClear(eDSP2Fault);
    ...
    if(g_SysFault.ubFaultSys.bit.APS12V_Fault)
    {
        ...
        {
            if(++s_u16Cnt9 >= SecondBy20msBase(5))
            {
                s_u16Cnt9 = 0;
    ...
    if(g_SysFault.ubFaultDc.bit.BusHwOVP)
    {
        if(liBUSVoltOver)
        {
            if(++s_u16Cnt10 >= SecondBy20msBase(5))
            {
                s_u16Cnt10 = 0;
    ...
    if(g_SysFault.ubFaultDc.bit.BatHwOVP)
    {
        if(liBatVoltOver)
        {
            if(++s_u16Cnt11 >= SecondBy20msBase(5))
            {
                s_u16Cnt11 = 0;
    ...
    if(g_SysFault.ubFaultAc.bit.GfciHardOCP)
    {
        if(hiGFCI_OCP)
        {
            if(++s_u16Cnt12 >= SecondBy20msBase(5))
            {
                s_u16Cnt12 = 0;
...
Parameter Name list:
ReturnType:
Refer doctuments: 周期检测硬件过流次数，工频周期内触发超过10次，连续两个周期以上即报故障
******************************************************************************/
static void Fault_InvHardWareOcpCheck(void)
{
    static Uint16 s_u16CntEnter = 0;
