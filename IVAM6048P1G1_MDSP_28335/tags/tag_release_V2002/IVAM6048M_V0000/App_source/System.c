...
//    uiSciCommandDataBag[eComRemoteOff] = TRUE;   // 这个不要了  默认开机 但是监控可以下发关机 @todo Ivan 240730
...
static void System_EcoModeProCess(void)
{
    ...
    // TOU模式的设置量要做限幅
    // static float32 fBatVoltSum = 0, fBatVoltAver = 0;	
       ...
       )
    {
        ...
        {
            Dnlimit(uiSciSetDataBag[eSetECO1_SOC], g_ComInfo.uwBat1ForceChgSocStart + 1);
            Dnlimit(uiSciSetDataBag[eSetECO2_SOC], g_ComInfo.uwBat1ForceChgSocStart + 1);
            Dnlimit(uiSciSetDataBag[eSetECO3_SOC], g_ComInfo.uwBat1ForceChgSocStart + 1);
            Dnlimit(uiSciSetDataBag[eSetECO4_SOC], g_ComInfo.uwBat1ForceChgSocStart + 1);
            Dnlimit(uiSciSetDataBag[eSetECO5_SOC], g_ComInfo.uwBat1ForceChgSocStart + 1);
            Dnlimit(uiSciSetDataBag[eSetECO6_SOC], g_ComInfo.uwBat1ForceChgSocStart + 1);
        }
        else
        {
            Dnlimit(uiSciSetDataBag[eSetECO1_Volt], g_ComInfo.fBat1ForceChgVoltStart + 10);
            Dnlimit(uiSciSetDataBag[eSetECO2_Volt], g_ComInfo.fBat1ForceChgVoltStart + 10);
            Dnlimit(uiSciSetDataBag[eSetECO3_Volt], g_ComInfo.fBat1ForceChgVoltStart + 10);
            Dnlimit(uiSciSetDataBag[eSetECO4_Volt], g_ComInfo.fBat1ForceChgVoltStart + 10);
            Dnlimit(uiSciSetDataBag[eSetECO5_Volt], g_ComInfo.fBat1ForceChgVoltStart + 10);
            Dnlimit(uiSciSetDataBag[eSetECO6_Volt], g_ComInfo.fBat1ForceChgVoltStart + 10);
        }
    }
            ...
            )
    {
        ...
        if(eLithiumSOC == g_ComInfo.Com1.bit.BatMode)
        {
            ...
            Dnlimit(uiSciSetDataBag[eSetECO1_SOC], uiSciSetDataBag[eSetOnGridBatAutoStartChargeSOC] + 1);
            Dnlimit(uiSciSetDataBag[eSetECO2_SOC], uiSciSetDataBag[eSetOnGridBatAutoStartChargeSOC] + 1);
            Dnlimit(uiSciSetDataBag[eSetECO3_SOC], uiSciSetDataBag[eSetOnGridBatAutoStartChargeSOC] + 1);
            Dnlimit(uiSciSetDataBag[eSetECO4_SOC], uiSciSetDataBag[eSetOnGridBatAutoStartChargeSOC] + 1);
            Dnlimit(uiSciSetDataBag[eSetECO5_SOC], uiSciSetDataBag[eSetOnGridBatAutoStartChargeSOC] + 1);
            Dnlimit(uiSciSetDataBag[eSetECO6_SOC], uiSciSetDataBag[eSetOnGridBatAutoStartChargeSOC] + 1);
        }
        else
        {
            Dnlimit(uiSciSetDataBag[eSetECO1_Volt], uiSciSetDataBag[eSetOnGridBat1AutoStartChargeVolt] + 10);
            Dnlimit(uiSciSetDataBag[eSetECO2_Volt], uiSciSetDataBag[eSetOnGridBat1AutoStartChargeVolt] + 10);
            Dnlimit(uiSciSetDataBag[eSetECO3_Volt], uiSciSetDataBag[eSetOnGridBat1AutoStartChargeVolt] + 10);
            Dnlimit(uiSciSetDataBag[eSetECO4_Volt], uiSciSetDataBag[eSetOnGridBat1AutoStartChargeVolt] + 10);
            Dnlimit(uiSciSetDataBag[eSetECO5_Volt], uiSciSetDataBag[eSetOnGridBat1AutoStartChargeVolt] + 10);
            Dnlimit(uiSciSetDataBag[eSetECO6_Volt], uiSciSetDataBag[eSetOnGridBat1AutoStartChargeVolt] + 10);
        }
    }
    // 上限不超过100
        ...
        && (eNoBatt != g_ComInfo.Com1.bit.BatMode) )
    {
        ...
        {
            uwEcoCalCnt  = 0;
            // fBatVoltAver = fBatVoltSum * 0.02f;
            // fBatVoltSum  = 0;
...
// if(eECOModeCharge == g_ComInfo.iEcoBatChgDisChgEn) { stateTest[0] ++; }
// else if(eECOModeDischarge == g_ComInfo.iEcoBatChgDisChgEn) { stateTest[1] ++; }
// stateTest[2] = (int16)(stValue.fAveReal.VBat * 10);
// stateTest[5] = g_ComInfo.iEcoBatChgDisChgEn;
// stateTest[6] = (int16)(fBatVoltSum * 10);
// stateTest[7] = (int16)(fBatVoltAver * 10);
// stateTest[8] = (int16)(g_ComInfo.fEcoModeStopVolt * 10);
}
...
static void System_InvPowerLimit(void)
{
    ...
    if((1 == uiSciSetDataBag[eSetGridChargeBatTotalEnable]) && (eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode) && (FALSE == g_ComInfo.Com1.bit.BatCharDis)) 
    {
        if(eDcDcMainState_Running == g_DcDcVar.DcDcFlag.bit.bMainSts)
        {
            ...
            else
            {
                ...
                {
                    fInvChrPowerLmt = -Set_Power(60);//0;//-Set_Power(20);
                }
                else
                {
                    // 电流放大10倍
        ...
        else
        {
            ...
            {
                // fInvChrPowerLmt = -Set_Power(100);
                if(g_ComInfo.Com1.bit.LiActiveFinish) { fInvChrPowerLmt = 0; }
                // else { fInvChrPowerLmt = -Set_Power(100); }
                else { fInvChrPowerLmt = -Set_Power(500); }
            }
        }
    ...
    else if((1 == uiSciSetDataBag[eSetGenChargeBatTotalEnable]) && (FALSE == g_SysFault.Flag.bit.GenFault) && (FALSE == g_ComInfo.Com1.bit.BatCharDis))
    {
        if(eDcDcMainState_Running == g_DcDcVar.DcDcFlag.bit.bMainSts)
        {
            ...
            {
                // 电流放大10倍
                fInvChrBatPower = -stValue.fAveReal.VBat * (float)uiSciSetDataBag[eSetGenChargeBatCurr] * 0.1f;
                fInvChrPowerLmt = Set_Power(fInvChrBatPower) * cKEffyDiv;
            }
            else
            {
                ...
                else
                {
                    ...
                    else // 非TOU 且 非强充 // TODO--add in 20250804--结合市电开始充电和停止充电来使用
                    {
                        fInvChrPowerLmt = -Set_Power(60);//0;//-Set_Power(20);
                    }
                }
    ...
    if(g_ComInfo.fInvChrPowerLmt > -Set_Power(60))//超过70W，市点下禁止，就会转至充电模式，因避免无法禁充的问题。
    {
        g_ComInfo.fInvChrPowerLmt = -Set_Power(60);//①作为环路限制，预留一定裕度给环路去积分 ②适当的调节回差
    }

// stateTest[2] = (int16)fReal_Power(fInvChrPowerLmt);
// stateTest[6] = (int16)fReal_Power(g_ComInfo.fInvChrPowerLmt);
    //---------------------------------------------------------------------//
    // INV放电功率限定
    if(   (eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode) 
       && (FALSE == g_ComInfo.Com1.bit.BatDischarDis)
       && (eDcDcMainState_Running == g_DcDcVar.DcDcFlag.bit.bMainSts)
       && (g_ComInfo.Com1.bit.UserTimeEn)
       && (0 == g_ComInfo.Com1.bit.GridShavingEn) //GPS禁止
       ) 
    {
        // 强充优先级最高
        // if(g_ComInfo.Com1.bit.BatForceChar)
        ...
        // else
        {
            fInvDischrPowerLmt = cKEffyDiv;

            // if(   (g_ComInfo.Com1.bit.UserTimeEn)
            //    && (eECOModeDischarge == g_ComInfo.iEcoBatChgDisChgEn)  // TOU模式且处于放电状态
            //     )
            {
                // 电流放大10倍
                // fInvDischrPowerLmt = Set_Power(((float)uiSciSetDataBag[eSetBat1MaxDisChargeCurrValue] * 0.1f * stValue.fAveReal.VBat)) * cKEffyDiv;

                // if(g_ComInfo.Com1.bit.UserTimeEn)  // TOU模式且充电标志勾选
                {
                    if(strBMSLogicInfo.uiBMS1DataAllowUse) // BMS连上的话,以SOC
                    {
                        fInvChrBatPower = Set_Power((stValue.fAveReal.VBat * strBatPowerDerate.fBatDisSocOrder)) * cKEffyDiv;
                        if(fInvDischrPowerLmt > fInvChrBatPower)
                        {
                            fInvDischrPowerLmt = fInvChrBatPower;
                        }
                    }
                    else
                    {
                        fInvChrBatPower = Set_Power((stValue.fAveReal.VBat * strBatPowerDerate.fBatDisVoltOrder)) * cKEffyDiv;
                        if(fInvDischrPowerLmt > fInvChrBatPower)
                        {
                            fInvDischrPowerLmt = fInvChrBatPower;
                        }
        ...
        //------------------------------
        if(fabs(fInvDischrPowerLmt - g_ComInfo.fInvDisChrPowerLmt) <= (Set_Power(10) * 2))
        {
            g_ComInfo.fInvDisChrPowerLmt = fInvDischrPowerLmt;
        }
        else
        {
            if(fInvDischrPowerLmt > g_ComInfo.fInvDisChrPowerLmt)
            {
                g_ComInfo.fInvDisChrPowerLmt += Set_Power(10);//1s/250w
            }
            else
            {
                g_ComInfo.fInvDisChrPowerLmt -= Set_Power(10);
            }
        }
    }
    else
    ...
    if(g_ComInfo.fInvDisChrPowerLmt > cKEffyDiv) { g_ComInfo.fInvDisChrPowerLmt = cKEffyDiv; }
    else if(g_ComInfo.fInvDisChrPowerLmt < 0)    { g_ComInfo.fInvDisChrPowerLmt = 0; }

// stateTest[1] = (int16)(strBatPowerDerate.fBatDisVoltOrder*10);
// stateTest[0] = (int16)fReal_Power(fInvDischrPowerLmt);
// stateTest[3] = (int16)(fReal_Power(g_SystemInfo.PowerBalance.PInvRefTemp));
// stateTest[4] = (int16)fReal_Power(g_ComInfo.fInvDisChrPowerLmt);
               ...
               || (   (eGridOnline == g_SystemInfo.Source.bit.GridSource || eGenOnline == g_SystemInfo.Source.bit.GenSource)   // 旁路输出
                   && (   (eNoBatt == g_ComInfo.Com1.bit.BatMode)
                    //    || (FALSE == g_ComInfo.Com1.bit.LiActiveEn && eBatLose == g_SystemInfo.Source.bit.BatSource)
                       ...
                       || (eActiveByAc == g_ComInfo.Com1.bit.LiActiveSource && !g_ComInfo.Com1.bit.LiActiveFinish)  // AC激活锂电
                       )
                    )
               )
           )
        {
            // BC相主机可以加模块数的判断  现在先不加 如果每个模块都有最低模块数的限制再加  todo @Ivan 250527
//            if((Master == uiSelfMstOrSlv) && (OutSeq_PhaseA != uiFrameParaPhaseSeq))
//            {
//                g_SystemInfo.SysFlag.bit.ParaOutEnable = (!g_ParaVar.ParaFlag.bit.NumNotEnough);
//            }
//            else
            {
           ...
           )
        {
            if(eNoBatt == g_ComInfo.Com1.bit.BatMode)//无电池模式，有市电则On，无市电则Off
            {
                if(   (eGridOnline == g_SystemInfo.Source.bit.GridSource)
                   || (eGenOnline == g_SystemInfo.Source.bit.GenSource)
                   || (ePvOnline == g_SystemInfo.Source.bit.Pv1 || ePvOnline == g_SystemInfo.Source.bit.Pv2) )
                //    || ((ePvOnline == g_SystemInfo.Source.bit.Pv1 || ePvOnline == g_SystemInfo.Source.bit.Pv2) && (FALSE == g_SysAlarm.ubAlarmSys.bit.EPOAlarm)) ) // 参考deye, 开机键按下后才驱动PV和LLC
                {
...
//                	g_SystemInfo.SysFlag.bit.ParaOutEnTemp = g_ParaVar.ParaFlag.bit.FrameOutEnable && (!g_ParaVar.ParaFlag.bit.NumNotEnough);
        ...
        // 并机主机允许开机的条件 自身允许开机,所有模块允许开机,系统上没有模块变少
        if(uiFrameParallelEnable)  // 并机使能且是L1且是主机
        {
    ...
    if(eSysAppRunStatus == g_SystemVar.SysRunFlag.bit.bSystemMainSts)//这里如果离网运行没问题看下把DCDCONOff改成LLCONOff
    {
           ...
           && (eNoBatt != g_ComInfo.Com1.bit.BatMode) )
        {
            ...
            else if(eGridOnline == g_SystemInfo.Source.bit.GridSource || eGenOnline == g_SystemInfo.Source.bit.GenSource)
            {
                g_ComInfo.Com1.bit.LiActiveSource = eActiveByAc; // TODO--20250714--此处因为开启锂电激活，导致电网旁路输出和离网输出都失败，系统一直处于待机态。
            }
            else
            {
        ...
        else if(g_ComInfo.Com1.bit.LiActiveFinish)//激活完成后退出标志
        {
            g_ComInfo.Com1.bit.LiActiveSource = eActiveByNull;
        }
        else if((FALSE == g_ComInfo.Com1.bit.LiActiveEn) || (eNoBatt == g_ComInfo.Com1.bit.BatMode)) // 未使能激活标志 或 非无电池类型
        {
            g_ComInfo.Com1.bit.LiActiveSource = eActiveByNull;
            g_ComInfo.Com1.bit.LiActLLCWork = FALSE;
            g_ComInfo.Com1.bit.LiActiveFinish = FALSE;
            uiBatVoltDownCnt = 0;
            uiLiActiveCnt = 0;
        }
        ...
        else
        {
            ...
            {
                if(g_ComInfo.Com1.bit.LiActiveFinish)//电池激活完成
                {
                    g_ComInfo.Com1.bit.DcDcOnOff = eTurnOff;
                    if((++uiBatVoltDownCnt >= cDelay15s_5ms) || (stValue.fAveReal.VBat < 38.0f)) // 激活完电池电容还有电压，所以先关LLC，待电池电压掉下去后再重新跑状态机开机
                    {
                        uiBatVoltDownCnt = 0;
                        g_ComInfo.Com1.bit.LiActiveFinish = FALSE;
                    }
                }
                else if(eBatNormal == g_SystemInfo.Source.bit.BatSource)
                {
                    g_ComInfo.Com1.bit.DcDcOnOff = g_ComInfo.Com1.bit.SysTurnOn;
                }
                // No.1.2 Bat Lose But Pv or Inv run
                else if(   (stValue.fAveReal.VBus > 360.0f)
        ...
        // 增加电池低压或SOC低关Ac
        //-------------------------------
        // if(g_InvVar.InvLogicJump.bit.bBatLowOffFlag)
        if(g_InvVar.InvLogicJump.bit.bBatLowOffFlag || g_SysAlarm.ubAlarmSys.bit.EPOAlarm) // 开机键不按下INV不运行，可运行PV和LLC
        {
            g_ComInfo.Com1.bit.InvOnOff = eTurnOff;
            g_SystemInfo.SysFlag.bit.InvWorkStart = FALSE;
        }
        // else if(eActiveByNull == g_ComInfo.Com1.bit.LiActiveSource) //电池激活时先不开逆变
        else if(eActiveByPv != g_ComInfo.Com1.bit.LiActiveSource)
        {
            if(eTurnOff == g_ComInfo.Com1.bit.InvOnOff)
            {
                ...
                // No.3.2 交流软起
                        ...
                        && (   (eActiveByAc == g_ComInfo.Com1.bit.LiActiveSource && !g_ComInfo.Com1.bit.LiActiveFinish)  // AC激活锂电
                            ...
                            || (FALSE == g_ComInfo.Com1.bit.LiActiveEn && eBatLose == g_SystemInfo.Source.bit.BatSource)
                           /* || (   FALSE == g_ComInfo.Com1.bit.LiActiveEn
                                //&& (ePvLose == g_SystemInfo.Source.bit.Pv1 && ePvLose == g_SystemInfo.Source.bit.Pv2)
                                && (eNoBatt == g_ComInfo.Com1.bit.BatMode || eBatLose == g_SystemInfo.Source.bit.BatSource) // 20250828--绑定无电池模式和非电池激活的话，会导致此模式下无法旁路输出。
                                )*/
                            )
                        )
                {
            ...
            else
            {
                ...
                if(    (   (stValue.fAveReal.VBus > 360.0f)  // 母线正常
        ...
        // 这里是不是应该加上 (eActiveByPv == g_ComInfo.Com1.bit.LiActiveSource) 的分支，
        // 防止并网过程断电池，判断到电池不存在又进入了激活动作 @Ivan 241101
        //-------------------------------
        // No.4 Pv 开关机信号
        //-------------------------------
            ...
            if(ePvOnline == g_SystemInfo.Source.bit.Pv1 || ePvOnline == g_SystemInfo.Source.bit.Pv2)
            {
                if(   ((eBatNormal == g_SystemInfo.Source.bit.BatSource) && (TRUE == g_DcDcVar.DcDcFlag.bit.LlcSoftStartFinish))    //电池正常且LLC软起成功
                    ...
                    || (   (g_ComInfo.Com1.bit.LiActiveEn)         // 使能电池激活 且 使用PV激活电池 且 激活未完成
                        ...
                        && (!g_ComInfo.Com1.bit.LiActiveFinish) )
                    )
                {
           ...
           )
        {
            ...
            if(stValue.fAveReal.VBat >= 57.0f)//相当于保护，一般是先触发下面的条件，此处关机标志置起后，电池电压还会继续抬升2~3V左右，可能会到60V
            {
                uiLiActiveCnt = 0;
                g_ComInfo.Com1.bit.DcDcOnOff = eTurnOff;
                g_ComInfo.Com1.bit.PvOnOff = eTurnOff;
                g_ComInfo.Com1.bit.LiActLLCWork = FALSE;
                g_ComInfo.Com1.bit.LiActiveFinish = TRUE;
                //g_ComInfo.Com1.bit.LiActiveSource = eActiveByNull;//不在这里清这个标志位，会改变PV BusRef，等PV BOOST关闭标志置起来后，下个状态机周期再清，大约5ms
            }
            // 20250813--电池激活一般只会运行不超过1min，在锂电恢复后电压会在正常范围内，所以市电无旁路输出的情况也是短时间。
            // 20250911--锂电池激活后电压较高(大于45V)，铅酸电池的激活补能后电池不会抬升，所以设定48V有可能导致时间很长。
            else if(stValue.fAveReal.VBat >= 45.0f)//48.0f)
            {
...
static void System_GenConnectCheck(void)
{
                ...
                )
            {
                if(0 == uiSciSetDataBag[eSetGenForceRun] && s_uiGenForceRunBack)
                {
                    ...
                    s_uiSignalExitCnt = 0;
                            ...
                            && (0 != g_ComInfo.unRuleEnable.all)
                            && (g_SystemInfo.SysFlag.bit.EcoGenCharTemp) ) // TOU且充电使能
                        || (0 == uiSciSetDataBag[eSetECO_TimeOfUse])       // 非TOU
                        || (uiSciSetDataBag[eSetGenForceRun]) )            // 强充
                {
                    s_uiSignalExitCnt = 0;
                    if((eLithiumSOC == g_ComInfo.Com1.bit.BatMode) && (strBMSLogicInfo.uiBMS1Connect))
                    {
                            ...
                            || (eLithiumSOC == g_ComInfo.Com1.bit.BatMode) && (0 == uiSciSetDataBag[eSetBMSComErrEnable]) )
                    {
                        ...
                        else if(stValue.fAveReal.VBat >= (g_ComInfo.fBat1ForceChgVoltStop * 0.1f))   // 大于吸收电压断开油机启动信号                     // 大于吸收电压断开油机启动信号
                        {
                            g_SystemInfo.SysFlag.bit.GenStartSignal = 0;
                        }
                    }
                ...
                if(1 == g_SystemInfo.SysFlag.bit.GenStartSignal)
                {
                    ...
                    if(uiSciSetDataBag[eSetGridStartSignal]) // 改为启动Gen_Value
                    {
                        g_SystemInfo.SysFlag.bit.GenStartSignal = 2;
                    }
                }
            }
            else            // todo Ivan 250826
            {
                g_SystemInfo.SysFlag.bit.GenStartSignal = 0;
            }
        }
           ...
           && (1 == uiSciSetDataBag[eSetGenConnectToGridPortEnable]) )
        {
            if(0 == uiSciSetDataBag[eSetGenForceRun] && s_uiGenForceRunBack)
            {
                ...
                s_uiSignalExitCnt = 0;
                        ...
                        && (0 != g_ComInfo.unRuleEnable.all)
                        && (g_SystemInfo.SysFlag.bit.EcoGridCharTemp) )// TOU且充电使能
                    || (0 == uiSciSetDataBag[eSetECO_TimeOfUse])       // 非TOU
                    || (uiSciSetDataBag[eSetGenForceRun]) )            // 强充
            {
                s_uiSignalExitCnt = 0;
    ...
    if(   ((g_ComInfo.Com1.bit.GenConnectEn) && (!g_SysFault.Flag.bit.GenFault))
       || ((g_ComInfo.Com1.bit.GenConnectToGridEn) && (!g_SysFault.Flag.bit.GridFault)) ) // && (uiSciSetDataBag[eSetGenForceRun])
    {
...
static void System_SourceCheck(void)
{
    ...
    if(   (g_SysFault.Flag.bit.GridFault)                                                                   // 市电端口正常接市电
       ...
       || (1 == uiSciSetDataBag[eSetInvForceOffGrid] && g_ComInfo.unSBUModeEn.bit.bSBUEn)                   // 强制离网（SBU），需要满足电网离网条件
       )
    {
        g_SystemInfo.Source.bit.GridSource = eGridOffline;
...
void System_PowerBalanceCalculate(void)
{
    ...
    // 负载功率不能为负数
    if(stPower.fPOutTotal < 0)
    {
        stPower.fPOutTotal = 0;
    }
    ...
    if(eGenPortMode_SmartLoad == g_ComInfo.Com1.bit.GenModeSet)
    {
        ...
        g_SystemInfo.PowerBalance.Load = stPower.fPOutTotal + stPower.fPGenTotal;
    }
    else
    {
        g_SystemInfo.PowerBalance.Load = stPower.fPOutTotal;
    }
    ...
    if((eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode) || (eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode))
    {
        if(DispatchMode != uiSciSetDataBag[eSetWorkMode] && DebugMode != uiSciSetDataBag[eSetWorkMode] && eNoBatt != g_ComInfo.Com1.bit.BatMode)
        {
            if(eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode && (0 == uiSciSetDataBag[eSetGenConnectToGridPortEnable]))
            {
                ...
                {
                    g_SystemInfo.PowerBalance.PInvRefTemp = -Set_Power(100);//-g_SystemInfo.PowerBalance.BatChargeMax;
                ...
                else
                {
                    if(g_ComInfo.Com1.bit.UserTimeEn)  // TOU使能
                    {
                            ...
                            else
                            {
                                ...
                                else if(LoadFirst == uiSciSetDataBag[eSetEnergyPattern])  // 负载能量来源优先级是 PV>Bat>Grid
                                {
                                    if(g_ComInfo.Com1.bit.GridShavingEn)//GPS使能
                                    {
                                        ...
                                        {
                                            g_ComInfo.Com1.bit.BatDischarDis = FALSE;  // 原来是FALSE PV>Bat>Grid
                                        }
                                        else
                                        {
                        ...
                        if(    (eECOModeCharge == g_ComInfo.iEcoBatChgDisChgEn)
                            ...
                            // && (BatFirst == uiSciSetDataBag[eSetEnergyPattern]) )
                            // && (LoadFirst == uiSciSetDataBag[eSetEnergyPattern]) )
                        {
                            g_ComInfo.Com1.bit.TOUBatFirstEn = TRUE;
                        }
                        else
                    ...
                    )
            {
                ...
                g_ComInfo.Com1.bit.TOUBatFirstEn = TRUE;
                // 强充
                if(g_ComInfo.Com1.bit.BatForceChar)  // 强充或者非TOU模式下充电   强充 目标到100 非TOU下充到95
                {
                    if(g_ComInfo.Com1.bit.GridShavingEn)  // GPS使能  电池可以放电
                    {
                        g_SystemInfo.PowerBalance.PInvRefTemp = -g_SystemInfo.PowerBalance.BatChargeMax;
    ...
    else if(QControlMode_PF == uiSciSetDataBag[eSetQControlMode]) //PF=Cos=P/S=P/√(P^2+Q^2) -> PF^2=P^2/(P^2+Q^2) -> Q=P*√((1/PF^2)-1)
    {
        ...
        {
            ftemp1 = (float)((int16)uiSciSetDataBag[eSetPFRef]) * 0.01;
...
// g_SystemInfo.PowerBalance.QInvRefTemp = g_InvVar.fCompenQ; // 添加无功补偿
    // 无功限幅
    ftemp = (float)g_GridManager.Rated.f32POutRateScale * 1.05 * (float)g_GridManager.Rated.f32POutRateScale * 1.05;
    ...
    UpDownLimit(g_SystemInfo.PowerBalance.PInvRef, ((float)g_GridManager.Rated.f32POutRateScale * 1.0),  -((float)g_GridManager.Rated.f32POutRateScale * 1.2));
    UpDownLimit(g_SystemInfo.PowerBalance.QInvRef, ((float)g_GridManager.Rated.f32POutRateScale * 1.05), -((float)g_GridManager.Rated.f32POutRateScale * 1.05));
    // 先写固定
...
void System_ShutdownChk(void)
{
        ...
        && (eIdleState == strI2cE2promBag.unFlag.bit.bE2promMainState) ) // 不需要读写操作
    {
        if((FALSE == g_ComInfo.Com1.bit.KeyOn) || (eSysFaultStatus == g_SystemVar.SysRunFlag.bit.bSystemMainSts))
        {
            ...
            else if(eSysFaultStatus == g_SystemVar.SysRunFlag.bit.bSystemMainSts) // 故障关机，延时5min
            {
                uiShutDownChkTime = MinuteBy100msBase(5);
            }
            else // 关机键关机，延时10S
        ...
        if(++uwShutDownChkCnt >= uiShutDownChkTime) //状态满足持续5S or 5Min以上
        {
            ...
            if(stValue.fAveReal.VBus < 80) { uwShutDownChkCnt2 ++; }
            else { uwShutDownChkCnt2 = 0; }
...
void System_LowPowerModeChk(void)
{
    ...
    if(TRUE == uiSciSetDataBag[eSetLowPowerModeEnable] && !uiSystermOffDecrateBusFlag)
    {
...
static void System_SBUModeProCess(void)
{
    static Uint16 uiSBUCnt = 0, uiSBUInTime = SecondBy20msBase(1);
    static Uint16 uiBattOverCurrCnt1 = 0, uiBattOverCurrCnt2 = 0, uiRecoverCnt = 0;
    float32 fSBUTemp = 0;
    float32 fBatMaxDisCurrTemp = 0, fBatDisCurrTemp = 0;
    ...
    // {
    //     fBatDisCurrTemp = (g_ParaVar.fBatCurrSummyInSys * 0.1f);
    // }
    // else
    {
    ...
    {
        if(strBMSLogicInfo.uiBMS1DataAllowUse) // 通讯上BMS，采用BMS内设置的最大放电电流
        {
            fBatMaxDisCurrTemp = strBatPowerDerate.fBatDisBmsOrder;
        }
        else // 否则采用用户设置的电池最大放电电流
        {
            fBatMaxDisCurrTemp = strBatPowerDerate.fBatDisCmdOrder;
        }
        ...
        {
            fSBUTemp = fReal_Power(((stPower.POut.PhaseA + stPower.SSmartLoad.PhaseA) * cKEffyDiv)) / stValue.fAveReal.VBat;
            ...
            // {
            //     if(    (fSBUTemp < (fBatMaxDisCurrTemp * 1.4f)) // I_BAT < (BAT_DIS_CURR_LIMIT * 1.4)
            //         && ((stPower.POut.PhaseA + stPower.SSmartLoad.PhaseA) < (Set_Power((fBatMaxDisCurrTemp * stValue.fAveReal.VBat)) + (g_SystemInfo.PowerBalance.PV)))
            //         )
            //     {
            ...
            // 有PV时，①PV > Load，I_bat=0；②PV < Load, I_bat = (Load - P_PV) / Vbat
            // 无PV时，I_bat = Load / Vbat;
            // I_bat < (0.9 * 阈值)，持续10Min，判定可以回切。
            if(ePvMainState_Running == strSlvState.ubState1.bit.PvStateMachine)
            {
                if(fSBUTemp < (fBatMaxDisCurrTemp * 1.4f)) // I_BAT < (BAT_DIS_CURR_LIMIT * 1.4)
                {
                    if(g_SystemInfo.PowerBalance.PV < ((stPower.POut.PhaseA + stPower.SSmartLoad.PhaseA) * cKEffyDiv))
                    {
                        fSBUTemp = Set_Power(((stPower.POut.PhaseA + stPower.SSmartLoad.PhaseA) * cKEffyDiv) - g_SystemInfo.PowerBalance.PV) / stValue.fAveReal.VBat;
                    }
                    else
                    {
                        fSBUTemp = 0;
    ...
    // 汇总SBU使能标志
    //---------------------------------------------------
    if(1 == uiSciSetDataBag[eSetInvForceOffGrid] && g_ComInfo.unSBUModeEn.bit.bBattAllowInSBU & g_ComInfo.unSBUModeEn.bit.bLoadAllowInSBU)
    {
...
Uint16 System_MpptMultiPointEnableGet(void)
{
	...
	{	
	    if((OffGridMode == g_InvVar.InvFlag.bit.bInvMainSts) && (eBatNormal == g_SystemInfo.Source.bit.BatSource))
	    {
...
//			   fBatMaxDisChargeCurr = Set_BatCurr((float)uiSciSetDataBag[eSetBat1MaxDisChargeCurrValue] * 0.1); //测试逻辑用
		...
		if(   (OnGridMode == g_InvVar.InvFlag.bit.bInvMainSts)
		   || (OnGenMode == g_InvVar.InvFlag.bit.bInvMainSts)
