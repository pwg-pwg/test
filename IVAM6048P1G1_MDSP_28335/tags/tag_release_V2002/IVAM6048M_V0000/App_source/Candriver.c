...
TestIdRestart = 222;
        ...
        if(uiIDDealStep < IDDeal_DataCopyFinish)
        {
            ...
            if((uiFrameParallelEnable) && (uiFrameParaPhaseSeq == uiSourcePhaseSeqSetTemp) && (1 == uiSourceMasterStateTemp))
            {
                ...
                if(1 == uiSourceFrameNumTemp)
                {
                    ...
                    uiRxFromSysAnalog_GridAvgPa    = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data12;
                    uiRxFromSysAnalog_GridAvgPb    = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data13;
                    uiRxFromSysAnalog_GridAvgPc    = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data14;
                }
                else if(2 == uiSourceFrameNumTemp)
                {
                    uiRxFromSysAnalog_Omega 	   = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data21;
                    uiRxFromSysAnalog_OutRmsAVolt  = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data22;
                    uiRxFromSysAnalog_OutRmsBVolt  = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data23;
                    uiRxFromSysAnalog_OutRmsCVolt  = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data24;
                }
                else if(3 == uiSourceFrameNumTemp)
                {
                    uiRxFromSysAnalog_OutAvgQa	 = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data31;
                    uiRxFromSysAnalog_OutAvgQb	 = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data32;
                    uiRxFromSysAnalog_OutAvgQc	 = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data33;
                    uiRxFromSysState_SysFlg2     = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data34;
                }
                else if(4 == uiSourceFrameNumTemp)
                {
                    uiRxFromSysSetDataID	= stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data41;
                    uiRxFromSysSetDataVaule = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data42;
                    // uiRxFromSysAnalog_BatDischCurr = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data43;
            ...
            if(TRUE == bSelfFrameFlagData_MdlMaster)  // 自身是主机
            {
                ...
                if(FALSE == uiSourceMasterStateTemp)//解析从机数据
                {
                    if(1 == uiSourceFrameNumTemp)
                    {
                        ...
                        uiFrameAnalogData_LoadQaSum(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data12);
                        uiFrameAnalogData_LoadQbSum(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data13);
                        uiFrameAnalogData_LoadQcSum(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data14);
                    }
                    else if(2 == uiSourceFrameNumTemp)
                    {
                        uiFrameAnalogData_CurrRmsA(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data21);
                        uiFrameAnalogData_CurrRmsB(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data22);
                        uiFrameAnalogData_CurrRmsC(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data23);
                        uiFrameAnalogData_DeratPerc(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data24);
                    }
                    else if(3 == uiSourceFrameNumTemp)
                    {
                        uiFrameAnalogData_GridPaSum(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data31);
                        uiFrameAnalogData_GridPbSum(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data32);
                        uiFrameAnalogData_GridPcSum(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data33);
                        uiFrameAnalogData_BatDisCurr(uiSourceFrameIDTemp) = ((int)stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data34);
                    }
                    // 主机的标志要在发送的时候置位  不要放这里
                }
        ...
        else if(uiSourceMasterStateTemp && bSelfFrameFlagData_MdlMaster) // 主机接收其它主机数据帧
        {
            ...
            {
                uiMFrameData_BatDisCurr(uiSourcePhaseSeqSetTemp) = stParaCan_Rx_DataTransmitAll[FRAME_20MS_TYPE].data43;
            }
            // A相接收B/C相主机
...
//                    g_ParaVar.ParaFlag.bit.FrameL2ExitTemp = TRUE;
                    // 可以用下面这个标志位
...
//                    g_ParaVar.ParaFlag.bit.FrameL3ExitTemp = TRUE;
...
//    if(0 != ECanbRegs.CANRMP.all)
    if(0 != ECanbRegs.CANRMP.bit.RMP17)
    {
        ...
        Syn_PLL.u32_Sub_CANCTR_Recei		= ECanbMOTSRegs.MOTS17;

        Syn_PLL.u32_Sub_CANCTR_Deal		= ECanbRegs.CANTSC;			/* 从机相位对齐帧处理时刻邮戳 */
        if(Syn_PLL.u32_Sub_CANCTR_Deal >= Syn_PLL.u32_Sub_CANCTR_Recei)
        {
        ...
        {
            Syn_PLL.u32_Sub_PWM_Deal = (Uint32)EPwm1Regs.TBCTR;
        }
        else
        {
            Syn_PLL.u32_Sub_PWM_Deal = 2 * PWM_HALF_PERIOD - (Uint32)EPwm1Regs.TBCTR;
        }
        ...
        if(Syn_PLL.u32_Sub_PWM_ReceiCMD1 > (2*PWM_HALF_PERIOD))
        {
    ...
    if(    (1 == ECanbRegs.CANRMP.bit.RMP18)\
        && (   (FALSE == bSelfFrameFlagData_MdlMaster)\
            || (OutSeq_PhaseB == uiFrameParaPhaseSeq)\
            || (OutSeq_PhaseC == uiFrameParaPhaseSeq)) )
    {
        ...
        if(strPllOfSys.fFinalRad < 0)
        {
            strPllOfSys.fFinalRad += cRad_2PI;
        }

        //Syn_PLL.f32_Slaver_Master_DTheta = Syn_PLL.f32_Sub_PhaseComp_CAN - Syn_PLL.f32_Sub_Phase_Delta;
...
void SynPWM_ADCISR_Func(void)
{
	...
	{
		Syn_PLL.s32_MainSub_PWM_Delta	= (int32)(Syn_PLL.u16_Sub_SynPWM_CAN - Syn_PLL.u32_Sub_PWM_ReceiCMD1 - 156);	// 0.25Mbit/s补偿4us 2018-2-8 16:39:14
	...
	if((Syn_PWM.f32_PWM_Sin_last <= 0.0) && (Syn_PWM.f32_PWM_Sin_now >= 0.0))
	{
...
void ParaCan_MasterCanTxData(Uint16 uSourceAddr, Uint16 uiMasterState, Uint16 FrameSendTime, Uint16 Length)
{
    ...
    static Uint16 s_uiCanFrameNum = 0;
    Struct_CanFrame *pTxMsgReg;
    pTxMsgReg = &strCanFrameTx;
...
//            pTxMsgReg->unMSGData.Word.uiData1 = uiSysTxAnalogData_GridAvgPa;
//            pTxMsgReg->unMSGData.Word.uiData2 = uiSysTxAnalogData_GridAvgPb;
//            pTxMsgReg->unMSGData.Word.uiData3 = uiSysTxAnalogData_GridAvgPc;
//
//            if(CanBoxHadTxIn(2))
//            {
//                pTxMsgReg->uiLen = 8;
...
//            pTxMsgReg->msg_id.Parabit.FrameNum = s_uiCanFrameNum+1;
//            pTxMsgReg->msg_id.Parabit.FrameEndFlag = (pTxMsgReg->msg_id.Parabit.FrameNum == uiCanSendFrameNum);
//            pTxMsgReg->unMSGData.Word.uiData0 = uiSysTxAnalogData_Omega;
//            pTxMsgReg->unMSGData.Word.uiData1 = uiSysTxAnalogData_OutRmsAVolt;
//            pTxMsgReg->unMSGData.Word.uiData2 = uiSysTxAnalogData_OutRmsBVolt;
//            pTxMsgReg->unMSGData.Word.uiData3 = uiSysTxAnalogData_OutRmsCVolt;
//            if(CanBoxHadTxIn(2))
//            {
//                pTxMsgReg->uiLen = 8;
//                CanDriver_SendBoxData(2,pTxMsgReg); //相应的帧用相应的邮箱发送
...
//            pTxMsgReg->msg_id.Parabit.FrameNum = s_uiCanFrameNum+1;
//            pTxMsgReg->msg_id.Parabit.FrameEndFlag = (pTxMsgReg->msg_id.Parabit.FrameNum == uiCanSendFrameNum);
//            pTxMsgReg->unMSGData.Word.uiData0 = uiSysTxAnalogData_OutAvgQa;
//            pTxMsgReg->unMSGData.Word.uiData1 = uiSysTxAnalogData_OutAvgQb;
//            pTxMsgReg->unMSGData.Word.uiData2 = uiSysTxAnalogData_OutAvgQc;
//            pTxMsgReg->unMSGData.Word.uiData3 = 0;
//
//            if(CanBoxHadTxIn(2))
//            {
//                pTxMsgReg->uiLen = 8;
...
//            pTxMsgReg->msg_id.Parabit.FrameNum = s_uiCanFrameNum+1;
//            pTxMsgReg->msg_id.Parabit.FrameEndFlag = (pTxMsgReg->msg_id.Parabit.FrameNum == uiCanSendFrameNum);
//            pTxMsgReg->unMSGData.Word.uiData0 = uiSysTxSetDataID;
//            pTxMsgReg->unMSGData.Word.uiData1 = uiSysTxSetDataVaule;
//            pTxMsgReg->unMSGData.Word.uiData2 = 0;
//            pTxMsgReg->unMSGData.Word.uiData3 = 0;
//
//            if(CanBoxHadTxIn(2))
//            {
...
//    pTxMsgReg->msg_id.Parabit.Protocol          = ParaCmdID;
    ...
    switch(s_uiCanFrameNum)
    {
        case 0x00:
        {
            pTxMsgReg->msg_id.Parabit.FrameNum = s_uiCanFrameNum+1;
            pTxMsgReg->msg_id.Parabit.FrameEndFlag = (pTxMsgReg->msg_id.Parabit.FrameNum == uiCanSendFrameNum);
            pTxMsgReg->unMSGData.Word.uiData0 = uiMasterFlag_Sys2UpmFlag;    // 0x01;    // 充当系统板下发;
            pTxMsgReg->unMSGData.Word.uiData1 = uiSysTxAnalogData_GridAvgPa; // 0x02;
            pTxMsgReg->unMSGData.Word.uiData2 = uiSysTxAnalogData_GridAvgPb; // 0x03;
            pTxMsgReg->unMSGData.Word.uiData3 = uiSysTxAnalogData_GridAvgPc; // 0x04;
        ...
        case 0x01:
        {
            pTxMsgReg->msg_id.Parabit.FrameNum = s_uiCanFrameNum+1;
            pTxMsgReg->msg_id.Parabit.FrameEndFlag = (pTxMsgReg->msg_id.Parabit.FrameNum == uiCanSendFrameNum);
            pTxMsgReg->unMSGData.Word.uiData0 = uiSysTxAnalogData_Omega;       // 0x1234;
            pTxMsgReg->unMSGData.Word.uiData1 = uiSysTxAnalogData_OutRmsAVolt; // 0x5678;
            pTxMsgReg->unMSGData.Word.uiData2 = uiSysTxAnalogData_OutRmsBVolt; // 0x55AA;
            pTxMsgReg->unMSGData.Word.uiData3 = uiSysTxAnalogData_OutRmsCVolt; // 0xAA00;
            if(CanBoxHadTxIn(s_uiCanFrameNum))
            {
                CanDriver_SendBoxData(s_uiCanFrameNum,pTxMsgReg); //相应的帧用相应的邮箱发送
            }
        ...
        case 0x02:
        {
            pTxMsgReg->msg_id.Parabit.FrameNum = s_uiCanFrameNum+1;
            pTxMsgReg->msg_id.Parabit.FrameEndFlag = (pTxMsgReg->msg_id.Parabit.FrameNum == uiCanSendFrameNum);
            pTxMsgReg->unMSGData.Word.uiData0 = uiSysTxAnalogData_OutAvgQa; // 0x2345;
            pTxMsgReg->unMSGData.Word.uiData1 = uiSysTxAnalogData_OutAvgQb; // 0x6789;
            pTxMsgReg->unMSGData.Word.uiData2 = uiSysTxAnalogData_OutAvgQc; // 0x5500;
            pTxMsgReg->unMSGData.Word.uiData3 = uiMasterFlag_Sys2UpmFlag2;  // 0xAA55;
        ...
        case 0x03:
        {
            pTxMsgReg->msg_id.Parabit.FrameNum = s_uiCanFrameNum+1;
            pTxMsgReg->msg_id.Parabit.FrameEndFlag = (pTxMsgReg->msg_id.Parabit.FrameNum == uiCanSendFrameNum);
            pTxMsgReg->unMSGData.Word.uiData0 = uiSysTxSetDataID;    // 0xABCD;
            pTxMsgReg->unMSGData.Word.uiData1 = uiSysTxSetDataVaule; // 0xBCDE;
            pTxMsgReg->unMSGData.Word.uiData2 = ((int16)g_ParaVar.fBatCurrSummyInPhase); // 该相放电电流总和
            pTxMsgReg->unMSGData.Word.uiData3 = 0;                   // 0x789A;
...
//    pTxMsgReg->msg_id.Parabit.Protocol          = ParaCmdID;
    ...
    switch(s_uiCanFrameNum)
    {
        case 0x00:
        {
            ...
            pTxMsgReg->unMSGData.Word.uiData1 = uiFrameAnalogData_LoadQaSum(uiSelfFrameCanID); // 0x06;
            pTxMsgReg->unMSGData.Word.uiData2 = uiFrameAnalogData_LoadQbSum(uiSelfFrameCanID); // 0x07;
            pTxMsgReg->unMSGData.Word.uiData3 = uiFrameAnalogData_LoadQcSum(uiSelfFrameCanID); // 0x08;
        ...
        case 0x01:
        {
            pTxMsgReg->msg_id.Parabit.FrameNum = s_uiCanFrameNum+1;
            pTxMsgReg->msg_id.Parabit.FrameEndFlag = (pTxMsgReg->msg_id.Parabit.FrameNum == uiCanSendFrameNum);
            pTxMsgReg->unMSGData.Word.uiData0 = uiFrameAnalogData_CurrRmsA(uiSelfFrameCanID); // 0x5678;
            pTxMsgReg->unMSGData.Word.uiData1 = uiFrameAnalogData_CurrRmsB(uiSelfFrameCanID); // 0xABCD;
            pTxMsgReg->unMSGData.Word.uiData2 = uiFrameAnalogData_CurrRmsC(uiSelfFrameCanID); // 0x5500;
            pTxMsgReg->unMSGData.Word.uiData3 = uiFrameAnalogData_DeratPerc(uiSelfFrameCanID);// 0xAA55;
            if(CanBoxHadTxIn(s_uiCanFrameNum))
            {
                CanDriver_SendBoxData(s_uiCanFrameNum,pTxMsgReg); //相应的帧用相应的邮箱发送
                s_uiCanFrameNum ++;
        ...
        case 0x02:
        {
            pTxMsgReg->msg_id.Parabit.FrameNum = s_uiCanFrameNum+1;
            pTxMsgReg->msg_id.Parabit.FrameEndFlag = (pTxMsgReg->msg_id.Parabit.FrameNum == uiCanSendFrameNum);
            pTxMsgReg->unMSGData.Word.uiData0 = uiFrameAnalogData_GridPaSum(uiSelfFrameCanID); // 0xABCD;
            pTxMsgReg->unMSGData.Word.uiData1 = uiFrameAnalogData_GridPbSum(uiSelfFrameCanID); // 0x789A;
            pTxMsgReg->unMSGData.Word.uiData2 = uiFrameAnalogData_GridPcSum(uiSelfFrameCanID); // 0x5050;
            pTxMsgReg->unMSGData.Word.uiData3 = uiFrameAnalogData_BatDisCurr(uiSelfFrameCanID); // 0x0A0A;
