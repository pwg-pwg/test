...
//    hoGridSlaveN_Rly_Off;
    ...
    // 确定继电器过零时间
    // if(   (strPllToGrid.fFinalRad >= cAngleToRad(0)   && strPllToGrid.fFinalRad <= cAngleToRad(4))
    //    || (strPllToGrid.fFinalRad >= cAngleToRad(180) && strPllToGrid.fFinalRad <= cAngleToRad(184)) )
    // if(   (strPllToGrid.fFinalRad >= cAngleToRad(130) && strPllToGrid.fFinalRad <= cAngleToRad(135))
    //    || (strPllToGrid.fFinalRad >= cAngleToRad(310) && strPllToGrid.fFinalRad <= cAngleToRad(315)) )
...
void Relay_InvRlyDrive(void)
{
    ...
    //A相过零点时是88(90-2)~92(94-2)
    //B相过零点时是268(88+180)~272(92+180)
    //硬件去掉倍压电路 闭合时间更新为16ms(-288)     ,16/20*360 = 288度(-288度)
    //-288 + 90 = -198度,-198+360 = 162,160(162-2)~164(166+2)
...
// 1/4分频时，电网64HZ为75.12个点，50HZ为96.154个点，40HZ为120.192个点
    ...
    if(uiFrameParallelEnable)  // 并机的相位要放大点
    {
        // if(   (strPllToInv.fFinalRad >= cAngleToRad(165) && strPllToInv.fFinalRad <= cAngleToRad(195))
        //    || (strPllToInv.fFinalRad >= cAngleToRad(0)   && strPllToInv.fFinalRad <= cAngleToRad(15))
        //    || (strPllToInv.fFinalRad >= cAngleToRad(345)) )
        // if(   (strPllToInv.fFinalRad >= cAngleToRad(114) && strPllToInv.fFinalRad <= cAngleToRad(146))
        //    || (strPllToInv.fFinalRad >= cAngleToRad(294) && strPllToInv.fFinalRad <= cAngleToRad(326))
        //    )
        // if(   (strPllToInv.fFinalRad >= cAngleToRad(124) && strPllToInv.fFinalRad <= cAngleToRad(141))
        //    || (strPllToInv.fFinalRad >= cAngleToRad(304) && strPllToInv.fFinalRad <= cAngleToRad(321))
        //    )
        // if((strPllToInv.fFinalRad >= cAngleToRad(131) && strPllToInv.fFinalRad <= cAngleToRad(148))) // 20250805--只在正半周过零点吸合
        if((strPllToInv.fFinalRad >= cAngleToRad((strPllToInv.fRlyOnPoint1 - 2)) && strPllToInv.fFinalRad <= cAngleToRad((strPllToInv.fRlyOnPoint2 + 2)))) // 20250810--只在正半周过零点吸合
        {
    ...
    {
        // if(   (strPllToInv.fFinalRad >= cAngleToRad(178) && strPllToInv.fFinalRad <= cAngleToRad(182))
        //    || (strPllToInv.fFinalRad >= cAngleToRad(0)   && strPllToInv.fFinalRad <= cAngleToRad(2))
        //    || (strPllToInv.fFinalRad >= cAngleToRad(358)) )
        // if(   (strPllToInv.fFinalRad >= cAngleToRad(129) && strPllToInv.fFinalRad <= cAngleToRad(136))
        //    || (strPllToInv.fFinalRad >= cAngleToRad(309) && strPllToInv.fFinalRad <= cAngleToRad(316))
        //    )
...
//    if(   (strPllToInv.fFinalRad >= cAngleToRad(358) && strPllToInv.fFinalRad <= cAngleToRad(362))
//       || (strPllToInv.fFinalRad >= cAngleToRad(178) && strPllToInv.fFinalRad <= cAngleToRad(182)))
//    {
//        strPllValue.Flag.bit.ParaMstSendDataRad = TRUE;
//    }
//    else
//    {
                ...
                case eRlyChkStep_GridShort:
                {
                    if(   eGridOnline == g_SystemInfo.Source.bit.GridSource
                       && (   (FALSE == uiFrameParallelEnable)
                           || (TRUE == uiFrameParallelEnable) && (unRxSysStaFlag.bit.bSysInvState < eInvMainState_SoftStart) )
                       )
                    {
...
//                            g_RealyVar.RlyCheck.bit.CheckStep = eRlyChkStep_InvShort;
                            if(++uiOutputVoltChkCnt >= 5)//防止市电旁路输出检测异常()
                            {
                                ...
                                fOutputVoltAvg = fOutputVoltSum * 0.2;
...
//                                            g_RealyVar.RlyCheck.bit.CheckStep = eRlyChkStep_Finish;
                        ...
                        else
                        {
                            ...
                            {
//                                g_RealyVar.RlyCheck.bit.InvSoftChkAsk = TRUE;
