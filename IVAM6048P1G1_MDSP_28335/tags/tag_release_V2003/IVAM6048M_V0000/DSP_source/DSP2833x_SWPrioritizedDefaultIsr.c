...
interrupt void EPWM4_TZINT_ISR(void)    // ePWM4 Trip Zone
{
    ...
    // Insert ISR Code here.......
	g_SysFault.ubFaultDc.bit.LlcTz = TRUE; // LLC高压侧硬件过流
...
// uiTzCnt = 222;
	//-------------------------------------------------
	// 20250605--调整为LLC未开启时，不判定电池过流，规避电池接入的大电流导致LLC低压侧过流
	if(g_DcDcVar.DcDcFlag.bit.bLLcSts == eLLCRunState_Idle)
	{
		uiTzCnt = 333;
		EALLOW;
	...
	{
		g_SysFault.ubFaultDc.bit.BatChgTz = TRUE; // LLC低压侧硬件过流--电池过流
...
{
//Originally EPWM1 interrupt is functioned. But the sampling time(A/D) is only set to be time when PWM counter is equal to 0 or TIMER_TBPRD.
//Then THD is not good. So EPWM4 interrupt is used. And the sampling time can be set at any time. The better performance is also got.
...
interrupt void DSP_REALTIME_ISR_PHASE1(void)    // 
{
	// Set interrupt priority:
	volatile Uint16 TempPIEIER = PieCtrlRegs.PIEIER5.all;
	IER |= M_INT5;
...
interrupt void SPIRXINTA_ISR(void)    // SPI-A
{
	...
	///////////////////////////////////////////
	for(i_ArrRec=0;i_ArrRec<4;i_ArrRec++)
	{
		u16_ArrRec[i_ArrRec]=SpiaRegs.SPIRXBUF; //读数据
	}

	if((u16_ArrRec[SPI_Add]>=1)&&(u16_ArrRec[SPI_Add]<SPI_From28062DataSize))//The received address is within range.
	{
		if(u16_ArrRec[SPI_Data]==~u16_ArrRec[SPI_CheckSum])//Checksum OK!
		{
			u16_Temp_Add=u16_ArrRec[SPI_Add];
			u16_Arr_From28062Data[u16_Temp_Add]=u16_ArrRec[SPI_Data];
			u16_Flag_SPI_CheckSumErr=FALSE;
		}
		else//Checksum is wrong.
		{
			if(TRUE==u16_Flag_Not1stTime_ChecksumErr)
			{
				u16_Flag_SPI_CheckSumErr=TRUE;//Here the protocol in the case of received wrong data is ignored for the reason of simplifying code.
			}
			u16_Flag_Not1stTime_ChecksumErr=TRUE;
		}
		u16_Cnt_SPI_Add_Abnormal=0;
		u16_Flag_SPI_AddErr=FALSE;
	}
	else//The com_fault can be confirmed if the received address is out of range  (Continuous 3times).
	{
		if(u16_Cnt_Delay_Enable_SPIFault>=TwentySecBased100ms)//After about 20s when power-on
		{
			 u16_Cnt_SPI_Add_Abnormal++;
			 if(u16_Cnt_SPI_Add_Abnormal>=3)
			 {
				 u16_Flag_SPI_AddErr=TRUE;
				 u16_Cnt_SPI_Add_Abnormal=3;
			 }
			 else
			 {
				 u16_Flag_SPI_AddErr=FALSE;
...
//////////////////////////////////////////////////////////////////
	//This counter is always enabled only if CPU steps into SPI interrupt.
	u16_Cnt_RecData_SPI++;//Avoid any Com_error resulting from wire or hardware.
//////////////////////////////////////////////////
...
interrupt void DSP_REALTIME_ISR_PHASE2(void)    // 
{
	// Set interrupt priority:
	volatile Uint16 TempPIEIER = PieCtrlRegs.PIEIER7.all;
	IER |= M_INT7;
	IER	&= MINT7;	  	                // Set "global" priority
	PieCtrlRegs.PIEIER7.all &= MG78;   // Set "group"  priority
	PieCtrlRegs.PIEACK.all = 0xFFFF;   // Enable PIE interrupts
	EINT;
...
interrupt void SCIRXINTC_ISR(void)    // SCI-C
{
	...
	EINT;
  	...
  	// Insert ISR Code here.......
	//SciaReceivedIntHappenedCnt++;
//	SCIA_COMM_ReceiveInt();
...
interrupt void SCIRXINTB_ISR(void)     // SCI-B
{
	...
	EINT;
  	...
  	// Insert ISR Code here.......
	//ScibReceivedIntHappenedCnt++;
	SCIB_COMM_ReceiveInt();
