...
//         fPinvLoopOutputRef = (-strBusVoltUpLmtRegu.Out - strBusVoltDownLmtRegu.Out);
//         if(strBusVoltUpLmtRegu.Out != 0) stateTest[9] ++;
// // stateTest[0] ++;
...
if(g_uiBatAbnormalFlg != eBat_Normal && !uiInvBatOverProtFlag && uiInvForbidCnt < OneMsByIntBase(5))
{
    // hoTestToggle;
    //if(strOnGridPaRegu.Ref < -0.01f) { strOnGridPaRegu.Ref = strOnGridPaRegu.Ref * 0.75f; }
    if(strOnGridPaRegu.Ref < 0)
    {
        strOnGridPaRegu.Ref = strOnGridPaRegu.Ref * 0.75f;
        g_InvVar.fPInvSoftStart = g_InvVar.fPInvSoftStart * 0.75f;
        fPinvLoopOutputRef = fPinvLoopOutputRef * 0.75f;
        strBusVoltUpLmtRegu.Out = strBusVoltUpLmtRegu.Out * 0.75f;
        strBusVoltUpLmtRegu.integrator = strBusVoltUpLmtRegu.integrator * 0.75f; // 馈
        strBusVoltDownLmtRegu.Out = strBusVoltDownLmtRegu.Out * 0.75f;
        strBusVoltDownLmtRegu.integrator = strBusVoltDownLmtRegu.integrator * 0.75f; // 取
        strPVInvCtrlBatChgPRegu.Out = strPVInvCtrlBatChgPRegu.Out * 0.75f;
        strPVInvCtrlBatChgPRegu.integrator = strPVInvCtrlBatChgPRegu.integrator * 0.75f; // 馈
        strInvCtrlBatDisChgPRegu.Out = strInvCtrlBatDisChgPRegu.Out * 0.75f;
        strInvCtrlBatDisChgPRegu.integrator = strInvCtrlBatDisChgPRegu.integrator * 0.75f; // 取
        g_InvVar.fRefluxLoopOut = g_InvVar.fRefluxLoopOut * 0.75f;
        g_InvVar.fRefluxLoopIng = g_InvVar.fRefluxLoopIng * 0.75f; // 取
        g_InvVar.fGridOutLoopOut = g_InvVar.fGridOutLoopOut * 0.75f;
        g_InvVar.fGridOutLoopIng = g_InvVar.fGridOutLoopIng * 0.75f; // 馈
    }
