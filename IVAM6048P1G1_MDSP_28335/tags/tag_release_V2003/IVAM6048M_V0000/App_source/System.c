                            ...
                            else if(LoadFirst == uiSciSetDataBag[eSetEnergyPattern])
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        // TODO -- 考虑非TOU模式下，是否优先电网限功率，电池可以转为放电。
                                        // if(g_InvVar.fGridInPowerLoopOut < 0)
                                        // {
                                        //     g_ComInfo.Com1.bit.BatDischarDis = FALSE;
                                        // }
                                        // else
...
void System_ShutdownChk(void)
{
    ...
    uiSysPowerDelayOffFlag = (uiSciCommandDataBag[eComATECommand] & 0x0002);
        ...
        && (eIdleState == strI2cE2promBag.unFlag.bit.bE2promMainState) ) // 不需要读写操作
    {
        if((FALSE == g_ComInfo.Com1.bit.KeyOn) || (eSysFaultStatus == g_SystemVar.SysRunFlag.bit.bSystemMainSts))
        {
            ...
            if(uiSysPowerDelayOffFlag) // 机器处于设置页，按下关机键，延时5min关机，否则立即关闭
            {
        ...
        if(++uwShutDownChkCnt >= uiShutDownChkTime) //状态满足持续5S or 5Min以上
        {
            ...
            if(uwShutDownChkCnt >= MinuteBy100msBase(8) || uwShutDownChkCnt2 >= SecondBy100msBase(5) || (FALSE == g_ComInfo.Com1.bit.KeyOn))	//20251114--测试部反馈松开开机键时间过长，所以加入10S后立即关闭BAT-SPS的处理，实测为20S左右，但考虑到客户可能来不及操作，经项目经理确定不作修改。
            {
                hoBATSPSSDOn;
            }
        }
