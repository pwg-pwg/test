...
//	}
    ...
    if(FALSE == g_SysFault.ubFaultAc.bit.LoadOver110)
    {
        ...
        {
            if(   (stPower.fSInv_Conver > fLoadOver2Point)                                           // 逆变 视在功率过载
        ...
        {
            if(   (stPower.fSInv_Conver > fLoadOver3Point)                                           // 逆变 视在功率过载
...
static void Fault_BatAbnormalCheck(void)
{
    ...
    // 25年45周前的机器电池电流采样滤波电容是105，后续是102，通过SN码识别。此开路检测用105会误报，所以此处做区分。
    if((4825 == uiSciSetDataBag[eATE_SN3]) && (4500 >= uiSciSetDataBag[eATE_SN4])) { return; }
    //待机下
    if(eBat_Normal == g_uiBatAbnormalFlg)
    {
            ...
            || (g_SystemInfo.SysFlag.bit.PvWork) )
        {
            if(eDcDcMainState_Running == g_DcDcVar.DcDcFlag.bit.bMainSts)
            {
                ...
                if(stValue.fAveScale.IDcDc < Set_BatCurr(-3))
                {
                    uwPowerCnt2 = 0;
                    if(uwPowerCnt1++ >= SecondByIntBase(2))//持续充电2s
                    {
                        uwPowerCnt1 = SecondByIntBase(2);//38403;
                        uwBatState = eBatState_Charge;
                    }
                }
                else if(stValue.fAveScale.IDcDc > Set_BatCurr(3))
                {
                    uwPowerCnt1 = 0;
                    if(uwPowerCnt2++ >= SecondByIntBase(2))//持续放电2s
                    {
                        uwPowerCnt2 = SecondByIntBase(2);//38403;	
                        uwBatState = eBatState_DisCharge;
                    }
                }
                //2、这里用平均值
                //A、只要有电池电流波动就认为电池正常,可实时更新数据
                if(fabs(stValue.fAveScale.IDcDc) >= Set_BatCurr(3) && g_AdcFlag.bit.DcOneCycleDone == 1) // 触发只在半个周期，所以平均值可能还没更新。
                {
                    s_fBatVoltMax1 = stValue.fAveReal.VBat;
                    s_fBatVoltMin1 = stValue.fAveReal.VBat;
                    s_fBatVoltMax2 = s_fBatVoltMax1;
                ...
                //B、电流小于3A,超时30s也实时更新数据
                if((fabs(stValue.fAveScale.IDcDc) < Set_BatCurr(3)) && (uwCleanCnt1++ >= SecondByIntBase(2)))
                {
                    uwCleanCnt1 = 0;
                    if(uwCleanCnt2++ >= 15)
                    {
                ...
                if((eBatState_Charge == uwBatState) && (fabs(stADC.fRealScale.IDcDc) < Set_BatCurr(3)))
                {//hoTestOn;
...
// hoTestToggle;
// stateTest[1] ++;
// stateTest[9] = stateTest[8];
// stateTest[8] = stateTest[7];
// stateTest[7] = stateTest[6];
// stateTest[6] = (int16)((s_fBatVoltMax1) * 10);
// stateTest[5] = stateTest[4];
// stateTest[4] = stateTest[3];
// stateTest[3] = stateTest[2];
// stateTest[2] = (int16)(mVBatRealCalc(stADC.fRealScale.VBat) * 10);
    ...
    // stateTest[1] ++;
                    ...
                    if(stADC.fRealScale.VBus > Set_BusVolt(f6BusMax))
                    {
                        s_u16BusMaxCnt3 ++;
// {
    // stateTest[2] = (int16)fReal_BusVolt(stADC.fRealScale.VBus);
// }
// stateTest[3] ++;
                    }
                }
                else if((eBatState_DisCharge == uwBatState) && (fabs(stADC.fRealScale.IDcDc) < Set_BatCurr(3)))
                {
                ...
                {
// if(g_uiBatAbnormalFlg == eBat_Normal)
// {
//          if(s_u16WaveMaxCnt1 >= 3) { stateTest[0] = 1; }//stateTest[4] = s_u16WaveMaxCnt1; stateTest[5] = (int16)((s_fBatVoltMax1 + 0.5) * 10); stateTest[6] = (int16)(mVBatRealCalc(stADC.fRealScale.VBat) * 10);}
//     else if(s_u16WaveMaxCnt2 >= 1) { stateTest[0] = 2; }//stateTest[4] = s_u16WaveMaxCnt2; stateTest[5] = (int16)((s_fBatVoltMax2 + 3.0) * 10) ; stateTest[6] = (int16)(mVBatRealCalc(stADC.fRealScale.VBat) * 10);}
//     else if(s_u16BusMaxCnt3 >= 3)  { stateTest[0] = 3; }//stateTest[4] = s_u16BusMaxCnt3; stateTest[5] = (int16)(f6BusMax * 10); stateTest[6] = (int16)(fReal_BusVolt(stADC.fRealScale.VBus) * 10); }
//     else if(s_u16BusMaxCnt4 >= 5)  { stateTest[0] = 4; }//stateTest[4] = s_u16BusMaxCnt4; stateTest[5] = strBMSLogicInfo.uiBMS1BatVolt; stateTest[6] = (int16)(stValue.fAveReal.VBat * 100); }
// }
                    s_u16WaveMaxCnt1 = 3;
                    s_u16WaveMaxCnt2 = 1;
                    s_u16BusMaxCnt3 = 3;//5;
...
// stateTest[6] ++;//hoTestToggle;//hoTestOff;
                }
...
{
    static Uint16 s_uiDisturbanceCnt = 0;
    static Uint16 s_uiDisturbanceDelay = 0;

    // 判断出电池异常了，需要加扰动，以便误检的时候可以退出

    if(eBat_ChgAbnormal == g_uiBatAbnormalFlg)
    {
        if(++s_uiDisturbanceDelay >= SecondBy20msBase(6))
        {
            s_uiDisturbanceDelay = SecondBy20msBase(6);

            if(FALSE == g_SystemInfo.SysFlag.bit.ChgBurstMode)
            {
                if(++s_uiDisturbanceCnt >= cDelay500ms_20ms)
                {
                    s_uiDisturbanceCnt = 0;
                    g_SystemInfo.SysFlag.bit.ChgBurstMode = TRUE;
                }
            }
            else
            {
                ...
                {
                    s_uiDisturbanceCnt = 0;
                    g_SystemInfo.SysFlag.bit.ChgBurstMode = FALSE;
                }
            }
        }
    }
