...
void Pll_LockGridPhase(void)   // 19.2/4
{
    ...
    if(stValue.fRmsScale.VGridA < 0.15)
    {
        ...
        // UpDownLimit(strGridVolt.fFreStepRadFilter, cTmDivFreToStepRad(70), cTmDivFreToStepRad(35));
    ...
    else
    {
        ...
        UpDownLimit(strGridVolt.fFreStepRad, cTmDivFreToStepRad(70), cTmDivFreToStepRad(35)); //(0.045815, 0.02618) // cTmDivFreToStepRad(50) = 0.0327249f
...
//	if(111 == uiSciSetDataBag[eSetRsvd356])  // 用这个
	// {
	// 	fLockGridErr = strGridVolt.fAxisQPos;
	// 	fLockGridInteg += fLockGridErr * 0.00005;// cfPll_Ki;
	// 	UpDownLimit(fLockGridInteg, strPllValue.fTmDivFreToStepRad5Hz, -strPllValue.fTmDivFreToStepRad5Hz);
	// 	strPllToGrid.fPhaseStepRad = fLockGridInteg + fLockGridErr * 0.001;// cfPll_Kp;
	// 	UpDownLimit(strPllToGrid.fPhaseStepRad, strPllValue.fTmDivFreToStepRad5Hz, -strPllValue.fTmDivFreToStepRad5Hz);
	// }
	...
	// if(stValue.fRmsScale.VGridA > 0.15)
	// {
	// 	strPllToGrid.fFinalStepRad = strPllToGrid.fFreStepRad + strPllToGrid.fPhaseStepRad;	
	// }
	// else
	// {
	// 	strPllToGrid.fFinalStepRad = strPllToGrid.fFreStepRad;	
...
void Pll_LockGenPhase(void)   // 19.2/4
{
    ...
    if(stValue.fRmsScale.VGenA < 0.15)
    {
        ...
        // UpDownLimit(strGenVolt.fFreStepRadFilter, cTmDivFreToStepRad(70), cTmDivFreToStepRad(35));
    ...
    else
    {
        ...
        UpDownLimit(strGenVolt.fFreStepRad, cTmDivFreToStepRad(70), cTmDivFreToStepRad(35)); //(0.045815, 0.02618) // cTmDivFreToStepRad(50) = 0.0327249f
...
void Pll_LockPccPhase(void)
{
    ...
    else //if(stValue.fRmsScale.VOutA > 0.15)// if( stValue.fRmsScale.VOutA > 0.15 && stValue.fRmsScale.VOutB > 0.15 )
    {
        ...
        UpDownLimit(strPCCVolt.fFreStepRad, cTmDivFreToStepRad(70), cTmDivFreToStepRad(35)); //(0.045815, 0.02618) // cTmDivFreToStepRad(50) = 0.0327249f
...
//	fDeltaThetaLittle = fBasicTheta * 0.0001;
    ...
    if(FALSE == uiFrameParallelEnable)
    {
        ...
        //        ) // 由于存在锁相时间差，导致过载后切并网时间过长问题，所以改为实时锁电网，频率不为额定频率。SBU模式下改为频率/相位跟踪，幅值不跟踪。
           )
        {
    ...
    else// 并机锁相
    {
            ...
            )
        {
            ...
            // fBasicTheta = strPllToGrid.fFinalStepRad * 0.25; // 0.25是因为锁电网/锁发电机的任务是1/4周期
            fFinalRadTemp = strPllToGrid.fFinalStepRad * 0.25f; // 0.25是因为锁电网/锁发电机的任务是1/4周期
            fDeltaTheta = fBasicTheta * 0.003f;
            fDeltaThetaLittle = fBasicTheta * 0.0001f;
            ...
            if(FALSE == g_ParaVar.ParaFlag.bit.bSysFollowGridPllOKFlag)     // 离网相位预同步
            {
                ...
                {
                    //注：离网转并网，实测这里并未进入，strPllOfSys.fFinalRad的角度这里在跟随市电，而同步方波也在同步修改，两个点在同时修改
                ...
                )
        {
            ...
            // fBasicTheta = strPllToGen.fFinalStepRad * 0.25; // 0.25是因为锁电网/锁发电机的任务是1/4周期
            fFinalRadTemp = strPllToGen.fFinalStepRad * 0.25f; // 0.25是因为锁电网/锁发电机的任务是1/4周期
            fDeltaTheta = fBasicTheta * 0.003f;
            fDeltaThetaLittle = fBasicTheta * 0.0001f;
            ...
            {
                // 变下垂参数
                if(fabs(fActivePower_Droop_Factor - 0.6666f) < (2 * 0.01f))
                {
                    fActivePower_Droop_Factor = Set_Power(4000) * 1;
                }
                else
                {
                    if(fActivePower_Droop_Factor < 0.6666f)
                    {
                        fActivePower_Droop_Factor += 0.01f;
                    }
                    else
                    {
                        fActivePower_Droop_Factor -= 0.01f;
        ...
        {  //一开始电网就故障或者电网正常跳到故障
            ...
            //----------------------------------------------
			// 变下垂参数
            if(fabs(fActivePower_Droop_Factor - 0.3333f) < (2 * 0.02f))
            {
                fActivePower_Droop_Factor = Set_Power(2000) * 1;
            }
            else
            {
                if(fActivePower_Droop_Factor < 0.3333f)
                {
                    fActivePower_Droop_Factor += 0.02f;
                }
                else
                {
                    fActivePower_Droop_Factor -= 0.02f;
            ...
            {
                uiSoftFreStepRadCnt = 0;
                if(fabs(strPllOfSys.fFreStepRad - g_SystemInfo.fSysFreStepRad) <= cFreToStepRad(0.05))
                {
                    strPllOfSys.fFreStepRad = g_SystemInfo.fSysFreStepRad;
                }
                else
                {
                         if(strPllOfSys.fFreStepRad > g_SystemInfo.fSysFreStepRad) { strPllOfSys.fFreStepRad -= cFreToStepRad(0.04); }
                    else if(strPllOfSys.fFreStepRad < g_SystemInfo.fSysFreStepRad) { strPllOfSys.fFreStepRad += cFreToStepRad(0.04); }
                }
            }
            strPllOfSys.fFinalStepRad = strPllOfSys.fFreStepRad;
            //限制频率
        ...
        if((bSelfFrameFlagData_MdlMaster)&&(OutSeq_PhaseA == uiFrameParaPhaseSeq))
        {
            if(TRUE == Syn_PLL.CalcCarrierFlg)
            {
                ...
                if(1 == ECanbShadow.CANTA.bit.TA6)
                {
                    ...
                    /* 相位对齐帧通讯延时补偿 */
                    Syn_PLL.u32_Main_CANCTR_Send			= ECanbMOTSRegs.MOTS6;
                    // if(Syn_PLL.u32_Main_CANCTR_Send >= Syn_PLL.u32_Main_CANCTR_Zero)

                    if(Syn_PLL.u32_Main_CANCTR_Zero >= Syn_PLL.u32_Main_CANCTR_Send)
                    {
                        //Syn_PLL.u32_Main_CANCTR_Delta 	= (Syn_PLL.u32_Main_CANCTR_Send - Syn_PLL.u32_Main_CANCTR_Zero);
                        Syn_PLL.f32_Main_CANCTR_Delta 	= (ECanbRegs.CANTSC - Syn_PLL.u32_Main_CANCTR_Send);
                    }
                    else
                    {
                        //Syn_PLL.u32_Main_CANCTR_Delta 	= (0xFFFFFFFF - Syn_PLL.u32_Main_CANCTR_Zero + Syn_PLL.u32_Main_CANCTR_Send);
                        Syn_PLL.f32_Main_CANCTR_Delta 	= ECanbRegs.CANTSC + (0xFFFFFFFF - Syn_PLL.u32_Main_CANCTR_Send);
                    }
                    ...
                    Syn_PLL.f32_Main_PhaseError_CAN		= Syn_PLL.f32_Main_CANCTR_Delta * 0.000001 * g_StrEcap.Freq.Inv * cRad_2PI; //DSOGIPLLRegs.PLL_BaseFreq;
            ...
            if(TRUE == Syn_PLL.SendCarrierFlg)
            {
                ...
                //    pTxMsgReg->msg_id.Parabit.Protocol          = ParaCmdID;
                    ...
                    pTxSyncMsgReg->unMSGData.Word.uiData0 = 0x0917;  // 0x01;    // 充当系统板下发;
                    pTxSyncMsgReg->unMSGData.Word.uiData1 = 0x0777; // 0x02;
...
//                    pTxSyncMsgReg->unMSGData.Word.uiData3 = (Uint16)((float32)Syn_PLL.u32_Main_TBCTR_Send);
                    ...
                    // ECanbMboxes.MBOX12.MDL.word.HI_WORD  = 0x0918;
                    // ECanbMboxes.MBOX12.MDL.word.LOW_WORD = 0x0777;
                    // ECanbMboxes.MBOX12.MDH.word.HI_WORD  = (Syn_PLL.f32_Main_Phase_Comp * 100000.0f);
                    // ECanbMboxes.MBOX12.MDH.word.LOW_WORD = (Uint16)((float32)Syn_PLL.u32_Main_TBCTR_Send);
                    // ECanbShadow.CANTRS.all = 0;
                    // ECanbShadow.CANTRS.bit.TRS12 = 1;
        ...
        ParaCan_RecieveSyncDeal(&strCanFrameRx);
        //=============================================================
    ...
    //        }
        ...
        // 只用来测试使用
    //		if(111 == uiSciSetDataBag[eSetRsvd354])
    //		{
    //			strPllToSys.fFinalRad += (strPllToSys.fFreStepRad+strPllToSys.fFinalStepRad);
    //		}
    //		else 
    //		{
    //			strPllToSys.fFinalRad += (strPllToSys.fFreStepRad+strPllToSys.fPhaseStepRad);
    //		}
    //		if(strPllToSys.fFinalRad >= cRad_2PI)
    //        {
    //            strPllToSys.fFinalRad -= cRad_2PI;
    //        }
