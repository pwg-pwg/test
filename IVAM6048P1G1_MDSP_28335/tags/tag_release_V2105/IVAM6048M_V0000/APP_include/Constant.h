...
#define cMachineMode        ((Uint16)6048)    // 机器型号
...
enum
{
    eSingleSeries = 0x040A, //单相,电网类型(Grid Typ)不可改;
    eSliptSeries  = 0x04FF  //裂相,电网类型可允许设置单相、三分之二；
};
...
#define cPowerRated             (float)6000.0
...
#define cIBatRated              (float)120.0  // cIBatRated = cPowerRated / cVBatRated = 6000W / 50V = 120A
#define cILlcRated              (float)120.0  // (float)25.0    // 这个频率太高 采不到
#define cIBusRated              (float)120.0  // (float)25.0    // 电感电流 平衡电路电感电流
#define cVNERated               (float)220.0  // 这个不确定
#define cIRelayRated            (float)35.5   // 继电器电流额定电流
// 额定值定标
#define cVBatRatedScaler        (float)(1.0 / cVBatRated) // 这个宏定义和常量耗时一样
...
#define cIInv_Hardware_Limit     (float)55.0f//(float)50.0f   // 逆变硬件过流保护电流
#define cIInv_Rms_Hardware_Limit (float)((float)45.0 / cSqrtTwo)   // 逆变硬件过流保护电流 = 45.0 / 1.414
#define cIInv_Offgrid_Hardware_Limit  (float)46.0f   // 逆变离网硬件过流保护电流
#define cIInv_Offgrid_Hardware_Limit2 (float)41.0f   // 逆变离网硬件过流保护电流
#if(cPvPowerRateScale == 20)
// PV配比  -- 硬件要改配比的话改这里 但是要注意过温降载曲线可能要改
#define cPvPowerMaxRate         (2.0)  // 2.0
#else
#define cPvPowerMaxRate         (1.5)  // 1.5
#endif
...
//---------------------------------------------
// LLC Freq 43.5k
#define cLLCPwmPeriod           1724    //43.5K=75000 / 1724
#define cLLCDutyMax             (Uint16)(cLLCPwmPeriod * 0.45)  //45% = cLLCPwmPeriod * Duty
#define cLLCDuty0001            (Uint16)(cLLCPwmPeriod * 0.001) //1   //0.1%
#define cLLCDuty001             (Uint16)(cLLCPwmPeriod * 0.01 ) //17  //1%
#define cLLCDuty002             (Uint16)(cLLCPwmPeriod * 0.02 ) //34  //2%
#define cLLCDuty003             (Uint16)(cLLCPwmPeriod * 0.03 ) //51  //3%
#define cLLCDuty006             (Uint16)(cLLCPwmPeriod * 0.06 ) //103 //6%
#define cLLCDuty008             (Uint16)(cLLCPwmPeriod * 0.08 ) //137 //8%
#define cLLCDuty010             (Uint16)(cLLCPwmPeriod * 0.10 ) //172 //10%
#define cLLCDuty015             (Uint16)(cLLCPwmPeriod * 0.15 ) //258 //15%
#define cLLCDuty020             (Uint16)(cLLCPwmPeriod * 0.20 ) //344 //20%
#define cLLCDuty030             (Uint16)(cLLCPwmPeriod * 0.30 ) //517 //30%
#define cLLCDuty044             (Uint16)(cLLCPwmPeriod * 0.44 ) //758 //44%
//------ LLC抖频用的范围 ------//
// LLC Freq 45.5kHz
