...
static void Llc_CtrlLoop(void)   // 软起时间太长 移到任务里执行
{
    ...
    // static Uint16 uwBatshortCheckCnt = 0;	

    // //电池保护
    // if(uwBatshortCheckCnt > 7)
    // {
    //     g_SysFault.ubFaultDc.bit.BatShort = TRUE;
    //     uwBatshortCheckCnt = 0;
    // }

    // if(eActiveByNull != g_ComInfo.Com1.bit.LiActiveSource)
    // {
    //     if((stValue.fAveReal.VBat < cVBatShort) && (fabs(stValue.fAveReal.IDcDc) > cIBatShort))//电池短路检测
    //     {
    //         uwBatshortCheckCnt++;
    ...
    //         g_DcDcVar.uwLlcPwmValue = 0;//重新LLC
    //     }
    // }
    ...
    else
    {
        ...
        else
        {
            ...
            else if(LLC_SOFTSTART2 == g_DcDcVar.uwLlcSoftStartStep)
            {
                ...
                {
                    // if(stValue.fBatChargeCurrAveReal > (-cLiActCurrLimitStart) && uwLlcPwmSoftStartCnt2 < 50) // 充电电流小于3A，每1S递增一点
                    // {
                    ...
                    // }
                    //------------------------------
                    if(g_DcDcVar.uwLlcPwmValue < cLLCDuty006) // 充电电流小于3A，每1S递增一点
                    {
                        if(uwLlcPwmSoftStartCnt2 < cDelay200ms_20ms) { uwLlcPwmSoftStartCnt2 ++; }
                        else { g_DcDcVar.uwLlcPwmValue ++; uwLlcPwmSoftStartCnt2 = 0; } //{ g_DcDcVar.uwLlcPwmValue += cLLCDutySoftStep1Lmt; }
                    }
                    else
                    {
                        if(uwLlcPwmSoftStartCnt2 < cDelay100ms_20ms)  { uwLlcPwmSoftStartCnt2 ++; }
                        else
                        {
                            if(g_DcDcVar.fBatMaxChrCurrSet > (-stValue.fAveScale.IDcDc))
                            {
                                g_DcDcVar.uwLlcPwmValue ++;
                            }
                            else if(fabs(g_DcDcVar.fBatMaxChrCurrSet - stValue.fAveScale.IDcDc) > Set_BatCurr(2))
                            {
                                g_DcDcVar.uwLlcPwmValue --;
                            }
                            uwLlcPwmSoftStartCnt2 = 0;
                        }
                        UpDownLimit(g_DcDcVar.uwLlcPwmValue, cLLCDuty044, cLLCDuty003);
                    }
                    //------------------------------
                    // if(g_DcDcVar.uwLlcPwmValue <= 60)
                    // {
                    //     if(++uwLlcPwmSoftStartCnt >= cDelay1s_20ms)//0.1%/1秒,累加1分钟,打到2.23%;短路保护
                    //     {
                    //         uwLlcPwmSoftStartCnt = 0;
                    //         g_DcDcVar.uwLlcPwmValue++;
                    //     }
                    ...
                    //     if(g_DcDcVar.fBatMaxChrCurrSet > (-stValue.fAveScale.IDcDc))
                    //     {
                    //         if(++uwLlcPwmSoftStartCnt >= cDelay100ms_20ms) // 300ms cycle increase 1,about 24s up to 80
                    //         {
                    //             uwLlcPwmSoftStartCnt = 0;
                    //             g_DcDcVar.uwLlcPwmValue++;
                    //         }
                ...
                if(uwLlcSoftOverTimeCnt >= cDelay2s_20ms)
                {
                    if(FALSE == g_DcDcVar.DcDcFlag.bit.LlcSoftStartFinish)
                    {
                        if(   ((fabs((stValue.fAveReal.VBat * cTransformerRatio) - stValue.fAveReal.VBus)) < 60.0)   // Bus与Bat的9倍偏差 < 60V
                           && (stValue.fAveReal.VBus >= cDcdcBusStartMin) )  // 35V的9倍是315V，低5V是采样误差，低于35V判定电池开路
                        {
