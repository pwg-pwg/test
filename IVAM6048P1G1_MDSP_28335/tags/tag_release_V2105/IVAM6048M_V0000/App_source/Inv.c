...
void Inv_Initialize(void)
{
    ...
    strLoadVoltARmsRegu.integTopLimit = 0.25;  // 额定值的+-25%
...
*/
    ...
    // 并机电流均流环 Pi 参数
    g_ParaVar.fDroopInvCurrAveKp = 0.025;
    g_ParaVar.fDroopInvCurrAveKi = 0.000;
    // 并机无功均流环 A
...
void Inv_OnGridBusVoltLoop(void)
{
    ...
    if(eInv_OnGridMode == g_InvVar.InvFlag.bit.InvWorkMode || eInv_OnGenMode == g_InvVar.InvFlag.bit.InvWorkMode)
    {
        ...
        if((eBat_Normal != g_uiBatAbnormalFlg) || (eNoBatt == g_ComInfo.Com1.bit.BatMode) || (eActiveByAc == g_ComInfo.Com1.bit.LiActiveSource))
        {
			...
			if(eNoBatt == g_ComInfo.Com1.bit.BatMode)
			{
            	strBusVoltDownLmtRegu.Kp = 1;//20 -- 20260305 在无电池模式下，PV+GRID馈电时，会因为这个KP过大导致振荡
            	strBusVoltDownLmtRegu.Ki = 0.005;
			}
        }
        else
...
//            g_InvVar.fCtrlLoopOutA = strOnGridCurrARegu.Out + stADC.fRealScale.VGridA;
        ...
        {
            g_InvVar.fCtrlLoopOutA = strOnGridCurrARegu.Out + (stADC.fRealScale.VGenA * fInvVoltFeedForwardKp);//(strGenVolt.fPosAlpha * fInvVoltFeedForwardKp);//
...
void Inv_OffGridRmsLoop(void)
{
    ...
    if(eInv_OffGridMode == g_InvVar.InvFlag.bit.InvWorkMode)
    {
        if(++uiRmsVoltLoopCnt >= cDelay20ms_1ms)// 20ms一次
        {
            ...
            UpDownLimit(strLoadVoltARmsRegu.Err, 0.2, -0.2); // about 44V
            // UpDownLimit(strLoadVoltARmsRegu.Err, 0.1f, -0.1f); // about 22V
...
void Inv_OffGridRealVoltLoop(void)
{
//    float32 fImage_Res = 0;
//    float32 wRRepetCtrlErr = 0;
//    Uint16  wSintemp = 0;
...
//    strStaticRealVoltOut.phaseA = strStaticRealVoltErrFilter.phaseA * strLoadVoltARealRegu.Kp * strPllToInv.fCosA;
    ...
    UpDownLimit(strLoadCurrARealRegu.Ref, strLoadVoltARealRegu.integTopLimit, strLoadVoltARealRegu.integDownLimit); // 不加入限幅会导致L载过载时，因为负载前馈导致硬件过流封波
...
//	else if(222 == uiSciSetDataBag[eSetRsvd348])  // 现在先用这两个环路已足够
    {
        ...
        {
            strLoadVoltRmsRealRef.fphaseA = strLoadVoltARmsRegu.Ref + g_ParaVar.fDroopDeltaVolt + fRxFromSysAnalog_OutRmsAVolt;// + g_ParaVar.fDroopQaAveOut;
        }
// stateTest[0] = (int16)(fReal_Power(strLoadVoltARmsRegu.Ref));
// stateTest[1] = (int16)(fReal_Power(g_ParaVar.fDroopDeltaVolt));
// stateTest[2] = (int16)(fReal_Power(fRxFromSysAnalog_OutRmsAVolt));
...
void Inv_ParaInvCurrAvgLoop(void)
{
    ...
    if(g_ParaVar.ParaFlag.bit.RxSysCtrlDataRenew)
    {
        ...
        {
            // Phase A
            fIaAveReguErr = fRxFromSysAnalog_OutAvgQa - stValue.fRmsScale.IInvA;//g_fInvCurrFilt;//stBasicPower.InvQa.fBasicWaveScale;
            // UpDownLimit(fIaAveReguErr, 0.25f, -0.25f);  // Set_Power(1000)
...
//	strPNBUSRmsReguVoltOnGrid.integDownLimit = -strPNBUSRmsReguVoltOnGrid.integTopLimit;
    ...
    // fReactivePower_Droop_Factor = Set_Power(uiSciSetDataBag[eSetRsvd347]) * 20 * g_GridManager.Rated.f32VOutInvt; // 调小了空载和大负载时不均流加剧
}
...
static void Inv_GridInputPowerLimit(void)
{
        ...
        )
    {
        ...
        UpDownLimit(g_InvVar.fGridInPowerLoopIng, 0.2f, -2);
...
static void Inv_PvMaxPowerLimit(void)
{
    ...
    //              }
                ...
                {
                    UpDownLimit(strPVBatChgPowerLimitLoop.Out, strPVBatChgPowerLimitLoop.integTopLimit, 0);
                }
...
static void Inv_DisChgCurrLimitByBatVolt(void)
{
    // 第二、三版的BUS电容减小300uf，电池电流纹波加大，51V以下会触发电池电流硬件过流保护320A，所以改为51V以下都削顶处理。--add in 20250829
    // 输出电压有220V、230V、240V，额定电流也会变化，此处做到直流侧恒功率，非恒流，IVAM6K测试各工况（不同电池电压和输出电压等级）都正常。--add in 20251211
    // if(g_SystemInfo.fBatVoltAvgReal >= 52)      { fLoopLimitDerateScaleSet = 1.135f; }
    // else if(g_SystemInfo.fBatVoltAvgReal <= 51) { fLoopLimitDerateScaleSet = 1.0f;   }
    if(g_SystemInfo.fBatVoltAvgReal >= 52)      { fLoopLimitDerateScaleSet = (46 * g_GridManager.Rated.f32IOutInvt); }
    else if(g_SystemInfo.fBatVoltAvgReal <= 51) { fLoopLimitDerateScaleSet = (41 * g_GridManager.Rated.f32IOutInvt); }
    // if(g_SystemInfo.fBatVoltAvgReal >= 52)      { fLoopLimitDerateScaleSet = 1.816f; }
    // else if(g_SystemInfo.fBatVoltAvgReal <= 51) { fLoopLimitDerateScaleSet = 1.6f;   }
    // if(g_SystemInfo.fBatVoltAvgReal >= 52)      { fLoopLimitDerateScaleSet = 1.7025f; } // 1.816f;
    // else if(g_SystemInfo.fBatVoltAvgReal <= 51) { fLoopLimitDerateScaleSet = 1.5f;   } // 1.6f
