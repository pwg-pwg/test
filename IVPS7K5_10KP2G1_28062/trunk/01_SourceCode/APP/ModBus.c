...
INT16U swModBusRdProc(INT16U *RxBuff, INT16U RxLen)
{
	...
	if(RdNums>64)//addin20241015限定读取长度,防止查询长度超过64个导致中断任务异常,实测查询实时数据大于80个,输出波形畸变。
	{
		swBuildFaultFrame(RxBuff,cErr_AskLengthEr);
		return 0;
	}
...
void sRealTimeDataUpdate(void)
{
	...
	uRealTimeData.Stru.wRInvWatt = sdwGetInvWatt();	// 市电电压*逆变电流
	...
	uRealTimeData.Stru.wROpWatt = sdwGetOPWatt();	// 市电电压*输出电流
	uRealTimeData.Stru.wROpVA = sdwGetOPVA();		// 市电电压*输出电流
	uRealTimeData.Stru.wRLineVolt = swGetRLineVolt();
	uRealTimeData.Stru.wRLineCurr = swGetOPShareCurr();	// 代替
	...
	uRealTimeData.Stru.wOPShareCurr = 0;
