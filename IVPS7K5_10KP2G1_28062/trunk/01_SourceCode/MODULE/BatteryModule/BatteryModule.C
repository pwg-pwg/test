...
*Battery module output	definition
...
/*******************************************************************************
*Battery module parameters	definition
...
/*******************************************************************************
Battery module function definition
These functions can be called by task
*******************************************************************************/
void 	sBatModuleInit(void);
void	sBatAvgVoltCalA(INT16U wBatAvgVoltOrg);
void 	sBatVoltLowChk(INT8U bFilter,INT8U bBackCheckEnable);
void 	sBatVoltUnderChk(INT8U bFilter);
void 	sBatVoltOverChk(INT8U bFilter);
...
void 	sBatVoltOverChk(INT8U bFilter)
{
	...
	{
		if(sbOverLevelChk(wBatAvgVoltNew, wBatOverChargeVolt, bFilter, &bBatOverChkCnt)==true)
		{
			fBat.OverChargedA= 1;
		}
	}
...
void 	sBatVoltOverFloatChk(INT8U bFilter)
{
	static INT8U bBatOverChkCnt=0;
	...
	{
		if((g_bDischgVoltSet >= cBat46v) && (g_bDischgVoltSet <= cBat58v))
		{
			wBatteryFloatVolt = g_bDischgVoltSet;
	...
	{
		if((g_bDischgVoltSet >= cBat23v) && (g_bDischgVoltSet <= cBat29v))
		{
			wBatteryFloatVolt = g_bDischgVoltSet;
	...
	{
		if((g_bDischgVoltSet >= cBat11v5) && (g_bDischgVoltSet <= cBat14v5))
		{
			wBatteryFloatVolt = g_bDischgVoltSet;
...
void 	sBatVoltLowChk(INT8U bFilter,INT8U bBackCheckEnable)
{
	static INT8U	bBatVoltLowChkCnt= 0;
	
	if(bPVMode != cBatteryMode && bPVMode != cFaultMode)
	{
		fBat.BatVoltLow= 0;
		bBatVoltLowChkCnt= 0;
	}
	else if(fBat.BatVoltLow == 0)
	{
...
void sBatVoltUnderChk(INT8U bFilter)
{
	static INT8U	bBatVoltUnderChkCnt = 0;
	static INT8U	bBatStartVoltUnderChkCnt = 0;
	INT16U	uwBatStartVolt;
	
	if(bPVMode == cStandbyMode  || bPVMode == cPowerOnMode)   // cold start voltage check
	{
		uwBatStartVolt = swGetBatteryPcs() * cBat11v5;
		//uwBatStartVolt = swGetBatteryPcs() * cBat11v;
		/*
		if(sbGetEepromBatteryType() == cLiBType)
		{
			uwBatStartVolt = swGetEEBattStartVolt();
		}
		else
		{
			uwBatStartVolt = swGetBatteryPcs() * cBat11v5;
		...
		if(!g_uwBatStartFail)
		{
			if(sbUnderLevelChk(wBatAvgVoltNew, uwBatStartVolt , bFilter, &bBatStartVoltUnderChkCnt))
			{
		...
		{
			if(sbOverLevelChk(wBatAvgVoltNew, uwBatStartVolt + cBat0v2, bFilter, &bBatStartVoltUnderChkCnt))
			{
	...
	else if(fBat.BatVoltUnder == 0 && bPVMode == cBatteryMode)
	{
