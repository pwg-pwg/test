...
void  swGetLineQuadraticSum(INT16S wRLineVoltRealTemp )
{
	...
	if(++wSinPointerCnt >= 2)	//
	{
		...
		gi_wLineVoltRealQuadratBuf[wQueueCnt] =(gi_wRLineVoltRealQuadTemp>>8);
		//gi_wRLineVoltRealQuadratic =gi_wRLineVoltRealQuadTemp + gi_wLineVoltRealQuadratBuf[wQueueCnt];
		if(wQueueCnt>=47)
		{
			gi_wRLineVoltRealQuadratic =gi_wLineVoltRealQuadratBuf[wQueueCnt] +gi_wLineVoltRealQuadratBuf[0] ;
			wQueueCnt=0;
		}
		else
		{
			gi_wRLineVoltRealQuadratic =gi_wLineVoltRealQuadratBuf[wQueueCnt] + gi_wLineVoltRealQuadratBuf[wQueueCnt+1];
			wQueueCnt++;
		}
