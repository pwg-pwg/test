	...
	else
	{
		...
		***************************************************************************
		...
		//	if(wLHB != 0)///attention
			...
			//CT饱和保护 //
			//if((wR_Vcmp_P < cPWMDutyLow)&&(wR_Is_P_D > cIS_LMT))//3000 && 118 
			{
			//	wLHB = 3;
			}
			//if(wLHB != 0)
			{
			//	wLHB--;
			//	wR_Vcmp_P_1 = wR_Vcmp_P;  
			//	wR_Vcmp_P = cCTRecoverDuty; //2000
			//	wR_Vcmp_N = 0;
			//	P_PFCCMP = PFCPRD - (INT16S)((((INT32S)wR_Vcmp_P)*13)>>7);
			//	N_PFCCMP = 0xFFFF;
			//	mDisPFCPwmOut_N();
			//	return;
			}
			
//			if(ubGridLineZeroFlag == 1)
//			{
//				ubGridLineZeroFlag =0;
//				wR_Vcmp_P = wR_PrevVcmp_P;   
//				//N_PFCCMP = 0xFFFF;       
//				//mDisPFCPwmOut_N();  
//				EPwm2Regs.AQCSFRC.bit.CSFA = 1;
//				EPwm1Regs.AQCSFRC.bit.CSFA = 1;
