...
//		else 
	...
	if(uwBMSCmdAddrIDTemp != g_uwBMSCmdAddrID)
	{
		do
		{
			if(g_uwBMSCmdAddrID > cBMSAddrIDMax) // 进入下一循环
			{
				g_uwBMSCmdAddrID = 0;
				//EMSCNT++;
				...
				{
					uwTemp = 0;
					if(g_uwBMSPollingNum++ > 3) { fBMSConnectFlg = 0; g_uwBMSPollingNum = 4; } // 轮询3次全部BMS ID后，判定为通讯异常。
				}
			...
			if(!g_strBMSComSts[g_uwBMSCmdAddrID].fBMSComReady) // 通讯已成功 或 未通讯成功的，未准备发送的
			{
				if(s_uwCycleNoOkChkEn)
				{
		...
		}while(g_uwBMSCmdAddrID > cBMSAddrIDMax || !g_strBMSComSts[g_uwBMSCmdAddrID].fBMSComReady); // 通讯已成功 或 未通讯成功
	}
}
