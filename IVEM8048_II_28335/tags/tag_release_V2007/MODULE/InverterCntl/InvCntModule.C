...
//	g_wVref = (INT16S)(((INT32S)g_wRSinAmp * pSinTab[g_wCtrlSinpointer] >> 14)) + g_wRInvDCVoltCntl;//+ g_wRInvDCSet
	...
	g_wRRepetCtrlValue = _IQsat(g_wRRepetCtrlValue, g_wRepetCtrlUp, g_wRepetCtrUnder);
	...
	g_dwVBeforeSaturation = _IQsat(g_dwVBeforeSaturation, g_wInvVoltCurrLimit, -g_wInvVoltCurrLimit); //43<<5
...
//	INT32S	dwBatDishgLimit;
	...
	if(++g_wLoopCnt >= 8)
	{
		...
		g_dwBus_I = _IQsat(g_dwBus_I, (dwDischgLimit << 8), -(dwChgLimit << 8));  // 1600 << 8
		...
		g_dwPDCDCImo = _IQsat(g_dwPDCDCImo, cDCDCCntlCurrntLimit, -cDCDCCntlCurrntLimit); // [45A, -45A]
	}
...
void sBuckSoftProc(void)
{
	...
	if(swGetConvertVoltReal() >= wTemp)//BUS电压大于电池电压7倍+10V时，封BUCK/BOOST驱动，待BUS电压下降
	{
		mDisPDCDCPWMOut();
		g_wPDCDCPWM = 1;
	}
...
//----------------------------------------------------------//
	...
	// g_wFBImo_P = _IQsat(g_wFBImo_P, 5000, -5000); // [-50A, 50A]
	g_wFBImo_P = _IQsat(wTemp, 6500, -6500); // [-65A, 65A]
	...
	g_wFBR_Ierr_P = _IQsat(g_wFBR_Ierr_P, 200, -200); // [-2A, 2A]
...
// if(mInvCurrReal(gi_wRInvCurrSample) > 0) { g_wFBPWM = g_wFBPWM - 50; }//g_wFBKcd_Y = 50时，过零处有改善
