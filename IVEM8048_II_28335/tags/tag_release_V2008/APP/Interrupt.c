				  ...
				  INT16S *pwGeneratorWaveChkCnt, INT16S *pwGeneratorVoltMax, INT16S *pwGeneratorVoltMin)
{
    ...
    static INT16U  wLineVoltHighChkCnt = 0;
	static INT16U  wLineQuarterVoltChkCnt = 0; 
...
//		wTemp = (INT16S)(((INT32S)gi_uwLinePeriodNew * 3) >> 9);	//52*3.3 = 172 = 3>>9
		...
		{
			if(++wLineVoltHighChkCnt >= 23)       //23*52us=1.2ms
			{
				wLineVoltHighChkCnt = 0;
				*pwLineWaveLossFlag = true;
				*pwLineWaveOKCnt = 0;
			}
		}
		...
		if(sQuarterFastCheck(wLinvVolt,g_uwRLineRms3timeAvg,&g_strLine))
		{
		    if(gi_uwWaitLineVoltSteady)
		    {
				if(++wLineQuarterVoltChkCnt >= 38)       //38*52us=2ms
				{
					wLineQuarterVoltChkCnt = 0;
					*pwLineWaveLossFlag = true;
					*pwLineWaveOKCnt = 0;
				}
		    }
...
{
	INT16S wTemp;
    static INT16U  wGenVoltHighChkCnt = 0;
    static INT16U  wGenVoltSteadyCnt = 0;
	static INT16U  wGenQuarterVoltChkCnt = 0; 
...
//		wTemp = (INT16S)(((INT32S)gi_uwLinePeriodNew * 3) >> 9);	//52*3.3 = 172 = 3>>9
	...
	if(abs(wLinvVolt) > 3980)  //g_uwRLineVolt	 
	{
		if(++wGenVoltHighChkCnt >= 23) 	  //19*52us=1ms
		{
			wGenVoltHighChkCnt = 0;
			*pwLineWaveLossFlag = true;
			*pwLineWaveOKCnt = 0;
		}
	}
	...
	if(g_uwWorkMode == cLineMode || g_uwWorkMode == cBypassMode)
	{
		...
		{
			if(++wGenVoltSteadyCnt >= gi_wSinePointMaxTwo)
			{
				wGenVoltSteadyCnt = 0;
	...
	if(sQuarterFastCheck(wLinvVolt, g_uwRGenRms3timeAvg,&g_strGen)) 
	{
		if(gi_uwWaitGenVoltSteady)
		{
			if(++wGenQuarterVoltChkCnt >= 38)       //38*52us=2ms
			{
				wGenQuarterVoltChkCnt = 0;
				*pwLineWaveLossFlag = true;
				*pwLineWaveOKCnt = 0;
			}
		}
...
INT16S swRSmartOPCurrCal(void)
{
	...
	if(gi_uwRSmartOPCurrSample2SumCnt > 0)
	{
		...
		if(wRSmartOPCurr < 0) { wRSmartOPCurr = 0; }
	}
