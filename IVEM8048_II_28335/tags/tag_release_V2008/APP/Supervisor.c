...
//	hoOPRLYOff;
	...
	if(gi_uwOPRelayOn)
	{
		   ...
		   || (swGetEESmartPortType() == cGenPort && (gi_uwSmartPortRelayOn || gi_uwSmartPortNRelayOn)) ) // 发电机已接入
		{
			...
			{
				sOSTimerStop();
				sRlyDelayProc(RLY_DELAY, cDelay10ms, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
				sRlyDelayProc(RLY_DELAY, cDelay10ms, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
				sRlyDelayProc(RLY_DELAY, cDelay10ms, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
				sOSTimerStart();
			}
				...
				if(g_uwPBusAvgVoltNew >= uwBusSoftPoint)
				{
					if(--uwBusVoltOKDelay == 0)
					{
						...
						if(gi_wParallelEnable)
						{
						    ...
						    {
							    sRlyDelayProc(RLY_CROSSZERO, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwGridRelayOn, true);
						    }
							else
							{
							    sRlyDelayProc(RLY_CROSSZERO, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwSmartPortRelayOn, true);
							}
						}
						else
						{
							if(g_LineModeJoinInWay == cLineModeJoinInByGrid)
						    {
							    sRlyDelayProc(RLY_CROSSZERO, 0, eSuperSelectPoint, cPrioSuper, &gi_uwGridRelayOn, 2);  // 闭合市电SCR
							    sRlyDelayProc(RLY_DELAY, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwGridRelayOn, true);	//延时约206uS，L载由于电流滞后90°，可能SCR未闭合。
						    }
						    else
						    {
							    sRlyDelayProc(RLY_CROSSZERO, 0, eSuperSelectPoint, cPrioSuper, &gi_uwSmartPortRelayOn, 2);  //闭合发电机SCR
							    sRlyDelayProc(RLY_DELAY, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwSmartPortRelayOn, true);
						    }
						}
...
INT16S swInvVoltSoft(void)
{
	...
	sOSTimerStop();
	sRlyDelayProc(RLY_CROSSZERO, cOPRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
	sOSTimerStart();
			...
			if(g_uwPBusAvgVoltNew >= uwBusSoftPoint)
			{
				if(--uwBusSoftOKDelay == 0)
				{
					...
					if(gi_wParallelEnable)
					{
						if(g_LineModeJoinInWay == cLineModeJoinInByGrid)
					    {
						    sRlyDelayProc(RLY_CROSSZERO, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwGridRelayOn, true);      // 接入市电
					    }
						else
						{
						    sRlyDelayProc(RLY_CROSSZERO, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwSmartPortRelayOn, true); // 接入发电机
						}
					}
					else
					{
					    ...
					    { 
					        sRlyDelayProc(RLY_CROSSZERO, 0, eSuperSelectPoint, cPrioSuper, &gi_uwSmartPortRelayOn, 2);   // 闭合市电SCR
						    sRlyDelayProc(RLY_DELAY, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwSmartPortRelayOn, true); // 延时约206uS，L载由于电流滞后90°，可能SCR未闭合。  
					    }
					}
					sOSTimerStart();
					return(cBypassMode);
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperOPRlyOff))
		{
			if(!gi_wParallelEnable)
			{
			    ...
			    if(swGetEESmartPortType() == cOutPort)    {gi_uwOP_TWINS_RelayOn = false;gi_uwOP_TWINS_N_RelayOn = false;}  //gi_uwOP_TWINS_RelayOn = false;
			}
			else
			{
				g_uwWorkMode = cStandbyMode;
		...
		if(g_uwSuperEvent & (1 << eSuperOPRlyOn))
		{
			   ...
			   )
			{
				...
				{
					sOSTimerStop();
					sRlyDelayProc(RLY_CROSSZERO, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwOPRelayOn, true);
					sOSTimerStart();
				}
				else
				{
					sOSTimerStop();
					sRlyDelayProc(RLY_CROSSZERO, cOPRlyDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwOPRelayOn, true);
					sOSTimerStart();					
				}
//				gi_uwOPRelayOn = true;//hoOPRLYOn;	
		...
		if(g_uwSuperEvent & (1 << eSuperToBat))
		{
                ...
                && mChkDCDCLLCOn() )
			{
				...
				{
					sOSTimerStop();
					sRlyDelayProc(RLY_ZEROPOINT, 0, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
					sRlyDelayProc(RLY_DELAY, cDelay2ms,eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
					sRlyDelayProc(RLY_ZEROPOINT, 0, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
...
//					sRlyDelayProc(RLY_ZEROPOINT, 0, eSuperSelectPoint, cPrioSuper);  // 尝试屏蔽这两行，可以缩短切换时间10mS左右，并尽量伏秒平衡。
				...
				{
					sOSTimerStop();
					sRlyDelayProc(RLY_DELAY, cDelay5ms, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
					sOSTimerStart();
				}
				else
				{
					sOSTimerStop();
					sRlyDelayProc(RLY_DELAY, cDelay8ms, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
					sOSTimerStart();
				}
...
//						sRlyDelayProc(RLY_CROSSZERO, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper);
						sRlyDelayProc(RLY_CROSSZERO, cInvOffToSCROnTime, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
						g_uwPVBoostWork = false;
						sSetFBInvCtrlSts(cFBWait);
						sSetDCDCCtrlSts(cDCDCWait);
//						mDCDCLLCOff();
...
//						sRlyDelayProc(RLY_ZEROPOINT, 0, eSuperSelectPoint, cPrioSuper);
						sRlyDelayProc(RLY_CROSSZERO, 0, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
						g_uwPVBoostWork = false;
						sSetFBInvCtrlSts(cFBWait);
						sSetDCDCCtrlSts(cDCDCWait);
//						mDCDCLLCOff();
						gi_uwGridRelayOn = 2;
						sRlyDelayProc(RLY_DELAY, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwGridRelayOn, true); // 延时约206uS，L载由电流滞后90°，可能SCR未闭合。
					}
//					gi_uwGridRelayOn = true;
					if(!g_wSolarSigPowerLoad)
					{
...
void sSmartOpControl(void)
{
    if(sGetSmartOutputRlyOn() == true)
    {
        ...
        {
            //断开顺序：先断开L继电器，再断开N继电器
            sOSTimerStop();
			sRlyDelayProc(RLY_CROSSZERO, 17, eSuperSelectPoint, cPrioSuper, &gi_uwOP_TWINS_RelayOn, false);
//		    gi_uwOP_TWINS_RelayOn = false;
			sRlyDelayProc(RLY_DELAY, cDelay10ms, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
			sRlyDelayProc(RLY_CROSSZERO, 17, eSuperSelectPoint, cPrioSuper, &gi_uwOP_TWINS_N_RelayOn, false);
            sOSTimerStart();
//			gi_uwOP_TWINS_N_RelayOn = false;
        }
    }
    else  if(swGetEESmartPortType() == cOutPort && !gi_wParallelEnable && !gi_uwOP_TWINS_RelayOn)
	{
	   ...
	   {
	       //闭合顺序：先闭合N继电器，再闭合L继电器
	       if(sChkSmartOpSts() == true)
	       {
			    sOSTimerStop();
				gi_uwOP_TWINS_N_RelayOn = true;
			    sRlyDelayProc(RLY_DELAY, cDelay10ms, eSuperSelectPoint, cPrioSuper, NULL, RLY_ACTION_NULL);
				sRlyDelayProc(RLY_CROSSZERO, 0, eSuperSelectPoint, cPrioSuper, &gi_uwOP_TWINS_RelayOn,2);
//				gi_uwOP_TWINS_RelayOn = 2; // 闭合SCR
				sRlyDelayProc(RLY_DELAY, cSFTRlySCRDelayTime, eSuperSelectPoint, cPrioSuper, &gi_uwOP_TWINS_RelayOn, true); // 延时约206uS，L载由于电流滞后90°，可能SCR未闭合。
				sOSTimerStart();
