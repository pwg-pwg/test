...
void sSetRlyPointer(INT8U relaytype,INT16U pointer,INT8U event,INT8U prio,INT16U* relay,INT8U relayaction)
{
...
//	}
	...
	else if(relaytype == RLY_ZEROPOINT)
	{
		...
		OS_ENTER_CRITICAL();
		wRlyActionPointerTemp = (((wRlyActionPointerTemp + g_wCtrlSinpointer + 2) / gi_wSinePointThreeSix + 1) * gi_wSinePointThreeSix) - wRlyActionPointerTemp;
		wRlyActionPointerTemp = wRlyActionPointerTemp % gi_wSinePointMax;
		OS_EXIT_CRITICAL();		
	}
	else
...
//	}
	if(wRlyActionPointer != RLY_ACTION_NULL)
	{
		...
		{
		    if(bRly1 != NULL)  *bRly1 = bRly1Action;
			OSISREventSend(bRlyRelatedTask, bRlyActionEvent);
			wRlyActionPointer = RLY_ACTION_NULL;
			bRlyActionEvent = RLY_EVENT_NULL;
			bRly1 = NULL;
			bRly1Action = RLY_ACTION_NULL;
		}
	}
	if(wRlyActionPointer2 != RLY_ACTION_NULL)
	{
		...
		{
		    if(bRly2 != NULL)  *bRly2 = bRly2Action;
			OSISREventSend(bRlyRelatedTask2, bRlyActionEvent2);
			wRlyActionPointer2 = RLY_ACTION_NULL;
			bRlyActionEvent2 = RLY_EVENT_NULL;
			bRly2 = NULL;
			bRly2Action = RLY_ACTION_NULL;
		}
	}
}

void sRlyDelayProc(INT8U relaytype,INT16U pointer,INT8U event,INT8U prio,INT16U* relay,INT8U relayaction)
{
	sSetRlyPointer(relaytype, pointer, event, prio, relay, relayaction); //(RLY_CROSSZERO,cSFTRlySCRDelayTime,eSuperSelectPoint,cPrioSuper);
