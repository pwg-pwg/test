...
const pFunc1 GetDataSubArray[100]=
{
    ...
    swGetFBR_Is_P,         swGetRFBR_Is_P,              swGetRFBPWM,     						//72-74 内环误差,内环输出经过调制之后的PWM，带R
	...
	swGetRGenCurrReal,         (pFunc1)swGetGenPeriodCntNew,    swReturnNull,       			//93-95 Gen电流实时真实值 Gen周期值
...
void sQ2Command(INT8U sciid)
{
    ...
    //OP2功率
  	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	sNumToASCII(g_dwSmartOPRealWatt, 6, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 6;
	//OP2 电压
  	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	sNumToASCII(g_uwRSmartOPVolt, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 4;
	//op2 电流
  	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	sNumToASCII(g_uwRSmartOPCurr, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 4;
/*-------------------------------------------------------------------------------*/
	
    //Gen功率
  	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	sNumToASCII(g_dwRGePower, 6, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 6;
	//Gen电压
  	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	sNumToASCII(g_uwRGenVolt, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 4;
	//Gen电流
  	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	sNumToASCII(g_uwRGenCurr, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 4;
