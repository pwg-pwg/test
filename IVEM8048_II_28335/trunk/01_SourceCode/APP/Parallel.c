...
typedef union
{
	...
	struct
	{
		INT16U wBatOpen         :1;
		INT16U wBatUnder        :1;
		INT16U wLineAbnormal    :2; // 0=正常，1=其它异常，2=电压异常，3=频率异常
		INT16U wInvSoftOK       :1;
...
enum
{
	eInverterType_HV,		// 1 = Hv 
 	eInverterType_LV,		// 2 = Lv
 	eInverterType_AIO		// 3 = 一体机
};

typedef union
{
	INT16U uwMachineInfo;
	struct
	{
		INT16U  uwParaVersion       :4;  //并机通讯版本，0~15
		INT16U  uwInverterType      :2;  //逆变器类型
		INT16U  uwIterationVersion  :2;  //迭代版本

		INT16U  uwPower			    :5;  //功率，0~32KW	
		INT16U  uwNumberOfBat 	    :3;  //电池节数，0-8
	}BIT;
}STRMachineInfo;
STRMachineInfo  g_strMachineInfoOwn;
...
void sMachineInfoInit(void)
{
    g_strMachineInfoOwn.BIT.uwParaVersion = cParaVersion;
    g_strMachineInfoOwn.BIT.uwInverterType = eInverterType_HV;	
    g_strMachineInfoOwn.BIT.uwIterationVersion = eVersion_II;
    g_strMachineInfoOwn.BIT.uwPower = 8;
    g_strMachineInfoOwn.BIT.uwNumberOfBat = 4;
}
			...
			else
			{
				if(g_uwParaMode == cNewMode)
				{
					...
					else if(g_uwCommand == cNewInquire)
					{
						...
						if(g_strCanReceiveFrame.Data2 == g_strMachineInfoOwn.uwMachineInfo)  //cParaVersion
						{
				        	sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh, cNullData, g_strLocalID.BIT.wLocalIDLow,\
		...
		if(g_uwParaEvent & (1 << eParallelTimer))
		{
			if(wParaCnt == 0)
			{
				sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh, cNullData, g_strLocalID.BIT.wLocalIDLow,\
				                ...
				                g_strLocalID.BIT.wLocalIDHigh, g_strLocalID.BIT.wLocalIDLow, g_strMachineInfoOwn.uwMachineInfo);  //cParaVersion
				sCan2Write9(&g_strCanTxCom2);
			}
...
void sModeSwitchEvent(void)
{
	if(g_uwParaEvent & (1 << eParaToLineMode))	
	{
		if(g_uwParaMode == cMasterMode && gi_wParallelEnable)
		{
			...
			sOSTimerStop();
			sRlyDelayProc(RLY_CROSSZERO, (cBatSwitchLineSCRDelayTime + cDelay10ms), eParaSelectPoint, cPrioPara, NULL, RLY_ACTION_NULL);	
			sOSTimerStart();
//				OS_ENTER_CRITICAL();
...
//				OS_EXIT_CRITICAL();
	...
	if(g_uwParaEvent & (1 << eParaOPRlyOn))
	{
		if(g_uwParaMode == cMasterMode)
		{
			...
			{
				sOSTimerStop();
				sRlyDelayProc(RLY_CROSSZERO, (cOPRlySCRDelayTime + cDelay10ms), eParaSelectPoint, cPrioPara, NULL, RLY_ACTION_NULL);
				sOSTimerStart();
				sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh, cNullData, g_strLocalID.BIT.wLocalIDLow,
	...
	for(wCnt = 0; wCnt < g_uwParaNumMaxSize; wCnt++)
	{
		if(g_uwSlaveExsitFlg[wCnt] == 1)
		{
			...
			*/
			...
			uwExcessLoadAvg |= (1 << g_strLocalParameterSet1Temp.BIT.uwOPMode); // 提取并机系统中，并机模式：单机 / 并机 / 三相并机R/S/T
...
//			}
	...
	if((uwExcessLoadAvg & 0x001C) != 0)			//    11100  00010
	{
		if(   (uwExcessLoadAvg & 0x0002) != 0
		   && (g_uwWorkMode != cFaultMode) ) // 设置3相并机，但同时使能单相并机时，判定为设定异常。
		{
