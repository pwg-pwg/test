...
void	sInitCMPSSLite3(INT16U wCurr)			// INV.I
{
	INT16U wTemp;
    //
...
//    CMPSSLITE_configLowComparator(CMPSSLITE3_BASE,(CMPSS_INSRC_DAC));	//  | CMPSS_INV_INVERTED
    ...
    // Sets the value of the internal DAC of the high comparator.
    // 设置高比较器的内部DAC的值。
    wTemp = (INT16U)((F32)wCurr / 6600.0f * 2048.0f + 2048.0f);
	CMPSSLITE_setDACValueHigh(CMPSSLITE3_BASE,wTemp);	// 48A(硬件保护55A) 4800/(6600/2048)+2048
    //
    // Sets the value of the internal DAC of the low comparator.	// 2048 + 
    // 设置低比较器的内部DAC的值。
	wTemp = (INT16U)(2048.0f - (F32)wCurr / 6600.0f * 2048.0f);
	CMPSSLITE_setDACValueLow(CMPSSLITE3_BASE,wTemp); 
    //
    //  Configures the digital filter of the high comparator.
    // 配置高比较器的数字滤波器。
