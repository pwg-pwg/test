...
INT16U	bControlRsvd;
INT16U	wControlHv;						// 控制板硬件版本

INT16U	wModelInitialFinish = false;	// 机型初始化完成标志
...
INT8U	sbInverterIdentify(INT8U bFilter)
{
	...
	if(bControlRsvd >= (bFilter>>1))
	{
		wControlHv = cCtrlHV01;
	}
...
void	sInvParaUpdate(void)
{
	if(g_uwModelVar == cCtrlIVCM3224P1G2)
	{
		...
		strInvParameterData.Field.uwControlHv = wControlHv;						// 控制板硬件版本
		strInvParameterData.Field.uwMainHv = cMainHV00;							// 功率板硬件版本
		...
		strInvParameterData.Field.uwInvDeratingVolt = cAcVoltReal170V;			// 逆变降额最低电压
		...
		strInvParameterData.Field.uwInvCurrLimit = 1000;						// 逆变控制器限流值 31.25A=1000/32
		...
		strInvParameterData.Field.uwLoadVoltLowUPS = cAcVoltReal160V;				// 输出低压值 UPS 160V
	...
	else if(g_uwModelVar == cCtrlIVCM2012P1G2)
	{
		...
		strInvParameterData.Field.uwInvCurrLimit = 704;							// 逆变控制器限流值 22A=704/32
    ...
    OS_ENTER_CRITICAL();
	sSetInvVoltCtrlParaCurrLimit(strInvParameterData.Field.uwInvCurrLimit); 	// 更新逆变控制器限流值
    OS_EXIT_CRITICAL();
//	if(strInvParameterData.Field.uwControlHv >= cCtrlHV01)
//	{
//		sSetInvVoltCtrlParaCurrLimit(cInvVoltCurrLimit); 						// 更新逆变控制器限流值
//		sInitCMPSSLite3(cInv48A);												// 更新TZ值 for test 待验证是否能生效
//	}
//	else
//	{
//		sSetInvVoltCtrlParaCurrLimit(cInvVoltCurrLimitMin); 
//	}
...
INT16U	uwGetModelInitialFinish(void) 
{
	return wModelInitialFinish;
}
...
INT16U	uwGetControlHv(void)
{
	return strInvParameterData.Field.uwControlHv;
}

INT16U	uwGetMainHv(void)
{
	return strInvParameterData.Field.uwMainHv;
}
...
INT16S	uwGetInvCurrLimit(void)
{
	return strInvParameterData.Field.uwInvCurrLimit;
}
