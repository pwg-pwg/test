...
INT16U wDCDCSoftPWMValue = 0;								// 软起完成时候的占空比
...
//		mDisDCDCHForcePWMOut();
	...
	else if(swGetwLLCAQChkFLAG() == false)
	{
//		wLLCAQChkFLAG = false;
...
//						}
				...
				{
					wDCDCSoftPWMValue = gi_wDCDCConvPWMValue;
				}
...
//			}
		...
		if(g_uwDCDCCtrlSts == cDCDCWork)
		{
			...
			else if(gi_wDCDCConvPWMValue > (wDCDCSoftPWMValue + 3))
			{
				mEnDCDCPWMOut();
				mEnDCDCHPWMOut();
				mEnDCDCForcePWMOut();
			...
			else if(gi_wDCDCConvPWMValue > wDCDCSoftPWMValue)			// 软起完成后停三个周期
			{
				sEpwmInDeadTimeChk(false);
				wLLCAQChkFLAG = false;
			}
...
{
	INT16U wDcDcPWMPeriod;

	wDcDcPWMPeriod = uwGetDcDcPWMPeriod();
	...
	&& (swGetwLLCAQChkFLAG() == false))	// 向下计数 1622 ~ 50
	{
		...
		if(uwGetModelInitialFinish() == true)
		{
			mDCDCConvPWM1 = wDcDcPWMPeriod;
			mDCDCConvPWM3 = wDcDcPWMPeriod;
		}
		else
		{
...
//		sSetLLCCtrlSts(false);
	...
	else if((EPwm4Regs.TBCTR <= (wDcDcPWMPeriod - 50)) && (EPwm4Regs.TBSTS.bit.CTRDIR == 1)\
...
void sSetDCDCCtrlSts(INT16U uwDCDCCtrlSts)					// 设置DCDC控制器状态
{
	...
	if(uwDCDCCtrlSts == 0)
	{
		swSetLLCAQChkFLAG(false);	// LLC过零封波标志
		sEpwmInDeadTimeChk(false);
	}
