...
// 查询电池包EMS数据
// 电池包V1xx,V2xx版本主机查询地址1
// 电池包V4xx,V5xx版本主机查询地址30
void sEmsDataFramePolling(INT8U sciid)
{
	INT16U wCRC;
	INT8U  TxLen;
	static INT8U  TxBuff[8] = {0};
	static INT8U  wAddrTemp = 0;
	if(uwGetBatSerialPcs() == 1)
	{
		TxBuff[cMODBUS_ADDR] = 1;						
	}
	else
	{
		if(sbGetEMSBMSConnect() == false) 			// 没有握手成功地址轮询
		{
			if(++wAddrTemp > 16)
			{
				wAddrTemp = 1;
			}
			TxBuff[cMODBUS_ADDR] = wAddrTemp;
		}
		else
		{
			TxBuff[cMODBUS_ADDR] = wAddrTemp;		// 握手成功使用上一个地址
		}
	}
...
extern union INV_PARAMETER_DATA_TABLE strInvParameterData;
extern INT16S g_wRInvOverCurrTzCnt;
extern INT16S g_wInvVoltCurrLimitTemp;
...
INT16U	swTestCmdRead(INT16U RdAddr)
{
	...
	switch(RdAddr)
	{
		...
		break;
		case DBG_XXXXXXX04:
			wTemp = uwGetControlHv();//控制板硬件版本
		break;
		case DBG_XXXXXXX05:
			wTemp = swVerVoltCal();
		break;
		case DBG_XXXXXXX06:
			wTemp = g_wRInvOverCurrTzCnt;
		break;
		case DBG_XXXXXXX07:
			wTemp = mBEEP;
		break;
		...
		break;
		case DBG_XXXXXXX11:
			wTemp = g_wInvVoltCurrLimitTemp;
		break;
		case DBG_XXXXXXX12:
			wTemp = sGetInvVoltCtrlParaCurrLimit();
		break;
...
//			wTemp = (cCPU_FREQ/2/cDCDCPeriod);
		...
		break;
		case DBG_XXXXXXX20:
			wTemp = uwGetDcDcPWMPeriod();
		break;
		case DBG_XXXXXXX21:
			wTemp = uwGetDcDcPWMDutySet();
		break;
		case DBG_XXXXXXX22:
			wTemp = 22;
		break;
		...
		break;
		case DBG_XXXXXXX93:		// Turbo模式降额输出电压
...
INT8U	swTestCmdWrite(INT16U WrAddr, INT16U WrData)
{
	...
	switch(WrAddr)
	{
		...
		break;
		case DBG_XXXXXXX20:		
			if((WrData < 2142) && (WrData > 1333))	// 28K-45K
			{
				strInvParameterData.Field.uwDcDcPWMPeriod = WrData;
				sDCDCParaInit();
			}
		break;
		case DBG_XXXXXXX21:		
			if((WrData < 300) && (WrData > 100))	// 28K-45K
			{
				strInvParameterData.Field.uwDcDcPWMDutySet = WrData;
				sDCDCParaInit();
			}
		break;
		case DBG_XXXXXXX22:		
