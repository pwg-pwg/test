...
interrupt void isrINV(void)
{
	...
	if((wTzFlagTemp == true) && ((!EPwm1Regs.TZFLG.bit.DCAEVT1) || (!EPwm2Regs.TZFLG.bit.DCAEVT1)))
	{
		wTzFlagTemp = false;
		mEnINVPWMOut();
	}
	...
	if(wInvCurrTemp >= cInv50A || wInvCurrTemp < -cInv50A)	// 继电器30A,1.5倍就是45A
	{
		g_wRInvOverCurrCnt++;
		g_wRInvOverCurrPCnt++;
		wRInvOverCurrFlg = 0;
//		wRInvOverCurrFlg = 1;			// 20260708 取消这一级逐波限流，使用TZ替代
	}
	else
	{
	...
	{
		if((!EPwm1Regs.TZFLG.bit.DCAEVT1) || (!EPwm2Regs.TZFLG.bit.DCAEVT1))
		{
			mEnINVPWMOut();
		}
	}
...
//			mDisDCDCHForcePWMOut();
	...
	// 母线&电池过压信号检测
	if((!mBUSBATVOVER) && (swGetWorkMode() != cPowerOnMode)\
		&& (uwGetBatBusOverVoltEn() == true))			// 触发后一直计数，在任务里面做检测触发进入故障模式
	{
		if(++gi_uwBusBatHiIntCnt >= 10)
		{
			gi_uwBusBatHiIntCnt = 10;
			sEpwmInDeadTimeChk(false);
//			mDisDCDCForcePWMOut();
//			mDisDCDCHForcePWMOut();
			sSetDCDCCtrlSts(cDCDCWait);
