...
void	subBusUnderChk(INT16U uwBusUnderLevel, INT16U uwFilter)
{
	...
	if(swGetWorkMode() == cLineMode)
	{
		if(uwGetModelLV())
		{
			uwBusUnderLevelTemp = swGetLineRms3timeAvgPeak() + 150;
		}
		else
		{
...
void	sBusCurrOverChk(INT16U uwBusCurrOverLevel, INT16U uwFilter)
{
	...
	{
		if(sb16UUnderLevelChk(swAbs(wBusCurrNew), uwBusCurrOverLevel - 500, uwFilter, &s_uwBusCurrChkCnt))
		{
	...
	{
		if(sb16UOverLevelChk(swAbs(wBusCurrNew), uwBusCurrOverLevel, uwFilter, &s_uwBusCurrChkCnt))
		{
