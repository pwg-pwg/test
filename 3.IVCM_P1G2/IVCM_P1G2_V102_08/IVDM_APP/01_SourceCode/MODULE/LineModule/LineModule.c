...
INT16S wRLineVAFilterConversion;	// 1VA	折算功率
...
//		dwTemp = swGetROpVANew() + swGetRInvVANew();	// 市电电流无采样，通过OP和INV折算
	...
	{
		dwTemp = ((INT32S)(uwGetGridConversionWatt()) * (uwGetGridConversionWatt())) \
				+ ((INT32S)wRLineVarFilter * wRLineVarFilter);
		wRLineVAFilterConversion = swRoot((INT32U)dwTemp);
	}
}
...
void	sGridCurrOverChk(INT16U uwGridOverCurrLevel, INT16U uwFilter)
{
	...
	{
		if(sb16UUnderLevelChk(swAbs(wRLineCurrNew), uwGridOverCurrLevel - 800, uwFilter, &s_uwGridOverLoadChkCnt))
		{
	...
	{
		if(sb16UOverLevelChk(swAbs(wRLineCurrNew), uwGridOverCurrLevel, uwFilter, &s_uwGridOverLoadChkCnt))
		{
...
INT16S	swGetRLineVAFilterConversion(void)
{
	return wRLineVAFilterConversion;
}
