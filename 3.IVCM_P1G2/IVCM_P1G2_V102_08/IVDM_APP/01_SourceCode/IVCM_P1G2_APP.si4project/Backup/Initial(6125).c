...
//	GpioCtrlRegs.GPAMUX1.bit.GPIO9 = 0;		// GPIO9 = GPIO
	// for test BEEP改为输入口，用于识别新旧控制板 旧版低电平，新版高电平
	GpioCtrlRegs.GPAPUD.bit.GPIO13 = 0;	  	// Disable pullup on 
	GpioDataRegs.GPACLEAR.bit.GPIO13 = 1;  	// Load output latch to high
	GpioCtrlRegs.GPADIR.bit.GPIO13 = 0;	  	// GPIO30 = input
	GpioCtrlRegs.GPAMUX1.bit.GPIO13 = 0;   	// GPIO30 = GPIO
...
//    CMPSSLITE_configLowComparator(CMPSSLITE3_BASE,(CMPSS_INSRC_DAC));	//  | CMPSS_INV_INVERTED
    ...
    // Sets the configuration for the internal comparator DACs.
    // 设置内部比较器dac的配置。
    CMPSSLITE_configDAC(CMPSSLITE3_BASE, CMPSS_DACVAL_PWMSYNC);
    //
    // Sets the value of the internal DAC of the high comparator.
    // 设置高比较器的内部DAC的值。
//    CMPSSLITE_setDACValueHigh(CMPSSLITE3_BASE,3506U);	// 47A(硬件保护49A)     4700/(6600/2048)+2048
	CMPSSLITE_setDACValueHigh(CMPSSLITE3_BASE,3537U);	// 48A(硬件保护55A) 4800/(6600/2048)+2048
//    CMPSSLITE_setDACValueHigh(CMPSSLITE3_BASE,3848U);	// 58A(硬件保护55A) 5000/(6600/2048)+2048
    //
    // Sets the value of the internal DAC of the low comparator.	// 2048 + 
    // 设置低比较器的内部DAC的值。
//    CMPSSLITE_setDACValueLow(CMPSSLITE3_BASE,590U);	// 47A
	CMPSSLITE_setDACValueLow(CMPSSLITE3_BASE,559U); // 48A
//	CMPSSLITE_setDACValueLow(CMPSSLITE3_BASE,248U);	// 58A 
    //
    //  Configures the digital filter of the high comparator.
    // 配置高比较器的数字滤波器。
...
//	EPWM_enableDigitalCompareTripCombinationInput(EPWM1_BASE,EPWM_DC_COMBINATIONAL_TRIPIN4,EPWM_DC_TYPE_DCBH);

	//
	// Enable DCA as OST
	// 启用DCA作为OST
	EPWM_enableTripZoneSignals(EPWM1_BASE, EPWM_TZ_SIGNAL_DCAEVT1);//EPWM_TZ_SIGNAL_DCAEVT1
//	EPWM_enableTripZoneSignals(EPWM1_BASE, EPWM_TZ_SIGNAL_DCBEVT2);//EPWM_TZ_SIGNAL_DCBEVT1

//	// 开启 DCAEVT1 锁存：输出使用锁存后的信号
//	EPwm1Regs.DCACTL.bit.EVT2LATSEL = 1;
//	EPwm1Regs.DCBCTL.bit.EVT2LATSEL = 1;
//	// 设置锁存清零时机：CTR = 0（每个周期起始）自动清零
//	// 00 = CTR=0清零	01 = CTR=PRD清零	10 = 两者都清零
//	EPwm1Regs.DCACTL.bit.EVT2LATCLRSEL = 0;
//	EPwm1Regs.DCBCTL.bit.EVT2LATCLRSEL = 0;
...
//	EPWM_setDigitalCompareEventSource(EPWM1_BASE,EPWM_DC_MODULE_B,EPWM_DC_EVENT_2,EPWM_DC_EVENT_SOURCE_ORIG_SIGNAL);
	...
	// Clear trip flags
	// 清除标志
	EPWM_clearTripZoneFlag(EPWM1_BASE, EPWM_TZ_INTERRUPT | EPWM_TZ_FLAG_OST | EPWM_TZ_FLAG_CBC \
							| EPWM_TZ_FLAG_DCAEVT2 | EPWM_TZ_FLAG_DCBEVT2);
...
//	// 开启 DCAEVT1 锁存：输出使用锁存后的信号
//	EPwm2Regs.DCACTL.bit.EVT2LATSEL = 1;
//	EPwm2Regs.DCBCTL.bit.EVT2LATSEL = 1;
//	// 设置锁存清零时机：CTR = 0（每个周期起始）自动清零
//	// 00 = CTR=0清零	01 = CTR=PRD清零	10 = 两者都清零
//	EPwm2Regs.DCACTL.bit.EVT2LATCLRSEL = 0;
//	EPwm2Regs.DCBCTL.bit.EVT2LATCLRSEL = 0;
...
//	EPWM_setDigitalCompareEventSource(EPWM2_BASE,EPWM_DC_MODULE_B,EPWM_DC_EVENT_2,EPWM_DC_EVENT_SOURCE_ORIG_SIGNAL);
	...
	// Clear trip flags
	// 清除标志
	EPWM_clearTripZoneFlag(EPWM2_BASE, EPWM_TZ_INTERRUPT | EPWM_TZ_FLAG_OST | EPWM_TZ_FLAG_CBC \
							| EPWM_TZ_FLAG_DCAEVT2 | EPWM_TZ_FLAG_DCBEVT2);
	...
	EPwm3Regs.AQSFRC.bit.RLDCSF = 2;//3;
	EDIS;
...
//	EPwm3Regs.ETSEL.bit.INTSEL = ET_CTR_PRD;		// Enable event time-base counter equal to period 
	...
	//10：时基计数器上的负载等于零或计数器等于周期
	//11：立即加载（活动寄存器由程序直接访问CPU，而不是从影子寄存器加载)。
	EPwm4Regs.AQSFRC.bit.RLDCSF = 2;//3;
	EDIS;
