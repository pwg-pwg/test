...
#pragma CODE_SECTION(sBusCurrDerating,".TI.ramfunc");
...
#define cInvVoltCurrLimit			1408		// Q5 1100W/220V*1.414*32
#define cInvVoltCurrLimitMin		1000		// Q5 1100W/220V*1.414*32
...
INT16S g_wInvVoltCurrLimit = cInvVoltCurrLimit;		// cCntl_InvCurrLimit;	
...
INT16S g_wInvVoltCurrLimitTemp = cInvVoltCurrLimit;
// 母线电流降额,带两倍R载时LLC管子应力高
void	sBusCurrDerating(void)
{
	INT16S wDisWattReal;
	
	static INT16S g_wFlag = 0;
	static INT16S wMaxDisWatt = 0;
	static INT16S wCnt = 0;
//	static INT16S g_wInvVoltCurrLimitTemp = cInvVoltCurrLimit;
	...
	if(swGetWorkMode() == cBatteryMode)
	{
		...
		{
			g_wFlag = 1;
//			wMaxDisCurr = ((INT32U)uwGetSettingMaxDchgCurr() * 8) >> 2;	// 额定2倍
//			wMaxDisCurr = wMaxDisCurr / uwGetTransformerRatio() * 10;	// 还原成母线电流
//			wMaxDisWatt = wMaxDisCurr * uwGetBatSerialPcs() * cBatVoltReal12V / 10000;
			wMaxDisWatt = (INT16S)(((INT32U)uwGetInvWattMax() * 7) >> 2);	// 额定1.75倍
		}
		
		wDisWattReal = (INT16S)((INT32U)swGetAdcBusCurrReal() * swGetAdcBusVoltReal() / 1000);

		// 软起，减缓功率波动
		if(wDisWattReal > wMaxDisWatt)								
		{
			g_wInvVoltCurrLimitTemp -= 15;
			wCnt = 0;
		}
		else if(wDisWattReal < wMaxDisWatt)						
		{
			if((++wCnt > 100) && (g_wRSinPointer == 0))		
			{
				wCnt = 0;
				g_wInvVoltCurrLimitTemp += 5;
			}
		}
		
		UpDownLimit(g_wInvVoltCurrLimitTemp,cInvVoltCurrLimit,cInvVoltCurrLimitMin);	// 31A-44A
		sSetInvVoltCtrlParaCurrLimit(g_wInvVoltCurrLimitTemp);
		
	}
	else
	{
		g_wInvVoltCurrLimitTemp = cInvVoltCurrLimit;
		sSetInvVoltCtrlParaCurrLimit(cInvVoltCurrLimit);
	}
...
INT16S	sGetInvVoltCtrlParaCurrLimit(void)
{
...
void	sSetInvVoltCtrlParaCurrLimit(INT32S wValue)
{
