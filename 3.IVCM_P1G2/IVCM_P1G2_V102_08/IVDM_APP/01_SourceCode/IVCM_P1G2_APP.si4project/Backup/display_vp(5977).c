	...
	switch(swGetWorkMode()) 
	{
		...
		case	cLineMode:
			...
			{		  
				...
				// 电池功率正放电，负充电；电网功率正放电，负馈电
				if(swGetBatWattFilter() > 50 && swGetRLineWattFilter() < 20)		// 考虑待机功耗以及误差				
				{
...
void sSettingPageACChgCurr(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(s_wSettingValueTemp < 0)
	{
		s_wSettingValueTemp = cMaxACInputCurr / 100;
		(*pwChangeValue) = (s_wSettingValueTemp - wSettingValue);
	}
	else if(s_wSettingValueTemp > cMaxACInputCurr / 100)
	{
		s_wSettingValueTemp = 0;
		(*pwChangeValue) = (s_wSettingValueTemp - wSettingValue);
	}

	if(s_wSettingValueTemp > cMaxACInputCurr / 100)
	{
		s_wSettingValueTemp = cMaxACInputCurr / 100;
	}
...
void sSettingPageACInPower(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	...
	if(s_wSettingValueTemp < 0)
	{
		s_wSettingValueTemp = cMaxACInputPower / 100;
		(*pwChangeValue) = (s_wSettingValueTemp - wSettingValue);
	}
	else if(s_wSettingValueTemp > cMaxACInputPower / 100)
	{
		s_wSettingValueTemp = 0;
		(*pwChangeValue) = (s_wSettingValueTemp - wSettingValue);
	}

	if(s_wSettingValueTemp > cMaxACInputPower / 100)
	{
		s_wSettingValueTemp = cMaxACInputPower / 100;
	}
