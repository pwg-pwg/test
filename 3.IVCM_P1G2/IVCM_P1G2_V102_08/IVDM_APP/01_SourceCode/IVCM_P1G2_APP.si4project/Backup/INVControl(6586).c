...
void	sBusCurrDerating(void)
{
	...
	static INT16S wMaxDisCurr;
	static INT16S g_wFlag = 0;
	static INT16S g_wInvVoltCurrLimitCnt = 0;
//	static INT16S g_wInvVoltCurrLimitTemp = cInvVoltCurrLimit;
	...
	if(swGetWorkMode() == cBatteryMode)
	{
		...
		{
			g_wFlag = 1;
			wMaxDisCurr = (INT16S)(((INT32U)uwGetSettingMaxDchgCurr() * 8) >> 2);	// 额定2倍
			wMaxDisCurr = wMaxDisCurr / uwGetTransformerRatio() * 10;	// 还原成母线电流
		}
		
		wDisCurrAvg = swGetAdcBusCurrReal();

		// 软起，减缓功率波动
		if(wDisCurrAvg > wMaxDisCurr)								
		{
			g_wInvVoltCurrLimitTemp -= 15;
			g_wInvVoltCurrLimitCnt = 0;
		}
		else if(wDisCurrAvg < wMaxDisCurr)						
		{
			if((++g_wInvVoltCurrLimitCnt > 100) && (g_wRSinPointer == 0))		
			{
				g_wInvVoltCurrLimitCnt = 0;
