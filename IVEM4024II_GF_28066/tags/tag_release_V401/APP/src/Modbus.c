...
INT16U swWriteEESetting(INT16U WrAddr, INT16U WrData)
{
    ...
    switch(WrAddr)
    {
		...
		case E2_FeedPower:
			if(WrData <= 4000)
			{
				if(WrData != swGetEEFeedPower())
				{
					sSetEEFeedPower(WrData);
					OSEventSend(cPrioInterface, eI2CEEpromUserSave);
				}
			}
			else
		...
		break;
	    case E2_IslandProtectEn:
			if(WrData <= 1)
			{
				if(swGetEEIslandProtectEn() != WrData)
				{
					sSetEEIslandProtectEn(WrData);
					OSEventSend(cPrioInterface, eI2CEEpromUserSave);
				}
			}
			else
...
//  unRealTimeData.Stru.wFaultMsg = 0;
	...
	{
		unRealTimeData.Stru.wROpVA = (g_dwLoadPowerConversion > g_dwOPVA)?(g_dwLoadPowerConversion):(g_dwOPVA);
	}
	else
	{
    ...
    unRealTimeData.Stru.wRLineWatt = (g_dwOPWatt + g_dwSmartOPWatt - g_dwInvWatt);
    if(g_uwLineModeJoinInWay == cLineModeJoinInByGen)
    {
    	unRealTimeData.Stru.wRLineWatt = 0;
    }

	if(!swGetEEFeedPowerEn() && unRealTimeData.Stru.wRLineWatt < 0)
	{
		unRealTimeData.Stru.wRLineWatt = 0;
	}
    dwLineWattTemp = unRealTimeData.Stru.wRLineWatt;
    unRealTimeData.Stru.wRLineCurr = (INT16S)(dwLineWattTemp*100/g_uwRLineVolt);

    unRealTimeData.Stru.wRLineVA   = 0;//swGetFBCtrl_CurrOut();
...
//    //通过子类来判断是否为馈网机，置位然后给到APP判断
//    if(cModbusSubType == 0x5206 || cModbusSubType == 0x540A || cModbusSubType == 0x540F || cModbusSubType == 0x5416)
//    {
//        unRealTimeData.Stru.State1 = 0x20;
//    }
//    else
//    {
//        unRealTimeData.Stru.State1 = 0;
//    }
    ...
    unRealTimeData.Stru.wFeedEnergyTotalHH    = (INT16U)((g_uldwFeedEnergyTotal >> 48) & 0xFFFF);
    unRealTimeData.Stru.wFeedEnergyTotalHL    = (INT16U)((g_uldwFeedEnergyTotal >> 32) & 0xFFFF);
    unRealTimeData.Stru.wFeedEnergyTotalLH    = (INT16U)((g_uldwFeedEnergyTotal >> 16) & 0xFFFF);
    unRealTimeData.Stru.wFeedEnergyTotalLL    = (INT16U)(g_uldwFeedEnergyTotal & 0xFFFF);
    unRealTimeData.Stru.wFeedEnergyThisYearH  = 0;
    unRealTimeData.Stru.wFeedEnergyThisYearL  = 0;
    unRealTimeData.Stru.wFeedEnergyThisMonthH = 0;
    unRealTimeData.Stru.wFeedEnergyThisMonthL = 0;
    unRealTimeData.Stru.wFeedEnergyThisDayH   = 0;
    unRealTimeData.Stru.wFeedEnergyThisDayL   = 0;
    unRealTimeData.Stru.wFeedEnergyYearH      = 0;
    unRealTimeData.Stru.wFeedEnergyYearL      = 0;
    unRealTimeData.Stru.wFeedEnergyMonthH     = 0;
    unRealTimeData.Stru.wFeedEnergyMonthL     = 0;
    unRealTimeData.Stru.wFeedEnergyDayH       = 0;
    unRealTimeData.Stru.wFeedEnergyDayL       = 0;


    //功率折算
    unRealTimeData.Stru.wLinePowerConvertion = g_dwLinePowerWattCversion;
    unRealTimeData.Stru.wLoadPowerConvertion = g_dwLoadPowerConversion;
    unRealTimeData.Stru.wGenPowerConvertion = g_dwGenPowerWattCversion;
    unRealTimeData.Stru.wSmartLoadPowerConvertion = g_dwSmartLoadPowerConversion;
    unRealTimeData.Stru.wBatPowerConvertion = g_wBatPowerConversion;
    unRealTimeData.Stru.wPV1PowerConvertion = g_uwPV1PowerConversion;
    unRealTimeData.Stru.wPV2PowerConvertion = 0;//g_uwPV2PowerConversion;

	//电流折算（耗时多久 TODO）
	if(g_uwRLineVolt > 0)   	unRealTimeData.Stru.wLineCurrConvertion = (INT16S)(g_dwLinePowerWattCversion*100/g_uwRLineVolt);
	else 						unRealTimeData.Stru.wLineCurrConvertion = 0;

	if(g_uwROPVolt > 0) 		unRealTimeData.Stru.wLoadCurrConvertion = (INT16S)(g_dwLoadPowerConversion*100/g_uwROPVolt);
	else						unRealTimeData.Stru.wLoadCurrConvertion = 0;

	if(g_wBatVoltAvg10 > 0)   	unRealTimeData.Stru.wBatCurrConvertion = (INT16S)((INT32S)g_wBatPowerConversion*1000/g_wBatVoltAvg10);
	else						unRealTimeData.Stru.wBatCurrConvertion = 0;

	if(g_uwSolarVolt1Avg > 0)	unRealTimeData.Stru.wPV1CurrConvertion = (INT16S)((INT32S)g_uwPV1PowerConversion*100/g_uwSolarVolt1Avg);
	else 						unRealTimeData.Stru.wPV1CurrConvertion = 0;

//	if(g_uwSolarVolt2Avg > 0)
//	{
//		unRealTimeData.Stru.wPV2CurrConvertion  = (INT16S)((INT32S)g_uwPV2PowerConversion*100/g_uwSolarVolt2Avg);
//	}
	if(g_uwRGenVolt > 0)		unRealTimeData.Stru.wGenCurrConvertion = (INT16S)(g_dwGenPowerWattCversion*100/g_uwRGenVolt);
	else						unRealTimeData.Stru.wGenCurrConvertion = 0;

	if(g_uwRSmartOPVolt > 0)	unRealTimeData.Stru.wSmartLoadCurrConvertion = (INT16S)(g_dwSmartLoadPowerConversion*100/g_uwRSmartOPVolt);
	else						unRealTimeData.Stru.wSmartLoadCurrConvertion = 0;
...
void sInfoDataUpdate(void)
{
    ...
    unInfoData.Stru.wMCU2SwVer  = cDSPPreVer;
...
void sEESettingUpdate(void)
{
	...
	strEepromDataStruct2.wEEFeedPower = swGetEEFeedPower();
	strEepromDataStruct2.wEEIslandProtectEn = swGetEEIslandProtectEn();
}
