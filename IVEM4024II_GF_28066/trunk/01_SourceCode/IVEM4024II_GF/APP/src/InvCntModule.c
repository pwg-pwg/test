...
void sFBInvCntParaInit(void)
{
    ...
    g_strInvCurrCtrlPara.wFBKcn_0 = 11321;//14000;//11321;//测试
    g_strInvCurrCtrlPara.wFBKcn_1 = 1269;//测试
    g_strInvCurrCtrlPara.wFBKcn_2 = 10051;//8000;//10051;//测试
...
//        g_strDCDCCtrlPara.wInvBusVoltErr = (INT16S)(((INT32S)g_strInvCurrCtrlPara.wFBVm_P - wBatChgInvLimit) >> 4);// 7
	...
	//-------------------------------------孤岛检测-------------------------------------
		...
		{
			if(g_wRSinPointerwThetaTemp < g_wSinePointThreeSix)
			{
				g_wRSinPointerTemp = (INT16S)((INT32S)g_wRSinPointerwThetaTemp * (wIslandDetectStep + wIslandRatio) / 50);
				if(g_wRSinPointerTemp > g_wSinePointThreeSix)
				{
					g_wRSinPointerTemp = g_wSinePointThreeSix;
				}
			}
			else
			{
				g_wRSinPointerTemp = (INT16S)(((INT32S)g_wRSinPointerwThetaTemp - g_wSinePointThreeSix) * \
										(wIslandDetectStep + wIslandRatio) / 50) + g_wSinePointThreeSix;
				if(g_wRSinPointerTemp > (g_wSinePointMax - 1))
				{
					g_wRSinPointerTemp = (g_wSinePointMax - 1);
				}
			}
		}
		else
		...
		g_strInvCurrCtrlPara.wFBImo_P = (INT16S)(((INT32S)pSinTab[g_wRSinPointerTemp] * g_strInvCurrCtrlPara.wFBVmo)>> 14);//待确认系数（左右移位）
    }
    else
    {
...
{
    return g_wRSinPointerTemp;//g_wInvOutput1;
}

INT16S swGetInvInvOutput2(void)
{
    return g_wRSinPointerwThetaTemp;//g_wInvOutput2;
}
