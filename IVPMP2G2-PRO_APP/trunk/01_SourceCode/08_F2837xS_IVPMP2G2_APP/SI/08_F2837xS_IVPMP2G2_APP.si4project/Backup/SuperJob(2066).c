...
//                            || g_uwBatStartFail == true))
            ...
            else if(g_uwWorkMode==cFaultMode)
            {
                if((fLoad.LoadOnCmd == false)&&(hiSWOn == 1))
                {
                    ...
                    {
                        ++wModeSwitchTimerCnt;
                        if(wModeSwitchTimerCnt>500)//10s
                        {
                            wModeSwitchTimerCnt = 0;
                            #ifdef ShutdownSPS
                                sSetFaultCode(0);
    ...
    while(1)
    {
        ...
        if(event&(1<<eSuperToLine))
        {
            ...
            if((g_wVAType == cb3500VAModel)||(swGetEepromOutputMode() == cOP_Single))
            {
				...
				sSetRlyPointer(RLY_CROSSZERO,60,eSuperSelectPoint,cPrioSuper);
                while(OSMaskEventPend(0x7FFF) == 0);
                if(g_LineModeJoinInWay == cLineModeJoinInByLine)
                {
                    hoGridRelayOn;
				...
				sSetRlyPointer(RLY_DELAY,cDelay10ms,eSuperSelectPoint,cPrioSuper);
                while(OSMaskEventPend(0x7FFF) == 0);
                sSetRlyPointer(RLY_DELAY,cDelay10ms+5,eSuperSelectPoint,cPrioSuper);
                while(OSMaskEventPend(0x7FFF) == 0);
                sSetFBControlStatus(cFBWaitSts);
                sSetBuckControlStatus(cBuckWaitSts);
            }
...
//            if((fBMSForceChgFlg ==0)&&((swGetPvOkFlag() == true && sbGetEepromChargerPriority() == cChargeSolarFrist)
//            || (sbGetEepromChargerPriority() == cChargeSolarOnly) || (sbGetEepromMaxGridChgBatCurr() == 0)\
//            ||fBMSStopChargeFlg))
			if((fBMSForceChgFlg ==0)&&((swGetPvOkFlag() == true && sbGetEepromChargerPriority() == cChargeSolarFrist)
			  || (sbGetEepromChargerPriority() == cChargeSolarOnly) || fBMSStopChargeFlg))
            {
    ...
    while(1)
    {
        ...
        if(event&(1<<eSuperTimer))   //20ms
        {
            if((sbGetInvVoltSoftFinishSts() != true)&&(++wDelayToLineCnt<1000)&&(InvVoltRelayAction ==0)\
                &&(wBatAvgVoltNew >=swGetBatUnderVolt())&&(InvVoltSoftFalseFlag == 0))//20S浠ュ悗鏈蒋璧锋垚鍔熶篃浼氬垏鎹㈠競鐢?
            {
                b2Cnt = 0;
...
//		hoPVSPS_ON;
    ...
    if(wTestModeFaultId[i] == 0)
    {
        if(abs(swAvgValueCal(&AcSampData[OUT_L1_VOLT].SampCal.AvgSampSum)) > 100)
        {
            if(++wFaultCnt[i] >= 25)
            {
                wFaultCnt[i] = 0;
