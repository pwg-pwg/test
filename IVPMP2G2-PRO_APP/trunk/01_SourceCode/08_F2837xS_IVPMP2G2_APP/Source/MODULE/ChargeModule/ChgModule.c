    ...
    if(wParaMode == cMasterMode)
    {
        ...
        else
        {
            if(swGetInvChgStatus() == cRespondToCCCharging || swGetInvChgStatus() == cRespondToCVCharging)
            {
                ...
                if(swGetEepromBatMode() != cNoBatmode)//无电池模式不转浮充
                {
                    wBatLowCnt = 0;
...
void sInvChgCurrCal(void)//20ms
{
    ...
    {
        if(sdwGetInvWattCalFilt() < 0)//充电
        {
            g_wChgEfficiency = 8800;
            /*
            if(sdwGetInvWattCalFilt() <= 1500)
            {
                g_wChgEfficiency = 8800;
            }
            else if(sdwGetInvWattCalFilt() < 3000)
            {
                g_wChgEfficiency = 8800 + (sdwGetInvWattCalFilt() - 1500)/5;//0.91
            }
            else if(sdwGetInvWattCalFilt() < 5000)
            {
                g_wChgEfficiency = 9100;  //0.91
            }
            else if(sdwGetInvWattCalFilt() < 8000)
            {
                g_wChgEfficiency = 9100 + (sdwGetInvWattCalFilt() - 5000)/15;//0.93
            }
            else if(sdwGetInvWattCalFilt() < 12000)
            {
                g_wChgEfficiency = 9300 - (sdwGetInvWattCalFilt() - 8000)/20;//0.91
            }
            else
            {
