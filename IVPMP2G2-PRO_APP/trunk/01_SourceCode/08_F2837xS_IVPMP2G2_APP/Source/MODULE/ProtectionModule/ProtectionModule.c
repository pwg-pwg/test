...
INT16S	wInvOverTempPoint;
INT16S	wInvOverTempBackPoint;
INT16S	wBatOverTempPoint;
INT16S	wBatOverTempBackPoint;
INT16S	wTxtOverTempPoint;
INT16S	wTxtOverTempBackPoint;
INT16S	wPvOverTempPoint = cPvHsTempOv15KVA;
INT16S	wPvOverTempBackPoint = cPvHsTempOvBack15KVA;
INT16S  wInternalOverTempPoint;
INT16S  wInternalOverTempBackPoint;
...
INT16S	wTxtTempDerate12KVA	=cTxtTempDerate12KVA;
INT16S	wTxtTempDerateUp12KVA =cTxtTempDerateUp12KVA;
INT16S	wTxtTempDerateBack12KVA =cTxtTempDerateBack12KVA;
...
INT16S	wTxtTempDerate8KVA	=cTxtTempDerate8KVA;
INT16S	wTxtTempDerateUp8KVA =cTxtTempDerateUp8KVA;
INT16S	wTxtTempDerateBack8KVA =cTxtTempDerateBack8KVA;
...
INT16U	wInvTempDerate12KVA =cInvTempDerate12KVA;
INT16U	wInvTempDerateUp12KVA =cInvTempDerateUp12KVA;
INT16U	wInvTempDerateBack12KVA =cInvTempDerateBack12KVA;
...
INT16U	wInvTempDerate8KVA =cInvTempDerate8KVA;
INT16U	wInvTempDerateUp8KVA =cInvTempDerateUp8KVA;
INT16U	wInvTempDerateBack8KVA =cInvTempDerateBack8KVA;
...
INT16U	wPvTempDerate12KVA	=cPvTempDerate12KVA;
INT16U	wPvTempDerateUp12KVA =cPvTempDerateUp12KVA;
INT16U	wPvTempDerateBack12KVA =cPvTempDerateBack12KVA;
...
INT16U	wPvTempDerate8KVA	=cPvTempDerate8KVA;
INT16U	wPvTempDerateUp8KVA =cPvTempDerateUp8KVA;
INT16U	wPvTempDerateBack8KVA =cPvTempDerateBack8KVA;
...
INT16S wInvVoltDcSampOvFaultFlag = 0;
...
void sTempModuleUpdate(void)
{
	...
	{
		wTxtOverTempPoint = cTxTempOv12KVA;
	    wTxtOverTempBackPoint = cTxTempOvBack12KVA;
	    wInvOverTempPoint = cInvHsTempOv12KVA;
	    wInvOverTempBackPoint = cInvHsTempOvBack12KVA;
	    wPvOverTempPoint = cPvHsTempOv12KVA;
	    wPvOverTempBackPoint = cPvHsTempOvBack12KVA;
	    wInternalOverTempPoint = cInternalTempOv;
	    wInternalOverTempBackPoint = cInternalTempOvBack;
		if(swGetEepromFanWorkMode() == cFanSilentMode)//静音模式下逆变变压器、PV散热器、逆变散热器保护点、恢复点均比性能模式下低
    	{
	        ...
	        //逆变散热器
	        wInvOverTempPoint = cInvHsTempOv12KVA - 5;
	        wInvOverTempBackPoint = cInvHsTempOvBack12KVA - 5;
	        //PV散热器
	        wPvOverTempPoint = cPvHsTempOv12KVA - 5;
	        wPvOverTempBackPoint = cPvHsTempOvBack12KVA - 5;
	        //内环境温度
	        wInternalOverTempPoint = cInternalTempOv - 50;
	        wInternalOverTempBackPoint = cInternalTempOvBack - 50;
		}
	}
	...
	{
		wTxtOverTempPoint = cTxTempOv8KVA;
	    wTxtOverTempBackPoint = cTxTempOvBack8KVA;
	    wInvOverTempPoint = cInvHsTempOv8KVA;
	    wInvOverTempBackPoint = cInvHsTempOvBack8KVA;
	    wPvOverTempPoint = cPvHsTempOv8KVA;
	    wPvOverTempBackPoint = cPvHsTempOvBack8KVA;
	    wInternalOverTempPoint = cInternalTempOv;
	    wInternalOverTempBackPoint = cInternalTempOvBack;
		if(swGetEepromFanWorkMode() == cFanSilentMode)//静音模式下逆变变压器、PV散热器、逆变散热器保护点、恢复点均比性能模式下低
    	{
...
//	        wTxtOverTempBackPoint = cTxTempOvBack10KVA - 10;
	        //逆变散热器
	        wInvOverTempPoint = cInvHsTempOv8KVA - 5;
	        wInvOverTempBackPoint = cInvHsTempOvBack8KVA - 5;
	        //PV散热器
	        wPvOverTempPoint = cPvHsTempOv8KVA - 5;
	        wPvOverTempBackPoint = cPvHsTempOvBack8KVA - 5;
	        //内环境温度
	        wInternalOverTempPoint = cInternalTempOv - 50;
	        wInternalOverTempBackPoint = cInternalTempOvBack - 50;
		}
...
void sTemperatureInvChk(INT16U bFilter)
{
	INT16S wOverTempWarningPoint;
	INT16S wOverTempWarningBackPoint;
...
void sNtcDisConnectChk(INT16U bFilter)
{
	...
	{
		if(sbUnderLevelChk(swGetTempDegree(TempSampData[TX1_TEMP]),NTC_DISCONNECT_POINT,bFilter,&bChkTxt1) == true)
		{
	...
	{
		if(sbOverLevelChk(swGetTempDegree(TempSampData[TX1_TEMP]), NTC_DISCONNECT_RECOVER_POINT,bFilter,&bChkTxt1) == true)
		{
	...
	{
		if(sbUnderLevelChk(swGetTempDegree(TempSampData[TX2_TEMP]),NTC_DISCONNECT_POINT,bFilter,&bChkTxt2) == true)
		{
	...
	{
		if(sbOverLevelChk(swGetTempDegree(TempSampData[TX2_TEMP]), NTC_DISCONNECT_RECOVER_POINT,bFilter,&bChkTxt2) == true)
		{
	...
	{
		if(sbUnderLevelChk(swGetTempDegree(TempSampData[INV1_TEMP]),NTC_DISCONNECT_POINT,bFilter,&bChkInv11) == true)
		{
	...
	{
		if(sbOverLevelChk(swGetTempDegree(TempSampData[INV1_TEMP]), NTC_DISCONNECT_RECOVER_POINT,bFilter,&bChkInv11) == true)
		{
	...
	{
		if(sbUnderLevelChk(swGetTempDegree(TempSampData[INV2_TEMP]),NTC_DISCONNECT_POINT,bFilter,&bChkInv12) == true)
		{
	...
	{
		if(sbOverLevelChk(swGetTempDegree(TempSampData[INV2_TEMP]), NTC_DISCONNECT_RECOVER_POINT,bFilter,&bChkInv12) == true)
		{
	...
	if(g_wVAType == cb15KVAModel)
	{
		...
		{
			if(sbUnderLevelChk(swGetTempDegree(TempSampData[BAT1_TEMP]),NTC_DISCONNECT_POINT,bFilter,&bChkInv21) == true)
			{
		...
		{
			if(sbOverLevelChk(swGetTempDegree(TempSampData[BAT1_TEMP]), NTC_DISCONNECT_RECOVER_POINT,bFilter,&bChkInv21) == true)
			{
		...
		{
			if(sbUnderLevelChk(swGetTempDegree(TempSampData[BAT2_TEMP]),NTC_DISCONNECT_POINT,bFilter,&bChkInv22) == true)
			{
		...
		{
			if(sbOverLevelChk(swGetTempDegree(TempSampData[BAT2_TEMP]), NTC_DISCONNECT_RECOVER_POINT,bFilter,&bChkInv22) == true)
			{
	...
	{
		if(sbUnderLevelChk(swGetTempDegree(TempSampData[PV_TEMP]),NTC_DISCONNECT_POINT,bFilter,&bChkLlcP) == true)
		{
	...
	{
		if(sbOverLevelChk(swGetTempDegree(TempSampData[PV_TEMP]), NTC_DISCONNECT_RECOVER_POINT,bFilter,&bChkLlcP) == true)
		{
	...
	{
		if(sbUnderLevelChk(swGetTempDegree(TempSampData[LLC_LOW_TEMP]),NTC_DISCONNECT_POINT,bFilter,&bChkLlcS) == true)
		{
	...
	{
		if(sbOverLevelChk(swGetTempDegree(TempSampData[LLC_LOW_TEMP]), NTC_DISCONNECT_RECOVER_POINT,bFilter,&bChkLlcS) == true)
		{
...
//		}
	...
	wTxtTempDerate12KVA = (INT16S)cTxtTempDerate12KVA + (2300 - swGetEepromOutputVolt())*7/10;
	wTxtTempDerateBack12KVA = (INT16S)cTxtTempDerateBack12KVA + (2300 - swGetEepromOutputVolt())*7/10;

	wInvTempDerate12KVA = (INT16S)cInvTempDerate12KVA + (2300 - swGetEepromOutputVolt())/2;     
	wInvTempDerateBack12KVA = (INT16S)cInvTempDerateBack12KVA + (2300 - swGetEepromOutputVolt())/2;

	wTxtTempDerate10KVA = (INT16S)cTxtTempDerate10KVA + (2300 - swGetEepromOutputVolt())*7/10;
	wTxtTempDerateBack10KVA = (INT16S)cTxtTempDerateBack10KVA + (2300 - swGetEepromOutputVolt())*7/10;
	...
	wTxtTempDerate8KVA = (INT16S)cTxtTempDerate8KVA + (2300 - swGetEepromOutputVolt())*7/10;
	wTxtTempDerateBack8KVA = (INT16S)cTxtTempDerateBack8KVA + (2300 - swGetEepromOutputVolt())*7/10;

	wInvTempDerate8KVA = (INT16S)cInvTempDerate8KVA + (2300 - swGetEepromOutputVolt())/2;     
	wInvTempDerateBack8KVA = (INT16S)cInvTempDerateBack8KVA + (2300 - swGetEepromOutputVolt())/2;
...
//	}
	...
	if(swGetEepromFanWorkMode() == cFanPerformanceMode)
	{
		...
		{
			//变压器温度降额
			if(wTempDegreeTxTemp < wTxtTempDerate12KVA)
			{
				wTempDerateRangeTx = 0;
			}
			else if(wTempDegreeTxTemp <= 1300)
			{
				wTempDerateRangeTx = ((wTempDegreeTxTemp - wTxtTempDerate12KVA)/10*96)>>6;
			}
			else if(wTempDegreeTxTemp <= 1400)
			{
				wTempDerateRangeTx = (swGetEepromOutputVolt() - 2000)/10 + (wTempDegreeTxTemp - 1300)/5;
			}
			else if(wTempDegreeTxTemp > 1400)
			{
				wTempDerateRangeTx = 60;
			}
			//Inv温度降额
			if(wTempDegreInvTemp < wInvTempDerate12KVA)
			{
				wTempDerateRangeInv = 0;
			}
			else if(wTempDegreInvTemp <= 780)
			{
				wTempDerateRangeInv = (wTempDegreInvTemp - wInvTempDerate12KVA)/5;
			}
			else if(wTempDegreInvTemp > 780)
			{
				wTempDerateRangeInv = 40;
		...
		else if(g_wVAType == cb10KVAModel)
		{
			...
			{
				wTempDerateRangeTx = ((wTempDegreeTxTemp - wTxtTempDerate10KVA)/10*96)>>6;
			}
			else if(wTempDegreeTxTemp <= 1400)
			{
				wTempDerateRangeTx = (swGetEepromOutputVolt() - 2000)/10 + (wTempDegreeTxTemp - 1300)/5;
		...
		{
			//变压器温度降额
			if(wTempDegreeTxTemp < wTxtTempDerate8KVA)
			{
				wTempDerateRangeTx = 0;
			}
			else if(wTempDegreeTxTemp <= 1300)
			{
				wTempDerateRangeTx = ((wTempDegreeTxTemp - wTxtTempDerate8KVA)/10*96)>>6;
			}
			else if(wTempDegreeTxTemp <= 1400)
			{
				wTempDerateRangeTx = (swGetEepromOutputVolt() - 2000)/10 + (wTempDegreeTxTemp - 1300)/5;
			...
			//Inv温度降额
			if(wTempDegreInvTemp < wInvTempDerate8KVA)
			{
				wTempDerateRangeInv = 0;
			}
			else if(wTempDegreInvTemp <= 780)
			{
				wTempDerateRangeInv = (wTempDegreInvTemp - wInvTempDerate8KVA)/5;
			}
			else if(wTempDegreInvTemp > 780)
			{
				wTempDerateRangeInv = 40;
	...
	if(swGetEepromFanWorkMode() == cFanSilentMode)
	{
		if(g_wVAType == cb15KVAModel)
		{
			...
			{
				wTempDerateRangeTx = ((wTempDegreeTxTemp - (wTxtTempDerate15KVA - 100))/10*96)>>6;
			}
			else if(wTempDegreeTxTemp <= 1300)
			{
				wTempDerateRangeTx = (swGetEepromOutputVolt() - 2000)/10 + (wTempDegreeTxTemp - 1200)/5;
			}
			else if(wTempDegreeTxTemp > 1300)
			{
				wTempDerateRangeTx = 60;
		...
		{
			//变压器温度降额
			if(wTempDegreeTxTemp < (wTxtTempDerate12KVA - 100))
			{
				wTempDerateRangeTx = 0;
			}
			else if(wTempDegreeTxTemp <= 1200)
			{
				wTempDerateRangeTx = ((wTempDegreeTxTemp - (wTxtTempDerate12KVA - 100))/10*96)>>6;
			}
			else if(wTempDegreeTxTemp <= 1300)
			{
				wTempDerateRangeTx = (swGetEepromOutputVolt() - 2000)/10 + (wTempDegreeTxTemp - 1200)/5;
			...
			//Inv温度降额
	        if(wTempDegreInvTemp < (wInvTempDerate12KVA - 80))
	        {
	            wTempDerateRangeInv = 0;
	        }
	        else if(wTempDegreInvTemp <= 700)
	        {
	            wTempDerateRangeInv = (wTempDegreInvTemp - (wInvTempDerate12KVA - 80))/5;
	        }
	        else if(wTempDegreInvTemp > 700)
	        {
	            wTempDerateRangeInv = 40;
			...
			//内环温降额
	        if(wTempDegreInternalTemp < (wInternalTempDerate15KVA - 80))
	        {
	            wTempDerateRangeInternal = 0;
	        }
	        else if(wTempDegreInternalTemp <= 690)
	        {
	            wTempDerateRangeInternal = ((wTempDegreInternalTemp - (wInternalTempDerate15KVA - 80)) * 39) >> 7;
	        }
	        else if(wTempDegreInternalTemp > 690)
	        {
	            wTempDerateRangeInternal = 40;
		...
		else if(g_wVAType == cb10KVAModel)
		{
			...
			{
				wTempDerateRangeTx = ((wTempDegreeTxTemp - (wTxtTempDerate10KVA - 100))/10*96)>>6;
			}
			else if(wTempDegreeTxTemp <= 1300)
			{
				wTempDerateRangeTx = (swGetEepromOutputVolt() - 2000)/10 + (wTempDegreeTxTemp - 1200)/5;
		...
		{
			//变压器温度降额
			if(wTempDegreeTxTemp < (wTxtTempDerate8KVA - 100))
			{
				wTempDerateRangeTx = 0;
			}
			else if(wTempDegreeTxTemp <= 1200)
			{
				wTempDerateRangeTx = ((wTempDegreeTxTemp - (wTxtTempDerate8KVA - 100))/10*96)>>6;
			}
			else if(wTempDegreeTxTemp <= 1300)
			{
				wTempDerateRangeTx = (swGetEepromOutputVolt() - 2000)/10 + (wTempDegreeTxTemp - 1200)/5;
			...
			//Inv温度降额
	        if(wTempDegreInvTemp < (wInvTempDerate8KVA - 80))
	        {
	            wTempDerateRangeInv = 0;
	        }
	        else if(wTempDegreInvTemp <= 700)
	        {
	            wTempDerateRangeInv = (wTempDegreInvTemp - (wInvTempDerate8KVA - 80))/5;
	        }
	        else if(wTempDegreInvTemp > 700)
	        {
	            wTempDerateRangeInv = 40;
...
void sAvrPvTempDeratingChk(INT16U bFilter)
{
	...
	static INT16S wPvTempChgCurrDerateRange4 = 150;//PV输出电流降额 8k
	...
	{
		if(((swGetPvOutCurrNew()) > 1350) && (!ubPVCurrDerateFlag))//超过0.9倍
		{
			uwPVCurrDeratecnt++;
		}
		else if((swGetPvOutCurrNew() < 1350) && (uwPVCurrDeratecnt > 0))
		{
			uwPVCurrDeratecnt--;
		}
	}
	...
	if(udwPVCurrDeratecnt1 == 90000)//30min,90000后再进行判断
	{
		...
		{
			if(swGetPvOutCurrNew() > 1350)
			{
				wPvTempChgCurrDerateRange4 = 127;//降低到最大的0.85倍
			}
		}
	}
	...
	if((udwPVCurrDeratecnt1 == 0) && (ubPVCurrDerateForwardCntFlag))
	{
		...
		wPvTempChgCurrDerateRange4 = 150;
	}
	//性能模式下降额
	if(swGetEepromFanWorkMode() == cFanPerformanceMode)
	{
...
//		}
		...
		else if(g_wVAType == cb12KVAModel)  //12K
		{
			//Pv散热器降额判断
				if(1 == fProtection.PvOverDerate)
				{
					if(sbUnderLevelChk(wTempDegreePv, wPvTempDerateBack12KVA, bFilter, &bPvTempOvDerate) == true)
					{
						fProtection.PvOverDerate = 0;
					}
				}
				...
				{
					if(sbOverLevelChk(wTempDegreePv, wPvTempDerate12KVA, bFilter, &bPvTempOvDerate) == true)
					{
						fProtection.PvOverDerate = 1;
					}
				}
				...
				if(fProtection.PvOverDerate)
				{
					wPvTempChgCurrDerateRange = 240 - (wTempDegreePv - 65) * 5;//一度降5A
				}
				else
				{
		...
		else
		{
			...
			if(1 == fProtection.PvOverDerate)
			{
				if(sbUnderLevelChk(wTempDegreePv, wPvTempDerateBack8KVA, bFilter, &bPvTempOvDerate) == true)
				{
					fProtection.PvOverDerate = 0;
				}
			}
			...
			{
				if(sbOverLevelChk(wTempDegreePv, wPvTempDerate8KVA, bFilter, &bPvTempOvDerate) == true)
				{
					fProtection.PvOverDerate = 1;
				}
			}
			...
			if(fProtection.PvOverDerate)
			{
				wPvTempChgCurrDerateRange = 150 - (wTempDegreePv - 60) * 4;//一度降4A
			}
			else
			{
				wPvTempChgCurrDerateRange = 150;
			}
			
			//内环境温度降额
			if(bInternalTempPvDerateFlag)
			{
				wPvTempChgCurrDerateRange1 = 150 - (wTempDegreeInternal -670) * 2;
			}
			else
			{
				wPvTempChgCurrDerateRange1 = 150;
			}
		}
	}
	...
	if(swGetEepromFanWorkMode() == cFanSilentMode)
	{
		...
		else if(g_wVAType == cb12KVAModel)  //12K
		{
			...
			if(1 == bPvTempPvDerateFlag1)
			{
				if(sbUnderLevelChk(wTempDegreePv, wPvTempDerateBack12KVA - 8, bFilter, &bPvTempOvDerate2) == true)
				{
					bPvTempPvDerateFlag1 = 0;
				}
			}
			...
			{
				if(sbOverLevelChk(wTempDegreePv, wPvTempDerate12KVA - 8, bFilter, &bPvTempOvDerate2) == true)
				{
					bPvTempPvDerateFlag1 = 1;
				}
			}
			...
			if(1 == bInternalTempPvDerateFlag1)
			{
				if(sbUnderLevelChk(wTempDegreeInternal, wPvCurInternalTempDerateBack15KVA - 80, bFilter, &bPvTempOvDerate3) == true)
				{
					bInternalTempPvDerateFlag1 = 0;
				}
			}
			...
			{
				if(sbOverLevelChk(wTempDegreeInternal, wPvCurInternalTempDerate15KVA - 80, bFilter, &bPvTempOvDerate3) == true)
				{
					bInternalTempPvDerateFlag1 = 1;
				}
			}
		...
		else
		{
			...
			if(1 == bPvTempPvDerateFlag1)
			{
				if(sbUnderLevelChk(wTempDegreePv, wPvTempDerateBack8KVA - 8, bFilter, &bPvTempOvDerate2) == true)
				{
					bPvTempPvDerateFlag1 = 0;
				}
			}
			...
			{
				if(sbOverLevelChk(wTempDegreePv, wPvTempDerate8KVA - 8, bFilter, &bPvTempOvDerate2) == true)
				{
					bPvTempPvDerateFlag1 = 1;
				}
			}
		...
		else if(g_wVAType == cb12KVAModel)  //12K
		{
			...
			if(bPvTempPvDerateFlag1)
			{
				wPvTempChgCurrDerateRange = 240 - (wTempDegreePv - 57) * 5;
			}
			else
			{
				wPvTempChgCurrDerateRange = 240;
			...
			if(bInternalTempPvDerateFlag1)
			{
				wPvTempChgCurrDerateRange1 = 240 - (wTempDegreeInternal - 590) * 2;
			}
			else
			{
				wPvTempChgCurrDerateRange1 = 240;
		...
		else
		{
			...
			if(bPvTempPvDerateFlag1)
			{
				wPvTempChgCurrDerateRange = 150 - (wTempDegreePv - 52) * 4;
			}
			else
			{
				wPvTempChgCurrDerateRange = 150;
			...
			if(bInternalTempPvDerateFlag1)
			{
				wPvTempChgCurrDerateRange1 = 150 - (wTempDegreeInternal - 590) * 2;
			}
			else
			{
				wPvTempChgCurrDerateRange1 = 150;
	...
	else if((g_wVAType == cb12KVAModel)||(g_wVAType == cb10KVAModel))
	{
	...
	{
		if(wPvTempChgCurrDerateRange > wPvTempChgCurrDerateRange4)
		{
			wPvTempChgCurrDerateRange = wPvTempChgCurrDerateRange4;
		}
	}
...
//	}
	...
	{
		if(g_wPvChgCurrDerateRange > 150)
		{
			g_wPvChgCurrDerateRange = 150;
		}
		else if(g_wPvChgCurrDerateRange < 90)
		{
			g_wPvChgCurrDerateRange = 90;
		}
...
INT16S sAvrInvTempDeratingChk(INT16U bFilter)
{
	...
	{
		wTempDerateRangeInv = 50;//散热器温度逆变电流降额
		wTempDerateRangeInv1 = 50;//内环境温度逆变电流降额
		wTempDerateRangeInv2 = 50;//变压器温度逆变电流降额
	}
	else if(g_wVAType == cb10KVAModel)
	{
		wTempDerateRangeInv = 42;//散热器温度逆变电流降额
		wTempDerateRangeInv1 = 42;//内环境温度逆变电流降额
		wTempDerateRangeInv2 = 42;//变压器温度逆变电流降额
	}
	else if(g_wVAType == cb8000VAModel)
	{
		wTempDerateRangeInv = 33;//散热器温度逆变电流降额
		wTempDerateRangeInv1 = 33;//内环境温度逆变电流降额
		wTempDerateRangeInv2 = 33;//变压器温度逆变电流降额
	}
	...
	//	}
		if(g_wVAType == cb15KVAModel)
		{
	        ...
	        if(wTempDegreeInv > cInvCurHSTempDerate15KVA)
	        {
				...
				if(wTempDerateRangeInv < 0)
				{
					wTempDerateRangeInv = 1;
				}
	        }
	        else
	        {
	        ...
	        if(wTempDegreeInternal > 670)
	        {
				...
				if(wTempDerateRangeInv1 < 0)
				{
					wTempDerateRangeInv1 = 1;
				}
	        }
	        else
	        {
	        ...
	        if(wTempDegreeTx > 110)
	        {
				...
				if(wTempDerateRangeInv2 < 0)
				{
					wTempDerateRangeInv2 = 1;
				}
	        }
	        else
	        {
		...
		{
			//散热器降额计算
	        if(wTempDegreeInv > cInvCurHSTempDerate12KVA)
	        {
	            wTempDerateRangeInv = 50 - (wTempDegreeInv - cInvCurHSTempDerate12KVA) * 15/10;
				if(wTempDerateRangeInv < 0)
				{
					wTempDerateRangeInv = 1;
				}
	        ...
	        {
	            wTempDerateRangeInv = 50;
	        }
	        //内环境温度降额计算
	        if(wTempDegreeInternal > 670)
	        {
	            wTempDerateRangeInv1 = 50 - (wTempDegreeInternal - 670)*26/100;
				if(wTempDerateRangeInv1 < 0)
				{
					wTempDerateRangeInv1 = 1;
				}
	        ...
	        {
	            wTempDerateRangeInv1 = 50;
	        }
	        //变压器温度降额计算
	        if(wTempDegreeTx > 110)
	        {
	            wTempDerateRangeInv2 = 50 - ((wTempDegreeTx - 110) * 160 >>6);
				if(wTempDerateRangeInv2 < 0)
				{
					wTempDerateRangeInv2 = 1;
				}
		...
		else if(g_wVAType == cb10KVAModel)
		{
	        ...
	        {
	            wTempDerateRangeInv = 42 - (wTempDegreeInv - cInvCurHSTempDerate10KVA) * 4;
				if(wTempDerateRangeInv < 0)
				{
					wTempDerateRangeInv = 1;
				}
	        ...
	        {
	            wTempDerateRangeInv = 42;
	        }
	        //内环境温度降额计算
	        if(wTempDegreeInternal > 670)
	        {
	            //wTempDerateRangeInv1 = 65 - (((wTempDegreeInternal - 620) * 26)>>6);
	            wTempDerateRangeInv1 = 42 - (wTempDegreeInternal - 670)*40/100;
				if(wTempDerateRangeInv1 < 0)
				{
					wTempDerateRangeInv1 = 1;
				}
	        ...
	        {
	            wTempDerateRangeInv1 = 42;
	        }
	        //变压器温度降额计算
	        if(wTempDegreeTx > 115)
	        {
	            wTempDerateRangeInv2 = 42 - (wTempDegreeTx - 115) * 28/10;
				if(wTempDerateRangeInv2 < 0)
				{
					wTempDerateRangeInv2 = 1;
				}
		...
		else//8k
		{
			//散热器降额计算
	        //if(fProtection.InvCurrHSDerate)
			if(wTempDegreeInv > cInvCurHSTempDerate8KVA)
	        {
	            wTempDerateRangeInv = 33 - (wTempDegreeInv - cInvCurHSTempDerate8KVA);//一度1A
				if(wTempDerateRangeInv < 0)
				{
					wTempDerateRangeInv = 1;
				}
	        ...
	        {
	            wTempDerateRangeInv = 33;
	        }
	        //内环境温度降额计算
	        if(wTempDegreeInternal > 670)
	        {
	            //wTempDerateRangeInv1 = 65 - (((wTempDegreeInternal - 620) * 26)>>6);
	            wTempDerateRangeInv1 = 33 - (wTempDegreeInternal - 670)*16/100;//一度1.6A
				if(wTempDerateRangeInv1 < 0)
				{
					wTempDerateRangeInv1 = 1;
				}
	        ...
	        {
	            wTempDerateRangeInv1 = 33;
	        }
	        //变压器温度降额计算
	        if(wTempDegreeTx > 110)
	        {
	            wTempDerateRangeInv2 = 33 - (((wTempDegreeTx - 110)*12)>>3);//一度降1.5A
	            if(wTempDerateRangeInv2 < 0)
				{
					wTempDerateRangeInv2 = 1;
				}
...
//
		...
		{
			//散热器降额计算
	        if(wTempDegreeInv > (cInvCurHSTempDerate12KVA - 8))
	        {
	            wTempDerateRangeInv = 50 - (wTempDegreeInv - (cInvCurHSTempDerate12KVA - 8)) * 15/10;
				if(wTempDerateRangeInv < 0)
				{
					wTempDerateRangeInv = 1;
				}
	        ...
	        //内环境温度降额计算
	        if(wTempDegreeInternal  > 590)
	        {
	            wTempDerateRangeInv1 = 50 - (wTempDegreeInternal - 590)*26/100;
				if(wTempDerateRangeInv1 < 0)
				{
					wTempDerateRangeInv1 = 1;
				}
	        ...
	        if(wTempDegreeTx > 100)
	        {
	            wTempDerateRangeInv2 = 50 - ((wTempDegreeTx - 100) * 160 >>6);
				if(wTempDerateRangeInv2 < 0)
				{
					wTempDerateRangeInv2 = 1;
				}
		...
		else if(g_wVAType == cb10KVAModel)
		{
	        ...
	        {
	            wTempDerateRangeInv = 42 - (wTempDegreeInv - (cInvCurHSTempDerate10KVA - 10)) * 4;
				if(wTempDerateRangeInv < 0)
				{
					wTempDerateRangeInv = 1;
				}
	        ...
	        {
	            //wTempDerateRangeInv1 = 65 - ((wTempDegreeInternal - 520) * 26)>>6;
	            wTempDerateRangeInv1 = 42 - (wTempDegreeInternal - 570)*40/100;
				if(wTempDerateRangeInv1 < 0)
				{
					wTempDerateRangeInv1 = 1;
				}
	        ...
	        {
	            wTempDerateRangeInv2 = 42 - (wTempDegreeTx - 105)* 28/10;
				if(wTempDerateRangeInv2 < 0)
				{
					wTempDerateRangeInv2 = 1;
				}
		...
		else//8k
		{
	        ...
	        if(wTempDegreeInv > (cInvCurHSTempDerate10KVA - 10))
	        {
	            wTempDerateRangeInv = 33 - (wTempDegreeInv - (cInvCurHSTempDerate10KVA - 10)) * 4;
				if(wTempDerateRangeInv < 0)
				{
					wTempDerateRangeInv = 1;
				}
	        ...
	        {
	            //wTempDerateRangeInv1 = 65 - ((wTempDegreeInternal - 520) * 26)>>6;
	            wTempDerateRangeInv1 = 33 - (wTempDegreeInternal - 570)*40/100;
				if(wTempDerateRangeInv1 < 0)
				{
					wTempDerateRangeInv1 = 1;
				}
	        ...
	        if(wTempDegreeTx > 100)
	        {
	            wTempDerateRangeInv2 = 33 - (((wTempDegreeTx - 100)*12)>>3);
				if(wTempDerateRangeInv2 < 0)
				{
					wTempDerateRangeInv2 = 1;
				}
...
//逆变电压直流分量检测
void sInvVoltDcChk(INT16U bFilter)
{
	static INT16U wInvVoltDcSampOvCnt = 0;
	if(1 == wInvVoltDcSampOvFaultFlag)
	{
		if(sbUnderLevelChk(abs(wInvVoltDcSamp), 200, 300, &wInvVoltDcSampOvCnt) == true)
		{
			wInvVoltDcSampOvFaultFlag = 0;
		}
	}
	else
	{
		if(sbOverLevelChk(abs(wInvVoltDcSamp), 200, bFilter, &wInvVoltDcSampOvCnt) == true)
		{
			wInvVoltDcSampOvFaultFlag = 1;
		}
	}
}
...
void sSetTempDegreeInvMax(void)
{
	...
	//TempSampData[INV1_TEMP].wTempDegree = TempSampData[INV1_TEMP].wTempDegree - 2;
	if(TempSampData[INV1_TEMP].wTempDegree < TempSampData[INV2_TEMP].wTempDegree)
	{
		wTempInvMax = TempSampData[INV2_TEMP].wTempDegree;
	}
	else
	{
		wTempInvMax = TempSampData[INV1_TEMP].wTempDegree;
	}
	OS_ENTER_CRITICAL();
...
void sSetTempDegreeTxtMax(void)
{
	...
	if(TempSampData[TX1_TEMP].wTempDegree < TempSampData[TX2_TEMP].wTempDegree)
	{
		wTempTxtMax = TempSampData[TX2_TEMP].wTempDegree;
	}
	else
	{
		wTempTxtMax = TempSampData[TX1_TEMP].wTempDegree;
	}
	OS_ENTER_CRITICAL();
...
void sSetTempDegreePvMax(void)
{
	...
	if(TempSampData[PV_TEMP].wTempDegree < TempSampData[LLC_LOW_TEMP].wTempDegree)
	{
		wTempLlcMax = TempSampData[LLC_LOW_TEMP].wTempDegree;
	}
	else
	{
		wTempLlcMax = TempSampData[PV_TEMP].wTempDegree;
	}
	OS_ENTER_CRITICAL();
...
void sSetTempDegreeBatMax(void)
{
	...
	if(TempSampData[BAT1_TEMP].wTempDegree < TempSampData[BAT2_TEMP].wTempDegree)
	{
		wTempBatMax = TempSampData[BAT2_TEMP].wTempDegree;
	}
	else
	{
		wTempBatMax = TempSampData[BAT1_TEMP].wTempDegree;
	}
	OS_ENTER_CRITICAL();
