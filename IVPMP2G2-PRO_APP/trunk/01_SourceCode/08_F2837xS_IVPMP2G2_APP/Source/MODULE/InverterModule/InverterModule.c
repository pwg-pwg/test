...
extern INT16S wOPSharePower;
extern INT16S wOPSharePowerCnt;
...
extern INT16U wInvVoltSampleCnt1Div8;
...
INT16U      wInvLCurrNew;
INT16U      wInvL1CurrNew;
INT16U      wInvL2CurrNew;
...
INT16S wVoltRmsLoopP = 0;
INT16S wVoltRmsLoopI = 0;
...
void    sInverterModuleInit(void)
{
    ...
    wInverterVoltSet = cVac230v;// 涓嶉渶瑕佹敼
...
{
    wInvLCurrNew = (INT16U)(((INT32U)wRInverterInvLCurrRMS*(INT32U)wRInverterOPCurrAdj) >> 11);
}

void    sInvL1CurrAdj(INT16U wInvCurrRMS)
{
    wInvL1CurrNew = wInvCurrRMS;
}

void    sInvL2CurrAdj(INT16U wInvCurrRMS)
{
    wInvL2CurrNew = wInvCurrRMS;
}
...
void    sRInverterVoltFilter(INT16U wFilter)
{
    ...
    {
        wRInverterVolt = (wRInverterVoltNew + wRInverterVolt * 3) >> 2;
    }
}
...
void sRVoltRegu(INT16U wRTrackVolt, INT16U wRKVoltrack1, INT16U wRKVoltrack2)
{
    ...
    static  INT16S  wInvVoltError0=0;
	//INT16S			temp;
    ...
    if(wInvVoltError>=100)
    {
        wInvVoltError=100;
    }
    else if(wInvVoltError<=-100)
    {
        wInvVoltError=-100;
		if(wVoltRmsLoopI > 0)
		{
    ...
    if((wInvVoltError < 40 && wInvVoltError > -40) && wParaMode != cMasterMode && !g_wPhaseMasterFlg)
    {
        wInvVoltError = 0;
		wVoltRmsLoopI = 0;
    }

    wVoltRmsLoopP=(INT16S)wRKVoltrack1*wInvVoltError-(INT16S)wRKVoltrack2*wInvVoltError0;
	wVoltRmsLoopI += (wVoltRmsLoopP>>1);
	if(wVoltRmsLoopI > 6000)
	{
		wVoltRmsLoopI = 6000;
	}
	else if(wVoltRmsLoopI < -4000)
	{
		wVoltRmsLoopI = -4000;
	}
	//wVoltRmsLoopI = 0;
	if(g_uwPowerSaveModeFlag&&(g_uwPowerSave1sCnt < ePowerSaveModeSoftTime))//节能模式且在软起
    {
		;
	}
	else
	{
		if(hoInv1Relay == 1)
		{
			wRNewSinAmp = wRNewSinAmpTemp + swGetInvVoltCompValueReal();
		}
		else
		{
			wRNewSinAmp = wRNewSinAmpTemp + wVoltRmsLoopI;
		}
	}
...
{
    if (wRNewSinAmp>wSinAmpLimitMax)
    {
        wRNewSinAmp=wSinAmpLimitMax;
    }
    else if (wRNewSinAmp<wSinAmpLimitMin)
    {
        wRNewSinAmp=wSinAmpLimitMin;
    }
...
void    sInverterPhaseLock(INT16U wInvPeriodCnt,INV_TD *pInvTd,INT16U wRefPeriodCnt,INT16U wStep)
{
    ...
    // if(sbGetEepromModeSelect() == 0 && wParaMode == cMasterMode)     //APL
    // {
    //     OS_ENTER_CRITICAL();
    //     fInv.PhaseLocked = 1;
    //     OS_EXIT_CRITICAL();
...
INT16U  swInverterStepCal(INT16U    factor,INT16U pointer)
{
    ...
    if(wInverterPeriodCntNew > wInverterPeriodCntHigh+100)//40Hz
    {
        wInverterPeriodCntNew = wInverterPeriodCntHigh+100;
    }
    else if(wInverterPeriodCntNew <wInverterPeriodCntLow-100)//65Hz
    {
        wInverterPeriodCntNew = wInverterPeriodCntLow-100;
    }
    //wInverterPeriodCntNew = 30000;
...
//  wNewStepResidueTemp=((INT32U)wInverterPeriodCntNew*factor)-((INT32U)wInverterStepTemp*pointer);
    ...
    wTemp = ((INT32U)cPeriod50Hz * 16384 + (wInverterPeriodCntNew >> 1)) / wInverterPeriodCntNew;
    OS_ENTER_CRITICAL();
    wRSinPionterDeltaQ15 = wTemp;
    OS_EXIT_CRITICAL();


    wPointerMaxTemp = (INT16U)(((INT32U)((INT32U)wInverterPeriodCntNew * factor * 10 / cINV_PWM_PERIOD) + 5) /10);
    dwInverterStepTemp = (INT32U)wInverterPeriodCntNew * factor * pointer / wPointerMaxTemp;
    ...
    gi_wPLLPoint1Div4 = wPointerMaxTemp >> 2;
    gi_wPLLPoint3Div4 = ((wPointerMaxTemp*3) >> 2);
    gi_wPLLPoint1Div8 = (wPointerMaxTemp >> 3)-1;
    if(gi_wPLLPoint1Div8 >wInvVoltSampleCnt1Div8)
    {
        gi_wPLLPoint1Div8 =wInvVoltSampleCnt1Div8;
    }
...
INT8U   sbRInverterVoltLowChk(INT8U bFilter)
{
    static INT8U    bRInterterVoltLowChkCnt = 0;
     ...
     if(sbGetInvVoltSoftFinishSts()==true && g_uwWorkMode == cBatteryMode && bLowPowerflag == false)
    {
...
INT8U   sbRInverterShortChk(INT16U wRInverterCurrentNew, INT8U bFilter)
{
    ...
    if((g_uwWorkMode == cBatteryMode)||((g_uwWorkMode == cStandbyMode )&&(wFBControlStatus == cFBInverterControl)))
    {
        if(((wRInverterVoltNew<wInverterPVoltShort)&&(wRInverterCurrentNew > wInverterPCurrShort))||\
            ((swGetOPL1VoltNEW()<(wInverterPVoltShort >> 1))&&(sbGetInvL1CurrNew() > wInverterPCurrShort))||\
            ((swGetOPL2VoltNEW()<(wInverterPVoltShort >> 1))&&(sbGetInvL2CurrNew() > wInverterPCurrShort)))//50V  20A->10A
        {
            fInv.RShort=1;
...
/************************************************************************************
Function name:  sRInverterNegPowChk                                                 *
parameters:     wActivePow:     active power between R inverter voltage and output  *
                                current                                             *
                limit1:         negtive power limit1                                *
                limit2:         negtive power limit2                                *
                bFilter:        check time                                          *
returns:        true:           negtive power fault                                 *
                false:          negtive power normal                                *
output:         none                                                                *
description:    when the output active power is negtive, if the active power value  *
                is under limit1,then return true,negtive power fault                *
                if active power value is under limit2,increase counter,if counter   *
                is greater than filter return true,negtive power fault              *
*************************************************************************************/
INT8U   sRInverterNegPowChk(INT16S wActivePow,INT16S limit1,INT16S limit2,INT8U bFilter)
{
    static INT8U    bRNegPowChkCnt=0;

    //if(hoInv1Relay != true || (swGetEepromOutputMode() == cOP_Single)|| g_uwWorkMode != cBatteryMode)
    if((swGetEepromOutputMode() == cOP_Single)|| g_uwWorkMode != cBatteryMode)
    {
        bRNegPowChkCnt = 0;
        return(false);
    }

	if((wActivePow < -5000) && (!fInv.RShort))
	{
		bRNegPowChkCnt = 0;
        return(true);
	}
    else if((wActivePow < limit1) && (!fInv.RShort))
    {
        bRNegPowChkCnt ++;
		if(bRNegPowChkCnt > 1)
		{
    ...
    else if((wActivePow < limit2) && (!fInv.RShort))
    {
        bRNegPowChkCnt ++;
...
{
    static  INT16U   bRInvVChkCnt = 0;

    if((g_wVAType != cb5000VAModel) && (g_wVAType != cb7500VAModel) \
        && (g_wVAType != cb10KVAModel))
    {
        bRInvVChkCnt = 0;
        fInv.RInvVoltHiFanStop = 0;
    }
    else if(fInv.RInvVoltHiFanStop)
    {
...
INT16U swGetInvL2CurrNew(void)
{
    return(wInvL2CurrNew);
}
...
INT16S sbGetInvVoltRmsLoopI(void)
{
    return(wVoltRmsLoopI);
}

INT16S sbGetInvL1CurrNew(void)
{
    return(wInvL1CurrNew);
}

INT16S sbGetInvL2CurrNew(void)
{
    return(wInvL2CurrNew);
}
...
void    sSetRNewSinAmpTemp(INT16U wValue)
{
    wRNewSinAmpTemp = wValue;
}
