...
INT16U	wGenCurrNew;
INT16U	wGenCurr;
INT16U	wGenL1CurrNew;
INT16U	wGenL1Curr;
INT16U	wGenL2CurrNew;
INT16U	wGenL2Curr;
...
INT32S dwGenWattNew;
INT32S dwGenVANew;
INT32S dwGenL1WattNew;
INT32S dwGenL1VANew;
INT32S dwGenL2WattNew;
INT32S dwGenL2VANew;
...
void sGenCurrFilter(INT16U wFilter)
{
	...
	{
		wGenCurr = (wGenCurrNew + wGenCurr * 3) >> 2;
	}
}

void sGenL1CurrAdj(INT16U wGenRms)
{
	wGenL1CurrNew = wGenRms;
}

void sGenL1CurrFilter(INT16U wFilter)
{
	if(abs(wGenL1CurrNew - wGenL1Curr) > wFilter)
	{
		wGenL1Curr = wGenL1CurrNew;
	}
	else
	{
		wGenL1Curr = (wGenL1CurrNew + wGenL1Curr * 3) >> 2;
	}
}


void sGenL2CurrAdj(INT16U wGenRms)
{
	wGenL2CurrNew = wGenRms;
}

void sGenL2CurrFilter(INT16U wFilter)
{
	if(abs(wGenL2CurrNew - wGenL2Curr) > wFilter)
	{
		wGenL2Curr = wGenL2CurrNew;
	}
	else
	{
		wGenL2Curr = (wGenL2CurrNew + wGenL2Curr * 3) >> 2;
	}
}
...
void sGenFreqFilter(INT16U wFilter)
{
	...
	{
		wGenFreq = (wGenFreqNew + wGenFreq * 3) >> 2;
	}
}	
/************************************************************************************
...
void sGenL1WattCal(INT32S dwGenWattNewTemp)
{
	dwGenL1WattNew = (dwGenWattNewTemp + dwGenL1WattNew * 3) >> 2;
	
}
void sGenL2WattCal(INT32S dwGenWattNewTemp)
{
	dwGenL2WattNew = (dwGenWattNewTemp + dwGenL2WattNew * 3) >> 2;
...
//	
//}
//void sGenL1VACal(INT16U wRVoltRmsNew)
//{
//	dwGenL1VANew=(INT32U)(((INT32U)wRVoltRmsNew * (INT32U)wGenCurrNew) /100);
//	
//}
//void sGenL2VACal(INT16U wRVoltRmsNew)
//{
//	dwGenL2VANew=(INT32U)(((INT32U)wRVoltRmsNew * (INT32U)wGenCurrNew) /100);
//	
//}
void sGenVACal(INT16U wRVoltRmsNew)
{
	dwGenVANew=(INT32U)wRVoltRmsNew;
	
}
void sGenL1VACal(INT16U wRVoltRmsNew)
{
	dwGenL1VANew=(INT32U)wRVoltRmsNew;
	
}
void sGenL2VACal(INT16U wRVoltRmsNew)
{
	dwGenL2VANew=(INT32U)wRVoltRmsNew;
...
void sGenPortWorkModeUpdate(void)//10ms
{
	...
	wSmartLoadStas = 0;
	INT16U uwTOUGenExitChargeVolt = 0;
	...
	else//发电机输入
	{
		...
		if(wTOUECOEnFlg)
		{	
			if(wTOUGenChargeEnFlg && (g_LineModeJoinInWay == cLineModeJoinInByGen))
			{
				if(wTOUChargeVolt > swGetEepromOnGenBatAutoExitChargeVolt())
				{
					wTOUChargeVolt = swGetEepromOnGenBatAutoExitChargeVolt();
				}
				if(wTOUChargeSOC > swGetEepromOnGenBatAutoExitChargeSOC())
				{
					wTOUChargeSOC = swGetEepromOnGenBatAutoExitChargeSOC();
				}
			}
			uwTOUGenExitChargeVolt = wTOUChargeVolt+15;//1.5V

			if(uwTOUGenExitChargeVolt > 600)
			{
				uwTOUGenExitChargeVolt = 600;
			}
			if(wTOUGenChargeEnFlg&&(!sGetGenInputChargeEn())&&\
			...
			else if((!wTOUGenChargeEnFlg)||(wTOUGenChargeEnFlg &&\
				((((swGetEepromBatMode() != cLiBMode)&&(swGetBatAvgVoltNew() >= uwTOUGenExitChargeVolt)))||\
				((swGetEepromBatMode() == cLiBMode)&&(fBMSConnectFlg)&&(mBMSBatSOC >= wTOUChargeSOC))||\
				((swGetEepromBatMode() == cLiBMode)&&(!fBMSConnectFlg)&&(swGetBatAvgVoltNew() >= uwTOUGenExitChargeVolt)))))
			{
				uwGenStartChargeCnt = 0;
...
INT16U	swGetGenL1CurrNew(void)
{
	return(wGenL1CurrNew);
}

INT16U	swGetGenL2CurrNew(void)
{
	return(wGenL2CurrNew);
}
...
INT16U	swGetGenL1Curr(void)
{
	return(wGenL1Curr);
}
INT16U	swGetGenL2Curr(void)
{
	return(wGenL2Curr);
}
/************************************************************************************
...
INT32S sbGetGenL1Watt(void)
{
	return(dwGenL1WattNew);
}

INT32S sbGetGenL1VA(void)
{
	return(dwGenL1VANew);
}

INT32S sbGetGenL2Watt(void)
{
	return(dwGenL2WattNew);
}

INT32S sbGetGenL2VA(void)
{
	return(dwGenL2VANew);
}
