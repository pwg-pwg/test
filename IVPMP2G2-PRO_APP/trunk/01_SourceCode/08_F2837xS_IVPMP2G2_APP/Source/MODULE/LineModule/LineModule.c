...
INT16U  wRLineCurrNew;
INT16U  wRLineCurr;
INT16U  wRLineL1CurrNew;
INT16U  wRLineL1Curr;
INT16U  wRLineL2CurrNew;
INT16U  wRLineL2Curr;
INT16U  wLineL1VoltNew;
INT16U  wLineL2VoltNew;
...
INT16U  wLineWattAdj = 2048;
INT32S dwLineWattNew;
INT32S dwLineVANew;
INT32S dwLineL1WattNew;
INT32S dwLineL1VANew;
INT32S dwLineL2WattNew;
INT32S dwLineL2VANew;
...
void    sLineModuleInit(void)
{
    ...
    wLineVoltHLoss = cVac270v;
    wLineVoltHBack = cVac260v;
    wLineFreqHLoss = cFreq65Hz;
    wLineFreqLLoss = cFreq45Hz;
    wLineFreqHBack = cFreq63Hz;
    wLineFreqLBack = cFreq47Hz;
...
void    sLineL1VoltAdj(INT16U wVoltRMS)
{
    wLineL1VoltNew = (wVoltRMS + wLineL1VoltNew * 3)>>2;
}

void    sLineL2VoltAdj(INT16U wVoltRMS)
{
    wLineL2VoltNew = (wVoltRMS + wLineL2VoltNew * 3)>>2;
}
...
void    sRLineVoltFilter(INT16U wFilter)
{
    ...
    {
        wRLineVolt = (wRLineVoltNew + wRLineVolt * 3) >> 2;
    }
}

void    sRLineCurrAdj(INT16U wRLineRms)
{
    wRLineCurrNew = (INT16U)(((INT32U)wRLineRms * (INT32U)wRLineCurrAdj) >> 11);
}

void    sRLineCurrFilter(INT16U wFilter)
{
    if(abs(wRLineCurrNew - wRLineCurr) > wFilter)
    {
        wRLineCurr = wRLineCurrNew;
    }
    else
    {
        wRLineCurr = (wRLineCurrNew + wRLineCurr * 3) >> 2;
    }
}

void    sRLineL1CurrAdj(INT16U wRLineRms)
{
    wRLineL1CurrNew = wRLineRms;
}

void    sRLineL1CurrFilter(INT16U wFilter)
{
    if(abs(wRLineL1CurrNew - wRLineL1Curr) > wFilter)
    {
        wRLineL1Curr = wRLineL1CurrNew;
    }
    else
    {
        wRLineL1Curr = (wRLineL1CurrNew + wRLineL1Curr * 3) >> 2;
    }
}

void    sRLineL2CurrAdj(INT16U wRLineRms)
{
    wRLineL2CurrNew = wRLineRms;
}

void    sRLineL2CurrFilter(INT16U wFilter)
{
    if(abs(wRLineL2CurrNew - wRLineL2Curr) > wFilter)
    {
        wRLineL2Curr = wRLineL2CurrNew;
    }
    else
    {
        wRLineL2Curr = (wRLineL2CurrNew + wRLineL2Curr * 3) >> 2;
    }
}
...
void    sRLineVoltChk(INT8U bFilter)
{
	...
	static  INT8U   bRLineOVChkCnt = 0;
	static  INT8U   bRLineOVRecChkCnt = 0;
	static  INT8U   bRLineUVChkCnt = 0;
	static  INT8U   bRLineUVRecChkCnt = 0;
...
//    }
	if(fLine.VoltLoss)
    {
        if(sbInRangeChk(wRLineVoltNew,swGetEepromGridOVValue()-100,swGetEepromGridUVValue()+100,bFilter,&bRLineVChkCnt) == true)
        {
            fLine.VoltLoss = 0;
        }
    }
    ...
    {
        if(sbOutRangeChk(wRLineVoltNew,swGetEepromGridOVValue(),swGetEepromGridUVValue(),bFilter,&bRLineVChkCnt) == true)
        {
            fLine.VoltLoss = 1;
        }
    }
	if(wRLineVoltNew > swGetEepromGridOVValue())
	{
		if(++bRLineOVChkCnt >= 5)
		{
			bRLineOVChkCnt = 0;
	...
	else if(wRLineVoltNew < swGetEepromGridOVValue()-100)
	{
		if(++bRLineOVRecChkCnt >= 5)
		{
			bRLineOVRecChkCnt = 0;
			fLine.VoltOver = 0;
		}
	...
	if(wRLineVoltNew < swGetEepromGridUVValue())
	{
		if(++bRLineUVChkCnt >= 60)//60个工频周期
		{
			bRLineUVChkCnt = 0;
			if(wRLineVoltNew > 500)//小于50V不管
			{
				fLine.VoltUnder = 1;
			}
		}
	...
	else if(wRLineVoltNew > swGetEepromGridUVValue()+100)
	{
		if(++bRLineUVRecChkCnt >= 5)
		{
			bRLineUVRecChkCnt = 0;
			fLine.VoltUnder = 0;
		}
...
{
    static  INT16U   bRLinesdChkCnt = 0;
    static  INT16U   bRLinesdChkCnt1 = 0;
...
void    sLineFreqFilter(INT16U wFilter)
{
    ...
    {
        wLineFreq = (wLineFreqNew + wLineFreq * 3) >> 2;
    }
}
/************************************************************************************
...
void    sLineFreqChk(INT8U bFilter)
{
	...
	static  INT8U  bRLineOFChkCnt = 0;
	static  INT8U  bRLineOFRecChkCnt = 0;
	static  INT8U  bRLineUFChkCnt = 0;
	static  INT8U  bRLineUFRecChkCnt = 0;
	...
	if(wLineFreqNew > wLineFreqHLoss)
	{
		if(++bRLineOFChkCnt >= 5)
		{
			bRLineOFChkCnt = 0;
	...
	else if(wLineFreqNew < wLineFreqHBack)
	{
		if(++bRLineOFRecChkCnt >= 5)
		{
			bRLineOFRecChkCnt = 0;
			fLine.FreqOver = 0;
		}
	...
	if(wLineFreqNew < wLineFreqLLoss)
	{
		if(++bRLineUFChkCnt >= 60)
		{
			bRLineUFChkCnt = 0;
			if(wRLineVolt > 500)
			{
	...
	else if(wLineFreqNew > wLineFreqLBack)
	{
		if(++bRLineUFRecChkCnt >= 5)
		{
			bRLineUFRecChkCnt = 0;
			fLine.FreqUnder = 0;
		}
...
*************************************************************************************/
INT16U LineLossState= 0;

void    sLineModuleUpdate(void)
{
    /*          voltage loss flag update        */
    if((!fLine.VoltLoss)&&(!fLine.VoltFastChkLoss))
    {
        OS_ENTER_CRITICAL();
        wRLineStatus &= ~cBitStatusVoltLoss;
        OS_EXIT_CRITICAL();
    }
    else
    {
    ...
    {
        OS_ENTER_CRITICAL();
        wRLineStatus &= ~cBitStatusVoltLongTimeLoss;
        OS_EXIT_CRITICAL();
    }
    else
    {
    ...
    {
        OS_ENTER_CRITICAL();
        wLineStatus |=cBitStatusVoltLoss;
        OS_EXIT_CRITICAL();
    }
    else
    {
    ...
    {
        OS_ENTER_CRITICAL();
        wLineStatus |=cBitStatusVoltLongTimeLoss;
        OS_EXIT_CRITICAL();
    }
    else
    {
    ...
    {
        OS_ENTER_CRITICAL();
        wRLineStatus &= ~cBitStatusFreqLoss;
        wLineStatus &=~cBitStatusFreqLoss;
        OS_EXIT_CRITICAL();
    }
    else
    {
    ...
    {
        OS_ENTER_CRITICAL();
        wRLineStatus &= ~cBitStatusWavLoss;
        OS_EXIT_CRITICAL();
    }
    else
    {
    ...
    //LineLossState = 0;
    if((!fLine.VoltLoss)&&(!fLine.FreqLoss)&&(!fLine.RWavLoss)&&(!fLine.RVoltLongTimeLoss)\
        &&(!fLine.OrderFault)&&(!fLine.VoltFastChkLoss)&&(!fLine.PhaseLoss))
    {
        OS_ENTER_CRITICAL();
        wRLineStatus &= ~cBitStatusLoss;
        OS_EXIT_CRITICAL();
    }
    else
    {
    ...
    if(!(wRLineStatus & cBitStatusLoss) && !fLine.RWavLoss)
    {
        OS_ENTER_CRITICAL();
        wLineStatus &= ~cBitStatusLoss;
        OS_EXIT_CRITICAL();
    }
    else
    {
        ...
        wLineStatus |= cBitStatusLoss;
        OS_EXIT_CRITICAL();
        if(wRLineStatus & cBitStatusLoss)
        {
            // LineLossState |= 0x80;
        }
        if(fLine.RWavLoss)
...
void sLinePeakFailChk(INT8U bFilter)
{
    ...
    static INT16U bHighVCnt = 0;
    static INT16U bHighLCnt = 0;
    ...
    //#warn 暂时改为30s
    if(wOrderChkTime < 1500)        // 1.5 min 9000
    {
        wOrderChkTime++;
        ...
        if((g_uwWorkMode == cBatteryMode)&&(!g_uwBatModeSteadyDelay)&&(!fLine.OrderFault))
        {
            wOrderChkTime =1500;//9000
        }
...
void sLineWattCal(INT32S dwLineWattNewTemp)
{
    dwLineWattNew = ( dwLineWattNewTemp * (INT32S)wLineWattAdj) >> 11;

}

//void sLineVACal(INT16U wRVoltRmsNew)
//{
//    dwLineVANew=(INT32U)(((INT32U)wRVoltRmsNew * (INT32U)wRLineCurrNew) /100);

//}
void sLineVACal(INT16U wRVoltRmsNew)
{
    dwLineVANew=(INT32U)wRVoltRmsNew;
}

void sLineL1WattCal(INT32S dwLineWattNewTemp)
{
    dwLineL1WattNew = (dwLineWattNewTemp + dwLineL1WattNew * 3) >> 2;

}

void sLineL1VACal(INT16U dwLineWattNewTemp)
{
    dwLineL1VANew=(INT32U)dwLineWattNewTemp;

}

void sLineL2WattCal(INT32S dwLineWattNewTemp)
{
    dwLineL2WattNew = (dwLineWattNewTemp + dwLineL2WattNew * 3) >> 2;

}

void sLineL2VACal(INT16U dwLineWattNewTemp)
{
    dwLineL2VANew=(INT32U)dwLineWattNewTemp;
...
INT16U  swGetLineL1VoltNew(void)
{
    return(wLineL1VoltNew);
}

INT16U  swGetLineL2VoltNew(void)
{
    return(wLineL2VoltNew);
}
...
*get line Curr new                                                              *
*************************************************************************************/
INT16U  swGetRLineCurrNew(void)
{
    return(wRLineCurrNew);
}

INT16U  swGetRLineL1CurrNew(void)
{
    return(wRLineL1CurrNew);
}

INT16U  swGetRLineL2CurrNew(void)
{
    return(wRLineL2CurrNew);
}
...
*get line voltage filter                                                            *
*************************************************************************************/
INT16U  swGetRLineCurr(void)
{
    return(wRLineCurr);
}
INT16U  swGetRLineL1Curr(void)
{
    return(wRLineL1Curr);
}
INT16U  swGetRLineL2Curr(void)
{
    return(wRLineL2Curr);
}
...
{
    OS_ENTER_CRITICAL();
    if(wRLineStatus & cBitStatusVoltLoss)
    {
        OS_EXIT_CRITICAL();
        return(true);
    }
...
{
    OS_ENTER_CRITICAL();
    if(wLineStatus & cBitStatusVoltLoss)
    {
        OS_EXIT_CRITICAL();
        return(true);
    }
...
{
    OS_ENTER_CRITICAL();
    if(wRLineStatus & cBitStatusFreqLoss)
    {
        OS_EXIT_CRITICAL();
        return(true);
    }
...
{
    OS_ENTER_CRITICAL();
    if(wLineStatus & cBitStatusFreqLoss)
    {
        OS_EXIT_CRITICAL();
        return(true);
    }
...
{
    OS_ENTER_CRITICAL();
    if((wLineStatus & cBitStatusLoss)||(0))//并机有一台出现丢失
    {
        OS_EXIT_CRITICAL();
        return(true);
    }
...
{
    OS_ENTER_CRITICAL();
    if(wRLineStatus & cBitStatusWavLoss)
    {
        OS_EXIT_CRITICAL();
        return(true);
    }
...
INT8U sbGetRLineLossState(void)
{
    return(LineLossState);
}

INT32S sbGetLineWatt(void)
{
    return(dwLineWattNew);
}

INT32S sbGetLineVA(void)
{
    return(dwLineVANew);
}

INT32S sbGetLineL1Watt(void)
{
    return(dwLineL1WattNew);
}

INT32S sbGetLineL1VA(void)
{
    return(dwLineL1VANew);
}

INT32S sbGetLineL2Watt(void)
{
    return(dwLineL2WattNew);
}

INT32S sbGetLineL2VA(void)
{
    return(dwLineL2VANew);
}
