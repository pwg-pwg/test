...
void sInitAdc(void)
{
    ...
    AdccRegs.ADCSOC14CTL.bit.CHSEL= 14;     // SOC14 will convert ADCIND14
    AdccRegs.ADCSOC15CTL.bit.CHSEL= 15;       // SOC1 will convert ADCINC15
	...
	AdccRegs.ADCSOC14CTL.bit.ACQPS   = wTemp;
    ...
    AdcbRegs.ADCSOC0CTL.bit.TRIGSEL     = wTrigSel0;
    AdcbRegs.ADCSOC1CTL.bit.TRIGSEL     = wTrigSel0;
    AdcbRegs.ADCSOC2CTL.bit.TRIGSEL     = wTrigSel0;
    AdcbRegs.ADCSOC3CTL.bit.TRIGSEL     = wTrigSel0;
	...
	AdccRegs.ADCSOC14CTL.bit.TRIGSEL     = wTrigSel0;
...
//    GpioCtrlRegs.GPAGMUX2.bit.GPIO19 = 1;   // GPIO17 = PWM       //LLC L LD
	...
	//BUS OVP
    GpioCtrlRegs.GPBPUD.bit.GPIO37 = 0;    // disable pullup on GPIO37
    ...
    GpioCtrlRegs.GPBPUD.bit.GPIO50 = 0;  // Enable pullup on GPIO75
    ...
    /*GPIO_TEST*/
    // GpioCtrlRegs.GPCPUD.bit.GPIO66 = 0;    // disable pullup on GPIO66
...
void sInitEpwm(void)
{
	INT16U wLLCPwmPeriod = cLLC_PWM_PERIOD_28KHz;
	
	if(g_wVAType == cb8000VAModel)
	{
		wLLCPwmPeriod = cLLC_PWM_PERIOD_32KHz;
	}
...
//    EPwm5Regs.TZCTL.bit.TZB = TZ_FORCE_LO;  //force low
    EPwm5Regs.AQSFRC.bit.RLDCSF = 1;
    EDIS;
	EPwm5Regs.AQCSFRC.all = 0x5; // CSFA = 1, CSFB = 1;
    EPwm5Regs.TBPRD = cBOOST_PWM_PERIOD; // Period = 5200 TBCLK counts
    EPwm5Regs.TBPHS.bit.TBPHS = 2; // Set Phase register to zero
    ...
    EPwm5Regs.TBCTL.bit.PHSDIR = TB_UP;//Count up after the synchronization event

    EPwm5Regs.CMPCTL.bit.SHDWAMODE = CC_SHADOW;  //load CMPA from shadow register
    EPwm5Regs.CMPCTL.bit.LOADAMODE = CC_CTR_PRD;  //load CMPA on CTR=0
...
//	EPwm5Regs.AQCTLB.bit.CBD = AQ_SET;	
    ...
    EPwm5Regs.DBFED.bit.DBFED = 240;//SCCM 96 cBUCKPeriod*2/100*4   0.8us
    EPwm5Regs.DBRED.bit.DBRED = 240;
...
//    EPwm6Regs.TBCTL.bit.PHSEN = TB_ENABLE; //Do not load the time-base counter (TBCTR) from the time-base phase register (TBPHS)
//    EPwm6Regs.TBCTL.bit.PRDLD = TB_SHADOW;  //loaded the TBPRD from shadow register when TBCTR is equal to zero

//    EPwm6Regs.TBCTL.bit.SYNCOSEL = TB_SYNC_IN;
//    EPwm6Regs.TBCTL.bit.PHSDIR = TB_UP;//Count up after the synchronization event 0:down 1:up
...
//    EPwm6Regs.ETSEL.bit.SOCASEL = 3;    // Select SOC on zero or period 3
...
//    EPwm6Regs.ETSEL.bit.SOCAEN = 1;     // Enable SOCA
	...
	EPwm7Regs.AQCSFRC.bit.CSFA = 0x0; // CSFA = 0, CSFB = 1;
	EPwm7Regs.TBPRD = 1667;		// Protect	PWM ,20KHz
	...
	EPwm7Regs.CMPA.bit.CMPA = 1200;
	EPwm7Regs.CMPB.bit.CMPB = 1200;		   //20%
...
void sInitCmpss(void)
{
	if(g_wVAType == cb12KVAModel)
	{
	    ...
	    InitCMPSS(&Cmpss2Regs,PRI_OCP_CMSS_H_PRO_POINT_12K,PRI_OCP_CMSS_L_PRO_POINT_12K);
	}
	else if(g_wVAType == cb10KVAModel)
	{
		InitCMPSS(&Cmpss1Regs,PRI_OCP_CMSS_H_PRO_POINT_10K,PRI_OCP_CMSS_L_PRO_POINT_10K);
	    InitCMPSS(&Cmpss2Regs,PRI_OCP_CMSS_H_PRO_POINT_10K,PRI_OCP_CMSS_L_PRO_POINT_10K);
	}
	else//8K
	{
		InitCMPSS(&Cmpss1Regs,PRI_OCP_CMSS_H_PRO_POINT_8K,PRI_OCP_CMSS_L_PRO_POINT_8K);
	    InitCMPSS(&Cmpss2Regs,PRI_OCP_CMSS_H_PRO_POINT_8K,PRI_OCP_CMSS_L_PRO_POINT_8K);
	}
    ...
    //Configure TRIP5 to be CTRIPH or CTRIPL CMPSS2
...
void sModelIdentify(void)
{
    ...
    AdccRegs.ADCSOC3CTL.bit.CHSEL= 3;       // SOC3 will convert ADCINC3
    AdccRegs.ADCSOC14CTL.bit.CHSEL= 14;     // SOC14 will convert ADCINC14
    INT16U wTemp = 19;//14
    AdccRegs.ADCSOC0CTL.bit.ACQPS   = wTemp;
    AdccRegs.ADCSOC1CTL.bit.ACQPS   = wTemp;
    AdccRegs.ADCSOC2CTL.bit.ACQPS   = wTemp;
    AdccRegs.ADCSOC3CTL.bit.ACQPS   = wTemp;
	AdccRegs.ADCSOC14CTL.bit.ACQPS   = wTemp;
    ...
    EALLOW;
	GpioCtrlRegs.GPCPUD.bit.GPIO80 = 0;  // Enable pullup on GPIO80
    GpioDataRegs.GPCSET.bit.GPIO80= 1; // Load output latch to low
	GpioCtrlRegs.GPCMUX2.bit.GPIO80 = 0; // GPIO80 = I/O
	GpioCtrlRegs.GPCDIR.bit.GPIO80 = 1;  // GPIO80 = output
    EDIS;
    ...
    for(;;)
    {
		AdccRegs.ADCSOCFRC1.all=0x4000;
        while(AdccRegs.ADCCTL1.bit.ADCBSY==1);
        wADResult = ADCCRESULT14;

        if((2457 <= wADResult) && (wADResult < 3003))  // 15048:2.00V(1.8~2.2V)
        {
            w15KBatt48VModuleCnt++;
        }
        else if((1582 <= wADResult) && (wADResult < 1882))  //10048:(1.15V~1.378V)
        {
            w10KBatt48VModuleCnt++;
        }
		else if((1886 <= wADResult) && (wADResult < 2186))  //12048:(1381V~1.601V)
		{
			w12KBatt48VModuleCnt ++;
		}
		else if((1237 <= wADResult) && (wADResult < 1537))//8048:(0.906V~1.125V)
		{
			w8KBatt48VModuleCnt ++;
		}
		else
		...
		else if(w12KBatt48VModuleCnt > 100)
		{
			g_wVAType = cb12KVAModel;
            sSetBatteryPcs(cbBatt48VType);
            break;
		}
		else if(w8KBatt48VModuleCnt > 100)//其他默认走8K
		{
...
void sUpsParaInit(void)
{
	memset(&DcSampData,0,sizeof(DcSampData));
	memset(&AcSampData,0,sizeof(AcSampData));
	memset(&WattSampData,0,sizeof(WattSampData));
	memset(&TempSampData,0,sizeof(TempSampData));
	memset(&DcFiltData,0,sizeof(DcFiltData));
	memset(&InvPri1Curr,0,sizeof(InvPri1Curr));
	memset(&InvPri2Curr,0,sizeof(InvPri2Curr));
	if(g_wVAType == cb12KVAModel)
    {
       //ADC Ratio Initial
        DcSampData[PV1_VOLT].wSampRatio = 3008;//601/4096*10*2048
        DcSampData[PV2_VOLT].wSampRatio = 3008;//601/4096*10*2048
		DcSampData[PV1_CURR].wSampRatio = 5000; //66.67/2048*100*2048
        DcSampData[PV2_CURR].wSampRatio = 3750; //37.5/2048*100*2048
        DcSampData[PV_BUS_VOLT].wSampRatio = 4010;//802/4096*10*4096
        DcSampData[PV_PBUS_VOLT].wSampRatio = 4010;//802/4096*10*4096
        DcSampData[PV_NBUS_VOLT].wSampRatio = 4010;//802/4096*10*4096
		// DcSampData[PV_OUT_CURR].wSampRatio = 2025;//202.5/3*10*2048
        DcSampData[PV_OUT_CURR].wSampRatio = 2700;//270.0
		DcSampData[BAT_CURR1].wSampRatio = 5760;  //576*10
		DcSampData[BAT_CURR2].wSampRatio = 5760;  //576*10
		DcSampData[BAT_VOLT].wSampRatio = 4010;// 15048:
        DcSampData[BUS_VOLT].wSampRatio = 4010;// 15048:
        DcSampData[PV_ISO_VOLT].wSampleReal = 5115;// 511.5
        AcSampData[GRID_VOLT].SampCal.wSampRatio = 5013;// 501.3
        AcSampData[GEN_VOLT].SampCal.wSampRatio = 5013;// 501.3
        AcSampData[S_LOAD_VOLT].SampCal.wSampRatio = 5013;// 501.3
        AcSampData[OUT_VOLT].SampCal.wSampRatio = 5013;// 501.3
        AcSampData[OUT_L1_VOLT].SampCal.wSampRatio = 5013;// 501.3
        AcSampData[OUT_L2_VOLT].SampCal.wSampRatio = 5013;// 501.3
        AcSampData[INV_VOLT].SampCal.wSampRatio = 5013;// 501.3 
        AcSampData[GRID_L1_VOLT].SampCal.wSampRatio = 5013;// 501.3 
        AcSampData[GRID_L2_VOLT].SampCal.wSampRatio = 5013;// 501.3 
        AcSampData[INV_L1_CURR].SampCal.wSampRatio = 3063;// 306.3A
        AcSampData[INV_L2_CURR].SampCal.wSampRatio = 3063;// 306.3A
        AcSampData[OUT_L1_CURR].SampCal.wSampRatio = 3063;// 306.3A
        AcSampData[OUT_L2_CURR].SampCal.wSampRatio = 3063;// 306.3A
        AcSampData[GRID_L1_CURR].SampCal.wSampRatio = 3063;// 306.3A
        AcSampData[GRID_L2_CURR].SampCal.wSampRatio = 3063;// 306.3A
		AcSampData[GEN_L1_CURR].SampCal.wSampRatio = 3063;// 306.3A
		AcSampData[GEN_L2_CURR].SampCal.wSampRatio = 3063;// 306.3A
		AcSampData[S_LOAD_L1_CURR].SampCal.wSampRatio = 3063;// 306.3A
		AcSampData[S_LOAD_L2_CURR].SampCal.wSampRatio = 3063;// 306.3A
		
        //Converter factor Initial
        g_wConverterFactor = 80;            // 15048:Converter TX Boost ratio230/28.75

        //Inverter Factor Initial
        g_wInvOverCurrentFactor = 10;        // 15048:6.684444 = 4096*InvLCurrRatio/3
        g_wInvLCurrLineLevel1 = 1638;        // 15048:oóD?òa??2.40±????¨μ?á÷85A*6.68444->127.5
        g_wInvLCurrLineLevel2 = 1638;        // 15048:oóD?òa??2.20±????¨μ?á÷80.0A*6.68444->120

        g_wInvOverCurrRated = 150; //127;         // 15048:2.40±????¨μ?á÷     180A
        g_wInvCurrLimitRated = 145;//125;         // 15048:×?′ó?T??μ?á÷15k/170

        g_wInvCurrRated = cIac44A;          // 12048:额定44A

        //Inverter Cntl Limit Parameters Initial
        g_wInvVCntlFactor = 251;             //Vmax/RefAd*2^5 15048
        g_wOPCurrCntlFactor = 184;           // 15048:31
        //g_wOPCurrCntlFactor = 153;
        g_wInvLCurrCntlFactor = 153;         // 15048:34

        //Battery and Charger Parameter Initial
        //sSetBatOpenVolt(cBat16v);           // 10048:
        //sSetBatOpenBackVolt(cBat36v);       // 10048:

        sSetBatOpenVolt(0);           // 10048:
        sSetBatOpenBackVolt(cBat36v);       // 10048:

        g_wFBInvLCurrRatio = 495;           // 10048://×?????óDê1ó?
        g_wChgEfficiency = 8800;            // 10048:if Charger Efficiency is 91.88%@80A
        ...
        g_wGridCurrMax = 500;               // 12048:50A
        g_wAcChgCurrMax = cIdc350A;//cIdc130A;//;            // 10048:80A
        ...
        //Watt/VA Rated
        wWattRated = c12KWattMaxPower;                  // 12048:
        wVARated   = c12KVAMaxPower;                 // 12048:
        wCurrRated = c12KInvMaxCurr;

        bCapaSize  = 150;                   // 10048:
        g_wDischgVoltSet = cBat52v;         // 10048:Aging Start Volt 52V
        g_wDischgCurrSet = cIdc120A;         // 10048:Discharge Current 80A

        wTxRatio = 2048;                    // 15048:230V:28.75.0V(Q8:230*256/28.75=2048)
        wBuckRatio = 225;                   // 15048:0.879*256  no use
    ...
    else if(g_wVAType == cb10KVAModel)
    {
        ...
        AcSampData[GRID_L1_VOLT].SampCal.wSampRatio = 5013;// 501.3 
        AcSampData[GRID_L2_VOLT].SampCal.wSampRatio = 5013;// 501.3 
        AcSampData[INV_L1_CURR].SampCal.wSampRatio = 2000;// 306.3A
        AcSampData[INV_L2_CURR].SampCal.wSampRatio = 2000;// 306.3A
        AcSampData[OUT_L1_CURR].SampCal.wSampRatio = 2000;// 306.3A
        AcSampData[OUT_L2_CURR].SampCal.wSampRatio = 2000;// 306.3A
        AcSampData[GRID_L1_CURR].SampCal.wSampRatio = 2000;// 306.3A
        AcSampData[GRID_L2_CURR].SampCal.wSampRatio = 2000;// 306.3A
		AcSampData[GEN_L1_CURR].SampCal.wSampRatio = 2000;// 306.3A
		AcSampData[GEN_L2_CURR].SampCal.wSampRatio = 2000;// 306.3A
		AcSampData[S_LOAD_L1_CURR].SampCal.wSampRatio = 2000;// 200.0A
		AcSampData[S_LOAD_L2_CURR].SampCal.wSampRatio = 2000;// 200.0A
        ...
        g_wInvCurrRated = cIac36A;          // 10048:???¨μ?á÷

        //Inverter Cntl Limit Parameters Initial
        g_wInvVCntlFactor = 251;             //Vmax/RefAd*2^5 15048
        g_wOPCurrCntlFactor = 184;           // 15048:31
        //g_wOPCurrCntlFactor = 153;
        g_wInvLCurrCntlFactor = 100;         // 15048:34
        ...
        //Watt/VA Rated
        wWattRated = c10KWattMaxPower;                  // 10048:
        wVARated   = c10KVAMaxPower;                 // 10048:
    ...
    else //if(g_wVAType == cb8000VAModel)
    {
        ...
        //Watt/VA Rated
        wWattRated = c8KWattMaxPower;                  // 10048:
        wVARated   = c8KVAMaxPower;                 // 10048:
