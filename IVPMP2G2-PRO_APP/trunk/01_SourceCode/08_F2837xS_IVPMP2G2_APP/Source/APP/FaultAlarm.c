...
union FAULT_INFO_SRC_TABLE
{
    struct FAULT_INFO_SRC_BIT_FEILD
    {
        ...
        INT16U eBatHNtcConnect:1;		//电池正端过温/未接
        INT16U eBatLNtcConnect:1;		//电池负端过温/未接
        INT16U eGridCurrOver:1;		    //市电过流

        INT16U eOPCurrOver:1;		//输出软件过流故障
        INT16U eModelFault:1;		//机型识别错误
        INT16U res:2;
    } bits;
    INT16U data;
} fFault7_SysSrc;
...
union ALARM4_BMS_TABLE
{
    struct ALARM4_BMS_BIT_FEILD
    {
        ...
        INT16U bPara_InvPhaselossAlarm:1;		// 并机逆变缺相告警
        INT16U bRsvd:10;                    // 预留
    } bits;
    INT16U data;
} fAlarm4_SysBms;
...
void sAlarmBitUpdate()
{
	...
	static INT16U uwGridPhaseLossRecCnt = 0;
    static INT16U uwInvPhaseLossCnt = 0;
	static INT16U uwInvPhaseLossRecCnt = 0;
	static INT16U uwGenPhaseLossCnt = 0;
	static INT16U uwGenPhaseLossRecCnt = 0;
    ...
    // {
	// 	uwGridPhaseLossCnt ++;
	// 	if(uwGridPhaseLossCnt > 750)//15s
	// 	{
	// 		uwGridPhaseLossCnt = 0;
	// 		sSetWarnCode(eGridLackPhaseAlarm);
	// 		swSetGridLackPhaseAlarm(1);
	// 	}
	// 	uwGridPhaseLossRecCnt = 0;
    // }
    // else
    // {
	// 	uwGridPhaseLossRecCnt ++;
	// 	if(uwGridPhaseLossRecCnt > 800)//16S
	// 	{
	// 		uwGridPhaseLossRecCnt = 0;
    ...
    //并机逆变缺相告警
//	if(g_uwParaInvLackPhaseFlag &&  wParaMode == cMasterMode)
//    {
//		uwInvPhaseLossCnt ++;
//		if(uwInvPhaseLossCnt > 750)//15s
//		{
//			uwInvPhaseLossCnt = 0;
//			sSetWarnCode(ePara_InvPhaselossAlarm);
//			swSetPara_InvPhaselossAlarm(1);
//		}
//		uwInvPhaseLossRecCnt = 0;
//    }
//    else
//    {
//		uwInvPhaseLossRecCnt ++;
//		if(uwInvPhaseLossRecCnt > 800)//16S
//		{
//			uwInvPhaseLossRecCnt = 0;
//	        if(swGetWarnCode()==ePara_InvPhaselossAlarm)
//	        {
//	            sSetWarnCode(0);
//	        }
//			swSetPara_InvPhaselossAlarm(0);
//		}
//		uwInvPhaseLossCnt = 0;
//    }
    ...
    {
		uwGenPhaseLossCnt ++;
		if(uwGenPhaseLossCnt > 750)
		{
			uwGenPhaseLossCnt = 0;
	        sSetWarnCode(eGenLackPhaseAlarm);
			swSetGenLackPhaseAlarm(1);	
		}
		uwGenPhaseLossRecCnt = 0;
    }
    else
    {
		uwGenPhaseLossRecCnt ++;
		if(uwGenPhaseLossRecCnt > 800)//16S
		{
			uwGenPhaseLossRecCnt = 0;
...
//        }
        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSVoltOverWarning)
        {
            sSetWarnCode(eBMS_OV);
            swSetBMSVoltOverAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_OV)
            {
                sSetWarnCode(0);
            }
			swSetBMSVoltOverAlarm(0);
        }

        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSVoltLowWarning)
        {
            sSetWarnCode(eBMS_UV);
            swSetBMSVoltLowAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_UV)
            {
                sSetWarnCode(0);
            }
			swSetBMSVoltLowAlarm(0);
        }

        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSTempOverWarning)
        {
            sSetWarnCode(eBMS_OT);
            swSetBMSTempOverAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_OT)
            {
                sSetWarnCode(0);
            }
			swSetBMSTempOverAlarm(0);
        }

        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSTempLowWarning)
        {
            sSetWarnCode(eBMS_UT);
            swSetBMSTempLowAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_UT)
            {
                sSetWarnCode(0);
            }
			swSetBMSTempLowAlarm(0);
        }

        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSCellVoltHiWarning)
        {
            sSetWarnCode(eBMS_CellOV);
            swSetBMSCellVoltHiAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_CellOV)
            {
                sSetWarnCode(0);
            }
			swSetBMSCellVoltHiAlarm(0);
        }
        ...
        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSCellVoltLowWarning)
        {
            sSetWarnCode(eBMS_CellUV);
            swSetBMSCellVoltLowAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_CellUV)
            {
                sSetWarnCode(0);
            }
			swSetBMSCellVoltLowAlarm(0);
        }

        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSChgCurrHiWarning)
        {
            sSetWarnCode(eBMS_ChgCurrHiAlarm);
            swSetBMSChgCurrHiAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_ChgCurrHiAlarm)
            {
                sSetWarnCode(0);
            }
			swSetBMSChgCurrHiAlarm(0);
        }

        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSDisChgCurrHiWarning)
        {
            sSetWarnCode(eBMS_DischgCurrHiAlarm);
            swSetBMSDisChgCurrHiAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_DischgCurrHiAlarm)
            {
                sSetWarnCode(0);
            }
			swSetBMSDisChgCurrHiAlarm(0);
        }

        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSCellTempHiWarning)
        {
            sSetWarnCode(eBMS_CellOT);
            swSetBMSCellTempHiAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_CellOT)
            {
                sSetWarnCode(0);
            }
			swSetBMSCellTempHiAlarm(0);
        }

        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSCellTempLowWarning)
        {
            sSetWarnCode(eBMS_CellUT);
            swSetBMSCellTempLowAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_CellUT)
            {
                sSetWarnCode(0);
            }
			swSetBMSCellTempLowAlarm(0);
        }

        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSSampSensorWarning)
        {
            sSetWarnCode(eBMS_SampSensor_Alarm);
            swSetBMSSampSensorAlarm(1);
        }
        else
        {
            if(swGetWarnCode()==eBMS_SampSensor_Alarm)
            {
                sSetWarnCode(0);
            }
			swSetBMSSampSensorAlarm(0);
        }

        if(uBMSData.Stru.wBMS1FaultAlarm & cBMSOtherWarning)
        {
            sSetWarnCode(eBMS_OtherAlarm);
            swSetBMSOtherAlarm(1);
        }
...
INT16U  swGetPara_InvPhaselossAlarm(void)        // 并机逆变缺相告警
{
    return fAlarm4_SysBms.bits.bPara_InvPhaselossAlarm;
}
/********************************************************************************
...
void swSetModelFault(INT16U uwTemp)// 机型识别故障
{
    fFault7_SysSrc.bits.eModelFault = uwTemp;
}
...
void    swSetPara_InvPhaselossAlarm(INT16U uwTemp)       // 并机逆变缺相告警
{
    fAlarm4_SysBms.bits.bPara_InvPhaselossAlarm = uwTemp;
}
