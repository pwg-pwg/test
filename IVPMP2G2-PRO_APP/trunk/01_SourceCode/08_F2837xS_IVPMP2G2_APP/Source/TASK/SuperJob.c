...
INT16U uwGridL2RelayLastState = 0;
// INT16U uwGridNRelayLastState = 0;
INT16U uwGenRelayLastState = 0;
INT16U uwGenL2RelayLastState = 0;
INT16U uwInv2RelayLastState = 0;
INT16U uwInvNRelayLastState = 0;
INT16U uwNPERelayLastState = 0;
...
INT16U	g_uwPowerSave1sCnt = 0;//节能模式计数器
...
INT16U ACONFLAG =0;
INT16U wAFCIStatusFlag = 0;
INT16U wRSDStatusFlag = 0;
...
void    sStandbyMode(void)
{
    ...
    while(1)
    {
        ...
        if(event&(1<<eSuperToLine))
        {
			...
			sSetSampleBias(&AcSampData[GRID_VOLT].SampCal,0);
			sSetSampleBias(&AcSampData[GEN_VOLT].SampCal,0);
            ...
            else
            {
				...
				if(abs(swGetSampleBiasTemp(AcSampData[GRID_VOLT].SampCal)) <= 250)
				{
					sSetSampleBias(&AcSampData[GRID_VOLT].SampCal,swGetSampleBiasTemp(AcSampData[GRID_VOLT].SampCal));
                }
				if(abs(swGetSampleBiasTemp(AcSampData[GEN_VOLT].SampCal)) <= 250)
				{
					sSetSampleBias(&AcSampData[GEN_VOLT].SampCal,swGetSampleBiasTemp(AcSampData[GEN_VOLT].SampCal));
                }
                InvVoltRelayAction = 0;
                g_uwWorkMode = cLineMode;
                return;
    ...
    while(1)
    {
        ...
        if(event&(1<<eSuperToLine))
        {
            ...
            if(swGetEepromOutputMode() == cOP_Single)
            {
				...
				{
					if(swGetLineFreqNew() > 5500)//>55Hz
					{
                        sSetRlyPointer(RLY_CROSSZERO,35,eSuperSelectPoint,cPrioSuper);
						// sSetRlyPointer(RLY_CROSSZERO,95,eSuperSelectPoint,cPrioSuper);//峰值
		                while(OSMaskEventPend(0x7FFF) == 0);
					}
					else
					{
                        sSetRlyPointer(RLY_CROSSZERO,cDelay8ms+10,eSuperSelectPoint,cPrioSuper);
						// sSetRlyPointer(RLY_CROSSZERO,cDelay10ms,eSuperSelectPoint,cPrioSuper);
		                while(OSMaskEventPend(0x7FFF) == 0);
					}
				}
				else
				{
                    if(swGetLineFreqNew() > 5500)//<55Hz
					{
						sSetRlyPointer(RLY_CROSSZERO,cDelay8ms+15,eSuperSelectPoint,cPrioSuper);
		                while(OSMaskEventPend(0x7FFF) == 0);
					}
					else
					{
						sSetRlyPointer(RLY_CROSSZERO,cDelay7ms+5,eSuperSelectPoint,cPrioSuper);
		                while(OSMaskEventPend(0x7FFF) == 0);
					}
				}
                if(g_LineModeJoinInWay == cLineModeJoinInByLine)
                ...
                if(g_wVAType == cb12KVAModel)
                {
                    ...
                    while(OSMaskEventPend(0x7FFF) == 0);
                    // if(sGetGridVoltInputType()&&(g_LineModeJoinInWay == cLineModeJoinInByLine))//1: 2/3相
                    if(g_LineModeJoinInWay == cLineModeJoinInByLine)//1: 2/3相
                    {
                        hoInvNRelayOff;
                    }
                    // sSetRlyPointer(RLY_DELAY,cDelay10ms+cDelay2ms,eSuperSelectPoint,cPrioSuper);
                    sSetRlyPointer(RLY_DELAY,cDelay10ms,eSuperSelectPoint,cPrioSuper);
                    while(OSMaskEventPend(0x7FFF) == 0);  
                }
                else
                {
                    ...
                    // sSetRlyPointer(RLY_DELAY,cDelay8ms,eSuperSelectPoint,cPrioSuper);
                    sSetRlyPointer(RLY_DELAY,cDelay8ms+cDelay3ms,eSuperSelectPoint,cPrioSuper);
                    while(OSMaskEventPend(0x7FFF) == 0);  
                }
                sSetFBControlStatus(cFBWaitSts);
                sSetBuckControlStatus(cBuckWaitSts);
            ...
            else
            {
				...
				{
					sSetRlyPointer(RLY_CROSSZERO,95,eSuperSelectPoint,cPrioSuper);
	                while(OSMaskEventPend(0x7FFF) == 0);
				}
                if(g_LineModeJoinInWay == cLineModeJoinInByLine)
                {
                ...
                sSetRlyPointer(RLY_DELAY,cDelay15ms,eSuperSelectPoint,cPrioSuper);
                while(OSMaskEventPend(0x7FFF) == 0);
                sSetRlyPointer(RLY_DELAY,35,eSuperSelectPoint,cPrioSuper);
                while(OSMaskEventPend(0x7FFF) == 0);
                // if(sGetGridVoltInputType()&&(g_LineModeJoinInWay == cLineModeJoinInByLine))//1: 2/3相
                if(g_LineModeJoinInWay == cLineModeJoinInByLine)//1: 2/3相
                {
                ...
                sSetRlyPointer(RLY_DELAY,cDelay10ms+25,eSuperSelectPoint,cPrioSuper);
                while(OSMaskEventPend(0x7FFF) == 0);
                sSetFBControlStatus(cFBWaitSts);
                sSetBuckControlStatus(cBuckWaitSts);
            }
        ...
        if(event&(1<<eSuperTimer))
        {
            ...
            if(g_uwToBatSCRDelayCnt < 2)//2/3相切到离网SCR关断延时 40ms
            {
                g_uwToBatSCRDelayCnt++;
            }
            else
            {
                hoSCRNRelayOff;
			...
			{
				g_uwPowerSave1sCnt ++;
				if(g_uwPowerSave1sCnt > ePowerSaveModePeriod)//1s
				{
					g_uwPowerSave1sCnt = 0;
				}
				if(g_uwPowerSave1sCnt < ePowerSaveModePWMOnTime)//200ms
				{
					if(g_uwPowerSave1sCnt < ePowerSaveModeSoftTime)
					{
						wVoltSoftTemp = (INT16U)((INT32U)2000 + (INT32U)g_uwPowerSave1sCnt * 800);
						OS_ENTER_CRITICAL();
						wRNewSinAmp = wVoltSoftTemp;
                ...
                if((g_wVAType == cb3500VAModel)||(swGetEepromOutputMode() == cOP_Single))
                {
					if(g_LineModeJoinInWay == cLineModeJoinInByLine)
					{
                        if(g_wVAType == cb12KVAModel)
                        {
                            ...
                            {
                                sOSTimerStop();
                                sSetRlyPointer(RLY_CROSSZERO,25,eSuperSelectPoint,cPrioSuper);
                                while(OSMaskEventPend(0x7FFF) == 0);
                                sOSTimerStart();
                            }
                            else
            ...
            if(fLoad.LoadOnCmd == true)
            {
                ...
                else
                {
					   ...
					   ((g_LineModeJoinInWay == cLineModeJoinInByGen)&&(sbGetGenFreqLoss()||sbGetGenVoltLoss())))
					{
                        ...
                        //if(sGetGridVoltInputType())//0：裂相  | 1: 2/3相
                        if(g_LineModeJoinInWay == cLineModeJoinInByLine)
                        {                        
                            if(g_wVAType == cb12KVAModel)
                            {
                                sSetRlyPointer(RLY_DELAY,cDelay4ms,eSuperSelectPoint,cPrioSuper);
                                while(OSMaskEventPend(0x7FFF) == 0);                                
                            }
                            else
                            {
                                sSetRlyPointer(RLY_DELAY,cDelay2ms,eSuperSelectPoint,cPrioSuper);
                                while(OSMaskEventPend(0x7FFF) == 0);                                    
                            }
                        }
					}
...
INT16U swSampBiasChk(void)
{
    ...
    OS_ENTER_CRITICAL();
	AcSampData[OUT_VOLT].SampCal.wSampBias = 0;
    AcSampData[OUT_L1_VOLT].SampCal.wSampBias = 0;
	AcSampData[INV_VOLT].SampCal.wSampBias = 0;
    AcSampData[INV_L1_CURR].SampCal.wSampBias = 0;
	AcSampData[INV_L2_CURR].SampCal.wSampBias = 0;
    AcSampData[GRID_VOLT].SampCal.wSampBias = 0;
	AcSampData[GRID_L1_CURR].SampCal.wSampBias = 0;
	AcSampData[GRID_L2_CURR].SampCal.wSampBias = 0;
	AcSampData[GEN_VOLT].SampCal.wSampBias = 0;
    AcSampData[GEN_L1_CURR].SampCal.wSampBias = 0;
	AcSampData[GEN_L2_CURR].SampCal.wSampBias = 0;
	DcSampData[BAT_CURR1].wSampBias = 0;
    DcSampData[BAT_CURR2].wSampBias = 0;
    DcSampData[PV_OUT_CURR].wSampBias = 0;
    DcSampData[PV1_CURR].wSampBias = 0;
    DcSampData[PV2_CURR].wSampBias = 0;
	InvPri1Curr.wSampBias = 0;
    OS_EXIT_CRITICAL();
    while(1)
    {
        event=OSEventPend();
        ...
        if(event&(1<<eSuperTimer))
        {
            ...
            {
                if((abs(swGetSampleBiasTemp(AcSampData[INV_VOLT].SampCal)) > 250))
                {
					sSetSampleBias(&AcSampData[INV_VOLT].SampCal,0);
                }
                else
                {
					sSetSampleBias(&AcSampData[INV_VOLT].SampCal,swGetSampleBiasTemp(AcSampData[INV_VOLT].SampCal));
                }
				if((abs(swGetSampleBiasTemp(AcSampData[OUT_VOLT].SampCal)) > 250))
                {
					sSetSampleBias(&AcSampData[OUT_VOLT].SampCal,0);
                }
                else
                {
					sSetSampleBias(&AcSampData[OUT_VOLT].SampCal,swGetSampleBiasTemp(AcSampData[OUT_VOLT].SampCal));
                }
				if((abs(swGetSampleBiasTemp(AcSampData[OUT_L1_VOLT].SampCal)) > 250))
                {
					sSetSampleBias(&AcSampData[OUT_L1_VOLT].SampCal,0);
                }
                else
                {
					sSetSampleBias(&AcSampData[OUT_L1_VOLT].SampCal,swGetSampleBiasTemp(AcSampData[OUT_L1_VOLT].SampCal));
                }
				if((abs(swGetSampleBiasTemp(AcSampData[GRID_VOLT].SampCal)) > 250))
                {
					sSetSampleBias(&AcSampData[GRID_VOLT].SampCal,0);
				...
				if((abs(swGetSampleBiasTemp(AcSampData[INV_L1_CURR].SampCal)) > 250))
                {
					wFaultFlag |= 0x01;
					sSetFaultCode(eINVCTFault);
					swSetInvCTSenserFault(true);
					sSetSampleBias(&AcSampData[INV_L1_CURR].SampCal,0);
                }
                else
                {
					wFaultFlag &= 0xFE;
					swSetInvCTSenserFault(false);
					sSetSampleBias(&AcSampData[INV_L1_CURR].SampCal,swGetSampleBiasTemp(AcSampData[INV_L1_CURR].SampCal));
                }
				if((abs(swGetSampleBiasTemp(AcSampData[INV_L2_CURR].SampCal)) > 250))
                {
					wFaultFlag |= 0x02;
					sSetFaultCode(eINVCTFault);
					swSetInvCTSenserFault(true);
					sSetSampleBias(&AcSampData[INV_L2_CURR].SampCal,0);
                }
                else
                {
					wFaultFlag &= 0xFD;
					if(swGetInvCTSenserFault() == false)
					{
						swSetInvCTSenserFault(false);
					}
					sSetSampleBias(&AcSampData[INV_L2_CURR].SampCal,swGetSampleBiasTemp(AcSampData[INV_L2_CURR].SampCal));
                }
				if((abs(swGetSampleBiasTemp(AcSampData[GRID_L1_CURR].SampCal)) > 250))
                {
					wFaultFlag |= 0x04;
					sSetFaultCode(eGridCTFault);
					swSetGridCTSenserFault(true);
					sSetSampleBias(&AcSampData[GRID_L1_CURR].SampCal,0);
                }
                else
                {
					wFaultFlag &= 0xFB;
					swSetGridCTSenserFault(false);
					sSetSampleBias(&AcSampData[GRID_L1_CURR].SampCal,swGetSampleBiasTemp(AcSampData[GRID_L1_CURR].SampCal));
                }
				if((abs(swGetSampleBiasTemp(AcSampData[GRID_L2_CURR].SampCal)) > 250))
                {
					wFaultFlag |= 0x08;
					sSetFaultCode(eGridCTFault);
					swSetGridCTSenserFault(true);
					sSetSampleBias(&AcSampData[GRID_L2_CURR].SampCal,0);
                }
                else
                {
					wFaultFlag &= 0xF7;
					if(swGetGridCTSenserFault() == false)
					{
						swSetGridCTSenserFault(false);
					}
					sSetSampleBias(&AcSampData[GRID_L2_CURR].SampCal,swGetSampleBiasTemp(AcSampData[GRID_L2_CURR].SampCal));
                }
				if((abs(swGetSampleBiasTemp(AcSampData[GEN_L1_CURR].SampCal)) > 250))
                {
					wFaultFlag |= 0x10;
					sSetFaultCode(eGenCTFault);
					swSetGenCTSenserFault(true);
					sSetSampleBias(&AcSampData[GEN_L1_CURR].SampCal,0);
                }
                else
                {
					wFaultFlag &= 0xEF;
					swSetGenCTSenserFault(false);
					sSetSampleBias(&AcSampData[GEN_L1_CURR].SampCal,swGetSampleBiasTemp(AcSampData[GEN_L1_CURR].SampCal));
                }
				if((abs(swGetSampleBiasTemp(AcSampData[GEN_L2_CURR].SampCal)) > 250))
                {
					wFaultFlag |= 0x20;
					sSetFaultCode(eGenCTFault);
					swSetGenCTSenserFault(true);
					sSetSampleBias(&AcSampData[GEN_L2_CURR].SampCal,0);
                }
                else
                {
					wFaultFlag &= 0xDF;
					...
					sSetSampleBias(&AcSampData[GEN_L2_CURR].SampCal,swGetSampleBiasTemp(AcSampData[GEN_L2_CURR].SampCal));
                }

				if((abs(swGetSampleBiasTemp(DcSampData[BAT_CURR1])) > 200))
                {
					wFaultFlag |= 0x40;
					sSetFaultCode(eBatCTFault);//使用输出电流上传两个电池电流传感器故障
					swSetOpCTSenserFault(true);
					sSetSampleBias(&DcSampData[BAT_CURR1],0);
                }
                else
                {
					wFaultFlag &= 0xBF;
					swSetOpCTSenserFault(false);
					sSetSampleBias(&DcSampData[BAT_CURR1],swGetSampleBiasTemp(DcSampData[BAT_CURR1]));
                }
				if((abs(swGetSampleBiasTemp(DcSampData[BAT_CURR2])) > 200))//20A
                {
					wFaultFlag |= 0x80;
					sSetFaultCode(eBatCTFault);
					swSetOpCTSenserFault(true);
					sSetSampleBias(&DcSampData[BAT_CURR2],0);
                }
                else
                {
					wFaultFlag &= 0x7F;
					sSetSampleBias(&DcSampData[BAT_CURR2],swGetSampleBiasTemp(DcSampData[BAT_CURR2]));
					if(swGetOpCTSenserFault() == false)//防止上面置位这里清掉
					{
						swSetOpCTSenserFault(false);
					}
                }
				if((abs(swGetSampleBiasTemp(DcSampData[PV_OUT_CURR])) > 200))//20A
                {
					wFaultFlag |= 0x100;
					sSetFaultCode(ePvOutCTFault);
					swSetDCDCCTSenserFault(true);
					sSetSampleBias(&DcSampData[PV_OUT_CURR],0);
                }
                else
                {
					wFaultFlag &= 0xEFF;
					swSetDCDCCTSenserFault(false);
					sSetSampleBias(&DcSampData[PV_OUT_CURR],swGetSampleBiasTemp(DcSampData[PV_OUT_CURR]));
                }
				if((abs(swGetSampleBiasTemp(DcSampData[PV1_CURR])) > 500))//5A
                {
					wFaultFlag |= 0x200;
					sSetFaultCode(ePVCTFault);
					swSetPvCTSenserFault(true);
					sSetSampleBias(&DcSampData[PV1_CURR],0);
                }
                else
                {
					wFaultFlag &= 0xDFF;
					swSetPvCTSenserFault(false);
					sSetSampleBias(&DcSampData[PV1_CURR],swGetSampleBiasTemp(DcSampData[PV1_CURR]));
                }
				if((abs(swGetSampleBiasTemp(DcSampData[PV2_CURR])) > 500))//5A
                {
					wFaultFlag |= 0x400;
					sSetFaultCode(ePVCTFault);
					swSetPvCTSenserFault(true);
					sSetSampleBias(&DcSampData[PV2_CURR],0);
                }
                else
                {
					wFaultFlag &= 0xBFF;
					...
					sSetSampleBias(&DcSampData[PV2_CURR],swGetSampleBiasTemp(DcSampData[PV2_CURR]));
                }
				if((abs(swGetSampleBiasTemp(InvPri1Curr)) > 500))//5A
                {
					sSetSampleBias(&InvPri1Curr,0);
                }
                else
                {
					sSetSampleBias(&InvPri1Curr,swGetSampleBiasTemp(InvPri1Curr));
                }
				if(wFaultFlag != 0)
				{
					return(false);
...
INT8U sbInvSoftStart(void)
{
    ...
    while(1)
    {
        ...
        if(event&(1<<eSuperTimer))   //20ms
        {
			// if( wLineBatVoltNormal &&((wGridL1L2Phase120Cnt >= 150)||(wGridL1L2Phase180Cnt >= 150)))
	...
	//				wRNewSinAmpTemp=wRNewSinAmp;
	                ...
	                if (sbGetInverterPLStatus()==true)
	                {
	                    ...
	                    if(bCnt == 3)
	                    {
                        	...
                        	//hoSFTNRLYOn; //市电继电器N??
                            // if(!sGetGridVoltInputType()||(g_LineModeJoinInWay == cLineModeJoinInByGen))//0:裂相  | 1: 2/3相
                            if(g_LineModeJoinInWay == cLineModeJoinInByGen)//0:裂相  | 1: 2/3相
                            {
                                hoInvNRelayOn;
                            }
	                        sSetRlyPointer(RLY_DELAY,cDelay10ms,eSuperSelectPoint,cPrioSuper);
...
INT16U swTestTaskProc(void) // 20ms
{
    ...
    // 任务计时
    #ifdef TESTTASK
    if(wTestMode20msCnt < 1350)    // 27s循环一次1000->1250 25s
    {
        wTestMode20msCnt++;
    }
    else
...
//    }
    // RLY
    //顺序动作，错开1s
    if((wTestMode1Sec >= 3) && (wTestMode1Sec < 27))//9
    {
        //市电继电器1
...
//			hoFanPowerOff;
		...
		//FAN SPS继电器
        if((wTestMode1Sec == 15) && (wTestMode20ms == 10))
        {
            hoFanPowerOn;
        }
        else if((wTestMode1Sec == 16) && (wTestMode20ms == 35))
        {
            hoFanPowerOff;
        }

		//Grid L2
        if((wTestMode1Sec == 17) && (wTestMode20ms == 10))
        {
            hoGridL2RelayOn;
        }
        else if((wTestMode1Sec == 18) && (wTestMode20ms == 35))
        {
            hoGridL2RelayOff;
        }

		//Grid N
        if((wTestMode1Sec == 19) && (wTestMode20ms == 10))
        {
            hoSCRNRelayOn;
        }
        else if((wTestMode1Sec == 20) && (wTestMode20ms == 35))
        {
            hoSCRNRelayOff;
        }

		//Inv N
        if((wTestMode1Sec == 21) && (wTestMode20ms == 10))
        {
            hoInvNRelayOn;
        }
        else if((wTestMode1Sec == 22) && (wTestMode20ms == 35))
        {
            hoInvNRelayOff;
        }
		//N PE
        if((wTestMode1Sec == 23) && (wTestMode20ms == 10))
        {
            hoNPERelayOn;
        }
        else if((wTestMode1Sec == 24) && (wTestMode20ms == 35))
        {
            hoNPERelayOff;
        }

		//Gen L2
        if((wTestMode1Sec == 25) && (wTestMode20ms == 10))
        {
            hoGenSmartLoadL2RelayOn;
        }
        else if((wTestMode1Sec == 26) && (wTestMode20ms == 35))
        {
            hoGenSmartLoadL2RelayOff;
        }
...
//		hoPVSPS_ON;
    ...
    if(wTestModeFaultId[i] == 0)
    {
        if((swAvgValueCal(&AcSampData[INV_VOLT].SampCal.AvgSampSum) < 0) || (swAvgValueCal(&AcSampData[INV_VOLT].SampCal.AvgSampSum) > 200))
        {
            if(++wFaultCnt[i] >= 25)
            {
                wFaultCnt[i] = 0;
    ...
    if(wTestModeFaultId[i] == 0)
    {
        if(abs(swAvgValueCal(&AcSampData[INV_L1_CURR].SampCal.AvgSampSum)) > 100)
        {
            if(++wFaultCnt[i] >= 25)
            {
                wFaultCnt[i] = 0;
    ...
    if(wTestModeFaultId[i] == 0)
    {
        if(abs(swAvgValueCal(&AcSampData[OUT_L1_CURR].SampCal.AvgSampSum)) > 100)
        {
            if(++wFaultCnt[i] >= 25)
            {
                wFaultCnt[i] = 0;
    ...
    if(wTestModeFaultId[i] == 0)
    {
        if((swAcRmsCal(&AcSampData[GRID_VOLT]) < 0) || swAcRmsCal(&AcSampData[GRID_VOLT]) > 200)
        {
            if(++wFaultCnt[i] >= 25)
            {
                wFaultCnt[i] = 0;
...
void RelaySignalDeal(void)
{
    ...
    if(hoInv1Relay&&(hoInv1Relay != uwInvRelayLastState)  ||\
	  ...
	  (hoInvNRelay&&(hoInvNRelay != uwInvNRelayLastState))||\
      (hoGridRelay&&(hoGridRelay != uwGridRelayLastState))||\
      (hoGridL2Relay&&(hoGridL2Relay != uwGridL2RelayLastState))||\
      (hoNPERelay&&(hoNPERelay != uwNPERelayLastState))||\
      (hoGenSmartLoadRelay&&(hoGenSmartLoadRelay != uwGenRelayLastState))||\
      (hoGenSmartLoadL2Relay&&(hoGenSmartLoadL2Relay != uwGenL2RelayLastState)))
    {
        hoRelayKeepOn;
        RelayStateChanged = 1;
    }
    ...
    else if((!hoInv1Relay)&&(!hoInv2Relay)&&(!hoGridRelay)&&(!hoGenSmartLoadRelay)&&(!hoNPERelay)&&\
		    (!hoInvNRelay)&&(!hoGridL2Relay)&&(!hoGenSmartLoadL2Relay))
    {
	...
	uwInvNRelayLastState = hoInvNRelay;
    uwGridRelayLastState = hoGridRelay;
	uwGridL2RelayLastState = hoGridL2Relay;
    uwGenRelayLastState = hoGenSmartLoadRelay;
	uwGenL2RelayLastState = hoGenSmartLoadL2Relay;
	uwNPERelayLastState = hoNPERelay;
}
...
void sSetInvProtectPwm(void)
{
    OS_ENTER_CRITICAL();
	//30K
	EPwm7Regs.CMPA.bit.CMPA = 1342;    //1342/1667=80.5%    
    if(g_wVAType == cb12KVAModel)
	{
    	EPwm7Regs.CMPA.bit.CMPA = 1440;    //1440/1667=86%    
	}
	EPwm7Regs.AQCTLA.all = 0;
	EPwm7Regs.AQCTLA.bit.CAD = AQ_SET;
	EPwm7Regs.AQCTLA.bit.CAU = AQ_CLEAR;
	EPwm7Regs.AQCSFRC.bit.CSFA = 0x0;
    OS_EXIT_CRITICAL();
}
...
void sSetPvControlStatus(INT16U wStatus)
{
    ...
    {
        // EPwm5Regs.AQCSFRC.all = 0x5;//关闭Boost
