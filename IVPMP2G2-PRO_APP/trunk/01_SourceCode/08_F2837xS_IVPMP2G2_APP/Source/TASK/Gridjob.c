...
INT16S wGridL1L2Phase120Cnt = 0;//市电L1L2相差120
INT16S wGridL1L2Phase180Cnt = 0;//市电L1L2相差180
INT16S g_wGridVoltInputType = 0;//0:裂项 1: 2/3相

F32 g_fGridPeakVoltFilt = 0;
F32 g_fGenPeakVoltFilt = 0;
/********************************************************************************
...
void sGridTask(void)
{
    ...
    while(1)
    {
        ...
        {
            sRLineVoltAdj(swAcRmsCal(&AcSampData[GRID_VOLT]));
            sRLineVoltFilter(100);
            sRLineCurrAdj(swAcRmsCal(&AcSampData[GRID_L1_CURR]));
            sRLineCurrFilter(50);
			sRLineL1CurrAdj(swGetRLineCurrNew());
            sRLineL1CurrFilter(50);
			sRLineL2CurrAdj(swAcRmsCal(&AcSampData[GRID_L2_CURR]));
            sRLineL2CurrFilter(50);
            sLineRms3timeAvgUpdate();
            sRLineVoltChk(5);        //  3

			sLineL1VoltAdj(swAcRmsCal(&AcSampData[GRID_L1_VOLT]));
			sLineL2VoltAdj(swAcRmsCal(&AcSampData[GRID_L2_VOLT]));

            sLineFreqCal(swGetLinePeriodNew());
            sLineFreqFilter(50);//0.1Hz
            sLineFreqChk(5);     //  3
            sLineZeroLossClr();
            sLineModuleUpdate();
            sBatLinePeakCompare();
				
            sLineL1WattCal(sdwAcWattCal(&WattSampData[GRID_L1_WATT]));      //WattSampData[GRID_L1_WATT]  VLine*ILine
            sLineL1VACal(sdwACVACal((INT16S)(swGetLineL1VoltNew()),swGetRLineL1CurrNew()));
            sLineL2WattCal(sdwAcWattCal(&WattSampData[GRID_L2_WATT]));      //WattSampData[GRID_L1_WATT]  VLine*ILine
            sLineL2VACal(sdwACVACal((INT16S)(swGetLineL2VoltNew()),swGetRLineL2CurrNew())); 

			sLineWattCal(sbGetLineL1Watt()+sbGetLineL2Watt());      //WattSampData[GRID_L1_WATT]  VLine*ILine
            // sLineVACal(swGetRLineVolt());      //LineVA
            sLineVACal(sbGetLineL1VA()+sbGetLineL2VA());      //LineVA
        ...
        {
            sGenVoltAdj(swAcRmsCal(&AcSampData[GEN_VOLT]));
            sGenVoltFilter(100);
            sGenCurrAdj(swAcRmsCal(&AcSampData[GEN_L1_CURR]));
            sGenCurrFilter(50);
			sGenL1CurrAdj(swGetGenCurrNew());
            sGenL1CurrFilter(50);
			sGenL2CurrAdj(swAcRmsCal(&AcSampData[GEN_L2_CURR]));
            sGenL2CurrFilter(50);
			...
			sGenL1WattCal(sdwAcWattCal(&WattSampData[GEN_L1_WATT]));      //WattSampData[GRID_L1_WATT]  VLine*ILine
            sGenL1VACal((sdwACVACal((INT16S)(swGetGenVolt()>>1),swGetGenL1CurrNew())));
			sGenL2WattCal(sdwAcWattCal(&WattSampData[GEN_L2_WATT]));      //WattSampData[GRID_L1_WATT]  VLine*ILine
            sGenL2VACal((sdwACVACal((INT16S)(swGetGenVolt()>>1),swGetGenL2CurrNew())));
			sGenWattCal(sbGetGenL1Watt()+sbGetGenL2Watt());      //WattSampData[GRID_L1_WATT]  VLine*ILine
            // sGenVACal(swGetGenVolt());      //OPVA4
            sGenVACal(sbGetGenL1VA()+sbGetGenL2VA());
...
//	static INT16U wAcAdjChkCnt3 = 0;
    ...
    INT16U wAcL1L2CurrMax;    // L1L2最大电流
...
//    wAcChgCurrMax = 152;
	...
	if(sbGetEepromACCurrLimEn())//AC限流
	{
        ...
        if(swGetRLineL1Curr() > swGetRLineL2Curr())
        {
            wAcL1L2CurrMax = swGetRLineL1Curr();
        }
        else
        {
            wAcL1L2CurrMax = swGetRLineL2Curr();
        }
		if(wAcL1L2CurrMax < (sbGetEepromPeakShavingCurr()*10 - 5))
		{
			uwAcLimitChgCurr += 1;
		...
		else if(wAcL1L2CurrMax > (sbGetEepromPeakShavingCurr()*10))
		{
...
void sCalReactiveCurr220V(void)
{
    ...
    wFixCapaReactivePower= -(INT16S)((((INT32S)wReactiveCurr220V * swGetRLineVoltNew() *7) /22 )>>10);//Amplify 700 times;
     // eg: AcSampData[GRID_VOLT]..SampFilt.wFiltredValue= 2300 , wFixReactive=wReactiveCurr220V * 2300 *400 /2200 /1024
}
...
void    sSYNZeroCrossLossChk(INT8U  bFilter)
{
    if(liHOSTRXD == 1 && swGetEepromOutputMode() != cOP_Single)
    {
        wSYNZeroCnt++;
...
void sGridPortWorkModeUpdate()
{
    ...
    INT16U uwTOUGridExitChargeVolt = 0;

    if(wTOUECOEnFlg)
    {   
        if(wTOUGridChargeEnFlg && (g_LineModeJoinInWay == cLineModeJoinInByLine))
        {
            if(wTOUChargeVolt > swGetEepromOnGridBatAutoExitChargeVolt())
            {
                wTOUChargeVolt = swGetEepromOnGridBatAutoExitChargeVolt();
            }
            if(wTOUChargeSOC > swGetEepromOnGridBatAutoExitChargeSOC())
            {
                wTOUChargeSOC = swGetEepromOnGridBatAutoExitChargeSOC();
            }
        }
        uwTOUGridExitChargeVolt = wTOUChargeVolt+15;// 1.5V
        if(uwTOUGridExitChargeVolt > 600)//60V
        {
            uwTOUGridExitChargeVolt = 600;
        }

        if(wTOUGridChargeEnFlg &&\
		((((swGetEepromBatMode() != cLiBMode)&&(swGetBatAvgVoltNew() < wTOUChargeVolt)))||\
        ...
        else if((!wTOUGridChargeEnFlg)||(wTOUGridChargeEnFlg &&\
            ((((swGetEepromBatMode() != cLiBMode)&&(swGetBatAvgVoltNew() >= uwTOUGridExitChargeVolt)))||\
            ((swGetEepromBatMode() == cLiBMode)&&(fBMSConnectFlg)&&(mBMSBatSOC >= wTOUChargeSOC))||\
            ((swGetEepromBatMode() == cLiBMode)&&(!fBMSConnectFlg)&&(swGetBatAvgVoltNew() >= uwTOUGridExitChargeVolt)))))
        {
...
void sLinePowerPercentCal()
{
	...
	INT16S wLineCurrL1Percent = 0;
    INT16S wLineCurrL2Percent = 0;

	// wLineVAPercent = ((INT32S)swGetRLineVolt()*swGetRLineCurr()/wVARated);
	// wLineCurrL1Percent = ((INT32S)swGetRLineCurr()*100/wCurrRated);
    wLineVAPercent = ((INT32S)sbGetLineVA()/wVARated);
	wLineCurrL1Percent = ((INT32S)swGetRLineL1Curr()*100/wCurrRated);
    wLineCurrL2Percent = ((INT32S)swGetRLineL2Curr()*100/wCurrRated);

	if(wLineVAPercent < wLineCurrL1Percent)
	{
        wLineVAPercent = wLineCurrL1Percent;
	}
    if(wLineVAPercent < wLineCurrL2Percent)
	{
        wLineVAPercent = wLineCurrL2Percent;
	}
...
void sGenPowerPercentCal()
{
    ...
    if(sbGetEepromGenInputMaxPower()>(INT16U)wVARated)
	{
		wGenVAPercent = ((INT32S)(sbGetGenVA()*100)/wVARated);
	}
	else
	{
		wGenVAPercent = ((INT32S)(sbGetGenVA()*100)/(sbGetEepromGenInputMaxPower() + 300));
	}
    if(swGetGenL1Curr()>swGetGenL2Curr())
    {
        wGenCurrMax = swGetGenL1Curr();
    }
    else
    {
        wGenCurrMax = swGetGenL2Curr();
    }
    
	wwGenCurrPercent = ((INT32S)abs(wGenCurrMax)*100/wCurrRated);
...
void sLinePhaseSequenceCheck()//20ms
{
	// if((g_wGridL1L2PhaseError > 110)&&(g_wGridL1L2PhaseError < 145))//>105°且小于135°
	// {
	// 	wGridL1L2Phase120Cnt ++;
	// 	if(wGridL1L2Phase120Cnt > 150)//连续判断3S
	// 	{
	// 		wGridL1L2Phase120Cnt = 150;
	// 	}
	// 	if(wGridL1L2Phase180Cnt > 0)
	// 	{
	// 		wGridL1L2Phase180Cnt --;
	// 	}
	// }
	// else
	// {
	// 	wGridL1L2Phase180Cnt ++;
	// 	if(wGridL1L2Phase180Cnt > 150)
	// 	{
	// 		wGridL1L2Phase180Cnt = 150;
	// 	}
	// 	if(wGridL1L2Phase120Cnt > 0)
	// 	{
	// 		wGridL1L2Phase120Cnt --;
	// 	}
	// }
    if((g_wGridL1L2PhaseError > 189)&&(g_wGridL1L2PhaseError < 195))//>180°±2°
	{
		wGridL1L2Phase180Cnt ++;
		if(wGridL1L2Phase180Cnt > 150)
		{
	...
	if(wGridL1L2Phase120Cnt >= 150)
	{
		g_wGridVoltInputType = 1;//2/3相输入
	}
	else if(wGridL1L2Phase180Cnt >= 150)
	{
		g_wGridVoltInputType = 0;
	}
}

void AcVoltFiltCal()//20ms
{
	static INT16S wFiltCnt = 0;
	static F32 fGridVoltFiltTemp[4] = {0};
	static F32 fGenVoltFiltTemp[4] = {0};
    static F32 fGridVoltSumTemp = 0;
	static F32 fGenVoltSumTemp = 0;
    ...
    fGridVoltSumTemp -= fGridVoltFiltTemp[wFiltCnt];
    fGridVoltFiltTemp[wFiltCnt] = (F32)swGetRLineVoltNew();
    fGridVoltSumTemp += fGridVoltFiltTemp[wFiltCnt];
    g_fGridPeakVoltFilt = (F32)fGridVoltSumTemp * 0.3535f; // 1/3*1.414*0.75

    fGenVoltSumTemp -= fGenVoltFiltTemp[wFiltCnt];
    fGenVoltFiltTemp[wFiltCnt] = (F32)swGetGenVoltNew();
    fGenVoltSumTemp += fGenVoltFiltTemp[wFiltCnt];
    g_fGenPeakVoltFilt = fGenVoltSumTemp * 0.3535f; // 1/3*1.414
}
/********************************************************************************
...
INT16S sGetGridVoltInputType()
{
	return(g_wGridVoltInputType);
}
