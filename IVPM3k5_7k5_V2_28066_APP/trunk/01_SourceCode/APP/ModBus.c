...
#include    "MODULE\MPPTSci\MpptSciProcess.h"
...
extern INT32U sdwGetL1InvVA(void);
extern INT32U sdwGetL2InvVA(void);
extern INT16S swGetL1LoadPowerWatt(void);
extern INT16S swGetL2LoadPowerWatt(void);
...
INT8U g_ubRequestBMSId; // WIFIï¿½ï¿½ï¿½ï¿½ï¿½Ñ¯BMSï¿½ï¿½IDï¿½Å£ï¿½ï¿½ï¿½ï¿½Ý¸ï¿½IDï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ï¢ï¿½ï¿½ÊµÊ±ï¿½ï¿½Ï¢ï¿½ï¿½Ïºï¿½ï¿½ï¿½ï¿½ï¿???0x5100ï¿½ï¿½Ê¼ï¿½Ä¼Ä´ï¿½ï¿½ï¿½ï¿½Ð£ï¿½ï¿½Ô¹ï¿½WIFIï¿½ï¿½Ñ¯
...
INT16U swModBusRdProc(INT16U *RxBuff, INT16U RxLen)
{
	...
	if(RdNums > 64) // add in 20241015 ï¿½Þ¶ï¿½ï¿½ï¿½È¡ï¿½ï¿½ï¿½È£ï¿½ï¿½ï¿½Ö¹ï¿½ï¿½Ñ¯ï¿½ï¿½ï¿½È³ï¿½ï¿½ï¿½64ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ð¶ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ì³£ï¿½ï¿½Êµï¿½ï¿½ï¿½Ñ¯ÊµÊ±ï¿½ï¿½ï¿½Ý´ï¿½ï¿½ï¿???80ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Î»ï¿½ï¿½ï¿???
	{
		swBuildFaultFrame(RxBuff, cErr_AskLengthErr);
		return 0;
	}
	...
	else if((RdAddr >= IOT_DATA_BASE_ADDR) && (RdAddr < (IOT_DATA_BASE_ADDR + IOT_DATA_LEN)))
    {
		...
		{
	    	swBuildRdFrame(RxBuff, cIOT_DATA_RD_REGION);
	        return 1;
		}
		else
		{
	...
	else if((RdAddr >= BMS_INFO_REG_BASE_ADDR) && (RdAddr < (BMS_INFO_REG_BASE_ADDR + cBMSInfoDataLen)))
	{
	    ...
	    {
	    	swBuildRdFrame(RxBuff, cBMSINFO_RD_REGION);
	        return 1;
	    }
	    else
	    {
...
INT16U swModBusWrProc(INT16U *RxBuff, INT16U RxLen)
{
	...
	else if((WrAddr >= IOT_DATA_BASE_ADDR) && (WrAddr < (IOT_DATA_BASE_ADDR + IOT_DATA_LEN)))
	{
	    ...
	    {
	    	swBuildWrFrame(RxBuff, swWriteIotData);
			g_uwWifiLossCnt = 6000; // 300S = 6000*50mS
...
INT16U swWriteEESetting(INT16U WrAddr, INT16U WrData)
{
	...
	switch(WrAddr)
	{
		...
		case E1_L2OpCurrAdj:
			if((1548 <= WrData) && (WrData <= 2548))
			{
				if(WrData != swGetEEL2OPCurrAdj())
				{
					sSetEEL2OPCurrAdj(WrData);
					OSEventSend(cPrioInterface, eEepromSaveFactorySetData);
				}
			}
			else
		...
		case E2_BattType:
			if(WrData <= cPylonBType)
			{
				if(sbGetEepromBatteryType() != WrData)
				{
					...
					//------ ï®µï¿½ï¿½ï¿½ï¿½ï¿½Í¶ï¿½ï¿½Ô¶ï¿½ï¿½ï¿½ï¿½?9600bpsï¿½ï¿½ï¿½ï¿½ï®µï¿½Ä¶ï¿½ï¿½Ô¶ï¿½ï¿½ï¿½ï¿½?2400bps ------//  // LMZ add in 20240430
		...
		case E2_MenuRtnDftPageEn:
			if(WrData <= 1)
			{
				// if(sbGetEepromBMSSetEn() != WrData) //ï¿½Ì¶ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½9600ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
				// {
		...
		break;
		case E3_BmsReqID:
			if((WrData > 0) && (WrData <= 15))
			{
				g_ubRequestBMSId = WrData;
			}
			else
			{
				ret = cErr_ParUnValid;
...
//		break;
		case ATE_UpdateFW:
			if(WrData <= 6)
			{
				sMpptUpdateCommand(1); //Ö»ï¿½ï¿½ï¿½ï¿½ï¿½Ó´ï¿½ï¿½Ú½ï¿½ï¿½ï¿½Í¸ï¿½ï¿½Ä£Ê½ï¿½ï¿½ï¿½Ë³ï¿½ï¿½ï¿½ï¿½ï¿½Îªï¿½ï¿½ï¿½ï¿½ï¿½Ý½ï¿½ï¿½ï¿½ï¿½ï¿½30Sï¿½ï¿½Ê±ï¿½Ë³ï¿½
			}
			else
			{
				ret = cErr_ParUnValid;
...
//		break;
		...
		break;
		case ATE_L2OpCurrAdj0:
			if(WrData == cEnable)
			{
				if(swGetEEL2OPCurrAdj() != 2048)
				{
					sSetEEL2OPCurrAdj(2048);
					OSEventSend(cPrioInterface, eEepromSaveFactorySetData);
				}
			}
			else
		...
		break;
		case ATE_L2OpCurrAdj2:
			if((-8 <= (INT16S)WrData) && ((INT16S)WrData <= 8))
			{
				sSetEEL2OPCurrAdj((INT16S)swGetEEL2OPCurrAdj() + (INT16S)WrData);
				if(swGetEEL2OPCurrAdj() > 2548)
				{
					sSetEEL2OPCurrAdj(2524);
				}
				else if(swGetEEL2OPCurrAdj() < 1548)
				{
					sSetEEL2OPCurrAdj(1524);
				}
				OSEventSend(cPrioInterface, eEepromSaveFactorySetData);
			}
			else
...
extern STRBMSComSts	g_strBMSComSts[];
extern STRBMSComSts	g_strPylonBMSComSts[];
extern INT16U wParaMode;
void sRealTimeDataUpdate(void)
{
	INT16U wTemp = 0;
	INT32U dwTemp =0;
	// uRealTimeData.Stru.wSettingDataSN; // ï¿½ï¿½eepromï¿½ï¿½ï¿½ï¿½Ê±ï¿½Ñ¾ï¿½ï¿½Þ¸Ä£ï¿½ï¿½Ë´ï¿½ï¿½ï¿½ï¿½Î¡ï¿½
	...
	else if(bPVMode != cPowerOnMode)
	{
		...
		uRealTimeData.Stru.wFaultId = 0;
	}
	//----------------------------
	uRealTimeData.Stru.wBattVolt = swGetBatAvgVolt10New();
	if(true == g_bDischgFlag)  { uRealTimeData.Stru.wBattCurr = swDisChgAvgCurr(); }
	else
	{
		if(g_uwDischgCurr > 1) { uRealTimeData.Stru.wBattCurr = -g_uwDischgCurr; }
		else                   { uRealTimeData.Stru.wBattCurr = g_wSCCACChgCurr; }
	}
	uRealTimeData.Stru.wBattWatt = ((INT32S)uRealTimeData.Stru.wBattVolt * uRealTimeData.Stru.wBattCurr) / 100;
	if(fBMSConnectFlg)
	{
		uRealTimeData.Stru.wBattSOC = mBMSBatSOC*100;
	}
	else
	{
		uRealTimeData.Stru.wBattSOC = swGetBatCapPercent()*100;
	}
	//----------------------------
	...
	uRealTimeData.Stru.wRInvWatt = sdwGetInvWatt();	// ï¿½ï¿½ï¿½ï¿½Ñ¹*ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
	uRealTimeData.Stru.wRInvVA = sdwGetInvVA();		// ï¿½ï¿½ï¿½ï¿½Ñ¹*ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿???
	...
	//----------------------------
	uRealTimeData.Stru.wROpVolt = swGetRInverterVolt();	// ï¿½ï¿½ï¿½ï¿½
	...
	// { uRealTimeData.Stru.wRLineCurr = uRealTimeData.Stru.wRInvCurr+uRealTimeData.Stru.wROpCurr; } //swGetOPShareCurr();	// ï¿½ï¿½ï¿½ï¿½
	// else { uRealTimeData.Stru.wRLineCurr = 0; }
	// uRealTimeData.Stru.wRLineFreq = swGetLineFreq();
	//----------------------------
	uRealTimeData.Stru.wRLoadWatt = swGetLoadPowerWatt();	// µç³ØÄ£Ê½:Äæ±äµçÑ¹*Êä³öµçÁ÷; ÊÐµçÄ£Ê½:ÊÐµçµçÑ¹*Êä³öµçÁ÷
	if(swGetLoadPowerVA() > swGetLoadPowerWatt())
	{
		uRealTimeData.Stru.wRLoadVA = swGetLoadPowerVA();	// µç³ØÄ£Ê½:Äæ±äµçÑ¹*Êä³öµçÁ÷; ÊÐµçÄ£Ê½:ÊÐµçµçÑ¹*Êä³öµçÁ÷
	}
	else
	{
	...
	if(bPVMode == cLineMode)
	{
		...
		uRealTimeData.Stru.wRLineCurr = uRealTimeData.Stru.wRInvCurr + uRealTimeData.Stru.wROpCurr;
	}
	else
	{
	...
	uRealTimeData.Stru.wBatTempC = 0;//swGetTempDegreeBat();
	...
	uRealTimeData.Stru.wSccFaultMsg = wSccAlarmFlag;

	uRealTimeData.Stru.wOPShareCurr = swGetOPShareCurr();
	...
	if(sbGetEepromBatteryType() < cLiBType) { uRealTimeData.Stru.wBMSFlag = 0; }
	/***********/
	...
	//----- BMS summy data end -----//
	//----- BMS private data start -----//
	uRealTimeData.Stru.wRT_CurrBMSAddr = g_ubRequestBMSId;
	//--------------------
	wTemp = 0;
	if(fBMSConnectFlg) wTemp |= 0x0001;
	if(g_strBMSComSts[1].fBMSComOK  || g_strPylonBMSComSts[1].fBMSComOK)   wTemp |= 0x0002;
	if(g_strBMSComSts[2].fBMSComOK  || g_strPylonBMSComSts[2].fBMSComOK)   wTemp |= 0x0004;
	if(g_strBMSComSts[3].fBMSComOK  || g_strPylonBMSComSts[3].fBMSComOK)   wTemp |= 0x0008;
	if(g_strBMSComSts[4].fBMSComOK  || g_strPylonBMSComSts[4].fBMSComOK)   wTemp |= 0x0010;
	if(g_strBMSComSts[5].fBMSComOK  || g_strPylonBMSComSts[5].fBMSComOK)   wTemp |= 0x0020;
	if(g_strBMSComSts[6].fBMSComOK  || g_strPylonBMSComSts[6].fBMSComOK)   wTemp |= 0x0040;
	if(g_strBMSComSts[7].fBMSComOK  || g_strPylonBMSComSts[7].fBMSComOK)   wTemp |= 0x0080;
	if(g_strBMSComSts[8].fBMSComOK  || g_strPylonBMSComSts[8].fBMSComOK)   wTemp |= 0x0100;
	if(g_strBMSComSts[9].fBMSComOK  || g_strPylonBMSComSts[9].fBMSComOK)   wTemp |= 0x0200;
	if(g_strBMSComSts[10].fBMSComOK || g_strPylonBMSComSts[10].fBMSComOK)  wTemp |= 0x0400;
	if(g_strBMSComSts[11].fBMSComOK || g_strPylonBMSComSts[11].fBMSComOK)  wTemp |= 0x0800;
	if(g_strBMSComSts[12].fBMSComOK || g_strPylonBMSComSts[12].fBMSComOK)  wTemp |= 0x1000;
	if(g_strBMSComSts[13].fBMSComOK || g_strPylonBMSComSts[13].fBMSComOK)  wTemp |= 0x2000;
	if(g_strBMSComSts[14].fBMSComOK || g_strPylonBMSComSts[14].fBMSComOK)  wTemp |= 0x4000;
	if(g_strBMSComSts[15].fBMSComOK || g_strPylonBMSComSts[15].fBMSComOK)  wTemp |= 0x8000;
	//--------------------
	uRealTimeData.Stru.wRT_BMSConnectStatus = wTemp;
	uRealTimeData.Stru.wRT_BMSCharVoltLimit = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSRcmdChgVolt / 10);
	uRealTimeData.Stru.wRT_BMSDischarVoltLimit = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSRcmdCutoffVolt / 10);
	uRealTimeData.Stru.wRT_BMSCharCurrLimit = g_strBMSRTInfo[g_ubRequestBMSId].wBMSRcmdMaxChgCurr;
	uRealTimeData.Stru.wRT_BMSDischarCurrLimit = g_strBMSRTInfo[g_ubRequestBMSId].wBMSRcmdMaxDischgCurr;
	uRealTimeData.Stru.wRT_BmsStatusHi = 0;
	//-------- 0x1300Ð­Òé×ª0x0200µÄÐÂµØÖ·ºóÔÙ·¢ËÍºóÌ¨ --------//
	if(g_strBMSRTInfo[g_ubRequestBMSId].wBMSFlag | 0x0001)  wTemp |= 0x0080; // ³äµçÔÊÐí
	if(g_strBMSRTInfo[g_ubRequestBMSId].wBMSFlag | 0x0004)  wTemp |= 0x0040; // ·ÅµçÔÊÐí
	if(g_strBMSRTInfo[g_ubRequestBMSId].wBMSFlag | 0x0040)  wTemp |= 0x1000; // ·Åµç×´Ì¬
	if(g_strBMSRTInfo[g_ubRequestBMSId].wBMSFlag | 0x0080)  wTemp |= 0x2000; // ³äµç×´Ì¬
	//--------------------//
	uRealTimeData.Stru.wRT_BmsStatusLo = wTemp; // 0x1300Ð­Òé×ª0x0200µÄÐÂµØÖ·ºóÔÙ·¢ËÍºóÌ¨
	uRealTimeData.Stru.wRT_BmsFaultFlagHi = 0;
	//--------------------//
	uRealTimeData.Stru.wRT_BmsFaultFlagLo = g_strBMSRTInfo[g_ubRequestBMSId].wBMSFaultFlag;
	uRealTimeData.Stru.wRT_BmsAlarmFlagHi = 0;
	uRealTimeData.Stru.wRT_BmsAlarmFlagLo = g_strBMSRTInfo[g_ubRequestBMSId].wBMSAlarmFlag; //ÐÂ¾ÉµØÖ·Êý¾ÝÄÚÈÝÒ»ÖÂ£¬Ö±½Ó¸³Öµ
	uRealTimeData.Stru.wRT_BmsNoticeFlagHi = 0;
	uRealTimeData.Stru.wRT_BmsNoticeFlagLo = g_strBMSRTInfo[g_ubRequestBMSId].wBMSAlarmFlag;
	uRealTimeData.Stru.wRT_BmsTotalCurr = -g_strBMSRTInfo[g_ubRequestBMSId].wBatCurr; // ¶ÔÍâÐ­ÒéÖÐ³äµçÎªÕý£¬BMS¶ÁÈ¡µ½³äµçÎª¸º¡£
	uRealTimeData.Stru.wRT_BmsTotalVolt = g_strBMSRTInfo[g_ubRequestBMSId].wBatVolt;
	uRealTimeData.Stru.wRT_BmsReservd1 = 0;
	uRealTimeData.Stru.wRT_BmsReservd2 = 0;
	uRealTimeData.Stru.wRT_BmsTotalSOC = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSSoc * 10);
	uRealTimeData.Stru.wRT_BmsTotalSOH = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSSoh * 10);
	dwTemp = ((INT32U)(((uRealTimeData.Stru.wRT_SN[1]%10)*100)+(uRealTimeData.Stru.wRT_SN[2]/100)) * 1000);
	uRealTimeData.Stru.wRT_BmsTotalCapaHigh = (INT16U)(dwTemp >> 16);
	uRealTimeData.Stru.wRT_BmsTotalCapaLow = (INT16U)(dwTemp & 0xFFFF);
	uRealTimeData.Stru.wRT_BmsParallelNumber = 0; // 0x1300µØÖ·ÎÞ¸ÃÊý¾Ý£¬ÉÏ´«0
	uRealTimeData.Stru.wRT_BmsParallelStatus = 0; // 0x1300µØÖ·ÎÞ¸ÃÊý¾Ý£¬ÉÏ´«0
	uRealTimeData.Stru.wRT_BmsReservd3 = 0;
	uRealTimeData.Stru.wRT_BmsReservd4 = 0;
	uRealTimeData.Stru.wRT_BmsMaxCellVoltNo = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVoltMaxNo;
	uRealTimeData.Stru.wRT_BmsMaxCellVolt = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVoltMax;
	uRealTimeData.Stru.wRT_BmsMinCellVoltNo = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVoltMinNo;
	uRealTimeData.Stru.wRT_BmsMinCellVolt = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVoltMin;
	uRealTimeData.Stru.wRT_BmsMaxCellTempNo = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTempMaxNo;
	uRealTimeData.Stru.wRT_BmsMaxCellTemp = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTempMax * 10);
	uRealTimeData.Stru.wRT_BmsMinCellTempNo = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTempMinNo;
	uRealTimeData.Stru.wRT_BmsMinCellTemp = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTempMin * 10);
	uRealTimeData.Stru.wRT_SN[0] = g_strBMSBaseInfo[g_ubRequestBMSId].wSerialNum1;
	uRealTimeData.Stru.wRT_SN[1] = g_strBMSBaseInfo[g_ubRequestBMSId].wSerialNum2;
	uRealTimeData.Stru.wRT_SN[2] = g_strBMSBaseInfo[g_ubRequestBMSId].wSerialNum3;
	uRealTimeData.Stru.wRT_SN[3] = g_strBMSBaseInfo[g_ubRequestBMSId].wSerialNum4;
	uRealTimeData.Stru.wRT_SN[4] = g_strBMSBaseInfo[g_ubRequestBMSId].wSerialNum5;
	//----- BMS private data end -----//
	//----- IVPA data start-----//
	uRealTimeData.Stru.wL1OpVolt = 0;
	uRealTimeData.Stru.wL1OpCurr = 0;
	uRealTimeData.Stru.wL1OpFreq = 0;
	uRealTimeData.Stru.wL1OpWatt = 0;
	uRealTimeData.Stru.wL1OpVA   = 0;
	uRealTimeData.Stru.wL2OpVolt = 0;
	uRealTimeData.Stru.wL2OpCurr = 0;
	uRealTimeData.Stru.wL2OpFreq = 0;
	uRealTimeData.Stru.wL2OpWatt = 0;
	uRealTimeData.Stru.wL2OpVA   = 0;
	//----- IVPA data end-----//
	uRealTimeData.Stru.wRT_BMSCellVolt[0] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt01;
	uRealTimeData.Stru.wRT_BMSCellVolt[1] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt02;
	uRealTimeData.Stru.wRT_BMSCellVolt[2] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt03;
	uRealTimeData.Stru.wRT_BMSCellVolt[3] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt04;
	uRealTimeData.Stru.wRT_BMSCellVolt[4] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt05;
	uRealTimeData.Stru.wRT_BMSCellVolt[5] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt06;
	uRealTimeData.Stru.wRT_BMSCellVolt[6] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt07;
	uRealTimeData.Stru.wRT_BMSCellVolt[7] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt08;
	uRealTimeData.Stru.wRT_BMSCellVolt[8] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt09;
	uRealTimeData.Stru.wRT_BMSCellVolt[9] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt10;
	uRealTimeData.Stru.wRT_BMSCellVolt[10] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt11;
	uRealTimeData.Stru.wRT_BMSCellVolt[11] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt12;
	uRealTimeData.Stru.wRT_BMSCellVolt[12] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt13;
	uRealTimeData.Stru.wRT_BMSCellVolt[13] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt14;
	uRealTimeData.Stru.wRT_BMSCellVolt[14] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt15;
	uRealTimeData.Stru.wRT_BMSCellVolt[15] = g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellVolt16;
	uRealTimeData.Stru.wRT_BMSCellTemp[0] = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTemp01 * 10);
	uRealTimeData.Stru.wRT_BMSCellTemp[1] = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTemp02 * 10);
	uRealTimeData.Stru.wRT_BMSCellTemp[2] = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTemp03 * 10);
	uRealTimeData.Stru.wRT_BMSCellTemp[3] = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTemp04 * 10);
	uRealTimeData.Stru.wRT_BMSCellTemp[4] = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTemp05 * 10);
	uRealTimeData.Stru.wRT_BMSCellTemp[5] = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTemp06 * 10);
	uRealTimeData.Stru.wRT_BMSCellTemp[6] = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTemp07 * 10);
	uRealTimeData.Stru.wRT_BMSCellTemp[7] = (g_strBMSRTInfo[g_ubRequestBMSId].wBMSCellTemp08 * 10);
	//----- MPPT Alarm data -----//
	uRealTimeData.Stru.wRT_SccFault[0] = wSccFaultFlag;
	uRealTimeData.Stru.wRT_SccFault[1] = wSccFaultHiFlag;
	uRealTimeData.Stru.wRT_SccAlarm[0] = suwGetMpptChgAlarm();
	uRealTimeData.Stru.wRT_SccAlarm[1] = suwGetMpptChargerAlarmHi();
}
...
void sFaultDataUpdate(void)
{
	...
	uFaultData.Stru.wEEBusVolt = 0;//sbGetEepromBusVolt();
...
void sInfoDataUpdate(void)
{
	INT16U uwTemp;
	#ifdef	IVPA
	uwTemp =swGetBatteryPcs()+0x10;
	#else
	uwTemp =swGetBatteryPcs();
	#endif
	...
	uInfoData.Stru.wDispSwVer = cDebugVersion; //ÏÔÊ¾µ÷ÊÔ°æ±¾£¬Ã¿´Îµ÷ÊÔµÝÔö1
...
void sOscSettingUpdate(void)
{
	...
	if(uRecordCmd.Stru.wOscStatus != OscStatusPre)
	{
		...
		else if(uRecordCmd.Stru.wOscStatus == 1)
		{
			...
			wRecordStatus = 1;
			uRecordCmd.Stru.wOscStatus = 2;	// Â¼²¨ÖÐ
...
void sOscSnatchGraph(void)
{
	...
	if(wRecordTrigEn)
	{
		if(++wRecordTrigCnt >= wRecordTrigLen)
		{
			...
			wRecordStatus = 2;	// Â¼²¨Íê³É£¬µ«»¹ÐèÒªÕûÀíÊý¾Ý
		}
	}
}
...
void sSnatchGraph(void)
{
	...
	if(wGraphWaitFaultFlag  == 1)
	{
		//for(i=0;i<4;i++)             //ï¿½ï¿½ï¿½ï¿½faultÊ±ï¿½ï¿½ï¿½ï¿½Í¨ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½4
		//{
...
// ÓÉÓÚÂ¼²¨º¬ÓÐ´¥·¢¹¦ÄÜ£¬Êý×éÊÇÏÈ½øÏÈ³ö½á¹¹
// Òò´ËÊý¾Ý²»Ò»¶¨ÊÇ¾ÉÊý¾ÝÔÚÊý×é¿ªÊ¼Î»ÖÃ,ÐÂÊý¾ÝÔÚÊý×éÄ©Î²Î»ÖÃ
// ËùÒÔÊý¾ÝÐèÒªÖØÐÂÕûÀíÔÙÊ¹ÓÃ
void sOscDataBuffProc(void)
{
	INT16U i;
	INT16U wStart, wHead, wEnd;
	
	if(wRecordStatus != 2)	// ²»´¦ÓÚÂ¼²¨ÖÐ
	{
		return;
	}
	...
	wRecordStatus = 0;
	uRecordCmd.Stru.wOscStatus = 3;	// Â¼²¨Íê³É
}
