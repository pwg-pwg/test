...
extern INT16U g_uwBMSPollingNum;
extern INT16U g_uwPylonBMSPollingNum;
...
{  
   ...
   {3, charM, charO, charD},      // Display MOD
};
...
enum  BAT_START_VOLT_ID
{
	...
	BAT_START_51V5_ID,
	BAT_START_52V_ID,
	MAX_BAT_START_VOLT_ID
};

const INT8U cBatStartVolt48TAB[][4]=
{
	{3, char4, char4, char0},
	{3, char4, char4, char5},
	{3, char4, char5, char0},
...
const INT8U cBatStartVolt24TAB[][4]=
{
	{3, char2, char2, char0},
	{3, char2, char2, char3},
	{3, char2, char2, char5},
...
const INT8U cBatStartVolt12TAB[][4]=
{
	{3, char1, char1, char0},
	{3, char1, char1, char1},
	{3, char1, char1, char3},
...
void sDisplayOPWatt(INT16U wDisPlayWatt);
...
void sDisplayL1VolCur(void);
void sDisplayL2VolCur(void);



// #pragma CODE_SECTION(sModeDisplay,"Codefuns2");//sSaveConfiguration
// #pragma CODE_SECTION(sModeDisplay,"ramfuncs");//
// #pragma CODE_SECTION(sSelectDataDisplay,"Codefuns2");
...
{ 
	INT16U Temp;
    if(swGetEEMachineModelOnOFF()==1) 
	{
		if(18 == swGetEepromIDLength())
		{
			Temp = swGetEepromwSerial1() + swGetEepromwSerial2() + (swGetEepromwSerial3()/100);
		 	    switch (Temp)
				{       
                 case 478:																//IVPA
				 {
					sSendBitToVram( cBIT_Machine_IVPA3524 , DISPLAY_ON, cALWAYS_LIGHT);
                    break;
				 }
                 case 652:
                 {
	                sSendBitToVram( cBIT_Machine_IVPA5048 , DISPLAY_ON, cALWAYS_LIGHT);
                	break;
                 }
                 case 902:
                 {
                    sSendBitToVram( cBIT_Machine_IVPA7548 , DISPLAY_ON, cALWAYS_LIGHT);
                    break;
                 }
                 case 1152:
                 {
                    sSendBitToVram( cBIT_Machine_IVPA10048 , DISPLAY_ON, cALWAYS_LIGHT);
					break;
                 }
                 case 651:                                                              //IVPM
				 {
					sSendBitToVram( cBIT_Machine_IVPM5048 , DISPLAY_ON, cALWAYS_LIGHT);
					break;
				 }
                 case 901:
                 {
                    sSendBitToVram( cBIT_Machine_IVPM7548 , DISPLAY_ON, cALWAYS_LIGHT);
                    break;
                 }
                 case 1151:
                 {
                    sSendBitToVram( cBIT_Machine_IVPM10048 , DISPLAY_ON, cALWAYS_LIGHT);
                    break;
                 }
                 case 184:                                                              //IVPS
                 {
                    sSendBitToVram( cBIT_Machine_IVPS0712 , DISPLAY_ON, cALWAYS_LIGHT);
                    break;
                 }
                 case 264:
				 {
					sSendBitToVram( cBIT_Machine_IVPS1512 , DISPLAY_ON, cALWAYS_LIGHT);
					break;
				 }
				 case 626:
				 {
				    sSendBitToVram( cBIT_Machine_IVPS5024 , DISPLAY_ON, cALWAYS_LIGHT);
					break;
				 }
				 case 650:
				 {
					sSendBitToVram( cBIT_Machine_IVPS5048 , DISPLAY_ON, cALWAYS_LIGHT);
					break;
				 }
				 case 900:
				 {
					sSendBitToVram( cBIT_Machine_IVPS7548 , DISPLAY_ON, cALWAYS_LIGHT);
					break;
				 }
				 case 1150:
				 {
					sSendBitToVram( cBIT_Machine_IVPS10048 , DISPLAY_ON, cALWAYS_LIGHT);
					break;
				 }
				 case 364:
				 {
					sSendBitToVram( cBIT_Machine_IVPS2512 , DISPLAY_ON, cALWAYS_LIGHT);
					break;
				 }
				 case 376:
				 {
					sSendBitToVram( cBIT_Machine_IVPS2524 , DISPLAY_ON, cALWAYS_LIGHT);
					break;
				 }
				 case 476:
				 {
					sSendBitToVram( cBIT_Machine_IVPS3524 , DISPLAY_ON, cALWAYS_LIGHT);
					break;
				 }
				 default: break;
		...
		// else if(14 == swGetEepromIDLength())                                                    //IVPS
		// {
		// 	Temp = (swGetEepromwSerial1()/100) +(swGetEepromwSerial1()%100)+ (swGetEepromwSerial2()/100);
		//  	switch (Temp)
		// 	{
		// 		case 39:
		// 		 {
		...
		// 	}
		// }
        else // SN is not 14 or 18, close model display
        {
            sSetEEMachineModelOnOFF(0);
        }
...
{ 
	...
	if((sbGetEepromBatteryType() == cLiBType) || (sbGetEepromBatteryType() == cPylonBType))
	{
		...
		if(fBMSConnectFlg) { mSetVramBit(cBIT_Bat_C, cALWAYS_LIGHT); }
	}
    //------------ WIFI State Display add in 20241227 ---------------//
	/*	Èç¹ûÁ¬ÉÏ·þÎñÆ÷(=3)£¬WIFI³£ÁÁ;
		Èç¹ûIOTÉè±¸Ã»ÓÐÓëMCU½¨Á¢Á¬½Ó(=0)£¬WIFIÃð;
		Èç¹ûÁ¬½Ó´íÎó(=1)£¬WIFIÁÁ1Ãë£¬Ãð2Ãë
		Èç¹ûÕýÔÚÁ¬½Ó(=4)£¬WIFIÁÁ1Ãë£¬Ãð2Ãë;
		Èç¹ûÁ¬ÉÏÂ·ÓÉÆ÷(=2)£¬WIFIÁÁ1Ãë£¬Ãð1Ãë;
		Èç¹ûÕýÔÚÉý¼¶IOTÉè±¸³ÌÐò(=5)£¬WIFIÁÁ0.5Ãë£¬Ãð0.5Ãë:  */
	if(uIotData.Stru.wState == 1)      { uwFlashTime = cFLASH_TIME_1S; mSetVramBit(cBIT_WIFI, uwFlashTime); }
	else if(uIotData.Stru.wState == 2) { uwFlashTime = cFLASH_TIME_1S; mSetVramBit(cBIT_WIFI, uwFlashTime); }
    else if(uIotData.Stru.wState == 3) { mSetVramBit(cBIT_WIFI, cALWAYS_LIGHT); }
	else if(uIotData.Stru.wState == 4) { uwFlashTime = cFLASH_TIME_1S; mSetVramBit(cBIT_WIFI, uwFlashTime); }
	else if(uIotData.Stru.wState == 5) { uwFlashTime = cFLASH_TIME_0S5; mSetVramBit(cBIT_WIFI, uwFlashTime); }
    else                               { mClearVramBit(cBIT_WIFI); }
}
...
{  
	...
	else
	{
		...
		{	
			...
			{  	
				bSlectIndex++;	  		  
				if((bSlectIndex == SELECT_PV_VOL_ID)||(bSlectIndex == SELECT_PV_POWER_ID)) 
				{
					if((fIntSccSciLoss == true) && (fExtSccSciLoss == true) \
						&& (swGetEEMpptChg() == cDisable))
					{
						bSlectIndex=SELECT_CHG_CURR_ID;
					}
				}				
				...
				#ifdef	SECIVPM
					if(bSlectIndex == SELECT_L1_VOL_CUR) 
					{
						bSlectIndex = SELECT_BAT_BMS_ID; 
					}
				#endif				
				if(bSlectIndex == SELECT_BAT_BMS_ID && !fBMSConnectFlg) //BMS?????,???SOC??
				{
					bSlectIndex ++;
				}
			...
			else 
			{
				...
				#ifdef	SECIVPM
				if(bSlectIndex == SELECT_L2_VOL_CUR) 
				{
					bSlectIndex = SELECT_CPU2_Version; 
				}
				#endif
				if(bSlectIndex==SELECT_CPU2_Version)
				{
				...
				if((bSlectIndex == SELECT_PV_POWER_ID)||(bSlectIndex == SELECT_PV_VOL_ID)) 
				{
					if((fIntSccSciLoss == true) && (fExtSccSciLoss == true) \
						&& (swGetEEMpptChg() == cDisable))
					{
						bSlectIndex=SELECT_BAT_VOLT_ID;
					}
				}
...
{	
	...
	else if(bSlectIndex == SELECT_CHG_CURR_ID)
	{
		sDisplayChgCurr();
		sDisplayOPVol();
	}
	...
	{
		sDisplayBatVol();
		sDisplayOPWatt(swGetLoadPowerWatt());
	}              
	...
	else if(bSlectIndex == SELECT_L1_VOL_CUR)
	{
		sDisplayL1VolCur();
	 	sDisplayOPWatt(swGetL1LoadPowerWatt());
	}
	else if(bSlectIndex == SELECT_L2_VOL_CUR)
	{
		sDisplayL2VolCur();
		sDisplayOPWatt(swGetL2LoadPowerWatt());
	}
	else if(bSlectIndex == SELECT_BAT_BMS_ID)
	{
		sDisplayBatBMSSOC();
		sDisplayBatBMSNum();
	}
...
//	bMipOffAlarmIDTemp = bMipOffAlarmID;
	...
	if(swGetBatteryPcs()==cbBatt48VType)
	{
		bBattStartVoltID = (INT8U)((swGetEEBattStartVolt() - cBat44v) / 5);
	}
	else if(swGetBatteryPcs()==cbBatt24VType)
	{
		bBattStartVoltID = (INT8U)(((swGetEEBattStartVolt() - cBat22v) * 13 + 16) >> 5);
	}
	else
	{
		bBattStartVoltID = (INT8U)(((swGetEEBattStartVolt() - cBat11v) * 13 + 8) >> 4);	
	}
	...
	*/
	...
	if((bBatTypeID == AGM_ID) || (bBatTypeID == LIB_ID)|| (bBatTypeID == PYL_ID))
	{
		bEqEnaID = false;
	}
	else
		...
		{      
			...
			{	
				...
				*/
				...
				{
					if( (bMenuIndex == CF_BM_INTERVAL_ID) \
				...
				if((g_wAcChgCurrMax == cIdc50A)&&(bMenuIndex == CF_MAX_CHG_CUR_ID))
				{wAdjustValue=21420;}//4*5*7*9*17
				//fPanleJob.SelectMode = 1;
				bModeTimeOut = CF_MODE_EXIT_TIME;                                 
			}            
				...
				if(bIsAdjustVal != 2)
				{
					...
					else
					{
						if(bMenuIndex != CF_MAX_SOLAR_CHG_CUR_ID)
						{
							...
							bModeTimeOut = CF_MODE_EXIT_TIME;
							if((g_wAcChgCurrMax == cIdc50A)&&(bMenuIndex == CF_MAX_CHG_CUR_ID))
							{wAdjustValue=21420;}//4*5*7*9*17 //¼æÈÝ5048×î´ó³äµçµçÁ÷ÉèÖÃÁ¿ÏÎ½Ó²»Ìø¶Î
						}
					}
				}
			}
		...
		else
		{
			if(bIsAdjustVal == 1) 
			{
				...
				|| (cInvOnKeyPressBit == wKeyVal)) 
				{
					...
					{
						if(g_wAcChgCurrMax == cIdc50A)
						{
							bMaxChgCurrentIDTemp = (INT16U)(bMaxChgCurrentID + wAdjustValue) % (INT8U)MEX_MAX_CHG_180A_ID;//5048
						}
						else if(g_wAcChgCurrMax == cIdc60A)
						{
							bMaxChgCurrentIDTemp = (INT16U)(bMaxChgCurrentID + wAdjustValue) % (INT8U)MEX_MAX_CHG_190A_ID;//7548
						}
						else if(g_wAcChgCurrMax == cIdc80A)
						{
							bMaxChgCurrentIDTemp = (INT16U)(bMaxChgCurrentID + wAdjustValue) % (INT8U)MEX_MAX_CHG_CUR_ID;//10048
						}
						else
						{
							bMaxChgCurrentIDTemp = (INT16U)(bMaxChgCurrentID + wAdjustValue) % (INT8U)MEX_MAX_CHG_130A_ID; //3.5K|2.5K|2K
						}
					}
					...
					else if(bMenuIndex == CF_BAT_TYPE_ID)  //5
					{
						...
						// if(bBatTypeIDTemp % MAX_BAT_TYPE_ID >= 3) { sSetEepromBMSOption(1); }//?????????9600??????????
						// else { sSetEepromBMSOption(0); }
					}
					...
					{
						// if((bBatTypeID == USER_ID) || (bBatTypeID == LIB_ID)|| (bBatTypeID == PYL_ID))
						// {
						...
						if((bBatTypeID == LIB_ID)||(bBatTypeID == PYL_ID))
						{
...
void	sConfigDisplay(void)
{
  ...
  {    
	...
	// 	//sFigGroupDisplayLetter(FIG6_7SEG, (INT8U *)&cMenuRtnTAB[bBMSSetIDTemp%MAX_MENU_RETURN_ID][0], bConfigContentFlashStatus);
...
//    }
    ...
    */
    ...
    if(bBatTypeID != sbGetEepromBatteryType())
    {
		...
		else if(sbGetEepromBatteryType() == cLiBType)//????????LIB
        {
			wVlaueTemp = cBat14v4 * swGetBatteryPcs();
        	sSetEepromBatCVVolt(wVlaueTemp);
            sSetEepromBatFloatVolt(wVlaueTemp);
...
//    bMipOffAlarmID = bMipOffAlarmIDTemp;
   ...
   bSolarPowerBalanceID = bSolarPowerBalanceIDTemp;
	...
	/*if((bBMActiveID != BM_ACTIVE_EN_ID) && (bBMActiveIDTemp == BM_ACTIVE_EN_ID))
	{
		bBMBattVoltIDTemp = 0;
		if(swGetBatteryPcs()==cbBatt48VType)
		{
			bBMBattVoltIDTemp = (INT8U)((cBat48v - cBat44v)/10);
		}
		else if(swGetBatteryPcs()==cbBatt24VType)
		{
			bBMBattVoltIDTemp = (INT8U)((cBat24v - cBat22v)/5);
		}
		else
		{
			bBMBattVoltIDTemp = (INT8U)(((cBat12v - cBat11v) * 13 + 16) >> 5);
		}
		bBMIntervalIDTemp = BM_INTERVAL_30D_ID;
		bBMTimeoutIDTemp = 4 - 1;
	}*/
	//bLibWakeupID = bLibWakeupIDTemp;
	bBMActiveID = bBMActiveIDTemp;
	bBMBattVoltID = bBMBattVoltIDTemp;
	bBMIntervalID = bBMIntervalIDTemp;
	bBMTimeoutID = bBMTimeoutIDTemp;
	
	if((bBatTypeID == cAGMType) || (bBatTypeID == cLiBType)||(bBatTypeID == cPylonBType))
	{
		bEqEnaID = false;
	}
	else
...
/************************************************************************************
Name		:sDisplayBatBMSSOC
Parameters	:-
Returns		:-
Description	:Display BMS SOC
************************************************************************************/
void sDisplayBatBMSSOC(void)
{
  	INT16U 	wTemp;
...
void sDisplayBatBMSNum(void)
{
	...
	sFigGroupDisplayNum(FIG7_7SEG, 2, mBMSConnectNum, cALWAYS_LIGHT);
	sSendBitToVram(cBIT_OP_BAT_INDICATE, DISPLAY_ON, cALWAYS_LIGHT);
}
...
//  else
  {
		wTemp=	wDisPlayWatt;//swGetLoadPowerWatt();
		if(wTemp<10)
		wTemp = 0;
		if(wTemp>99999)	                         
		{	
...
extern INT16U wL1InverterVoltNew;
extern INT16U wL2OPCurrentNew;
...
void sDisplayL2VolCur(void)
{
	INT16U	wTemp=0;
	//INT16U 	wTemp2 =0;
	//INT16U wTemp wL2OPCurrentNew
	if(bPVMode == cBatteryMode)
	{
		wTemp = (swGetRInverterVoltNew()-wL1InverterVoltNew+5)/10;
	}
	else if((bPVMode == cBypassMode ||bPVMode == cLineMode ||bPVMode == cFaultMode) 
		&& hoSFTRLY == 1 && mOPRLYChk()==cOPRLYOn)
	{
	...
	//sFigGroupDisplayNum(FIG1_7SEG, 3, wTemp2, cALWAYS_LIGHT);
	sFigGroupDisplayNum(FIG1_7SEG, 3, wTemp, cALWAYS_LIGHT);

	 //mSetVramBit(cBIT_FIG_DOT2, cALWAYS_LIGHT); 
	mSetVramBit(cBIT_IP_L2, cALWAYS_LIGHT);
	mSetVramBit(cBIT_OP_L2, cALWAYS_LIGHT);
	sSendBitToVram(cBIT_OP_SEL, DISPLAY_ON, cALWAYS_LIGHT); 
	sSendBitToVram(cBIT_IP_UNIT_VOL, DISPLAY_ON, cALWAYS_LIGHT);
	//sSendBitToVram(cBIT_OP_UNIT_VOL, DISPLAY_ON, cALWAYS_LIGHT);
}
...
void sDisplayL1VolCur(void)
{
	INT16U	wVoltTemp=0;
	//INT16U wTemp;
	if(((wL1InverterVoltNew+5)/10)>999)
	{
		wVoltTemp =999;
	}
	else
	{
		wVoltTemp =(wL1InverterVoltNew+5)/10;
	}
	 
	//sFigGroupDisplayNum(FIG1_7SEG, 3, wTemp, cALWAYS_LIGHT);
	sFigGroupDisplayNum(FIG1_7SEG, 3, wVoltTemp, cALWAYS_LIGHT);
	
	//mSetVramBit(cBIT_FIG_DOT2, cALWAYS_LIGHT); 
	mSetVramBit(cBIT_IP_L1, cALWAYS_LIGHT);
	mSetVramBit(cBIT_OP_L1, cALWAYS_LIGHT);
	sSendBitToVram(cBIT_OP_SEL, DISPLAY_ON, cALWAYS_LIGHT); 
	sSendBitToVram(cBIT_IP_UNIT_VOL, DISPLAY_ON, cALWAYS_LIGHT);
	//sSendBitToVram(cBIT_OP_UNIT_VOL, DISPLAY_ON, cALWAYS_LIGHT);
}
...
INT8U sbDisplayWarning(void)
{
	...
	if(ubWarnCodeDisPlayFlag)
	{
		...
		if(fSccSciLoss == false)
		{
			if(wPVCharger == cwMPPTChg)
			{
				// if((wSccFaultFlag & cMpptSCILossFault) != 0)
				// {
				// 	ubWarnCodeTab[ubdisWarnCnt++] = cPwmSciLoss;
				// }
				if((wSccFaultFlag & cMpptBattChgCurrHiFault) != 0)
				{
					ubWarnCodeTab[ubdisWarnCnt++] = cPwmChgCurrOv;
				}
				if((wSccFaultFlag & cMpptBattVoltLoFault) != 0)
				{
					ubWarnCodeTab[ubdisWarnCnt++] = cPwmBatLow;
				}
				if((wSccFaultFlag & cMpptBattVoltHiFault) != 0)
				{
					ubWarnCodeTab[ubdisWarnCnt++] = cPwmBatHigh;
				}
				if((wSccFaultFlag & cMpptHsTempHiFault) != 0 || (wSccFaultFlag & cMpptHs2TempHiFault) != 0)
				{
					ubWarnCodeTab[ubdisWarnCnt++] = cPwmOverTemp;
				}
				if((wSccFaultFlag & cMpptPvVoltHiFault) != 0)
				{
					ubWarnCodeTab[ubdisWarnCnt++] = cPwmPvHigh;
				}
				if((wSccFaultFlag & cMpptFlagOtherFault) != 0 ||(wSccFaultHiFlag & 0x0001) != 0)
				{
					ubWarnCodeTab[ubdisWarnCnt++] = cPwmOtherFault;
				}
				//----------------------------------------------//
				if((wSccAlarmFlag & cMpptPvVoltLossAlarm) != 0)
				{
					ubWarnCodeTab[ubdisWarnCnt++] = cPwmPvLost;
				}
				if((wSccAlarmFlag & 0x0F00) != 0)
				{
					ubWarnCodeTab[ubdisWarnCnt++] = cPwmOtherAlarm;
				}
			}
		}
		...
		if(   (sbGetEepromBatteryType() == cLiBType || sbGetEepromBatteryType() == cPylonBType)
		   && !fBMSConnectFlg
		   && ((g_uwBMSPollingNum > 3) || (g_uwPylonBMSPollingNum > 3)) )  // ?????????????0x80
		{
			ubWarnCodeTab[ubdisWarnCnt++] = WR_BMSConnect_Code;
...
void sDisplayFaultCode(void)
{
    INT8U 	wVaueTemp2 = (INT8U)swGetFaultCode();
    INT8U  bFaultTemp = 0;
    INT8U  bFaultRem = 0;
    ...
    {   
        ...
        bFaultTemp = wVaueTemp2 /10;                
        bFaultRem  = wVaueTemp2 - (bFaultTemp *10);
...
void sFactoryModeDisplay(void)
{
	...
	INT8U   bConfigItemFlashStatus  ;
	INT8U   bConfigContentFlashStatus ;
	INT16U	wVaueTemp1,wVaueTemp2;
	static INT16U	wVaueTemp3;
  ...
  {      
	...
	else
	{
      ...
      else if(bMenuIndex ==  CF_DCI_ADJUST_ID)
      {
			...
			/********************/
			//sFigGroupDisplayLetter(FIG6_7SEG, (INT8U *)&wInvVoltBiasVaue, bConfigItemFlashStatus);
			if(DCI)
			{
				wVaueTemp1=abs(wInvVoltBiasVaue);
				wVaueTemp3=wVaueTemp1%10;
				wVaueTemp2=(wVaueTemp1%100)/10;
				wVaueTemp1=wVaueTemp1/100;
				sViewFigModule(cFIG_TYPE_7, FIG6_7SEG, wVaueTemp1, cALWAYS_LIGHT);
				sViewFigModule(cFIG_TYPE_7, FIG7_7SEG, wVaueTemp2, cALWAYS_LIGHT);
				sViewFigModule(cFIG_TYPE_7, FIG8_7SEG, wVaueTemp3, cALWAYS_LIGHT);
				if(wInvVoltBiasVaue>=0)
				{
					//sViewFigModule(cFIG_TYPE_7, FIG5_7SEG, charAdd, bConfigContentFlashStatus);
				}
				else
				{
					sViewFigModule(cFIG_TYPE_7, FIG5_7SEG, charSub, cALWAYS_LIGHT);
				}
			}			
			/**********************/
			else
			{
				if(wVaueTemp3<6)
				{
					sFigGroupDisplayLetter(FIG4_7SEG, (INT8U *)&cFactoryConfigNum[CF_DCI_ADJUST_ID][0], cALWAYS_LIGHT);	
					sFigGroupDisplayLetter(FIG1_7SEG, (INT8U *)&FactoryRemindTab[CF_DCI_ADJUST_ID][0], cALWAYS_LIGHT);
					wVaueTemp3+=3;
				}		
				...
				sViewFigModule(cFIG_TYPE_7, FIG7_7SEG, 0, bConfigContentFlashStatus); 
	  ...
	  else if(bMenuIndex == CF_MACHINE_MODEL_ID)
  	 {
  	 		sFigGroupDisplayLetter(FIG4_7SEG, (INT8U *)&cFactoryConfigNum[CF_MACHINE_MODEL_ID][0], bConfigItemFlashStatus);	 //??ï¿½ï¿½?2?ï¿½Ì£ï¿½??3?ï¿½ï¿½?D??
			sFigGroupDisplayLetter(FIG1_7SEG, (INT8U *)&FactoryRemindTab[CF_MACHINE_MODEL_ID][0], bConfigItemFlashStatus);   //ï¿½ï¿½a??ï¿½ï¿½?ï¿½ï¿½????? FIG1  MOD 
			sFigGroupDisplayLetter(FIG6_7SEG, (INT8U *)&cEnaDisTAB[wMachineModelTemp][0], bConfigContentFlashStatus); //enableoï¿½ï¿½disableï¿½ï¿½ï¿½ï¿½ 
  	 }
	}    
  }
			...
			{
				{
					if((cDownKeyPressBit == wKeyVal) || (cUpKeyPressBit == wKeyVal))
					{
						...
						{				   
							...
							{
								if(wInvVoltBiasVaue < 180) // if(wInvVoltBiasVaue <115)
								{
									wInvVoltBiasVaue += 5;
								}
						...
						else if(cDownKeyPressBit == wKeyVal) 
						{
							/*if(bFactoryIsAdjustVal ==1)
							{
								bFactoryIsAdjustVal = 2;
					...
					else if(cEnterKeyPressBit == wKeyVal) 
					{
						...
						{
								if(bMenuIndex ==CF_KP_SETTING_ID)
								{
								...
								{
									wMachineModel = wMachineModelTemp; //ENTER?ï¿½ï¿½ï¿½ï¿½?ï¿½ï¿½???????????ï¿½ï¿½ï¿½ï¿½?ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½?????????ï¿½ï¿½?ï¿½ï¿½?Dï¿½ï¿½???
									bFactoryIsAdjustVal = 3;    //wMachineModelTempï¿½ï¿½??ï¿½ï¿½?yï¿½ï¿½??3?ï¿½ï¿½??wMachineModel
								}                               
						}
						bModeTimeOut = CF_MODE_EXIT_TIME;
					}
...
void sSetFactoryConfigMode(void)
{
	...
	wMachineModel = swGetEEMachineModelOnOFF();
	wMachineModelTemp = wMachineModel;
...
void  sSaveFactoryConfiguration(void) 
{
	...
	if(wMachineModel !=swGetEEMachineModelOnOFF())  
	{
		sSetEEMachineModelOnOFF(wMachineModel);
		bSaveFlag = 2;
	}

	if(bSaveFlag == 2)  
	{
		OSEventSend(cPrioInterface, eEepromSaveUserSetData); //Æ¬Çø2µÄ¸üÐÂ
		bSaveFlag = 0;
	} //OSEventSend(cPrioInterface, eEepromSaveUserSetData) ¸üÐÂÆ¬Çø2µÄÈÎÎñ·¢Æð
