...
INT16S wL2OpCurrSampleBiasTemp =0;
...
INT16S wL1InvVoltReal = 0;
INT16S wL1InvVoltSample1 = 0;
INT16S wL1InvVoltSample2 = 0;
INT32S dwL1InvVolt2SumTemp = 0;
INT16S wL1InvVolt2SumCntTemp=0;
INT32S dwL1InvVolt2Sum=0;
INT16S wL1InvVolt2SumCnt=0;

INT16S wL1InvVoltCntl;

INT16S wL2OPCurrReal = 0;
INT16S wL2OPCurrSample1;
INT16S wL2OPCurrSample = 0;
INT32S dwL2OPCurr2SumTemp=0;
INT16S wL2OPCurr2SumCntTemp=0;
INT32S dwL2OPCurr2Sum=0;
INT16S wL2OPCurr2SumCnt=0;
INT32S dwL2OpCurrSumTemp=0;
INT16S wL2OpCurrSumCntTemp=0;
INT32S dwL2OpCurrSum=0;
INT16S wL2OpCurrSumCnt=0;

INT32S dwL2BatWattRealSum;
INT32S dwL2BatWattRealSumTemp;
INT16U wL2BatWattRealSumCnt;
INT16U wL2BatWattRealSumCntTemp;
INT32S dwL2LowBatWattRealSumTemp;
INT16U wL2LowBatWattRealSumCntTemp;
INT32S dwL2LowBatWattRealSum;
INT16U wL2LowBatWattRealSumCnt;

INT32S dwL2OPWattRealSum;
INT32S dwL2OPWattRealSumTemp;
INT16U wL2OPWattRealSumCnt;
INT16U wL2OPWattRealSumCntTemp;

INT32S dwL2InvWattRealSum;
INT32S dwL2InvWattRealSumTemp;
INT16U wL2InvWattRealSumCnt;
INT16U wL2InvWattRealSumCntTemp;
...
extern INT16S wL2OpCurrSampleBias ;
...
INT16U ProtectFlag2 =0;
INT16S gi_wPLLPointer = 0;         //0��
INT16S gi_wPLLPointerMax = 384;    //360��
INT16S gi_wPLLPointFourSix = 256;  //240��
INT16S gi_wPLLPointTwoSix = 128;   //120��
INT16S gi_wPLLPointThreeSix = 192; //180��
INT16S gi_wPLLPoint1Div4 =96;      //90��
INT16S gi_wPLLPoint3Div4 =288;     //270��
INT16S gi_wLineCrossZeroPointer =0;
INT16U gi_wPLLPoint1Div8 =47;      //44��
...
INT16U CHOP =0;
extern INT16S g_wOPVoltRealOverDerat;

INT16S g_wBatVoltSlopeCal = cBat53v;//?????��??????????
INT16S g_wBatMinVoltDerate = cBat53v;//?????��???????��???
INT16S g_wBatMaxVoltDerate = cBat60v;//?????��??????????

INT16S swBatVoltSolpe[4] = {0};
INT16S swBatVoltSolpeCnt = 0;
INT16S swBatVoltSolpeSum = 0;//4?????��???
INT8U  bBatVoltSolpeProtectFlag = 0;//��????????��
INT8U  bBatVoltSolpeChkCnt = 0;//��???????
INT16S swBatVoltQueueCnt = 0;//????????��?
INT16S swCurrBatVoltSlope = 0;//????????��??
INT8U  bBatAvg_RealVoltChkCnt = 0;//????????
INT8U  bBatAvg_RealVoltProtectFlag = 0;//��????????��

INT16S swBatVoltQueue[20] = {0};
...
Function:
	1.INV Control
	2.????AD?????K????????????????????????????????
	3.??wSinPointer??=0?r???l?????PRIO_INVERTERMODULE,EVENTID_SIN_ZERO
...
//	wBusVoltReal = (INT16S)(((INT32S)wBusVoltSample * wBusVoltRatio)>>11);//0.1V
	...
	wL2OPCurrReal = (INT16S)(((INT32S)wL2OPCurrSample1 * wOPCurrRatio)>>11);//0.1A
	wL1InvVoltReal = (INT16S)(((INT32S)wL1InvVoltSample1 * wInvVoltRatio)>>11);//0.1V
	//wOPShareCurrReal=(INT16S)(((INT32S) wOPShareCurrSample *  wOPShareCurrRatio)>>11);

	swBatVoltSlopeCal();
	
	if((swBatVoltSolpeSum > cBatVoltSolpeSumHighLever) && (wBatVoltReal  > g_wBatVoltSlopeCal))//��?????
	{
		if(++bBatVoltSolpeChkCnt > 5)
		{
			bBatVoltSolpeProtectFlag = true;
		}
	}
	else
	{
		bBatVoltSolpeChkCnt = 0;
	}

	if(wBatVoltReal > wBatAvgVoltNew+cBat1v5)//?????
	{
		if(++bBatAvg_RealVoltChkCnt > 2)
		{
			bBatAvg_RealVoltProtectFlag = true;
		}
	}
	else
	{
		bBatAvg_RealVoltChkCnt = 0;
	}
	
	if((gi_wPLLPointer % 8) == 0)					//sum value every 8 counts
	{
		#ifdef IVPA
		{
		dwRInvWattRealSumTemp += ((INT32S)wLineVoltReal *( wInvLCurrReal-((wOPCurrReal+wL2OPCurrReal)>>1)));    //Charge  power
		...
		dwRBatWattRealSumTemp += ((INT32S)wL1InvVoltReal * wOPCurrReal);  
		wRBatWattRealSumCntTemp ++;
		dwL2BatWattRealSumTemp += ((INT32S)(wL1InvVoltReal-wInvVoltReal) * wL2OPCurrReal);  
		wL2BatWattRealSumCntTemp ++;
		
		
		dwOPWattRealSumTemp +=((INT32S)wL1InvVoltReal * wOPCurrReal);
		wOPWattRealSumCntTemp ++;	
		dwL2OPWattRealSumTemp +=((INT32S)(wL1InvVoltReal-wLineVoltReal) * wL2OPCurrReal);
		wL2OPWattRealSumCntTemp ++;	
		
		dwL1InvVolt2SumTemp += (INT32S)wL1InvVoltReal * wL1InvVoltReal;
		wL1InvVolt2SumCntTemp++;
		}
		#else
		{
		dwRInvWattRealSumTemp += ((INT32S)wInvVoltReal * wInvLCurrReal);//((INT32S)wLineVoltReal * wInvLCurrReal);    //Charge  power
		wRInvWattRealSumCntTemp ++;
		dwRBatWattRealSumTemp += ((INT32S)wInvVoltReal * wOPCurrReal);         //Bat mode Load power
		wRBatWattRealSumCntTemp ++;
		dwLowBatWattRealSumTemp += ((INT32S)wInvVoltReal * wOPLowCurrReal);     //Line mode Load power
		...
		dwL2BatWattRealSumTemp += 0;  
		wL2BatWattRealSumCntTemp ++;
		dwL2OPWattRealSumTemp +=0;
		wL2OPWattRealSumCntTemp++;
		}
		#endif
	}
...
//	wBatTempSampleSumCntTemp++;
	...
	dwL2OpCurrSumTemp+=wL2OPCurrSample;
	wL2OpCurrSumCntTemp++;

	dwL2OPCurr2SumTemp += (INT32S)wL2OPCurrReal * wL2OPCurrReal;
	wL2OPCurr2SumCntTemp++;
	HardProtectChk();
	if(++gi_wPLLPointer >= gi_wPLLPointerMax)        //wRCountsPerPeriod=384,50Hz
	{
		gi_wPLLPointer = 0;
		...
		dwL2OPCurr2Sum = dwL2OPCurr2SumTemp;
		wL2OPCurr2SumCnt = wL2OPCurr2SumCntTemp;
		dwL2OPCurr2SumTemp = 0;
		wL2OPCurr2SumCntTemp = 0;
		dwL1InvVolt2Sum = dwL1InvVolt2SumTemp;
		wL1InvVolt2SumCnt = wL1InvVolt2SumCntTemp;
		dwL1InvVolt2SumTemp = 0;
		wL1InvVolt2SumCntTemp = 0;
	...
	wCtrlSinpointer = (INT16S)((INT32S)gi_wPLLPointer * wRCountsPerPeriod / gi_wPLLPointerMax);
...
//		wBusVoltSample   = 0;            //1V = 7.6561CNT
		...
		wL1InvVoltSample1  = ADCRESULT4-2048;	         //1V = 4.3977CNT
		wL2OPCurrSample1 = ADCRESULT2-2048-wL2OpCurrSampleBias;	  //1A = 12.784CNT
...
//			wBusVoltSample   = 0;            //1V = 7.6561CNT
		...
		wInvTempSample   = ADCRESULT15;
		wTestModeSample  = ADCRESULT10;
		wL1InvVoltSample1  = 0;	         //1V = 4.3977CNT
		wL2OPCurrSample1 = 0;
...
//	wSwEnterSample   = 0;
	...
	wL2OPCurrSample = (INT16S)(((INT32S)wL2OPCurrSample * mEEL2OPCurrAdj()) >> 11);
	wL1InvVoltSample1 = (INT16S)(((INT32S)wL1InvVoltSample1 * mEEInvVoltAdj()) >> 11);
	...
	if((bPVMode == cBatteryMode )||((bPVMode == cStandbyMode )&&(wFBControlStatus == cFBInverterControl)))
	{
		if ((abs(wInvLCurrSample1) > wInvOverCurrentLimit*g_wInvOverCurrentFactor)\
			||(abs(wOPCurrSample) > wInvOverCurrentLimit*g_wInvOverCurrentFactor))
		{
			mDisInvPwmOut();		
	...
	else if(bPVMode == cLineMode )
	{
		...
		if(wBatVoltReal > (mEEBatVoltOverShut()-swGetBatteryPcs()*cBat0v7))
		{
			ubBatVoltOveCnt ++;
			if(wChgPwmLimit > 5)
			{
		...
		if((abs(wInvLCurrSample1) > g_wInvLCurrLineLevel2)//40*25=960 
			||(wBatAvgVoltNew >(mEEBatVoltOverShut()-swGetBatteryPcs()*cBat0v5))||(ubBatVoltOveCnt > 5)\
			||((swGetBatDisconnectedA()==true)&&(wFBControlStatus == cFBKeepBatSts))\
			||((mGetLineWavLossSts() == true)&&(wFBControlStatus == cFBDischg))\
			||(bBatVoltSolpeProtectFlag == true)||(bBatAvg_RealVoltProtectFlag == true))
		{
			mDisBuckPwmOut();
			mDisInvPwmOut();	
			...
			bBatVoltSolpeChkCnt = 0;
			bBatVoltSolpeProtectFlag = false;
			bBatAvg_RealVoltChkCnt = 0;
			bBatAvg_RealVoltProtectFlag = false;
		}		
		else
		{
			mEnBuckPwmOut();
...
//				wChgCurrHigh=1;
	...
	{
		if( !g_fBatFastLow
			&&(wBatVoltReal < (cBat9v6 * swGetBatteryPcs()))
			&&abs(wInvVoltReal) > cVac25v )
		{
			if(++g_wBatFastLowCnt > 20)  //1.04ms
			{
				...
				{
					sSetFaultCode(cBusUnder);//(cInvSoftTimeOut);
					OSISREventSend(cPrioSuper, eSuperFault);
				}
			}
		}
		...
		if(LiHardProtectCNT>=3)//Ӳ���������� 
		{
			if((wInvOffPoint ==0)&&(wContinueTrig==0))
			{
				wHardProtectDelreatOutVolt =10;
				...
				//wInvOffPoint =128;
				wInvOffPoint =192;// �ر�150??
				wContinueTrig =1;
				if(ubHardProtectCnt >= 30)       
				{
					ubHardProtectCnt = 0;
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>20))//((wInvOffTimes ==0)&&(++wContinueTrigCNT>20))
			{
				wContinueTrigCNT =20;
				wContinueTrig =0;
			}
		...
		if(wInvOffTimes >0)
		{
			if((wInvOffsetpointer ==50)&&(wInvOffSinpointer == wCtrlSinpointer))
			{
				wInvOffPoint = 150;
//				wContinueTrig = 1;
				wInvOffTimes--;
				wInvOffsetpointer =0;
				wInvOffSinpointer +=5;// �رսǶ�����
				if((wInvOffSinpointer >wRCountsPerPeriod )\
					||((wInvOffSinpointer > gi_wPLLPointThreeSix)&&((wInvOffSinpointer-5) < gi_wPLLPointThreeSix)))
				{
					wInvOffPoint = 0;
					wContinueTrig = 0;
					wInvOffTimes =0;
...
//			wInvOffFlg =1;
			...
			ProtectFlag2 =1;
			//mDisInvPwmOut();
			if((wCtrlSinpointer == 0)||(wCtrlSinpointer == gi_wPLLPointThreeSix)\
				||(wCtrlSinpointer == (gi_wPLLPointThreeSix+1)))
			{
				wInvOffPoint = 0;
			}
		}
...
//		}
	...
	else if((wFBControlStatus == cFBKeepBatSts) || (wFBControlStatus == cFBDischg))
	{
		...
		// if((uEepromCfg1.EepromStructCfg1.wChgParameter ==cMaxChgSettingPara)||(CHOP))
		// {
		// 	sFBINVController();
		// }
		// else
...
//				EDIS;
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>20))//if((wInvOffTimes ==0)&&(++wContinueTrigCNT>20))
			{
				wContinueTrigCNT =20;
				wContinueTrig =0;
			}
...
//	}
	...
	{
		wLineWaveChkCnt++;
		if(wLineWaveChkCnt > gi_wPLLPoint1Div4)
		{
	...
	else	// UPS
	{
		if((wLineWaveChkCnt >5)&&((wRSinPointer == 0) || (wRSinPointer == 1)\
			|| (wRSinPointer == gi_wPLLPoint1Div4) || (wRSinPointer == (gi_wPLLPoint1Div4+1))\
			|| (wRSinPointer == gi_wPLLPointThreeSix) || (wRSinPointer == (gi_wPLLPointThreeSix+1))\
			|| (wRSinPointer == gi_wPLLPoint3Div4) || (wRSinPointer == (gi_wPLLPoint3Div4+1))))
		{
			if((abs(wLineWaveVoltMax - wLineWaveVoltMin) < cVac50v)&& (abs(wOPCurrReal) < 50))
			{
				if((bPVMode == cLineMode)||(bPVMode == cBypassMode))
	...
	sRlyActionDriver(gi_wPLLPointer);
...
interrupt void OSTimeBase(void)  //ÿ��һ��ʱ���һ�Σ����һ��
{
...
if(!hoSFTRLY)  {hoGRIDL_12VRLYOn;  uwGRIDCnt = 0;  }
if(!hoSFTNRLY) {hoGRIDN_12VRLYOn;  uwGRIDNCnt = 0; }
if(!hoOPRLY)   {hoOP1_12VRLYOn;    uwOPCnt = 0;    }
...
INT16S swL2OpCurrAveCal(void)
{
	INT16S wROpCurrTemp = 0;
	
	OS_ENTER_CRITICAL();
	dwL2OpCurrSum = dwL2OpCurrSumTemp;
	dwL2OpCurrSumTemp = 0;
	wL2OpCurrSumCnt = wL2OpCurrSumCntTemp;
	wL2OpCurrSumCntTemp = 0;
	OS_EXIT_CRITICAL();
	if(wL2OpCurrSumCnt > 0)
	{
		wROpCurrTemp = (INT16S)(dwL2OpCurrSum/wL2OpCurrSumCnt);
	}
	return(wROpCurrTemp);
}
...
INT16S	swL1InvVoltRmsCal(void)
{
	INT16S wInvVoltRms = 0;

	if(wL1InvVolt2SumCnt > 0)
	{
		wInvVoltRms = (INT16S)swRoot(dwL1InvVolt2Sum / wL1InvVolt2SumCnt);
	}
...
INT16S	swL2OPCurrRmsCal(void)
{
	INT16S wOPCurrRms = 0;

	if(wL2OPCurr2SumCnt > 0)
	{
		wOPCurrRms = (INT16S)swRoot(dwL2OPCurr2Sum / wL2OPCurr2SumCnt);
	}
...
INT16S swGetL2OpCurrSampleBiasTemp(void)
{
	return(wL2OpCurrSampleBiasTemp); 
}

void sSetL2OpCurrSampleBiasTemp(INT16S wVaule)
{	
	wL2OpCurrSampleBiasTemp = wVaule;
}
...
void sLineWaveChk(INT16U wCmpPoint)
{
	...
	else 
	{
		...
		else if(wCmpPoint == 0 || wCmpPoint == gi_wPLLPointThreeSix\
			||wCmpPoint == (gi_wPLLPointThreeSix+1))
		{
			s_ubCTSaturationChkCnt = 0;
		}
		if(((wLineVoltReal > wLineWaveVoltHighLoss) || (wLineVoltReal < wLineWaveVoltLowLoss))
		      &&(abs(wOPCurrReal) < 680) && s_ubCTSaturationChkCnt < 15)    // (5000/140*1.414=50.5)*2.5=126	  ����??ͶRCD�أ�??����??L�oss
		{
			wLineLossTemp = true;
		}
		else
...
{
	return(0);//(wBusVoltSample);	
}

INT16S swGetBusVoltReal(void)
{
	return(0);//(wBusVoltReal);
}
...
INT32S sdwL2BatWattCal(void)
{
	INT32S dwBatWattNewTemp = 0;
	
	OS_ENTER_CRITICAL();
	dwL2BatWattRealSum = dwL2BatWattRealSumTemp;
	dwL2BatWattRealSumTemp = 0;
	wL2BatWattRealSumCnt = wL2BatWattRealSumCntTemp;
	wL2BatWattRealSumCntTemp = 0;
	OS_EXIT_CRITICAL();
	if(wL2BatWattRealSumCnt > 0)
	{
		dwBatWattNewTemp=dwL2BatWattRealSum/wL2BatWattRealSumCnt;
	}
...
INT32S sdwL2OPWattCal(void)
{
	INT32S dwOpWattNewTemp = 0;

	OS_ENTER_CRITICAL();
	dwL2OPWattRealSum = dwL2OPWattRealSumTemp;
	dwL2OPWattRealSumTemp = 0;
	wL2OPWattRealSumCnt = wL2OPWattRealSumCntTemp;
	wL2OPWattRealSumCntTemp = 0;
	OS_EXIT_CRITICAL();

	if(wL2OPWattRealSumCnt > 0)
	{
		dwOpWattNewTemp=dwL2OPWattRealSum/wL2OPWattRealSumCnt;
	}
...
/*INT32S sdwL2InvWattCal(void)
{
	INT32S dwInvWattNewTemp = 0;

	OS_ENTER_CRITICAL();
	dwL2InvWattRealSum = dwL2InvWattRealSumTemp;
	dwL2InvWattRealSumTemp = 0;
	wL2InvWattRealSumCnt = wL2InvWattRealSumCntTemp;
	wL2InvWattRealSumCntTemp = 0;
	OS_EXIT_CRITICAL();
	
	if(wL2InvWattRealSumCnt > 0)
	{
		dwInvWattNewTemp=dwL2InvWattRealSum/wL2InvWattRealSumCnt;
	}
...
void  swGetLineQuadraticSum(INT16S wRLineVoltRealTemp )
{
	...
	//wRLineVoltRealTemp=wRLineVoltRealTemp+50;//5V电压偏置
...
void HardProtectChk(void)
{
// 	if(LiHardProtect==0)//Ӳ����������
// 	{
// //		LiHardProtectCNT++;
// 		if(++LiHardProtectCNT>4)
// 		{
// 			LiHardProtectCNT =4;
// 		}
// 	}
...
/*******************************************************************************
* Function Name  : swBatVoltSlopeCal
* Description	 : ?????��?????swBatVoltSolpeSum???????4?????????��?????
* Input 		 : 
					INT16S swBatVoltTemp:??????
					INT16S swBatVoltQueueCntTemp?????????????��?
* Output		 : None
* Date			 : 2021.02.28
* Author		 : CM
********************************************************************************/
void swBatVoltSlopeCal(void)
{
	static INT16S swBatVoltSolpeStartFlag = 0;

	swBatVoltQueue[swBatVoltQueueCnt] = wBatVoltReal;

	if(swBatVoltQueueCnt == 19)
	{
		swCurrBatVoltSlope = swBatVoltQueue[19] - swBatVoltQueue[0];
	}
	else
	{
		swCurrBatVoltSlope = swBatVoltQueue[swBatVoltQueueCnt]-swBatVoltQueue[swBatVoltQueueCnt+1];//???20?��?��??
	}

	if(swBatVoltSolpeStartFlag < 20)
	{
		swBatVoltSolpeStartFlag++;
		swBatVoltSolpeSum = 0;
	}
	else
	{
	
		if(swCurrBatVoltSlope < 0)
		{
			swCurrBatVoltSlope = 0;
		}
		
		swBatVoltSolpeSum = swBatVoltSolpeSum + swCurrBatVoltSlope - swBatVoltSolpe[swBatVoltSolpeCnt];
	}

	swBatVoltSolpe[swBatVoltSolpeCnt] = swCurrBatVoltSlope;

	if(++swBatVoltQueueCnt >= 20)
	{
		swBatVoltQueueCnt = 0;
	}

	if(++swBatVoltSolpeCnt >= 4)
	{
		swBatVoltSolpeCnt = 0;
	}
}
