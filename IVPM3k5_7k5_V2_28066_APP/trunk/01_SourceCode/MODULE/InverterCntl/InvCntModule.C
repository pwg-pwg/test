...
INT16S wDCVoltSet = 0;   //过零点优化，2024.10.30
...
void sInverterControl(void)
{
	...
	{
		wRSinAmp=640;
	} //Vpeak=20V*32  =640	确保软起起始电压
	else if(wRSinAmp >4500)// 100V
	{
...
//	wRLoopOutput = (INT16S)(((INT32S)wRemp * pSinTab[wCtrlSinpointer] >> 14)) + wDCVoltSet;// + wInvDCVoltCntlReal + wDCVoltSet;
	...
	if(wR_PWM >= 0)    
	{
		...
		wR_PWMTemp = wR_PWM;
		// if((wCtrlSinpointer<=10)||(wCtrlSinpointer>=182))
		// {
			wR_PWMTemp += wDCVoltSet;
			// g_wInvVCntlFactor1++;
			// if(g_wInvVCntlFactor1>400)
			// {
			// 	g_wInvVCntlFactor1 = 0;
			// }
		// }
		if(wR_PWMTemp > (wPwmPeriod - 2))   wR_PWMTemp = wPwmPeriod - 2;
       	EPwm2Regs.CMPA.half.CMPA = wPwmPeriod - wR_PWMTemp;//wR_PWMTemp   	  //L
		EPwm1Regs.CMPA.half.CMPA = wPwmPeriod-1;           //1                //N	
	...
	else                
	{
		...
		// }
		wR_PWMTemp = wR_PWMNegtive;	
		// if((wCtrlSinpointer<=202)||(wCtrlSinpointer>=374))
		// {
			wR_PWMTemp += wDCVoltSet;
		// 	g_wInvVCntlFactor1++;
		// 	if(g_wInvVCntlFactor1>400)
		...
		// }
		if(wR_PWMTemp > (wPwmPeriod - 2))   wR_PWMTemp = wPwmPeriod - 2;
		EPwm2Regs.CMPA.half.CMPA = wPwmPeriod - 1;//1   low 
...
void sFBINVPFCInitial(void)
{
	...
	{
		wRKvBase =wRKv = 42;//33;//23;//10;   //48;             //Q7  0.18*128 //George 141217 48->23
	...
	else
	{
		...
		***************************************************************************/
		...
		if(++wFBCntLoop&0x08)
		{
			...
			wFBP_VoltPIAdjCtl =(INT32S)((INT32S)wFBP_VoltPIAdjCtl*3+ wVm_P)>>2;// 鍋忓樊涓?0 鏃讹紝鍓嶉涓嶅彈鎺?
		...
		*  鐢垫祦鐜瘡1涓偣涓�娆?
		***************************************************************************/
		wLineVoltCntlQ5=(INT16S)((INT32S)gi_wLineVoltQ5_0*3+(INT32S)(abs(wLineVoltCntlQ5+128))>>2);// 4*32 鏄數鍘嬮噰鏍峰亸缃ˉ鍋?
		...
		if(wBatVoltReal>0)
		{
			...
			wR_Is_P =abs(wInvLCurrReal);//0.1V  //闇�瑕佸姞鍏ョ數鍘嬪垽鏂鐞嗙數娴佽繃娴?
		}
		else
		{
			wR_Is_P = 0;
...
//			wKCurrentForward = TestwFB_CurPI_Kp;
			...
			if(uWFBP_CurPI_IderatCnt >0)//杩囬浂鐐瑰噺灏戠Н鍒?
			{
			...
			if(abs(wInvLCurrSample1) > 1000)//32A 杩囨祦淇濇姢
			{
				mDisInvPwmOut();
			}
			//杈撳嚭PWM
