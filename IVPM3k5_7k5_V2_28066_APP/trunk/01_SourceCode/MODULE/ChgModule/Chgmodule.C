...
INT16S wDisChgAvgCurrNew;		// ��طŵ����(�ϻ�ģʽ)(1A)
...
INT16S g_wBatDischgCurr = 0;	// ��طŵ����(0.1A)
INT16S g_wSCCACChgCurr = 0;		// ��س�����?(SCC��AC֮��)��������ʾ���ж�,�����ڿ��ƣ����ŵ�ʱ��ص���?=PV����-BUS�ŵ����?-3A��������ģ��������ʱ��ص���?=PV����+�������������������AC�е���
...
//	if((g_wSCCACChgCurr < (INT16S)wFloatCurrTemp) && (swGetBatAvgVoltNew() > swGetEepromBatFloatVolt()))
	if((g_wSCCACChgCurr < (INT16S)wFloatCurrTemp) && (swGetBatAvgVoltNew() > g_wBatFloatVolt))	
	{
	...
	if(sbGetInvChgStep() == cChgstepthree)
	{
		...
		if(swGetBatAvgVoltNew() > (g_wBatCVVolt- swGetBatteryPcs()*cBat0v3))
		{
...
//		if(swGetBatAvgVoltNew() > (swGetEepromBatFloatVolt()- swGetBatteryPcs()*cBat0v3))
		if(swGetBatAvgVoltNew() > (g_wBatFloatVolt - swGetBatteryPcs()*cBat0v3)) // Vbat > 54.4v - 1.2V
		{
			g_uwSysVoltCVorFloatFlg = true;
		}
		else
	...
	if(   (fBMSForceChgFlg==0) 
	   && (   (fBMSStopChargeFlg)
	       || (    swGetSccOkFlag() == false
		       && (   swGetFBControlStatus() != cFBKeepBatSts 
	...
	if(wChgStsTemp == cCCCharging || wChgStsTemp == cCVCharging)
	{
		...
		sSetBatVoltRef(g_wBatCVVolt);// CC 20210617 
//		wFloatDelay  = 0;
	}
...
//			wFloatDelay = 0;
			...
			if(swGetBatVoltRef() > g_wBatFloatVolt) // ���������ѹ���Զ��ص��������ѹ��
			{
			...
			{
				sSetBatVoltRef(g_wBatFloatVolt);//CC 20210617
			}
		}
	}
...
void sInvChgCurrCal(void)
{
	...
	{
		if(sdwGetInvWatt() > 0)//��磬sdwGetInvWattָ������ʱ��书�ʣ��Դ˻����ص���?
		{
		...
		{
			wTemp = abs(sdwGetInvWatt());//�ŵ�--���ϻ�
			wTemp = (INT16U)(((INT32U)wTemp * 100000 / g_wChgEfficiency + (swGetBatAvgVoltNew() >> 1))  / swGetBatAvgVoltNew()); // �����ع��ʺ󣬵�ص���?=��ع���?/��ص��? (�Ŵ�100��)
	...
	if(bPVMode == cBatteryMode)
	{
		...
		{
			//g_wBatDischgCurr = 0;
			// for test,�����ذٷֱȣ�2%ʱ,��ʾΪ0,���Ƕ���10kVA���͵������ǱȽϴ�,��ʾ0���ܴ�
			if(sdwGetBatWatt() > 0) //L��N������书��?
			{
		...
		// for test
		// ���ģʽ��?,���������д�Լ3A�����?,��Ҫȥ����
		g_wSCCACChgCurr = (wSccChgCurr - g_wBatDischgCurr) / 10; // SCC�������? - ���ػ��㵽�ĵ�ص�����Ϊ��ʱ��ʾ����ڳ�磬Ϊ��ʱ��ʾ����ڷŵ硣
		if(wSccChgCurr != 0)
		{
			g_wSCCACChgCurr -= 3;
		}
