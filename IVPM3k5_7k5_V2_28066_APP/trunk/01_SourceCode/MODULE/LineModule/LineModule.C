...
void	sRLineVoltSPShutdown(INT8U bFilter)
{
  ...
  {
	if(sbOverLevelChk(wRLineVoltNew,cVac285v+cVac5v,bFilter,&bRLinesdChkCnt1) == true)
	{ 
	   bRVoltstatusLoss = 1;
	}
  }
...
{  
	if(sbUnderLevelChk(wRLineVoltNew,cVac285v,bFilter,&bRLinesdChkCnt) == true)
	{
	   bRVoltstatusLoss = 0;
	}
}
...
void	sLineModuleUpdate(void)
{
	...
	if((!fLine.RVoltLoss)&&(!fLine.FreqLoss)&&(!fLine.RWavLoss)&&(!fLine.RVoltLongTimeLoss)\
		&&(!fLine.OrderFault)&&(!fLine.VoltFastChkLoss))
	{
		OS_ENTER_CRITICAL();
...
void sLinePeakFailChk(INT8U bFilter)
{
	...
	// 每次开机强制检�?3min,防止无电池启动时市电能量反灌到电�?
	if(wOrderChkTime < 9000)		// 3min
	{
		wOrderChkTime++;
