...
void	sSetRlyPointer(unsigned char relaytype,unsigned int pointer,unsigned char event,unsigned char prio)
{
	...
	if(relaytype == RLY_DELAY)
	{
		...
		OS_ENTER_CRITICAL();
		wRlyActionPointerTemp = (wRlyActionPointerTemp + gi_wPLLPointer + 1) % gi_wPLLPointerMax; //wRlyActionPointerTemp：在一个正弦周期的百分比位置
		OS_EXIT_CRITICAL(); 
	}
	...
	else if(relaytype == RLY_ZEROPOINT)
	{
		...
		OS_ENTER_CRITICAL();	
		wRlyActionPointerTemp = ((gi_wPLLPointer + wRlyActionPointerTemp+2) / gi_wPLLPointThreeSix + 1) * gi_wPLLPointThreeSix
	...
	{
		OS_ENTER_CRITICAL();	
		wRlyActionPointerTemp = (((((gi_wPLLPointer + 1) * 2) + gi_wPLLPointThreeSix) / gi_wPLLPointerMax) * gi_wPLLPointThreeSix)\
