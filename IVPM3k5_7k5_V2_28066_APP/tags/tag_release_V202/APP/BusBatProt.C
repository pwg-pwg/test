...
void sFanSpeedControl(void)	//Fan Control
{
	...
	if(false == wSciForeceFanCtrl)
	{
		...
		else
		{
				...
				|| (sbGetTxTempStatus() >= 1) || (sbGetInvTempStatus() >= 1))
			{
				...
				{ g_wACFanOnFlag = 1; } //AC Fan Contral -- SCC???????????????锟斤拷??????
				bFan1status = true;
				bFan2status = true;
			}
	...
	{
		if(hoACFAN==0)// CC 20210621 锟斤拷锟狡斤拷锟斤拷锟斤拷锟饺继碉拷??
		{
			sOSTimerStop();
...
void 	sBatWeakChk(INT8U bFilter)
{
	...
	//if((!g_uwBatWeakFlg)&&(sbGetBatLow() == true))// 锟斤拷氐锟窖筨atlowback锟斤拷batlow之前锟斤拷一直锟斤拷锟斤拷压锟斤拷锟斤拷锟叫碉拷????
	//{
	//	g_uwBatWeakFlg = 1;
	//}
...
void sLineModeInvOverCurProtCnt(void)
{
	...
	if(bPVMode!=cLineMode)
	{
		wSetInvOverCurrentLineModeCnt = 4;//???锟斤拷??锟斤拷??????????????4
		uwTranformCnt = 0;
	}
	else
	{
...
/*
*******AC辅源********
1.市电接入，继电器连接为常闭触电，默认AC辅源工作唤醒逆变器。
2.检测到市电为正常范围，延时10S后，发送低电平不动作继电器，AC辅源继续工作工作。开机唤醒5min后，继电器断开，AC辅源停止工作。
3.否则检测到市电异常，继电器断开，AC辅源停止工作。
*/
void sACSPSControl(void)
{
	...
	else
	{
		...
		bACSpsCtrlOffFlag = 1;
		wACspsCtrlSecCnt =0;
		if((bRVoltstatusLoss)||(0 != sbGetRLineFreqLoss())&& (0 != sbGetRLineWaveLoss())) //只针对280~290VAC屏幕正常显示，其他条件维持
		{
			if(!hoACSPS)
			{
				hoACSPS_OFF;
