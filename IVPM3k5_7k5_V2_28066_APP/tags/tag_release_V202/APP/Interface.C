...
#define MAX_SCI_RX_BUF_SIZE	    150
...
// #pragma DATA_SECTION(bRxBuffer,"CommandBuff");
// INT8U	bRxBuffer[MAX_SCI_NO][MAX_SCI_RX_BUF_SIZE];
...
INT16U g_uwDischgCurr = 0;			// ?????????????
INT16S wBat1Ref;
INT16S wBat2Ref;
INT16S wBat1Real;
INT16S wBat2Real;
INT16U wSCCReflash = false;			// SCC????????????
INT16U wSCCReflashFinish = false;	// SCC????????????
...
INT16U wBatVerifyFlg = false;	// =1?????锟斤拷??,PWM???????????
...
INT16S gi_wInvOverValue = cVac360v;
INT16S g_wSCCComGapCnt = 0;
INT8U  g_SCITouchFlag = 0;
...
extern INT16S g_BatCurOPVoltCntl;
...
void 	sMpptUpdateCommand(INT8U sciid);
...
void sSCITouchOnOff(INT8U b_FlagBit);
INT16S swGetSCITouchFlag(void);
void sWIFICommand(INT8U sciid);
INT8U sMpptUpdateProc(void);
void sMpptUpdateDeal(void);
...
INT8U g_bDischgVoltSet = cBat52v;	// 11.5-14.5V/pcs??????????????????
INT8U g_bDischgCurrSet = cIdc50A;	// 0-100A??????????????????
INT8U   bStartMpptUpdateFlag = 0;
INT16U  wMpptUpdateWaitCnt = 0;
...
{
	swReturnNull,
	swGetLineVoltSample,		swGetLineVoltReal,			swGetBatVoltSample,			//1-3
	swGetBatVoltReal,			swGetBusVoltSample,			swGetBusVoltReal,			//4-6
	swGetInvVoltSample,			swGetInvVoltReal,			swGetOPCurrSample,			//7-9
	swGetOPCurrReal,			swGetInvLCurrSample,		swGetInvLCurrReal,			//10-12
	swGetInvDCVoltSample,		swGetInvDCVoltReal,			swGetInvLowCurrSample,		//13-15  
	swGetInvLowCurrReal,		swGetLineZeroCrossFlag,		swGetInvZeroCrossFlag,		//16-18
	swGetSynZeroCrossFlag,		swGetRSinPointer,			swGetSineTableVaule,		//19-21
	(pFunc1)swGetPeriodDelta,	(pFunc1)swGetPhaseDelta,	(pFunc1)swGetPhaseLockStep,	//22-24
	(pFunc1)spGetInvTd,			swGetPBusVRef,				swGetPBusRealAvg,			//25-27
	swGetFBVerr,				(pFunc1)swGetFBVm_P,		swGetRVBeforeSaturation,	//28-30
	swGetRInvVoltCntl,			swGetFBR_Imo_P,				swGetFBR_Is_P,				//31-33
	swGetFBR_Ierr_P,			swGetFBR_Vcmp_P,			swGetFBR_PWM,				//34-36
	swGetBuckPWM,				swGetBuckVerrP,				swGetBuckVerrI,				//37-39
	swGetRInvVoltCntl,			swGetRVref,					swGetRInvVoltError,			//40-42
	swGetRInvCurrCntl,			(pFunc1)sdwGetRRVoltLimit,	swGetRDCVoltCntl,			//43-45
	(pFunc1)swGetRSinAmp,		(pFunc1)swGetRK1,			swGetRLoadCurrCntl,			//46-48
	swGetInvLCurrSample1,		swGetInvLCurrSample2,		swGetInvDCVolt,				//49-51
	swGetRdeltaVref,			swGetdeltaVref,				swGetVoltLoopTemp,			//52-54
	swGetInvVoltVerrP,			swGetInvVoltVerrI,			swGetRepetCtrlValue,		//55-57
	swGetLineVoltSample2,		swGetInvDCVoltError,		swGetDCVoltP,				//58-60
	swGetDCVoltI,				swGetDCVoltI_Res,			swGetInvDCVolt,				//61-63
	swGetDCVoltCntl,			swGetLoadPowerWatt,			swReturnNull,				//64-66
	(pFunc1)swGetLoadPowerPercent,swGetOPShareCurrSample,	swGetKpwm,					//67-69
	swGetInvCurrSampleBiasTemp,	swGetOpCurrSampleBiasTemp,	swReturnNull,				//70-72
	swReturnNull,				swReturnNull,				swReturnNull,				//73-75
	swReturnNull,				swReturnNull,				swReturnNull,				//76-78
	swReturnNull,				swReturnNull,				swReturnNull,				//79-81
	swReturnNull,				swReturnNull,				swReturnNull,				//82-84
	swReturnNull,				swReturnNull,				swReturnNull,				//85-87
	swReturnNull,				swReturnNull,				swReturnNull,				//88-90
	swReturnNull,				swReturnNull,				swReturnNull,				//91-93
	swReturnNull,				swReturnNull,				swReturnNull,				//94-96
	swReturnNull,				swReturnNull,				swGetSCITouchFlag,			//97-99
};
...
INT8U ucSciIdleCnt[2] = {0,0};	// 1=50ms,????????ModBus??????????????????????????????????????????????????????????????????????????????????锟斤拷???????
...
void sSciTask(void)
{
	...
	gi_wInvVoltBias = swGetInvVoltBias();//
	...
	while(1)
	{
		...
		if(event&(1<<eInterfaceTimer))
		{
			...
			if(g_uwWifiLossCnt > 0)        { g_uwWifiLossCnt --; }				   //非Li电池，IOT下发间隔60S； Li电间隔120S
			else if(g_uwWifiConnectCnt)    { g_uwWifiConnectCnt = 0; sIOTInit(); } //wifi IOT下发指令后，超出延时就清除IOT数据
			/*************************************/
			if(bStartMpptUpdateFlag)
            {
				if(bStartMpptUpdateFlag == 1 && wMpptUpdateWaitCnt >= 20)
				{ sMpptUpdateDeal(); }

                if(cMpptUpdateCompleted == sMpptUpdateProc()) // 通讯超时，判定为完成，清零接收数组和长度。
                {
                    bStartMpptUpdateFlag = 0;
			...
			else // 非升级，正常解析
			{
				...
				else if(sbGetEepromBatteryType() == cLiBType && g_bWifiConnectedDelay == 0)
				{
			...
			//--------- 32.5S后，清零内置/外置MPPT数据 ---------------//
			if((++bExtSccSciCnt > 650) || (false == bExtSccActive)) // 650 * 50ms = 32.5s
			{
				bExtSccSciCnt = 0;
				sExtSccInit();
			}
			...
			sOscSettingUpdate();
			sOscDataBuffProc();
			if(!bStartMpptUpdateFlag) { sSciReInit(); } // 升级中不自动复位SCI
		}
		//---------------------------------------------//
...
void sSciRxDebugCommand(INT8U sciid)
{
	...
	while(1)
	{
		...
		else if((sciid == 1) && (bCommandBuffer[sciid][0] == g_uwBMSCmdAddrID))
		{
...
void	sQ2Command(INT8U sciid)
{
	...
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXX(sbGetInvVoltSoftFinishSts(),pDataBuf);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXX(bLowPowerflag,pDataBuf);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXX(wFanControlStatus,pDataBuf);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
	
	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(abs(sdwGetBatWatt()),pDataBuf);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
	
	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(abs(sdwGetLowBatWatt()),pDataBuf);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	  pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	 sbNumToXXXX(abs(sdwGetInvVA()),pDataBuf);
	 bStrLen += 4;
	 bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 
	
	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(abs(sdwGetLowInvVA()),pDataBuf);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 

	  pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	 sbNumToXXXx(swGetRInverterVolt(),pDataBuf);
	 bStrLen += 5;
	 bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 
	
	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXxx(swGetRLowOPCurrentNew(),pDataBuf);
	bStrLen += 5;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 

	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXx(swGetROPCurrentNew(),pDataBuf);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 
	
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXX(bPowerLimitflag,pDataBuf);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXX(sbGetRLineWaveLoss(),pDataBuf);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
		
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXX(sbGetRLineVoltLoss(),pDataBuf);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXX(sbGetRLineFreqLoss(),pDataBuf);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXX(sbGetInverterPLStatus(),pDataBuf);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	sNumToASCII(swGetEepromBatVoltAdj(), 4, 0, &bUserDataBuf0[sciid][bStrLen]);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++] = CHAR_SPACE;

	if(sdwGetEepromBatVoltBias() < 0)
	{
		bUserDataBuf0[sciid][bStrLen++] = '-';
	}
	sNumToASCII(abs(sdwGetEepromBatVoltBias()), 5, 0, &bUserDataBuf0[sciid][bStrLen]);
	bStrLen += 5;
	bUserDataBuf0[sciid][bStrLen++] = CHAR_SPACE;

	if(sdwGetEepromBatVoltBiasH() < 0)
	{
		bUserDataBuf0[sciid][bStrLen++] = '-';
	}
	sNumToASCII(abs(sdwGetEepromBatVoltBiasH()), 2, 0, &bUserDataBuf0[sciid][bStrLen]);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++] = CHAR_SPACE;

	if(gi_dwBatAdjBias < 0)
	{
		bUserDataBuf0[sciid][bStrLen++] = '-';
		sNumToASCII(-gi_dwBatAdjBias/1000, 3, 0, &bUserDataBuf0[sciid][bStrLen]);
		bStrLen += 3;
		sNumToASCII(-gi_dwBatAdjBias%1000, 3, 0, &bUserDataBuf0[sciid][bStrLen]);
		bStrLen += 3;	
	}
	...
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXx(swGetBatAvgVoltNew(),pDataBuf);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	
	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXX(swGetChgAvgCurr(),pDataBuf);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	   pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXX(swGetLoadPowerPercent(),pDataBuf);	
	bStrLen += 3;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
	
	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(swGetLoadPowerVA(),pDataBuf);  
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;

	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(abs(swGetLoadPowerWatt()),pDataBuf);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
	
	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(sdwGetInvVA(),pDataBuf);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
	...
	else
	{
	 ...
	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(abs(sdwGetInvWatt()),pDataBuf);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE; 

	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(sdwGetOPWatt(),pDataBuf);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
	
	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(sdwGetOPVA(),pDataBuf);
	bStrLen += 4;
...
*/
	...
	bUserDataBuf0[sciid][0]='(';
	 ...
	 pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(wChgPwmLimit,pDataBuf);
	bStrLen += 4;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_ENTER; 
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXX(g_bDischgFlag,pDataBuf);
	bStrLen += 2;
	bUserDataBuf0[sciid][bStrLen++]=CHAR_ENTER; 
...
extern 	INT16S  TestVlotwVm_P;
extern 	INT16S  wfeedFaword;
extern 	INT16U wRSinAmp;
extern INT16U INVLoop;
extern 	INT16S g_wInvCurrLimitRated;
extern INT16U CHOP;
extern INT16S TestwFB_CurPI_Ki ;
extern INT16S TestwFB_CurPI_Kp ;
extern INT16S TestwFB_VoltPI_Kp;
extern INT16U wATECalbDCIFlag;
extern INT16U uWFBP_CurPI_IderatCnt;
extern INT16S uWFBP_CurPI_Iderat ;
...
void	sTestCommand(INT8U sciid)
{
	...
	if((bCommandBuffer[sciid][3] == 'D') && (bCommandBuffer[sciid][4] == 'C')
		&& (bCommandBuffer[sciid][5] == 'V') &&(bCommandBuffer[sciid][6] == '?'))
	{
		bUserDataBuf0[sciid][0] = '(';
		wTemp = wDCVoltSet;
		if(wTemp < 0)
	...
	{
		wTemp = (bCommandBuffer[sciid][7] - '0') * 100 + (bCommandBuffer[sciid][8] - '0') * 10
			+ bCommandBuffer[sciid][9] - '0';
	...
	else if((bCommandBuffer[sciid][3] == 'K') && (bCommandBuffer[sciid][4] == 'P'))
	{
		wTemp = (bCommandBuffer[sciid][5] - '0') * 100 + (bCommandBuffer[sciid][6] - '0') * 10
			+ bCommandBuffer[sciid][7] - '0';
		if(wTemp >= 0 && wTemp <= 400)
	...
	else if((bCommandBuffer[sciid][3] == 'F') && (bCommandBuffer[sciid][4] == 'D'))
	{
		wTemp = (bCommandBuffer[sciid][5] - '0') * 100 + (bCommandBuffer[sciid][6] - '0') * 10
			+ bCommandBuffer[sciid][7] - '0';
	...
	else if((bCommandBuffer[sciid][3] == 'C') && (bCommandBuffer[sciid][4] == 'L')&& (bCommandBuffer[sciid][5] == '?'))
	{
		
		bUserDataBuf0[sciid][0] = '(';
		wTemp = g_wInvCurrLimitRated;//g_wInvOverCurrRated;
		
		sbNumToXXX(wTemp,&bUserDataBuf0[sciid][1]);
		bUserDataBuf0[sciid][4] = 0x0D;
		sSciWrite(sciid,bUserDataBuf0[sciid],5);
	}
	else if((bCommandBuffer[sciid][3] == 'C') && (bCommandBuffer[sciid][4] == 'L'))
	{
		wTemp = (bCommandBuffer[sciid][5] - '0') * 10 + (bCommandBuffer[sciid][6] - '0');
		
		if(wTemp >= 0 && wTemp <= 100)
		{
			g_wInvCurrLimitRated =wTemp;//g_wInvOverCurrRated =wTemp;
			sACK(sciid);
		}
		else
		{
	...
	else if((bCommandBuffer[sciid][3] == 'I') && (bCommandBuffer[sciid][4] == 'N')&&(bCommandBuffer[sciid][5] == 'O'))
	{
		wTemp =  (bCommandBuffer[sciid][6] - '0');
	...
	else if((bCommandBuffer[sciid][3] == 'C') && (bCommandBuffer[sciid][4] == 'H')&&(bCommandBuffer[sciid][5] == 'O'))
	{
		wTemp =  (bCommandBuffer[sciid][6] - '0');
	...
	else if((bCommandBuffer[sciid][3] == 'C') && (bCommandBuffer[sciid][4] == 'H')&&(bCommandBuffer[sciid][5] == 'C')\
		&&(bCommandBuffer[sciid][6] == 'P'))
	{
		wTemp =  (bCommandBuffer[sciid][7] - '0')*100+(bCommandBuffer[sciid][8] - '0')*10+(bCommandBuffer[sciid][9] - '0');
		
		if(wTemp >= 20 && wTemp <= 300)
		{
			TestwFB_CurPI_Kp =wTemp;
			sACK(sciid);
		}
		else
		{
		...
		&&(bCommandBuffer[sciid][6] == 'I'))
	{
		wTemp =  (bCommandBuffer[sciid][7] - '0')*100+(bCommandBuffer[sciid][8] - '0')*10+(bCommandBuffer[sciid][9] - '0');
		
		if(wTemp >= 20 && wTemp <= 300)
		{
			TestwFB_CurPI_Ki =wTemp;
			sACK(sciid);
		}
		else
		{
	...
	else if((bCommandBuffer[sciid][3] == 'C') && (bCommandBuffer[sciid][4] == 'H')&&(bCommandBuffer[sciid][5] == 'V')\
		&&(bCommandBuffer[sciid][6] == 'P'))
	{
		wTemp =  (bCommandBuffer[sciid][7] - '0')*100+(bCommandBuffer[sciid][8] - '0')*10+(bCommandBuffer[sciid][9] - '0');
	...
	else if((bCommandBuffer[sciid][3] == 'T') && (bCommandBuffer[sciid][4] == 'I')&&(bCommandBuffer[sciid][5] == 'D'))
	{
		wTemp =  (bCommandBuffer[sciid][7] - '0')*100+(bCommandBuffer[sciid][8] - '0')*10+(bCommandBuffer[sciid][9] - '0');
		
		if(wTemp >= 20 && wTemp <= 300)
		{
			uWFBP_CurPI_Iderat =wTemp;
			sACK(sciid);
		}
		else
		{
	...
	else if((bCommandBuffer[sciid][3] == 'T') && (bCommandBuffer[sciid][4] == 'I')&&(bCommandBuffer[sciid][5] == 'C'))
	{
		uWFBP_CurPI_IderatCnt =  (bCommandBuffer[sciid][6] - '0');
		sACK(sciid);
	}
	*/
	/*
	...
	if((bCommandBuffer[sciid][1] == 'R') && (bCommandBuffer[sciid][2] == 'L')
	&& (bCommandBuffer[sciid][3] == 'Y'))
	{
		if(bCommandBuffer[sciid][4] == 'S')
		{
			if(bCommandBuffer[sciid][5] == '1')
			{
				hoSFTRLYOn;
				sACK(sciid);
			}
			else if(bCommandBuffer[sciid][5] == '0')
			{
				hoSFTRLYOff;
				sACK(sciid);
			}
		...
		else if(bCommandBuffer[sciid][4] == 'A')
		{
			if(bCommandBuffer[sciid][5] == '1')
			{
				ho172VRLY_ON;
		...
		else if(bCommandBuffer[sciid][4] == 'B')
		{
			if(bCommandBuffer[sciid][5] == '1')
			{
				ho253VRLY_ON;
		...
		else if(bCommandBuffer[sciid][4] == 'C')
		{
			if(bCommandBuffer[sciid][5] == '1')
			{
				ho207RLY_ON;
		...
		else if(bCommandBuffer[sciid][4] == 'D')
		{
			if(bCommandBuffer[sciid][5] == '1')
			{
				ho143RLY_ON;
		...
		else if(bCommandBuffer[sciid][4] == 'E')
		{
			if(bCommandBuffer[sciid][5] == '1')
			{
				ho220RLY_ON;
		...
		else if(bCommandBuffer[sciid][4] == 'F')
		{
			if(bCommandBuffer[sciid][5] == '1')
			{
				ho130RLY_ON;
	...
	else if((bCommandBuffer[sciid][1] == 'D') && (bCommandBuffer[sciid][2] == 'C')
	&& (bCommandBuffer[sciid][3] == 'H') && (bCommandBuffer[sciid][4] == 'G')&& (bCommandBuffer[sciid][5] == 'T'))
	{
		sSetFBControlStatus(cFBInverterTest);	
		sACK(sciid);		
	}
	else if((bCommandBuffer[sciid][1] == 'P') && (bCommandBuffer[sciid][2] == 'W')
	&& (bCommandBuffer[sciid][3] == 'M') && (bCommandBuffer[sciid][4] == '0'))
	{
	...
	&& (bCommandBuffer[sciid][3] == 'H') && (bCommandBuffer[sciid][4] == 'G'))
	{
		sSetFBControlStatus(cFBInverterControl);
		sACK(sciid);
	}
	 else if((bCommandBuffer[sciid][1] == 'S') && (bCommandBuffer[sciid][2] == 'T'))
	{ 
		if((bCommandBuffer[sciid][3] == 'F') && (bCommandBuffer[sciid][4] == 'A')
		&& (bCommandBuffer[sciid][5] == 'N') &&(bCommandBuffer[sciid][6] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(wFanPWM,&bUserDataBuf0[sciid][1]);
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'F') && (bCommandBuffer[sciid][4] == 'A')
		&& (bCommandBuffer[sciid][5] == 'N'))
		{
			if(bCommandBuffer[sciid][6]=='A')
			{
				if(bCommandBuffer[sciid][7]=='O')
				{
					mFan1On();
					wSciForeceFanCtrl = true;
					sACK(sciid);
				}
				else if(bCommandBuffer[sciid][7]=='F')
				{
					mFan1Off();
					wSciForeceFanCtrl = true;
					sACK(sciid);					
			...
			else if(bCommandBuffer[sciid][6]=='B')
			{
				if(bCommandBuffer[sciid][7]=='O')
				{
					mFan2On();
			...
			{
				wTemp = (bCommandBuffer[sciid][6] - '0') * 100 + (bCommandBuffer[sciid][7] - '0') * 10
						+ bCommandBuffer[sciid][8] - '0';
				if(wTemp >= 0 && wTemp <= 100)
				{
					wFanPWM = wTemp;
					wFANPWMTempNew = wTemp;
					wSciForeceFanCtrl = true;
					sACK(sciid);
				}
				else
		...
		else if((bCommandBuffer[sciid][3] == 'T') && (bCommandBuffer[sciid][4] == 'B')
		&& (bCommandBuffer[sciid][5] == 'A') && (bCommandBuffer[sciid][6] == 'T')
		&&(bCommandBuffer[sciid][7] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetTempDegreeBat(),&bUserDataBuf0[sciid][1]);
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);			
		}
		else if((bCommandBuffer[sciid][3] == 'T') && (bCommandBuffer[sciid][4] == 'B')
		&& (bCommandBuffer[sciid][5] == 'A')&& (bCommandBuffer[sciid][6] == 'T'))
		{
			wTemp = (bCommandBuffer[sciid][7] - '0') * 100 + (bCommandBuffer[sciid][8] - '0') * 10
					+ bCommandBuffer[sciid][9] - '0';
			if(wTemp >= 0 && wTemp <= 180)
			{
				bSetForceTemp = true;
				sSetTempDegreeTxt(wTemp);
				sACK(sciid);
			}
			else
			{
		...
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'U')
		&& (bCommandBuffer[sciid][5] == 'C') && (bCommandBuffer[sciid][6] == 'K') 
		&& (bCommandBuffer[sciid][7] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetBuckDuty(),&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'U')
		...
		else if((bCommandBuffer[sciid][3] == 'F') && (bCommandBuffer[sciid][4] == 'B')
		&& (bCommandBuffer[sciid][5] == 'I') && (bCommandBuffer[sciid][6] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXXX(swGetFBVm_P()/700,&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][5] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],6);
		}
		else if((bCommandBuffer[sciid][3] == 'F') && (bCommandBuffer[sciid][4] == 'B')
		&& (bCommandBuffer[sciid][5] == 'I'))
		{
			wTemp = (bCommandBuffer[sciid][6] - '0') * 10 + (bCommandBuffer[sciid][7] - '0') ;
			
			if(wTemp >= 0 && wTemp <= 45)
			{
				sSetFBVm_P(wTemp*700);
				sACK(sciid);
			}
			else
			{
		...
		&& (bCommandBuffer[sciid][5] == 'S') && (bCommandBuffer[sciid][6] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetPBusVRef()/10,&bUserDataBuf0[sciid][1]);
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'U')
		&& (bCommandBuffer[sciid][5] == 'S'))
		{
			wTemp = (bCommandBuffer[sciid][6] - '0') * 100 + (bCommandBuffer[sciid][7] - '0') * 10
					+ bCommandBuffer[sciid][8] - '0';
			if(wTemp >= 50 && wTemp <= 450)
			{
				sSetPBusVRef(wTemp*10);
				sACK(sciid);
			}
			else
			{
		...
		&& (bCommandBuffer[sciid][5] == 'K') && (bCommandBuffer[sciid][6] == 'P')
		&&(bCommandBuffer[sciid][7] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetInvDCVoltKp(),&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'D') && (bCommandBuffer[sciid][4] == 'C')
		&& (bCommandBuffer[sciid][5] == 'K')&& (bCommandBuffer[sciid][6] == 'P'))
		{
			wTemp = (bCommandBuffer[sciid][7] - '0') * 100 + (bCommandBuffer[sciid][8] - '0') * 10
					+ bCommandBuffer[sciid][9] - '0';
			if(wTemp >= 0 && wTemp <= 999)
			{
				sSetInvDCVoltKp(wTemp);
				sACK(sciid);
			}
			else
			{
		...
		&& (bCommandBuffer[sciid][5] == 'K') && (bCommandBuffer[sciid][6] == 'I')
		&&(bCommandBuffer[sciid][7] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetInvDCVoltKi(),&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'D') && (bCommandBuffer[sciid][4] == 'C')
		...
		&& (bCommandBuffer[sciid][5] == 'R') && (bCommandBuffer[sciid][6] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			if(swGetInvDCVRef() < 0)
			{
				bUserDataBuf0[sciid][1] = '-';
				sbNumToXXXX(abs(swGetInvDCVRef()),&bUserDataBuf0[sciid][2]);	
				bUserDataBuf0[sciid][6] = 0x0D;
				sSciWrite(sciid,bUserDataBuf0[sciid],7);
			}
			else
			{
				sbNumToXXXX(swGetInvDCVRef(),&bUserDataBuf0[sciid][1]);	
				bUserDataBuf0[sciid][5] = 0x0D;
				sSciWrite(sciid,bUserDataBuf0[sciid],6);
			}
		}
		else if((bCommandBuffer[sciid][3] == 'D') && (bCommandBuffer[sciid][4] == 'C')
		&& (bCommandBuffer[sciid][5] == 'R'))
		{
			if(bCommandBuffer[sciid][6] != '-')
			{
				wTemp = (bCommandBuffer[sciid][6] - '0') * 1000 + (bCommandBuffer[sciid][7] - '0')*100 + (bCommandBuffer[sciid][8] - '0')*10+ (bCommandBuffer[sciid][9] - '0'); 
			}
			else
			{
				wTemp = (bCommandBuffer[sciid][7] - '0') * 1000 + (bCommandBuffer[sciid][8] - '0')*100 + (bCommandBuffer[sciid][9] - '0')*10+ (bCommandBuffer[sciid][10] - '0'); 
			}
		...
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'A')
		&& (bCommandBuffer[sciid][5] == 'T') && (bCommandBuffer[sciid][6] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetBatVoltRef(),&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'A')
		&& (bCommandBuffer[sciid][5] == 'T'))
		{
			wTemp = (bCommandBuffer[sciid][6] - '0') * 100 + (bCommandBuffer[sciid][7] - '0') * 10
					+ bCommandBuffer[sciid][8] - '0';
			if(wTemp >= 400 && wTemp <= 600)
			{
				sSetBatVoltRef(wTemp);
				sACK(sciid);
			}
			else
			{
		...
		&& (bCommandBuffer[sciid][5] == 'G') && (bCommandBuffer[sciid][6] == 'I')
		&&(bCommandBuffer[sciid][7] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetChgCurrHigh(),&bUserDataBuf0[sciid][1]);
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'C') && (bCommandBuffer[sciid][4] == 'H')
		...
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'K')
		&& (bCommandBuffer[sciid][5] == 'P') && (bCommandBuffer[sciid][6] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXXX(swGetBuckKp(),&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][5] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],6);
		}
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'K')
		&& (bCommandBuffer[sciid][5] == 'P'))
		{
			wTemp = (bCommandBuffer[sciid][6] - '0') * 1000 + (bCommandBuffer[sciid][7] - '0')*100 + (bCommandBuffer[sciid][8] - '0')*10+ (bCommandBuffer[sciid][9] - '0'); 
		...
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXXX(swGetBuckKi(),&bUserDataBuf0[sciid][1]);
			bUserDataBuf0[sciid][5] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],6);
		}
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'K')
		...
		else if((bCommandBuffer[sciid][3] == 'V') && (bCommandBuffer[sciid][4] == 'K')
		&& (bCommandBuffer[sciid][5] == 'P') && (bCommandBuffer[sciid][6] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXXX(swGetChgKp(),&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][5] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],6);
		}
		else if((bCommandBuffer[sciid][3] == 'V') && (bCommandBuffer[sciid][4] == 'K')
		...
		if((bCommandBuffer[sciid][3] == 'V') && (bCommandBuffer[sciid][4] == 'K')
		&& (bCommandBuffer[sciid][5] == 'I') && (bCommandBuffer[sciid][6] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXXX(swGetCCKi(),&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][5] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],6);
		}
		else if((bCommandBuffer[sciid][3] == 'V') && (bCommandBuffer[sciid][4] == 'K')
		...
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetFBKVoltageForward(),&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'F') && (bCommandBuffer[sciid][4] == 'B')
		...
		&& (bCommandBuffer[sciid][5] == 'C') && (bCommandBuffer[sciid][6] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXX(swGetFBKCurrentForward(),&bUserDataBuf0[sciid][1]);
			bUserDataBuf0[sciid][3] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],4);
		}
		else if((bCommandBuffer[sciid][3] == 'F') && (bCommandBuffer[sciid][4] == 'B')
		&& (bCommandBuffer[sciid][5] == 'C'))
		{
			wTemp = (bCommandBuffer[sciid][6] - '0') * 10 + (bCommandBuffer[sciid][7] - '0'); 
		
			if(wTemp >= 0 && wTemp <= 99)
			{
				sSetFBKCurrentForward(wTemp);
				sACK(sciid);
			}
			else
			{
		...
		else if((bCommandBuffer[sciid][3] == 'R')&&(bCommandBuffer[sciid][4] == 'P')
			&& (bCommandBuffer[sciid][5] == 'K')&&(bCommandBuffer[sciid][6] == 'P')	&& (bCommandBuffer[sciid][7] == '?') )
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetRctrlFvalue(),&bUserDataBuf0[sciid][1]);
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'R')&&(bCommandBuffer[sciid][4] == 'P')
		...
		else if((bCommandBuffer[sciid][3] == 'B')&&(bCommandBuffer[sciid][4] == 'V')
			&& (bCommandBuffer[sciid][5] == 'K')&&(bCommandBuffer[sciid][6] == 'I')	&& (bCommandBuffer[sciid][7] == '?') )
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetVoltKi(),&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'B')&&(bCommandBuffer[sciid][4] == 'V')
		...
		else if((bCommandBuffer[sciid][3] == 'M') && (bCommandBuffer[sciid][4] == 'P')
		&& (bCommandBuffer[sciid][5] == '?') )
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXX(swGetMovePoint(),&bUserDataBuf0[sciid][1]);
			bUserDataBuf0[sciid][3] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],4);
		}
		else if((bCommandBuffer[sciid][3] == 'M') && (bCommandBuffer[sciid][4] == 'P'))
		...
		else if((bCommandBuffer[sciid][3] == 'D') && (bCommandBuffer[sciid][4] == 'K')
		&& (bCommandBuffer[sciid][5] == '?') )
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(swGetDeltaK(),&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'D') && (bCommandBuffer[sciid][4] == 'K'))
		...
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'W')
		&& (bCommandBuffer[sciid][5] == '?'))
		{
			bUserDataBuf0[sciid][0] = '(';
			sbNumToXXX(g_uwBatWeakFlg,&bUserDataBuf0[sciid][1]);	
			bUserDataBuf0[sciid][4] = 0x0D;
			sSciWrite(sciid,bUserDataBuf0[sciid],5);
		}
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'W')
...
void sQGSCommand(INT8U sciid)		// SCC PWM
{
	...
	if(wBatVerifyFlg == true)	// 锟斤拷氐锟窖剐Ｗ际憋拷锟轿伙拷锟街?,一锟斤拷锟接猴拷锟皆讹拷锟斤拷锟?
	{
	...
	// 15-18:锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷(0.1A)(PS:锟斤拷锟斤拷锟斤拷锟?50A,未锟斤拷锟斤拷时+锟斤拷胤诺锟斤拷锟斤拷)
	...
	// 32-33:锟斤拷锟阶刺?10=no charge, 11=CC, 12=CV, 13=Float
...
void	sIIACommand(INT8U sciid)
{
	...
	if(wTemp < 6)
	{
		...
		wTemp =swGetInvVoltBias()+wTemp;
		// if((wTemp<120)&&(wTemp> -120))
		if((wTemp < 180) && (wTemp > -180))
		{
	//		sSetInvVoltBias(swGetInvVoltBias()+wTemp);
			sSetInvVoltBias(wTemp);
			OS_ENTER_CRITICAL();
			gi_wInvVoltBias = swGetInvVoltBias();//gi_wInvVoltBias + wTemp;
			OS_EXIT_CRITICAL();
...
void	sBOOTLOADIAPCommand(INT8U sciid)
{
	...
	OS_ENTER_CRITICAL();
	hoFanSpeed_OFF;
	//for(dwCnt = 0; dwCnt < 500; dwCnt++);//20000000
...
INT16U cal_crc_half(INT8U far *pin, INT8U len)
{
	...
	while(len--!=0) 
	{
		da=((INT8U)(crc>>8))>>4; /* ???CRC?????锟斤拷 */

		crc<<=4; /* CRC????4锟斤拷???????CRC???12锟斤拷??*/

		crc^=crc_ta[da^(*ptr>>4)]; /* CRC???4锟斤拷???????????????????????CRC?????????????CRC?????? */

		da=((INT8U)(crc>>8))>>4; /* ???CRC???4锟斤拷 */

		crc<<=4; /* CRC????4锟斤拷?? ????CRC???12锟斤拷?? */

		crc^=crc_ta[da^(*ptr&0x0f)]; /* CRC???4锟斤拷??????????????????????CRC???????????????CRC?????? */
...
void sSccDataProc(void)
{
	...
	// else
	// {
	// 	wSccDispPvVolt = suwGetMpptPVVolt()/10;//wIntSccPvVolt;
	...
	{
		if(suwGetMpptModelType() & 0x0010) // IntScc
		{
			bIntSccSciCnt = 0;
			wIntSccFwVersion = suwGetMpptFWVersion();
			wIntSccPvVolt = suwGetMpptPVVolt();
			wIntSccBattVolt = suwGetMpptBatVolt();
			wIntSccChgCurr = suwGetMpptBatCurr()/10;
		...
		else // ExtScc
		{
			...
			wExtSccPvVolt = suwGetMpptPVVolt();
	...
	else { fSccSciLoss = true; }

	if((fIntSccSciLoss == true) && (fExtSccSciLoss == false))
	{
		wSccDispPvVolt = wExtSccPvVolt/10;
		wSccDispChgCurr = wExtSccChgCurr;
		wSccDispChgPower = wExtSccChgPower;
		wSccDispHsTemp = wExtSccHsTemp;
		...
		if(wExtSccFaultFlag & cMpptHs2TempHiFault)    { wSccFaultSummyFlag |= 0x0002; } // 过温故障
		if(wExtSccFaultFlag & cMpptSCILossFault)      { wSccFaultSummyFlag |= 0x1000; } // 通讯异常
		if(wExtSccFaultFlag & cMpptSampSensorFault)   { wSccFaultSummyFlag |= 0x2000; } // 传感器异常
		if(wExtSccFaultFlag & cMpptFlagParallelFault) { wSccFaultSummyFlag |= 0x4000; } // 并机故障
		if(wExtSccFaultFlagHi)                        { wSccFaultSummyFlag |= 0x8000; } // 其它故障
	...
	{
		wSccDispPvVolt = wIntSccPvVolt/10;
		...
		if(wIntSccFaultFlag & cMpptHs2TempHiFault)    { wSccFaultSummyFlag |= 0x0002; } // 过温故障
		if(wIntSccFaultFlag & cMpptSCILossFault)      { wSccFaultSummyFlag |= 0x1000; } // 通讯异常
		if(wIntSccFaultFlag & cMpptSampSensorFault)   { wSccFaultSummyFlag |= 0x2000; } // 传感器异常
		if(wIntSccFaultFlag & cMpptFlagParallelFault) { wSccFaultSummyFlag |= 0x4000; } // 并机故障
		if(wIntSccFaultFlagHi)                        { wSccFaultSummyFlag |= 0x8000; } // 其它故障
...
void sTFANCommand(INT8U sciid)
{
	...
	wFanSpeedCtrl= (bCommandBuffer[sciid][5]-'0')*100 +(bCommandBuffer[sciid][6]-'0')+bCommandBuffer[sciid][7]-'0';
...
void sSCCMaxcurr(void)
{
    ...
    //---------------------------------------------
    // 1. 逆变输出，空载损耗 = 54V * 3.5A = 189W;
    // 2. 关逆变下，电池支持辅助电源并开启直流风机低速转动功耗约20W.
    // 3. BMS的最大充电电流、MPPT的充电电流倍率都是10，eeprom的充电电流设置是1倍
    if(fBMSConnectFlg)
    {
        if(bPVMode == cBatteryMode)
        {
            if(mBMSBatSOC == 100)
            {
                ...
                if(lBmsCurrSum > 0) { wTemp += (INT16S)lBmsCurrSum; } // 电池在放电，将电池放电电流加入PV输出功率中，降低电池能量释放速度。
            }
            else
            { 
        ...
        else
        {
            ...
            }//20240522 实测并网下，电池100%，PV不充电，电池有0.3A左右放电，PV无法控制到1A一下，设定0.2A实际无法实现，设定1A又会导致BMS报出#51。
        }
    }
    else
    {
        ...
        // {
        //     if(bPVMode != cStandbyMode) { wTemp = 10 - (INT16S)(sdwGetInvWatt() * g_wDisChgEfficiency / (INT16S)swGetBatAvgVoltNew()); }
        //     else { wTemp = 10; }
        // }
    }
    // limit
	if(wTemp < 0) { wTemp = 30; }//  限3A防止关MPPT
    ...
    if(suwGetMpptSciComOK() == true && (wSccChgMode == cMpptChargeMode) && swGetAllowSccOnFlag())
    {
...
* Mppt Update Check															        *
********************************************************************************/
INT8U sMpptUpdateProc(void)//任务执行周期50ms,一帧数据可能需要两次透传，且间隔时间50ms
{
    INT16U uwSciTemp;
	...
	while(1)
	{
		...
		else
		{
			...
			wMpptUpdateWaitCnt = 0;
		    wCommandLength[cSourceSerialPort] ++;
			pwCommandIn[cSourceSerialPort] ++;
		}
	}
	
	if(wMpptUpdateWaitCnt >= 1200)	//60s
	{
	    //g_ubSciIdleCnt[cSourceSerialPort] = 0;
		wMpptUpdateWaitCnt = 0;
		return (cMpptUpdateCompleted); 
	}
	else
	{
	    return (cMpptUpdateInPro); 
...
{
    //-----------------------------------------------
    // 电池正常 或 PV正常 或 电网正常，且 从RS232输入 且 非锂电类型 --> 开启MPPT透传升级
    //-----------------------------------------------
    ...
    else
    {
      ...
      //  pwCommandIn[cSourceSerialPort] = bCommandBuffer[cSourceSerialPort];
       // wCommandLength[cSourceSerialPort] = 0;
        wMpptUpdateWaitCnt = 0;   //g_ubSciIdleCnt[cSourceSerialPort] = 0;
        bStartMpptUpdateFlag = 1;
        OSEventSend(cPrioSuper, eSuperToStandby);//TODO 并机是否集体转待机，仍需实测。特别是三相系统时是否掉电问题。
    }
}
/********************************************************************************
* Mppt Update Check														        *
...
void sMpptUpdateDeal(void)
{
	sInitialSci(cSourceSerialPort, bRxBuffer[cSourceSerialPort], MAX_SCI_RX_BUF_SIZE, cSciTypeSci);
	sSciStopTx(cSourceSerialPort);  //只需要发送A\CR，不用队列和中断去发
	pwCommandIn[cSourceSerialPort] = bCommandBuffer[cSourceSerialPort];
	wCommandLength[cSourceSerialPort] = 0;
	bStartMpptUpdateFlag = 2;
	wMpptUpdateWaitCnt = 0;
}

INT16S swGetSCITouchFlag(void)
{
	return(g_SCITouchFlag);
}
