...
extern void sBatModeDeratInit(void);
...
//				if(sbGetEepromModeSelect() == 0)	// APL
				{
					...
					*/
					...
					if(gi_wPLLPointer > gi_wPLLPointThreeSix)
					{
					...
					if((true == sbGetRLineFreqLossStatus()) && (swGetLineFreqNew() > cFreq63Hz)&& (swGetRLineVoltNew() > cVac250v))// 160105
					{
...
INT16U swCTBiasChk(void)
{
	...
	wTemp4 = wL2OpCurrSampleBias;
	
	wOpCurrSampleBias = 0;
	wInvCurrSampleBias = 0;
	wLowOpCurrSampleBias =0;
	wL2OpCurrSampleBias =0;
	OS_EXIT_CRITICAL();
	while(1)
	{
		event=OSEventPend();		
		...
		if(event&(1<<eSuperTimer))              
		{
			...
			if(wTempCnt ++ > wSteadyTime)
			{
				...
				|| abs(swGetL2OpCurrSampleBiasTemp()) > 100)			//1A = 24cnt     120
				{
				...
				else
				{
					...
					wL2OpCurrSampleBias =swGetL2OpCurrSampleBiasTemp();
					OS_EXIT_CRITICAL();
					return(true);
				}
			}
	...
	while(1)
	{
		...
		if(event&(1<<eSuperTimer))   //20ms
	    {
	    	if((sbGetInvVoltSoftFinishSts() != true)&&(++wDelayToLineCnt<1000)&&(InvVoltRelayAction ==0)\
				&&(wBatAvgVoltNew >=swGetBatUnderVolt())&&(InvVoltSoftFalseFlag == 0))//20Sï¿½Ôºï¿½????ï¿½ï¿½É¹ï¿½Ò²ï¿½ï¿½ï¿½Ð»ï¿½ï¿½ï¿????
			{
				b2Cnt = 0;
...
INT8U sbOutputVoltChk(void)
{
	...
	if(g_fBatFastLow == 1) { uwSteadyDelayCnt = 100; } // ¼ÓÈëÑÓÊ±2S£¬±ÜÃâµç³Ø¿ìËÙµÍÑ¹µ¼ÖÂÀ´»ØÇÐ»»
	else { uwSteadyDelayCnt = 0; } // ·Çµç³Ø¿ìËÙµÍÑ¹±£»¤µÄ£¬²»×öÑÓÊ±
	...
	while(1)
	{
		...
		if(event & (1<<eSuperTimer))	//20ms
		{
			if(uwSteadyDelayCnt > 0) // ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ê±2Sï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ø¿ï¿½ï¿½Ùµï¿½Ñ¹ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ð»ï¿½ï¿½ï¿½ï¿½Çµï¿½Ø¿ï¿½ï¿½Ùµï¿½Ñ¹ï¿½ï¿½ï¿½ï¿½ï¿½Ä£ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ê?
			{
				uwSteadyDelayCnt --;
			}
			else
...
void swSccChgChk(void)		// SCC PWM
{
    ...
    //----------------------
    // ½ûÖ¹MPPT³äµç£º´ý»ú¡¢¹Ø»ú¡¢¹ÊÕÏ »ò ²¢ÍøÏÂÉèÖÃµçÍøÓÅÏÈ³äµç »ò ³äµçµçÁ÷Îª0 »ò SCCÍ¨Ñ¶Òì³£ »ò BMSÍ¨Ñ¶Òì³£¸æ¾¯
    //----------------------
...
void sWorkModeInfoUpdate(void)
{
	...
	if((sbGetBatUnder() == true) || (g_uwBatStartFail == true) \
		|| ((sdwGetWarningCode() & cBatOpen) == cBatOpen)||(fBMSStopDischgFlg))		//|| wParaBatAbnormalFlg != false
	{
...
INT16U swTestTaskProc(void)	// 20ms
{
	...
	if(wTestModeFaultId[i] == 0)
	{
		if(abs(swL2OpCurrAveCal()) > 100)
		{
			if(++wFaultCnt[i] >= 25)
			{
				wFaultCnt[i] = 0;
				wTestModeFaultId[i] = 90;
			}
		}
	}
	else
