...
const INT16U cEepromDaultTable1[cEepromTotalLength1/2] = 
{
	...
	2048,			// INT16U  wEEL2OPCurrAdj;
	0,			// INT16U  wKpKiParameter;
...
const INT16U cEepromDault48VTable2[cEepromTotalLength2/2] = 
{
	...
	0,				// INT16U MPPT chg;
	1,			// INT16U wEE2Reseverd14;    //是否显示机型
...
void sEEpromDataRangeChk(void)
{
	...
	if(mEEL2OPCurrAdj() < 1548 || mEEL2OPCurrAdj() > 2548)
	{
		OS_ENTER_CRITICAL();
		mEEL2OPCurrAdj() = 2048;
		OS_EXIT_CRITICAL();
	...
	// if(swGetInvVoltBias() > 120 || swGetInvVoltBias() < -120)
	if(swGetInvVoltBias() > 185 || swGetInvVoltBias() < -185)
	{
		sSetInvVoltBias(0);
		bFactoryChangFlag = 1;	
	}
	...
	if(sbGetEepromMaxACChgCur() != 0
		...
		&& sbGetEepromMaxACChgCur() != 90
		&& sbGetEepromMaxACChgCur() != 100
		&& sbGetEepromMaxACChgCur() != 110
		&& sbGetEepromMaxACChgCur() != 120)
	{
	...
	{
		if(sbGetEepromMaxACChgCur() > cIdc130A)
		{
			sSetEepromMaxACChgCur(cIdc30A);
			bUserChangeFlag = 1;
		}
	}
	...
	if(swGetBatteryPcs() == cbBatt48VType)
	{
		...
		if((swGetEepromBatCVVolt() > cBat60v) \
		...
		if((swGetEepromBatFloatVolt() > cBat60v) \
	...
	else if(swGetBatteryPcs()==cbBatt24VType)
	{
		...
		if((swGetEepromBatCVVolt()> cBat30v) \
		...
		if((swGetEepromBatFloatVolt() > cBat30v) \
	...
	else //cbBatt12VType
	{
		...
		if(swGetEepromBatCVVolt()> cBat15v || swGetEepromBatCVVolt()< cBat12v)
		{
			if(sbGetEepromBatteryType() != cFLDType)	// cAGM or User Defined
			{
		...
		if(swGetEepromBatFloatVolt() > cBat15v || swGetEepromBatFloatVolt() < cBat12v)
		{
			if(sbGetEepromBatteryType() != cFLDType)	// cAGM or User Defined
			{
	...
	// if(sbGetEepromBMSSetEn() > 1)//固定波特率9600，所以屏蔽
	// {
	// 	sSetEepromBMSOption(0);////DISABLE
	// 	bUserChangeFlag = 1;
	// }
...
//	}
	...
	if(swGetEEOutputVolt() != 2200 && swGetEEOutputVolt() != 2300 && swGetEEOutputVolt() != 2400)
	{
	...
	if((swGetEEEqEna() > 1) \
		...
		|| ((sbGetEepromBatteryType() == cPylonBType) && (swGetEEEqEna() == 1)))
	{
		sSetEEEqEna(0);
		bUserChangeFlag = 1;
	}
...
void    sSetEepromBMSOption(INT8U bBoolean) // 锟斤拷SetEepromBMSSetEn()
{
...
/**************************************/
INT16U	 swGetEEMachineModelOnOFF(void)
{
	return(uEepromCfg2.EepromStructCfg2.wMachineModel); 
}

void	sSetEEMachineModelOnOFF(INT16U wValue)
{
   uEepromCfg2.EepromStructCfg2.wMachineModel = wValue;
}
/*********************** */
...
INT16U   swGetEEL2OPCurrAdj(void) 
{
    return(uEepromCfg1.EepromStructCfg1.wEEL2OPCurrAdj); 
}

void    sSetEEL2OPCurrAdj(INT16U wValue) 
{
   uEepromCfg1.EepromStructCfg1.wEEL2OPCurrAdj = wValue; 
}
...
INT8U *sGet48V230DefaultTable2(void)
{
	return((INT8U *)cEepromDault48VTable2);
}

INT8U *sGet24V230DefaultTable2(void)        
{
