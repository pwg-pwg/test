...
void sNumToAscii_16Hex(INT16U wNumber,INT8U *pbBuffer)
{
	...
	{
		if(sbDigitExtract_16Hex(wNumber, bIndex) != '0') // 寻出最高位
		{
			break;
		}
	}
	...
	if(uwBMSCmdAddrIDTemp != g_uwPylonBMSCmdAddrID)
	{
		...
		{	//若BMS ID大于最大查询数据(当轮询到最后一个)
			...
			if(g_uwPylonBMSCmdAddrID > cPylonBMSAddrIDMax)
			{
				...
				{
					uwTemp = 1;
					if(++g_uwPylonBMSPollingNum > 3) { fBMSConnectFlg = 0; g_uwPylonBMSPollingNum = 4; } //If the total number of polls is greater than 3, the BMS is considered disconnected锛坒BMSConnectFlg = 0锛?
				}
