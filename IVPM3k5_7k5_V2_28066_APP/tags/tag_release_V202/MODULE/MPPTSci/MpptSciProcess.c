...
void swSetMpptRdData(INT16U wAddr, INT16U wData)
{
	if(wAddr < (sizeof(STR_MPPT_SYNC_RD)))
	{
		...
		if(++wDspSciComOKCount > 5) { wMpptSciComOK = true; wDspSciComOKCount = 6; }
...
void sDspToMpptDataUpdate(void)
{
	...
	// ???????????????
	if(fBMSConnectFlg && !mBMSFaultCode)
	{
		strMpptWrData.Field.uwChgCurrMax	= swGetMpptMaxChgCurr(); // ?????ï¿½ï¿½??5A
		strMpptWrData.Field.uwMpptEn 		= swGetAllowSccOnFlag();	     // MPPT??????????
		...
		if((fBMSStopChargeFlg)&&(swGetInvChgStatus()%10)==0)  //Ö»Õë¶ÔÓÚÍ¨ÐÅ³É¹¦£¬SOC=100%£¬BMSÍ£Ö¹³äµç£¬È¡ÏûÍ¬²½³äµç£¬MPPTÈÔÔÚ³äµç¹Ò»úÎ¬³Ö
			{strMpptWrData.Field.uwInvForceChgMode	= 0;}
		else
			{strMpptWrData.Field.uwInvForceChgMode	= 1;}
		strMpptWrData.Field.uwInvChgMode		= (INT16U)swGetInvChgStatus()%10;
	...
	else
	{
		...
		strMpptWrData.Field.uwSccProtocol	= 0;
		strMpptWrData.Field.uwInvForceChgMode	= 1; //×Ô¶¨Òå||Ç¦Ëá||½ºÌåÊ¹ÄÜÍ¬²½³äµç
		strMpptWrData.Field.uwInvChgMode		= (INT16U)swGetInvChgStatus()%10;
...
* MPPT Alarm														*
********************************************************************************/
INT16U	suwGetMpptAlarmPVHighDerat(void) 	//ï¿½ï¿½ï¿½ï¿½ï¿½Ñ¹ï¿½ß½ï¿½ï¿½ï¿?
{
	return fMpptChargerAlarmHi.bits.cfPVHighDerating;   
}

INT16U	suwGetMpptAlarmTempHighDerat(void)  //ï¿½ï¿½ï¿½ï¿½ï¿½Â¶È¸ß½ï¿½ï¿½ï¿½
{
	return fMpptChargerAlarmHi.bits.cfTempHighDerating;   
}

INT16U	suwGetMpptAlarmBatTempLow(void) 	//ï¿½ï¿½ï¿½ï¿½Â¶ÈµÍ±ï¿½ï¿½ï¿?
{
	return fMpptChargerAlarmHi.bits.cfBatTempLowAlarm; 
}

INT16U	suwGetMpptAlarmBatLow(void)  		//ï¿½ï¿½Øµï¿½Ñ¹ï¿½Í±ï¿½ï¿½ï¿?
{
	return fMpptChargerAlarmHi.bits.cfBatLow; 
}

INT16U	suwGetMpptAlarmPvVoltLoss(void) 	//ï¿½ï¿½ï¿½ï¿½ï¿½Ñ¹ï¿½ï¿½Ê§ï¿½ï¿½ï¿½ï¿?
{
