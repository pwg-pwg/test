...
extern INT16U g_uwDelayToInquireBmsCnt;
extern INT16U g_uwDelayToReplyModbusCnt;
...
//void sRDEMSRTInfoCmd(INT8U sciid, INT8U* pubTxBuff, INT8U ubAddr);
...
//void sRDEMSRTInfoData(INT8U* pubDataBuff, INT8U ubAddr);
INT16S wConvertOCTToHEX(INT8U* pubDataBuff);
void sBMSDataAnalyze(void);
// void sEMSDataAnalyze(void);
// void sEMSDataAnalyze(INT16S wCnt);
...
BMSRDCMD g_FuncBMSRDCMDGroup[cBMSCmd_Max + 1] = 
{
	sRDBMSBaseInfoCmd,
//	sRDBMSRatedInfoCmd,
	sRDBMSRTInfoCmd,
...
BMSRXCMD g_FuncBMSRXCMDGroup[cBMSCmd_Max + 1] = 
{
	sRDBMSBaseInfoData,
//	sRDBMSRatedInfoData,
	sRDBMSRTInfoData,
...
void sBMSCommandProc(INT8U sciid, INT8U* pubTxBuff, INT16U uwTimeInterval)
{
	...
	else if(g_uwBMSCmdTimeCnt==0 && g_uwDelayToInquireBmsCnt==0)
	{
		...
		// if(EMS)
		// {
		// 	g_FuncBMSRDCMDGroup[2](sciid,pubTxBuff,EMS-1);
		// 	if(uiEmsAnalyzeDelay >= 3) { sEMSDataAnalyze(EMS-1); uiEmsAnalyzeDelay = 3; } // ��ʱ3���ٽ���, ������ν���ʱ�����˾ɵ��?�����ݵ��³�����??
		// }
		// else
		{
...
void sBMSCommandRollProc(void)
{
	...
	if(g_strBMSComSts[g_uwBMSCmdAddrID].fBMSComRecieve) // �ж���ǰ���͵�ID
	{
...
//		else
		...
		else if(g_strBMSComSts[g_uwBMSCmdAddrID].fBMSCmdIndex < (cBMSCmd_Max - 1)) // ����ǰID�Ļ������ݡ�ʵʱ���ݶ���ѯ�꣬ת��һ֡
		{
			g_strBMSComSts[g_uwBMSCmdAddrID].fBMSCmdIndex ++;
		}
		else // ��ǰIDȫ�����ݲ��꣬�ж���ǰIDͨѶ�ɹ���ͨѶ׼��ȡ����ת��һID
		{
	...
	{
		if(!g_strBMSComSts[g_uwBMSCmdAddrID].fBMSComOK) // ԭʧ����ID��δ�յ��ظ�ֱ����ת��һID
		{// ���ж�ID2 �� 3 ��ͨѶ�쳣��
		...
		{
			if(++g_strBMSComSts[g_uwBMSCmdAddrID].fBMSComTimeoutCnt >= cTimeoutLimit) // ԭͨѶ������ID����ʱ�ж���  ��ͬID��ָ����������5�Σ�
			{
	...
	if(uwBMSCmdAddrIDTemp != g_uwBMSCmdAddrID)
	{
		do
		{
			if(g_uwBMSCmdAddrID > cBMSAddrIDMax) // ������һѭ��
			{
				g_uwBMSCmdAddrID = 0;
				//EMSCNT++;
				...
				{
					uwTemp = 0;
					if(g_uwBMSPollingNum++ > 3) { fBMSConnectFlg = 0; g_uwBMSPollingNum = 4; } // ��ѯ3��ȫ��BMS ID���ж�ΪͨѶ�쳣��
				}
			...
			if(!g_strBMSComSts[g_uwBMSCmdAddrID].fBMSComReady) // ͨѶ�ѳɹ� �� δͨѶ�ɹ��ģ�δ׼�����͵�
			{
				if(s_uwCycleNoOkChkEn)
				{
		...
		}while(g_uwBMSCmdAddrID > cBMSAddrIDMax || !g_strBMSComSts[g_uwBMSCmdAddrID].fBMSComReady); // ͨѶ�ѳɹ� �� δͨѶ�ɹ�
	}
}
...
void sRDBMSRTInfoCmd(INT8U sciid, INT8U* pubTxBuff, INT8U ubAddr)
{
	...
	pubTxBuff[cMODBUS_ADDR] = ubAddr;
	pubTxBuff[cMODBUS_CMD] = cREAD_CMD;
	pubTxBuff[cMODBUS_REG_ADDR_H] = (cBMSRTInfo_Addr >> 8) & 0xFF;//13
...
void sRDEMSRTInfoCmd(INT8U sciid, INT8U* pubTxBuff, INT8U ubAddr)
{
	...
	pubTxBuff[cMODBUS_ADDR] = ubAddr;
	pubTxBuff[cMODBUS_CMD] = cREAD_CMD;
	pubTxBuff[cMODBUS_REG_ADDR_H] = (cEMSRTInfo_Addr >> 8) & 0xFF;//01
	pubTxBuff[cMODBUS_REG_ADDR_L] = cEMSRTInfo_Addr & 0xFF;
	pubTxBuff[cMODBUS_REG_NUM_H] = (cBMSRTInfo_RdLen >> 8) & 0xFF;
	pubTxBuff[cMODBUS_REG_NUM_L] = cBMSRTInfo_RdLen & 0xFF;	
	CRC = CRC16_MODBUS((INT8U *)&pubTxBuff[cMODBUS_ADDR], 6);
	pubTxBuff[6] = CRC >> 8;
...
void sRDEMSRTInfoData(INT8U* pubDataBuff, INT8U ubAddr)
{
	INT16S* pwBMSBaseInfoBuff = (INT16S*)&g_strEMSRTInfo;
	INT16S wDataLength;
	wDataLength = pubDataBuff[cMODBUS_RX_REG_NUM] >> 1;
	pubDataBuff += (cMODBUS_RX_REG_NUM + 1);
	if(ubAddr > cBMSAddrIDMax)
...
INT32S lBmsCurrSum = 0;
void sBMSDataAnalyze(void)
{
	INT16S wCnt;
	INT16S wTemp=0;
	INT16S wComOKCnt = 0;
	INT32S lBmsCurrSumTemp = 0;
	UNIBMSFlg uniBMSFlg;
	INT16U uiBMSMinID = cBMSAddrIDMax;
	g_strBMSCtrlLogicInfo.wBMSBatFloatVolt = wBMSBatFloatVoltMax;//swGetEepromBatFloatVolt(); // ������õ�ѹ���BMS�ĸ���㻹�?�ͣ��п��ܻ��ػ��޷����븡�䡣��Ϊ��ʼ����?
	g_strBMSCtrlLogicInfo.wBMSBatCVVolt = wBMSBatCVVoltMax;//swGetEepromBatCVVolt();
	g_strBMSCtrlLogicInfo.wBMSBatCutOffVol = wBMSBatCutOffVoltMin;//swGetEEBatteryVoltUnder();
...
//				}
				...
				lBmsCurrSumTemp += g_strBMSRTInfo[wCnt].wBatCurr;
			}
			//-----------------------------------
		...
		lBmsCurrSum = lBmsCurrSumTemp;
		//-----------------------------------
		if(!g_strBMSCtrlLogicInfo.ubBMSConnect)
		{
			g_strBMSCtrlLogicInfo.ubBMSBatStopChargeFlag = false;
			g_strBMSCtrlLogicInfo.ubBMSBatStopDischgFlag = false;
			g_strBMSCtrlLogicInfo.wBMSBatFloatVolt = swGetEepromBatFloatVolt();//BMSͨѶ�쳣ʱ��ǿ��ʹ��������
...
//	if(EMSCNT > 3)
	{
		if(wComOKCnt > 0) { g_strBMSCtrlLogicInfo.wBMSBatSOC /= wComOKCnt; }//EMS = uiBMSMinID + 1; }
//		EMSCNT = 0;
	}
}
...
// 		{
// 			g_strBMSCtrlLogicInfo.ubBMSConnect = true;
// 			wTemp = g_strEMSRTInfo.ChargeVoltageLimit;
// 			if(g_strBMSCtrlLogicInfo.wBMSBatFloatVolt > wTemp)
// 			{
// 				g_strBMSCtrlLogicInfo.wBMSBatFloatVolt = wTemp;
// 			}
...
// 			wTemp = g_strEMSRTInfo.DischargeVoltageLimit;
// 			if(g_strBMSCtrlLogicInfo.wBMSBatCutOffVol < wTemp)
// 			{
// 				g_strBMSCtrlLogicInfo.wBMSBatCutOffVol = wTemp;
// 			}

// 			g_strBMSCtrlLogicInfo.wBMSBatMaxChgCurrent = g_strEMSRTInfo.ChargeCurrentLimit;
// 			if(g_strBMSCtrlLogicInfo.wBMSBatMaxChgCurrent > wBMSBatChgCurrMax)
// 			{
// 				g_strBMSCtrlLogicInfo.wBMSBatMaxChgCurrent = wBMSBatChgCurrMax;
// 			}

// 			g_strBMSCtrlLogicInfo.wBMSMaxDisChgCurr = g_strEMSRTInfo.DischargeCurrentLimit;
// 			if(g_strBMSCtrlLogicInfo.wBMSMaxDisChgCurr > wBMSBatDischgCurrMax)
// 			{
// 				g_strBMSCtrlLogicInfo.wBMSMaxDisChgCurr = wBMSBatDischgCurrMax;
// 			}

// 			g_strBMSCtrlLogicInfo.wBMSBatSOC = g_strEMSRTInfo.TotalSOC/10;

// 			if(g_strBMSCtrlLogicInfo.wBMSFaultCode == 0)
// 			{
// 				g_strBMSCtrlLogicInfo.wBMSFaultCode = g_strEMSRTInfo.FaultFlagHi;//����
// 			}

// 			uniEMSFlg.uwEMSFlg = g_strEMSRTInfo.BmsStatusHi;
// //				if(!uniEMSFlg.BIT.fBMSChgEn)
// //				{
// //					g_strBMSCtrlLogicInfo.ubBMSBatStopChargeFlag = true;
// //				}
// 			if(uniEMSFlg.BIT.fBMSChgEn)
...
// 				g_strBMSCtrlLogicInfo.ubBMSBatStopChargeFlag = false;
// 			}
// 			if(!uniEMSFlg.BIT.fBMSDischgEn)
// 			{
// 				g_strBMSCtrlLogicInfo.ubBMSBatStopDischgFlag = true;
// 			}
// 			if(uniEMSFlg.BIT.fBMSForeChg2 || uniEMSFlg.BIT.fBMSForeChg1)
// 			{
// 				g_strBMSCtrlLogicInfo.ubBMSForceCharge = true;
// 			}
// 			lBmsCurrSumTemp = g_strEMSRTInfo.TotalCurrent;
// 		}
// 		//-----------------------------------
// 		if(g_strBMSComSts[wCnt].fBMSFwVerErr)
// 		{
...
// 		lBmsCurrSum = 0;
// 	}
// 	g_strBMSCtrlLogicInfo.wBMSConnectNum = g_strEMSRTInfo.ParallelNumber;
