...
//                        sSetEepromEqEna(false);
        ...
        break;
		case eSetGridRelayOnDelayComp://市电继电器吸合延时补偿值
			if(((INT16S)WrData >= -30)&&((INT16S)WrData <= 30))//±3ms
			{
				if(WrData != sbGetEepromGridRelayOnDelayComp())
                {
					sSetEepromGridRelayOnDelayComp((INT16S)WrData);
	                OSEventSend(cPrioInterface, eEepromSaveUserSetData);
				}
			}
		break;
		case eSetGenRelayOnDelayComp://发电机继电器吸合延时补偿值
			if(((INT16S)WrData >= -30)&&((INT16S)WrData <= 30))//±3ms
			{
				if(WrData != sbGetEepromGenRelayOnDelayComp())
                {
					sSetEepromGenRelayOnDelayComp((INT16S)WrData);
	                OSEventSend(cPrioInterface, eEepromSaveUserSetData);
				}
			}
		break;
...
//      break;
        ...
        break;
        case eComGridRelayOnAdj:
            if(WrData <= 1)
            {
                uATEData.Stru.eComRelayOnDelayAdj = WrData;
            }
            else
            {
                ret = cErr_ParUnValid;
...
//	extern INT16S wTempDegreePower;
	extern INT16S wInvDisChgPowerLimit;
	extern INT16U g_wDCDCPv1VoltRef;
	extern INT16S wAcWattRef;
	extern INT16S wInvCtrlCurrRef;
...
//	extern INT16S wAcChgPowerLimit;
	uRealTimeData.Stru.eAnalogRsvd130 = (INT16U)wInvDisChgPowerLimit;
	uRealTimeData.Stru.eAnalogRsvd131 = wAcWattRef;
	uRealTimeData.Stru.eAnalogRsvd132 = wAcWattRef;
...
//	uRealTimeData.Stru.eAnalogRsvd138 = g_wInvCtrlPowerRef;
	uRealTimeData.Stru.eAnalogRsvd139 = g_wInvCtrlPowerRef;
	uRealTimeData.Stru.eAnalogRsvd144 = wInvCtrlCurrRef;
