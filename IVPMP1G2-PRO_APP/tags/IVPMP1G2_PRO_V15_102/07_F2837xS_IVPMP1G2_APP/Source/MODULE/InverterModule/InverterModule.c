...
//		wRNewSinAmp = wRNewSinAmpTemp + wVoltRmsLoopI;//测试
	}
	else
	{
...
//	if((sbGetEepromModeSelect() == 0 && wParaMode == cMasterMode)||mGenConnectToGridEn||\
//		(g_LineModeJoinInWay == cLineModeJoinInByGen))	 //APL
//	if((wParaMode == cMasterMode)&&((sbGetEepromModeSelect() == 0)||mGenConnectToGridEn||\
//		(g_LineModeJoinInWay == cLineModeJoinInByGen)))	 //APL或发电机接入，主机都按照锁上处理，保持主机稳定
    {
        OS_ENTER_CRITICAL();
        fInv.PhaseLocked = 1;
        OS_EXIT_CRITICAL();
...
INT8U   sbRInverterShortChk(INT16U wRInverterCurrentNew, INT8U bFilter)
{
    ...
    if((g_uwWorkMode == cBatteryMode)||((g_uwWorkMode == cStandbyMode )&&(wFBControlStatus == cFBInverterControl)))
    {
        if((wRInverterVoltNew<wInverterPVoltShort)&&(wRInverterCurrentNew > wInverterPCurrShort))     //50V  20A->10A
        {
            fInv.RShort=1;
...
INT8U   sRInverterNegPowChk(INT16S wActivePow,INT16S limit1,INT16S limit2,INT8U bFilter)
{
	...
	static INT8U    bLineToBatteryModeCnt=0;

    //if(hoInv1Relay != true || (swGetEepromOutputMode() == cOP_Single)|| g_uwWorkMode != cBatteryMode)
    if((swGetEepromOutputMode() == cOP_Single)|| g_uwWorkMode != cBatteryMode||wLineToBatteryModeFlag)
    {
    	//防止并离网切换电流过大误报
    	if(wLineToBatteryModeFlag)
		{
			bLineToBatteryModeCnt ++;
		}
		if(bLineToBatteryModeCnt > 1)
		{
			bLineToBatteryModeCnt = 0;
			wLineToBatteryModeFlag = 0;
		}
        bRNegPowChkCnt = 0;
        return(false);
    }
