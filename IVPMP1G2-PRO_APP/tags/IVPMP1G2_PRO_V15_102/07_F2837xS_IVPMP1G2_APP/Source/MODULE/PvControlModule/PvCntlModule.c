...
void sPvModuleInit()
{
	...
	else if(g_wVAType == cb12KVAModel)
	{
		...
		wPv1ChgCurrTempLimit = cPv1RatedCurr>>1;
		g_wPv1RatedCurr = cPv1RatedCurr>>1;
		wPv1MaxPower = cPv1RatedPower12K;
		wPv2PowerLimitMaxCurr = cPv2RatedCurr;
		dwPv2CurrLimit = cPv2RatedCurr;
		wPv1ChgCurrTempLimit = cPv2RatedCurr;
		g_wPv2RatedCurr = cPv2RatedCurr;
		wPv2MaxPower = cPv2RatedPower12K;
		gi_wLLCConvPWMDTSet = cLLCConvertDuty1_28KHz;
		g_wLLCConvPWMPeriod = cLLC_PWM_PERIOD_28KHz;
		uwLLCConvertDutyStart = cLLCConvertDuty3_28KHz;
		uwLLCConvertDuty2 = cLLCConvertDuty2_28KHz;
	...
	{
		wPv1PowerLimitMaxCurr = 2700;
		dwPv1CurrLimit = 2700;
		wPv1ChgCurrTempLimit = 2700;
		g_wPv1RatedCurr = 2700;
		wPv1MaxPower = cPv1RatedPower8K;
		wPv2PowerLimitMaxCurr = cPv2RatedCurr;
		dwPv2CurrLimit = cPv2RatedCurr;
		wPv1ChgCurrTempLimit = cPv2RatedCurr;
		g_wPv2RatedCurr = cPv2RatedCurr;
		wPv2MaxPower = cPv2RatedPower8K;
		gi_wLLCConvPWMDTSet = cLLCConvertDuty10_32KHz;
		...
		if(g_wVAType == cb16KVAModel)
		{
			if((wPv1ToChgMode >= 1)||(wPv2ToChgMode >= 1))
			{
				if((wFBControlStatus == cFBInverterControl))//||\
				   //((sbGetEepromOutputPriority() == cOutputSolarFristSUB)&&(!sbGetEepromACCurrLimEn())))
				{
					dwPvLoadPowerCompReal = (INT32S)(fLoadPowerP*1.07f*10000.0f);
				}
				else
				{
					dwPvLoadPowerCompReal = (INT32S)(g_wInvCtrlPowerRef*1.07f*10000.0f);
				}
			}
			else
			{
		...
		else
		{
			if((wPv1ToChgMode >= 1)||(wPv2ToChgMode >= 1))
			{
				if((wFBControlStatus == cFBInverterControl))//||\
				   //((sbGetEepromOutputPriority() == cOutputSolarFristSUB)&&(!sbGetEepromACCurrLimEn())))
				{
					dwPvLoadPowerCompReal = (INT32S)(fLoadPowerP*1.18f*10000.0f);
				}
				else
				{
					dwPvLoadPowerCompReal = (INT32S)(g_wInvCtrlPowerRef*1.18f*10000.0f);
				}
			}
			else
			{
...
//			}
	...
	if(wMpptVoltTemp < PvVolt.wValueFilter - 200)
	{
		wMpptVoltTemp = PvVolt.wValueFilter - 200;
	}
	if(wMpptVoltTemp > PvVolt.wValueFilter + 200)
	{
		wMpptVoltTemp = PvVolt.wValueFilter + 200;
...
void	sControllerRefUpdate(void)
{
	...
	if((swGetPv1DCDCCtrlSts()>=cDCDCSoft)||(swGetPv2DCDCCtrlSts()>=cDCDCSoft))//软启之后
	{
		...
		if(g_wVAType == cb8000VAModel)//8KLLC变压器匝比整体偏低
		{
...
void	sPvPowerPriorityProc(void)
{
	...
	if(swGetEepromBatMode() != cNoBatmode)
	{
		if(g_wVAType == cb16KVAModel)
		{
			if((dwBatChgCurrMaxTemps > (INT32S)swGetPvOutCurrNew()*3*10 + 2000)&&(dwBatChgCurrMaxTemps > 2000))
			{
				dwBatChgCurrMaxTemps = (INT32S)swGetPvOutCurrNew()*3*10 + 2000;
			}
		}
...
void	sPvBatChgCurrMng(void)
{
	...
	if(g_wVAType == cb16KVAModel)
	{
		if(dwMaxBatChgPowerLimit >((INT32S)g_wPvChgCurrDerateRange * (INT32S)swGetBatAvgVoltNew()*1110))
		{
			dwMaxBatChgPowerLimit = (INT32S)g_wPvChgCurrDerateRange * (INT32S)swGetBatAvgVoltNew()*1110;
		}
	}
	else if((g_wVAType == cb10KVAModel))
	{
	...
	{
		if(dwMaxBatChgPowerLimit >((INT32S)g_wPvChgCurrDerateRange * (INT32S)swGetBatAvgVoltNew()*1115))
		{
			dwMaxBatChgPowerLimit = (INT32S)g_wPvChgCurrDerateRange * (INT32S)swGetBatAvgVoltNew()*1115;
		}
	}
	...
	{
		//PV1
		dwChg1Limit = dwMaxBatChgPowerLimit / PV1Volt.wValueFilter / 10;//10/2
	...
	else if(swGetPv2DCDCCtrlSts() != cDCDCWait)
	{
		...
		//PV2
		dwChg2Limit = dwMaxBatChgPowerLimit / PV2Volt.wValueFilter / 10;//10
...
void sPv2OverVoltChk()
{
	...
	{
		if(sbUnderLevelChk(PV2Volt.wNewCalValue, cPv2OverVoltRecPoint, cPvOverRecCnt, &s_uwPvChkCnt))
		{
			s_uwPvChkCnt = 0;
...
void sPv1OverCurrChk()
{
	...
	{
		wPv1OverCurrRecPoint = 2700;//27A
		wPv1OverCurPoint = 3000;
	}
	else
	{
...
void sPvOverPowerChk()//过功率检测20ms
{
	...
	else if(g_wVAType == cb8000VAModel)
	{
		if(Pv1Status.bit.bPvOverPower == false)
		{
			if((swGetPv1WattNew() > cPvOverPowerPoint_8K)||(swGetPv2WattNew() > cPvOverPowerPoint_8K))
			{
				if(++s_uwPvChkCnt > 250)//5s
				{
					Pv1Status.bit.bPvOverPower = true;
		...
		{
			if((swGetPv1WattNew() < cPvOverPowerRecPoint_8K)||(swGetPv2WattNew() < cPvOverPowerRecPoint_8K))
			{
				if(++s_uwPvRecChkCnt > cPvOverRecCnt)//20s
				{
					Pv1Status.bit.bPvOverPower = false;
