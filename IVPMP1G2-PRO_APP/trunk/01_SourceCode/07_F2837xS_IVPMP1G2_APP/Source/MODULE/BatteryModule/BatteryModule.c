...
void sBatLowShutDownChk(INT16U bFilter)
{
	...
	if(fBMSConnectFlg)
	{
		...
		{
			if(sbUnderLevelChk(mBMSBatSOC, swGetEepromBatLowShutDownSOC() - 2, bFilter, &bBatVoltLowChkCnt)==true)
			{
...
void sBatVoltUnderChk(INT16U bFilter)
{
	...
	//if(g_uwWorkMode == cStandbyMode  || g_uwWorkMode == cPowerOnMode)   // cold start voltage check
	{
		...
		if(fBMSConnectFlg)
		{
			if(!g_uwBatStartFail)
			{
				if(sbUnderLevelChk(mBMSBatSOC, swGetEepromBatLowShutDownSOC() , bFilter, &bBatStartVoltUnderChkCnt))
				{
...
//	}
	if(fBMSConnectFlg)
	{
		...
		{
			if(sbOverLevelChk(mBMSBatSOC, swGetEepromBatACStartSOC(), bFilter, &bBatStartVoltUnderRecChkCnt))
		...
		{
			if(sbUnderLevelChk(mBMSBatSOC, swGetEepromBatLowShutDownSOC(), bFilter, &bBatVoltUnderChkCnt))
			{
...
void sNoBatChk(INT16U bFilter)
{
	...
	if(g_wNoBatFlag == 0)
	{
		if(sbUnderLevelChk(wBatAvgVoltNew, 360, bFilter, &bBatVoltLowChkCnt)==true)
		{
			g_wNoBatFlag = 1;
		}
	}
	else 
	{
		...
		if(sbOverLevelChk(wBatAvgVoltNew, 380, bFilter, &bBatVoltLowChkCnt)==true)
		{
			g_wNoBatFlag = 0;
		}
	}
...
void	sSetNewValue(PVValue_TypeDef *PVData,INT16S wVolt)
{
	static INT16S s_wRes = 0;
	INT32S dwTemp;
	
	PVData->wNewCalValue= wVolt;
	
	// Fc=a/(2*PI*t)=(1/32)/2/3.1415927/0.02s=0.25Hz
	dwTemp = (INT32S)PVData->wNewCalValue + (INT32S)PVData->wValueFilter * 31 + (INT32S)s_wRes;
	s_wRes = dwTemp & 0x001F;
	PVData->wValueFilter = dwTemp >> 5;
}

INT16S	swGetBatVoltFilter(void)
{
	return BatVolt.wValueFilter;
}

INT16S	swGetBatCurr1Filter(void)
{
	return (BatCurr1.wValueFilter);
}

INT16S	swGetBatCurr2Filter(void)
{
	return (BatCurr2.wValueFilter);
}


INT16S	swGetBatCurrFilter(void)
{
	return (BatCurr1.wValueFilter+BatCurr2.wValueFilter);
}

INT16S	swGetBatCurr1New(void)
{
	return BatCurr1.wNewCalValue;
}

INT16S	swGetBatCurr2New(void)
{
	return BatCurr2.wNewCalValue;
}
