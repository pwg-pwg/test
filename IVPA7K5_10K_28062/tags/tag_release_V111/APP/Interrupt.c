...
Function:
	1.INV Control
	2.ˆÌÐÐAD’ñ˜Ó£¬KŒ¢Ò»‚€Äæ×ƒÖÜÆÚƒÈ¸÷‚€’ñ˜ÓÀÛ¼ÓÖµ±£´æËÍ³ö¡£
	3.®”wSinPointer£½=0•r£¬°lËÍÊÂ¼þPRIO_INVERTERMODULE,EVENTID_SIN_ZERO
...
//	wSwEnterSample   = 0;
	...
	else if(bPVMode == cLineMode )
	{
		...
		if((abs(wInvLCurrSample1) > g_wInvLCurrLineLevel2)//40*25=960 
			||(wBatAvgVoltNew >(mEEBatVoltOverShut()-swGetBatteryPcs()*cBat0v5))||(ubBatVoltOveCnt > 20)\
...
//				wContinueTrig = 1;
				wInvOffTimes--;
				wInvOffsetpointer =0;
				wInvOffSinpointer +=5;// å…³é—­è§’åº¦è½¯èµ·
...
void sLineWaveChk(INT16U wCmpPoint)
{
	...
	else 
	{
		...
		if(((wLineVoltReal > wLineWaveVoltHighLoss) || (wLineVoltReal < wLineWaveVoltLowLoss))
		      &&(abs(wOPCurrReal) < 680) && s_ubCTSaturationChkCnt < 15)    // (5000/140*1.414=50.5)*2.5=126	  ±ÜÃâË²Í¶RCDÔØ£¬Îó±¨²¨ÐÎLåoss
		{
			wLineLossTemp = true;
		}
		else
...
void  swGetLineQuadraticSum(INT16S wRLineVoltRealTemp )
{
	...
	//wRLineVoltRealTemp=wRLineVoltRealTemp+50;//5Vç”µåŽ‹åç½®
...
void HardProtectChk(void)
{
	if(LiHardProtect==0)//ç¡¬ä»¶è§¦å‘ä¿æŠ¤
	{
//		LiHardProtectCNT++;
