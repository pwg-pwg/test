...
#define	cSmartPortPowerDefault			12000
...
INT16U	g_uwIDHighTemp = 0;//并机用的随机ID，用于主机竞争（序号小的为主机），和单机逻辑控制无关。
...
extern INT16S g_wOPWattPercentFilter;
...
void sUpdateLocalData(void)
{
	...
	g_strLocalParameterSet1.BIT.uwSmartPortType= swGetEESmartPortType();
	...
	{
		g_strLocalInvStatus.BIT.wLineAbnormal = 0;
	}
...
//-------------------------------------------------------------	
	if(g_uwFaultCode == cOverLoad && g_uwWorkMode == cFaultMode)
	{
		g_strLocalInvStatus.BIT.wOverLoadFail = 1;
	}
	else
	{
		g_strLocalInvStatus.BIT.wOverLoadFail = 0;
	}
//-------------------------------------------------------------	
	if(!g_uwSolarLoss)
	{
					...
					else if(g_uwCommand == cParaOPRlyOn)
					{
						if(   (g_uwWorkMode == g_strCanReceiveFrame.Data0)
						   || (   (g_uwWorkMode == cLineMode || g_uwWorkMode == cBypassMode)
						       && (g_strCanReceiveFrame.Data0 == cLineMode || g_strCanReceiveFrame.Data0 == cBypassMode)) )
						{
...
//							}
					...
					{
						if(g_strCanReceiveFrame.Data0 == g_uwPhysicalAddr || g_strCanReceiveFrame.Data0 == cNullData)
						{
							sToBeNewModule();	
						}
					}
       	             ...
       	             INT16U Word1, INT16U Word2, INT16U Word3)
{
	...
	g_strCanTxCom2.CanData0 = ((wCommandSend << 8) + (wSourceSend << 7) + wUPSNumSend) & 0x7fff;
       	             ...
       	             INT16U Word1, INT16U Word2, INT16U Word3)
{
	...
	g_strCanTxEve.CanData0 = ((wSortSend << 13) + (wNumSend << 9) + (wOrderSend << 5) + wUPSNumSend) | 0x8000;
		...
		if(g_uwParaEvent & (1 << eParallelTimer))
		{
			for(; wCnt < cParaMaxSize; wCnt ++)
			{
				if(   (g_strSystemID[wCnt].wPhysicalAdd == 0)
				   && (g_strSystemID[wCnt].wIDHigh != 0 || g_strSystemID[wCnt].wIDLow != 0) )
				{
...
INT16U swChkOPRly(void)
{
	...
	for(wCnt = 0; wCnt < g_uwParaNumMaxSize; wCnt++)
	{
		if(g_uwSlaveExsitFlg[wCnt] == 1)
		{
			...
			//-------------------------------------------------
			if(   g_strInvParaStatus.BIT.wOPRlyOn
			   && (   g_strLocalBatVoltWorkModeTemp.BIT.uwWorkMode == cBatteryMode
			       || g_strLocalBatVoltWorkModeTemp.BIT.uwWorkMode == cLineMode
			       || g_strLocalBatVoltWorkModeTemp.BIT.uwWorkMode == cBypassMode )
			  )
	...
	if(gi_wParallelEnable > cOP_Parallel) // 三相并机
	{
		...
		else if(wParaOPWaitOnCnt > 0 && uwOPModeOPRlySts == 0x1C)	//11100 -- R/S/T 三相齐全，延时8*700=5600mS 或 其它机器继电器已经吸合后160mS
		{
			g_uwOPWaitOnChkCnt ++;
...
//	INT16S	wOutputWork = false;
	...
	for(wCnt = 0; wCnt < g_uwParaNumMaxSize; wCnt++)
	{
		if(g_uwSlaveExsitFlg[wCnt] == 1)
		{
			...
			else if(g_strInvParaStatus2.BIT.wGenAbnormal == 0)
			{
				ubGenStatus |= 0x02;
			}
			//-------------------------------
...
//			}
	...
	if(   (uwExcessLoadAvg & 0x00FF) == 0x1C   // 三相并机：11100
	   || (uwExcessLoadAvg & 0x00FF) == 0x02   // 单相并机：00010
	   || (uwExcessLoadAvg & 0x00FF) == 0x01 ) // 单机 ：   00001
	{
	...
	g_uniInputStatus2.BIT.uwGenInputPhaseLoss = bGenPhaseStatus;
	//-----------------------------
	uwExcessLoadAvg >>= 8; // 提取运行状态
	if(uwExcessLoadAvg == 0x1C || uwExcessLoadAvg == 0x02 || uwExcessLoadAvg == 0x01) // 三相并机 / 单相并机 / 单机
	{
		wParaExistMachine.BIT.uwSystemMainRlyOn = 1;
	}
	else
...
//				}
	...
	*/
	...
	//==========================================================
	if(uwExcessLoadAvg == 0x02)
	{
		if( (dwLoadPercentSum != 0) && (wSystemOverloadNum != 0))     
		{
	        g_uwFaultCode = (cOverLoad);
	        sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh,cNullData,g_strLocalID.BIT.wLocalIDLow,\
	                        cToFault,cMasterSend,cNullData,\
...
//		sInverterModuleInitail();
	...
	if(g_strLocalParameterSet1Temp.BIT.uwSmartPortType != swGetEESmartPortType())
	{
		sSetEESmartPortType(g_strLocalParameterSet1Temp.BIT.uwSmartPortType);
		bSaveFlag = 1;
	}	
