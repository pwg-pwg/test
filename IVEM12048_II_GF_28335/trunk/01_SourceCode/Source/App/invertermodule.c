...
INT16S	g_wRSinAmp = 10407;//Q5  230*1.414*32
...
void sInvWattAdj(INT32S dwRInvWatt, INT32S dwSInvWatt, INT32S dwTInvWatt)
{
	...
	g_dwInvWattAvg = ((INT32S)g_dwInvWattAvg * 31 + (INT32S)(g_dwInvWatt * 1)) >> 5;
}
...
//		wStepTemp=wStep;
	...
	if(wInvPeriodCnt >= wRefPeriodCnt) // 逆变周期点数大于市电周期点数 ==> 逆变频率低于电网频率
	{
		fInv.LineFGInvF = 0;
		g_uwPeriodDelta = wInvPeriodCnt - wRefPeriodCnt; // 取正的周期点数差
	}
	else if(wInvPeriodCnt < wRefPeriodCnt)
	{
		fInv.LineFGInvF = 1;
		g_uwPeriodDelta = wRefPeriodCnt - wInvPeriodCnt; // 取正的周期点数差
	}
...
INT8U sRInverterNegPowChk(INT16S wActivePow, INT16S limit1, INT16S limit2, INT8U bFilter)
{
	...
	static INT16U uwWorkModeBack = 0;
	INT8U ubWorkModeChanged = 0;

	if(uwWorkModeBack != g_uwWorkMode)  { ubWorkModeChanged = 1; }
	uwWorkModeBack = g_uwWorkMode;

	if(!mChkOPRlyOn() || g_uwWorkMode != cBatteryMode || g_uwFaultCode == cInvShort || ubWorkModeChanged)	//!gi_wParallelEnable
	{
		bRNegPowChkCnt = 0;
		return(false);
	}
