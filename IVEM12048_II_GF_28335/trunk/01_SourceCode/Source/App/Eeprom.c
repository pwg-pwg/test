...
const struct EepromStruct2 cEepromDaultTable2 =
{
    ...
    0,                    //        INT16S    wEEACRange; -- 0 = APL, 1 = UPS
    ...
    1,                    //      INT16U  wEEPowerDisplayType;
    12000,                //      INT16U  wEEFeedPower;
    0,                    //      INT16U  wEEIslandProtectEn;
    cSettingMaxDisChgCurr*10,//		INT16U	  wEEBatDisChgCurrMax;
	0xAAAA,
    0
};
...
void sEEpromDataRangeChk(void)
{
    ...
    if(swGetEEDSPDcdcCurrAdj() > 2548 || swGetEEDSPDcdcCurrAdj() < 1548)
    {
...
//    if(swGetEEOutputVolt() != 2300)    
    ...
    {
        sSetEESmartPortType(0);
        bUserChangeFlag = 1;
    }
    // R_Gen_Max_Power
    if(swGetEEGenPowerMax() < 0 || swGetEEGenPowerMax() > 800) // 0~80kW
    {
        sSetEEGenPowerMax(120);
        bUserChangeFlag = 1;
    }
    // R_Gen_Charge_Enable
    ...
    // Smart_port_Out_In_ACmode
    if(swGetEEOP2OnInACModeEn() < 0 || swGetEEOP2OnInACModeEn() > 1) // OFF,ON
    {
        sSetEEOP2OnInACModeEn(1);
        bUserChangeFlag = 1;
    }
    //Display Type
    if(swGetEEPowerDisplayType() > 1)
    {
        sSetEEPowerDisplayType(0);
        bUserChangeFlag = 0;
    }
    // Feed Power Set
    if(swGetEEFeedPower() < 0 || swGetEEFeedPower() > 12000)
    {
        sSetEEFeedPower(12000);
        bUserChangeFlag = 1;
    }
	if(swGetEEBatDisChgCurrMax() > (cSettingMaxDisChgCurr*10) || swGetEEBatDisChgCurrMax() < 10)
	{
		sSetEEBatDisChgCurrMax(cSettingMaxDisChgCurr*10);
		bUserChangeFlag = 1;
	}
    //------------------------------------
...
//--------------------------------------------------
void sSetEEDSPDcdcCurrAdj(INT16S wValue)
{
    OS_ENTER_CRITICAL();
    uEepromCfg1.EepromStructCfg1.wEEDSPDcdcCurrAdj = wValue;
    OS_EXIT_CRITICAL();
}
INT16S swGetEEDSPDcdcCurrAdj(void)
{
    return(uEepromCfg1.EepromStructCfg1.wEEDSPDcdcCurrAdj);
}
...
//-------------------------------------------------- 功率折算
void sSetEEPowerDisplayType(INT16S wValue)
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.wEEPowerDisplayType = wValue;
    OS_EXIT_CRITICAL();
}
INT16S swGetEEPowerDisplayType(void)
{
    return(uEepromCfg2.EepromStructCfg2.wEEPowerDisplayType);
}

//-------------------------------------------------- 馈电
void sSetEEFeedPower(INT16S wValue)
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.wEEFeedPower = wValue;
    OS_EXIT_CRITICAL();
}
INT16U swGetEEFeedPower(void)
{
    return(uEepromCfg2.EepromStructCfg2.wEEFeedPower);
}

//--------------------------------------------------
void sSetEEIslandProtectEn(INT16S wValue)
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.wEEIslandProtectEn = wValue;
    OS_EXIT_CRITICAL();
}
INT16U swGetEEIslandProtectEn(void)
{
    return(uEepromCfg2.EepromStructCfg2.wEEIslandProtectEn);
}
//--------------------------------------------------
...
void sSetEEBatDisChgCurrMax(INT16U uwValue)
{
	OS_ENTER_CRITICAL();
	uEepromCfg2.EepromStructCfg2.wEEBatDisChgCurrMax = uwValue;
	OS_EXIT_CRITICAL();
}

INT16U swGetEEBatDisChgCurrMax(void)
{
	return(uEepromCfg2.EepromStructCfg2.wEEBatDisChgCurrMax);
}
