...
void	sSetRlyPointer(unsigned char relaytype,unsigned int pointer,unsigned char event,unsigned char prio)
{
	...
	{
		wRlyActionPointerTemp = (INT16U)((INT32U)pointer * cSystemBusFreq * 50 / g_wPwmPeriod);
		OS_ENTER_CRITICAL();
	...
	else if(relaytype == RLY_ZEROPOINT)
	{
		...
		OS_ENTER_CRITICAL();
		wRlyActionPointerTemp = (((wRlyActionPointerTemp + gi_wPLLCtrlPointer + 2) / gi_wPLLPointThreeSix + 1) * gi_wPLLPointThreeSix)
