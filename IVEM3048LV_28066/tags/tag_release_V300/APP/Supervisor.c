	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			if(subGetParaBatOpenSts() || uwLLCWorkFlg)
			{
				...
				uwBusSoftPoint = (INT16U)(((INT32U)uwBusSoftPoint * 181) >> 7) - 900;//cBusVoltReal50V; 
...
void sFaultMode(void)
{
	...
	|| uwFaultCodeTemp == cConvertHCircirtTempOver || uwFaultCodeTemp == cInnelTempOver)
	{
		g_uwOverTempCnt++;
	}
				...
				|| uwFaultCodeTemp == cConvertHCircirtTempOver || uwFaultCodeTemp == cInnelTempOver)
			{
				if(uwOverTempRestoreCnt > 1800)//60*30 = 1800 = 30Min Single recovery
				{
					if(swGetEEOverTempRstEn() && !fInvTempOver && !fConvertLTempOver
						&& !fInnelTempOver && !fConvertHTempOver && g_uwOverTempCnt < 3)
					{
						uwFaultRestartFlg = true;
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			else
			{
				...
				&& g_uwSolarVolt1Avg < 800 && g_uwRLineVolt < 550 && hoEEWPON && !mLCDPage)
				{
					uwShutDownChkCnt++;
