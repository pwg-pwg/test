...
//	g_dwROPWatt = ((INT32S)g_dwROPWatt * g_wOPVoltTransferRatio) >> 10;
...
//	if(g_uwMachineType == cOffGrid3LoadBat)
	{
		if(g_uwWorkMode == cBatteryMode)
		{
			g_wOPLineRatioPercent = g_wOPPercent;
		}
		else if(g_uwWorkMode == cLineMode || g_uwWorkMode == cBypassMode)
		{
			g_wROPLineRatioPercent = (INT32S)g_wOPPercent * swGetEEOutputVolt() / g_uwRLineVolt;
	...
	if(g_uwWorkMode == cBatteryMode)
	{
		...
		if(++s_uwInvVoltAvgCnt > 5)
		{
			...
			if(swGetEEOutputVolt() < cACOPRateVolt)
			{
				...
				wOPVoltRatio = (INT16S)(((INT32S)cACOPRateVolt * wOPVoltRatio) / s_uwInvVoltAvg);//Ratio = Prated/Preal  P = U^2/R   
			}
			else
			{
				wOPVoltRatio = (INT16S)(((INT32S)swGetEEOutputVolt() << 10) / s_uwInvVoltAvg);
				wOPVoltRatio = (INT16S)(((INT32S)swGetEEOutputVolt() * wOPVoltRatio) / s_uwInvVoltAvg);//Ratio = Prated/Preal  P = U^2/R
			}
