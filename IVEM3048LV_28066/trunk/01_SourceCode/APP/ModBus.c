...
//				}
		...
		case E2_BattVoltBackToUtility:
			...
			{
				if((swGetEEBatWeakVolt() != WrData) && ((WrData +(cBatVoltReal1V * cBatSerialPcs))< swGetEEBatWeakBackVolt()))//(BatWeakVolt + 4V) >  BatWeakBackVolt
				{
					g_uwParameterSetChange = true;
		...
		case E2_BattVoltBackToBatt:
			...
			{
				if((swGetEEBatWeakBackVolt() != WrData) && ((swGetEEBatWeakVolt() + (cBatVoltReal1V * cBatSerialPcs))< WrData))//(BatWeakVolt + 4V) >  BatWeakBackVolt
				{
					g_uwParameterSetChange = true;
...
void sRealTimeDataUpdate(void)
{
	...
	uRealTimeData.Stru.wAlarmMsg[0] = 0;//(INT16U)(uniWarningCode.uwWarningInfo >> 16);
	uRealTimeData.Stru.wAlarmMsg[1] = uniWarningCode.uwWarningInfo;//(INT16U)(uniWarningCode.uwWarningInfo & 0x0000FFFF);
	if(g_uwWorkMode == cFaultMode)
	{
	...
	uRealTimeData.Stru.wInvTempC = g_wConvertHTemp/10;
	...
	uRealTimeData.Stru.wSccHsTemp = g_wInvTemp/10;
