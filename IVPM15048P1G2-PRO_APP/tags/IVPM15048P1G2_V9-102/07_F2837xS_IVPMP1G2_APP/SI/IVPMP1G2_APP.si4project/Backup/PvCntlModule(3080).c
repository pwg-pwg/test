...
extern INT16S wPVBusVoltReal;
extern INT16S wPV1VoltReal;
extern INT16S wPV2VoltReal;
extern INT16S wPV3VoltReal;
extern INT16S wPV4VoltReal;
extern INT16S wPV1CurrReal;
extern INT16S wPV2CurrReal;
extern INT16U wFaultCode;
extern INT16U wPvOverTempPoint;
extern INT16U wPvOverTempBackPoint;
...
INT16S  wPv1PowerLimitMaxCurr = cDCDCCntlCurrntLimit;
INT16S  wPv2PowerLimitMaxCurr = cDCDCCntlCurrntLimit>>1;
...
//INT32S 	dwPv1ChgLimit = 350;		// PV1充电电流限制（设定充电电流）BAT = 3.50 * 11 = 38.5
INT32S 	dwPv1ChgLimit = cDCDCCntlCurrntLimit;
INT16S 	wPv1ChgCurrTempLimit = cDCDCCntlCurrntLimit;
...
INT32S 	dwPv2ChgLimit = cDCDCCntlCurrntLimit>>1;		// PV2充电电流限制（设定充电电流）BAT = 3.50 * 11 = 38.5
INT16S 	wPv2ChgCurrTempLimit = cDCDCCntlCurrntLimit>>1;
...
INT16U gi_wDCDCConvPWMValue = 0;
INT16U gi_wDCDCConvPWMValueLast = 0;
INT16U gi_wDCDCConvPWMPeriod = 0;
#ifdef DEBUG_INVERTER
INT16U gi_wDCDCConvPWMDTSet = cDCDCConvertDuty1;
#else
//INT16U gi_wDCDCConvPWMDTSet = cDCDCConvertDuty3;//25k
INT16U gi_wDCDCConvPWMDTSet = cDCDCConvertDuty1;
#endif
...
INT16U g_uwLLCSoftFreqRate = 20;	// 200ms @ 1cnt=50us
INT16U gi_wDCDCConvPWMPeriodStart = cDCDCConvertPeriodMax;
INT16U gi_wDCDCConvPWMPeriodSet = cDCDCConvertPeriodDef;

INT16S gi_wPv1ChgEnDisable = true;
INT16S gi_wPv2ChgEnDisable = true;
INT16S g_wPvUnderVolt = 1200;
...
void sMPPT1ParaInit(void)
{
	...
	Mppt1.wMpptVoltRef = (INT16S)((INT32S)PV1Volt.wNewCalValue * 218 >> 8);	// MPP=0.85*OPEN
...
void sMPPT2ParaInit(void)
{
	...
	Mppt2.wMpptVoltRef = (INT16S)((INT32S)PV2Volt.wNewCalValue * 218 >> 8);	// MPP=0.85*OPEN
...
void sPvControlTest(void)
{
	...
	if(++wSoftCnt >= 10)
	{
		...
		if(wLlcDuty < (cLLC_PWM_PERIOD - gi_wDCDCConvPWMDTSet))
		{
			wLlcDuty ++;
		}
	}
	...
	mPVBoost2PWMCOMP = wBoostDuty;

	mLLCHPwmPrd = cLLC_PWM_PERIOD;
	mLLCLPwmPrd = cLLC_PWM_PERIOD;
	mLLCHLUPWMCOMP = wLlcDuty;
	mLLCHLDPWMCOMP = cLLC_PWM_PERIOD - wLlcDuty;
	mLLCLLUPWMCOMP = wLlcDuty;
	mLLCLLDPWMCOMP = cLLC_PWM_PERIOD - wLlcDuty;
...
void sPv1Control(void)
{
	...
	g_dwPv1BusVoltSum += wPVBusVoltReal;
	g_dwPv1VoltSum += wPV1VoltReal;
	...
	{
		dwChgLimitMax = dwPv1ChgLimit + g_wPv1CurrComp;	// 目标电流
		if(dwChgLimitMax > wPv1ChgCurrTempLimit)
		{
			dwChgLimitMax = wPv1ChgCurrTempLimit;
		}	
	}
	else
	{
...
//			}
		...
		if(g_dwPDCDCImo1 > cDCDCCntlCurrntLimit)		// 系统最大允许单路最大的充电电流
		{
			g_dwPDCDCImo1 = cDCDCCntlCurrntLimit;
		}
	...
	g_wPDCDCIErr1 = g_dwPDCDCImo1 - wPV1CurrReal;	// PV1电流	// 控制的是PV侧功率
	if(wPV1CurrReal < 0)
	{
...
void sPv2Control(void)
{
	...
	g_dwPv2BusVoltSum += wPVBusVoltReal;
	g_dwPv2VoltSum += wPV2VoltReal;
	...
	{
		dwChgLimitMax = dwPv2ChgLimit + g_wPv2CurrComp;	// 目标电流
		if(dwChgLimitMax > wPv2ChgCurrTempLimit)
		{
			dwChgLimitMax = wPv2ChgCurrTempLimit;
		}
	}
	else
	{
...
//			}
		...
		if(g_dwPDCDCImo2 > (cDCDCCntlCurrntLimit>>1))		// 系统最大允许单路最大的充电电流
		{
			g_dwPDCDCImo2 = cDCDCCntlCurrntLimit>>1;
		}
	...
	g_wPDCDCIErr2 = g_dwPDCDCImo2 - wPV2CurrReal;	// PV2电流	// 控制的是PV侧功率
	if(wPV2CurrReal < 0)
	{
...
//	}
	...
	if(!mChkDCDCLLCOn())
	{
		...
		mLLCHPwmPrd = cDCDCConvertPeriodMax;
		mLLCLPwmPrd = cDCDCConvertPeriodMax;

		mLLCHLUPWMCOMP = 0;
		mLLCHLDPWMCOMP = cDCDCConvertPeriodMax;
		mLLCLLUPWMCOMP = 0;
		mLLCLLDPWMCOMP = cDCDCConvertPeriodMax;
	}
	else
	{
		...
		if(gi_wDCDCConvPWMDTSet < cDCDCConvertDuty2)
		{
			gi_wDCDCConvPWMDTSet = cDCDCConvertDuty2;
		}
		else if(gi_wDCDCConvPWMDTSet > cDCDCConvertDuty10)
		{
			gi_wDCDCConvPWMDTSet = cDCDCConvertDuty10;
		}
		...
		else if(gi_wDCDCConvPWMValue < (cDCDCConvertPeriodMax - gi_wDCDCConvPWMDTSet))	// 占空比软起
		{
			if(++uwLLCSoftCnt >= g_uwLLCSoftDutyRate)
			{
				uwLLCSoftCnt = 0;
				gi_wDCDCConvPWMValue+=2;
			}
			mEnLLCPWMOut();
			mDisLlcForcePWMLow();
		}
		...
		{
			gi_wDCDCConvPWMValue = gi_wDCDCConvPWMPeriod  - gi_wDCDCConvPWMDTSet;
		}
		
		if(gi_wDCDCConvPWMPeriod > cLLC_PWM_PERIOD)
		{
			gi_wDCDCConvPWMPeriod = cLLC_PWM_PERIOD;
		}
		...
		wTemp = gi_wDCDCConvPWMPeriod - gi_wDCDCConvPWMDTSet;	// Max Duty = 47%
		if(gi_wDCDCConvPWMValue > wTemp)
		{
			gi_wDCDCConvPWMValue = wTemp;
		}

		if(gi_wDCDCConvPWMValueLast != gi_wDCDCConvPWMValue)
		{
            mLLCHPwmPrd = gi_wDCDCConvPWMPeriod;	// H
            mLLCHLUPWMCOMP = gi_wDCDCConvPWMValue;
            mLLCHLDPWMCOMP = gi_wDCDCConvPWMPeriod - gi_wDCDCConvPWMValue;

            mLLCLPwmPrd = gi_wDCDCConvPWMPeriod;	// L
            mLLCLLUPWMCOMP = gi_wDCDCConvPWMValue;
            mLLCLLDPWMCOMP = gi_wDCDCConvPWMPeriod - gi_wDCDCConvPWMValue;

			//EPwm10Regs.CMPA.bit.CMPA = gi_wDCDCConvPWMValue;
			//EPwm10Regs.CMPB.bit.CMPB = gi_wDCDCConvPWMPeriod - gi_wDCDCConvPWMValue;
		}
		gi_wDCDCConvPWMValueLast = gi_wDCDCConvPWMValue;
	}
}
...
void sPvPowerCompCal(void)
{
	if(wPvControlStatus != cPVChg)
	{
		g_wPv1CurrComp = 0;
		g_wPv2CurrComp = 0;
		g_wPv1CurrCompTemp = 0;
		g_wPv2CurrCompTemp = 0;
		return;
	}
	...
	if((wFBControlStatus == cFBDischg)||(wFBControlStatus == cFBInverterControl))
	{
		if((wPv1ToChgMode >= 1)&&(wPv2ToChgMode >= 1))
		{
			...
			{
				g_wPv1CurrCompTemp = fLoadPowerP*666.67f*1.07f/swGetPv1VoltNew();//2/3*1000
				g_wPv2CurrCompTemp = fLoadPowerP*333.33f*1.07f/swGetPv2VoltNew();
			}
			else
			{
				g_wPv1CurrCompTemp = (F32)g_wInvCtrlPowerRef*666.67f*1.07f/swGetPv1VoltNew();//2/3*1000
				g_wPv2CurrCompTemp = (F32)g_wInvCtrlPowerRef*333.33f*1.07f/swGetPv2VoltNew();
			}
			if(g_wPv1CurrCompTemp < 0)
			{
				g_wPv1CurrCompTemp = 0;
			}
		...
		else if(wPv1ToChgMode >= 1)
		{
			if(wFBControlStatus == cFBInverterControl)
			{
				g_wPv1CurrCompTemp = fLoadPowerP*1000.0f*1.07f/swGetPv1VoltNew();
			}
			else
			{
				g_wPv1CurrCompTemp = (F32)g_wInvCtrlPowerRef*1000.0f*1.07f/swGetPv1VoltNew();
			}
			if(g_wPv1CurrCompTemp < 0)
			{
				g_wPv1CurrCompTemp = 0;
		...
		else if(wPv2ToChgMode >= 1)
		{
			...
			if(wFBControlStatus == cFBInverterControl)
			{
				g_wPv2CurrCompTemp = fLoadPowerP*1000.0f*1.07f/swGetPv2VoltNew();
			}
			else
			{
				g_wPv2CurrCompTemp = (F32)g_wInvCtrlPowerRef*1000.0f*1.07f/swGetPv2VoltNew();
			}
			if(g_wPv2CurrCompTemp < 0)
			{
				g_wPv2CurrCompTemp = 0;
			}
		...
		//上升速度需要比逆变并网放电功率慢
		if(g_wPv1CurrComp < g_wPv1CurrCompTemp - 1)
		{
			g_wPv1CurrComp += 1;//0.05A
		}
		else
		{
			g_wPv1CurrComp = g_wPv1CurrCompTemp;
		}
		if(g_wPv2CurrComp < g_wPv2CurrCompTemp - 1)
		{
			g_wPv2CurrComp += 1;//0.05A
		}
		else
		{
			g_wPv2CurrComp = g_wPv2CurrCompTemp;
		}
	}
	else//充电或其他
	{
...
void sBoostMpptMethod(MpptCal_TypeDef *Mppt,PVValue_TypeDef PvVolt,PVValue_TypeDef PvCurr)
{
	INT16S	wVoltTemp = 0;
	INT32S	dwWattTemp = 0;
	...
	if(fBatChgCtrl.bits.bMpptPause == true)
	{
		...
		Mppt->wMpptVoltRef = PvVolt.wValueFilter;
		Mppt->dwMpptWattPre = 0;
		Mppt->wMpptVoltPre = Mppt->wMpptVoltRef + Mppt->wMpptVoltStep;
		return;
	}
	
	Mppt->dwPvWattSum += (INT32S)PvVolt.wValueFilter * PvCurr.wValueFilter/1000;
	...
	if(PvCurr.wValueFilter > 0)
	{
		
		dwWattTemp = (INT32S)PvVolt.wValueFilter * PvCurr.wValueFilter;	// 追踪功率参考
		//dwWattTemp = (INT32S)swGetPvBusVoltNew() * PvCurr.wValueFilter;	// 追踪功率参考
	    //dwWattTemp = (INT32S)BatVolt.wValueFilter * PvCurr.wValueFilter; // 追踪功率参考
	    //dwWattTemp = (INT32U)BatVolt.wValueFilter * PVOutCurr.wValueFilter*3;   // 追踪功率参考
	}
	else
	{
		dwWattTemp = 0;
	...
	wVoltTemp = PvVolt.wValueFilter;
	//dwWattTemp = Mppt->dwPvWattSum;	// 追踪功率参考
	
	
	wInputVoltUpLimit = PvVolt.wValueFilter + cPV_INPUT_VOLT_ERR;
...
//			}
		...
		if(dwWattTemp > Mppt->dwSweepMaxWatt)//记录全范围最大功率及电压点
		{
			...
			Mppt->wMpptPeekPowerVolt = PvVolt.wValueFilter;
		}
	...
	else
	{
		...
		{
			if(PvVolt.wValueFilter < Mppt->wMpptPeekPowerVolt-60)
			{
				wMpptVoltTemp = Mppt->wMpptVoltRef + Mppt->wMpptVoltStep * 3;
			}
			else if(PvVolt.wValueFilter > Mppt->wMpptPeekPowerVolt+60)
			{
				wMpptVoltTemp = Mppt->wMpptVoltRef - Mppt->wMpptVoltStep * 3;
			}
			else
		...
		else if(PvVolt.wValueFilter > wInputVoltUpLimit)
		{
			wMpptVoltTemp = Mppt->wMpptVoltRef - Mppt->wMpptVoltStep;
		}
...
void	sControllerRefUpdate(void)
{
	...
	{
		g_uwPvBusSoftVolt = (INT16U)(((INT32U)uwBatChgVoltRef * cTransformerRatio));	
...
void	sPvPowerPriorityProc(void)
{
	...
	{
	  if(dwBatChgCurrMaxTemps > mBMSBatMaxChgCurr*10)
	  {
		  dwBatChgCurrMaxTemps = mBMSBatMaxChgCurr*10;
	  }
	}
...
void	sPvBatChgCurrMng(void)
{
	...
	dwBatVoltRef = swGetBatVoltFilter();//0.01V
	//if(sbGetBatOpen())
	if(0)
	{
		dwChgLimit = 500;
	...
	else if(dwBatChgCurrMaxAdj > 0)
	{
		...
		else
		{
			...
			if((wPv1ToChgMode>=3)||(wPv2ToChgMode>=3))
			{
				//dwMaxBatChgPowerLimit = (INT32U)swGetEEBatChgCurrMax() * dwBatVoltRef;	// 获得最大电池充电功率
				dwMaxBatChgPowerLimit = (INT32S)dwBatChgCurrMaxAdj * dwBatVoltRef;	// 获得最大电池充电功率
				//dwMaxBatChgPowerLimit = (INT32S)((dwBatChgCurrMaxAdj * dwBatVoltRef * 34)>>5);
			}
			else
			{
				//dwMaxBatChgPowerLimit = (INT32U)swGetEEBatChgCurrMax() * swGetBatVoltNew();	// 获得最大电池充电功率
				dwMaxBatChgPowerLimit = (INT32S)dwBatChgCurrMaxAdj * dwBatVoltRef;	// 获得最大电池充电功率
			}
			...
			if((wPv1ToChgMode>=3)||(wPv2ToChgMode>=3))
			{
				...
				if(swGetBatChgCurrMax() == dwBatChgCurrMaxTemps)	
				{
					uwCurrDelay++;
					if(uwCurrDelay >= 2)//150
					{
						uwCurrDelay  = 0;
						...
						else if(wPvBatChgCurrComp < -2500)
						{
							wPvBatChgCurrComp = -2500;
						}
						//wPvBatChgCurrComp = 0;//不补偿
					}
				}
				else	// 希望在目标电流存在调整(如降额时)，误差量不会突变
				{
					uwCurrDelay = 0;
					if(wPvBatChgCurrComp > 20)
					{
...
//					if(wBatChgCurrErr < -300)//-3A
//					{
//						wPvBatChgCurrComp -= 25;
//					}
				}
//				if((g_wPv1CurrComp > 20)||(g_wPv2CurrComp > 20))//如果再带负载，则先去掉补偿
//				{
//					wPvBatChgCurrComp = 0;
//				}
			}
	...
	wPv1PowerLimitMaxCurr = ((INT32S)cPv1MaxPower*1000/PV1Volt.wValueFilter);
	wPv2PowerLimitMaxCurr = ((INT32S)cPv2MaxPower*1000/PV2Volt.wValueFilter);
	if(wPv1PowerLimitMaxCurr > cDCDCCntlCurrntLimit)
	{
		wPv1PowerLimitMaxCurr = cDCDCCntlCurrntLimit;
	}
	if(wPv2PowerLimitMaxCurr > (cDCDCCntlCurrntLimit>>1))
	{
		wPv2PowerLimitMaxCurr = cDCDCCntlCurrntLimit>>1;
	}

	if((swGetPv1DCDCCtrlSts() != cDCDCWait)&&(swGetPv2DCDCCtrlSts() != cDCDCWait))
	{
		//PV1
		dwChgLimit = (dwMaxBatChgPowerLimit / PV1Volt.wValueFilter) * 2 / 30;//2/3功率
		wPvChgCurrErr = dwPvBatChgPowerErrComp / PV1Volt.wValueFilter * 2 / 30;//为0
	    dwPv1ChgLimitTemp = dwChgLimit + wPvChgCurrErr;
		if(dwPv1ChgLimitTemp > (cDCDCCntlCurrntLimit))
		{
			dwPv1ChgLimitTemp = (cDCDCCntlCurrntLimit);
		}
		...
		//PV2
		dwChgLimit = dwMaxBatChgPowerLimit / PV2Volt.wValueFilter / 30;//1/3功率
		wPvChgCurrErr = dwPvBatChgPowerErrComp / PV2Volt.wValueFilter / 30;//为0
		dwPv2ChgLimitTemp = dwChgLimit + wPvChgCurrErr;
		if(dwPv2ChgLimitTemp > (cDCDCCntlCurrntLimit>>1))
		{
			dwPv2ChgLimitTemp = (cDCDCCntlCurrntLimit>>1);
		}
	...
	{
		//PV1
		dwChgLimit = dwMaxBatChgPowerLimit / PV1Volt.wValueFilter / 10;//10/2 一半功率
		wPvChgCurrErr = dwPvBatChgPowerErrComp / PV1Volt.wValueFilter/10;//为0
	    dwPv1ChgLimitTemp = dwChgLimit + wPvChgCurrErr;
		if(dwPv1ChgLimitTemp > (cDCDCCntlCurrntLimit))
		{
			dwPv1ChgLimitTemp = (cDCDCCntlCurrntLimit);
	...
	else if(swGetPv2DCDCCtrlSts() != cDCDCWait)
	{
		...
		//PV2
		dwChgLimit = dwMaxBatChgPowerLimit / PV2Volt.wValueFilter / 10;//10/2 一半功率
		wPvChgCurrErr = dwPvBatChgPowerErrComp / PV2Volt.wValueFilter/10;//为0
	    dwPv2ChgLimitTemp = dwChgLimit + wPvChgCurrErr;
		if(dwPv2ChgLimitTemp > (cDCDCCntlCurrntLimit>>1))
		{
			dwPv2ChgLimitTemp = (cDCDCCntlCurrntLimit>>1);
...
{
	static INT8U s_uwPvVoltChkCnt = 0;
	
	if(Pv1Status.bit.bPvOverVolt)	// PV1过压
	{
		if(sbUnderLevelChk(PV1Volt.wNewCalValue, cPv1OverVoltRecPoint, cPvOverRecCnt, &s_uwPvVoltChkCnt))
		{
			s_uwPvVoltChkCnt = 0;
	...
	{
		if(sbOverLevelChk(PV1Volt.wNewCalValue, cPv1OverVoltPoint, cPvOverCnt, &s_uwPvVoltChkCnt))
		{
			s_uwPvVoltChkCnt = 0;
...
{
	static INT8U s_uwPvChkCnt = 0;
	
	if(Pv2Status.bit.bPvOverVolt)	// PV1过压
	{
		if(sbUnderLevelChk(PV2Volt.wNewCalValue, cPv2OverVoltPoint - 300, cPvOverRecCnt, &s_uwPvChkCnt))
		{
			s_uwPvChkCnt = 0;
	...
	{
		if(sbOverLevelChk(PV2Volt.wNewCalValue, cPv2OverVoltPoint, cPvOverCnt, &s_uwPvChkCnt))
		{
			s_uwPvChkCnt = 0;
...
{
	static INT8U s_uwPvCurrChkCnt = 0;
	
	if(Pv1Status.bit.bPvOverCurr)	// PV1过流
	{
		if(sbUnderLevelChk(swGetPv1CurrNew(), cPv1OverCurrRecPoint, cPvOverRecCnt, &s_uwPvCurrChkCnt))
		{
			s_uwPvCurrChkCnt = 0;
	...
	{
		if(sbOverLevelChk(swGetPv1CurrNew(), cPv1OverCurrPoint, cPvOverCnt, &s_uwPvCurrChkCnt))
		{
			s_uwPvCurrChkCnt = 0;
...
{
	static INT8U s_uwPvChkCnt = 0;
	static INT8U s_uwPvRecChkCnt = 0;

	if(Pv1Status.bit.bPvOverPower == false)
	{
		if((swGetPv1WattNew() > cPv1OverPowerPoint)||(swGetPv2WattNew() > cPv2OverPowerPoint))
		{
	...
	{
		if((swGetPv1WattNew() < cPv1OverPowerRecPoint)||(swGetPv2WattNew() < cPv2OverPowerRecPoint))
		{
...
{
	static INT8U s_uwPvHwChkCnt = 0;
	static INT8U s_uwPvHwRecChkCnt = 0;
...
{
	static INT8U s_uwPvLlcHwChkCnt = 0;
	static INT8U s_uwPvLlcHwRecChkCnt = 0;
...
{
	return dwPv1ChgLimit;
}

INT16U	swGetPv2ChgLimit(void)
{
	return dwPv2ChgLimit;
}
...
{
	OS_ENTER_CRITICAL();
	dwPv1ChgLimit = dwPv1ChgLimitTemp;
	OS_EXIT_CRITICAL();
}
...
{
	OS_ENTER_CRITICAL();
	dwPv2ChgLimit = dwPvChgLimitTemp;
	OS_EXIT_CRITICAL();
}
...
void sSetPv1ChgCurrTempLimit(INT16S wPvCurr)
{
	OS_ENTER_CRITICAL();
	wPv1ChgCurrTempLimit = wPvCurr;
	OS_EXIT_CRITICAL();
}

void sSetPv2ChgCurrTempLimit(INT16S wPvCurr)
{
	OS_ENTER_CRITICAL();
	wPv2ChgCurrTempLimit = wPvCurr;
	OS_EXIT_CRITICAL();
}
...
{
	return PV1Volt.wNewCalValue;
}

INT16S	swGetPv2VoltNew(void)
{
	return PV2Volt.wNewCalValue;
}


INT16S	swGetPv1CurrNew(void)
{
	return PV1Curr.wNewCalValue;
}

INT16S	swGetPv2CurrNew(void)
{
	return PV2Curr.wNewCalValue;
}
...
{
	return PV1Power.wNewCalValue;
}

INT16S	swGetPv2WattNew(void)
{
	return PV2Power.wNewCalValue;
}
...
{
	return PVOutCurr.wNewCalValue;
}

INT16S	swGetPvBusVoltNew(void)
{
	return PVBusVolt.wNewCalValue;
}
INT16S	swGetPvPBusVoltNew(void)
{
	return PVPBusVolt.wNewCalValue;
}
