...
//    EPwm5Regs.ETCLR.bit.INT = 1;            //Clears the ETFLG[INT] flag bit
	...
	EPwm5Regs.AQCSFRC.all = 0x5; // CSFA = 1, CSFB = 1;
	EPwm5Regs.AQCSFRC.bit.CSFA = 0x0; // CSFA = 0, CSFB = 1;
	EPwm5Regs.TBPRD = cBOOST_PWM_PERIOD;		// Protect	PWM ,20KHz
	EPwm5Regs.TBPHS.bit.TBPHS = 0;	   // Set Phase register to zero
	EPwm5Regs.TBCTR = 0x0000;			   // Clear counter
	EPwm5Regs.TBCTL.bit.CLKDIV = CHP_DIV1;  // TBCLK = SYSCLKOUT / (HSPCLKDIV*CLKDIV)
	EPwm5Regs.TBCTL.bit.HSPCLKDIV = CHP_DIV1;//=100M/(1*1)
...
//    EPwm7Regs.ETSEL.bit.SOCAEN = 1;     // Enable SOCA
    ...
    EPwm8Regs.TBPRD = cLLC_PWM_PERIOD_23KHz;           // Period = 1200 TBCLK counts
	...
	EPwm8Regs.CMPB.all = cLLC_PWM_PERIOD_23KHz;
    ...
    EPwm9Regs.TBPRD = cLLC_PWM_PERIOD_23KHz;           // Period = 1200 TBCLK counts
	...
	EPwm9Regs.CMPB.bit.CMPB = cLLC_PWM_PERIOD_23KHz;
...
void InitCMPSS(volatile struct CMPSS_REGS *CmpssReg)
{
    EALLOW;
    ...
    CmpssReg->DACHVALS.bit.DACVAL = 2855;//+PRI_OCP_CMPSS_PRO_POINT;//Set DAC to midpoint for arbitrary reference
    CmpssReg->DACLVALS.bit.DACVAL = 866;//test770A
...
//    EDIS;
    ...
    InitCMPSS(&Cmpss1Regs);
    InitCMPSS(&Cmpss3Regs);
...
void sModelIdentify(void)
{
    ...
    for(;;)
    {
        ...
        {
            b15KBatt48VModuleCnt++;
        }
        else if((2048 <= wADResult) && (wADResult < 2457))  //10048:1.6667V(1.5-1.8V)
        {
        ...
        if(b10KBatt48VModuleCnt > 100)
        {
            g_wVAType = cb10KVAModel;
            sSetBatteryPcs(cbBatt48VType);
            break;
        }
        else if(b15KBatt48VModuleCnt > 100)
        {
            g_wVAType = cb15KVAModel;
            sSetBatteryPcs(cbBatt48VType);
            break;
        }
		else if(b7K5Batt48VModuleCnt > 100)//其他默认走15K
		{
			g_wVAType = cb15KVAModel;
            sSetBatteryPcs(cbBatt48VType);
            break;
...
void sUpsParaInit(void)
{
    if(g_wVAType == cb10KVAModel)
    {
        ...
        wBatVoltRatio = 4010;               // 15048:
        wBusVoltRatio = 4010;               // 15048:
        ...
        g_wInvOverCurrentFactor = 6;        // 15048:6.684444 = 4096*InvLCurrRatio/3
        g_wInvLCurrLineLevel1 = 936;        // 15048:oóD?òa??2.40±????¨μ?á÷85A*6.68444->127.5
        g_wInvLCurrLineLevel2 = 936;        // 15048:oóD?òa??2.20±????¨μ?á÷80.0A*6.68444->120

        g_wInvOverCurrRated = 200; //127;         // 15048:2.40±????¨μ?á÷     180A
        g_wInvCurrLimitRated = 190;//125;         // 15048:×?′ó?T??μ?á÷15k/170

        g_wInvCurrRated = cIac36A;          // 10048:???¨μ?á÷

        //Inverter Cntl Limit Parameters Initial
        g_wInvVCntlFactor = 303;             //Vmax/RefAd*2^5 15048
        ...
        pEeDefaultTable = sGetDefaultTable();

        //Watt/VA Rated
        //wWattRated = 12000;                  // 15048:
        //wVARated   = 15000;                 // 15048:

        wWattRated = c15KWattMaxPower;                  // 15048:
        wVARated   = c15KVAMaxPower;                 // 15048:
     ...
     else if(g_wVAType == cb15KVAModel)
    {
        //ADC Ratio Initial
        wPVVoltRatio = 3008;//601/4096*10*2048
		wPV1CurrRatio = 6667; //66.67/2048*100*2048
        ...
        g_wInvOverCurrRated = 223; //127;         // 15048:2.40±????¨μ?á÷     200A
        g_wInvCurrLimitRated = 190;//125;         // 15048:×?′ó?T??μ?á÷15k/170
    ...
    //  g_wBMSTestDischgCurr = g_wDischgCurrSet;//cBatDischgCurrMax;
