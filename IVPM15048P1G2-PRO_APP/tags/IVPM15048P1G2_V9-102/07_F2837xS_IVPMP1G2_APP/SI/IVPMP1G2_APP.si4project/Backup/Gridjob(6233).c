...
void    sSYNLossEventHandling(void)
{
    ...
    {
        if(!(g_uwParaWarning & eZeroSyncFault))
        {
...
void sLinePowerPercentCal()
{
	...
	wLineVAPercent = ((INT32S)swGetRLineVolt()*swGetRLineCurr()/cVAMaxPower);
	wLineCurrPercent = ((INT32S)swGetRLineCurr()*100/cInvMaxCurr);
...
void sGenPowerPercentCal()
{
	...
	if(sbGetEepromGenInputMaxPower()>(INT16U)cVAMaxPower)
	{
		wGenVAPercent = ((INT32S)swGetGenVolt()*abs(swGetGenCurr())/cVAMaxPower);
	}
	else
	{
		wGenVAPercent = ((INT32S)swGetGenVolt()*abs(swGetGenCurr())/sbGetEepromGenInputMaxPower());
	}
	wwGenCurrPercent = ((INT32S)abs(swGetGenCurr())*100/cInvMaxCurr);
