...
extern INT16U cal_crc_half(INT8U *pin, INT8U len);
...
//		INT8U 	temp;
		...
		if(sbSciGetRxRdy(sciid)==cSciRxRdy)
		{
			...
			if(bStartBMSUpdateFlag && (sciid == cGoalSerialPort) && data != 0x00)
            {
...
INT8U	sSciWrite(INT8U sciid,INT8U *pstart,INT16U wLength)
{
	...
	if(wCRCChkFlg == true)
	{
		wCRCValue = cal_crc_half((INT8U *)pstart, wLength);
