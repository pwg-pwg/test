...
void sPvModuleInit()
{
	...
	{
		wPv1PowerLimitMaxCurr = cPv1RatedCurr;
		dwPv1CurrLimit = cPv1RatedCurr;
		wPv1ChgCurrTempLimit = cPv1RatedCurr;
		g_wPv1RatedCurr = cPv1RatedCurr;
		wPv1MaxPower = cPv1RatedPower15K;
		...
		wPv2MaxPower = cPv2RatedPower15K;
		gi_wLLCConvPWMDTSet = cLLCConvertDuty1_23KHz;
		g_wLLCConvPWMPeriod = cLLC_PWM_PERIOD_23KHz;
		uwLLCConvertDutyStart = cLLCConvertDuty3_23KHz;
		uwLLCConvertDuty2 = cLLCConvertDuty2_23KHz;
		uwLLCConvertDuty10 = cLLCConvertDuty10_23KHz;
	}
	else if(g_wVAType == cb10KVAModel)
	{
		wPv1PowerLimitMaxCurr = cPv1RatedCurr>>1;
		dwPv1CurrLimit = cPv1RatedCurr>>1;
		wPv1ChgCurrTempLimit = cPv1RatedCurr>>1;
		g_wPv1RatedCurr = cPv1RatedCurr>>1;
...
void sPv1Control(void)
{
	...
	{
		dwChgLimitMax = dwPv1CurrLimit + g_wPv1CurrComp;	// 目标电流
...
void sPv2Control(void)
{
	...
	{
		dwChgLimitMax = dwPv2CurrLimit + g_wPv2CurrComp;	// 目标电流
		...
		if((wPv1ToChgMode >= 1)&&(wPv2ToChgMode >= 1))
		{
			if(g_wVAType == cb15KVAModel)
			{
				if(wFBControlStatus == cFBInverterControl)
				{
					...
					dwLoadPowerReal = (INT32S)(fLoadPowerP*1.18f*10000.0f);
				}
				else
				{
					...
					dwLoadPowerReal = (INT32S)(g_wInvCtrlPowerRef*1.18f*10000.0f);
				}
			...
			else
			{
				if(wFBControlStatus == cFBInverterControl)
				{
					g_wPv1CurrCompTemp = fLoadPowerP*500.0f*1.07f/swGetPv1VoltNew();//2/3*1000
					g_wPv2CurrCompTemp = fLoadPowerP*500.0f*1.07f/swGetPv2VoltNew();
					dwLoadPowerReal = (INT32S)(fLoadPowerP*1.18f*10000.0f);
				}
				else
				{
					g_wPv1CurrCompTemp = (F32)g_wInvCtrlPowerRef*500.0f*1.07f/swGetPv1VoltNew();//2/3*1000
					g_wPv2CurrCompTemp = (F32)g_wInvCtrlPowerRef*500.0f*1.07f/swGetPv2VoltNew();
					dwLoadPowerReal = (INT32S)(g_wInvCtrlPowerRef*1.18f*10000.0f);
				}
				if(g_wPv1CurrCompTemp < 0)
				{
...
//				}
	...
	wPv1PowerLimitMaxCurr = ((INT32S)wPv1MaxPower*1000/PV1Volt.wValueFilter);
	wPv2PowerLimitMaxCurr = ((INT32S)wPv2MaxPower*1000/PV2Volt.wValueFilter);
	...
	{
		dwMaxBatChgPowerLimit = dwMaxBatChgPowerLimit + dwLoadPowerReal;
		dwPv1PowerTemp = (INT32S)PV1Volt.wValueFilter*PV1Curr.wValueFilter*10;
		dwPv2PowerTemp = (INT32S)PV2Volt.wValueFilter*PV2Curr.wValueFilter*10;
		...
		if(g_wVAType == cb15KVAModel)
		{
			if(dwPv1PowerTemp > ((dwMaxBatChgPowerLimit*2/3)+2500000))
			{
				if(dwBatPowerDiff > 0)
				{
					dwChg1Limit = ((dwPv1PowerTemp+dwBatPowerDiff/5) / PV1Volt.wValueFilter) / 10;//2/3功率
				}
				else
				{
					dwChg1Limit = ((dwMaxBatChgPowerLimit-dwPv2PowerTemp-10000) / PV1Volt.wValueFilter) / 10;//2/3功率
				}
				dwChg2Limit = ((dwMaxBatChgPowerLimit-dwPv1PowerTemp) / PV2Volt.wValueFilter) / 10;//1/3功率
			}
			else if(dwPv2PowerTemp > ((dwMaxBatChgPowerLimit*1/3)+2500000))
			{
				if(dwBatPowerDiff > 0)
				{
					dwChg2Limit = ((dwPv2PowerTemp+dwBatPowerDiff/5) / PV2Volt.wValueFilter) / 10;
				}
				else
				{
					dwChg2Limit = ((dwMaxBatChgPowerLimit-dwPv1PowerTemp-10000) / PV2Volt.wValueFilter) / 10;
				}
				dwChg1Limit = ((dwMaxBatChgPowerLimit-dwPv2PowerTemp) / PV1Volt.wValueFilter) / 10;
			}
			else
			{
				dwChg1Limit = (((dwMaxBatChgPowerLimit + dwBatPowerDiff)*2/3) / PV1Volt.wValueFilter) / 10;
				dwChg2Limit = (((dwMaxBatChgPowerLimit + dwBatPowerDiff)/3) / PV2Volt.wValueFilter) / 10;
			}
		...
		else
		{
			if(dwPv1PowerTemp > ((dwMaxBatChgPowerLimit>>1)+2500000))
			{
				if(dwBatPowerDiff > 0)
				{
					dwChg1Limit = ((dwPv1PowerTemp+dwBatPowerDiff/5) / PV1Volt.wValueFilter) / 10;
				}
				else
				{
					dwChg1Limit = ((dwMaxBatChgPowerLimit-dwPv2PowerTemp-10000) / PV1Volt.wValueFilter) / 10;
				}
				dwChg2Limit = ((dwMaxBatChgPowerLimit-dwPv1PowerTemp) / PV2Volt.wValueFilter) / 10;
			}
			...
			{
				dwChg1Limit = (((dwMaxBatChgPowerLimit + dwBatPowerDiff)>>1) / PV1Volt.wValueFilter) / 10;
				dwChg2Limit = (((dwMaxBatChgPowerLimit + dwBatPowerDiff)>>1) / PV2Volt.wValueFilter) / 10;
				
			}
			wPvChgCurrErr = dwPvBatChgPowerErrComp / PV1Volt.wValueFilter / 20;//为0
		}
	...
	{
		//PV1
		dwChg1Limit = dwMaxBatChgPowerLimit / PV1Volt.wValueFilter / 10;//10/2 一半功率
	...
	else if(swGetPv2DCDCCtrlSts() != cDCDCWait)
	{
		...
		//PV2
		dwChg2Limit = dwMaxBatChgPowerLimit / PV2Volt.wValueFilter / 10;//10/2 一半功率
