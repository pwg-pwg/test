...
Project:		INPM15048
File Name:		RlyDriver.c
Author:			X
Date:			2024.08.06
Description:	None
********************************************************************************/
#define __RLYDRIVER_C__
...
void	sSetRlyPointer(unsigned char relaytype,unsigned int pointer,unsigned char event,unsigned char prio)
{
	...
	{
		wRlyActionPointerTemp = (INT16U)((INT32U)pointer * 6000 / wPwmPeriod);//George 141224 5000->4500
		OS_ENTER_CRITICAL();
	...
	{
		wRlyActionPointerTemp = (INT16U)((INT32U)pointer * 6000 / wPwmPeriod);
		OS_ENTER_CRITICAL();	
...
void	sRlyActionDriver(unsigned int pointer)
{
	if(wRlyActionPointer!=RLY_ACTION_NULL)
	{
		if(((wRlyActionPointer >=pointer )&&(wRlyActionPointer<=(pointer+1 )))\
	...
	{
		if(((wRlyActionPointer2 >=pointer )&&(wRlyActionPointer2<=(pointer+1 )))\
	...
	{
		if(((wRlyActionPointer3 >=pointer )&&(wRlyActionPointer3<=(pointer+1 )))\
