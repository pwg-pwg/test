...
INT16U swModBusRdProc(INT16U *RxBuff, INT16U RxLen,INT8U sciid)
{
    ...
    if((RdAddr >= REALDATA_REG_BASE_ADDR) && (RdAddr < (REALDATA_REG_BASE_ADDR + REALDATA_REG_BASE_LEN)))
    {
        ...
        else
        {
			...
			if(ubCommunicationFaultCnt > 12)//DSP与MCU通信异常判断，6s后故障置位
			{
				ubCommunicationFaultCnt = 0;
...
//					sGenVoltageRangeInit();
        ...
        case eSetMaxSolarCurr:
            if((WrData >= (INT16U)cIdc10A)&&(WrData <= (INT16U)cIdc350A))
            {
...
//      break;
        ...
        case eComFaultClear:
            ...
            {
                //故障清除
                uATEData.Stru.eComFaultClear = WrData;
            }
            else
            {
                ret = cErr_ParUnValid;
        ...
        case eATEPVBusVoltAdj1:
            ...
            if((swGetEepromPVBusVoltAdj() == 2048) && (wTemp1 >= -150) && (wTemp1 <= 150) \
        ...
        case eATEPVOutCurrAdj1:
            wTemp1 = (INT16S)WrData - ((INT16S)PVOutCurr.wValueFilter*3);
            if((swGetEepromPVOutCurrAdj() == 2048) && (wTemp1 >= -250) && (wTemp1 <= 250) \
                && ((INT16S)WrData >= 200) && ((INT16S)WrData <= 3500))
            {
                wTemp1 = (INT16S)PVOutCurr.wValueFilter*3;
                wTemp2 = ((INT32S)WrData * 2048 + (wTemp1 >> 1)) / wTemp1;
                if((wTemp2 >= 1548) && (wTemp2 <= 2548))
                {
...
//	}
    ...
    uRealTimeData.Stru.eAnalogPV1Power = (swGetPv1WattNew()+50)/100;
	if(uRealTimeData.Stru.eAnalogPV1Power < 0)
	{
		uRealTimeData.Stru.eAnalogPV1Power = 0;
	}
    uRealTimeData.Stru.eAnalogPV2Power = (swGetPv2WattNew()+50)/100;
	if(uRealTimeData.Stru.eAnalogPV2Power < 0)
	{
...
//    }
	...
	{
		uRealTimeData.Stru.eAnalogBatWatt = ((INT32S)uRealTimeData.Stru.eAnalogBatCurr*swGetBatAvgVoltNew()+5000)/10000;
	}
	else
	{
		uRealTimeData.Stru.eAnalogBatWatt = ((INT32S)uRealTimeData.Stru.eAnalogBatCurr*swGetBatAvgVoltNew()-5000)/10000;
	...
	if(swGetEepromBatMode() == cLiBMode) //wifi上传SOC，若选择了锂电上传BMS的SOC
	{
		...
		if(uBMSData.Stru.wBMS1PackCurrent > 2)//0.2A
	    {
	        uRealTimeData.Stru.eAnalogBMSChargmode = 1;//Charging
	    }
	    else if(uBMSData.Stru.wBMS1PackCurrent < -2)//0.2A
	    {
    ...
    else
    {
		...
		if(uRealTimeData.Stru.eAnalogBatCurr > 2)
        {
            uRealTimeData.Stru.eAnalogBMSChargmode = uRealTimeData.Stru.eAnalogChgMode;//Charging
        }
        else if(uRealTimeData.Stru.eAnalogBatCurr < -2)
        {
    ...
    uRealTimeData.Stru.eAnalogGridCurr = swGetRLineCurrNew();
    uRealTimeData.Stru.eAnalogGridFreq = swGetLineFreq();
    //uRealTimeData.Stru.eAnalogLineFreq = (INT16U)(GridVoltSpllData.fo*100.0);
    uRealTimeData.Stru.eAnalogGridWatt = (sbGetLineWatt()+50)/100;
    uRealTimeData.Stru.eAnalogGridVA = (sbGetLineVA()+50)/100;
    ...
    uRealTimeData.Stru.eAnalogGenCurr = swGetGenCurrNew();
    uRealTimeData.Stru.eAnalogGenFreq = swGetGenFreq();
    uRealTimeData.Stru.eAnalogGenWatt = (sbGetGenWatt()+50)/100;
    uRealTimeData.Stru.eAnalogGenVA = (sbGetGenVA()+50)/100;
...
//    uRealTimeData.Stru.eAnalogLoadDCV = 0;
    ...
    uRealTimeData.Stru.eAnalogInvWatt = (sdwGetInvWatt()+50)/100;   // 市电电压*逆变电流
    uRealTimeData.Stru.eAnalogInvVA = (sdwGetInvVA()+50)/100;       // 逆变电压*输出电流
    uRealTimeData.Stru.eAnalogInvDCV = wInvVoltSampleBias;
    uRealTimeData.Stru.eAnalogInvDCI = wInvCurrSampleBias;

    uRealTimeData.Stru.eAnalogLoadWatt = (swGetLoadPowerWatt()+50)/100; // 电池模式:逆变电压*输出电流;市电模式:市电电压*输出电流
	uRealTimeData.Stru.eAnalogLoadVA = (swGetLoadPowerVA()+50)/100; // 电池模式:逆变电压*输出电流;市电模式:市电电压*输出电流
...
//	}
	...
	if(sGetGenPortWorkMode() == eSmartLoatOutput)	
	{
		...
		uRealTimeData.Stru.eAnalogSLoadWatt = (swGetSmartLoadWatt()+50)/100;
		if(uRealTimeData.Stru.eAnalogSLoadWatt < 0)
		{
			uRealTimeData.Stru.eAnalogSLoadWatt = 0;
		}
		uRealTimeData.Stru.eAnalogSLoadVA = (swGetSmartLoadVA()+50)/100;
...
//	uRealTimeData.Stru.eAnalogRsvd139 = wAcChgPowerLimit;
...
void swMobusBmsDataUpdate(void)
{
	...
	g_strBMSCtrlLogicInfo.wBMSBatWorkMode = uRealTimeData.Stru.eAnalogBMSChargmode;
