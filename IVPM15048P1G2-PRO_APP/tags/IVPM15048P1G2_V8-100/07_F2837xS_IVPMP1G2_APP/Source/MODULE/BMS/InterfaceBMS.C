...
//		else 
		...
		if(g_strBMSComSts[g_uwBMSCmdAddrID].fBMSFwVerErr) // 软件版本异常，直接查询下一ID
		{
			g_strBMSComSts[g_uwBMSCmdAddrID].fBMSComOK = false;
			g_strBMSComSts[g_uwBMSCmdAddrID].fBMSCmdIndex = cBMSCmd_BaseInfo;
			g_uwBMSCmdAddrID++;
		}
		else if(g_strBMSComSts[g_uwBMSCmdAddrID].fBMSCmdIndex < (cBMSCmd_Max - 1)) // 轮询该ID的所有数据
		{
	...
	if(uwBMSCmdAddrIDTemp != g_uwBMSCmdAddrID)
	{
		...
		{	
			...
			{
				g_uwBMSCmdAddrID = 0;
				s_uwComOKFlgPre = s_uwComOKFlg; //璁板綍閫氫俊鐘舵�佹爣蹇?
				s_uwComOKFlg = 0;               //閲嶇疆閫氫俊鐘舵�佹爣蹇?
				if(s_uwCycleNoOkChkEn)
				{
					s_uwCycleNoOkChkEn = false;
				}
				uwTemp = s_uwCycleNoOkChkID + 1; //涓嬩竴涓狪D
				if(uwTemp > cBMSAddrIDMax)
				{
					uwTemp = 0;
					if(g_uwBMSPollingNum++ > 3) { fBMSConnectFlg = 0; g_uwBMSPollingNum = 4; } //If the total number of polls is greater than 3, the BMS is considered disconnected锛坒BMSConnectFlg = 0锛?
				}
				for(uwBMSCmdAddrIDTemp = 0; uwBMSCmdAddrIDTemp <= cBMSAddrIDMax; uwBMSCmdAddrIDTemp++)
				{
					if(g_strBMSComSts[uwBMSCmdAddrIDTemp].fBMSComOK)
					{
						...
						if(uwTemp == uwBMSCmdAddrIDTemp)         //  鏇存柊涓存椂鍙橀噺锛屾洿鏂颁笅涓�涓狪D
						{
							uwTemp ++;
						}
					}
...
void sRDBMSRTInfoCmd(INT8U sciid, INT8U* pubTxBuff, INT8U ubAddr)
{
	...
	//uRealTimeData.Stru.wRLineWatt+=1;//*/
	// RXSTATUS=0;
