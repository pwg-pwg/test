...
#include "Constant.h"
#include "Eeprom.h"
#include "Source\MODULE\BMS\InterfaceBMS.h"
...
INT16U g_uwPylonBMSPollingNum = 0;
...
union PYLON_BMS_STATUS_TABLE
{
	struct PYLON_BMS_STATUS_BIT_FEILD
	{
		...
		// BIT4-7
		INT16U FullChgRequest:1;		// ????????
		INT16U ChargeImmediately1:1;	// ?????
...
//		else 
	...
	else
	{
		...
		if(!g_strPylonBMSComSts[g_uwPylonBMSCmdAddrID].fBMSComOK)//??ID??BMS????????(????��?????)
		{
			//?????ID????????
	...
	if(uwBMSCmdAddrIDTemp != g_uwPylonBMSCmdAddrID)
	{
		do
		{	//��BMS ID��������ѯ����(����ѯ�����һ��?)
		    //< TAG1: ���ͨ���ȶ���s_uwCycleNoOkChkID��¼��ʱ��һ��ʧ�ܵ�ID,��⵽ͨ��û�б仯��������cBMSCmdNoOKRollGap�Σ�
		    //<         ���˵�cBMSCmdNoOKRollGap�Σ�ʹ��s_uwCycleNoOkChkEn������TAG2
			if(g_uwPylonBMSCmdAddrID > cPylonBMSAddrIDMax)
			{
				g_uwPylonBMSCmdAddrID = 1;          //��1��ʼ������һ��
				...
				{
					uwTemp = 1;
					if(++g_uwPylonBMSPollingNum > 3) { fBMSConnectFlg = 0; g_uwPylonBMSPollingNum = 4; } //If the total number of polls is greater than 3, the BMS is considered disconnected（fBMSConnectFlg = 0�?
				}
				...
				for(uwBMSCmdAddrIDTemp = 1; uwBMSCmdAddrIDTemp <= cPylonBMSAddrIDMax; uwBMSCmdAddrIDTemp++)
				{
					...
					{
						s_uwComOKFlg |= (1<<uwBMSCmdAddrIDTemp);//��¼����ͨ�Ż���
				...
				s_uwCycleNoOKRollDelayCnt++;//δͨ������ʱ���� 3
...
//						}
			...
			//< TAG2 : s_uwCycleNoOkChkEn��ʹ�ܣ�g_uwPylonBMSCmdAddrID����s_uwCycleNoOkChkIDʱ������fBMSComReady=true;�˳��жϣ���һ�ξͲ�ѯ��ID
			if(!g_strPylonBMSComSts[g_uwPylonBMSCmdAddrID].fBMSComReady)//׼��δ���?
			{
				if(s_uwCycleNoOkChkEn)
				{
				...
				if(g_strBMSCtrlLogicInfo.wBMSFaultCode == 0)
				{
					...
					if(ucTemp1 != 0 || ucTemp2 != 0)
					{
						if(ucTemp2 !=0 )
						{
							if(ucTemp1 & 0x08)
							{
								g_strBMSCtrlLogicInfo.wBMSFaultCode = 14;//?????????
							}
...
//????????????????600ms
void	sPylontechPolling(INT8U sciid,INT8U* pubTxBuff)
{
	INT8U  *TxBuff;
	INT8U  TxLen;
	INT8U  bComLength;
	static INT8U  wComIdCnt = 0;
//	static INT8U  wAddrIdCnt = 1;	//?????1??????
	INT8U  wAddrIdCnt = 1;	//?????1??????
//	static INT16U wTimeCnt = 0;
	...
	sNumToAscii_16Hex(sPylonClhksum(bComLength),&TxBuff[cPYLON_LENGTHSUM_BIT]);
	sNumToAscii_16Hex(((bComLength >> 8) & 0x0F),&TxBuff[cPYLON_LENGTHID3_BIT]);
	sNumToAscii_16Hex(((bComLength & 0x00FF) >> 4),&TxBuff[cPYLON_LENGTHID2_BIT]);
	sNumToAscii_16Hex((bComLength & 0x000F),&TxBuff[cPYLON_LENGTHID1_BIT]);
	sNumToAscii_16Hex(wAddrIdCnt,&TxBuff[cPYLON_INFOH_BIT]);
	TxBuff[cPYLON_INFOL_BIT] = cPYLON_ADRL_DATA;	
	TxLen = cPYLON_INFOL_BIT + 1;
	//CHKSUM
