...
void sAvrPvTempDeratingChk(INT8U bFilter)
{
	...
	{
		wPvChgCurrDerateRange = wPvTempChgCurrDerateRange2;
	}
	else
	{
...
//		}
	...
	if((swGetPv1DCDCCtrlSts() != cDCDCWait)&&(swGetPv2DCDCCtrlSts() != cDCDCWait))//PV1 和 PV2 都在工作
	{
		...
		UPDNLMT(wPv1CurrTempLimit,cDCDCCntlCurrntLimit,0);
		UPDNLMT(wPv2CurrTempLimit,cDCDCCntlCurrntLimit>>1,0);
	...
	{
		wPv1CurrTempLimit = cDCDCCntlCurrntLimit;
		wPv2CurrTempLimit = cDCDCCntlCurrntLimit>>1;
		sSetPv1ChgCurrTempLimit(wPv1CurrTempLimit);
		sSetPv2ChgCurrTempLimit(wPv2CurrTempLimit);
	}
}
