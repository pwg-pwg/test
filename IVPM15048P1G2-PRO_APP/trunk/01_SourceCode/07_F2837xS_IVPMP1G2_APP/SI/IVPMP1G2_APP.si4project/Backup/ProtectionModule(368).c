...
INT16U	wPvOverTempPoint = cPvHsTempOv;
INT16U	wPvOverTempBackPoint = cPvHsTempOvBack;
...
void    sTemperaturePvChk(INT8U bFilter);
void    sTemperatureInternalChk(INT8U bFilter);
...
{
    wTxtOverTempPoint = cTxTempOv;
    wTxtOverTempBackPoint = cTxTempOvBack;
    wInvOverTempPoint = cInvHsTempOv;
    wInvOverTempBackPoint = cInvHsTempOvBack;
    wPvOverTempPoint = cPvHsTempOv;
    wPvOverTempBackPoint = cPvHsTempOvBack;
    ...
    if(swGetEepromFanWorkMode() == cFanSilentMode)//静音模式下逆变变压器、PV散热器、逆变散热器保护点、恢复点均比性能模式下低
    {
    	...
    	{
	        //变压器
	        wTxtOverTempPoint = cTxTempOv - 10;
	        wTxtOverTempBackPoint = cTxTempOvBack - 10;
	        //逆变散热器
	        wInvOverTempPoint = cInvHsTempOv - 6;
	        wInvOverTempBackPoint = cInvHsTempOvBack - 6;
	        //PV散热器
	        wPvOverTempPoint = cPvHsTempOv - 6;
	        wPvOverTempBackPoint = cPvHsTempOvBack - 6;
	        //内环境温度
...
//	        wTxtOverTempBackPoint = cTxTempOvBack - 10;
	        //逆变散热器
	        wInvOverTempPoint = cInvHsTempOv - 10;
	        wInvOverTempBackPoint = cInvHsTempOvBack - 10;
	        //PV散热器
	        wPvOverTempPoint = cPvHsTempOv - 10;
	        wPvOverTempBackPoint = cPvHsTempOvBack - 10;
	        //内环境温度
...
void sTemperaturePvChk(INT8U bFilter)
{
	INT16U wOverTempWarningPoint;
	INT16U wOverTempWarningBackPoint;		
	static INT8U bLlcOverTempCnt=0; 
	static INT8U bLlcOverTempWarningCnt=0;
...
void sTemperatureInternalChk(INT8U bFilter)
{
    static INT8U bInternalOverTempCnt=0;
...
{
	static INT8U bChkTxt1 = 0;
	static INT8U bChkTxt2 = 0;
	static INT8U bChkInv11 = 0;
	static INT8U bChkInv12 = 0;
	static INT8U bChkInv21 = 0;
	static INT8U bChkInv22 = 0;
	static INT8U bChkLlcP = 0;
	static INT8U bChkLlcS = 0;
...
//	}
	...
	if(swGetEepromFanWorkMode() == cFanPerformanceMode)
	{
		if(g_wVAType == cb15KVAModel)
		{
			...
			else if(wTempDegreeTxTemp <= 1190)
			{
	...
	if(swGetEepromFanWorkMode() == cFanSilentMode)
	{
		if(g_wVAType == cb15KVAModel)
		{
			...
			else if(wTempDegreeTxTemp <= 1090)
			{
		...
		else
		{
			...
			//Inv温度降额
	        if(wTempDegreInvTemp < (wInvTempDerate15KVA - 100))
	        {
	            wTempDerateRangeInv = 0;
	        }
	        else if(wTempDegreInvTemp <= 650)
	        {
	            wTempDerateRangeInv = (wTempDegreInvTemp - (wInvTempDerate15KVA - 100))>>1;
	        }
	        else if(wTempDegreInvTemp > 650)
	        {
	            wTempDerateRangeInv = 40;
...
//Pv电流降额
void sAvrPvTempDeratingChk(INT8U bFilter)
{
	static INT8U bPvTempOvDerate = 0;
	static INT8U bPvTempOvDerate1 = 0;
	static INT8U bPvTempOvDerate2 = 0;
	static INT8U bPvTempOvDerate3 = 0;
	...
	static INT8U bInternalTempPvDerateFlag = 0;//性能模式内环境降额标志位
	static INT8U bPvTempPvDerateFlag1 = 0;//静音模式Pv散热器降额标志位
	static INT8U bInternalTempPvDerateFlag1 = 0;//静音模式内环境降额标志位
	INT16S wTempDegreeInternal = uBMSData.Stru.wEnvironmentTemp;//内环境温度
	INT32S dwPvBatPowerTempLimit = 0;
	INT32S dwPv1PowerTempLimit = 0;
	INT32S dwPv2PowerTempLimit = 0;
	INT16S wPv1CurrTempLimit = 0;
	INT16S wPv2CurrTempLimit = 0;
	INT16S wPvChgCurrDerateRange = 0;//最终降额值
	static INT16U uwPVCurrDeratecnt = 0;
	static INT8U  ubPVCurrDerateFlag = 0;//1min
	static INT8U  ubPVCurrDerateForwardCntFlag = 0;//正向30min
	...
	if(swGetEepromFanWorkMode() == cFanPerformanceMode)
	{
		...
		else
		{
			...
			if(fProtection.PvOverDerate)
			{
				wPvTempChgCurrDerateRange = 240 - (wTempDegreePv - 57) * 6;//一度降6A
			}
			else
			{
	...
	if(swGetEepromFanWorkMode() == cFanSilentMode)
	{
		...
		else
		{
			...
			if(1 == bPvTempPvDerateFlag1)
			{
				if(sbUnderLevelChk(wTempDegreePv, wPvTempDerateBack15KVA - 10, bFilter, &bPvTempOvDerate2) == true)
				{
					bPvTempPvDerateFlag1 = 0;
				}
			}
			...
			{
				if(sbOverLevelChk(wTempDegreePv, wPvTempDerate15KVA - 10, bFilter, &bPvTempOvDerate2) == true)
				{
					bPvTempPvDerateFlag1 = 1;
				}
			}
		...
		else
		{
			...
			if(bInternalTempPvDerateFlag1)
			{
				wPvTempChgCurrDerateRange1 = 240 - (wTempDegreeInternal - 610) * 2;
			}
			else
			{
				wPvTempChgCurrDerateRange1 = 240;
	...
	wPvChgCurrDerateRange = wPvTempChgCurrDerateRange;
...
//	}
	...
	if(g_wVAType == cb15KVAModel)
	{
		if(wPvChgCurrDerateRange > 350)
		{
			wPvChgCurrDerateRange = 350;
		}
		else if(wPvChgCurrDerateRange < 250)
		{
			wPvChgCurrDerateRange = 250;
		}
	...
	{
		if(wPvChgCurrDerateRange > 240)
		{
			wPvChgCurrDerateRange = 240;
		}
		else if(wPvChgCurrDerateRange < 180)
		{
			wPvChgCurrDerateRange = 180;
		}
...
//	if(wPvTempChgCurrDerateRange < 250)//限幅
//	{
//		wPvTempChgCurrDerateRange = 250;
//	}
//	wPvTempChgCurrDerateRange = 350;
//	
//		if(wTempDegreePv < wPvTempDerate15KVA)//100度
//		{
//			wPvTempChgCurrDerateRange = 0;
//		}
//		else if(wTempDegreePv <= 130)
//		{
//			wPvTempChgCurrDerateRange = cPvPowerDefault - (wTempDegreePv - wPvTempDerate15KVA) * cPvDerateCoe;
//		}
	

	dwPvBatPowerTempLimit = (INT32S)wPvChgCurrDerateRange * swGetBatAvgVoltNew()*139>>7;//10倍 看是否需要考虑效率
	if((swGetPv1DCDCCtrlSts() != cDCDCWait)&&(swGetPv2DCDCCtrlSts() != cDCDCWait))//PV1 和 PV2 都在工作
	{
		if(g_wVAType == cb15KVAModel)
		{
			dwPv1PowerTempLimit = dwPvBatPowerTempLimit*200/3;
			dwPv2PowerTempLimit = dwPvBatPowerTempLimit*100/3;
		}
		else if(g_wVAType == cb10KVAModel)
		{
			dwPv1PowerTempLimit = dwPvBatPowerTempLimit*100/2;
			dwPv2PowerTempLimit = dwPvBatPowerTempLimit*100/2;
		}
		wPv1CurrTempLimit = dwPv1PowerTempLimit/swGetPv1VoltNew();  //1000/10
		wPv2CurrTempLimit = dwPv2PowerTempLimit/swGetPv2VoltNew();
		UPDNLMT(wPv1CurrTempLimit,g_wPv1RatedCurr,0);
		UPDNLMT(wPv2CurrTempLimit,g_wPv2RatedCurr,0);
		sSetPv1ChgCurrTempLimit(wPv1CurrTempLimit);
		sSetPv2ChgCurrTempLimit(wPv2CurrTempLimit);
	}
	else if(swGetPv1DCDCCtrlSts() != cDCDCWait)
	{
		dwPv1PowerTempLimit = dwPvBatPowerTempLimit*100;
		dwPv2PowerTempLimit = 0;
		wPv1CurrTempLimit = dwPv1PowerTempLimit/swGetPv1VoltNew();  //1000/10
		wPv2CurrTempLimit = dwPv2PowerTempLimit/swGetPv2VoltNew();
		UPDNLMT(wPv1CurrTempLimit,g_wPv1RatedCurr,0);
		UPDNLMT(wPv2CurrTempLimit,g_wPv2RatedCurr,0);
	...
	{
		dwPv1PowerTempLimit = 0;
		dwPv2PowerTempLimit = dwPvBatPowerTempLimit*100;
		wPv1CurrTempLimit = dwPv1PowerTempLimit/swGetPv1VoltNew();  //1000/10
		wPv2CurrTempLimit = dwPv2PowerTempLimit/swGetPv2VoltNew();
		UPDNLMT(wPv1CurrTempLimit,g_wPv1RatedCurr,0);
		UPDNLMT(wPv2CurrTempLimit,g_wPv2RatedCurr,0);
	...
	{
		wPv1CurrTempLimit = g_wPv1RatedCurr;
		wPv2CurrTempLimit = g_wPv2RatedCurr;
		sSetPv1ChgCurrTempLimit(wPv1CurrTempLimit);
		sSetPv2ChgCurrTempLimit(wPv2CurrTempLimit);
	}
}
...
//逆变器电流降额
INT16S sAvrInvTempDeratingChk(INT8U bFilter)
{
...
//		}
	...
	//	}
		if(g_wVAType == cb15KVAModel)
		{
	        ...
	        //变压器温度降额计算
	        if(wTempDegreeTx > 104)
	        {
	            wTempDerateRangeInv2 = 65 - (wTempDegreeTx - 104)*3;
	        }
	        else
	        {
		...
		else
		{
	        ...
	        if(wTempDegreeInv > cInvCurHSTempDerate15KVA)
	        {
	            wTempDerateRangeInv = 44 - (wTempDegreeInv - cInvCurHSTempDerate15KVA) * 4;
	        }
	        else
	        {
...
//
		if(g_wVAType == cb15KVAModel)
		{
	        ...
	        if(wTempDegreeTx > 94)
	        {
	            wTempDerateRangeInv2 = 65 - (wTempDegreeTx - 94)*3;
	        }
	        else
	        {
	            wTempDerateRangeInv2 = 65;
		...
		{
			//散热器降额计算
	        if(wTempDegreeInv > (cInvCurHSTempDerate15KVA - 10))
	        {
	            wTempDerateRangeInv = 44 - (wTempDegreeInv - (cInvCurHSTempDerate15KVA - 10)) * 4;
	        }
	        else
	        {
	            wTempDerateRangeInv = 44;
	        ...
	        {
	            //wTempDerateRangeInv1 = 65 - ((wTempDegreeInternal - 520) * 26)>>6;
	            wTempDerateRangeInv1 = 44 - (wTempDegreeInternal - 610)*40/100;
	        }
	        else
	        {
	            wTempDerateRangeInv1 = 44;
