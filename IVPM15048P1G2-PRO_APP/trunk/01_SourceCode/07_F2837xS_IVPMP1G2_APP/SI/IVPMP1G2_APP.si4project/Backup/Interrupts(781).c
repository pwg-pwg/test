	...
	if(!HiLLCOverCurr)//再检测一遍,防止误触
	{
		...
		if(uwLlcOCPOffPwmFlag != 1)
		{
			if((EPwm8Regs.TBCTR>30)&&(EPwm8Regs.TBSTS.bit.CTRDIR==0))
			{
				...
				mLLCHLDPWMCOMP = cLLC_PWM_PERIOD;
				mLLCLLUPWMCOMP = cLLC_PWM_PERIOD;
			}
			else if((EPwm8Regs.TBCTR<(cLLC_PWM_PERIOD-30))&&(EPwm8Regs.TBSTS.bit.CTRDIR==1))
			{
				mLLCHLUPWMCOMP = 0;
				mLLCLLDPWMCOMP = 0;
...
interrupt void INVInterrupt(void)
{
    ...
    if(wPhaseLockPoint == 0)
    {
        dwCapTemp = ECap2Regs.TSCTR;         //INV.V
        dwSineZeroTimeNewTemp = dwCapTemp;
        wSinePeriodNewTemp=(INT16U)((dwSineZeroTimeNewTemp-dwSineZeroTimeOld)/200);
        ...
        wInvertTd = (INT16U)((dwSineZeroTimeNewTemp-dwLineZeroTimeNewTemp)/200);
        wInvertGenTd = (INT16U)((dwSineZeroTimeNewTemp-dwGenZeroTimeNewTemp)/200);
        wSynchroPeriodCntNew=wSynchroPeriodNewTemp;
        wInvertSynTd = (INT16U)((dwSineZeroTimeNewTemp-dwSynchroZeroTimeNewTemp)/200);
...
//	}
    ...
    if((g_uwWorkMode == cBatteryMode) || ((g_uwWorkMode == cStandbyMode) && (wFBControlStatus == cFBInverterControl)))
    {
        ...
        if((abs(wInvPriCurr1Sample1) > PRI_OCP_PRO_POINT)||(abs(wInvPriCurr2Sample1) > PRI_OCP_PRO_POINT))//200/1153.85*2048=355
        {
        ...
        else
        {
            if(g_uwInvOverPtiCurCnt1 >= 2)
            {
                if((abs(wInvPriCurr1Sample1) < PRI_OCP_REC_POINT)&&(abs(wInvPriCurr2Sample1) < PRI_OCP_REC_POINT))
                {
                    PriOCPFlag1 = 0;
    ...
    else if(g_uwWorkMode == cLineMode )
    {
	   ...
	   if(abs(wInvLCurrSample1) > g_wInvLCurrLineLevel1)  //80*25=1920 //George 150112 2141->g_wInvLCurrLineLevel1
	   {
        ...
        if((abs(wInvPriCurr1Sample1) > PRI_OCP_PRO_POINT)||(abs(wInvPriCurr2Sample1) > PRI_OCP_PRO_POINT))
        {
            g_uwInvOverPtiCurCnt1++;
...
//    wPV2CurrSample2 = ADCDRESULT3;
   ...
   if(wSample8Select1Cnt==SAM_8SEL1_A1)
    ...
    if((g_uwWorkMode == cBatteryMode )||((g_uwWorkMode == cStandbyMode )&&(wFBControlStatus == cFBInverterControl)))
    {
        ...
        if((abs(wInvPriCurr1Sample2) > PRI_OCP_PRO_POINT)||(abs(wInvPriCurr2Sample2) > PRI_OCP_PRO_POINT))//450/1153.85*2048=355
        {
        ...
        else
        {
            if(g_uwInvOverPtiCurCnt2 >= 2)
            {
                if((abs(wInvPriCurr1Sample2) < PRI_OCP_REC_POINT)&&(abs(wInvPriCurr2Sample2) < PRI_OCP_REC_POINT))//300A
                {
                    //mEnInvPwmOut();
                    PriOCPFlag2 = 0;
    ...
    else if(g_uwWorkMode == cLineMode )
    {
        ...
        if((abs(wInvLCurrSample2) > g_wInvLCurrLineLevel2)//80A
            ||(wBusVoltReal > cBus650v)
            ||((wBusVoltReal > cBus620v)&&(wFBControlStatus == cFBKeepBatSts))
...
//            || (wRSinPointer == wInvVoltSampleCnt3Div4) || (wRSinPointer == (wInvVoltSampleCnt3Div4+1))))
   ...
   else
   {
	...
	{
		//if(wPV1CurrReal > cDCDCCntlCurrntLimit+400)	// PV1快速过流保护
		if(wPV1CurrReal > 7000)	// PV1快速过流保护1.75倍
		{
		...
		//if(wPV2CurrReal > (cDCDCCntlCurrntLimit>>1)+200)	// PV2快速过流保护
...
void sLineWaveChk(INT16U wCmpPoint)
{
	...
	{
		if(((wLineVoltReal < 500 && wLineVoltReal > -500) || wLineVoltReal > 3950 || wLineVoltReal < -3950)
			&& wLineCurrReal < 750 && wLineCurrReal > -750)
		{
...
void HardProtectChk(void)
{
    ...
    if((sWLiHardProtectCnt > 1)||((liparaTx==0)&&(mEepromOutputMode() > cOP_Single)))//&&LiHardProtect==0
    {
        //ESTOP0;
//      LiHardProtectCNT++;
