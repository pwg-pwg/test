...
//	if (g_OP_TWINS_OutDuration > 24*60)//1day
//	{
...
//	}
	...
	{
		//优先级2
		if((( (CurrentTime.ubHour*60+ CurrentTime.ubMin)-(g_HistoricalTiming.ubHour*60+g_HistoricalTiming.ubMin) )>= g_OP_TWINS_OutDuration)
		...
		else if(((( (CurrentTime.ubHour*60+ CurrentTime.ubMin)-(g_HistoricalTiming.ubHour*60+g_HistoricalTiming.ubMin) )< g_OP_TWINS_OutDuration)||g_OP_TWINS_OutDuration == 0xFFFF)
			&&g_uwBatVoltAvg > swGetEEThresholdVoltMin_OP2())
		{
			//TESEOPCODE |= 0X04;
			return true;
...
{	
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			if(mChkOPRlyOn() && mChkGridRlyOn()&& (gi_wParallelEnable==0))
			{
...
void sShutdownChk(void)
{
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			{
				if((!g_uwLoadOnSts || subGetParaBatUnderSts() || (g_uwWorkMode == cFaultMode && g_wOverTempFaultRestartFlg && g_uwFaultCode != cFanLock)) 
