...
INT16S g_wSolarParaEn = false;//没有使用


extern INT32S Test_dwSolarPower1Limit;
extern INT16S Test_wSolarLimitCurrMax;
extern INT16S Test_wSolarLimitCurrMin;
...
void sSolarPowerAbnormalChk(void)
{
	...
	{
	    if((mPVCurrReal(gi_uwSolarCurr1Sample) > g_wPv1CurrLimit+1000)&&(g_wR_Boost1_PWM == 1))
	    {
		    if(++s_uwSolar1PowerAbnormalChkCnt > 50)
		    {
	...
	{
	    if((mPVCurrReal(gi_uwSolarCurr2Sample) > g_wPv2CurrLimit+1000)&&(g_wR_Boost2_PWM == 1))
	    {
		    if(++s_uwSolar2PowerAbnormalChkCnt > 50)
		    {
...
void sSolarLimitCurrUpdate(INT16U uwBoost1Sts, INT16U uwBoost2Sts)
{
	...
	if(--wDelayCnt == 0)
	{
       ...
       // 第二： 分类：每路最大的功率
        ...
        Test_dwSolarPower1Limit = dwSolarPower1Limit;
		
//第三：PV1  最大功率/当前电压得到最大电流
	...
	Test_wSolarLimitCurrMax =  wSolarLimitCurrMax;
    Test_wSolarLimitCurrMin =  wSolarLimitCurrMin;
	
//第四：最终的结果使用：dwSolarPower1Limit 5000*10
    // PV1融入功率去限制和软起电流
