...
const pFunc1	GetDataSubArray[100]=
{
	...
	//OnGrid
	swGetInvBusVoltRef,    swGetFBVm_P,                 swGetBusVoltErr,     //66-68并网母线电压给定,外环的输出,基于并网母线外环输出得到的电池控母线电压误差
	swGetRFBVm_P,          swGetFBImo_P,                swGetFBR_Is_P,       //69-71 并网母线外环的最终输出，带R,内环电流给定,内环采样
    swGetRFBR_Is_P,        swGetRFBPWM,                 swGetFBPWM,          //72-74内环误差,内环输出经过调制之后的PWM，带R
	swReturnNull,          swReturnNull,                swReturnNull,        //75-77
	//OffGrid
	...
	//DCDC	
	swGetBusVmo,           swGetPDCDCImo,               swGetPDCDCPWM,       //90-92DCDC外环输出  //DCDC内环给定  //DCDC内环的输出														
	(pFunc1)swGetSinePeriodCntNew,(pFunc1)swGetLinePeriodCntNew,swGetInvStep,//93-95																						  
	swGetBusVoltErr,       swGetBatVoltErr,                swReturnNull,        //96-98
	swReturnNull															 //99
};
