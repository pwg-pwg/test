...
INT16S g_wSolarParaEn = false;//√ª”– π”√
...
//	g_wKCurrent1Forward = 48;  //100(–°‘ÿ≤ªŒ»∂®)
//	g_wPv1CurrLimit = 187 * 8;
//
//	g_wKVoltage2Forward = 128;//16
//	g_wKCurrent2Forward = 48;  //100(–°‘ÿ≤ªŒ»∂®)
//	g_wPv2CurrLimit = 187 * 8;
	...
	g_wKVoltage1Forward = cCntl_PVMPPTKp;
	g_wKCurrent1Forward = 5;  //100(–°ÔøΩÿ≤ÔøΩÔøΩ»∂ÔøΩ)

	g_wPv1CurrLimit = cSolarCurrMax;

	g_wKVoltage2Forward = cCntl_PVMPPTKp;
	g_wKCurrent2Forward = 5;  //100(–°ÔøΩÿ≤ÔøΩÔøΩ»∂ÔøΩ)
...
void sSolarLimitCurrUpdate(INT16U uwBoost1Sts, INT16U uwBoost2Sts)
{
	...
	if(--wDelayCnt == 0)
	{
		...
		g_dwTotalPower = (INT32S)g_wSolarPowerMax * g_wSolarDeratePer / 100;
		...
		if(wSolarLimitCurrMin > cSolarCurrMax)
		{
