...
//		mDisForceRPPWMLow();
		...
		if(g_wR_PWMDirectionPre != 0)
		{
			...
			mDisINVPWMOut(cSwitchDisPWM);
		}
		else
		{
			mEnINVPWMOut(cSwitchDisPWM);
		}
//		else
		{
...
INT16S	swGetFBVm_P(void)//并网母线外环的输出
{
	return g_wFBVm_P;
}
...
INT16S	swGetBusVoltErr(void)//并网时候母线电压的误差
{
	return g_wBusVoltErr;
}


INT16S	swGetRFBR_Is_P(void)
{
