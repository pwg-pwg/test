...
const pFunc1	GetDataSubArray[100]=
{
	...
	swGetBoost1_PWM,		   swReturnNull,			swReturnNull,        //57-59
	(pFunc1)suwGetBoost2CtrlSts,swGetVerr_P2,           swGetVm_P2,          //60-62
	swGetPV2CurrLimit,     swGetImo_P2,				    swGetR_Is_P2,        //63-65	
	swGetBoost2_PWM,		   swReturnNull,			swReturnNull,        //66-68
	//OnGrid
	swGetInvBusVoltRef,    swGetFBVm_P,                 swGetBusVoltErr,     //69-71并网母线电压给定,外环的输出,基于并网母线外环输出得到的电池控母线电压误差
	swGetRFBVm_P,          swGetFBImo_P,                swGetFBR_Is_P,       //72-74 并网母线外环的最终输出，带R,内环电流给定,内环采样
    swGetRFBR_Is_P,        swGetRFBPWM,                 swGetFBPWM,          //75-77内环误差,内环输出经过调制之后的PWM，带R
	swReturnNull,          swReturnNull,                swReturnNull,        //78-80
	//OffGrid
	swGetRSinPointer,      swGetRLineVoltTest,          swGetRInvDCVoltCtrl,//81-83原始的PLL角度,离网的相位,电压直流分量输出
	swGetVref,             swGetRInvVoltCntl,           swGetInvVoltError,  //84-86离网逆变电压环给定//离网逆变电压环采样  真实值*10*32//离网逆变电压环误差
    swGetRLoadCurrCntl,	   swGetVBeforeSaturation,      swGetRInvCurrCntl,  //87-89离网重复控制外环输出会叠加这个//离网重复控制外环输出//离网内环采样
	swGetVoltLimit,        swGetKpwm,                   swGetLoopOutput,    //90-92离网内环输出//离网内环最终输出 							
	//DCDC	
	swGetBusVmo,           swGetPDCDCImo,               swGetPDCDCPWM,       //93-95DCDC外环输出  //DCDC内环给定  //DCDC内环的输出														
...
swGetInvBusVoltRef, 	swGetPDCDCPWM,				swGetBusVoltErr,						//72-74   72 母线电压给定 	   74并网母线电压误差 
...
swGetFBR_Is_P,			swGetFBImo_P,				swGetFBVm_P,							//87-89  87 并网内环采样	88 并网内环给定 89 并网母线外环输出
swGetRInvDCVoltCtrl,	swReturnNull,				swReturnNull,							//90-92
swReturnNull,			swReturnNull,				swReturnNull,							//93-95
swGetRFBR_Is_P, 		swGetRFBVm_P,				swGetRFBPWM,							//96-98  96并网内环误差 97并网母线外环最终输出	98 内环最终输出
swReturnNull																				//99
*/
