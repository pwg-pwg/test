...
//TEST  PV Overload
//extern INT16S g_wSolarDeratePer;上面有声明了
extern INT32S g_dwTotalPower;
extern INT32S g_dwSolarPowerLimitSig;
INT32S Test_dwSolarPower1Limit;
extern INT16U g_uwSolarVolt1Avg;
INT16S Test_wSolarLimitCurrMax;
INT16S Test_wSolarLimitCurrMin;
...
void sQ1Command(INT8U sciid)
{
	...
	sNumToASCII(g_wSolarDeratePer, 3, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//100  降额百分比
	wSciLength += 3;	
	g_ubUserDataBuf0[sciid][wSciLength++] = 'B';
	sNumToASCII(g_dwTotalPower, 5, 0, &g_ubUserDataBuf0[sciid][wSciLength]);// 10000W
	wSciLength += 5;
	g_ubUserDataBuf0[sciid][wSciLength++] = 'C';
	sNumToASCII(g_dwSolarPowerLimitSig, 5, 0, &g_ubUserDataBuf0[sciid][wSciLength]);// 5000*10
	wSciLength += 5;
	g_ubUserDataBuf0[sciid][wSciLength++] = 'D';
	sNumToASCII(Test_dwSolarPower1Limit, 5, 0, &g_ubUserDataBuf0[sciid][wSciLength]);// 5000*10
	wSciLength += 5;
	g_ubUserDataBuf0[sciid][wSciLength++] = 'E';
	sNumToASCII(g_uwSolarVolt1Avg, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//3600  360V
	wSciLength += 4;
	g_ubUserDataBuf0[sciid][wSciLength++] = 'F';
	sNumToASCII(Test_wSolarLimitCurrMax, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//2000  20A
	wSciLength += 4;
	g_ubUserDataBuf0[sciid][wSciLength++] = 'G';
	sNumToASCII(Test_wSolarLimitCurrMin, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//2000  20A
	wSciLength += 4;
	g_ubUserDataBuf0[sciid][wSciLength++] = 'H';
	sNumToASCII(g_uwSolarPower1Avg, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//6000W
	wSciLength += 4;
	g_ubUserDataBuf0[sciid][wSciLength++] = 'I';
	sNumToASCII(g_wPv1CurrLimit, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);//2000  20A
	wSciLength += 4;
