...
#define cLLC_TXStarTempPoint			636		//63.0C
#define cLLC_TXEndTempPoint				1200	//126.0C
#define cLLC_TXTempRange				500		//63.0C
...
#define cLLC_MosTjProtectPoint			1050	//120C
...
//Temprature Derat
#define cLLC_MosTjDeratPoint			980	//112.5C
...
INT16S g_wSolarBSTOTPoint = 760;//850;
INT16S g_wSolarBSTOTBackPoint = 610;//600;
INT16S g_wInvOTPoint = 880;
INT16S g_wInvOTBackPoint = 730;
INT16S g_wCovertLOTPoint = 760;//850;
INT16S g_wCovertLOTBackPoint = 610;//600;
INT16S g_wCovertHOTPoint = 880;
INT16S g_wCovertHOTBackPoint = 730;
...
void sOverTempParaInit(void)
{
	g_wSolarBSTOTPoint = 760;//850;
	g_wSolarBSTOTBackPoint = 610;//700;
	g_wCovertLOTPoint = 760;//850;
	g_wCovertLOTBackPoint = 610;//700;
	g_wCovertHOTPoint = 880;//950;
	g_wCovertHOTBackPoint = 730;//800;
	g_wInnelOTPoint = 950;
	g_wInnelOTBackPoint = 650;
	g_wLLC_TXOTPoint = 1440;
	g_wLLC_TXOTBackPoint = 1290;
	g_wInvOTPoint = 880;//950;  20230509 LMF 修改风扇之后可以降低温度
	g_wInvOTBackPoint = 730;//800;
}
...
INT16S swFanSpeedCal_TjCtr(void)
{
	...
	{
		wFanSpeedTemp2 = (INT16S)(((INT32S)g_wLLC_TXTemp*cFanControlPercentum/cLLC_TXTempRange+5)/10-58);
	}
	else
	{
		wFanSpeedTemp2 = 0;
...
INT16S swGetLLC_MosTj(void)
{
	return(swComponentTjCal(cLLC_MosKt,abs(g_wPDCDCCurrAvg),g_wConvertLTemp,cNTCTempDelta26,10));
}
INT16S swGetBUCK_IGBTTj(void)
{
	return(swComponentTjCal(cBUCK_IGBTKt,abs(g_wPDCDCCurrAvg),g_wConvertHTemp,cNTCTempDelta22,10));
}
INT16S swGetINV_IGBTTj(void)
{
	return(swComponentTjCal(cINV_IGBTKt,g_uwRInvCurr,g_wInvTemp,cNTCTempDelta14,10));
}
INT16S swGetPV_BOOST_DiodeTj(INT16S swPVNumber)
{
	...
	return(swComponentTjCal(cPV_BOOST_DiodeKt,uwSolarCurrAvgTemp,g_wSolarBSTTemp,cNTCTempDelta23,100));
}
