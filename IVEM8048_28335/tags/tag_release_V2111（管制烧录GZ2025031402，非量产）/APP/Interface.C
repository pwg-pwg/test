...
INT8U  g_ubTimeUpdateFlag = false;  //MCUÏÂ·¢ÊµÊ±Ê±¼ä±êÖ¾Î»
...
const pFunc1	GetDataSubArray[100]=
{
	...
	swGetPDCDCAvgCurrReal,	swGetPBusVoltReal,			swGetRInvVoltReal,						//33-35  34 Ä¸ÏßµçÑ¹
	...
	//PV Loop
	(pFunc1)suwGetBoost1CtrlSts,swGetVerr_P1,           swGetVm_P1,          //51-53PV1×´Ì¬            â»·Îó²î         Íâ»·Êä³ö
	swGetPV1CurrLimit,     swGetImo_P1,				    swGetR_Is_P1,        //54-56PV1Íâ»·Êä³öÏÞ·ù		 PV1ÄÚ»·¸ø¶¨      PV1ÄÚ»·²ÉÑù
	...
	//OnGrid  
	swGetInvBusVoltRef,    swGetInvBusVoltfdb,          swGetFBVm_P,     //66-68²¢ÍøÄ¸ÏßµçÑ¹¸ø¶¨,Íâ»·µÄÊä³ö,»ùÓÚ²¢ÍøÄ¸ÏßÍâ»·Êä³öµÃµ½µÄµç³Ø¿ØÄ¸ÏßµçÑ¹Îó²î
	swGetBusVoltErr,       swGetRFBVm_P,                swGetFBImo_P,    //69-71 ²¢ÍøÄ¸ÏßÍâ»·µÄ×îÖÕÊä³ö£¬´øR,ÄÚ»·µçÁ÷¸ø¶¨,ÄÚ»·²ÉÑù
    swGetFBR_Is_P,         swGetRFBR_Is_P,              swGetRFBPWM,     //72-74ÄÚ»·Îó²î,ÄÚ»·Êä³ö¾­¹ýµ÷ÖÆÖ®ºóµÄPWM£¬´øR
	swGetFBPWM,            swReturnNull,                swReturnNull,    //75-77
	//OffGrid
	swGetRSinPointer,      swGetRLineVoltTest,          swGetRInvDCVoltCtrl,//78-80Ô­Ê¼µÄPLL½Ç¶È,ÀëÍøµÄÏàÎ»,µçÑ¹Ö±Á÷·ÖÁ¿Êä³ö
	swGetVref,             swGetRInvVoltCntl,           swGetInvVoltError,  //81-83ÀëÍøÄæ±äµçÑ¹»·¸ø¶¨//ÀëÍøÄæ±äµçÑ¹»·²ÉÑù  ÕæÊµÖµ*10*32//ÀëÍøÄæ±äµçÑ¹»·Îó²î
    swGetRLoadCurrCntl,	   swGetVBeforeSaturation,      swGetRInvCurrCntl,  //84-86ÀëÍøÖØ¸´¿ØÖÆÍâ»·Êä³ö»áµþ¼ÓÕâ¸ö//ÀëÍøÖØ¸´¿ØÖÆÍâ»·Êä³ö//ÀëÍøÄÚ»·²ÉÑù
	swGetVoltLimit,        swGetKpwm,                   swGetLoopOutput,    //87-89ÀëÍøÄÚ»·Êä³ö//ÀëÍøÄÚ»·×îÖÕÊä³ö 							
	//DCDC	
	swGetBusVoltErr,	   swGetBatVoltErr, 			swReturnNull,	 //90-92
	swGetBusVmo,           swGetPDCDCImo,               swGetPDCDCPWM,       //93-95 DCDCÍâ»·Êä³ö  //DCDCÄÚ»·¸ø¶¨  //DCDCÄÚ»·µÄÊä³ö														
	(pFunc1)swGetSinePeriodCntNew,(pFunc1)swGetLinePeriodCntNew,swGetInvStep,//96-98																						  
...
INT16U cal_crc_half(INT8U far *pin, INT8U len)
{
	...
	while(len--!=0) 
	{
		da=((INT8U)(crc>>8))>>4; /* ÔÝ´æCRCµÄ¸ßËÄÎ» */

		crc<<=4; /* CRCÓÒÒÆ4Î»£¬Ïà±ÓÚÈ?RCµÄµÍ12Î»£©*/

		crc^=crc_ta[da^(*ptr>>4)]; /* CRCµÄ¸ß4Î»ºÍ±¾×Ö½ÚµÄÇ°°ë×Ö½ÚÏà¼Óºó²é±í¼ÆËãCRC£¬È»ºó¼ÓÉÏÉÏÒ»´ÎCRCµÄÓàÊý */

		da=((INT8U)(crc>>8))>>4; /* ÔÝ´æCRCµÄ¸ß4Î» */

		crc<<=4; /* CRCÓÒÒÆ4Î»£¬ Ïàµ±ÓÚCRCµÄµÍ12Î»£© */

		crc^=crc_ta[da^(*ptr&0x0f)]; /* CRCµÄ¸ß4Î»ºÍ±¾×Ö½ÚµÄºó°ë×Ö½ÚÏà¼Óºó²é±í¼ÆËãCRC£¬È»ºóÔÙ¼ÓÉÏÉÏÒ»´ÎCRCµÄÓàÊý */
