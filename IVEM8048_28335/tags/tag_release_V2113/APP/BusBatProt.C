...
#define cLLC_MosTjProtectPoint			970 	//99C
#define cOtherTjProtectPoint			1310	//131C 		//cBUCK_IGBTKt\cINV_IGBTKt\cPV_BOOST_DiodeKt
#define cInnelTempProtectPoint			g_wInnelOTPoint		//70C
#define cLLC_TXTempProtectPoint			1380    //138C
//Temprature Derat
#define cLLC_MosTjDeratPoint			880		//88C
#define cOtherTjDeratPoint				1220	//122C 		//cBUCK_IGBTKt\cINV_IGBTKt\cPV_BOOST_DiodeKt
#define cInnelTempDeratPoint			g_wInnelOTBackPoint	//60C
#define cLLC_TXDeratPoint				1290   //129C	

//cOtherTjDeratPercent应该是1111,1136比较大，可以不处理
#define cLLC_MosTjDeratPercent			1111     //5.88%     //1/(cLLC_MosTjDeratPoint-cLLC_MosTjProtectPoint)*100000
...
void sBusBatProtectTask(void)
{
	...
	INT16U uwBatLowCapProRecoCnt = 0;
	INT16S g_wBatLowCapProCnt = 0;
...
void sOverTempParaInit(void)
{
	...
	g_wCovertLOTPoint = 700;//850;
	g_wCovertLOTBackPoint = 550;//700;
...
//			wOPVoltMinTmep = g_uwLineInputVoltLLoss;//90V
		    wOPVoltMinTmep = g_uwLineVoltLLoss;//170V
		}
		else//UPS
		{
...
INT16S swFanSpeedCal_TjCtr(void)
{
	...
	{
		wFanSpeedTemp = (INT16S)(((INT32S)wTjTemp*cFanControlPercentum/cLLC_MosTempRange+5)/10-57);
	}
	else
	{
		wFanSpeedTemp = 0;
...
void sSolarPowerOverLoadChk(INT8U bFilter)
{
    ...
    static INT16U s_uwSolar1PowerOverLoadFlag = false;
    static INT16U s_uwSolar2PowerOverLoadFlag = false;

    if(s_uwSolar1PowerOverLoadFlag == false)
    {
        if(sbOverLevelChk(g_uwSolarPower1Avg, (INT16U)((g_dwTotalPower >> 1) + 100), bFilter, &s_uwSolar1PowerOverLoadChkCnt))
        { 
    ...
    {
        if(sbUnderLevelChk(g_uwSolarPower1Avg, (INT16U)((g_dwTotalPower >> 1) - 100), bFilter, &s_uwSolar1PowerOverLoadChkCnt))
        { 
	...
	{
		if(sbOverLevelChk(g_uwSolarPower2Avg, (INT16U)((g_dwTotalPower >> 1) + 100), bFilter, &s_uwSolar2PowerOverLoadChkCnt))
		{ 
	...
	{
		if(sbUnderLevelChk(g_uwSolarPower2Avg, (INT16U)((g_dwTotalPower >> 1) - 100), bFilter, &s_uwSolar2PowerOverLoadChkCnt))
		{ 
    ...
    {
        if(s_uwSolar1PowerOverLoadFlag || s_uwSolar2PowerOverLoadFlag)
        {
...
//      if(++g_wPVPowerOverLoadCurrLimit > swGetEEBatChgCurrMax())
    ...
    {
         if(   (g_uwSolarPower1Avg > ((g_dwTotalPower >> 1) + 1000))  //6000
		   || (g_uwSolarPower2Avg > ((g_dwTotalPower >> 1) + 1000)) )
        {
            g_wPVPowerOverLoadCurrLimit -= 50;	
        }
		if(   (g_uwSolarPower1Avg > ((g_dwTotalPower >> 1) + 500))  //5500
		   || (g_uwSolarPower2Avg > ((g_dwTotalPower >> 1) + 500)) )
        {
            g_wPVPowerOverLoadCurrLimit -= 20;//15; 
        }
		else if(   (g_uwSolarPower1Avg > ((g_dwTotalPower >> 1) + 250))  //5250
