...
INT64U gi_udwSolarPower1SampleSumTemp;
INT64U gi_udwSolarPower1SampleSum;
INT16U gi_uwSolarPower1SampleSumCntTemp;
INT16U gi_uwSolarPower1SampleSumCnt;

INT64U gi_udwSolarPower2SampleSumTemp;
INT64U gi_udwSolarPower2SampleSum;
...
INT8U  sQuarterFastCheck(INT16S wLineVoltReal,INT32U wLineVoltAvgRms);
...
//		wTemp = (INT16S)(((INT32S)gi_uwLinePeriodNew * 3) >> 9);	//52*3.3 = 172 = 3>>9
		...
		{
			if(++wVoltHighChkCnt >= 38)       //19*52us=1ms
			{
				wVoltHighChkCnt = 0;
				*pwLineWaveLossFlag = true;
				*pwLineWaveOKCnt = 0;
		...
		if(sQuarterFastCheck(wLinvVolt,g_uwRLineRms3timeAvg)) 
		{
		    if(gi_uwWaitLineVoltSteady)
		    {
...
INT8U  sQuarterFastCheck(INT16S wLineVoltReal,INT32U wLineVoltAvgRms)
{	
	static INT32S  	dwLineVoltFastAdcSum = 0;		 
	static INT16U 	wLineVoltBlockCnt = 0;
    static INT16U   wLineFastCheckNumScale = 0;
	static INT16S	wQuarterCycleData[cSinQuarterCycleNum]={0};			//Fast Check 
	INT16U 	wLineVoltQuarterAverScale;	
    
	dwLineVoltFastAdcSum += abs(wLineVoltReal);			//快检测时电压的绝对值累加，以求平均值
	if(dwLineVoltFastAdcSum > abs(wQuarterCycleData[wLineVoltBlockCnt]))
	{
		dwLineVoltFastAdcSum -= abs(wQuarterCycleData[wLineVoltBlockCnt]);
	}
	wQuarterCycleData[wLineVoltBlockCnt] = abs(wLineVoltReal);
	
	if(++wLineVoltBlockCnt >= cSinQuarterCycleNum)
	{
		wLineVoltBlockCnt = 0;
	}

    if (wLineFastCheckNumScale < cSinQuarterCycleNum)   // 更新样本计数，直到达到窗口大小
	{
        wLineFastCheckNumScale++;
    }
	
	wLineVoltQuarterAverScale = (INT16U)(dwLineVoltFastAdcSum / wLineFastCheckNumScale);

    if(wLineVoltQuarterAverScale < (INT16U)((wLineVoltAvgRms * cLineQuickChkRatio)>>12))
    {
        return 1;    
    }
	else
