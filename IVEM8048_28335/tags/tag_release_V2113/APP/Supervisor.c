...
INT16S swLineModeReady(void)
{
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			if(subGetParaBatOpenSts() || uwLLCWorkFlg || uwForceGridSoftBusFlag == 1)//无电池 或 (市电建立患禕US经过buck启动二级BUS后，LLC工作，再关闭DCDC)
			{
				hoBUSSOFTOff;
		...
		//					return(swInvVoltSoft());
					...
					else
					{
						...
						if(   !uwForceGridSoftBusFlag && ((g_uwBatVoltAvg <= 460) || (uwBusSotfOverDelay < 1000)) //subGetBatWeakSts() 
						   && subGetPalLineLossStatus() == false && g_uwRLineRms3timeAvg >= 1800 )
						{
...
INT8U mChkOP_TWINS_Sts(void)
{
    ...
    //20231010  SOC判断，锂电池放电时没有通讯不允许第二路输出
	...
	if (Ret == true)
	{
		...
		else  if(swGetEEOP2OnInACModeEn() && (g_uwWorkMode ==cLineMode || g_uwWorkMode ==cBypassMode))//||g_wSolarSigPowerLoad)
		{
		    //TESEOPCODE |= 0X0010;
...
{	
	...
	while(1)
	{
		...
		if (mChkOP_TWINS_RlyOn())
		{
			...
			{
				sOSTimerStop();
				sRlyDelayProc(RLY_CROSSZERO,cOP2RlyOffDelayTime,eSuperSelectPoint,cPrioSuper);
				sOSTimerStart();
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			if(mChkOPRlyOn() && mChkGridRlyOn()&& (gi_wParallelEnable==0) && gi_uwOPRelayOnSteady)
			{
				if(true == mChkOP_TWINS_Sts())// ||mBMSBatSOC >= 80
				{
					...
					{
						sOSTimerStop();
						sRlyDelayProc(RLY_CROSSZERO,cOP2RlyOnDelayTime,eSuperSelectPoint,cPrioSuper);
						sOSTimerStart();
					}
...
//					mDCDCLLCOff();
					gi_uwGridRelayOn = 2;
					sRlyDelayProc(RLY_DELAY,cSFTRlySCRDelayTime,eSuperSelectPoint,cPrioSuper);
				}
				gi_uwGridRelayOn = true;
				if(!g_wSolarSigPowerLoad)
				{
...
//					}
			...
			if(gi_uwOPRelayOn && sbGetInverterPLStatus() == true&& (gi_wParallelEnable==0) && gi_uwOPRelayOnSteady)//单机才允许
			{
				if(true == mChkOP_TWINS_Sts())// ||mBMSBatSOC >= 80
				{
					if (!gi_uwOP_TWINS_RelayOn)
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			else
			{
				...
				&& g_uwSolarVolt1Avg < 800 && g_uwSolarVolt2Avg < 800 && g_uwRLineVolt < 800 && hoEEWPON && !wSetPageFlag)  //!mLCDPage)
				{
					uwShutDownChkCnt++;
