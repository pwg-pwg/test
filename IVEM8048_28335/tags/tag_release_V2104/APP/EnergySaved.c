...
INT8U  g_ubClrPVEnergyLogFlag;
INT8U  g_ubClrLoadEnergyLogFlag;
...
void sPowerEnergySaved(void)
{
	...
	if(swGetEEEnergyStoredEn() && !g_ubClrPVEnergyLogFlag)
	{		
		if(!g_uwSolarLoss)
		{
			dwPower += g_uwSolarPower1Avg;
			if(s_uwPVStableCnt < 300)				//5Min * 60
			{
				s_uwPVStableCnt++;
			}
		}
		...
		&& g_strChkEnergyDateTime.ubYear >= cEnergySaveStartYear)
		{
			pdwEnergySaved = &uEepromCfg3.EepromStructCfg3.dwEEPowerSaveYear1;
...
void sLoadEnergySaved(void)
{
	...
	if(swGetEEEnergyStoredEn() && !g_ubClrLoadEnergyLogFlag)
	{		
		if((g_uwWorkMode == cBatteryMode || g_uwWorkMode == cBypassMode || g_uwWorkMode == cLineMode)
			&& gi_uwOPRelayOn)
		{
			dwPower += g_dwOPWatt;
			if(s_uwLoadStableCnt < 300)				//10Min * 60
			{
				s_uwLoadStableCnt++;
			}
		}
