...
INT8U mChkOP_TWINS_Sts(void)
{
    INT16S  wBatLevel;            //20231010 SOC判断
		...
		//&&(g_uwWorkMode == cBatteryMode && !g_wSolarSigPowerLoad))
    {
	    ...
	    {
		    wBatLevel = g_wBMSTestSoc;//mBMSBatSOC;
	    }
		else
		{
		    //TESEOPCODE |= 0X0001;
	...
	if (Ret == true)
	{
			 ...
			 /*
		{
		    ...
		    if(((swGetEEBatteryType() != cBatType_LIB)&&(!bVoltLowCutOffFlag))
				||((swGetEEBatteryType() == cBatType_LIB)&&(!bSocLowCutOffFlag)))
			{
			    //TESEOPCODE |= 0X40;
				return true;
			}
