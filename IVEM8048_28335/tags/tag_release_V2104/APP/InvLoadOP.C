...
void sEnergyInfoUpdate(void)
{
	...
	&& ((swGetEEOPPriority() != cOutputUtilityFirst) || subGetParaBatOpenSts()))
	{
...
void sInvPowerCal(void)
{
	...
	//以下两句直接用 g_dwOPWatt 和 g_dwInvWatt
	//g_dwOPWatt = g_dwROPWatt + g_dwSOPWatt + g_dwTOPWatt;
	//g_dwInvPowerWatt = g_dwRInvWatt + g_dwSInvWatt + g_dwTInvWatt;
	...
	//--放电时，Pinv为正，Pbat为负，且|Pbat| > Pinv
	//--充电时，Pinv为负，Pbat为正，且|Pinv| > Pbat
	//--inv只能充电或者放电，对于取电功率来说，
	//--无PV和有PV时，都只需要考虑inv是充电还是放电
	//--inv放电时，市电只给负载提供功率(或不提供)，Pline = Pload-Pinv
	//--inv充电时，市电提供充电和负载功率(或不提供)，Pline = Pload-Pinv
	...
	if(labs(g_dwLinePowerWattCversion) < 80)  //--防止侦测误差
	{
	...
	if(g_uwWorkMode == cLineMode)
	{
		...
		if(subGetBatOpenSts() || !suwGetDCDCCtrlSts())
		{
			...
			//--这一块可能有问题，如果PV能提供负载需要的所有功率，
			//--那么市电应该为0，但是由于Pinv > Pload，会导致Pline不等于0
			else//PV + Grid -> load
			{
			    //--无电池，即没有充电，那么PV只能给负载供电，同理市电也是
			    //--而Ppv>Pinv(有损耗)，因此让Ppv = Pinv
			    //--Pline = Pload - Pinv
			    //--这样，Ppv+Pline=Pinv+Pload-Pinv=Pload，实现功率相等
			    //--这样可能存在问题是LCD显示的PV功率与APP的不相等
			    //--但是不能放到市电，空载的时候会显示为馈电
				g_uwPVPowerConversion = abs(g_dwInvWatt);
			}
		...
		{
		    //--Pinv<100W，认为负载功率全由市电提供;否则负载功率由市电和逆变提供
		    //--这一段，好像去掉也行？
		    /*
		    if(abs(g_dwInvWatt)<100)
		    {
				g_dwLoadPowerConversion = g_dwLinePowerWattCversion;
			}
			else
			{
			...
			*/
			...
			if(g_uwSolarLoss)//batt + grid -> load
			{
				...
				//--电池停止放电或空载的情况下，侦测有33W左右的放电(实际测试一下？)
				...
				{
					g_wBatPowerConversion = g_uwPVPowerConversion - g_dwInvWatt;//电池=逆变功率，不考虑空载损耗
					g_dwLoadPowerConversion = g_dwLinePowerWattCversion + g_dwInvWatt;
				}
			}
			else// if(!g_uwSolarLoss)//pv + batt + grid -> load
			{
                ...
                if(labs(g_wPVPowerCal - g_dwInvWatt) < 100)//仅是逆变输出下，PV和逆变的功率相差接近，PV等于逆变功率，规避电池小功率输出
                {
                ...
                // Bat = PV - INV
                g_wBatPowerConversion = g_uwPVPowerConversion - g_dwInvWatt;
				g_dwLoadPowerConversion = g_dwOPWatt;
			}
		}
	}
	else if(g_uwWorkMode == cBatteryMode)//离网模式
	{
		...
		{
			g_uwPVPowerConversion = g_wPVPowerCal;
			g_wBatPowerConversion = g_uwPVPowerConversion - g_dwLoadPowerConversion;
		}
	}
