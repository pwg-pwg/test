...
{ 
	//static INT8U bBatChkCnt = 0;
  {
	...
	if(sbGetEepromBatteryType() == cLiBType&&fBMSConnectFlg)
	{
		mSetVramBit(cBIT_Bat_C, cALWAYS_LIGHT);
	}
}
...
INT8U sbDisplayWarning(void)
{
	...
	if(ubWarnCodeDisPlayFlag)
	{
		...
		if(sbGetEepromBatteryType()==cLiBType && !fBMSConnectFlg && (RXSTATUS>0)){
  ...
  {      
	...
	else
	{
      ...
      else if(bMenuIndex ==  CF_DCI_ADJUST_ID)
      {
			...
			if(DCI){
				a=abs(wInvVoltBiasVaue);
				c=a%10;
				b=(a%100)/10;
				a=a/100;
				sViewFigModule(cFIG_TYPE_7, FIG6_7SEG, a, bConfigContentFlashStatus);
				sViewFigModule(cFIG_TYPE_7, FIG7_7SEG, b, bConfigContentFlashStatus);
				sViewFigModule(cFIG_TYPE_7, FIG8_7SEG, c, bConfigContentFlashStatus);
				if(wInvVoltBiasVaue>=0){
					//sViewFigModule(cFIG_TYPE_7, FIG5_7SEG, charAdd, bConfigContentFlashStatus);
				}
				else{
					sViewFigModule(cFIG_TYPE_7, FIG5_7SEG, charSub, bConfigContentFlashStatus);
				}
			}
