...
//		else 
	...
	{
		do
		{	//若BMS ID大于最大查询数据(当轮询到最后一个)
			if(g_uwPylonBMSCmdAddrID > cPylonBMSAddrIDMax)
			{
				g_uwPylonBMSCmdAddrID = 1;//从零开始继续下一轮
				uwTemp = s_uwCycleNoOkChkID;//备份上次轮询检测的ID
				if(uwTemp >= cPylonBMSAddrIDMax)
				{
					uwTemp = cPylonBMSAddrIDMax + 1;
				}
				...
				for(uwBMSCmdAddrIDTemp = 1; uwBMSCmdAddrIDTemp <= cPylonBMSAddrIDMax; uwBMSCmdAddrIDTemp++)
				{
					...
					else if(uwTemp == s_uwCycleNoOkChkID && uwBMSCmdAddrIDTemp > uwTemp && uwTemp < cPylonBMSAddrIDMax)
					{
						uwTemp = uwBMSCmdAddrIDTemp;//需要循环检测到的机架
					}
				}

				s_uwCycleNoOKRollDelayCnt++;//避免连续检查三个轮次都不退出？
			...
			{
				//若通信正常，则认为准备完成？(检测ID的通信此前已存在)
				if(g_strPylonBMSComSts[g_uwPylonBMSCmdAddrID].fBMSComOK)
				{
					g_strPylonBMSComSts[g_uwPylonBMSCmdAddrID].fBMSComReady = true;
				}
				else if(s_uwCycleNoOkChkEn)
				{
					if(g_uwPylonBMSCmdAddrID == s_uwCycleNoOkChkID || s_uwCycleNoOkChkID == (cPylonBMSAddrIDMax + 1))
					{
						s_uwCycleNoOkChkEn = false;
						s_uwCycleNoOkChkID = g_uwPylonBMSCmdAddrID;
						g_strPylonBMSComSts[g_uwPylonBMSCmdAddrID].fBMSComReady = true;
					}
					else
					{
		...
		}while(!g_strPylonBMSComSts[g_uwPylonBMSCmdAddrID].fBMSComReady);
	}
}
