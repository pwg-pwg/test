...
//					wFANPWMTempNew = cFanLowerSpeed;
				...
				g_wACFanOnFlag =0;
				    ...
				    || (sbGetTxTempStatus() >= 3) || (sbGetInvTempStatus() >= 3) )//MPPT没有风扇，温度过高开逆变器风扇降温
			{
...
//				wFANPWMTempNew = cFanFullSpeed;//CC 20210402
				...
				{ g_wACFanOnFlag = 1; } //AC Fan Contral -- SCC大电流运行时不运行交流风机
				bFan1status = true;
				bFan2status = true;
			}
...
//					wFANPWMTempNew = cFanLowerSpeed;
	...
	{
		if(hoACFAN==0)// CC 20210621 鎺у埗浜ゆ祦椋庢墖缁х數鍣?
		{
			sOSTimerStop();
...
void 	sBatWeakChk(INT8U bFilter)
{
	...
	//if((!g_uwBatWeakFlg)&&(sbGetBatLow() == true))// 鐢垫睜鐢靛帇batlowback鍜宐atlow涔嬪墠锛屼竴鐩存姤浣庡帇涓嶅垏甯傜數闂?棰?
	//{
	//	g_uwBatWeakFlg = 1;
	//}
...
void sACSPSControl(void)
{
		...
		&& (0 == sbGetRLineWaveLoss()))
	{
		...
		{
			if(++wACspsCtrlMSCnt>20)// 10S 甯傜數姝ｅ父寤舵椂10S鎵嶉棴鍚?
			{
				wACspsCtrlMSCnt =0;
				bACSpsCtrlOffFlag =0;
				hoACSPS_ON;
		...
		if( ++wACspsCtrlSecCnt>620)//5 minute 浠ュ悗鏂?寮�缁х數鍣?
		{
			wACspsCtrlSecCnt =0;
			if(!hoACSPS)
			{
