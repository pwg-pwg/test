...
//		}
		...
		if(LiHardProtectCNT>=3)//ç¡¬ä»¶è§¦å‘ä¿æŠ¤ 
		{
			if((wInvOffPoint ==0)&&(wContinueTrig==0))
			{
				...
				wInvOffPoint =100;// å…³é—­50ç‚?wInvOffPoint =150;// å…³é—­150ç‚?
				wContinueTrig =1;
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>20))//if((wInvOffTimes ==0)&&(++wContinueTrigCNT>30))//é˜²æ­¢è¿žç»­è§¦å‘
			{
				wContinueTrigCNT =30;
				wContinueTrig =0;
			}
...
interrupt void MPPTInterrupt(void)
{
	...
	if(bPVMode == cBatteryMode )
	{
		if(LiHardProtectCNT>=3)//ç¡¬ä»¶è§¦å‘ä¿æŠ¤ 
		{
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>20))//if((wInvOffTimes ==0)&&(++wContinueTrigCNT>30))//å»¶æ—¶30ä¸ªç‚¹ é˜²æ­¢è¿žç»­è§¦å‘
			{
				wContinueTrigCNT =30;
				wContinueTrig =0;
			}
...
void sLineWaveChk(INT16U wCmpPoint)
{
	...
	if(1)//ä¸è¿›è¡Œæ³¢å½¢æ£€æµ?//20210111Chandler
	{
	   wLineLossTemp = false;
	}
	else 
...
void HardProtectChk(void)
{
	if((LiHardProtect==0)||((liparaTx==0)&&(mEepromOutputMode() > cOP_Single)))//ç¡¬ä»¶è§¦å‘ä¿æŠ¤
	{
//		LiHardProtectCNT++;
