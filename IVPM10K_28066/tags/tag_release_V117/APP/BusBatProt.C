...
void 	sBatWeakChk(INT8U bFilter)
{
	...
	//if((!g_uwBatWeakFlg)&&(sbGetBatLow() == true))// 鐢垫睜鐢靛帇batlowback鍜宐atlow涔嬪墠锛屼竴鐩存姤浣庡帇涓嶅垏甯傜數闂
	//{
	//	g_uwBatWeakFlg = 1;
	//}
...
void sLineModeInvOverCurProtCnt(void)
{
	...
	if(bPVMode!=cLineMode)
	{
		wSetInvOverCurrentLineModeCnt = 4;//鍒氬垏鍒板競鐢垫ā寮忕殑鏃跺�欙紝鍛ㄦ湡鏀逛负4
		uwTranformCnt = 0;
	}
	else
	{
...
void sACSPSControl(void)
{
		...
		&& (0 == sbGetRLineWaveLoss()))
	{
		...
		if( ++wACspsCtrlSecCnt>620)//5 minute 浠ュ悗鏂紑缁х數鍣?
		{
			wACspsCtrlSecCnt =0;
			if(!hoACSPS)
			{
