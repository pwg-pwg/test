...
//	if((mEepromOutputMode() == cOP_Single)||(mGetEEKpKiParameter ==5))//CC DeLETE
	if((mGetEEKpKiParameter ==5)&&(mEepromOutputMode() == cOP_Single))//
	{
	...
	else                
	{
		...
		if(wInvVoltReal<(-500))//-50V 
		{
			if(++IsoReverseCnt>50)//
			{
				...
				if(++IsoReverseChkCnt>60)//  60/2=30  Á∫?0‰∏™Â∏ÇÁîµÂë®Êúü‰ª•ÂêéÊâçÊ£ÄÊµ?
				{
					IsoReverseChkCnt =60;
	...
	else
	{
		...
		***************************************************************************/
		...
		if(++wFBCntLoop&0x08)
		{
			...
			wFBP_VoltPIAdjCtl =(INT32S)((INT32S)wFBP_VoltPIAdjCtl*3+ wVm_P)>>2;// ÂÅèÂ∑Æ‰∏? Êó∂ÔºåÂâçÈ¶à‰∏çÂèóÊé?
			
			//wVm_P =TestVlotErr;
		}
		
		/**************************************************************************
		*  ÁîµÊµÅÁéØÊØè1‰∏™ÁÇπ‰∏ÄÊ¨?
		***************************************************************************/
		wLineVoltCntlQ5=(INT16S)((INT32S)gi_wLineVoltQ5_0+(INT32S)(abs(wLineVoltCntlQ5+200))>>1);// 200ÊòØÁîµÂéãÈááÊ†∑ÂÅèÁΩÆË°•ÂÅ?
		...
		if(wBatVoltReal>0)
		{
			...
			wR_Is_P =abs(wInvLCurrReal);//0.1V  //ÈúÄË¶ÅÂä†ÂÖ•ÁîµÂéãÂà§Êñ≠Â§ÑÁêÜÁîµÊµÅËøáÊµ?
		}
		else
		{
			wR_Is_P = 0;
