...
INT16U InvDurationTime = 0;
INT16U EepromInvCnt = 0;
INT16U EepromInvOKFlg = 0;
...
************************************************************************************/
INT8U	wInvOKChkCnt = 0;
void	sBatteryMode(void)
{
	TASK_EVENT	event;
	INT16U	wLineOKChkCnt = 0,tepm = 0;
	INT8U	bInvSoftTimeOutCnt = 0;
	
	AvrAllOff;
	//bLowPowerflag = true;
	g_uwBatModeLineOKFlag = false;
	//EepromInvCnt = (INT8U)(swGetEERestartInvCnt());//
	if (swGetEERestartInvCnt() >= 3)
	{
		mInvSoftOKFlg = false;
		tepm |= 0x04;
		//sSetEERestartInvCnt(3);
		//OSEventSend(cPrioInterface, eEepromSaveUserSetData);
		if(swGetEepromBatCVVolt()<=wBatAvgVoltNew)
		{
			sSetEERestartInvCnt(0);
			OSEventSend(cPrioInterface, eEepromSaveUserSetData);
		}else
		{
			OSEventSend(cPrioSuper, eSuperToStandby);
	...
	{
		tepm |= 0x40;
		sSetEERestartInvCnt(swGetEERestartInvCnt()+1);
		OSEventSend(cPrioInterface, eEepromSaveUserSetData);
	}
	fChargeOn=0;
...
//				while(OSMaskEventPend(0x7FFF) == 0);
		...
		if(event&(1<<eSuperTimer))
		{
			...
			if (mInvSoftOKFlg == true)
			{

				if (InvDurationTime < 15000)
				{
					InvDurationTime++;
					//EepromInvCnt |= 0x10;
				}else{
...
//			EepromInvCnt = TEMP;
			...
			if(wInvCrrentLimit != g_wInvCurrLimitRated || wInvOverCurrentLimit != g_wInvOverCurrRated)
			{
				...
				if(mEepromOutputMode() == cOP_2P2)// 缁勫弻鐏嚎闇�鏀逛袱涓笉鍚岀殑闄愭祦鍊�
				{
...
void	sLineMode(void)
{
	...
	while(1)
	{
		...
		if(event&(1<<eSuperOPRlyOn))
		{
			...
			{
				if((g_wVAType == cb3500VAModel)||(swGetEepromOutputMode() == cOP_Single))// 3.5K 娌℃湁SCR
				{					
					sOSTimerStop();
					sSetRlyPointer(RLY_CROSSZERO,cOPRlySpeedUpOnDelayTime,eSuperSelectPoint,cPrioSuper);
					while(OSMaskEventPend(0x7FFF) == 0);
...
INT8U sbInvSoftStart(void)
{
	...
	INT16U  eepromdata = swGetEERestartInvCnt();
	...
	if(InvVoltSoftFalseFlag == 0)
	{
		...
		sSetRSinAmp(0);// 0V
		sSetRNewSinAmp(0);//0V
//		while (eepromdata==swGetEERestartInvCnt())
//		{
//			sSetEERestartInvCnt(swGetEERestartInvCnt()+1);
//			OSEventSend(cPrioInterface, eEepromSaveUserSetData);
//		}
...
INT8U sbOutputVoltChk(void)
{
	...
	while(1)
	{
		...
		if(event & (1<<eSuperTimer))	//20ms
		{
			if(swGetEepromOutputMode() == cOP_Single)
			{
				// 不能≤220V/2,因为实际应用可能N线连接在一起,导致输出检测到电压,出现误判
...
INT8U  sbTestModeChk(INT8U b20ms)
{
	...
	while(bTemp)
	{
		...
		if(event & (1 << eSuperTimer))
		{
			// 测试模式需接入75Ω电阻,所以采样值为75/(5100+75)=59(-1%=-40,+0.5%=20)
...
INT16S swWorkModeChk(void)
{
	...
	else
	{
		...
		{
			// 电池管理优化,切换至电池优先模式
			// EEPROM输出优先模式不做修改,防止未保存断电导致模式设置改变
			if(swGetBattMgtFlag())
			{
