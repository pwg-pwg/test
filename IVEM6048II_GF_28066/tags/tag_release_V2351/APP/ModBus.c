...
void sFaultRecordProc(INT16U uwFaultID, INT16S wFalutValue);
...
extern INT16U g_uwAPPLoadOnSts;
extern INT16U g_uwAPPFaultRestart;
...
//	uRealTimeData.Stru.wFaultMsg = 0;
	...
	if(!swGetEEFeedPowerEn() && uRealTimeData.Stru.wRLineWatt < 0)
	{
	    uRealTimeData.Stru.wRLineWatt = 0;
	}
	...
	uRealTimeData.Stru.wSccHsTemp = wTemp/10;
	//-------------------------------------
...
//	uRealTimeData.Stru.wBMSFaultCode = mBMSFaultCode;
	...
	uRealTimeData.Stru.wFeedEnergyTotalHH = (INT16U)((g_uldwFeedEnergyTotal >> 48) & 0xFFFF);
	uRealTimeData.Stru.wFeedEnergyTotalHL = (INT16U)((g_uldwFeedEnergyTotal >> 32) & 0xFFFF);
	uRealTimeData.Stru.wFeedEnergyTotalLH = (INT16U)((g_uldwFeedEnergyTotal >> 16) & 0xFFFF);
	uRealTimeData.Stru.wFeedEnergyTotalLL = (INT16U)(g_uldwFeedEnergyTotal & 0xFFFF);
    uRealTimeData.Stru.wFeedEnergyThisYearH  = 0;
    uRealTimeData.Stru.wFeedEnergyThisYearL  = 0;
    uRealTimeData.Stru.wFeedEnergyThisMonthH = 0;
    uRealTimeData.Stru.wFeedEnergyThisMonthL = 0;
    uRealTimeData.Stru.wFeedEnergyThisDayH   = 0;
    uRealTimeData.Stru.wFeedEnergyThisDayL   = 0;
    uRealTimeData.Stru.wFeedEnergyYearH      = 0;
    uRealTimeData.Stru.wFeedEnergyYearL      = 0;
    uRealTimeData.Stru.wFeedEnergyMonthH     = 0;
    uRealTimeData.Stru.wFeedEnergyMonthL     = 0;
    uRealTimeData.Stru.wFeedEnergyDayH       = 0;
    uRealTimeData.Stru.wFeedEnergyDayL       = 0;
	//功率折算
	uRealTimeData.Stru.wLinePowerConvertion = (INT16S)g_dwLinePowerWattCversion;
    uRealTimeData.Stru.wLoadPowerConvertion = (INT16S)g_dwLoadPowerConversion;
    uRealTimeData.Stru.wGenPowerConvertion = (INT16S)g_dwGenPowerWattCversion;
    uRealTimeData.Stru.wSmartLoadPowerConvertion = (INT16S)g_dwSmartLoadPowerConversion;
    ...
    uRealTimeData.Stru.wPV2PowerConvertion = 0;//g_uwPV2PowerConversion;
	//电流折算
...
void sInfoDataUpdate(void)
{
	...
	uInfoData.Stru.wMCU2SwVer = cDSPVersion2;
...
#define cFaultListLength  5  			//历史故障记录长度


INT16U  uwFaultLength = 0;
INT16U  uwFaultOffset = 0;
INT16U  uwFaultChangeFlag = 0;
INT16U  uwFaultList[cFaultListLength] = {0}; 
INT16U  uwFaultIndex = 0;
INT16U  uwFaultIDBufList[cFaultListLength] = {0}; 
INT16U  uwFaultValueBufList[cFaultListLength] = {0}; 
INT16U  uwFaultBufferIndex = 0;
...
void sFaultRecord(INT16U uwFaultID, INT16S wFalutValue, INT16U uwFlag)
{
    ...
    {
        for(btemp = 0; btemp<cFaultListLength; btemp++)  
        {
            if(uwFaultID == uwFaultList[btemp])  return;
        }
		uwFaultList[uwFaultIndex] = uwFaultID;
		if(++uwFaultIndex >= cFaultListLength)  uwFaultIndex = 0; 

		if(!g_ubReadTimeOKFlg && (uwFaultBufferIndex<cFaultListLength))  //未获取到实时时间，先把故障记录到缓冲区
	    {
	        uwFaultIDBufList[uwFaultBufferIndex] = uwFaultID; 
			uwFaultValueBufList[uwFaultBufferIndex] = wFalutValue;
	        uwFaultBufferIndex++;
	        return; 
	    }

		sFaultRecordProc(uwFaultID,wFalutValue);		
    }
	else
	{
	    sFaultListClr();
...
INT8U sFaultBufferRecord(void)
{
    INT8U  bFaultBufIndex = 0;
	INT8U  bSaveFaultCodeFlag = 0;
	
    while(uwFaultBufferIndex > 0 && g_ubReadTimeOKFlg)
    {
        sFaultRecordProc(uwFaultIDBufList[bFaultBufIndex],uwFaultValueBufList[bFaultBufIndex]);
		uwFaultIDBufList[bFaultBufIndex] = 0;
		uwFaultValueBufList[bFaultBufIndex] = 0;
		bFaultBufIndex++;
		uwFaultBufferIndex--;
		bSaveFaultCodeFlag = 1;
    }
	return bSaveFaultCodeFlag;
}
...
void sFaultListClr(void)
{
    ...
    {
        uwFaultList[btemp] = 0;
		uwFaultIDBufList[btemp] = 0;
		uwFaultValueBufList[btemp] = 0;
    }
	uwFaultIndex = 0;
	uwFaultBufferIndex = 0;
}
