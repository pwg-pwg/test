...
#define cOverTempSolarSlopeSum			210		//21C-----15S/1 point;slope=15 point
#define cOverTempInvSlopeSum			280//185		//190//180//308		//30.8C
#define cOverTempConvertLSlopeSum		150//118		//128		//125//115//175		//17.5C
#define cOverTempConvertHSlopeSum		320//283		//305		//300//310//300//380		//38C
#define cOverTempLLC_TXSlopeSum			3000	//300C
...
//Temprature Derat
#define cLLC_MosTjDeratPoint			1050	//1125	//112.5C
#define cOtherTjDeratPoint				1312	//1312	//131.2C	//cLLC_IGBTKt\cINV_IGBTKt\cPV_BOOST_DiodeKt
#define cInnelTempDeratPoint			g_wInnelOTBackPoint	//60C
...
#define cOtherTjDeratPercent			1136	//11.36%
#define cInnelTempDeratPercent			1000	//10.00%
#define cLLC_TXTempDeratPercent			1170	//11.11%
...
#define BatLowCapProReco8Min 	24000   // 8Min = 24000*20ms
#define BatLowCapProReco10Min 	30000   // 10Min
...
void sBusLowToForcedChgProc(INT16U uwFilter);
...
void sBusBatProtectTask(void)
{
	...
	while(1)
	{
		...
		if(event&(1<<eBusBatProtTimer))			
		{
			...
			sBusLowToForcedChgProc(15000);//5min清除故障
			sBatParaUpdate();
...
/*
	...
	*/
	...
	{
		if(swUnderLevelChk(g_wConvertLTemp, g_wConvertLOTBackPoint, 500, &s_uwConvertLTempWRChkCnt))
		{
	...
	{
		if(swOverLevelChk(g_wConvertLTemp, g_wConvertLOTPoint, 50, &s_uwConvertLTempWRChkCnt)||\
	...
	{
		if(swUnderLevelChk(g_wConvertHTemp, g_wConvertHOTBackPoint, 500, &s_uwConvertHTempWRChkCnt))
		{
	...
	{
		if(swOverLevelChk(g_wConvertHTemp, g_wConvertHOTPoint, 50, &s_uwConvertHTempWRChkCnt)||\
...
//	if(fInnelTempOver)
//	{
//		if(swUnderLevelChk(g_wInnelTemp, g_wInnelOTBackPoint, 500, &s_uwInnelTempWRChkCnt))
//		{
...
//	else
//	{
//		if(swOverLevelChk(g_wInnelTemp, g_wInnelOTPoint, 50, &s_uwInnelTempWRChkCnt))
//		{
//			fInnelTempOver = true;
//		}		
//	}
...
void sSolarPowerOverLoadChk(INT8U bFilter)
{
	...
	if(g_wSolarPowerDeratFlag == false)
	{
		if(    g_uwSolar1BypassFlag
			&& sbOverLevelChk((INT16U)((INT16S)g_uwSolarPower1Avg + (INT16S)g_uwSolarPower2Avg),(INT16U)(g_dwTotalPower+100),bFilter,&s_uwSolarPowerOverLoadChkCnt))
		{
...
//	}
	...
	if(g_uwWorkMode == cLineMode && swGetEEOPPriority() == cOutputUtilityFrist && !swGetEEFeedPowerEn() && !subGetParaBatOpenSts())
	{
