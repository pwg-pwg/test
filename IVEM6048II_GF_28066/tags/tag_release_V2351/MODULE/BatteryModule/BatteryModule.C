...
//	INT16S wBmsUnderVolt = cBatVoltReal10V5*cBatSerialPcs;
...
//			}
		...
		else
		{
			...
			|| g_wBatChgStage == cBatChgFloatStage /*|| g_fBMSSOCFull*/ || g_wBatChgStage == cBatChgEqStage || !g_fBatVoltUnder)
			{
		...
		if(!g_fBatOpen)
		{
			...
			else
			{
				...
				|| g_wBatChgStage == cBatChgFloatStage || g_wBatChgStage == cBatChgEqStage || g_fBMSSOCFull)
				{
					s_uwBatVoltUnderChkCnt = 0;
					g_fBatVoltDanger = 0;
				}
...
//	   ||(g_uwWorkMode == cBatteryMode && (gi_wDCDCChgDischgEnDisable & cDCDCChgEnable))	 
	   ||(g_uwWorkMode == cBatteryMode && !g_uwSolarLoss)  //PV小功率充电DCDC电流不稳定，不能用于判断
