...
extern INT16S g_wRInvHWOverCurrPCnt;
...
{
	INT16S wShareCtrlEn = (gi_wParallelEnable && mChkPARRlyOn());
	...
	if(g_wLoopOutput > 0 || (g_wR_PWMDirectionPre == 0 && g_wLoopOutput == 0))    //正半周
	{
...
{
	g_wRSinAmp = 4977;//=110*1.414*32  =230*1.414*32	Q5
	g_wKpwm = 833;									//	Q7
	...
	{
//		g_wInvPWM = g_wLoopOutput + 75;//75;// + 450;
...
void sDCDCControl(INT16U *uwDCDCCtrlSts)
{
	...
	INT32S	dwDischgLimit = 160; //Reserve 2A to avoid detect error
//	INT32S	dwBatDishgLimit;
	...
	if(++g_wLoopCnt >= 8)
	{
		...
		{
			g_wBatVoltErr = g_wBatVoltAvg - g_wBatVoltRef;//电池参考电压建议软起
			if(g_wBatVoltErr > 10)
			{
				if(g_wBatVoltErr > 25)
				{
		...
		{
			g_wBusVoltErr = g_wPBusVoltErr;
			// ( BUS_V > (BUS给定+30V) 且 PV_V < (BUS给定+20V) ) 或 (BUS_V < (BUS给定-30V))  或 (BUS_V_Error < (-50V) 且 PV_V < 430 )  ==> 加大调整步径
			if(   ((g_uwPBusAvgVoltNew > (g_wDCDCBusVoltRef + cBusVoltReal30V)) && (g_uwSolarVolt1Avg < (g_wDCDCBusVoltRef + cBusVoltReal20V)))
			   || (g_wDCDCBusVoltRef > (g_uwPBusAvgVoltNew + cBusVoltReal30V)) //可能进入？？？
		...
		if(g_wPDCDCPWM < 0 && g_wBatVoltErr > g_wBusVoltErr)
		{
			g_wBusVoltErr = g_wBatVoltErr;
			    ...
			    && g_wPDCDCPWM > 0 && g_wBatVoltErr < g_wBusVoltErr )//自老化选用电池电压为BUS电压给定
		{
			g_wBusVoltErr = g_wBatVoltErr;
		}
		// 限幅误差在[-100,100]
		...
		g_dwPDCDCImo = _IQsat(g_dwPDCDCImo, cDCDCCntlCurrntLimit, -cDCDCCntlCurrntLimit);
	}
	//------ 电流内环 ------//
	g_wPDCDCIErr = (INT16S)(g_dwPDCDCImo + mDCDCCurrReal(gi_wPDCDCCurrSample * 8));//DISCHG- CHG+
	g_wPDCDCIErr = _IQsat(g_wPDCDCIErr, 80, -80);//对应为10A，处理系数乘以8
	...
	if(gi_wDCDCChgDischgEnDisable & cDCDCDischgEnable)
	{
		dwDischgLimit = 786432; //(INT32S)((INT32S)g_wPwmPeriod * 3) << 6;	//3072 = 4096*3 << 6		75%=4倍升压
	}
	else
	{
		dwDischgLimit = 0;
	...
	g_wPDCDCPWM = (INT16S)sdwDCDCCurrCntl(g_wPDCDCIErr, dwDischgLimit, dwChgLimit);
...
void sFBINVController(void)
{
	...
	{ 
	    wACVoltRealTemp = gi_wRGenVoltReal; 
	    uwACRms3timeAvgPeak = g_uwRGenRms3timeAvgPeak; 
	}
	else 
	{ 
	    wACVoltRealTemp = gi_wRLineVoltReal; 
	    uwACRms3timeAvgPeak = g_uwRLineRms3timeAvgPeak; 
    }
	
	g_wFBR_Duty = sdwInvCurrCntl(gi_wBusVoltReal, g_wInvBusVoltRef, wACVoltRealTemp,
								 uwACRms3timeAvgPeak, wInvCurr10, gi_wBatChgInvLimit,
								 gi_wInvChgCurrLimit, gi_wInvFeedCurrLimit);

	g_wFBR_Duty = (INT16S)(((INT32S)g_wFBR_Duty * g_wPwmPeriod) >> 12);		// 4096

	g_wFBR_Duty = _IQsat(g_wFBR_Duty, (PwmPeriodMaxtemp), -(PwmPeriodMaxtemp));
	wInvCurr10 = (INT16S)(((INT32S)wACVoltRealTemp * 205) >> 8); //204.8/256=0.8

	g_wFBPWM = g_wFBR_Duty - (INT16S)(((INT32S)wInvCurr10 * g_wPwmPeriod) / gi_wBusVolt8CAvg);//g_wInvBusVoltRef
	g_wFBPWM = _IQsat(g_wFBPWM, (PwmPeriodMaxtemp), -(PwmPeriodMaxtemp));
//++++++++++++++++++++++++++
// 死区补偿
if(mInvCurrReal(gi_wRInvCurrSample) > 0) { g_wFBPWM = g_wFBPWM - 40; }//g_wFBKcd_Y = 40时，过零处无明显突变
...
void sSetDCDCCtrlSts(INT16U uwDCDCCtrlSts)
{
	...
	else
	{
		...
		{
			g_wDCDCBusVoltRef = g_uwPBusAvgVoltNew;// + cBusVoltReal10V;有效时间点在哪？
		}
	}
...
INT16S	swGetInvCurrSaturaion(void)
{
	return g_wInvBusVoltRef;
}
...
//========================================================================Buckboost
INT16S	swGetDcdcVRef(void)//DCDC外环给定
{
	return (g_wDCDCBusVoltRef);
}

INT16S	swGetDcdcVFed(void)//DCDC外环反馈
{
	return (g_wPBusVoltAvg);
}

INT16S	swGetDcdcVError(void)//DCDC外环误差
{
	return (g_wBusVoltErr);
}
...
INT16S	swGetPDCDCImErr(void)//DCDC内环给定
{
	return (g_wPDCDCIErr);
}
...
INT16S	swGetUserData2(void)//电网dq锁相相位 96
{
	return (g_wRSinPointerwThetaTemp);
}
INT16S	swGetUserData3(void)//过零锁相相位 97
{
	return (g_wCtrlSinpointer);
}

INT16S	swGetUserData4(void)//DCDC的母线误差 98
{
	return (swGetDCDCCtrlInvBusVoltErr());
}
