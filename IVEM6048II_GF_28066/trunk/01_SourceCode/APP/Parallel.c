...
enum
{
	...
	cBatVoltWorkMode,
	cInvStatus,
	cLoadWattBatCur,//汇总成百分比的形式 curcur/cMaxCur
	cInvVoltLoadPercent,
	cChgStatus,
	cInvStatus2,
	cChgStatus2,
...
typedef union
{
	...
	struct
	{
		...
		INT16U  uwIslandProtectEn:1;
		INT16U  uwFeedPower:7;
		INT16U	uwReserved:5;
	}BIT;
}STRParameterSet7;
...
UNIInputStatus2 g_uniInputStatus2;
UNIInputStatus2 g_uniInputStatus2Temp;
...
typedef union
{
	...
	struct
	{
		INT16U wBatDisChgCur   :9;    //电池放电电流 0~511A
		INT16U wLoadWattPer       :7;    //有功负载百分比0~127%
	}BIT;
}STRInvDataInfo;
STRInvDataInfo g_strInvParaData;
STRInvDataInfo g_strLocalInvData;
...
extern INT8U  g_ubBatEqState;
extern INT32U g_dwSysBatDisChgCur;
extern INT32U g_dwSysLoadWatt;
extern INT16S g_wOPWattPercent;
...
extern INT16U sNewDate(INT8U ubCurYear, INT8U ubCurMonth, INT8U ubCurDay, INT8U ubDaysToAdd);
...
void sUpdateLocalData(void)
{
	...
	g_strLocalParameterSet7.BIT.uwFeedPowerEn = swGetEEFeedPowerEn();
	g_strLocalParameterSet7.BIT.uwIslandProtectEn = swGetEEIslandProtectEn();
	g_strLocalParameterSet7.BIT.uwFeedPower= swGetEEFeedPower()/100;
	...
	if((     (g_uwWorkMode == cLineMode || g_uwWorkMode == cBypassMode)
		  &&  mChkOPRlyOn()
		  && (   (g_LineModeJoinInWay == cLineModeJoinInByGrid && mChkGridRlyOn())
		       ||(g_LineModeJoinInWay == cLineModeJoinInByGen && mChkSmart_Port_Rly_On()))
		 )
	...
	if(g_wDCDCCurrAvg < 0)
	{
		g_strLocalInvData.BIT.wBatDisChgCur = (-g_wDCDCCurrAvg)/10;
	}
	else
	{
		g_strLocalInvData.BIT.wBatDisChgCur = 0;
	}
	if(g_wOPWattPercent > 127)
	{
		g_wOPWattPercent = 127;
	}
	g_strLocalInvData.BIT.wLoadWattPer = g_wOPWattPercent;
	...
	g_uwSystemData[g_uwPhysicalAddr][cLoadWattBatCur] = g_strLocalInvData.wStatus;
...
//						OSEventSend(cPrioSuper, eSuperToShutDown);
				...
				else if(g_uwCommand == cSourceStatus)
				{
					if(g_uwParaMode != cMasterMode)
					{
						...
						g_uniInputStatus2Temp.BIT.uwGenAbnormal = g_strParaExistInfo.BIT.uwGenAbnormal;
						
						if(g_uniInputStatus.BIT.uwLineAbnormal == false)
						{
						    if(g_uniInputStatusTemp.BIT.uwLineAbnormal == true)
						    {
						...
						{
							if(g_uniInputStatus2.BIT.uwGenAbnormal == false && g_uniInputStatus2Temp.BIT.uwGenAbnormal == true)
							{
								OSEventSend(cPrioSuper, eSuperGenLoss);
							}
						}
						g_uniInputStatus.uwInputStatus = g_uniInputStatusTemp.uwInputStatus;
						g_uniInputStatus2.uwInputStatus2 = g_uniInputStatus2Temp.uwInputStatus2;   
						/*
...
//	INT16S	wOutputWork = false;
	...
	g_uniInputStatus2.uwInputStatus2 = 0;
	...
	for(wCnt = 0; wCnt < g_uwParaNumMaxSize; wCnt++)
	{
		if(g_uwSlaveExsitFlg[wCnt] == 1)
		{
			...
			g_strInvParaData.wStatus = g_uwSystemData[wCnt][cLoadWattBatCur];
			...
			{
				g_uniInputStatus2.BIT.uwGenAbnormal = 2;
			}
			else if(g_strInvParaStatus2.BIT.wGenAbnormal == 1 && g_uniInputStatus2.BIT.uwGenAbnormal != 2)
			{
				g_uniInputStatus2.BIT.uwGenAbnormal = 1;
			}
			//---------------------------------------------------
			...
			*/
			...
			g_dwSysBatDisChgCur += g_strInvParaData.BIT.wBatDisChgCur * 10;
			g_dwSysBatDisChgCur += 5;
			g_dwSysLoadWatt += g_strInvParaData.BIT.wLoadWattPer;
			//g_dwSysLoadWatt = g_dwSysLoadWatt * cInvWattMax / 100;
...
//			}
	...
	g_dwSysBatDisChgCur = g_dwSysBatDisChgCur/g_uwExsitNum;
	g_dwSysLoadWatt = g_dwSysLoadWatt * cInvWattMax / 100;
	g_dwSysLoadWatt = g_dwSysLoadWatt/g_uwExsitNum;
...
//		sInverterModuleInitail();
	...
	//FeedPower
	if(g_strLocalParameterSet7Temp.BIT.uwFeedPowerEn != swGetEEFeedPowerEn())
	{
		sSetEEFeedPowerEn(g_strLocalParameterSet7Temp.BIT.uwFeedPowerEn);
		bSaveFlag = 1;
	}
	if(g_strLocalParameterSet7Temp.BIT.uwIslandProtectEn != swGetEEIslandProtectEn())
	{
		sSetEEIslandProtectEn(g_strLocalParameterSet7Temp.BIT.uwIslandProtectEn);
		bSaveFlag = 1;
	}
	if(g_strLocalParameterSet7Temp.BIT.uwFeedPower != swGetEEFeedPower()/100)
	{
		sSetEEFeedPower(g_strLocalParameterSet7Temp.BIT.uwFeedPower * 100);
		bSaveFlag = 1;
	}
	//
...
void sChgStatusChk(INT16U uwIndex)
{
	...
	else if(g_uniChgStatusTemp.BIT.uwChgMode == cBatChgEqStage
