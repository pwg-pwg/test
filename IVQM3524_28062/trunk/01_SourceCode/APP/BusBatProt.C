...
//				sMainTempForFanCtlChk();
				sLineVolDegJudg();
				sFanSpeedControl();//���Ȳ���ͳһ����
				sControlParameterSeting();//���ؽ�������
			}
			sDryRelayAction();
			sBattManagementTime();
...
//				g_wACFanOnFlag =1;
		...
		if((sbGetTxTempStatus() >= 1)||FanStatus.HalfLoad)// 交流风扇控制
		{
			g_wACFanOnFlag = 1;
		}
		else
	...
	{
		if(hoACFAN==0)// CC 20210621 控制交流风扇继电�?
		{
			sOSTimerStop();
...
void sLoadForFanCtlChk(void) 
{
	...
	{
		if(sbUnderLevelChk(wLoadPercentTemp, 70, 20,&bLightLoadChkCnt) == 1)	//4s
		{
	...
	{
		if(sbOverLevelChk(wLoadPercentTemp, 80, 5,&bLightLoadChkCnt) == 1)	//1s
		{
	...
	{
		if(sbUnderLevelChk(wLoadPercentTemp, 50, 20,&bFanHalfChkCnt) == 1)//4s
		{
	...
	{
		if(sbOverLevelChk(wLoadPercentTemp, 60, 5,&bFanHalfChkCnt) == 1)//1s   
		{
			FanStatus.HalfLoad = 1;
			FanStatus.FanOnLoad = 1;
		}
	...
	{
		if(sbUnderLevelChk(wLoadPercentTemp, 20, 20,&bFanLoadChkCnt) == 1)	//4s
		{
	...
	{
		if(sbOverLevelChk(wLoadPercentTemp, 30, 5,&bFanLoadChkCnt) == 1)	//1s   
		{
			FanStatus.FanOnLoad = 1;
		} 
	}
	...
	{
		if(sbUnderLevelChk(wTemp, 300, 200,&bFanDCIChkCnt) == 1)	//40s
		{
			FanStatus.DCIHigh = 0;
		}
	}
	...
	{
		if(sbOverLevelChk(wTemp, 350, 200,&bFanDCIChkCnt) == 1)	//40s   
		{
			FanStatus.DCIHigh = 1;
		}
	}
...
void 	sBatWeakChk(INT8U bFilter)
{
	...
	//if((!g_uwBatWeakFlg)&&(sbGetBatLow() == true))// 电池电压batlowback和batlow之前，一直报低压不切市电问题
	//{
	//	g_uwBatWeakFlg = 1;
	//}
	...
	{
		g_uwBatWeakTrigFlg = g_uwBatWeakFlg;
	}
	else
	{
...
void sLineModeInvOverCurProtCnt(void)
{
	...
	/*uwNewMode =bPVMode;
	if(uwPreMode!= uwNewMode)
	{
		if(uwNewMode ==cLineMode)
		{
			wSetInvOverCurrentLineModeCnt = 4;//刚切到市电模式的时候，周期改为4
			uwTranformCnt = 0;
		}
		uwPreMode = uwNewMode;
	}
	...
	{
		if(++uwTranformCnt >15) //delay 15Cycle
		{
...
void sACSPSControl()
{
		...
		&& (0 == sbGetRLineWaveLoss()))
	{
		...
		{
			if(++wACspsCtrlMSCnt>20)// 10S 市电正常延时10S才闭�?
			{
				wACspsCtrlMSCnt =0;
				bACSpsCtrlOffFlag =0;
				hoACSPS_ON;
		...
		if( ++wACspsCtrlSecCnt>620)//5 minute 以后断开继电�?
		{
			wACspsCtrlSecCnt =0;
			if(!hoACSPS)
			{
