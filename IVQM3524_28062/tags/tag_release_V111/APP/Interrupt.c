...
INT16U wInvOffPoint =0;//硬件过流保护关断时间
INT16U wInvOffFlg =0;
...
INT16S wInvOverCurrenProtectCnt =0;
INT16U ProtectFlag2 =0;//过流保护将驱动限小

//降载保护
INT16S g_wBatMinVoltDerate = cBat53v;//电池电压斜率降载最小电压
INT16S g_wBatMaxVoltDerate = cBat60v;//电池电压斜率降载最大电压
//斜率保护
INT16S g_wBatVoltSlopeCal = cBat53v;//电池电压斜率开始计算电压
INT16S swBatVoltSolpe[4] = {0};//4个点的斜率
INT16S swBatVoltSolpeCnt = 0;
INT16S swBatVoltSolpeSum = 0;//4个点的斜率和
INT8U  bBatVoltSolpeProtectFlag = 0;//斜率保护标志位
INT8U  bBatVoltSolpeChkCnt = 0;//斜率滤波数
INT16S swBatVoltQueueCnt = 0;//电池电压队列号
INT16S swCurrBatVoltSlope = 0;//当前电池电压斜率
INT16S swBatVoltQueue[20] = {0};//20次差不多1ms算一次斜率
//压差保护
INT8U  bBatAvg_RealVoltChkCnt = 0;//电池压差比较计数
INT8U  bBatAvg_RealVoltProtectFlag = 0;//压差保护标志位
...
Funciont:
************************************************************************************/
interrupt void HardOverCurrInterrupt(void)
{
...
//	wBusVoltReal = (INT16S)(((INT32S)wBusVoltSample * wBusVoltRatio)>>11);//0.1V
	...
	if((swBatVoltSolpeSum > cBatVoltSolpeSumHighLever) && (wBatVoltReal  > g_wBatVoltSlopeCal))//斜率保护
	{
	...
	if(wBatVoltReal > wBatAvgVoltNew+cBat1v5)//压差保护
	{
...
//	wCtrlSinpointer	= 						0-384			  * 384(最大点数)			  / 动态调节过程，保证输出频率
...
//	wBatTempSample   = 4096;
	...
	else if(bPVMode == cLineMode )
	{
		...
		if(wBatVoltReal >(mEEBatVoltOverShut()-swGetBatteryPcs()*cBat0v7))//60.2V
		{
			ubBatVoltOveCnt ++;
			if(wChgPwmLimit > 5)
			{
	...
	if(bPVMode == cBatteryMode )
	{
		if(LiHardProtectCNT>=3)//硬件过流保护 
		{
			if((wInvOffPoint ==0)&&(wContinueTrig==0))
			{
				...
				//wInvOffPoint =128;
				wInvOffPoint =192;//硬件过流保护关断192个点
				wContinueTrig =1;
				if(ubHardProtectCnt >= 100)//触发很多次才能进行报故障     
				{
					ubHardProtectCnt = 0;
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>20))
			{
				wContinueTrigCNT =20;
				wContinueTrig =0;
			}
		...
		if(wInvOffPoint>0)
		{
			...
			if((wCtrlSinpointer == 0)||(wCtrlSinpointer == gi_wPLLPointThreeSix)\
				||(wCtrlSinpointer == (gi_wPLLPointThreeSix+1)))//硬件触发过流后保护到过零点10个点
			{
				wInvOffPoint = 10;
			}
		}
...
//		}
/***************************************************************************************
//参考电压AD值计算电压：wInvVoltReal = (INT16S)(((INT32S)wInvVoltSample1 * wInvVoltRatio)>>11);//其中wInvVoltRatio = 4133，逆变电压采样变比
//wRInvVoltCntl = (INT16S)(((INT32S)wInvVoltSample1  * 206) >> 5);//206 = (4133/10)/2048*32*32
****************************************************************************************/
		wRInvVoltCntl = (INT16S)(((INT32S)wInvVoltSample1  * 206) >> 5);//206
	...
	if(bPVMode == cBatteryMode )
	{
		if(LiHardProtectCNT>=3)//硬件过流保护 
		{
			if((wInvOffPoint ==0)&&(wContinueTrig==0))
			{
				...
				ubHardProtectFlag =true;
				wInvOffPoint =192;//硬件过流保护，关断192个点
				wContinueTrig =1;
				ProtectFlag2 =1;
//				wInvOffFlg =1;
//				EALLOW;
...
//				EDIS;
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>30))
			{
				wContinueTrigCNT =30;
				wContinueTrig =0;
			}
...
//			mDisInvPwmOut();
	...
	if((bPVMode == cBatteryMode)||((bPVMode == cStandbyMode )&&(wFBControlStatus == cFBInverterControl)))
	{
		...
		if((abs(wInvVoltReal)  >= gi_wInvOverValue)&&(bBatToLineModeflag ==false))        //254.6*1.414=360v    360/8=45V   //amy20121214     
		{
...
////INT16S swGetInvVolt2SumCnt(void)
////{
////	INT16S swInvVolt2SumCntTemp = 0;
////	swInvVolt2SumCntTemp = wInvVolt2SumCnt ;
////	return(swInvVolt2SumCntTemp);	
////}
////
////INT16S swGetInvVolt2Sum(void)
////{
////	INT16S wInvVolt2SumTemp = 0;
////	if(wInvVolt2SumCnt <= 1)
////	{
////		wInvVolt2SumTemp = (INT16S)((INT32S)dwInvVolt2Sum/1000);
////	}
////	else
////	{
////		wInvVolt2SumTemp = (INT16S)((INT32S)dwInvVolt2Sum/100000);
////	}
////	return(wInvVolt2SumTemp);	
////}
////
////INT16S swGetInvVolt2SumTempCnt(void)
////{
////	INT16S swInvVolt2SumCntTemp = 0;
////	swInvVolt2SumCntTemp = wInvVolt2SumCntTemp ;
////	return(swInvVolt2SumCntTemp);	
////}
////
////INT16S swGetInvVolt2SumTemp(void)
////{
////	INT16S wInvVolt2SumTemp = 0;
////	if(wInvVolt2SumCntTemp <= 1)
////	{
////		wInvVolt2SumTemp = (INT16S)((INT32S)dwInvVolt2SumTemp/wInvVolt2SumCntTemp);
////	}
////	else
////	{
////		wInvVolt2SumTemp = (INT16S)((INT32S)dwInvVolt2SumTemp/wInvVolt2SumCntTemp);
...
* Description	 : 电池电压斜率计算；swBatVoltSolpeSum：计算出4个采样点的斜率总和
* Input 		 : 
					INT16S swBatVoltTemp:电池电压值
					INT16S swBatVoltQueueCntTemp：当前电池电压队列号
* Output		 : None
* Date			 : 2021.02.28
* Author		 : CM
********************************************************************************/
void swBatVoltSlopeCal(void)
{
	...
	{
		swCurrBatVoltSlope = swBatVoltQueue[swBatVoltQueueCnt]-swBatVoltQueue[swBatVoltQueueCnt+1];//间隔20次的斜率
	}

	if(swBatVoltSolpeStartFlag < 20)//去掉第一组数据，第一组数据无效
	{
