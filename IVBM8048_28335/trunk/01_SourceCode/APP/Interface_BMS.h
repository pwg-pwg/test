...
typedef struct
{
	INT16S wDeviceType; //电池类型
	INT16S wDeviceSubType;//子类型
	INT16S wProtocolVer;//协议版本
	INT16S wComInfo;// 端口信息
	INT16S wSerialNum1;
	INT16S wSerialNum2;
	INT16S wSerialNum3;
	INT16S wSerialNum4;
...
typedef struct
{
	...
	INT8U  ubBMSConnect; // 有电池链接, 只要有一块, 这个就为真
...
typedef struct
{
	...
	INT16U fBMSCmdIndex:3; // 这个应该是询问自己的位置
	INT16U fBMSComTimeoutCnt:3;
	INT16U fBMSComRecieve:1; //  是否接收到应答信息
	INT16U fBMSSettingChg:1;
	INT16U fBMSFwVerErr:1; // 版本错误
	INT16U fReserved:5;
}STRBMSComSts;
