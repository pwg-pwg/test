...
const struct  EepromStruct2	cEepromDaultTable2 = 	
{
    ...
    0,                    //        INT16U    wEESmartPortType; -- 0 = GEN, 1 = GRID
    8000,                 //        INT16U    wEEGenPowerMax;
...
//	if(swGetEEOutputVolt() != 2300)	
    ...
    // R_Gen_Max_Power
    if(swGetEEGenPowerMax() < 0 || swGetEEGenPowerMax() > 12000) // 0~12000
    {
        sSetEEGenPowerMax(8000);
        bUserChangeFlag = 1;
    }
    // R_Gen_Charge_Enable
...
void sSetEEOPPriority(INT16S wValue)
{
	...
	if(wValue == 1){
		uEepromCfg2.EepromStructCfg2.wEEOPPriority = 2;
	}
	OS_EXIT_CRITICAL();
}
