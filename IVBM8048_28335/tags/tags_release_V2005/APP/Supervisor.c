		...
		//					return(swInvVoltSoft());
					...
					// else
					{
						...
						if(   !uwForceGridSoftBusFlag && ((g_uwBatVoltAvg <= 460) || (uwBusSotfOverDelay < 1000)) //subGetBatWeakSts() 
						   && ((g_LineModeJoinInWay == cLineModeJoinInByGrid && g_uwRLineRms3timeAvg >= 1800 )||(g_LineModeJoinInWay == cLineModeJoinInByGen && g_uwRGenRms3timeAvg >= 1800 )))
						{
			...
			else//电池没有问题
			{
				...
				else// 这里 先启动 llc 然后跳转到上面
				{
					...
					else // 先启动llc ,然后启动 dcdc
					{
						...
						// if(g_uwPBusAvgVoltNew >= uwBatBusVolt)// bus 线电压软启动后 启动llc	//	删除BUS软启
						{
							// hoBUSSOFTOff;
							uwDCDCBoostDelay = 150;//3050;
							uwBUSSPSSoftOK = true;
							// if((g_uwConvertVoltAvg + cBusVoltReal30V) > g_uwPBusAvgVoltNew
							// || ((g_uwConvertVoltAvg + cBusVoltReal30V) > (g_uwBatVoltAvg * cTransformerRatio)))	//	删除BUS软启
							{
								sSetDCDCCtrlSts(cDCDCWait);	//Turn off Buck Soft
								mDCDCLLCOn();
								if(uwBusSotfOverDelay < 500)//3500)
