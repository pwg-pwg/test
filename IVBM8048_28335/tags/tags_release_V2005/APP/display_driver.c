...
//Dispaly buffer
VIEWBUF	ViewBuf[VRAMSIZE];//所有的元素控制都放在这个位置， 用来控制元素亮与不亮
VIEWBUF View1SFlash;
...
//description: update display VRAM  更新元素，屏幕是定制屏幕
//=================================================================================
...
//用于延迟 cFLASH_TIME_1S 如果使用这个参数， 那么下一个可能无法更新
//=================================================================================
...
//    }
	// 在显示开始的时候,   ViewBuf[bitAddr].BIT.val 会全部设置为0
	if(bitAddr < VRAMSIZE)
	{
		if(Time == cFLASH_TIME_1S)//应该是用于闪烁
		{
			View1SFlash.BIT.val = 1;//下一个执行的sSendBitToVram  时候， View1SFlash.BIT.timeout != 0 \
			下个图标会不变 Time == cFLASH_TIME_1S && View1SFlash.BIT.timeout == 0 条件不会满足，则不会动
		}
		...
		else if((Time == cFLASH_TIME_1S && View1SFlash.BIT.timeout == 0) || Time == 0) //如何设置 闪烁, 则不会更新
		{
			ViewBuf[bitAddr].BIT.val = bVal;
			ViewBuf[bitAddr].BIT.timeout = Time;			
		}
