		...
		if(g_wMPPTEn)
		{
			if(--s_uwMPPT1DelayCnt == 0) // 50 * 20ms 执行一次
			{
				...
				if(g_wMPPTCntlVolt == 0)//0:Auto // 模式在自动模式
				{
					...
					if(g_uwSolarCurr1Avg < 50) // 太阳能电流<0.5A的话
					{
						//OS_ENTER_CRITICAL();
...
INT16S swMPPTTrack(INT16U uwSolarIndex,  INT16U uwSolarVolt, INT16U uwBusLimitPoint)
{
	...
	// if(uwSolarIndex == 0)
	// {
	// 	if(g_uwSolarCurr1Avg > 1800)
	// 	{
	// 		wMPPTTemp = (cSolarCurrMax - g_uwSolarCurr1Avg)/10;
	// 		g_wBoostMPPTSetp[uwSolarIndex] -= wMPPTTemp;
	// 		if(g_wBoostMPPTSetp[uwSolarIndex] > 50)
	// 		{
	// 			g_wBoostMPPTSetp[uwSolarIndex] = 50;
	// 		}
	// 		else if(g_wBoostMPPTSetp[uwSolarIndex] < 20)
	// 		{
	// 			g_wBoostMPPTSetp[uwSolarIndex] = 20;
	...
	// if(uwSolarIndex == 1)
	// {
	// 	if(g_uwSolarCurr2Avg > (cSolarCurrMax - 200))
	// 	{
	// 		wMPPTTemp = (cSolarCurrMax - g_uwSolarCurr2Avg)/10;
	// 		g_wBoostMPPTSetp[uwSolarIndex] -= wMPPTTemp;
	// 		if(g_wBoostMPPTSetp[uwSolarIndex] > 50)
	// 		{
	// 			g_wBoostMPPTSetp[uwSolarIndex] = 50;
	...
	// g_wBoostMPPTSetp[uwSolarIndex] = 10;
...
/*****************PID算法**********************/
	...
	/*******PV控制使用均流加权处理******/
	g_wR_Is_P1 = mPVCurrReal(gi_uwSolarCurr1SampleTemp);
...
void sBoost2Control(INT16S wBusVoltRef)
{
	...
	if(g_wImo_P2 > (g_wPv2CurrLimit - gPVCurrOverCnt))   g_wImo_P2 = (g_wPv2CurrLimit - gPVCurrOverCnt);
	//if(g_wImo_P1 < -80)		g_wImo_P1 = -80;
	if(g_wImo_P2 < wPvCurrLowLimit) 	  g_wImo_P2 = wPvCurrLowLimit;

	/*******PV控制使用均流加权处理******/
	g_wR_Is_P2 = mPVCurrReal(gi_uwSolarCurr2SampleTemp);
	...
	if(suwGetBoost1CtrlSts() >= cBoostMppt)
	{
		...
		)//多1A且PWM达到极限
	    {
		    if(++s_uwSolar1PowerAbnormalChkCnt > 250)//5s
		    {
				...
				{
					// TestMode |= 0x01;
					// TestMode1 = g_uwSolarCurr1Avg;
					// TestMode2 = g_uwSolarPower1Avg;
