...
FaultSaved uFault_log[cFault_log_num];


INT16U 	g_uwEepromWRFlg = 0;
INT16U 	g_uwEepromRDFlg = 0;
INT16U	uFault_Cnt = 0;
INT16U	uFaultSaveFlag=0;
/******************************************************************************
...
const struct  EepromStruct2	cEepromDaultTable2 = 	
{
	...
	cBatVoltReal14V1*cBatSerialPcs,//		INT16U  wEEBatCVVolt; //56.4V均冲点
	cBatVoltReal13V5*cBatSerialPcs,//		INT16U  wEEBatFloatVolt;	//54V浮充点
	...
	0, 			        //		INT16S	wEESolarPowerBalanceEn;PV平  
	0, 			        //		INT16U	wEETimingOP2StartTime;
	0, 			        //		INT16U	wEETimingOP2EndTime;
	0xFFFF,             //		INT16U	wEEDurationTime_OP2;
	420, 			    //		INT16U	wEEThresholdVoltMin_OP2;
	250, 			    //		INT16U	wEEThresholdSOCMin_OP2;
	0,					//		INT16U	wEEBatUnderSoc;
	10, 				//		INT16U	wEEBatUnderBackSoc;
	10, 				//		INT16U	wEEBatWeakSoc;
	30,					//		INT16U	wEEBatWeakBackSoc;
    1,                    //        INT16U    wEESmartPortType; -- 0 = GEN, 1 = GRID
    80,                 //        INT16U    wEEGenPowerMax;
    0,                    //        INT16U    wEEGenChargeEn;
    8000,                 //        INT16U    wEEOutPowerMax_OP2;
	// 2048,                 //        INT16U    wEEDSPDcdcCurrAdj;
	// 2048,                 //        INT16U    wEEDSPDcdcCurrAvgAdj;
	0,						//        INT16U    wEEPowerConvertionType;
	1,						//        INT16S    wEEBMSDisComWork;
	0xAAAA,
	0
};
...
void sEEDefaultWrite4(INT8U *pDefaultTable, INT8U bIndex, INT8U bLen)
{
	INT8U	i=0;
	INT8U 	*bTemp;
...
void sReadEeprom1(void)			
{
	...
	sEEDefaultWrite1(pDefaultTable,bEepromDefaultIndex,(cEepromTotalLength1/2 - 1));	//读EEPROM数据到EE1
...
void sReadEeprom4(void)			
{
	...
	pDefaultTable = (INT8U *)&cEepromDaultTable3;
	sEEDefaultWrite4(pDefaultTable,bEepromDefaultIndex,(cEepromTotalLength3/2 - 1));
}

void sReadFaultLogSave(void)			
{
	// INT8U	bEepromDefaultIndex = 0;
	// INT8U   *pDefaultTable;
	// INT8U	bCnt = 3;
...
void sFaultLogSave(void)
{
	...
	bTemp = (INT8U*)uFault_log[0].dwfault_code;
	

	do
	{
		if(cEepromSuccess == sEepromWrite(cEepromFaultAdderss + cFault_log_len * i ,cFault_log_len ,(INT8U *)uFault_log[i].dwfault_code))
		{
			i++;
			bCnt = 3;
			// if(sEepromRead(cEepromFaultAdderss + cFault_log_len*i-1,2,(INT8U *)&wEepromChkSumTemp) == cEepromSuccess
			//     && uEepromCfg4.EepromStructCfg3.wEECheckSum3 == wEepromChkSumTemp)
			// {
			// 	g_uwEepromWRFlg |= 0x80;
			// }
...
//	if(swGetEEOutputVolt() != 2300)	
	...
	{
		sSetEEBatWeakBackVolt(cBatVoltReal13V5*cBatSerialPcs);//54V
		bUserChangeFlag = 1;
	}
	...
	{
		sSetEEBatUnderVolt(cBatVoltReal10V5*cBatSerialPcs);//42V
		bUserChangeFlag = 1;
	}
	...
	if(swGetEEModbusAddr() < cMODBUS_ADDR_MIN || swGetEEModbusAddr() > /*cMODBUS_ADDR_MIN*/cMODBUS_ADDR_MAX)
	{
	...
	{
		sSetEEThresholdVoltMin_OP2(420);
		bUserChangeFlag = 1;
	}
	//wEEThresholdSOCMin_OP2
	if(swGetEEThresholdSOCMin_OP2() > 950)
	{
		sSetEEThresholdSOCMin_OP2(250);
		bUserChangeFlag = 1;
	}
    //----------------------------------
    // R_Gen_Volt
	...
	// DCDC_Curr
    if(swGetEEDSPRDCDCCurrAdj() > 2548 || swGetEEDSPRDCDCCurrAdj() < 1548)
    {
        sSetEEDSPRDCDCCurrAdj(2048);
        bFactoryChangFlag = 1;
    }
	// DCDC_Curr_Avg
    if(swGetEEDSPRDCDCCurrAvgAdj() > 2548 || swGetEEDSPRDCDCCurrAvgAdj() < 1548)
    {
        sSetEEDSPRDCDCCurrAvgAdj(2048);
        bFactoryChangFlag = 1;
    }
    //----------------------------------
    // Smart_port_type
    if(swGetEEGridGenType() < 0 || swGetEEGridGenType() > 1)
    {
        sSetEEGridGenType(1);
        bUserChangeFlag = 1;
    }
    // R_Gen_Max_Power
    if(swGetEEGenPowerMax() < 5 || swGetEEGenPowerMax() > cGen_PowerMaxSet) // 500~50000 W
    {
        sSetEEGenPowerMax(80);
        bUserChangeFlag = 1;
    }
    // R_Gen_Charge_Enable
    ...
    // Smart_port_Out_Power_Max_OP2
    if(swGetEEOutPowerMax_OP2() < 0 || swGetEEOutPowerMax_OP2() > 12000) // 0~12000
    {
        sSetEEOutPowerMax_OP2(8000);
        bUserChangeFlag = 1;
    }
	if(swGetPowerConvertionType() < 0 || swGetPowerConvertionType() > 1)
    {
        sSetPowerConvertionType(0);
        bUserChangeFlag = 1;
    }
	if(swGetBMSConnectEN() < 0 || swGetBMSConnectEN() > 1)
    {
        sSetBMSConnectEN(1);
        bUserChangeFlag = 1;
    }
...
//--------------------------------------------------
void sSetEEDSPRDCDCCurrAdj(INT16S wValue)
{
    OS_ENTER_CRITICAL();
    // uEepromCfg2.EepromStructCfg2.wEEDSPDcdcCurrAdj = wValue;
    OS_EXIT_CRITICAL();
}
INT16S swGetEEDSPRDCDCCurrAdj(void)
{
    // return(uEepromCfg2.EepromStructCfg2.wEEDSPDcdcCurrAdj);
}

//--------------------------------------------------
void sSetEEDSPRDCDCCurrAvgAdj(INT16S wValue)
{
    OS_ENTER_CRITICAL();
    // uEepromCfg2.EepromStructCfg2.wEEDSPDcdcCurrAvgAdj = wValue;
    OS_EXIT_CRITICAL();
}
INT16S swGetEEDSPRDCDCCurrAvgAdj(void)
{
    // return(uEepromCfg2.EepromStructCfg2.wEEDSPDcdcCurrAvgAdj);
}

//--------------------------------------------------
void sSetEEGridGenType(INT16S wValue)
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.wEEGridGenType = wValue;
    OS_EXIT_CRITICAL();
}
INT16S swGetEEGridGenType(void)
{
    return(uEepromCfg2.EepromStructCfg2.wEEGridGenType);
}
...
//--------------------------------------------------
void sSetEEOutPowerMax_OP2(INT16S wValue)
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.wEEOutPowerMax_OP2 = wValue;
    OS_EXIT_CRITICAL();
}
INT16S swGetEEOutPowerMax_OP2(void)
{
    return(uEepromCfg2.EepromStructCfg2.wEEOutPowerMax_OP2);
}

//--------------------------------------------------
void sSetPowerConvertionType(INT16U wValue)
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.wEEPowerConvertionType = wValue;
    OS_EXIT_CRITICAL();
}
INT16U swGetPowerConvertionType(void)
{
    return(uEepromCfg2.EepromStructCfg2.wEEPowerConvertionType);
}

//--------------------------------------------------
void sSetBMSConnectEN(INT16U wValue)
{
    OS_ENTER_CRITICAL();
    uEepromCfg2.EepromStructCfg2.wEEBMSDisComWork = wValue;
    OS_EXIT_CRITICAL();
}
INT16U swGetBMSConnectEN(void)
{
    return(uEepromCfg2.EepromStructCfg2.wEEBMSDisComWork);
}
//--------------------------------------------------
...
{
    // if(swGetEESmartPortType() == cGenPort && mChkGENRLY_On() == true) 
	if(mChkGENRLY_On() == true)
    { return true;  }
    else
    { return false; }
}
...
// {
    // if(swGetEESmartPortType() == cOutPort && mChkGENRLY_On() == true) 
    // { return true;  }
    // else
    // { return false; }
// }
...
//  故障录波
void sWriteFaultLog(INT16U wfault_code,INT16U wfault_value)
{
	INT16U wfault_times = 0;


    if (cFault_log_cnt > cFault_log_num) {
        cFault_log_cnt = 1;
    }
	wfault_times = g_strDateTime.ubMonth;
	wfault_times = (wfault_times << 8) + g_strDateTime.ubDay;

	uFault_log[cFault_log_cnt].dwfault_code = wfault_code;
	uFault_log[cFault_log_cnt].dwfault_value = wfault_value;
	uFault_log[cFault_log_cnt].dwfault_times = wfault_times;
