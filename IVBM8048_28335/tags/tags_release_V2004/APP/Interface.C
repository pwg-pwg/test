...
INT8U	g_ubCommandLength[MAX_SCI_NO];// 长度
INT8U	*g_pubCommandIn[MAX_SCI_NO];
INT8U 	g_ubSciIdleCnt[MAX_SCI_NO] = {0};	// 空闲计数,
...
INT16S g_wOPRMSRatedVolt = 2300;//输出目标电压  220V  230v  240v   0.1V 给用户输出的有效电压
INT16S g_wOPSinVpp = 10407;//峰峰值 (INT16S)((((INT32S)g_uwInvRMSCtrlVolt * 181) >> 2) / 10); 2300 *181 /4/10 =10407
...
extern INT16S wP_KVinComp;
extern INT16S wKcd_1;
extern INT16S wKcd_2;
extern INT16S wKcn_0;
extern INT16S wKcn_1;
extern INT16S wKcn_2;

extern INT16U uFaultSaveFlag;

INT8U  g_ubRTCNeedReadFlg = true;// 是否重新读时间, 看看保存的时间对不对,   在人为操作,设置时间后, 发生
INT8U  g_ubReadTimeOKFlg = false; // 时间到了, 保存数据
INT8U  g_ubRTCOKFlg = false;// rtc 正常
...
const pFunc1	GetDataSubArray[100]=
{
	...
	swGetGenVoltSample,		swGetGenCurrSample,			swGetInvTempSample,						//15-17
	...
	swGetRInvDCVoltReal,	swGetRGenVoltReal,			swGetRGenCurrReal,							//42-44
	...
	//PV Loop
	(pFunc1)suwGetBoost1CtrlSts,swGetVerr_P1,           swGetVm_P1,          //51-53PV1状态            外环误差         外环输出
	swGetPV1CurrLimit,     swGetImo_P1,				    swGetR_Is_P1,        //54-56PV1外环输出限幅		 PV1内环给定      PV1内环采样
	swGetBoost1_PWM,	   (pFunc1)suwGetBoost2CtrlSts, swGetVerr_P2,        //57-59
	swGetVm_P2,            swGetPV2CurrLimit,           swGetImo_P2,         //60-62
	swGetR_Is_P2,          swGetBoost2_PWM,             swReturnNull,        //63-65	
	//OnGrid  
	swGetInvBusVoltRef,    swGetInvBusVoltfdb,          swGetFBVm_P,     //66-68并网母线电压给定,外环的输出,基于并网母线外环输出得到的电池控母线电压误差
	swGetBusVoltErr,       swGetRFBVm_P,                swGetFBImo_P,    //69-71 并网母线外环的最终输出，带R,内环电流给定,内环采样
    swGetFBR_Is_P,         swGetRFBR_Is_P,              swGetRFBPWM,     //72-74内环误差,内环输出经过调制之后的PWM，带R
	swGetFBPWM,            swReturnNull,				swReturnNull,    //75-77
	//OffGrid
	swGetRSinPointer,      swGetRLineVoltTest,          swGetRInvDCVoltCtrl,//78-80原始的PLL角度,离网的相位,电压直流分量输出
	swGetVref,             swGetRInvVoltCntl,           swGetInvVoltError,  //81-83离网逆变电压环给定//离网逆变电压环采样  真实值*10*32//离网逆变电压环误差
    swGetRLoadCurrCntl,	   swGetVBeforeSaturation,      swGetRInvCurrCntl,  //84-86离网重复控制外环输出会叠加这个//离网重复控制外环输出//离网内环采样
	swGetVoltLimit,        swGetKpwm,                   swGetLoopOutput,    //87-89离网内环输出//离网内环最终输出 							
	//DCDC	
	swGetBusVoltErr,	   swGetBatVoltErr, 			swGetInvStep,	 //90-92
	swGetBusVmo,           swGetPDCDCImo,               swGetPDCDCPWM,       //93-95 DCDC外环输出  //DCDC内环给定  //DCDC内环的输出														
	// (pFunc1)swGetSinePeriodCntNew,(pFunc1)swGetLinePeriodCntNew,(pFunc1)swGetGenPeriodCntNew,//96-98																						  
	
	// swReturnNull															 //99
	swGet1,swGet2,swGet3,swGet4,
};
...
extern void    DelayUs(volatile unsigned int);
...
void sSciTask(void)
{
	...
	DelayUs(12000);//20ms
	//g_ubRTCOKFlg = subRTCSoftwareReset();	// SCI  软复位
	...
	//sReadEeprom4();
	sReadFaultLogSave();
	sEEpromDataRangeChk();// eerpom 的数据有错误的话,自动回正

	g_wOPRMSRatedVolt = swGetEEOutputVolt();
	g_wOPSinVpp = (INT16S)((((INT32S)g_wOPRMSRatedVolt * 181) >> 2) / 10);// sin波 控制电池

	g_uwLoadOnSts = 0;//swGetEEStartUpKeyOn();

	gi_wParallelEnable = swGetEEParallelEn();

	if(swGetEEOutputFreq() == 5000)// 输出频率是50hz
	{
		sInitial50HzPara();// 初始化50hz的控制参数
	}
	else
	{
		sInitial60HzPara();// 初始化60hz的控制参数
	}

	sLineModuleInit();//  市电输入的参数初始化
	sGenModuleInit();
	sBatteryModuleInit();//关于电池参数的初始化
	sInverterModuleInitail();//逆变的相关参数

	sCanEventEnable();//使能can接收
	sBMSParaInit();//初始bms 电池的数据
	while(1)
	{
		event=OSEventPend();
		if(event&(1<<eInterfaceTimer))
		{
			...
			sSciRxDebugCommand(1);	//for RS232	//mosbus协议 SCIB串口
			sRS485RxDebugCommand(2);	//for RS485// 和电池通信
			sBMSCommandProc(2,g_ubUserDataBuf0[2],40);	//2s		50ms/1// 解析关于电池通信的部分
			//hoLED4ON^=1;
			// rtc 正常
...
{
	INT16U uwSciTemp;
	sSCIProtection(sciid,&g_uwSciBusyCnt[sciid],&g_uwSciErrorCnt[sciid]);//串口保护
	while(1)
	{
		uwSciTemp =sSciRead(sciid,g_pubCommandIn[sciid]);//先读取串口 接收队列是否为空
		if(uwSciTemp ==cSciRxBufEmpty)
		{
			if(g_ubSciIdleCnt[sciid] >= 2)//存在空闲,则归零
			{
...
//		sSciWrite(sciid,g_ubUserDataBuf0[sciid],5);
		...
		g_ubSciIdleCnt[sciid] = 0;
		// TestMode = g_ubCommandLength[sciid];
		// TestMode1 = g_ubCommandBuffer[sciid][0];
		// TestMode2 = swGetEEModbusAddr();
		if(g_ubCommandLength[sciid] >= (MAX_SCI_RX_BUF_SIZE - 1))// 如果一次性接收数据过大, 则丢弃这条命令
		{
			g_pubCommandIn[sciid]=g_ubCommandBuffer[sciid];
			g_ubCommandLength[sciid]=0;
		}
		else if((g_ubCommandBuffer[sciid][0] == swGetEEModbusAddr())// 一个设置好的地址
		|| (g_ubCommandBuffer[sciid][0] == cMODBUS_ADDR_MAX))	// ModBus的最大地址, 也是默认地址,可以叫这个为广播地址吧
		{
		...
		else if(((*g_pubCommandIn[sciid])==0x0D||(*g_pubCommandIn[sciid])==0x0A))//0D 或者 0A 是调试命令的开头
		{
			sParsing(sciid);	
...
/// @brief  rs485解析    这里用来解析电池数据  modbus协议
/// @param sciid
void sRS485RxDebugCommand(INT8U sciid)
{
	INT16U uwSciTemp;
	...
	while(1)
	{
		uwSciTemp =sSciRead(sciid,g_pubCommandIn[sciid]);// 先从数据缓冲队列中读取一个字节
		if(uwSciTemp ==cSciRxBufEmpty)
		{
			if(g_ubSciIdleCnt[sciid] >= 2) //空闲计时 大于2, 很久没有收到数据, 重新解析modbus协议
			{
				g_ubSciIdleCnt[sciid] = 0;
				g_pubCommandIn[sciid] = g_ubCommandBuffer[sciid];
				g_ubCommandLength[sciid] = 0;
	...
	if(g_ubSciIdleCnt[sciid] >= 2 && g_ubCommandLength[sciid] > 0)//  并且有长度,则解析
	{
...
INT16S Test_OtherTjDeratePerTemp;// dcdc  pv inv  输出功率的百分比
INT16S Test_LLCTjDeratePerTemp;// llc 输出百分比
INT16S Test_LLC_TXDeratePerTemp;
INT16U Test_g_uwFaultSolarCurr1Avg;
INT16S Test_swOtherMaxTjSum;//  g_swBUCK_IGBTTj  g_swINV_IGBTTj  g_swPV_BOOST_DiodeTj 的最大值(推算温度最大值)的平均值
INT16S Test_swLLC_MosTjSum;
INT16S Test_swInnelTempSum;
INT16S Test_swPV_BOOST_DiodeMaxTjSum;// pv输出 的百分比
...
//	wSciLength += 5;
	...
	g_ubUserDataBuf0[sciid][wSciLength++] = '0' + hoGENRLY;					//11
	g_ubUserDataBuf0[sciid][wSciLength++] = '0' + 0;//hoGRIDSCR;					//10
...
void	sQ3Command(INT8U sciid)
{
	...
	OS_ENTER_CRITICAL();
	wSaveDataCnt=0;
	wSnatchDataCnt=wSnatchDataCntTemp;	//点数
	wInterval1=wInterval=wIntervalTemp;	//间隔点数
	wDataKind[0]=wDataKindTemp1;	//CH1
	wDataKind[1]=wDataKindTemp2;	//CH2
	wDataKind[2]=wDataKindTemp3;	//CH3
	wDataKind[3]=wDataKindTemp4;	//CH4
...
INT8U sSnatchGraph(void)
{
	...
	if(wGraphWaitFaultFlag == 1)
	{
		// if(wTempCnt < 500)
		// {
		// 	wGraphDataBuff[0][wTempCnt] = (INT8U) swGet1();
		// 	wGraphDataBuff[1][wTempCnt] = (INT8U) swGet2();
		// 	wGraphDataBuff[2][wTempCnt] = (INT8U) swGet3();
		// 	wGraphDataBuff[3][wTempCnt] = (INT8U) swGet4();
		// 	wTempCnt++;
		// }
		if(wTempCnt < 2000)
		{
			wGraphDataBuff[0][wTempCnt>>2] = (INT8U) swGet1();
			wGraphDataBuff[1][wTempCnt>>2] = (INT8U) swGet2();
			wGraphDataBuff[2][wTempCnt>>2] = (INT8U) swGet3();
			wGraphDataBuff[3][wTempCnt>>2] = (INT8U) swGet4();
			wTempCnt++;
		}
	}
	else
...
void sTestCommand(INT8U sciid)
{
	...
	if((g_ubCommandBuffer[sciid][1] == 'S') && (g_ubCommandBuffer[sciid][2] == 'T'))
	{
			...
			&& (g_ubCommandBuffer[sciid][7] == 'V'))//BATKV
		{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'I') && (g_ubCommandBuffer[sciid][6] == 'K')
			&& (g_ubCommandBuffer[sciid][7] == 'P'))//DCIKP
		{
			if(g_ubCommandBuffer[sciid][8] == '?')
			{
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(g_wDCDCIKp, 2, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][5] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],6); 		
			}
			else
			...
			&& (g_ubCommandBuffer[sciid][5] == 'I') && (g_ubCommandBuffer[sciid][6] == 'K')
			&& (g_ubCommandBuffer[sciid][7] == 'I'))//DCIKI
		{
			if(g_ubCommandBuffer[sciid][8] == '?')
			{
				g_ubUserDataBuf0[sciid][0] = '(';
			...
			&& (g_ubCommandBuffer[sciid][7] == 'R') && (g_ubCommandBuffer[sciid][8] == 'D'))//TSTLLCPRD
		{
			if(g_ubCommandBuffer[sciid][9] == '?')
			{
				g_uwDCDCConvFrq = (INT16S)((INT32S)cSystemBusFreq * 5000 / g_uwDCDCConvPrd);
				g_ubUserDataBuf0[sciid][0] = '(';
			    ...
			    && (g_ubCommandBuffer[sciid][5] == 'U') && (g_ubCommandBuffer[sciid][6] == 'S') && (g_ubCommandBuffer[sciid][7] == 'E'))//REUSE
		{
			...
			if((g_ubCommandBuffer[sciid][8] == 'L'))
			{
				if(g_ubCommandBuffer[sciid][9] == '1')
				{
					hoGENRLY_On;
			...
			&& (g_ubCommandBuffer[sciid][9] == 'E'))
		{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'K') && (g_ubCommandBuffer[sciid][6] == 'V')
			&& (g_ubCommandBuffer[sciid][7] == 'N') && (g_ubCommandBuffer[sciid][8] == '1'))//TSTPVKVN1
		{
			if(g_ubCommandBuffer[sciid][9]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(wKvn_1, 4, 0, &g_ubUserDataBuf0[sciid][1]);
			...
			&& (g_ubCommandBuffer[sciid][5] == 'K') && (g_ubCommandBuffer[sciid][6] == 'V')
			&& (g_ubCommandBuffer[sciid][7] == 'N') && (g_ubCommandBuffer[sciid][8] == '2'))//TSTPVKVN2
		{
			if(g_ubCommandBuffer[sciid][9]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(wKvn_2, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'K') && (g_ubCommandBuffer[sciid][6] == 'V')
			&& (g_ubCommandBuffer[sciid][7] == 'D') && (g_ubCommandBuffer[sciid][8] == '1'))//TSTPVKVD1
		{
			if(g_ubCommandBuffer[sciid][9]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(wKvd_1, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'K') && (g_ubCommandBuffer[sciid][6] == 'V')
			&& (g_ubCommandBuffer[sciid][7] == 'D') && (g_ubCommandBuffer[sciid][8] == '2'))//TSTPVKVD2
		{
			if(g_ubCommandBuffer[sciid][9]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(wKvd_2, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'K') && (g_ubCommandBuffer[sciid][6] == 'C')
			&& (g_ubCommandBuffer[sciid][7] == 'N') && (g_ubCommandBuffer[sciid][8] == '1'))
		{
			if(g_ubCommandBuffer[sciid][9]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(wKcn_1, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'K') && (g_ubCommandBuffer[sciid][6] == 'C')
			&& (g_ubCommandBuffer[sciid][7] == 'N') && (g_ubCommandBuffer[sciid][8] == '2'))
		{
			if(g_ubCommandBuffer[sciid][9]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(wKcn_2, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'K') && (g_ubCommandBuffer[sciid][6] == 'C')
			&& (g_ubCommandBuffer[sciid][7] == 'D') && (g_ubCommandBuffer[sciid][8] == '1'))
		{
			if(g_ubCommandBuffer[sciid][9]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(wKcd_1, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'K') && (g_ubCommandBuffer[sciid][6] == 'C')
			&& (g_ubCommandBuffer[sciid][7] == 'D') && (g_ubCommandBuffer[sciid][8] == '2'))
		{
			if(g_ubCommandBuffer[sciid][9]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(wKcd_2, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'K') && (g_ubCommandBuffer[sciid][6] == 'C')
			&& (g_ubCommandBuffer[sciid][7] == 'N') && (g_ubCommandBuffer[sciid][8] == '0'))
		{
			if(g_ubCommandBuffer[sciid][9]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(wKcn_0, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
			...
			&& (g_ubCommandBuffer[sciid][7] == 'I') && (g_ubCommandBuffer[sciid][8] == 'N'))
		{
			if(g_ubCommandBuffer[sciid][9]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(wP_KVinComp, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
		...
		else if((g_ubCommandBuffer[sciid][3] == 'T') && (g_ubCommandBuffer[sciid][4] == 'E')
			&& (g_ubCommandBuffer[sciid][5] == 'S') && (g_ubCommandBuffer[sciid][6] == 'T')
			&& (g_ubCommandBuffer[sciid][7] == '0'))//TSTTEST0
		{
			if(g_ubCommandBuffer[sciid][8]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(TestMode, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'S') && (g_ubCommandBuffer[sciid][6] == 'T')
			&& (g_ubCommandBuffer[sciid][7] == '1'))//TSTTEST1
		{
			if(g_ubCommandBuffer[sciid][8]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(TestMode1, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
			...
			&& (g_ubCommandBuffer[sciid][5] == 'S') && (g_ubCommandBuffer[sciid][6] == 'T')
			&& (g_ubCommandBuffer[sciid][7] == '2'))//TSTTEST2
		{
			if(g_ubCommandBuffer[sciid][8]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(TestMode2, 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
			else{
		...
		else if((g_ubCommandBuffer[sciid][3] == 'M') && (g_ubCommandBuffer[sciid][4] == 'D')
			&& (g_ubCommandBuffer[sciid][5] == 'A') && (g_ubCommandBuffer[sciid][6] == 'D')
			&& (g_ubCommandBuffer[sciid][7] == 'D'))//MDADD
		{
			if(g_ubCommandBuffer[sciid][8]=='?'){
				g_ubUserDataBuf0[sciid][0] = '(';
				sNumToASCII(swGetEEModbusAddr(), 4, 0, &g_ubUserDataBuf0[sciid][1]);
				g_ubUserDataBuf0[sciid][6] = cChar_Enter;
				sSciWrite(sciid,g_ubUserDataBuf0[sciid],7);
			}
...
void	sJMPToIAPReflash(INT8U sciid)
{
	...
	for(udwCnt = 0; udwCnt < 5000; udwCnt++);
	OS_ENTER_CRITICAL();
    asm("    MOVL XAR7, #0x338000");
    asm("    LB *XAR7");	
}
/**
 * @description: 串口如果有问题,会重启串口 判断串口硬件是否发生错误 如果发生怎复位硬件
 * @param {INT8U} ubSciid
 * @param {INT16U*} pwSCIBusyCnt
 * @param {INT16U*} pwSCIErrorCnt
 * @return {*}
 */
void sSCIProtection(INT8U ubSciid, INT16U* pwSCIBusyCnt, INT16U* pwSCIErrorCnt)
{
...
/// @brief 获取实时时间
/// @param pubTime   时间指针,获取的时间要放在这里
/// @param pubCheckRTC  检查时间使能
/// @return
INT8U subGetRealTime(STRDateTime * pubTime, INT8U * pubCheckRTC)
{
	...
	{
		if(++(pubTime->ubSecond) > 59)	//59 sec	 时间大于59秒 有问题
		{
			pubTime->ubSecond = 0;
			if(++(pubTime->ubMin) > 59)
			{
