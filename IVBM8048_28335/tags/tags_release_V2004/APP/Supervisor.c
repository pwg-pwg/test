...
extern INT16S gi_wRGenCurrSampleBias1;
extern INT16S gi_wRGenCurrSampleBias2;
extern INT16S gi_wRGenCurrSampleAvg;
extern INT16S gi_wRGenCurrSampleAvg2;
...
INT16U g_wACVolt = 0;
// INT16U  g_uwLineToBatFlag = 0;
extern INT16U g_wOP_TWINS_SETCnt;
INT16U OpenTimeTemp = 0;
INT16U CloseTimeTemp = 0;
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			if(--uwPowerOnDelayCnt == 0)
			{
				...
				{
					OS_ENTER_CRITICAL();
					gi_wRGenCurrSampleBias1 = gi_wRGenCurrSampleAvg;
				...
				if((abs(gi_wRGenCurrSampleAvg2) < 200) && (gi_wRGenCurrSampleBias1 == 0))
				{
					OS_ENTER_CRITICAL();
					gi_wRGenCurrSampleBias2 = gi_wRGenCurrSampleAvg2;
					gi_wRGenCurrSampleBias = gi_wRGenCurrSampleAvg2;
					OS_EXIT_CRITICAL();
				}
				else if((abs(gi_wRGenCurrSampleAvg2) > 200))
				{
					g_uwFaultCode = cGenCTSenserFault;
					//sWriteFaultLog(g_uwFaultCode,gi_wRGenCurrSampleAvg);
					g_uwWorkMode = cFaultMode;
...
INT16S swInvVoltSoft(void)
{
	...
	while(1)
	{
		...
		if(g_uwSuperEvent & (1 << eSuperTimer))
		{
			...
			{
				if(g_uwRInvVolt > (g_uwInvRMSCtrlVolt - 100))// 等待逆变电压起来后
				{
					//转离网
					return(cBatteryMode);
				}
...
INT8U mChkOP_TWINS_Sts(void)
{
	...
	INT16U CurrentTimeTemp = 0;

	if(g_wOP_TWINS_SETCnt > 0)
	{
		g_wOP_TWINS_SETCnt--;
	}
	else
	{
		OpenTimeTemp = ((swGetEETimingOP2StartTime()>>8) & 0x00FF) * 60 + ((swGetEETimingOP2StartTime()) & 0x00FF);
		CloseTimeTemp = ((swGetEETimingOP2EndTime()>>8) & 0x00FF) *60 + ((swGetEETimingOP2EndTime()) & 0x00FF);	
	}
	g_OP_TWINS_OutDuration = swGetEEDurationTime_OP2() & 0xFFFF;
	CurrentTimeTemp = (INT16U)g_strDateTime.ubHour * 60 + (INT16U)g_strDateTime.ubMin;
	...
	if(((OpenTimeTemp == 0) && (CloseTimeTemp == 0)) ||
	(OpenTimeTemp == CloseTimeTemp) ||
	(g_OP_TWINS_OutDuration == 0))
	{
		return false;
	}
	if(OpenTimeTemp < CloseTimeTemp)
	{
		if((CurrentTimeTemp >= OpenTimeTemp) && (CurrentTimeTemp < CloseTimeTemp))
		{
			Ret = true;
		}
		else
	...
	else if(OpenTimeTemp > CloseTimeTemp)
	{
		if((CurrentTimeTemp >= CloseTimeTemp) && (CurrentTimeTemp < OpenTimeTemp))
		{
			return false;
		}
		else
	...
	if(Ret == true)
	{
		if((g_OP_TWINS_OutDuration == 0)
			||((g_OP_TWINS_OutDuration != cDurationTimeDisable)&&((CurrentTimeTemp-OpenTimeTemp)>= g_OP_TWINS_OutDuration))
			...
			||(swGetEEBatteryType() == cBatType_LIB && wBatLevel <= swGetEEThresholdSOCMin_OP2()))//输出时间0,或已开启时间大于输出时间，或锂电电压电量低于设定值
		{
			...
			return false;
			    ...
			    || ((CurrentTimeTemp-OpenTimeTemp)< g_OP_TWINS_OutDuration))//输出时间设为全天，或已开启时间小于输出时间
		{
			if(swGetEEBatteryType() != cBatType_LIB)
		    {
