...
INT16S gi_wInvChgCurrLimitStep = 1;// 逆变充电电流
...
INT16S wGridOverVOut = 0;
INT16S wGridOverVOutCnt = 0;
 ...
 */
void sInvLoadOPTask(void) //100hz/10ms
{
	TASK_EVENT	event;
	...
	while(1)
	{
		...
		if(event&(1<<eInvLoadOPSinZero))//INVInterrupt 采集完电流，然后设置相关数据,在逆变周期的第5个点,此时会发射 eInvLoadOPSinZero 事件
		{
			...
			if(swGetRInvOverCurrPCnt() > 20)// 逆变电流过载
			{
				...
				&& g_uwWorkMode != cPowerOnMode) /*&& (!wGridOverVOut)*/)
				{
...
//no.1 从设置量中取最小值给到：电网给电池充电最大值g_dwBatACChgLimit，属于电池侧充电 
	...
	{ 
		...
		else
		{
			...
			if(g_dwInvChgLimit > cGen_CurrMax)
			{
				g_dwInvChgLimit = cGen_CurrMax;
			}
	...
	{//整流情况下, 充电电流会有限制
		...
		if(g_LineModeJoinInWay == cLineModeJoinInByGen)
		{
			g_dwBatChgLimit = (INT32S)cGen_CurrMax - (INT32S)g_uwROPCurr;
		}
...
//g_dwPVFeedLoadLimit
	// 1s 500W
	if(gi_ATECurrOverCheckEn == 4)
	{
		gi_wInvChgCurrLimitStep = 2;
	}
	else if(gi_ATECurrOverCheckEn == 5)
	{
		gi_wInvChgCurrLimitStep = 4;
	}
	...
	{// 工作模式 是市电, 整流模式	// 逆变充电 限制慢慢增加
		...
		{
			gi_wInvChgCurrLimit += gi_wInvChgCurrLimitStep;
			// gi_wInvChgCurrLimit ++;
		}
		else
		{
...
void sSystemWorkSts(void)
{
	...
	else if(g_uwWorkMode == cLineMode)
	{
		...
		if(suwGetFBInvCtrlSts() == cFBInvCtrlBus)
		{
			...
			if(((suwGetBoost1CtrlSts() != cBoostWait) || (suwGetBoost2CtrlSts() != cBoostWait)) //太阳工作
			&& (g_dwInvWatt > -150 || (suwGetDCDCCtrlSts() != cDCDCWork)))
			{
...
INT16U  g_dwGenPowerWattCversion;

INT32S  g_wLinePowerTConversion = 0;
INT32S  g_wLoadPowerTConversion = 0;
INT16S  g_wBatPowerTConversion = 0;
INT16S  g_wPV1PowerTConversion = 0;
INT16S  g_wPV2PowerTConversion = 0;
INT32S  g_wGenPowerTConversion = 0;
...
void sInvPowerConversionCal(void)
{
	...
	{  
		...
		if(g_LineModeJoinInWay == cLineModeJoinInByGrid)	 
		{
		    ...
		    {
		        s_dwGridPower = s_dwOPWatt;
		    }	
			else
			{
				s_dwGridPower = s_dwOPWatt - g_dwRInvWatt;	//逆变 : <0 整流  >0逆变
			}	
		}
		else if(g_LineModeJoinInWay == cLineModeJoinInByGen)  
		{
			s_dwGenPower = (INT16U)((INT32U)g_uwRGenVolt * g_uwRGenCurr / 100);
	...
	//2、功率折算
	//需求功率
	s_dwRequiredPower = s_dwOPWatt + Get_Power(s_wBatPower);
	...
	//3、最终赋值	
	//需求功率
	g_wLoadPowerTConversion      = (INT16U)((s_dwOPWatt * s_dwRequiredPowerRatio) >> 12);	
	//供给功率
	g_wPV1PowerTConversion       = (INT16U)(((INT32U)g_uwSolarPower1Avg * s_dwSupplyPowerRatio) >> 12);
	g_wPV2PowerTConversion       = (INT16U)(((INT32U)g_uwSolarPower2Avg * s_dwSupplyPowerRatio) >> 12);
	g_wLinePowerTConversion   = (s_dwGridPower * s_dwSupplyPowerRatio) >> 12;   
    g_wGenPowerTConversion    = (s_dwGenPower * s_dwSupplyPowerRatio) >> 12;

    //电池单独处理
	if(s_wBatPower >= 0)  
	{
		g_wBatPowerTConversion = (INT16S)(((INT32S)s_wBatPower * s_dwRequiredPowerRatio)>> 12);
	}
    else                 
	{
		g_wBatPowerTConversion = -(INT16S)(((INT32S)-s_wBatPower * s_dwSupplyPowerRatio)>> 12); 
    }
	//4、赋值结果额外处理
	//①、如果有供给功率<需求功率的情况，需求功率减小相应的部分(余数导致1~2w误差)
	s_dwRequiredPower = g_wLoadPowerTConversion + Get_Power(g_wBatPowerTConversion);
		
	s_dwSupplyPower = g_wPV1PowerTConversion + g_wPV2PowerTConversion + g_wLinePowerTConversion + g_wGenPowerTConversion + Get_Power(-g_wBatPowerTConversion);
	s_wBalancePower = s_dwSupplyPower - s_dwRequiredPower;

    if(s_wBalancePower < 0)
    {
        if(g_wLoadPowerTConversion > -s_wBalancePower)       	
		{
			g_wLoadPowerTConversion += s_wBalancePower;
        }
		else if(g_wBatPowerTConversion > 0 && g_wBatPowerTConversion > -s_wBalancePower)      
		{
			g_wBatPowerTConversion += s_wBalancePower;
		}
    }
	...
	if(g_uwWorkMode == cLineMode && swGetEEOPPriority() == cOutputUtilityFirst)
	{
		...
		{
			s_dwAcPower = &g_wLinePowerTConversion;
			s_wBalancePower = g_wLoadPowerTConversion - g_wLinePowerTConversion;
		}
		else if(g_LineModeJoinInWay == cLineModeJoinInByGen)
		{
		    s_dwAcPower = &g_wGenPowerTConversion;
			s_wBalancePower = g_wLoadPowerTConversion - g_wGenPowerTConversion;
		}

		if(s_wBalancePower > 0 && g_wBatPowerTConversion >= 0 && (g_wPV1PowerTConversion + g_wPV2PowerTConversion) >= (s_wBalancePower + g_wBatPowerTConversion))
		{
		    *s_dwAcPower += s_wBalancePower;
			if(g_wPV1PowerTConversion <= (s_wBalancePower + g_wBatPowerTConversion))
			{
				g_wPV2PowerTConversion -= s_wBalancePower;
			}
			else if(g_wPV2PowerTConversion <= (s_wBalancePower + g_wBatPowerTConversion))
			{
				g_wPV1PowerTConversion -= s_wBalancePower;
			}
			else
			{
				g_wPV1PowerTConversion -= s_wBalancePower/2;
				g_wPV2PowerTConversion -= s_wBalancePower/2;
			}
			if(g_wPV1PowerTConversion <= 3)
			{
			    g_wPV1PowerTConversion = 0;
			}
			if(g_wPV2PowerTConversion <= 3)
			{
			    g_wPV2PowerTConversion = 0;
			}
		}
	}
...
void sInvPowerCal(void)
{
	...
	if(subGetPalLineLossStatus() == true)
	{
		g_dwLinePowerWattCversion = 0;
	}
