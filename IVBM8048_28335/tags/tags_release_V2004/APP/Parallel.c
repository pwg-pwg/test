...
#define  	cComID           					0x00 //通用ID
#define  	cEveID           					0x01 // 事件Id

#define  	cSlaveSend    					0x00// 从机发送
#define  	cMasterSend      					0x01// 主机发送

#define	cSlaveInfo						0x00
#define	cSlaveInfoInq					0x01// 从机数据询问

#define	cSlaveInfoGroup					((cSystemDataSize+2)/3)// 4
...
INT16U g_uwSystemData[cParaMaxSize][(cSystemDataSize+2)/3*3] = {0};//这个是系统状态 , 保存其他主机设备数据

INT16U g_uwSlaveExsitFlg[cParaMaxSize] = {0};//从机存在标志
INT16U g_uwSlaveExsitFlgSys[cParaMaxSize] = {0};// 从机 存在标志位
...
INT16U g_uwParaMode = cNewMode;//一

INT16U g_uwPhysicalAddr = 0;

union
{
	INT32U dwIDValue;// 本地的CAN ID
	struct
	{
		INT16U wLocalIDLow;
		INT16U wLocalIDHigh;
...
typedef union
{
	...
	struct
	{
		INT16U	wParameterChange:1;//参数改变过
		INT16U 	uwOutputVolt:3;//输出电压设置 0::220V 1:230V 2: 240V
		INT16U 	uwOutputFreq:1;//0 50Hz 1: 60Hz
		INT16U 	uwBatType:3;//电池类型
		INT16U 	uwOPPriority:2;//s输出优先级    U:市电  S（PV）：太阳鞥  B：电池   ；； 0 USB； 1 SUB；2,：SBU
		INT16U 	uwACRange:1;//市电输入范围
		INT16U 	uwOPMode:3;// 输出模式 同并网模式  0: 单独模式
		INT16U 	uwReserved:2;
	}BIT;
}STRParameterSet1;
...
typedef union
{
	...
	struct
	{
		INT16U	uwBatFloatVolt:8;//恒流 浮动充电
		INT16U	uwBatCVVolt:8;//恒压充电
	}BIT;
}STRParameterSet2;
...
enum
{
	...
	cNewResponse,
	cMachineNumSet,
	cMultiMasterChk,// 做个主机检测
	cParaOPRlyOff,
	cParaOPRlyOn,// 输出继电器打开
	cLineToBat,
	cBatToLine,// 电池变在线模式
	cToFault,
	cToStandby,
...
UNIInputStatus g_uniInputStatus;// 并网下, 别的设备的一下 相关输入信息
...
typedef union
{
	...
	struct
	{
		INT16U wBatOpen:1;
		INT16U wBatUnder:1;
		INT16U wLineAbnormal:2;//line 是否正藏  1:电压频率还没有问题 其他有问题  2:电压和频率都有问题
		INT16U wInvSoftOK:1;
		...
		INT16U wInvOnRlyOn:1; // 1 :开机
		INT16U wBatWeak:1;
		INT16U wOverLoadFail:1;
		INT16U wBatOverVolt:1;
		INT16U wSCCOK:1;// 太阳能没有问题  true
...
INT16U   g_uwComEveID;// 这个判断接收的 命令 模式是  cComID    cEveID :第32位 是1 ,这个应该很难,需要命令ID>128
INT16U   g_uwAllMsgCnt;//消息计数
INT16U   g_uwOrderMsg;// 消息序号  对应 cParamerterSet1 *.....
INT16U   g_uwSortEve;// 事件来源 主机 还是从机
INT16U   g_uwMachineNum;// 机器num
INT16U   g_uwSourceComEve;//  是主机 还是从机
INT16U   g_uwCommand;//命令
...
INT16U   g_uwMasterNewReceiveCnt = 0;// 收到的设备应答数量计数
INT16U   g_uwMasterSlaveReceiveCnt = 0;// 主机接收计数
...
INT16U   g_uwParaBatWeakChgFlg = false;//* (g_fBatWeak) 电池电压低, 要切换到社电供电) 状态更改过
INT16U   g_uwParaLoadAbnormalFlg = true;
INT16U   g_uwNewPhysicalAssignFlg = false;
INT16U	g_uwIDAutoGenerateStep = cPrioSuper;//这个像是一个随机数的产生
INT16U	g_uwIDHighTemp = 0;
INT16U	g_uwIDLowTemp = 0;
INT16U	g_uwParameterSetChange = false;//参数被改变了 标志位
INT16U	g_uwParaNumMaxSize = cParaMaxSize;
INT16U	g_uwParaModeSwitchFlg = 0;
INT16U	g_uwParaSlaveRecieveFlg = false;// 收到从机flag
...
INT16U 	g_uwMasterWorkMode = cPowerOnMode; // master 当前工作模式
...
INT16S g_wPhaseMasterFlg = false;// 这个可能是多相
...
void sParallelTask(void)
{
	...
	while(1)
	{
		sCanEventEnable2();// 使能CAN接收, 使能接收数据
		if(g_uwParaMode == cMasterMode)//主模式情况下
		{
			sParaMasterModeProc();// 主模式下并网处理
		}
		else if(g_uwParaMode == cSlaveMode)
		{
			sParaSlaveModeProc();// 从机模式下协议解析
		}
		else
		{// 该开始的时候 g_uwParaMode =  cNewMode,然后进入 sParaNewModeProc
...
/**
 * @description: 刷新本地状态 将状态都加载到g_uwSystemData 中
 * @return {*}
 */
void sUpdateLocalData(void)
{
	...
	g_strLocalParameterSet1.BIT.wParameterChange = g_uwParameterSetChange; // 参数设置 修改标志
	if(swGetEEOutputVolt() == 2200)// 对外输出电压
	{
	...
	if(swGetEEOutputFreq() == 5000)//输出频率
	{
	...
	{
		if(subGetLineVoltLossStatus() == false && subGetLineFreqLossStatus() == false)//电压和频率没问题
		{
	...
	if(subGetGenLossStatus() == true && g_uwWorkMode != cPowerOnMode)
	{
		...
		{
			g_strLocalInvStatus2.BIT.wGenAbnormal = 1;// TODO YXF
		}
		else if(subGetGenVoltLossStatus())
		{
			g_strLocalInvStatus2.BIT.wGenAbnormal = 2;
		}
		else if(subGetGenFreqLossStatus())
		{
		    g_strLocalInvStatus2.BIT.wGenAbnormal = 3;
		}
	}
	else
	{
		g_strLocalInvStatus2.BIT.wGenAbnormal = 0;
	}

	g_strLocalInvStatus.BIT.wLoadAbnormal = g_wOverLoadBypass;

	if(((g_uwWorkMode == cLineMode || g_uwWorkMode == cBypassMode) && (   (g_LineModeJoinInWay == cLineModeJoinInByGrid && mChkGridRlyOn())
			    || (g_LineModeJoinInWay == cLineModeJoinInByGen  && mChkGENRLY_On()) )
			 && mChkOPRlyOn()) 
	...
	//充电状态
	g_uwSystemData[g_uwPhysicalAddr][cChgStatus] = g_uniChgStatus.uwChgStatus;//保存系统状态
}
/// @brief 并网模式处理
/// @param
void sParaNewModeProc(void)
{
	...
	while(1)
	{
		...
		sUpdateLocalData();// 跟新本地状态
		g_uwParaEvent = OSEventPend();
		sCanParsingEvent(); // CAN 数据解析
		if(g_uwParaMode != cNewMode)
		{
			return;
		}
				...
				if(liHOSTRXD == 0) // 有别的并网消息
				{
					...
					else
					{
						if(g_uwParaNumSet > 1)
						{
							...
							//sWriteFaultLog(g_uwFaultCode,0);
...
void sParaSlaveModeProc(void)
{
	...
	INT16U wIndex;
//	INT16U wCanOKCnt = 0;
	INT16U uwPhaseMasterSearch = false;// 主机寻找flag
	...
	while(1)
	{
		if(!gi_wParallelEnable)// 如果是单机模式 则跳到主机
		{
			sToBeMasterModule();
			return;			
		}
		...
		sUpdateLocalData();
		g_uwParaEvent = OSEventPend();
		sCanParsingEvent();//作为从机解析CAN消息
		if(g_uwParaMode != cSlaveMode)
		{
			return;
		}
		sModeSwitchEvent(); // 处理 相关事件
		if(g_uwParaEvent & (1 << eParallelTimer))
		{
		    if(ubMasterShutdown)
		    {
			...
			if(liHOSTRXD == 1)// 没有收到并网的数据
			{
				wParaCnt++;
				if(wParaCnt == 2)
				{
					if(g_uwRsvMaster == true)//  变成主模式
					{
						sToBeMasterModule();
						return;					
					}
...
void sParaMasterModeProc(void)
{
	...
	INT16U	wCanOKCnt = 0;
	INT16U	wIndex;
	INT16U	wModeSelectPre = gi_wParallelEnable; // 之前的工作模式
	hoHOSTTXDOn;
	// TestMode1 |= 16;
	sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh,cNullData,g_strLocalID.BIT.wLocalIDLow,
                    cToBeNew,cMasterSend,cNullData,
                    cNullData,cNullData,cNullData); // 发送一帧开始信号的消息  这真消息是一帧主机发送 		 cMasterSend
	sCan2Write9(&g_strCanTxCom2);//CAN 发送消息
	while(1)
	{
		...
		sUpdateLocalData();//更新 master下的参数
		g_uwParaEvent = OSEventPend();
		sCanParsingEvent();// 解析CAN 数据，接收从机消息
		if(g_uwParaMode != cMasterMode)
		{
			return;
		}
		sModeSwitchEvent();// 模式切换事件
		if(g_uwParaEvent & (1 << eParallelTimer))
		{
			if(gi_wParallelEnable)// 并网使能
			{
				if(liHOSTRXD == 1)// 并机口没有收到消息的话,  认为并网丢失
				{
					wCanOKCnt = 0;
					if(wMasterChkCnt++ > 10)
					// if((wMasterChkCnt++ > 10)&& (g_uwPBusAvgVoltNew >= g_uwBatVoltAvg * cTransformerRatio) )
					{
						g_uwFaultCode = (cParaHOSTLineLoss);
						mAddParaWarning(cHostLineLossAlarm);// 设置通信丢失
						//sWriteFaultLog(g_uwFaultCode,0);
						OSEventSend(cPrioSuper, eSuperFault);
						sToBeNewModule();
						return;
			...
			sSetMasterWorkMode(g_uwWorkMode); // 设置主机工作模式
			g_uwParaModeSwitchFlg = 0;
			if(gi_wParallelEnable)// 并网模式的话
			{
				if(!wModeSelectPre)
				{
					sToBeNewModule();
				...
				sInquiryNewMachine();// 获取新的机械参数,并设置其他设备的机器码
				sInquirySlaveMachine();// 查询从机设备
				sCheckSystemConfig();// 检查系统设置
			...
			else // 单机模式 取消一些标志
			{
				hoHOSTTXDOff;
				// TestMode1 |= 32;
				sCanEventDisable2(); // 取消CAN接收
				g_uwNewPhysicalAssignFlg = false;
				g_uwParaNumMaxSize = 1;
			}
			wModeSelectPre = gi_wParallelEnable;
			if(g_uwNewPhysicalAssignFlg == false && g_uwParaMode == cMasterMode) //在主模式下
			{
				sCheckSynAction();
			}
		}
...
void sCanParsingEvent(void)
{
	...
	if(g_wSPSSDProcFlg)//软启动保护的话,不解析
	{
		if(g_uwParaMode != cNewMode)
		{
		    //告知其他逆变器本机处于关机状态
	...
	{
		while(sCanRead2(&g_strCanRxData2) != cCanRxBufEmpty)// 读一包数据出来
		{
			...
			if(g_uwComEveID == cEveID) //命令大于8
			{
				if(g_uwSortEve == cSlaveInfo)// 别机发过来的设备数据  从机发生过来的数据会放在 g_uwSystemData数组中, 最大有12个系统数据
				{
...
//						}
				...
				else if(g_uwSortEve == cSlaveInfoInq)// 别 发过来的自身数据, 并返回该数据
				{
					if(g_uwAllMsgCnt == cSlaveInfoGroup && g_uwOrderMsg < cSlaveInfoGroup && g_uwMachineNum < cParaMaxSize)
					{
						if((g_uwMachineNum == 0 || g_uwMachineNum == g_uwPhysicalAddr) && (g_uwParaMode == cSlaveMode))  // 如果是从机模式,  保存数据到0 数组
						{
...
//						}
					if(g_uwParaMode == cSlaveMode && g_uwOrderMsg == (cSlaveInfoGroup - 1))
					{
						...
						sSetMasterWorkMode(g_strLocalBatVoltWorkModeTemp.BIT.uwWorkMode); // 更改工作模式
						g_uwBatDiffVolt = g_strLocalBatVoltWorkModeTemp.BIT.uwBatVolt;// 电池电压
			...
			else // g_uwComEveID == cEveID
			{
				if(g_uwParaMode == cNewMode)
				{
					...
					else if(g_uwCommand == cNewInquire)// 新设备需求, 发送响应指令
					{
...
//					}
				...
				else if(g_uwParaMode == cSlaveMode)// 从机模式
				{
					if(g_uwCommand == cParaOPRlyOff)// 关闭输出继电器
					{
						OSEventSend(cPrioSuper, eSuperOPRlyOff);
					}
					else if(g_uwCommand == cParaOPRlyOn)// 打开
					{
...
//							}
					...
					else if(g_uwCommand == cNewResponse)// 新设备应答
					{
						...
						{// 查找设备列表中,查找设备
							...
							{ // 获取到对应的ID
								...
								{
									g_strSystemID[wCnt].wPhysicalAdd = 0;
									g_uwMasterNewReceiveCnt++;// 收到了设备应答
								}
								break;
							}
						}
				...
				if(g_uwCommand == cMachineNumSet)// 机器码设置
				{
					if(g_uwParaMode != cMasterMode)// 不是主机模式
					{
				...
				else if(g_uwCommand == cNewResponse)// 收到应答
				{
					dwTemp = ((INT32U)g_strCanReceiveFrame.Data0 << 16) + g_strCanReceiveFrame.Data1;
					while(dwTemp == g_strLocalID.dwIDValue)
					{
						sParallelInit(); // 重新初始化ID
					}
				}
				else if(g_uwCommand == cMasterRelease)
				{
					...
					{
						sToBeNewModule();// 和主机不一样的话, 则转到别的模式
					}
				}
...
/// @brief 一帧数据解码
/// @param stRxFrame
void sDecodeCanRxFrame(CANFRAME stRxFrame) // 解析数据
{
	INT16U temp;
	temp = stRxFrame.CanData0;
	if(temp & 0x8000)// 如果最高的一个是1 ,则是 事件消息
	{
		g_uwComEveID = cEveID;
		temp = stRxFrame.CanData0;
...
void	sInquiryNewMachine(void)
{
	...
	while(1)
	{
		...
		if(g_uwParaEvent & (1 << eParallelTimer))
		{
			...
			{
				sPhysicalAddrAssignChk();	// 物理地址检查, 检查后退出
				return;
			}
			wParaCnt++;
		}
	...
	while(1)
	{
		...
		if(g_uwParaEvent & (1 << eParallelTimer))
		{
			if(wCnt < (cSlaveInfoGroup * 2))
			{
				...
				if((wCnt % 2) == 0)// 寻味别的机器的状态数据, 并把自己的数据发送出去
				{
//						sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh,cNullData,g_strLocalID.BIT.wLocalIDLow,
...
void sPhysicalAddrAssignChk(void)
{
	...
	{// 循环检测, 设备地址
		if(g_strSystemID[wCnt].wPhysicalAdd != wCnt) // 如果设备机制和计数器, 不一样, 则置0
		{
			g_strSystemID[wCnt].wPhysicalAdd = 0;
		}
		for(wCnt2 = wCnt + 1; wCnt2 < cParaMaxSize; wCnt2++)// 前后两个一样 也置0
		{
	...
	while(1)
	{
		...
		if(g_uwParaEvent & (1 << eParallelTimer))
		{
			for(; wCnt < cParaMaxSize; wCnt++)
			{
				...
				|| (g_strSystemID[wCnt].wIDLow != 0)))// 如果 物理设备地址为0 ,  CAN ID 不为0, 则设置CANID
				{// 依次去设置 别的CAN的设备的物理地址,通过CAN 设置
...
/// @brief 模式开关事件, ,在并网下,,判断切换相关模式,并发送相关协议, 以主机的名义发送, 发送相关的事件
/// @param
void sModeSwitchEvent(void)
{
	if(g_uwParaEvent & (1 << eParaToLineMode))	
	{
		if(g_uwParaMode == cMasterMode && gi_wParallelEnable)// 发送主机模式 // 发送电池变在线模式
		{
...
//				OS_EXIT_CRITICAL();
	...
	if(g_uwParaEvent & (1 << eParaToBatMode))	//发送电池模式
	{
...
//				OS_EXIT_CRITICAL();
	...
	if(g_uwParaEvent & (1 << eParaToStandbyMode))	//发送准备模式
	{
		if(g_uwParaMode == cMasterMode)
		{
			sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh,cNullData,g_strLocalID.BIT.wLocalIDLow,
	...
	if(g_uwParaEvent & (1 << eParaToShutDownMode))	// 发送关机模式
	{
		if(g_uwParaMode == cMasterMode)
		{
			sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh,cNullData,g_strLocalID.BIT.wLocalIDLow,
...
//				OSEventSend(cPrioSuper, eSuperToShutDown);
	...
	if(g_uwParaEvent & (1 << eParaToBypassMode))	//发送旁路模式
	{
		if(g_uwParaMode == cMasterMode && gi_wParallelEnable)
		{
...
//				OS_EXIT_CRITICAL();
	...
	if(g_uwParaEvent & (1 << eParaToMasterRelease))// 发送主机 释放模式
	{
		if(g_uwParaMode == cMasterMode)
		{
			sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh,cNullData,g_strLocalID.BIT.wLocalIDLow,
	...
	if(g_uwParaEvent & (1 << eParaOPRlyOff)) // 输出继电器关闭
	{
		if(g_uwParaMode == cMasterMode)
		{
			if(gi_wParallelEnable)
...
INT16U swChkOPRly(void)
{
	...
	INT16U wParaWorkOn = false;// 并网工作打开
...
void	sChkSourceStatus(void)
{
	...
	while(1)
	{
		...
		if(g_uwParaEvent & (1 << eParallelTimer))
		{
			if(g_uniSysChgStatus.BIT.uwBMSConnected) // 电池链接
			{
				if(wCnt < 10)
				{
...
INT16U	swChkOverLoadStatus(void)
{
	...
	INT8U  ubGenStatus = 0;            //bit0:Volt Loss  bit1:Others
	...
	static INT8U wGenLossChkCnt = 0;
	static INT8U bGenPhaseStatus = 0;
...
//	INT16S	wOutputWork = false;
	...
	for(wCnt = 0; wCnt < g_uwParaNumMaxSize; wCnt++)
	{// 遍历设备列表, 如果存在的话, 根据每个列表信息
		if(g_uwSlaveExsitFlg[wCnt] == 1)//一般[0] =1
		{
			g_uwExsitNum++;
			//先从系统状态集中读取相关 数据 从0中读取祥光状态
			g_strInvParaStatus.wStatus = g_uwSystemData[wCnt][cInvStatus];
            ...
            //-------------- Gen Abnormal summy ------------------//
            g_strInvParaStatus2.BIT.wGenAbnormal = g_strLocalInvStatus2.BIT.wGenAbnormal; // TODO YXF 由于并机数据未处理，此处为临时处理
			...
			*/
			...
			uwExcessLoadAvg |= (1 << g_strLocalParameterSet1Temp.BIT.uwOPMode); //输出模式 0 1 2 3
			if(g_strInvParaStatus.BIT.wInvOnRlyOn == 1
			&& g_strLocalBatVoltWorkModeTemp.BIT.uwWorkMode != cFaultMode
			&& g_strLocalBatVoltWorkModeTemp.BIT.uwWorkMode != cPowerOnMode)
			{
				uwExcessLoadAvg |= (1 << (g_strLocalParameterSet1Temp.BIT.uwOPMode + 8));//输出模式 0 1 2 3
			}
...
INT8U sbParameterSetChk(void)
{
	...
	INT16U wParameterDiff = false;// 参数有差异
	for(wCnt = 0; wCnt < cParaMaxSize; wCnt++)
	{// 遍历设备列表
		if(g_uwSlaveExsitFlg[wCnt] == 1)// 如果从设备存在
		{			
			if((g_uwSystemData[wCnt][cParamerterSet1] & 0x0001))
			{
				wParameterChange = true; // 从设备的参数有过改变
				break;
			}
			else if(((g_uwSystemData[wCnt][cParamerterSet1] & cParameterSet1Mask)
			!= (g_uwSystemData[0][cParamerterSet1] & cParameterSet1Mask))// 别的设备和次设备参数有差异
...
//			sCan2Write9(&g_strCanTxCom2);
	...
	while(1)
	{
		...
		if(g_uwParaEvent & (1 << eParallelTimer))	
		{
			if(wParameterChange || wParameterDiff) // 如果参数有变化, 那么发送参数设置
			{
				sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh,cNullData,g_strLocalID.BIT.wLocalIDLow,\
						        cParameterSet,cMasterSend,cNullData,\
				         		g_uwSystemData[wCnt][cParamerterSet1],\
