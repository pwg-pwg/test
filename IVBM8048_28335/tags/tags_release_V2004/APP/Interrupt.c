...
INT32S gi_dwRGenCurrSampleSumTemp2;
INT16S gi_wRGenCurrSampleSumCntTemp;
INT16S gi_wRGenCurrSampleSumCntTemp2;
INT32S gi_dwRGenCurrSampleSum;
INT32S gi_dwRGenCurrSampleSum2;
INT16S gi_wRGenCurrSampleSumCnt;
INT16S gi_wRGenCurrSampleSumCnt2;
INT16S gi_wRGenCurrSampleAvg;
INT16S gi_wRGenCurrSampleAvg2;
INT16S gi_wRGenCurrSampleBias = 0;
INT16S gi_wRGenCurrSampleBias1 = 0;
INT16S gi_wRGenCurrSampleBias2 = 0;
...
INT32U gLLCPWM=0;
INT16U gPVCurrOverCnt = 0;
INT16U gi_wLEDFlag = 0;
...
void GENZeroCrossInterrupt2(void)
{
	...
	INT32U dwCapTemp=0;
	dwCapTemp = CpuTimer0Regs.TIM.all;
	gi_udwGenZeroTimeNewTemp = (INT32U)gPeriodCnt * 75000 - dwCapTemp;
	gi_uwGenPeriodNewTemp=(INT16U)((gi_udwGenZeroTimeNewTemp+gi_udwGenZeroTimeOld)/150);
...
interrupt void INVInterrupt(void)//这个是逆变PWM 中断函数 52us
{
	...
	g_wCtrlSinpointer2 = g_wCtrlSinpointer + 380;
	if(g_wCtrlSinpointer2 >= gi_wSinePointMax) // 如果控制点 大于最大分辨率 , 后移一个周期
	{
		g_wCtrlSinpointer2 -= gi_wSinePointMax;	
	}
	...
	g_wSinValueR2 = pSinTab[g_wCtrlSinpointer2];
	...
	if(g_uwWorkMode == cPowerOnMode)// 第一次上电 对逆变电流  输出电流 输出均流 积分呢
	{
		...
		gi_dwRGenCurrSampleSumTemp2 += gi_wRGenCurrSample;
		gi_wRGenCurrSampleSumCntTemp2 ++;
	}
	// 下面都是积分
...
/*******************hoADCSWITCH1 = 1 && hoADCSWITCH2 = 1 **********************/
	...
	// gi_wPDCDCAvgCurrSample = (INT16S)(((INT32S)gi_wPDCDCAvgCurrSample * uEepromCfg2.EepromStructCfg2.wEEDSPDcdcCurrAvgAdj) >> 11);
...
//			if(g_wRInvOverCurrCnt > gi_wSinePointTwoSix)
			if((g_wRInvOverCurrCnt > 3) /*&& (!wGridOverVOut)*/)
			{
				g_wRInvOverCurrCnt = 0;
		...
		{
			if((g_wRInvOverCurrCnt > 5) /*&& (!wGridOverVOut)*/)
			{
				g_wRInvOverCurrCnt = 0;
				g_uwFaultCode = cInvCurrOver;
				//sWriteFaultLog(g_uwFaultCode,g_uwRInvCurr);
...
/************************hoADCSWITCH1 = 0 && hoADCSWITCH2 = 0****************************/
	...
	// gi_wPDCDCCurrSample = (INT16S)(((INT32S)gi_wPDCDCCurrSample * uEepromCfg2.EepromStructCfg2.wEEDSPDcdcCurrAdj) >> 11);
	...
	if((gi_wBusVoltReal > gi_wBusRealHighLevel) || (gi_wBusVoltReal > cBusVoltReal460V))	//cBusVoltReal480V
	{
	...
	// bus 电池超过 500 V ,
	if((gi_wBusVoltReal > gi_wBusRealOverLevel) || (gi_wBusVoltReal > cBusVoltReal470V))	//cBusVoltReal500V
	{
...
//							&gi_wRGeneratorWaveChkCnt, &gi_wRGeneratorVoltMax, &gi_wRGeneratorVoltMin);
			sLineWaveChk(gi_wRLineVoltReal,g_wSinValueR2,g_uwRLineRms3timeAvgPeak,gi_wROPCurrSample, \
							&g_uwRLineWaveLossCnt, &g_uwRLineWaveOKCnt, &g_uwRLineWaveLossFlag,\
							&gi_wRGeneratorWaveChkCnt, &gi_wRGeneratorVoltMax, &gi_wRGeneratorVoltMin);
	...
	{
		if(gPVCurrOverCnt > 0 && gi_wLEDFlag == 0)
		{
			gPVCurrOverCnt--;
		}
		g_wSolar2OverCurrCnt = 0;
...
////	}
	...
	else
	{
		...
		{
			if(g_wSolar2OverCurrCnt > 0)
			{
				if((gPVCurrOverCnt < 1700))
				{
					gPVCurrOverCnt += 10;
				}
			}
			if(g_wBusRealVoltHighFlg || g_wSolar2OverCurrCnt > 10 || g_uwFaultCode != 0
...
INT16S swRGenCurrCal2(void)
{
	if(g_uwWorkMode == cPowerOnMode)
	{
		OS_ENTER_CRITICAL();
		gi_dwRGenCurrSampleSum2 = gi_dwRGenCurrSampleSumTemp2;
		gi_dwRGenCurrSampleSumTemp2 = 0;
		gi_wRGenCurrSampleSumCnt2 = gi_wRGenCurrSampleSumCntTemp2;
		gi_wRGenCurrSampleSumCntTemp2 = 0;
		OS_EXIT_CRITICAL();

		if(gi_wRGenCurrSampleSumCnt2 > 0)
		{
			gi_wRGenCurrSampleAvg2 = (INT16S)(gi_dwRGenCurrSampleSum2 / gi_wRGenCurrSampleSumCnt2);
		}
	}
}
