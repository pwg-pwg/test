...
void sDisplayTask(void)//显示任务  100ms 一次
{
	...
	while(1)
	{
		...
		if(event&(1<<eDisplayTimer))	 //100ms
		{
			...
			(g_uwWorkMode != cChgMode) && (g_uwWorkMode != cBypassMode))
			{
				sBUSLOWGJ();
			}
...
void sSwitchOnButtonProc(void)
{
	static INT16S s_wLoadOnOffCnt = 4;
	// if(s_wLoadOnOffCnt == 0)
	// {
	// 	GJOFF;
	// 	s_wLoadOnOffCnt = 5;
	// }
	...
	{
		if(!liBUTTONSWON && !hoBUZ)//开机
		{
			if(++s_wLoadOnOffCnt > 5)
			{
				s_wLoadOnOffCnt = 1;
				g_uwLoadOnSts = true;
				hoBUZON;
