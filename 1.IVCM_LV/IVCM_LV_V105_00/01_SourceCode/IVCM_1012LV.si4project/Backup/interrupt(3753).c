...
//		OSISREventSend(cPrioGrid, eGridLineZero);
	...
	{
		sInverterVoltCtrl();	//IVCM正常情况是离网模式，控输出电压
	}
	else
	{
		if(suwGetFBInvCtrlSts() == cFBInvCtrlBus)
		{
			sFBINVController();	//并网的时候由于输出电压被钳位没法控，只能控电流
		}
		else if(suwGetFBInvCtrlSts() == cFBInvOpenLoop)
		{
			sInverterCtrl();	//开环，用于调试
		}
	}
...
interrupt void isrDCDC(void)
{
	...
	if((uwGetModelLV() == false && swGetAdcBusVoltReal() >= cBusVoltReal550V)\
	|| (uwGetModelLV() == true && swGetAdcBusVoltReal() >= cBusVoltReal450V))
	{
