...
void	sDCDCControl(void)
{
	...
	if(swGetAdcBusVoltReal() <= (g_wDCDCBusVoltRef >> 1) && (g_uwDCDCCtrlSts == cDCDCSoft))
	{
		g_wPDCDCPWM = wGetPDCDCPWMSoftDuty();//2024LV
	}
	else
	{
		g_wPDCDCPWM = wGetPDCDCPWMSoftDutyNext();//2024LV
	}
