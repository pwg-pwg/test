...
//	static INT16U s_uwLowBatCVChkCnt = 0;
	...
	/******************************************************************
		...
		1、电池电压大于等于CV电压，禁止充电
		2、BMS连接且处于锂电模式且电池充满电，禁止充电
		3、老化模式使能，禁止充电
		4、PV进入了Float充电模式，禁止市电充电
		5、以上必须是没有在上电模式下检测到的才有效
	******************************************************************/
...
void	sFeedPowerProc(void)
{
	if(swGetAgingStatus() == true)
	{
		...
		{
			s_uwAgingChkCnt = 0;
			if(swGetBusVoltNew() <= cBusVoltReal275V || swGetRInvVANew() > uwGetInvVAMax())
			{
...
//	if(0)
	{
		if(uwBusVoltRefTemp < cBusVoltReal250V)
		{
			uwBusVoltRefTemp = cBusVoltReal250V;
		}
		else if(uwBusVoltRefTemp > cBusVoltReal350V)
		{
			uwBusVoltRefTemp = cBusVoltReal350V;
		}
	}
	else
	{
...
//	if(0)
	{
		if(wBusVoltRef < cBusVoltReal250V)
		{
			wBusVoltRef = cBusVoltReal250V;
		}
		else if(wBusVoltRef > cBusVoltReal350V)
		{
			wBusVoltRef = cBusVoltReal350V;
		}
	}
	else
	{
	...
	if(swGetAgingStatus() == false)
	{
		...
		if(uwGetModelLV() == true && g_wDCDCBusVoltRef > cBusVoltReal350V)				// max 345V				
		{
			g_wDCDCBusVoltRef = cBusVoltReal350V;
			//g_wDCDCBusVoltRef = cBusVoltReal480V;
		}
		else if(g_wDCDCBusVoltRef > cBusVoltReal480V)			// max 480V				
		{
			g_wDCDCBusVoltRef = cBusVoltReal480V;
		}
		OS_EXIT_CRITICAL();
	...
	{	
		...
		if(uwGetModelLV())
		{
			g_wInvBusVoltRef = cBusVoltReal290V;	// max 200V
			//g_wInvBusVoltRef = cBusVoltReal400V;	// max 200V
		}
		else
		{
...
void	sDsgPriorityProc(void)
{
	...
	if(wDsgCtrlMode == cDsgPriorityLINE) 		// 市电 --> (PV + BAT)	// USB
	{
		...
		if((!sbGetPalLineLossStatus()) && (swGetRLineVoltFilter() > cLineVolt60V))
		{
	...
	else if(wDsgCtrlMode == cDsgPriorityPV)		// (PV + BAT) --> 市电   PV不在则切市电	// SUB
	{
		...
		else if((!sbGetPalLineLossStatus()) && (swGetRLineVoltFilter() > cLineVolt60V) \
		&& (g_uwLineOKFlag == true))
		{
	...
	else if(wDsgCtrlMode == cDsgPriorityBAT)	// (PV + BAT) --> 市电   BAT不在则切市电	SBU
	{
		...
		|| (g_strBMSCtrlLogicInfo.ubBMSConnect && (swGetEEBatteryType() == cBatType_LIB) && (sbGetEMSBatSOC() >= suwGetEESocOffGridSwitchThr()))))						
		{
			wBatToLineEnDisable = false;
		}
		else if((!sbGetPalLineLossStatus()) && (swGetRLineVoltFilter() >= cLineVolt60V) \
				&& ((swGetBatVoltNew() <= swGetEEBatWeakVolt() && (!g_strBMSCtrlLogicInfo.ubBMSConnect)) \
				|| (g_strBMSCtrlLogicInfo.ubBMSConnect && (swGetEEBatteryType() == cBatType_LIB) && (sbGetEMSBatSOC() <= suwGetEESocGridSwitchThr()))))						
		{
			wBatToLineEnDisable = true;
		}
		else
...
void	sACChgCurrProc(void)
{
	...
	{	
		if(uwGetModelLV())
		{
			...
			||(swGetRLineVoltNew() < cLineVolt95V))
			{
				// 5S后才降档
				s_uwACChgCurrCnt++;
			...
			&& (swGetRLineVoltNew() >= cLineVolt95V) && (wLineChgHsTemp <= cHsTemp65C))
			{
				s_uwACChgCurrCnt++;
			...
			if(wGRIDChgDischgSwitch > (wLineChgMaxCurr / 10))
			{
				wGRIDChgDischgSwitch = uwGetLineChgCurr2();
			}
		}
...
//	wGRIDChgDischgSwitch = uwGetLineChgCurr2();	// 默认市电20A档位充电 此时机型还没初始化完成
...
//	wHsTempDeratROpVolt = cAcVoltReal230V;		// 过温降额负载电压
//	wHsInvTempDeratROpVolt = cAcVoltReal230V;	// 过温降额负载电压
//	wBatDeratROpVolt = cAcVoltReal230V;			// BAT欠压降额负载电压
//	wWattDeratROpVolt = cAcVoltReal230V;		// 功率降额负载电压
...
{
	if(uwGetModelLV() == true)	// LV 110V
	{
		g_wBusVoltRef = cBusVoltReal250V;
		g_wInvBusVoltRef = cBusVoltReal250V;
		g_wDCDCBusVoltRef = cBusVoltReal250V;
		wHsTempDeratROpVolt = cAcVoltReal120V;		// 过温降额负载电压
		wHsInvTempDeratROpVolt = cAcVoltReal120V;	// 过温降额负载电压
		wBatDeratROpVolt = cAcVoltReal120V;			// BAT欠压降额负载电压
		wWattDeratROpVolt = cAcVoltReal120V;		// 功率降额负载电压
		g_wOPRMSRatedVolt = swGetEEOutputVolt();	// 1200;
		g_uwTurboMode = false;						// Turob模式默认关闭
...
void	sInvTempDerating(void)
{
	...
	{										
		...
		else
		{
			if(uwGetModelLV())
			{
				if(wHsInvTempDeratROpVolt < cAcVoltReal95V)
		 		{
		 			wHsInvTempDeratROpVolt = cAcVoltReal95V;
				}
			}
			else
			{
...
void	sTwTempDerating(void)
{
	...
	{										
		...
		else
		{
			if(uwGetModelLV())
			{
				if(wHsTempDeratROpVolt < cAcVoltReal95V)
		 		{
		 			wHsTempDeratROpVolt = cAcVoltReal95V;
				}
			}
			else
			{
...
void	sBatDerating(void)
{
	...
	else
	{
		...
		else
		{
			if(uwGetModelLV())
			{
				if(wBatDeratROpVolt < cAcVoltReal95V)
		 		{
		 			wBatDeratROpVolt = cAcVoltReal95V;
				}
			}
			else
			{
...
extern INT16U g_uwModelVar;
void	sWattDerating(INT16U wFilter1, INT16U wFilter2)
{
	INT16S wWattTemp;
	
	if(swGetTurobMode() == false || uwGetModelLV() == true)	// LV机型没有此项功能
	{
...
INT16S sOutputVoltDrtProc(INT16S wOutputVoltRated)
{
	...
	if(swGetWorkMode() == cBatteryMode && (g_strBMSCtrlLogicInfo.ubBMSConnect))// 离网降电压 // 必须在电池模式并且BMS存在
	{
		...
		wMaxDisCurr = sbGetEMSMaxDisChgCurr();				// 最大放电电流	// 0.1A
...
//		}
		...
		if(g_wOPVolt > g_wOPVoltDereByTemp)	// 不能大于额定（1200）
		{
...
//		}
	...
	else
	{
		...
		g_wOPVolt = wOutputVoltRated;					// 默认额定电压（1200）
