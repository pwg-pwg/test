...
//			sBatChgDischgCurrMng();			// 支持市电通过逆变器充电后启用(第二代机器
			...
			if(uwGetModelLV())
			{
				//sBusOverChk(cBusVoltReal250V, 50, cBusVoltReal275V, 5);
				sBusOverChk(cBusVoltReal400V, 50, cBusVoltReal450V, 5);
			}
			else
			{
				sBusOverChk(cBusVoltReal500V, 50, cBusVoltReal550V, 5);
			}
			if(sbGetBusOverSts())
			{
				if(swGetWorkMode() != cFaultMode)
			...
			if((uwGetModelLV() == false && subBusUnderChk(cBusVoltReal100V, 250))\
			|| (uwGetModelLV() == true && subBusUnderChk(cBusVoltReal50V, 250)))	// 5S
			{
				if(swGetWorkMode() != cFaultMode)
				{
