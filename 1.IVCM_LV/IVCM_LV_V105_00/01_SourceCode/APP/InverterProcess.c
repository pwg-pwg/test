...
void	sInvParaUpdate(void)
{
	if(g_uwModelVar == cCtrlIVCM2024LV)
	{
		...
		g_wPDCDCPWMSoftDutyFirst  = 32;
		g_wPDCDCPWMSoftDutySecond = 33;
		g_wPDCDCPWMSoftDutyThird  = 38;
	...
	else if(g_uwModelVar == cCtrlIVCM2024LV_II)
	{
		...
		g_uwLineChgCurr2 = 15;						// 市电档位 15A
		g_uwCTdirection = cReverse;					// CT方向 // 反接
		g_uwInvCurrRatio = 11348;//11352;//6876;	// 逆变电流采样系数
		g_uwInvWattRes = 720;						// 功率百分比折算 // = (1200 * 1200 / 2000)
		g_uwSettingMaxChgCurr = 75;					// 75A
		g_uwSettingMaxACChgCurr = 15;				// 15A
		wInvCtrlPower = -4000;
...
INT16U uwGetPDCDCPWMSoftDutyFirst(void)
{
	return g_wPDCDCPWMSoftDutyFirst;
}

INT16U uwGetPDCDCPWMSoftDutySecond(void)
{
	return g_wPDCDCPWMSoftDutySecond;
}

INT16U uwGetPDCDCPWMSoftDutyThird(void)
{
