...
void	sBatSOCCheck(INT16U uwFilter)
{
	...
	static INT16S uwBatSOC = 0;
	static INT16S s_wBatChgRecoverSOC = 0;
	static INT16S s_wGridChgRecoverSOC = 0;
	static INT16S s_uwBatLowSOC = 0;

	s_wBatChgRecoverSOC  = (INT16S)suwGetEEBatChgCutoffSOC()  - 1;
	s_wGridChgRecoverSOC = (INT16S)suwGetEEBatChgCutoffSOC()  - 5;
	s_uwBatLowSOC  = (INT16S)suwGetEEBatLowSOC() + 1;
	
	Dnlimit(s_wBatChgRecoverSOC  ,0);
	Dnlimit(s_wGridChgRecoverSOC ,0);
	Uplimit(s_uwBatLowSOC,100);

	// 获取当前SOC值
	uwBatSOC = (INT16S)(sbGetEMSBatSOC() / 10);

	if(fBatteryStatus.bits.bSOCFull)//在PV充电下，判断PV是否充满
	{
		//如果充满，检查后续是否跌落到恢复充电阈值
		if(sb16UUnderLevelChk(uwBatSOC,s_wBatChgRecoverSOC,uwFilter,&s_uwChgRecoverSOCCnt))
		{
	...
	{
		//如果没有充满，检查是否到达阈值
		if(sb16UOverLevelChk(uwBatSOC,(INT16S)suwGetEEBatChgCutoffSOC(),uwFilter,&s_uwChgStopSOCCnt))
		{
	...
	{
		//如果充满，检查后续是否跌落到恢复充电阈值
		if(sb16UUnderLevelChk(uwBatSOC,s_wGridChgRecoverSOC,uwFilter,&s_uwGridChgRecoverSOCCnt))
		{
	...
	{
		//如果没有充满，检查是否到达阈值
		if(sb16UOverLevelChk(uwBatSOC,(INT16S)suwGetEEBatChgCutoffSOC(),uwFilter,&s_uwGridChgStopSOCCnt))
		{
	...
	{
		//如果已经低电警告，检查后续是否恢复
		if(sb16UOverLevelChk(uwBatSOC,s_uwBatLowSOC,uwFilter,&s_uwLowBatRecoverSOCCnt))
		{
	...
	{
		//如果没有警告，检查是否低于警告阈值
		if(sb16UUnderLevelChk(uwBatSOC,(INT16S)suwGetEEBatLowSOC(),uwFilter,&s_uwLowBatSOCCnt))
		{
	...
	{
		//如果已经放电截止，检查后续是否恢复
		if(sb16UOverLevelChk(uwBatSOC,(INT16S)suwGetEEBatRecoverOutSOC(),uwFilter,&s_uwDischgRecoverSOCCnt))
		{
	...
	{
		//如果没有警告，检查是否低于警告阈值
		if(sb16UUnderLevelChk(uwBatSOC,(INT16S)suwGetEEBatDischCutoffSOC(),uwFilter,&s_uwDischgStopSOCCnt))
		{
...
{
	if((fBMSStopDischgFlg && (swGetWorkMode() != cStandbyMode)) 
		|| (swGetWorkMode() == cChgMode) || fBatteryStatus.bits.bSOCDischCutoff)
	{
...
{
	if(fBatteryStatus.bits.bBatUnder || fBMSStopDischgFlg || fBatteryStatus.bits.bSOCDischCutoff)	// || g_uniInputStatus.BIT.uwBatUnder || fBMSStopDischgFlg)
