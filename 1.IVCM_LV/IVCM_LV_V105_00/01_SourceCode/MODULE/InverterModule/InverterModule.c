...
void	sInverterInitial(void)
{
	...
	if(swGetEEOutputFreq() == cFreq60hzConst || swGetEEOutputFreq() == cFreq60hzAuto)
	{
	...
	else if(swGetEEOutputFreq() == cFreq50hzConst || swGetEEOutputFreq() == cFreq50hzAuto)
	{
...
extern INT16U gi_uwPreSinePeriodCntNew;	// 软件市电周期值,50Hz=20000,60Hz=1666
void	sFrePhaseLockProc(void)	// 将这里改成与软件锁相结合在一起调整机器本身的相位
{
	INT16U wTemp;
	
//	if(sbGetLineVoltLossStatus() == false && sbGetLineFreqLossStatus() == false && swGetWorkMode() != cPowerOnMode)//&& wParaLineAbnormalFlg != 2 cBatteryMode
	if(sbGetLineVoltLossStatus() == false && swGetWorkMode() != cPowerOnMode)//&& wParaLineAbnormalFlg != 2 cBatteryMode
	{
		//					逆变周期			逆变与市电过零差值		市电周期值		  调节步长
		//sInverterPhaseLock(swGetSinePeriodCntNew(),spGetInvTd(),swGetLinePeriodCntNew(),swGetPhaseLockStep());
		sInverterPhaseLock(swGetSinePeriodCntNew(),spGetInvTd(),gi_uwPreSinePeriodCntNew,swGetPhaseLockStep());
	...
	if(swGetEEOutputFreq() == cFreq60hzConst || swGetEEOutputFreq() == cFreq60hzAuto)
	{
		wTemp = ((INT32U)cPeriod60Hz * 19661 + (g_uwInverterPeriodCntNew >> 1)) / g_uwInverterPeriodCntNew;
	}
	else if(swGetEEOutputFreq() == cFreq50hzConst || swGetEEOutputFreq() == cFreq50hzAuto)
	{
...
INT16U	uwInvValueTest = 12669;//11312;//10408;//12669;//11312;//15000;	// for test 220230705
...
INT16U uwOPCurr = 950;	//9.5A
void	sOPCurrHighChk(INT8U ubFilter)	// OP过流
{
	static INT16U s_uwROPCurrHighCnt = 0;
	INT8U ubOPCurrHigh = false;
	
	if(swGetWorkMode() == cBatteryMode || swGetWorkMode() == cBypassMode || swGetWorkMode() == cLineMode)
	{
		// 根据市电充电功率动态调节保护值
		// IVCM1012 市电输入保险是10A的，OP继电器是10A的
		// IVCM2024 市电输入保险是10A的，OP继电器是16A的，
		if(uwGetModelLV())
		{
			if(uwGetModelVar() == cCtrlIVCM1012LV)
			{
				if(suwGetRLineChgCurrNew() == cInv20A)
				{
					uwOPCurr = cOp14A;
				}
				else if(suwGetRLineChgCurrNew() == cInv10A)
				{
					uwOPCurr = cOp14A;
				}
				else
			...
			else if((uwGetModelVar() == cCtrlIVCM2024LV) || (uwGetModelVar() == cCtrlIVCM2024LV_II))
			{
				if(suwGetRLineChgCurrNew() == cInv20A)
				{
					uwOPCurr = cOp24A;
			...
			{
				if(suwGetRLineChgCurrNew() == cInv30A)
				{
					uwOPCurr = cOp44A;
				}
				else if(suwGetRLineChgCurrNew() == cInv20A)
				{
					uwOPCurr = cOp48A;
				}
				else
		...
		if(sb16UOverLevelChk(swGetROpCurrNew(), uwOPCurr, ubFilter, &s_uwROPCurrHighCnt))
		{
			ubOPCurrHigh = true;
		}
...
void	sInvVoltHighChk(INT8U ubFilter)
{
	...
	if(swGetWorkMode() == cBatteryMode && (suwGetFBInvCtrlSts() == cFBInvCtrlVolt || suwGetFBInvCtrlSts() == cFBInvOpenLoop))
	{
		...
		{
			if(sb16UOverLevelChk(wRInvVoltNew, cAcVoltReal135V, ubFilter, &s_uwRInvVoltHighCnt))
			{
				ubInvVoltHigh = true;
			}
		}
		else if(sb16UOverLevelChk(wRInvVoltNew, cAcVoltReal280V, ubFilter, &s_uwRInvVoltHighCnt))
		{
			ubInvVoltHigh = true;
		}
...
void	sInvVoltLowChk(INT8U ubFilter)
{
	...
	INT8U ubInvVolt = 1700;
	
	if(swGetWorkMode() == cBatteryMode && (suwGetFBInvCtrlSts() == cFBInvCtrlVolt || suwGetFBInvCtrlSts() == cFBInvOpenLoop))
	{
		if(swGetEEACRange() == cAPL || uwGetModelLV() == true)
		{
			ubInvVolt = cAcVoltReal90V;
		}
		else if(swGetEEACRange() == cUPS)
		{
			ubInvVolt = cAcVoltReal170V;
		}
		else
		{
			ubInvVolt = cAcVoltReal170V;
		}
		if(sb16UUnderLevelChk(wRInvVoltNew, ubInvVolt, ubFilter, &s_uwRInvVoltLowCnt))
		{
			ubInvVoltLow = true;
		}
		if(ubInvVoltLow)
...
void	sSetRInvCurrNew(INT16S wCurr)
{
	...
	//wRInvCurrNew = wCurr;				//for test 20241224
...
void	sSetRInvWattNew(INT16S wWatt)
{
	...
	//wRInvWattNew = wWatt;				//for test 20241224
	if(swGetWorkMode() == cBatteryMode)
	{
		wRInvWattNew = wWatt;
	}
...
void	sSetRInvDCVNew(INT16S wVolt)
{
	static INT16S s_wRes = 0;
	INT32S dwTemp;
	...
	// Fc=a/(2*PI*t)=(1/32)/2/3.1415927/0.02s=0.25Hz
	dwTemp = (INT32S)wRInvDCVNew + (INT32S)wRInvDCVFilter * 31 + (INT32S)s_wRes;
	s_wRes = dwTemp & 0x001F;
	wRInvDCVFilter = dwTemp >> 5;
}

void	sSetRInvDCINew(INT16S wCurr)
{
	static INT16S s_wRes = 0;
	INT32S dwTemp;
	...
	// Fc=a/(2*PI*t)=(1/32)/2/3.1415927/0.02s=0.25Hz
	dwTemp = (INT32S)wRInvDCINew + (INT32S)wRInvDCIFilter * 31 + (INT32S)s_wRes;
	s_wRes = dwTemp & 0x001F;
	wRInvDCIFilter = dwTemp >> 5;
}
