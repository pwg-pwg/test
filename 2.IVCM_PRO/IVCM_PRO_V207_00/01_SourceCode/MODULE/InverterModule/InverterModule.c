...
void	sOPCurrHighChk(INT8U ubFilter)	// OP过流
{
	...
	if(swGetWorkMode() == cBatteryMode || swGetWorkMode() == cBypassMode || swGetWorkMode() == cLineMode)
	{
		...
		if(uwGetModelVar() == cCtrlIVCM1012 || uwGetModelVar() == cCtrlIVCM1012PRO)
		{
		...
		else if(uwGetModelVar() == cCtrlIVCM2024 || uwGetModelVar() == cCtrlIVCM2024PRO)
		{
			if(suwGetRLineChgCurrNew() == cInv20A)
			{
				uwOPCurr = cOp11A;
...
void	sInvVoltHighChk(INT8U ubFilter)
{
	...
	{
		if(sb16UOverLevelChk(wRInvVoltNew, cAcVoltReal280V, ubFilter, &s_uwRInvVoltHighCnt))
		{
			ubInvVoltHigh = true;
		}
