...
* External variables															*
********************************************************************************/
extern INT16S wGRIDChgDischgSwitch;
...
INT16U	g_uwLineChgCurr1 = 1;
INT16U	g_uwLineChgCurr2 = 2;
...
INT16U	g_uwTurboWattRef = 0;

//LLC软起占空比三段限幅参数
INT16U g_wPDCDCPWMSoftDutyFirst  = 0;
INT16U g_wPDCDCPWMSoftDutySecond = 0;
INT16U g_wPDCDCPWMSoftDutyThird  = 0;
...
void	sInvParaUpdate(void)
{
	if(g_uwModelVar == cCtrlIVCM3024)
	{
		...
		g_uwTurboWattRef = 3000;

		g_wPDCDCPWMSoftDutyFirst  = 25;
		g_wPDCDCPWMSoftDutySecond = 27;
		g_wPDCDCPWMSoftDutyThird  = 31;
		sSetInvVoltCtrlParaCurrLimit(cInvVoltCurrLimit3024PRO);
	...
	else if(g_uwModelVar == cCtrlIVCM2024PRO)
	{
		g_uwType = MACHINE_TYPE_IVCM;				// 设备大类
		g_uwSubType = MACHINE_SUB_TYPE_2024PRO;		// 设备子类
		g_uwBatSerialPcs = 2;
		g_uwTransformerRatio = cTransformerRatio165;// 推挽变比
		g_uwInvVAMax = 2000;						// 最大视在功率						
		g_uwInvWattMax = 2000;						// 最大有功功率	
		g_uwMpptBatModule = 2;						// 24V
		g_uwLineChgCurr1 = 1;						// 市电档位 10A
		g_uwLineChgCurr2 = 2;						// 市电档位 20A
		...
		g_uwMpptPvVoltMax = cMpptPvVolt145V;		// 最大PV电压
		g_uwPvVoltRange = 2;						// PV电压范围 1，最大105V, 2,最大145V
		g_uwTurboWattRef = 2000;

		g_wPDCDCPWMSoftDutyFirst  = 24;
		g_wPDCDCPWMSoftDutySecond = 26;
		g_wPDCDCPWMSoftDutyThird  = 30;
		sSetInvVoltCtrlParaCurrLimit(cInvVoltCurrLimit2024PRO);
	...
	else if(g_uwModelVar == cCtrlIVCM2024)
	{
		...
		wInvCtrlPower = -4000;
		wInvIerrLimit = 3000;						// 2500;	// 2500 = 1.22k
		g_uwMpptPvVoltMax = cMpptPvVolt95V;			// 最大PV电压
		g_uwPvVoltRange = 1;						// PV电压范围 1，最大105V, 2,最大145V
		g_uwTurboWattRef = 2000;

		g_wPDCDCPWMSoftDutyFirst  = 31;
		g_wPDCDCPWMSoftDutySecond = 31;
		g_wPDCDCPWMSoftDutyThird  = 31;
		sSetInvVoltCtrlParaCurrLimit(cInvVoltCurrLimit2024PRO);
	...
	else if(g_uwModelVar == cCtrlIVCM1012PRO)
	{
		g_uwType = MACHINE_TYPE_IVCM;				// 设备大类
		g_uwSubType = MACHINE_SUB_TYPE_1012PRO;		// 设备子类
		...
		g_uwPvVoltRange = 1;						// PV电压范围 1，最大105V, 2,最大145V
		g_uwTurboWattRef = 1000;

		g_wPDCDCPWMSoftDutyFirst  = 28;
		g_wPDCDCPWMSoftDutySecond = 30;
		g_wPDCDCPWMSoftDutyThird  = 35;
		sSetInvVoltCtrlParaCurrLimit(cInvVoltCurrLimit1012PRO);
...
void	sInverterRealtimeDataUpdate(void)
{
	...
	strInverterRealtimeData.Field.wPDCDCCurr = 0;
...
INT16U	uwGetTurboWattRef(void)
{
	return g_uwTurboWattRef;
}

INT16U wGetPDCDCPWMSoftDutyFirst(void)
{
	return g_wPDCDCPWMSoftDutyFirst;
}

INT16U wGetPDCDCPWMSoftDutySecond(void)
{
	return g_wPDCDCPWMSoftDutySecond;
}

INT16U wGetPDCDCPWMSoftDutyThird(void)
{
	return g_wPDCDCPWMSoftDutyThird;
}
