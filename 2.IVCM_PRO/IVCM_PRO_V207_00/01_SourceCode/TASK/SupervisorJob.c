		...
		{	
			...
			else if(!g_uwStayStandby)
			{
				...
				else
				{
					...
					if(sbGetPalLineLossStatus() == false && wLineSuddenlyLoss == false \
					&& swGetBatToLineEnDisable() == true && g_wAgingMode == false)
					{
						s_OnInvDelay = 0;
    ...
    while(1)
    {
		...
		if(wEvent & (1 << eSuperToBat))
		{
			...
			if(1)	// 默认认为电池存在
			{
				...
				sSetOPRelayOn(false);
				sSetLoadOnSts(2);
				if((swGetRInvVoltFilter() >= cAcVoltReal110V))
				{
        ...
        if(wEvent & (1 << eSuperTimer))
        {
			...
			if((g_wAgingMode == true) && (suwGetDCDCCtrlSts() == cDCDCWork))
			{
...
//			if((swGetBatToLineEnDisable() == true) && (swGetRLineVoltFilter() >= cLineVolt90V))
			if((swGetRLineVoltFilter() >= cLineVolt90V) && sbGetInverterLinePhaseStatus())
			{	
...
void	sToBatteryMode(void)
{
	...
	INT16U wToBatMode = 0;
	//INT16U uwBatBusVolt;
	INT16U wBusSoftTimeout = 3000;	// 60S
	...
	while(1)
	{
		...
		{
			//if(subGetParaBatOpenSts())
			if(g_uwLoadOnSts == 0 || subGetLowBatCutOffSts() || subGetSOCDischCutoff())
			{
				g_uwWorkMode = cStandbyMode;
				return;
			}
			else
			{
				...
				else if(wToBatMode == 1)
				{
					...
					{
						sSetInvRMSCtrlVolt(cAcVoltReal230V);
