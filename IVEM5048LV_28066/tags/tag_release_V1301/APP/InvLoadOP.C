...
void sPowerPriorityProc(void)
{
	...
	if(!g_uwSolarLoss || g_uniInputStatus.BIT.uwSysSCCOK)
	{
		...
		|| g_fBatDanger || g_wSysLiBatActFlg || fBMSForceChgFlg)
		{
			...
			{
				g_dwBatACChgLimit += 100;//+10A, DCDC control to Max AC change current
			}
		}
		else
		{
...
//	g_dwBatACChgLimit = (g_dwBatACChgLimit * 136) >> 7;	//0.94 efficiency
	...
	if(mChkOPRlyOn())
	{
		g_dwBatChgLimit = (INT32S)cMaxACInputCurr - (INT32S)g_uwROPCurr;
...
//		}
		...
		{
			gi_wBatChgInvLimit = g_dwInvChgLimit;//gi_wBatChgInvLimit = g_dwInvChgLimit = 6773
		}
		else
		{
			gi_wBatChgInvLimit = gi_wInvChgCurrLimit;//gi_wInvChgCurrLimit = 7070
		}
	}
	else
	{
