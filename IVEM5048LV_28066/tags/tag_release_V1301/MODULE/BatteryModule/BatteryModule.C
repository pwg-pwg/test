	...
	//延时10s 
		...
		if(!g_fBatPowerdown && s_uwBatUnderDelayCnt == 0)
		{
			...
			else
			{
				...
				g_wBatPowerDownVoltTemp = wBatUnderVolt - (cBatVoltReal0V5*cBatSerialPcs);//电池低压保护点 - 2V
				if(g_wBatPowerDownVoltTemp < (cBatVoltReal10V5*cBatSerialPcs))	g_wBatPowerDownVoltTemp = (cBatVoltReal10V5*cBatSerialPcs);//最小限定为42V
