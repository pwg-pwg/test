...
INT8U  g_ubBMSFirstCommDone= false;//BMS的首次通信
...
void sBMSDataAnalyze(void)
{
	...
	INT16S wTemp;
	INT16S wComOKCnt = 0;
	INT16S wComOKPal = 0;//通信并联状态
	INT16S wComOKPalFlg = 0;//通讯地址连接状态
	UNIBMSFlg uniBMSFlg;
...
//	g_strBMSCtrlLogicInfo.ubBMSBatStopChargeFlag = false;
    ...
    g_strBMSCtrlLogicInfo.wBMSComPal = 0;
    g_strBMSCtrlLogicInfo.wBMSComFlg = 0;
...
//	g_strBMSRTInfo[cBMSAddrIDMax].wBMSFlag = 0;
    ...
    wComOKPalFlg = wComOKPal;//通讯地址连接状态
    if(wComOKPalFlg)
    {
        wComOKPalFlg |= 1;
    }
	g_strBMSCtrlLogicInfo.wBMSConnectNum = wComOKCnt;
	g_strBMSCtrlLogicInfo.wBMSComPal = wComOKPal;//并联状态
	g_strBMSCtrlLogicInfo.wBMSComFlg = wComOKPalFlg;//通讯地址连接状态
