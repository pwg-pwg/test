...
void sBatUnderChk(INT16U uwFilter)
{
	...
	INT16S wBmsUnderVoltBack = wBmsUnderVolt+(cBatVoltReal1V*cBatSerialPcs);//46V+4=50V
	...
	if(g_uwBatType == cBatVoltType)	
	{
		...
		else
		{
			...
			|| g_wBatChgStage == cBatChgFloatStage /*|| g_fBMSSOCFull*/ || !g_fBatVoltUnder)
			{
...
{
	//if( (g_fBatOpen && !g_wSysLiBatActFlg)|| (!fBMSConnectFlg && g_uwBatType == cBatSocType) || fBMSStopChargeFlg)
	//{
	//	return false;
	//}
	//else
...
{
	//if(g_fBatOpen || g_fBatDanger || g_fBatUnder || fBMSStopDischgFlg || (!fBMSConnectFlg && g_uwBatType == cBatSocType))
	//{
	//	return false;
	//}
	//else
