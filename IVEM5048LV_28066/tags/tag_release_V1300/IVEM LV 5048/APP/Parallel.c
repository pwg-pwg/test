...
void sUpdateLocalData(void)
{
	...
	g_strLocalParameterSet5.wParameterValue = 0;
			...
			for(wIndex = 0; wIndex < cParaMaxSize; wIndex++)
			{
				if(g_strParaExistInfo.BIT.uwSystemExist & (1 << wIndex))
				{
					...
					if(uwPhaseMasterSearch)
					{
						...
						&& (swGetEEParallelEn() > cOP_3P1)&&(swGetEEParallelEn() != cOP_2P1))
						{
...
INT16U swMasterReleaseChk(void)
{
	...
	if(gi_wParallelEnable > cOP_Parallel)
	{
		...
		if((gi_wParallelEnable == cOP_3P1)||(gi_wParallelEnable == cOP_2P1))
		{
		...
		if(g_uwSlaveExsitFlg[wCnt] == 1)
		{
			...
			if(uwMasterReleaseChk == true)
			{
				...
				if(gi_wParallelEnable > cOP_Parallel)
				{
					...
					if((uParaSet1Temp.BIT.uwOPMode == cOP_3P1)||(uParaSet1Temp.BIT.uwOPMode == cOP_2P1))
					{
...
INT16U swChkOPRly(void)
{
	...
	if(gi_wParallelEnable > cOP_Parallel)
	{
		...
		else if(wParaOPWaitOnCnt > 0 && ((uwOPModeOPRlySts == 0x1C)||(uwOPModeOPRlySts == 0x60)))	//11100
		{
			g_uwOPWaitOnChkCnt++;
...
//			}
	...
	uwExcessLoadAvg >>= 8;
	// 3P:0x1C: 00011100         同相并机0x10                单机0x01                      双火线；0x6：0  0110 0000
	if(uwExcessLoadAvg == 0x1C || uwExcessLoadAvg == 0x02 || uwExcessLoadAvg == 0x01|| uwExcessLoadAvg == 0x60)
	{
		wParaExistMachine.BIT.uwSystemMainRlyOn = 1;
	}
	else
			...
			else
			{
				sEncodeCanTxCom(g_strLocalID.BIT.wLocalIDHigh,cNullData,g_strLocalID.BIT.wLocalIDLow,\
						        cParameterSet2,cMasterSend,cNullData,\
				         		...
				         		g_uwSystemData[wCnt][cParamerterSet5],cNullData);	
				sCanWrite9(&g_strCanTxCom);	
				return true;
			}
		}
...
void	sParameterSetSave(void)
{
	...
	if(g_strLocalParameterSet1Temp.BIT.uwOutputVolt == 0 && swGetEEOutputVolt() != 1100)
	{
		sSetEEOutputVolt(1100);
		bSaveFlag = 1;
	}
	else if(g_strLocalParameterSet1Temp.BIT.uwOutputVolt == 1 && swGetEEOutputVolt() != 1200)
	{
		sSetEEOutputVolt(1200);
		bSaveFlag = 1;
	}
	else if(g_strLocalParameterSet1Temp.BIT.uwOutputVolt == 2 && swGetEEOutputVolt() != 1270)
	{
