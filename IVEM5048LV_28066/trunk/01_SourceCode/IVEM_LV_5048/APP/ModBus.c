...
INT16U swWriteEESetting(INT16U WrAddr, INT16U WrData)
{
	...
	switch(WrAddr)
	{
		...
		break;
		case E2_OutputVolt:
			if((WrData == 1100) || (WrData == 1200) || (WrData == 1270))
			{
...
//				}
		...
		else//5~95之间且是5的倍数
		{
			if((swGetEEBatWeakSoc() != WrData))
			{
				...
				if( swGetEEBatUnderSoc()  > swGetEEBatWeakSoc())//检查电池低于 SOC 的值是否大于电池弱 SOC 的值//电池截止电量＞电池低电量转充电
				{
					sSetEEBatUnderSoc(swGetEEBatWeakSoc());//更新电池截止电量
				}
				
				if((swGetEEBatWeakSoc() + 5) > swGetEEBatWeakBackSoc())//电池低电量转充电+5>电池高电量转放电
				{
...
//				OSEventSend(cPrioInterface, eEepromSaveLoadEnergy);
		...
		case ATE_LineVoltAdj1:
			...
			if((swGetEEDSPRLineVoltAdj() == 2048) && (wTemp1 >= -200) && (wTemp1 <= 200) \
				&& ((INT16S)WrData >= 900) && ((INT16S)WrData <= 1400))
			{
...
INT16U swWriteIOTData(INT16U WrAddr, INT16U WrData)
{
	...
	switch(WrAddr)
	{
		...
		case IOT_SN1:
				strIOTDataStruct.wSIM_SN[0] = WrData;
		break;

		case IOT_SN2:
				strIOTDataStruct.wSIM_SN[1] = WrData;
		break;

		case IOT_SN3:
				strIOTDataStruct.wSIM_SN[2] = WrData;
		break;

		case IOT_SN4:
				strIOTDataStruct.wSIM_SN[3] = WrData;
		break;

		case IOT_SN5:
				strIOTDataStruct.wSIM_SN[4] = WrData;
		break;

		case IOT_SN6:
				strIOTDataStruct.wSIM_SN[5] = WrData;
		break;

		case IOT_SN7:
				strIOTDataStruct.wSIM_SN[6] = WrData;
		break;

		case IOT_SN8:
				strIOTDataStruct.wSIM_SN[7] = WrData;
		break;

		case IOT_SN9:
				strIOTDataStruct.wSIM_SN[8] = WrData;
		break;

		case IOT_SN10:
				strIOTDataStruct.wSIM_SN[9] = WrData;
		break;

		case IOT_SN11:
				strIOTDataStruct.wSIM_SN[10] = WrData;
		break;

		case IOT_SN12:
				strIOTDataStruct.wSIM_SN[11] = WrData;
		break;

		case IOT_SN13:
				strIOTDataStruct.wSIM_SN[12] = WrData;
		break;

		case IOT_SN14:
				strIOTDataStruct.wSIM_SN[13] = WrData;
		break;

		case IOT_SN15:
				strIOTDataStruct.wSIM_SN[14] = WrData;
		break;

		case IOT_SN16:
				strIOTDataStruct.wSIM_SN[15] = WrData;
		break;
...
void sRealTimeDataUpdate(void)
{
	...
	uRealTimeData.Stru.wRInvWatt = (INT16S)(g_dwRInvWatt);
	...
	uRealTimeData.Stru.wROpWatt = (INT16S)g_dwOPWatt;
	...
	uRealTimeData.Stru.wRLoadWatt = (INT16S)g_dwOPWatt;
...
{
//    INT16U BMScp1 = 0;//调用SN2码的最后一位数字
//    INT16U BMScp2 = 0;//调用SN3的前两个数字
    INT32U udwBMScp = 0;//合在一起当电池容量
    INT16U uwBMSFlag = 0;
    INT16U uwTemp = 0;


    uRealTimeBMSData.Stru.wCurrBMSAddr = wCurrBMSAddr;//当前电池包通信地址
    uRealTimeBMSData.Stru.wBMS_Flg = g_strBMSCtrlLogicInfo.wBMSComFlg;//通讯地址连接状态
    ...
    //充电允许 bit0  强制充电 bit1 放电允许bit2  输出软起标志 bit4 SOC定时校准bit5   充电状态bit6  充电状态bit7
    if(uwTemp & (1<<0))
    {
        uwBMSFlag |= 0x0080;
    }
...
//    uRealTimeBMSData.Stru.wBmsStatusLo = g_strBMSRTInfo[wCurrBMSAddr].wBMSFlag;
    ...
    udwBMScp = ((INT32U)((g_strBMSBaseInfo[wCurrBMSAddr].wSerialNum2%10)*100 + g_strBMSBaseInfo[wCurrBMSAddr].wSerialNum3/100))*1000;
    uRealTimeBMSData.Stru.wTotalCapacityHigh = (INT16U)((udwBMScp >> 16) & 0xFFFF);//高十六位
    uRealTimeBMSData.Stru.wTotalCapacityLow = (INT16U)(udwBMScp & 0xFFFF);//低十六位

    uRealTimeBMSData.Stru.wParallelNumber = g_strSysBMSCtrlInfo.uniBMSInfo.BIT.uwBMSNum;
    uRealTimeBMSData.Stru.wParallelStatus = g_strBMSCtrlLogicInfo.wBMSComPal;
    ...
    uRealTimeBMSData.Stru.MaximumCellTemperature = g_strBMSRTInfo[wCurrBMSAddr].wBMSBatCellTempH*10;
    uRealTimeBMSData.Stru.MinimumCellTemperatureNo = g_strBMSRTInfo[wCurrBMSAddr].wBMSBatCellTempLNum;
    uRealTimeBMSData.Stru.MinimumCellTemperature = g_strBMSRTInfo[wCurrBMSAddr].wBMSBatCellTempL*10;
