...
extern INT8U g_ubBMSFirstCommDone;
...
void sBuzzerProc(void)
{
	if(swGetEEBuzzEn())
	{
		...
		else if(  (uniWarningCode.BIT.uwBatLow && g_uwWorkMode != cStandbyMode /* && swGetEEBatteryType() != cBatType_LIB && !fBMSConnectFlg*/)
...
void sLEDDisplay(void)
{
	...
	else
	{
		...
		if((g_uwWorkMode == cFaultMode)||((ubInverterWarningLEDFlash /*|| (fBMSConnectFlg && ubBMSWarningLEDFlash)*/) && (wLEDDispalyCnt > 4)))
		{
			hoFAULTLEDOn;
		}
		else
...
{ 
	...
	{		
	    ...
	    sSendBitToVram(cBIT_IVEM3048LV, DISPLAY_ON, cALWAYS_LIGHT);
#elif mModel == cModel5048
		sSendBitToVram(cBIT_IVEM5048LV, DISPLAY_ON, cALWAYS_LIGHT);
#endif
...
//	}
	...
	//WIFI状态
	//0：如果连上服务器，WIFI常亮;
	//1：如果IOT设备没有与MCU建立连接，WIFI灭:
	//2：如果连接错误，WIFI亮1秒，灭2秒;
	//3：如果正在连接，WIFI亮1秒，灭2秒;
	//4：如果连上路由器，WIFI亮1秒，灭1秒;
	//5：如果正在升级IOT设备程序，WIFI亮0.5秒，灭0.5秒，
	//注：特殊/异常状态全改为闪亮
	if(swGetIOTState() == 3)
	{
...
void sDisplayVersionNum2(void)
{
	sFigGroupDisplayNum(FIG4_7SEG , 1, char0, cALWAYS_LIGHT);
	sFigGroupDisplayNum(FIG5_7SEG , 1, char2, cALWAYS_LIGHT);
	sFigGroupDisplayNum(FIG6_7SEG , 3, cDSPVersion2%1000, cALWAYS_LIGHT);
	if(cDSPVersion2 / 100 == 0)
	{
		sFigGroupDisplayNum(FIG6_7SEG , 1, 0, cALWAYS_LIGHT);
	}
	if(cDSPVersion2 / 10 == 0)
	{
		sFigGroupDisplayNum(FIG7_7SEG , 1, 0, cALWAYS_LIGHT);
	}
}
	...
	do
	{
		if(ubChkCnt == FW_ParallelMode)
		{
			if(swGetEEParallelEn() != cOP_Single)
			{
				...
				//双火线添加
				else if(swGetEEParallelEn() == cOP_2P1)
				{
					mSetVramBit(cBIT_MIDNUM_FRM, cALWAYS_LIGHT);
					sFigSpecialDisplayLetter(charP, char1, cALWAYS_LIGHT);
				}
				else if(swGetEEParallelEn() == cOP_2P2)
				{
					mSetVramBit(cBIT_MIDNUM_FRM, cALWAYS_LIGHT);
					sFigSpecialDisplayLetter(charP, char2, cALWAYS_LIGHT);
				}
		...
		{
			if(!fBMSConnectFlg && g_uwBatType == cBatSocType && g_ubBMSFirstCommDone)//加了g_ubBMSFirstCommDone，80故障上电延时的问题
			{
...
void sSettingPageOutputVolt(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	INT8U ubDisplayBuff[3][3] = {{char1,char1,char0},
								 {char1,char2,char0},
								 {char1,char2,char7}};
	INT8U ubDisplayTime;
	INT16S wSettingValue;
...
void sSettingPageParallel(INT8U* pubOprtStep, INT16S* pwChangeValue)
{
	INT8U ubDisplayBuff[7][3] = {{charS,charI,charG},
								 {charP,charA,charL},
								 {char3,charP,char1},
								 {char3,charP,char2},
								 {char3,charP,char3},
	...
	*pwChangeValue = 0;
	...
	if(subSettingValueOprt(pubOprtStep, pwChangeValue, &wSettingValue, &s_wSettingValueTemp, &ubDisplayTime, 6, 0))
	{
