...
INT16S Test_InnelTjDeratePerTemp;
...
const pFunc1	GetDataSubArray[100]=
{
	...
	swGetILLCAvgCurrSample,	swReturnNull,				swReturnNull,							//24-26
	...
	sdwGetInvVoltCtrlVoltLimit,			swReturnNull,				swReturnNull,							//90-92
		...
		if(event&(1<<eInterfaceTimer))
		{
			...
			else
			{
				...
				if(g_uwBatType == cBatSocType)
				{
					...
					if(g_ubBatProcotolPre != swGetEEBatteryType())
					{
					...
					g_ubBatProcotolPre = swGetEEBatteryType();
				}
				else
				{
					if(bRS485EventEn)
...
//9600波特率传输1字节需要1.041667ms,一帧数据最长69字节，需要至少71.8ms
//任务执行周期50ms,一帧数据可能需要两次透传，且间隔时间50ms
//需要确认是否会超过BMS对一帧数据的超时判断
//考虑DSP接收完一帧（帧超时5ms）再触发事件去做透传
INT8U sBMSUpdateProc(void)
{
    INT16U uwSciTemp;
...
//	wSciLength += 4;			
	...
	wSciLength += 5;	
	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	sNumToASCII(g_wPv1CurrLimit, 2, 2, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 5;	
	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	sNumToASCII(g_uwSolarPower1Avg, 5, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 5;	
	g_ubUserDataBuf0[sciid][wSciLength++] = cChar_Space;
	sNumToASCII(g_dwTotalPower, 5, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 5;
	...
	sNumToASCII(uniEnergyInfo.uwEnergyInfo, 4, 0, &g_ubUserDataBuf0[sciid][wSciLength]);
	wSciLength += 4;
//	g_ubUserDataBuf0[sciid][wSciLength++] = 'S';
...
INT16U g_uwDCDCConvPeriod = 410;//41K
...
void sTestCommand(INT8U sciid)
{
	...
	if((g_ubCommandBuffer[sciid][1] == 'S') && (g_ubCommandBuffer[sciid][2] == 'T'))
	{
		...
		&& (g_ubCommandBuffer[sciid][5] == 'F') && (g_ubCommandBuffer[sciid][6] == '?'))//GWF?
		{
			g_ubUserDataBuf0[sciid][0] = '(';
		...
		else if((g_ubCommandBuffer[sciid][3] == 'D') && (g_ubCommandBuffer[sciid][4] == 'B'))
		{
		...
		&& (g_ubCommandBuffer[sciid][5] == 'F'))//GWF
		{
		...
		&& (g_ubCommandBuffer[sciid][5] == '1') && (g_ubCommandBuffer[sciid][6] == '?'))//CH1?
		{
			g_ubUserDataBuf0[sciid][0] = '(';
		...
		&& (g_ubCommandBuffer[sciid][5] == '1'))//CH1
		{
			wTemp = (g_ubCommandBuffer[sciid][6] - '0') * 10 + (g_ubCommandBuffer[sciid][7] - '0'); 
		...
		&& (g_ubCommandBuffer[sciid][5] == '2') && (g_ubCommandBuffer[sciid][6] == '?'))//CH2?
		{
			g_ubUserDataBuf0[sciid][0] = '(';
		...
		&& (g_ubCommandBuffer[sciid][5] == '2'))//CH2
		{
			wTemp = (g_ubCommandBuffer[sciid][6] - '0') * 10 + (g_ubCommandBuffer[sciid][7] - '0'); 
		...
		&& (g_ubCommandBuffer[sciid][5] == '3') && (g_ubCommandBuffer[sciid][6] == '?'))//CH3?
		{
			g_ubUserDataBuf0[sciid][0] = '(';
		...
		&& (g_ubCommandBuffer[sciid][5] == '3'))//CH3
		{
			wTemp = (g_ubCommandBuffer[sciid][6] - '0') * 10 + (g_ubCommandBuffer[sciid][7] - '0'); 
		...
		&& (g_ubCommandBuffer[sciid][5] == '4') && (g_ubCommandBuffer[sciid][6] == '?'))//CH4?
		{
			g_ubUserDataBuf0[sciid][0] = '(';
		...
		&& (g_ubCommandBuffer[sciid][5] == '4'))//CH4
		{
			wTemp = (g_ubCommandBuffer[sciid][6] - '0') * 10 + (g_ubCommandBuffer[sciid][7] - '0'); 
		...
		&& (g_ubCommandBuffer[sciid][5] == 'T'))//ICT
		{
...
//			&& (g_ubCommandBuffer[sciid][5] == 'I') && (g_ubCommandBuffer[sciid][6] == 'K')//FBIKI
...
//		}
//		else if((g_ubCommandBuffer[sciid][3] == 'F') && (g_ubCommandBuffer[sciid][4] == 'B')
//			&& (g_ubCommandBuffer[sciid][5] == 'I') && (g_ubCommandBuffer[sciid][6] == 'K')//FBIKV
