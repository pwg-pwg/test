...
{
	sInitInvVoltCntlPara();//第一次赋值
	sSetInvVoltCtrlParaKv(g_wKv);
	sSetInvVoltCtrlParaKi(g_wKi);
	sSetInvVoltCtrlParaCurrLimit(g_wInvVoltCurrLimit);//再次赋值
...
//		g_wInvPWM = g_wLoopOutput + 75;//+ 75;// + 450;
		g_wInvPWM = g_wLoopOutput + Test_DBCnt;// + 450;
...
//		mDisForceRPPWMLow();
	...
	else                
	{
		...
		g_wInvPWM = g_wInvPWM +Test_DBCnt;// 75;// + 450;
//		if(g_wInvPWM > (g_wPwmPeriod -2)) g_wInvPWM = (g_wPwmPeriod -2);		
...
//	INT32S	dwBatDishgLimit;
	...
	{
		dwChgLimit = gi_wChgCurrLimit;//1333
	}
	else
	{
	...
	{
		dwDischgLimit = gi_wDischgCurrLimit;//2400
	}
	else
	{
	...
	if(++g_wLoopCnt >= 8)
	{
		...
		{
			g_wBatVoltErr = g_wBatVoltAvg - g_wBatVoltRef;//-84 = (480 - 564) < -40
			if(g_wBatVoltErr > 10)
			{
				if(g_wBatVoltErr > 25)
				{
		...
		if(g_wPDCDCPWM < 0 && g_wBatVoltErr > g_wBusVoltErr)// g_wBatVoltErr = -462 , g_wBusVoltErr = [-16, 16]
		{
	...
	{
		dwChgLimit = 0;
		// if(g_wPBusVoltAvg > 4600) dwChgLimit = 205;//测试BUS升高大于460V时，开启BUCK电路将BUS控制下来
	}
	if(gi_wDCDCChgDischgEnDisable & cDCDCDischgEnable)
	{
...
//		if(g_wDCDCDirection != 0 || g_wDCDCOverCurrCnt > 0 || g_wBusOverVoltCnt > 0)
		...
		|| !(gi_wDCDCChgDischgEnDisable & cDCDCDischgEnable) || g_uwFaultCode != 0)
		{
			...
			{
				if(sdwGetDCDCCtrlBusVolt_I() > 0) sSetDCDCCtrlBusVolt_I(sdwGetDCDCCtrlBusVolt_I()>>1); //触发过流封驱动，积分减半
