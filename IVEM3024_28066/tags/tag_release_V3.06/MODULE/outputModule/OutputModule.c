...
void sOPPercentCal(void)
{
	...
	{
		g_wGridCurrPercent = (INT16S) ((INT32S)g_uwRGridCurr * 100 / cGridRateCurr );
		//g_wGridCurrPercent = (INT16S) (g_uwRGridCurr * 67 / cMaxACInputCurr); 	//100/1.5
		//g_uwRGridCurr * 67 / cMaxACInputCurr / g_dwOPMaxPower
...
//	}
	...
	//市电模式下，市电基本不充电时(市电全供给   输出，充电电流限制基本0),才采用市电电流负载百分比，
	//避免带市电带轻载、小载时，大部分功率现在在充电，功率显示了市电带载百分比，非输出负载率，负载率不显示
	//if(g_uwWorkMode == cBypassMode || (g_uwWorkMode == cLineMode && g_dwOPMaxPower > 3000))
	if(g_uwWorkMode == cBypassMode || (g_uwWorkMode == cLineMode && g_dwInvChgLimit < 100))
	{
...
void sOverLoadChk(void)
{
	...
	else
	{
		...
		if( g_uwOverLoadLevel1Cnt >= 20 || g_uwOverLoadLevel2Cnt >= 1050)
		{
			g_uwFaultCode = cOverLoad;
			g_uwOverLoadLevel1Cnt = 0;
			g_uwOverLoadLevel2Cnt = 0;
