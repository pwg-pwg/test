...
INT16S	g_wDCDCCurrAvg;
INT16S	g_wChgCurrAvg;
...
void sBatUnderChk(INT16U uwFilter)
{
	...
	static INT16U s_uwBatPowerDownChkCnt = 0;
	static INT16U s_uwBatUnderDelayCnt = 0;
	INT16U g_wBatPowerDownVoltTemp = 0;
	...
	//其他模式下 -- 低压返回点为
	//①(浮充电压 - 0.4V) 
	//②高于g_wBatWeakBackVolt时，取值g_wBatWeakBackVolt
	//③不低于48V
	...
	if(!g_fBatPowerdown && s_uwBatUnderDelayCnt == 0)
	{

		if(fBMSConnectFlg)
		{
			g_wBatPowerDownVoltTemp = mBMSBatCutOffVolt;
		}
		else
		{
			//无通信的情况下：低压点小于2V，关闭辅源
			g_wBatPowerDownVoltTemp = wBatUnderVolt - (cBatVoltReal0V5*cBatSerialPcs);
			if(g_wBatPowerDownVoltTemp < (cBatVoltReal10V5*cBatSerialPcs))	g_wBatPowerDownVoltTemp = (cBatVoltReal10V5*cBatSerialPcs);

		}
		//BMS通信正常不关机
		if( !fBMSConnectFlg && sbUnderLevelChk(g_uwBatVoltAvg, g_wBatPowerDownVoltTemp, uwFilter, &s_uwBatPowerDownChkCnt))
		{
			g_fBatPowerdown = 1;
		}
	}
	...
	{
		g_wBatPowerDownVoltTemp = g_wBatUnderBackVolt - (cBatVoltReal0V5*cBatSerialPcs);
		if(g_wBatPowerDownVoltTemp < (cBatVoltReal10V5*cBatSerialPcs))	g_wBatPowerDownVoltTemp = ((cBatVoltReal10V5*cBatSerialPcs) + (cBatVoltReal0V5*cBatSerialPcs));
		//恢复值为电池低压的恢复点-2V
		if(sbOverLevelChk(g_uwBatVoltAvg, g_wBatPowerDownVoltTemp, uwFilter, &s_uwBatPowerDownChkCnt)
		|| g_wBatChgStage == cBatChgFloatStage || g_fBMSSOCFull || !g_fBatUnder)
		{
			s_uwBatPowerDownChkCnt = 0;
			g_fBatPowerdown = 0;
		}
	}
	...
	if(g_fBatUnder && g_uniInputStatus.BIT.uwBatPowerDown)
	{
		if(!g_fBatPowerdown)
		{
			g_fBatPowerdown = 1;
