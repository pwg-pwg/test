...
void sSolarVolt1Chk(INT16U uwFilter)
{
	...
	if(g_uwSolarVoltChkEn)
	{
		...
		{
			if(sbOverLevelChk(g_uwSolarVolt1Avg, (g_uwSolarLowLossPoint + 200), \
...
//	g_wKCurrent1Forward = 48;  //100(Ð¡ÔØ²»ÎÈ¶¨)
//	g_wPv1CurrLimit = 187 * 8;
//
//	g_wKVoltage2Forward = 128;//16
//	g_wKCurrent2Forward = 48;  //100(Ð¡ÔØ²»ÎÈ¶¨)
//	g_wPv2CurrLimit = 187 * 8;
	...
	g_wKVoltage1Forward = cCntl_PVMPPTKp;
	g_wKCurrent1Forward = 5;  //100(ÃÂ¡Ã¯Â¿Â½Ã˜Â²Ã¯Â¿Â½Ã¯Â¿Â½ÃˆÂ¶Ã¯Â¿Â½)

	g_wPv1CurrLimit = 3000;

	g_wKVoltage2Forward = cCntl_PVMPPTKp;
	g_wKCurrent2Forward = 5;  //100(Ð¡ï¿½Ø²ï¿½ï¿½È¶ï¿½)

	g_wPv2CurrLimit = 3000;
...
void sBoostControl(INT16U wControlMode)
{
	...
	if(g_wCnt1Loop++ == 7)
	{
		...
		g_wBusVoltVerr = g_wPV1KeepBusVRef - g_wBUS1Avg; 				//control bus voltage
	...
	if(g_wR_Vcmp_P1 > 3686)   g_wR_Vcmp_P1 = 3686;		//4096 * 90%
	if(g_wR_Vcmp_P1 < 0 )	   g_wR_Vcmp_P1 = 0;
	g_wR_Vcmp_P1_2 = g_wR_Vcmp_P1;

	g_wR_PWMDuty_P1 = (INT16S)(((INT32S)g_wR_Vcmp_P1 * g_wPwmPeriod)>>12);		// 4096
...
void sBoost2Control(INT16U wControlMode)
{
	...
	if(g_wCnt2Loop++ == 7)
	{
		...
		g_dwBUS2AvgSum = 0;
		if(wControlMode != cBoostMppt)
		{
			g_wPV2KeepBusVRef = g_wDCDCBusVoltRef + cBusVoltReal20V;
			g_wVerr_P2 = g_wPV2KeepBusVRef - g_wBUS2Avg; 		//control bus voltage
			//g_wKVoltage2Forward = 128;
			//g_wKCurrent2Forward = 48;
		}
		else
		{
			g_wVerr_P2 = g_wPv2RealAvg - g_wSolarVolt2Ref;             //controll Upv voltage
			//g_wKVoltage2Forward = 32;
			//g_wKCurrent2Forward = 16;
		}
		if(g_wVerr_P2 > 300) g_wVerr_P2 = 300;
		if(g_wVerr_P2 < -300) g_wVerr_P2 = -300;
		g_wVerr_P2 = (INT16S)(((INT32S)g_wVerr_P2 * g_wKVoltage2Forward) >> 5);
		...
		//g_wVerr_P1_1 = g_wVerr_P1;
		//if(g_wVm_P1 > 15*80)   g_wVm_P1 = 15*80;
		if(g_wVm_P2 > cBoostCntlVmoLimit)   g_wVm_P2 = cBoostCntlVmoLimit;
	...
	g_wR_Is_P2 = mPVCurrReal((gi_uwSolarCurr2Sample << 3)) / 10;
	...
	if(g_wR_Vcmp_P2 > 3686)   g_wR_Vcmp_P2 = 3686;		//4096 * 90%
	if(g_wR_Vcmp_P2 < 0 )	   g_wR_Vcmp_P2 = 0;
	g_wR_Vcmp_P2_2 = g_wR_Vcmp_P2;

	g_wR_PWMDuty_P2 = (INT16S)(((INT32S)g_wR_Vcmp_P2 * g_wPwmPeriod)>>12);		// 4096
	g_wR_Boost2_PWM = g_wR_PWMDuty_P2;
//	if(g_wR_Boost2_PWM < 1)	g_wR_Boost2_PWM = 1;								
//	if(g_wR_Boost2_PWM > g_wPwmPeriod)	g_wR_Boost2_PWM = g_wPwmPeriod;
	...
	mPV2BOOSTPWM = g_wPwmPeriod - g_wR_Boost2_PWM;
}
...
void sSolarPowerAbnormalChk(void)
{
	...
	if(suwGetBoost1CtrlSts() >= cBoostMppt)
	{
	    ...
	    if(   (g_uwSolarCurr1Avg > g_wPv1CurrLimit+100)
			//&&(g_wR_Boost1_PWM == 1)
			&&((INT32S)g_uwSolarPower1Avg > (g_dwTotalPower+500))
		)//¶à1AÇÒPWM´ïµ½¼«ÏÞ
	    {
...
INT16S swGetImo_P1(void)
{
	return g_wImo_P1;
}

INT16S swGetR_Is_P1(void)
{
	return g_wR_Is_P1;
}
...
INT16S swGetVerr_P1(void)
{
	return g_wVerr_P1;
}


INT16S swGetVm_P(void)
{
	return g_wVm_P1;
}
