...
INT16U g_uwBMSCmdBaseInfoFlag = 0xFFFF;
...
INT8U  g_ubBMSFirstCommDone= false;
...
//		else 
		...
		else
		{
			...
			if(g_uwBMSCmdBaseInfoFlag & (1<<g_uwBMSCmdAddrID))
            {
			    g_strBMSComSts[g_uwBMSCmdAddrID].fBMSCmdIndex = g_wBMSCMDRollIndex;
			}
			else
            {
                g_uwBMSCmdBaseInfoFlag |= (1<<g_uwBMSCmdAddrID);
                g_strBMSComSts[g_uwBMSCmdAddrID].fBMSCmdIndex = g_wBMSCMDStartIndex;
            }
			g_uwBMSCmdAddrID++;
		}
...
void sBMSDataAnalyze(void)
{
	...
	INT16S wTemp;
	INT16S wComOKCnt = 0;
	INT16S wBMSParaStatus = 0;   //串/并联状态
	INT16S wBMSComOKFlg = 0;     //通讯地址连接状态
	UNIBMSFlg uniBMSFlg;
...
//	g_strBMSCtrlLogicInfo.ubBMSBatStopChargeFlag = false;
	...
	g_strBMSCtrlLogicInfo.wBMSParallelStatus = 0;
	g_strBMSCtrlLogicInfo.wBMS_Flg = 0;
...
//	g_strBMSRTInfo[cBMSAddrIDMax].wBMSFlag = 0;
	...
	else
	{
		for(wCnt = 0; wCnt <= cBMSAddrIDMax; wCnt++)
		{
			if(g_strBMSComSts[wCnt].fBMSComOK)
			{
				wComOKCnt++;
				g_strBMSCtrlLogicInfo.ubBMSConnect = true;
				wBMSParaStatus |= (1<<(wCnt));//记录电池串/并联状态
    ...
    wBMSComOKFlg = wBMSParaStatus;  //通讯地址连接状态
    if(wBMSComOKFlg)    			//只要有电池包连接，bit0置1
    {
        wBMSComOKFlg |= 1;
    }
	g_strBMSCtrlLogicInfo.wBMSConnectNum = wComOKCnt;
	g_strBMSCtrlLogicInfo.wBMSParallelStatus = wBMSParaStatus;
	g_strBMSCtrlLogicInfo.wBMS_Flg = wBMSComOKFlg;
