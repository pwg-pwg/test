...
void sBMSParaInit(void)
{
	...
	sBMSDataAnalyze(); //所有id基础信息清除后，再进行解析一次清除标志位
}
...
//		else 
		...
		if(g_strBMSComSts[g_uwBMSCmdAddrID].fBMSFwVerErr) // �����汾�쳣��ֱ�Ӳ�ѯ��һID
		{
			g_strBMSComSts[g_uwBMSCmdAddrID].fBMSComOK = false;
			g_strBMSComSts[g_uwBMSCmdAddrID].fBMSCmdIndex = cBMSCmd_BaseInfo;
			g_uwBMSCmdAddrID ++;
		}
		else if(g_strBMSComSts[g_uwBMSCmdAddrID].fBMSCmdIndex < (cBMSCmd_Max - 1)) // ��ѯ��ID����������
		{
	...
	if(uwBMSCmdAddrIDTemp != g_uwBMSCmdAddrID)
	{
		...
		{	
			...
			{
				g_uwBMSCmdAddrID = 0;
				s_uwComOKFlgPre = s_uwComOKFlg; //记录通信状态标�?
				s_uwComOKFlg = 0;               //重置通信状态标�?
				if(s_uwCycleNoOkChkEn)
				{
					s_uwCycleNoOkChkEn = false;
				}
				uwTemp = s_uwCycleNoOkChkID + 1; //下一个ID
				if(uwTemp > cBMSAddrIDMax)
				{
					uwTemp = 0;
					if(g_uwBMSPollingNum++ > 3) { fBMSConnectFlg = 0; g_uwBMSPollingNum = 4; } //If the total number of polls is greater than 3, the BMS is considered disconnected（fBMSConnectFlg = 0�?
				}
				for(uwBMSCmdAddrIDTemp = 0; uwBMSCmdAddrIDTemp <= cBMSAddrIDMax; uwBMSCmdAddrIDTemp++)
				{
					if(g_strBMSComSts[uwBMSCmdAddrIDTemp].fBMSComOK)
					{
						...
						if(uwTemp == uwBMSCmdAddrIDTemp)         //  更新临时变量，更新下一个ID
						{
							uwTemp ++;
						}
					}
