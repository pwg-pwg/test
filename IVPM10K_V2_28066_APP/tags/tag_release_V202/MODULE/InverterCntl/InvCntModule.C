...
INT16S wInvCrrentLimit = 30;	// Œ¥ πÔøΩÔøΩ
...
INT16S g_wInvCurrLimitRated = 43;	// Œ¥ πÔøΩÔøΩ
...
//	}
	...
	// INT16S wRSintemp;
	if(wRSinAmp<640){wRSinAmp=640;} //Vpeak=20V*32  =640	»∑±£»Ì∆∆ ºµÁ—π
	...
	wRVBeforeSaturation = wVoltLoopTemp + wRLoadCurrCntl_1;//wRKv = 10;  

	if(hoOP_RLY == 1 && mEepromOutputMode() != cOP_Single)
	{
	...
	dwRK1 = (dwRK1*dwRK1_4 + dwRK1_1*dwRK1_3)>>8;
	...
	//dwRVoltLimit = dwRK1  +wRdeltaVref;  //÷±ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ1V->32 µÔøΩÔøΩ÷ªÔøΩ‹µÔøΩÔøΩÔøΩ0.2V,192->1.2V
	//dwRVoltLimit = dwRK1  + wRInvVoltCntl_1;  //Q5    wRInvVoltCntl; //wRInvVoltCntl   wRKi=250
...
//	if((mEepromOutputMode() == cOP_Single)||(mGetEEKpKiParameter ==5))//CC DeLETE
	if((mGetEEKpKiParameter ==5)&&(mEepromOutputMode() == cOP_Single))//ËÆæÁΩÆÔø??5VÊó∂Ôºå‰∏∫ÂºÄÁéØËæìÔø??
	{
	...
	else                
	{
		...
		if(wInvVoltReal<(-500))//-50V 
		{
			if(++IsoReverseCnt>50)//
			{
				...
				if(++IsoReverseChkCnt>60)//  60/2=30  Ôø???0‰∏™Â∏ÇÁîµÂë®Êúü‰ª•ÂêéÊâçÊ£ÄÔø???
				{
					IsoReverseChkCnt =60;
		...
		wR_PWMTemp += wDCVoltSet;
		if(wR_PWMTemp > (wPwmPeriod - 2))   wR_PWMTemp = wPwmPeriod - 2;
		EPwm2Regs.CMPA.half.CMPA = wPwmPeriod - 1;//1;
		EPwm1Regs.CMPA.half.CMPA = wPwmPeriod - wR_PWMTemp;//wR_PWMTemp;
...
void sFBINVPFCInitial(void)
{
		...
		|| (g_wVAType == cb10KVAModel))
	{
		...
		wFBKCurrentFeedback = 7;  //7*ÔøΩÔøΩ–µÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ»£Ôø??100ÔøΩÔøΩ=700ÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩŒ™700 
		...
		wRKvBase = wRKv = 58;//48;//23;//10;   //48;             //Q7  0.18*128 
		wRKiBase = wRKi = 380;//305;//320;//300;//274;   //250;           //Q7  8*128//  
		wMovePoint = 0;
		wDeltaK=0;
		wCCKi = 64;            //2*32
		wChgKp = 2000;
...
void sFBINVController2(void)
{
	...
	{
		wR_PWM = wChgPwmLimit;
		wTemp1 = (INT16S)(((INT32S)wPwmPeriod * 973) >> 10);  //2343*973/1024=2226
		if(wR_PWM >  wTemp1) wR_PWM =  wTemp1;
		if(wR_PWM < -wTemp1) wR_PWM = -wTemp1;
		wPwmTemp = (INT16S)(((INT32S)wR_PWM * pSinTab[wCtrlSinpointer] >> 14));
		...
		if(wLineVoltReal < 0)
		{
			...
			wR_PWMTemp = abs(wPwmTemp);
	       	...
	       	EPwm2Regs.CMPA.half.CMPA = wPwmPeriod - wR_PWMTemp;	//L  wR_PWMTemp
			EPwm1Regs.CMPA.half.CMPA = wPwmPeriod - 1;			//N	
			if(wLPWMForwardFlg != 0)
			{
				wLPWMForwardFlg = 0;
	...
	else
	{
		...
		***************************************************************************/
		...
		if(++wFBCntLoop&0x08)                    
		{
			...
			wVerr_P=(INT16S)(((INT32S)229*wVerr_P_1+(INT32S)27*wVerr_P)>>8);  //Êª§Ê≥¢
			wVerr_P_1=wVerr_P;	
			...
			wFBP_VoltPIAdjCtl =(INT32S)((INT32S)wFBP_VoltPIAdjCtl*3+ wVm_P)>>2;// ÂÅèÂ∑ÆÔø??? Êó∂ÔºåÂâçÈ¶à‰∏çÂèóÔø???
		...
		*  µÁ¡˜ª∑√ø1∏ˆµ„“ª¥Œ
		***************************************************************************/
		wLineVoltCntlQ5=(INT16S)((INT32S)gi_wLineVoltQ5_0+(INT32S)(abs(wLineVoltCntlQ5+200))>>1);// 200 «µÁ—π≤…—˘∆´÷√≤π≥•
		...
		if(wBatVoltReal>0)
		{
			...
			wR_Is_P =abs(wInvLCurrReal);//0.1V  //–Ë“™º”»ÎµÁ—π≈–∂œ¥¶¿ÌµÁ¡˜π˝≥‰
		}
		else
		{
			wR_Is_P = 0;
...
//			wKCurrentForward =wFB_CurPI_KpTest;
			...
			//if(abs(wInvLCurrSample1) > 1000)//32A ËøáÊµÅ‰øùÊä§
			{
			//	mDisInvPwmOut();
			}
			// ‰≥ˆPWM
