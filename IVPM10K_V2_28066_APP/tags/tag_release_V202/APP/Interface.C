...
INT8U	bExtSccMode;/*Only*/
INT8U	fExtSccPvLost;
INT16S	wExtSccOkFlag;
INT16S	wExtSccBatType;/*Only*/
INT16S	wExtSccBatCVVolt;/*Only*/
INT16S	wExtSccBatFloatVolt;/*Only*/
INT16S	wExtSccChgCurrMax;/*Only*/
INT8U	bExtSccQ1Ack;/*Only*/
INT8U	bExtSccParaAutoSync;/*Only*/
INT8U	bExtSccParaTimerSync;/*Only*/
INT8U	bExtSccActive;/*Only*/
INT8U	bExtSccSciCnt;
...
// INT16U wSCCAllowONFlag = false;	// SCC?????????????
...
INT8U g_bDischgVoltSet = cBat52v;	// 11.5-14.5V/pcs锟斤拷锟斤拷锟斤拷
INT8U g_bDischgCurrSet = cIdc50A;	// 0-100A锟斤拷锟斤拷锟斤拷
...
INT8U ucSciIdleCnt[2] = {0,0};	// 1=50ms,???ModBus锟斤拷??????????????????????????????????
...
void sSciTask(void)
{
	...
	while(1)
	{
		...
		if(event&(1<<eInterfaceTimer))
		{
			...
			if(g_uwWifiLossCnt > 0)        { g_uwWifiLossCnt --; }					//非Lion电池，IOT下发间隔60S； Li电间隔120S
...
void sSciRxDebugCommand(INT8U sciid)
{
	...
	while(1)
	{
		...
		else if((sciid == 1) && (bCommandBuffer[sciid][0] == g_uwBMSCmdAddrID))	// ModBus--BMS
		{
			...
			{
				//sBMSDataParsing(bCommandBuffer[sciid],g_ubCommandLength[sciid]);
...
void	sTestCommand(INT8U sciid)
{
	...
	{ 
		...
		else if((bCommandBuffer[sciid][3] == 'B') && (bCommandBuffer[sciid][4] == 'M')&& (bCommandBuffer[sciid][5] == 'S'))
		{
			...
			{
				g_ubBMSForceChgTestFlg=(bCommandBuffer[sciid][8] - '0');// 1?????强锟斤拷?????
				sACK(sciid);
			}
			else if ((bCommandBuffer[sciid][6] == 'S')&&(bCommandBuffer[sciid][7] == 'C'))
			{
				g_ubBMSStopChgTestFlg=(bCommandBuffer[sciid][8] - '0');// 1?????锟斤拷???锟斤拷?????
				sACK(sciid);
			}
			else if ((bCommandBuffer[sciid][6] == 'S')&&(bCommandBuffer[sciid][7] == 'D'))
			{
				g_ubBMSStopDischgTestFlg=(bCommandBuffer[sciid][8] - '0');// 1?????锟斤拷???锟斤拷?????
				sACK(sciid);
			}
			else
			{
		...
		// 	}
		// }
		// else if((bCommandBuffer[sciid][3] == 'T') && (bCommandBuffer[sciid][4] == 'B'))   //test
		// {
		// 	if(bCommandBuffer[sciid][5] == '?')
		// 	{
		// 		wTemp = wSCCAllowONFlag;
		// 		bUserDataBuf0[sciid][0] = '(';
		// 		sbNumToXXX(wTemp,&bUserDataBuf0[sciid][1]);
		// 		bUserDataBuf0[sciid][4] = 0x0D;
		// 		sSciWrite(sciid,bUserDataBuf0[sciid],5);
...
void sQPIGSCommand(INT8U sciid)
{
	...
	if(bPVMode == cBatteryMode)
	{
		...
		|| bPVMode == cFaultMode) && hoGRID_L_RLY == 1 && mOPRLYChk()==cOPRLYOn)
	{
		wTemp = swGetRInverterVoltNew();
