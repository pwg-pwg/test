...
void sInvLoadOPTask(void)       //10ms
{
	...
	while(1)
	{
		...
		if(event&(1<<eInvLoadOPSinZero))
		{
			...
			sSetInvVA(sdwInvVACal(swGetRInverterVolt(),swGetRInverterInvLCurrNew()));//swGetROPCurrentNew()));	  //INVVA=wInvVolt * wOPCurr 
...
void    sParaShareCurChk(void)
{
	...
	if (swGetFBControlStatus() == cFBInverterControl && hoOP_RLY == 1 && (swGetEepromOutputMode() >= cOP_Parallel))
	{
		//share current >= 5A
...
//					g_wOPVoltCntl=wOutputVoltRated -(s_wTempDerateRange*cVac1v);
					...
					else if((g_wOPVoltCntl + 100) < wOutputVoltRated)// 降低10V
					{
						g_wOPVoltCntl = wOutputVoltRated - 100;
					}
				}
			}
		}
