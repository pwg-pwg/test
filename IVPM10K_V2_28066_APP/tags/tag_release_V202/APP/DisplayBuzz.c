...
INT8U fEnterSwPress;            
INT8U fPrimarySourceInterrupt;		// PVԴ/ACԴ����
...
void sLEDControl(void)
{
	...
	else if(   (   (bPVMode == cLineMode && swGetFBControlStatus() == cFBKeepBatSts)
	            || (((bSccChgStatus% 10) !=cNoCharging)) )
	        && g_uwDischgCurr == 0 )
  	{
  	...
  	if(bPVMode == cTestMode)
	{
  	...
  	else if((bPVMode == cPowerOnMode) ||(mOPRLYChk()==cOPRLYOn && hoGRID_L_RLY==1)) 
  	{
		bLineLedTimer = 0;
	...
	{
		sShowLineLed(cNull,cNull,cLedBlink,&bLineLedTimer,cDuty,cPeriod);	
	}
  	else
  	{
		bLineLedTimer = 0;
	...
	{
		sShowINVLed(cNull,cNull,cLedBlink,&bInvLedTimer,cDuty,cPeriod);	
	}	
  	else
  	{
