...
INT16S g_wInvVCntlFactor_1 = 207;
INT16S g_wOPCurrCntlFactor = 31;
INT16S g_wOPCurrCntlFactor_1 = 50;
...
extern INT16S wRInvVoltCntl_1;
extern INT16S wRLoadCurrCntl;
extern INT16S wRLoadCurrCntl_1;
...
//		}
		...
		if(LiHardProtectCNT>=3)//�??�?�瑙﹀彂淇濇姢
		{
			if((wInvOffPoint ==0)&&(wContinueTrig==0))
			{
				...
				wInvOffPoint =100;// 鍏抽�??50�???wInvOffPoint =150;// 鍏抽�??150�???
				wContinueTrig =1;
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>20))//if((wInvOffTimes ==0)&&(++wContinueTrigCNT>30))//闃叉杩炵画瑙﹀�??
			{
				wContinueTrigCNT =30;
				wContinueTrig =0;
			}
...
//		}
		...
		wRLoadCurrCntl_1 = (INT16S)(((INT32S)wOPCurrSample * g_wOPCurrCntlFactor_1) >> 5);	
		wRInvVoltCntl_1 = (INT16S)(((INT32S)wInvVoltSample1  * g_wInvVCntlFactor_1) >> 5);
		sInverterControl();
	}
...
interrupt void MPPTInterrupt(void)
{
	...
	if(bPVMode == cBatteryMode )
	{
		if(LiHardProtectCNT>=3)//�??�?�瑙﹀彂淇濇姢
		{
 		...
 		{
 			if((wInvOffPoint ==0)&&(++wContinueTrigCNT>20))//if((wInvOffTimes ==0)&&(++wContinueTrigCNT>30))//寤舵�??30涓�?? 闃叉杩炵画瑙﹀�??
			{
				wContinueTrigCNT =30;
				wContinueTrig =0;
			}
...
interrupt void OSTimeBase(void)
{
	...
	static INT16U s_uwOPSCRPreSts = 0;//hoOP_RLY;
	...
	if(hoOP_RLY != s_uwOPSCRPreSts && mEepromOutputMode() != cOP_Single && (bPVMode != cPowerOnMode))
	{
		hoOPSCROn;
	}
	s_uwOPSCRPreSts = hoOP_RLY;
	...
	//========= control relay ===========/
	// 1.Relay is closd, change Relay-power to 9V. 2.Relay changed, time is need to fix up, for Relay close steady.
	if(hoGRID_L_RLY)  
	{
		if(++uwGRIDLCnt > 12)  //  7ms
		{
			hoGRID_L_RLY_12V_DRV_OFF;
		}
	}
	...
	if(hoOP_RLY)
	{
		if(++uwOPCnt > 12)  
		{
			hoOP1_RLY_12V_DRV_OFF;
		}
	}

	if(!hoGRID_L_RLY) { hoGRID_L_RLY_12V_DRV_ON;  uwGRIDLCnt = 0; }
	if(!hoGRID_N_RLY) { hoGRID_N_RLY_12V_DRV_ON;  uwGRIDNCnt = 0; }
	if(!hoOP_RLY)     { hoOP1_RLY_12V_DRV_ON;     uwOPCnt = 0;    }
	//=====================================//
}
...
void sLineWaveChk(INT16U wCmpPoint)
{
	...
	else 
	{
		...
		if(((wLineVoltReal > wLineWaveVoltHighLoss) || (wLineVoltReal < wLineWaveVoltLowLoss))
		      &&(abs(wOPCurrReal) < 680) && s_ubCTSaturationChkCnt < 15)    // (5000/140*1.414=50.5)*2.5=126	  避免�??投RCD载，�??报波�??L錹ss
		{
			wLineLossTemp = true;
		}
		else
...
//	(Tdtemp =1000);//强制写死不进入硬件保�?  
//	if((Tdtemp > 1200)||((liparaTx==0)&&(mEepromOutputMode() > cOP_Single)))//&&LiHardProtect==0
	if((liparaTx==0)&&(mEepromOutputMode() > cOP_Single))//&&LiHardProtect==0
	{
//		LiHardProtectCNT++;
