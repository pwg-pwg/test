...
extern INT8U  wBMSFailtoStanbyFlag ;
...
//	hoGRID_N_RLY_OFF;
	hoOP_RLY_OFF;
	AvrAllOff;
	if(swGetEepromOutputMode() != cOP_Single && hoGRID_L_RLY)
	{
		hoGRID_L_RLY_Off;
		hoGRID_L_SCROn;
	...
	while(1)
	{
		...
		// 20241230--Updating Mppt, keep in standby mode.
		...
		//20250326--BMS停充停放
		// if(sbGetLineLossStatus() == false) // #80 市电正常退出待机模式
		// {
		// 	;
		// }
		// else if((wBMSFailtoStanbyFlag)&&(fLoad.LoadOnCmd))// 因为BMS故障进入待机模式，当关机键断开时能完成关机
		// {
		// 	bPVMode = cStandbyMode;
		// 	return;
		// }
...
void sBatteryMode(void)
{
	...
	while(1)
	{
		...
		if(event&(1<<eSuperToLine))
		{
			...
			{
				hoGRID_N_RLYOn;
				//sSetRlyPointer(RLY_DELAY,cSFTRlyDelayTime+50,eSuperSelectPoint,cPrioSuper);
				//while(OSMaskEventPend(0x7FFF) == 0);

				sSetRlyPointer(RLY_CROSSZERO,cSFTRlyDelayTime,eSuperSelectPoint,cPrioSuper);
				while(OSMaskEventPend(0x7FFF) == 0);
				hoGRID_L_RLY_On;
				sSetRlyPointer(RLY_CROSSZERO,cSFTRlyDelayTime-37,eSuperSelectPoint,cPrioSuper);
				while(OSMaskEventPend(0x7FFF) == 0);
				sSetFBControlStatus(cFBWaitSts);
				sSetBuckControlStatus(cBuckWaitSts);	
...
//				while(OSMaskEventPend(0x7FFF) == 0);
				// Testend
				sSetRlyPointer(RLY_CROSSZERO,38,eSuperSelectPoint,cPrioSuper);
				while(OSMaskEventPend(0x7FFF) == 0);
				hoGRID_L_RLY_On;
		...
		if(event&(1<<eSuperTimer))
		{
			...
			if(wInvCrrentLimit != g_wInvCurrLimitRated || wInvOverCurrentLimit != g_wInvOverCurrRated)
			{
				...
				if(mEepromOutputMode() == cOP_2P2)// 缂佸嫬寮婚悘顐ゅ殠闂囷拷顪呴弨閫涜⒈娑擃亙绗夐崥宀�娈戦梽鎰ウ閸婏拷
				{
...
void sLineMode(void)
{
	...
	while(1)
	{
		...
		// if((sdwGetWarningCode() & cBMSCommFail) == cBMSCommFail) // #80 BMS断开市电带载不充电
		// {
		// 	sTurnOffCharger();
		// }
		...
		if(event&(1<<eSuperOPRlyOn))
		{
			...
			{
				if((g_wVAType == cb3500VAModel)||(swGetEepromOutputMode() == cOP_Single))// 3.5K 濞屸剝婀丼CR
				{					
					sOSTimerStop();
					sSetRlyPointer(RLY_CROSSZERO,cOPRlySpeedUpOnDelayTime,eSuperSelectPoint,cPrioSuper);
					while(OSMaskEventPend(0x7FFF) == 0);
...
//					}					
	  ...
	  {				
			...
			if(   (fBMSForceChgFlg ==0)
			   && (   (swGetSccOkFlag() == true && sbGetEepromChargerPriority() == cChargeSolarFrist)
...
void sShutDownMode(void)
{
	...
	hoDRYOff;
	AvrAllOff;
	if(bStartMpptUpdateFlag) { return; }
	hoLCDBLOff;
	hoLEDAOff;
	hoLEDBOff;
	hoLEDCOff;
...
璁＄畻CT鐨勯噰鏍峰亸宸紝骞舵妸鍋忓樊琛ュ伩鍒癆D閲囨牱涓婏紝瀹炵幇
0锟??
	...
	while(1)
	{
		...
		if(event&(1<<eSuperTimer))   //20ms
	    {
	    	if((sbGetInvVoltSoftFinishSts() != true)&&(++wDelayToLineCnt<1000)&&(InvVoltRelayAction ==0)\
				&&(wBatAvgVoltNew >=swGetBatUnderVolt())&&(InvVoltSoftFalseFlag == 0))//20S娴犮儱鎮楅張顏囪拫鐠ч攱鍨氶崝鐔剁瘍娴兼艾鍨忛幑銏犵閻拷
			{
				b2Cnt = 0;
...
INT8U sbOutputVoltChk(void)
{
	TASK_EVENT event;
	INT8U  bCnt = 0;
	INT16U uwSteadyDelayCnt = 100;

	if(g_fBatFastLow == 1) { uwSteadyDelayCnt = 100; } // 加入延时2S，避免电池快速低压导致来回切换
	else { uwSteadyDelayCnt = 0; } // 非电池快速低压保护的，不做延时
	...
	while(1)
	{
		...
		if(event & (1<<eSuperTimer))	//20ms
		{
			if(uwSteadyDelayCnt > 0) // 加入延时2S，避免电池快速低压导致来回切换；非电池快速低压保护的，不做延时
			{
				uwSteadyDelayCnt --;
			}
			else
			{
				if(swGetEepromOutputMode() == cOP_Single)
				{
					// 涓嶈兘锟??220V/2,鍥犱负瀹為檯搴旂敤鍙兘N绾胯繛鎺ュ湪涓�锟??,瀵艰嚧杈撳嚭妫�娴嬪埌鐢靛帇,鍑虹幇璇垽
...
INT8U  sbTestModeChk(INT8U b20ms)
{
	...
	while(bTemp)
	{
		...
		if(event & (1 << eSuperTimer))
		{
			// 娴嬭瘯妯″紡闇�鎺ュ叆75惟鐢甸樆,鎵�浠ラ噰鏍峰�间负75/(5100+75)=59(-1%=-40,+0.5%=20)
...
void swSccChgChk(void)		// SCC PWM
{
    ...
    //----------------------
    // 禁止MPPT充电：待机、关机、故障 或 并网下设置电网优先充电 或 充电电流为0 或 SCC通讯异常 或 BMS通讯异常告警
    //----------------------
    if(   (bPVMode == cPowerOnMode || bPVMode == cShutdownMode || bPVMode == cFaultMode)
       || (bPVMode == cLineMode && sbGetEepromChargerPriority() == cChargeUtilityFrist)
       || swGetMpptMaxChgCurr() == 0
       || swGetSccOkFlag() == false
...
INT16S swWorkModeChk(void)
{
	...
	else
	{
		...
		{
			// 鐢垫睜绠＄悊浼樺寲,鍒囨崲鑷崇數姹犱紭鍏堟ā锟??
			// EEPROM杈撳嚭浼樺厛妯″紡涓嶅仛淇敼,闃叉鏈繚瀛樻柇鐢靛鑷存ā寮忚缃敼锟??
			if(swGetBattMgtFlag())
			{
