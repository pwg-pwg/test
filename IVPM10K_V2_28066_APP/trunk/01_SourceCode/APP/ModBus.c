...
INT8U g_ubRequestBMSId; //  WIFI锟斤拷锟斤拷锟窖疊MS锟斤拷ID锟脚ｏ拷锟斤拷锟捷革拷ID锟斤拷锟斤拷锟斤拷锟斤拷锟斤拷息锟斤拷实时锟斤拷息锟斤拷虾锟斤拷锟斤拷锟?0x5100锟斤拷始锟侥寄达拷锟斤拷锟叫ｏ拷锟皆癸拷WIFI锟斤拷询
...
INT16U swModBusRdProc(INT16U *RxBuff, INT16U RxLen)
{
	...
	if(RdNums > 64) // Limit read data length less than 64
	{
...
INT16U swWriteEESetting(INT16U WrAddr, INT16U WrData)
{
	...
	switch(WrAddr)
	{
		...
		case E2_BattType:
			if(WrData <= cPylonBType)
			{
				if(sbGetEepromBatteryType() != WrData)
				{
					...
					//------ 锂碉拷锟斤拷锟斤拷投锟斤拷远锟斤拷锟轿?9600bps锟斤拷锟斤拷锂碉拷亩锟斤拷远锟斤拷锟轿?2400bps ------//  // LMZ add in 20240430
		...
		case E2_MenuRtnDftPageEn:
			if(WrData <= 1)
			{
				// if(sbGetEepromBMSSetEn() != WrData) //固定波特率9600，所以屏蔽
				// {
...
//		break;
		case ATE_UpdateFW:
			if(WrData <= 6)
			{
				sMpptUpdateCommand(1); //只允许从串口进入透传模式，退出机制为无数据接收下30S后超时退出
			}
			else
			{
				ret = cErr_ParUnValid;
...
void sRealTimeDataUpdate(void)
{
	INT16U wTemp =0;
	INT32U dwTemp =0;
	// uRealTimeData.Stru.wSettingDataSN; // 锟斤拷eeprom锟斤拷锟斤拷时锟窖撅拷锟睫改ｏ拷锟剿达拷锟斤拷锟轿★拷
	...
	uRealTimeData.Stru.wRInvWatt = sdwGetInvWatt();	// 锟斤拷锟斤拷压*锟斤拷锟斤拷锟斤拷
	uRealTimeData.Stru.wRInvVA = sdwGetInvVA();		// 锟斤拷锟斤拷压*锟斤拷锟斤拷锟斤拷锟?
	...
	//----------------------------
	uRealTimeData.Stru.wROpVolt = swGetRInverterVolt();	// 锟斤拷锟斤拷
	...
	// { uRealTimeData.Stru.wRLineCurr = uRealTimeData.Stru.wRInvCurr+uRealTimeData.Stru.wROpCurr; } //swGetOPShareCurr();	// 锟斤拷锟斤拷
	...
	uRealTimeData.Stru.wRLineVolt = swGetRLineVolt();
	// uRealTimeData.Stru.wRLineVolt =wSccChgCurr;
	...
	if(mEepromOutputMode() >= cOP_Parallel)
	{
	    uRealTimeData.Stru.wMachineStatus = wParaMode;
	}
	else
	{
	    uRealTimeData.Stru.wMachineStatus = 0;
	}
	//----- BMS summy data start -----//
	...
	//-------- 0x1300协议转0x0200的新地址后再发送后台 --------//
	if(g_strBMSRTInfo[g_ubRequestBMSId].wBMSFlag | 0x0001)  wTemp |= 0x0080; // 充电允许
	if(g_strBMSRTInfo[g_ubRequestBMSId].wBMSFlag | 0x0004)  wTemp |= 0x0040; // 放电允许
	if(g_strBMSRTInfo[g_ubRequestBMSId].wBMSFlag | 0x0040)  wTemp |= 0x1000; // 放电状态
	if(g_strBMSRTInfo[g_ubRequestBMSId].wBMSFlag | 0x0080)  wTemp |= 0x2000; // 充电状态
	//--------------------//
	uRealTimeData.Stru.wRT_BmsStatusLo = wTemp; // 0x1300协议转0x0200的新地址后再发送后台
	...
	uRealTimeData.Stru.wRT_BmsAlarmFlagLo = g_strBMSRTInfo[g_ubRequestBMSId].wBMSAlarmFlag; //新旧地址数据内容一致，直接赋值
	...
	uRealTimeData.Stru.wRT_BmsTotalCurr = -g_strBMSRTInfo[g_ubRequestBMSId].wBatCurr; // 对外协议中充电为正，BMS读取到充电为负。
	...
	uRealTimeData.Stru.wRT_BmsParallelNumber = 0; // 0x1300地址无该数据，上传0
	uRealTimeData.Stru.wRT_BmsParallelStatus = 0; // 0x1300地址无该数据，上传0
