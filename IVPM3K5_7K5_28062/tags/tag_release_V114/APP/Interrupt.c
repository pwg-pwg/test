...
INT8U  bBatAvg_RealVoltChkCnt = 0;//电池压差比较
INT8U  bBatAvg_RealVoltProtectFlag = 0;//斜率保护标志位
	...
	if(bPVMode == cBatteryMode )
	{
		if(LiHardProtectCNT>=3)//纭欢瑙﹀彂淇濇姢 
		{
			if((wInvOffPoint ==0)&&(wContinueTrig==0))
			{
				...
				wInvOffTimes =0;//2; // 鍏?涓懆鏈?
				wInvOffsetpointer =0;

				ubHardProtectFlag =true;
				//wInvOffPoint =128;
				wInvOffPoint =192;// 鍏抽棴150鐐?
				wContinueTrig =1;
