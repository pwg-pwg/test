...
void sFanSpeedControl(void)	//Fan Control
{
	static INT16U wFANSpeedCtrlDly = 0;
	INT16U	wFanSpdPrd;
	...
	if(false == wSciForeceFanCtrl)
	{
		...
		{
			wFANPWMTemp = cFanClosed;//cFanClosed;
			wFANPWMTempPre = cFanClosed;//cFanClosed;
			wFANPWMTempNew = cFanClosed;//cFanClosed;
			wFANPWMTempSet = cFanClosed;//cFanClosed;
			g_wACFanOnFlag =0;
			bFan1status = true;
			bFan2status = true;
		}
		else
		{
			...
			else if(FanStatus.HeavyLoad || FanStatus.DCIHigh ||(bMpptHsTempInfo >= 3) \
				...
				|| (sbGetTxTempStatus() >= 1) || (sbGetInvTempStatus() >= 1))
			{
				...
				wFANPWMTempSet = cFanFullSpeed;
				g_wACFanOnFlag =0;//1;;//AC Fan Contral
				bFan1status = true;
				bFan2status = true;
			}
			...
			else
			{
				#ifdef FANSLEEP
					//wFANPWMTempNew = cFanClosed;
					wFANPWMTempSet = cFanLowerSpeed;//cFanClosed;
					bFan1status = true;//false;
					bFan2status = true;//false;
				#else
					//wFANPWMTempNew = cFanLowerSpeed;
					wFANPWMTempSet = cFanLowerSpeed;
					bFan1status = true;
...
//			}
	...
	if(wFANPWMTempPre > wFANPWMTempNew)
	{
		...
		if(++wFANSpeedCtrlDly >= wFanSpdPrd)
		{
			wFANPWMTempPre -= 1;
...
void 	sBatWeakChk(INT8U bFilter)
{
	...
	//if((!g_uwBatWeakFlg)&&(sbGetBatLow() == true))// 电池电压batlowback和batlow之前，一直报低压不切市电�?�?
	//{
	//	g_uwBatWeakFlg = 1;
	//}
...
void sACSPSControl()
{
		...
		&& (0 == sbGetRLineWaveLoss()))
	{
		...
		if( ++wACspsCtrlSecCnt>620)//5 minute 以后�?开继电�?
		{
			wACspsCtrlSecCnt =0;
			if(!hoACSPS)
			{
...
void sControlParameterSeting(void)
{
	if(bPVMode !=cBatteryMode)
	{
		if(wControlParaMeter != swGetEEKpKiParameter())
		{
			...
			{
//				wRKv = 48;//45;            //Q7  0.18*128 
//				wRKi = 305;//320;            //Q7  8*128// 
