...
#define	cCancleCRCComIndexMppt		4//8
#define cCancleCRCComIndexScc       4//8
...
////	bBoot=1;//tEST
	...
	//wBootVersion=cNewBootLoader;	// for remove bootloader
	//bBoot=1;
...
//	sGetIDSeriesForPar();
	...
	while(1)
	{
		...
		if(event&(1<<eInterfaceTimer))
		{
			...
			{
				 sBMSCommandProc(1, bUserDataBuf0[1], 20);
			}
...
//	sbNumToXXX(swGetTempDegreeBat(),pDataBuf);
	...
	*/
	...
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(swGetLoadPowerVAPre(),pDataBuf);	
	bStrLen += 4;	
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
	...
	pDataBuf=&bUserDataBuf0[sciid][bStrLen];
	sbNumToXXXX(g_BatCurOPVoltCntl,pDataBuf);	
	bStrLen += 4;	
	bUserDataBuf0[sciid][bStrLen++]=CHAR_SPACE;
...
void	sPSDVCommand(INT8U sciid)
{
	...
	if((sbGetEepromBatteryType() < cUseType) || (sbGetEepromBatteryType() > cLiBType) \
