	...
	else                
	{
		...
		if(wInvVoltReal<(-500))//-50V
		{
			if(++IsoReverseCnt>50)//
			{
				...
				if(++IsoReverseChkCnt>60)//  60/2=30  Á∫?30‰∏™Â∏ÇÁîµÂë®Êúü‰ª•ÂêéÊâçÊ£ÄÊµ?
				{
					IsoReverseChkCnt =60;
	...
	else
	{
		...
		***************************************************************************/
		...
		if(++wFBCntLoop&0x08)
		{
			...
			wFBP_VoltPIAdjCtl =(INT32S)((INT32S)wFBP_VoltPIAdjCtl*3+ wVm_P)>>2;// ÂÅèÂ∑Æ‰∏?0 Êó∂ÔºåÂâçÈ¶à‰∏çÂèóÊé?
		...
		***************************************************************************/
		wLineVoltCntlQ5=(INT16S)((INT32S)gi_wLineVoltQ5_0*3+(INT32S)(abs(wLineVoltCntlQ5+128))>>2);// 4*32 ÊòØÁîµÂéãÈááÊ†∑ÂÅèÁΩÆË°•ÂÅ?
...
//			wKCurrentForward = TestwFB_CurPI_Kp;
			...
			if(uWFBP_CurPI_IderatCnt >0)//ËøáÈõ∂ÁÇπÂáèÂ∞ëÁßØÂà?
			{
