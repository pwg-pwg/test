...
INT16U wloadCurrent[5]={0,0,0,0,0};
INT16S wChangeCurLimit = cIac10A;
...
void sOPPowerFilter(INT16U wFilter)
{
	...
	if(dwOPWatt > dwOPVA)//Chandler 2020 1116
	{
		dwOPVA = dwOPWatt;
	}
}
...
{
	INT16U wTempWatt = 0;
	INT16U w50WTempWatt =0;
	
	w50WTempWatt =(50*dwBatWatt)/wOPWatt100;//50W virtul power
	if(w50WTempWatt>50)
	{
		w50WTempWatt =50;
	}
	...
	wTempWatt =wTempWatt+w50WTempWatt;
	wBatWattTransferRatio = (INT16U)(((INT32U)wTempWatt * g_wOPVoltTransferRatio) >> 10);
	wInvWattPercent = (INT16U)(((INT32U)wBatWattTransferRatio * 100) / wOPWatt100);
...
INT16U sLoadCurrentChangeChk(INT16U wOutPutCurrent)
{
	//static INT16U wloadCurrent[4]={0,0,0,0};
	static INT16U woutCnt=0;
	INT16U wShiftCnt=0;
		
	if((((INT16S)wOutPutCurrent-(INT16S)wloadCurrent[4])>wChangeCurLimit)&&(wOutPutCurrent>wChangeCurLimit)&&(bPVMode == cBatteryMode))//cIac5A
	{
		woutCnt++;
	}
	else
	...
	for(wShiftCnt=4;wShiftCnt>0;wShiftCnt--)
	{
		wloadCurrent[wShiftCnt]=wloadCurrent[wShiftCnt-1];
	}
	wloadCurrent[0]=wOutPutCurrent;
