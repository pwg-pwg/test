...
{                                   
    ...
    CF_MAX_AC_CHG_CUR_ID,	// 6
    CF_BAT_CHG_CV_ID,		// 7
    CF_BAT_CHG_FLTV_ID,		// 8
...
//   {3, charP, charR, charT},	   //   CF_MENU_RTN_ID
...
{
	{3, cCharSpace, cCharSpace, char0},
	{3, cCharSpace, cCharSpace, char3},		 
	{3, cCharSpace, cCharSpace, char6},		 
	{3, cCharSpace, cCharSpace, char9},		
	{3, cCharSpace, char1, char2},
	{3, cCharSpace, char1, char5},
	{3, cCharSpace, char1, char8},	
};
...
{ 
	...
	{
		if(sbGetEepromMenuRtnDftPageEn() == 1)
		{
			bSlectIndex = 0;
		}
	}
...
void sSetConfigMode(void)
{
  ...
  bOuputVoltIDTemp =  bOuputVoltID;
	...
	bMenuRtnDftPageEnID = sbGetEepromMenuRtnDftPageEn();
	bMenuRtnDftPageEnIDTemp = bMenuRtnDftPageEnID;
		...
		else
		{
			if(bIsAdjustVal == 1) 
			{
				...
				|| (cInvOnKeyPressBit == wKeyVal)) 
				{
					...
					else if(bMenuIndex == CF_MAX_AC_CHG_CUR_ID)//AC最大充电电流
					{
						...
						if(bDownFlag)
						{
							...
							else if(g_wAcChgCurrMax == cIdc18A)//(0~18A)
							{
								if(bMaxACChgCurrIDTemp >= MAX_AC_CHG_CUR_ID)
								{
									bMaxACChgCurrIDTemp = MAX_AC_CHG_0A_ID;
									wAdjustValue += MAX_AC_CHG_CUR_ID - MAX_AC_CHG_60A_ID;
								}
							}
							else if(g_wAcChgCurrMax == cIdc12A)
							{
								if(bMaxACChgCurrIDTemp >= MAX_AC_CHG_60A_ID)//(0~15A)
								{
									bMaxACChgCurrIDTemp = MAX_AC_CHG_0A_ID;
									wAdjustValue += MAX_AC_CHG_CUR_ID - MAX_AC_CHG_60A_ID;
								}
						...
						else
						{
							...
							else if(g_wAcChgCurrMax == cIdc18A)
							{
								if(bMaxACChgCurrIDTemp >= MAX_AC_CHG_CUR_ID)
								{
					...
					else if(bMenuIndex == CF_MENU_RTN_ID)      
					{
						bMenuRtnDftPageEnIDTemp = (INT16U)(bMenuRtnDftPageEnID + wAdjustValue) % (INT8U)MAX_MENU_RETURN_ID;   
					}
					...
					else if(bMenuIndex == CF_BAT_CHG_FLTV_ID)
					{
						if(bBatTypeID == USER_ID)
						{
							...
							{
    							if((swGetBatteryPcs()==cbBatt48VType)&&(wTemp > 120))//lory 151127 132->130
    							{
    								wTemp = 0;
    								wAdjustValue = (INT16S)cw_AdjustValue_MID + 0 - (INT16S)bBatChgFVID;
    							}
    							else if((swGetBatteryPcs()==cbBatt24VType)&&(wTemp > 50))//25V->31.5V
    							{
    								wTemp = 0;
    								wAdjustValue = (INT16S)cw_AdjustValue_MID + 0 - (INT16S)bBatChgFVID;
    							}
...
void	sConfigDisplay(void)
{
  ...
  {    
	...
	{
		sFigGroupDisplayLetter(FIG4_7SEG, (INT8U *)&cConfigNum[CF_MENU_RTN_ID][0], bConfigItemFlashStatus);
		//sFigGroupDisplayLetter(FIG6_7SEG, (INT8U *)&cMenuRtnTAB[bMenuRtnDftPageEnIDTemp%MAX_MENU_RETURN_ID][0], bConfigContentFlashStatus);
		sFigGroupDisplayLetter(FIG6_7SEG, (INT8U *)&cEnaDisTAB[bMenuRtnDftPageEnIDTemp%MAX_MENU_RETURN_ID][0], bConfigContentFlashStatus);
		//sFigGroupDisplayLetter(FIG1_7SEG, (INT8U *)&RemindTab[CF_MENU_RTN_ID][0], bConfigItemFlashStatus);
	}
	else if(bMenuIndex ==  CF_BACKLIGNT_ONOFF_ID)
	{
...
void  sSaveConfiguration(void) 
{
  ...
  if(wVlaueTemp != swGetEepromCHGCVTimer())
  {
	...
	else
	{
		...
		wVlaueTemp = bMaxACChgCurrID * 3;//Chandler 20201107 for1512
	}
	...
	if(bMenuRtnDftPageEnID != sbGetEepromMenuRtnDftPageEn())
	{
		sSetEepromMenuRtnDftPageEn(bMenuRtnDftPageEnID);
		bSaveFlag = 1;
	}
...
void  sUpdateConfiguration(void) 
{
    ...
    bMenuRtnDftPageEnID = bMenuRtnDftPageEnIDTemp;
...
{                                          
  ...
  {
    sSendBitToVram(cBIT_IP_AC_OLDLCD, DISPLAY_ON, cALWAYS_LIGHT);   //150407
  }
...
void sDisplayFaultWarning(void)
{
	...
	else if(bPVMode != cPowerOnMode  && wDisplayCnt < 10 && sbDisplayWarning() == true)
	{
...
INT8U sbDisplayWarning(void)
{
    ...
    else if(wTemp & cFanLockWarning) 
    {
        bWarningCodeTemp = cWarningFanlock; 
    } 
    else if((wTemp & cOverTemperatureWarning) || (wTemp & cDustproofWarning))
    {
        bWarningCodeTemp = cWarningOverTemp; 
    } 
    else if (wTemp & cBatLow) 
    {
        bWarningCodeTemp = cWarningBatLow;   
    }
    else if (bPowerLimitflag) 
    {
        bWarningCodeTemp = cPowerDerate;   
    } 
	else if (wTemp & cTxNtcDisconnect) 
    {
        bWarningCodeTemp = cTxNtcDis;   
    }
	else if (wTemp & cInvNtcDisconnect) 
    {
        bWarningCodeTemp = cInvNtcDis;   
    }
	else if(wTemp & cEQWarning)
    {
        bWarningCodeTemp = 0x0B;   
    }
	/*else if(wTemp & cBatOpen)
	{
		bWarningCodeTemp = cWarningBatOpen;
	}*/
	else if(wTemp & cBMWarning)
    {
        bWarningCodeTemp = cWarningBM;
    }
	else if(wTemp & cHeavyLoadChange)
    {
        bWarningCodeTemp = cWarnHeavyLoadChange;
    }
	else if(fSccSciLoss == false)
	{
		if(fIntSccSciLoss == false)
		{
			if(wPVCharger == cwMPPTChg)
			{
				...
				{
					bWarningCodeTemp = cPwmChgCurrOv;
				}
				else if((wSccAlarmFlag & cMpptFlagBatVoltLo) != 0)
				{
					bWarningCodeTemp = cPwmBatLow;
				}
				else if((wSccAlarmFlag & cMpptFlagBatVoltHi) != 0)
				{
					bWarningCodeTemp = cPwmBatHigh;
				}
				else if((wSccAlarmFlag & cMpptFlagHsTempHi) != 0)
				{
					bWarningCodeTemp = cPwmOverTemp;
				}
				else if((wSccAlarmFlag & cMpptFlagPvVoltHi) != 0)
				{
					bWarningCodeTemp = cPwmPvHigh;
				}
			}
			else
			{
				if((wSccAlarmFlag & cPwmFlagChgCurrHi) != 0)
				{
					bWarningCodeTemp = cPwmChgCurrOv;
				}
				else if((wSccAlarmFlag & cPwmFlagBatVoltLo) != 0)
				{
					bWarningCodeTemp = cPwmBatLow;
				}
				else if((wSccAlarmFlag & cPwmFlagBatVoltHi) != 0)
				{
					bWarningCodeTemp = cPwmBatHigh;
				}
				else if((wSccAlarmFlag & cPwmFlagHsTempHi) != 0)
				{
					bWarningCodeTemp = cPwmOverTemp;
				}
				else if((wSccAlarmFlag & cPwmFlagPvVoltLoss) != 0)
				{
					bWarningCodeTemp = cPwmPvLost;
				}
			}
		}
