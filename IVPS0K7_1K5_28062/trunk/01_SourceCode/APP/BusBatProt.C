...
//	sBusModuleInit();
	...
	while(1)
	{
		...
		{
//			sBusAvgVoltAdj(swBusAvgVoltCal(),0);//û��Bus��ѹ
...
//	INT16U	wFanSpdPrd;
	...
	if(false == wSciForeceFanCtrl)
	{
		...
		{
			if((g_wVAType == cb10KVAModel) && (sbGetInvVoltHiFanStop()))
			{
				#ifdef FANSLEEP
			...
			else if(FanStatus.HeavyLoad || FanStatus.DCIHigh || FanStatus.SCCTempOver80C \
				|| (sbGetTxTempStatus() >= 3) || (sbGetInvTempStatus() >= 3))
			{
				wFANPWMTempNew = cFanFullSpeed;
				bFan1status = true;
				bFan2status = true;
			}
			else if(FanStatus.HalfLoad || FanStatus.ACChgOver20A || FanStatus.SCCChgOver20A \
			...
			else if(FanStatus.FanOnLoad || FanStatus.ACChgOver10A || FanStatus.SCCChgOver10A \
	...
	#ifdef IVPL1512
	if(wFANPWMTempNew == cFanClosed)
	{
		hoFanSpeed_OFF;
		hoFanSpeedSlow_OFF;
	}
	else if(wFANPWMTempNew == cFanLowerSpeed)
	{
		hoFanSpeed_OFF;
		hoFanSpeedSlow_ON;
	}
...
void sLoadForFanCtlChk(void) 
{
	...
	{
		if(sbUnderLevelChk(wLoadPercentTemp, 25, 8,&bFanHalfChkCnt) == 1)//4s
		{
	...
	{
		if(sbOverLevelChk(wLoadPercentTemp, 35, 2,&bFanHalfChkCnt) == 1)//1s   
		{
			FanStatus.HalfLoad = 1;
			FanStatus.FanOnLoad = 1;
		}
	...
	{
		if(sbUnderLevelChk(wLoadPercentTemp, 15, 8,&bFanLoadChkCnt) == 1)	//4s
		{
	...
	{
		if(sbOverLevelChk(wLoadPercentTemp, 20, 2,&bFanLoadChkCnt) == 1)	//1s   
		{
			FanStatus.FanOnLoad = 1;
		} 
	}
