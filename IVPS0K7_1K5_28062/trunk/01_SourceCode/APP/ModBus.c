...
#pragma DATA_SECTION(wGraphDataBuff, "DataBuff");
#pragma DATA_SECTION(wTranmitDataBuff, "DataBuff");
...
INT16U swWriteEESetting(INT16U WrAddr, INT16U WrData)
{
	...
	switch(WrAddr)
	{
		...
		break;
		case E2_BattCVVolt:
			if((sbGetEepromBatteryType() < cUseType) || (sbGetEepromBatteryType() > cUseType) \
			...
			if(WrData <= cLiBType)
			{
				if(sbGetEepromBatteryType() != WrData)
				{
					...
					else if(sbGetEepromBatteryType() == cLiBType)
					{
						...
						sSetEEBatteryVoltUnder(cBat12v2 * temp);
						sSetEEEqEna(false);
					}
					sRSTTxCommand();
					OSEventSend(cPrioInterface, eEepromSaveUserSetData);
		...
		case E2_MenuRtnDftPageEn:
			if(WrData <= 1)
			{
				if(sbGetEepromMenuRtnDftPageEn() != WrData)
				{
					sSetEepromMenuRtnDftPageEn(WrData);
					OSEventSend(cPrioInterface, eEepromSaveUserSetData);
				}
			}
			else
...
void sRealTimeDataUpdate(void)
{
	...
	uRealTimeData.Stru.wSccChgPower = wSccDispChgPower / 10;
...
void sInfoDataUpdate(void)
{
	...
	uInfoData.Stru.wSubType = (swGetBatteryPcs() << 8) + g_wVAType;
...
void sSnatchGraph(void)
{
	...
	if(wGraphWaitFaultFlag == 1)
	{
		if(wFaultCode == 0)
		{
			//wGraphDataBuff[0][wTempCnt] = (INT8U) GetDataSubArray[7]();
			//wGraphDataBuff[1][wTempCnt] = (INT8U) GetDataSubArray[11]();
			wGraphDataBuff[0][wTempCnt] = (INT8U) GetDataSubArray[37]();
