...
extern INT16U g_wModeSwitchToBatWaitSec;
extern INT16U wVoltSoft;
...
void sModeSwitchTask(void)
{
	...
	INT16U  wModeSwitchToBatDelay = 0;
	INT16U  wModeSwitchToBatCnt = 0;
		...
		if(event&(1<<eModeSwitchTimer))
		{
				...
				|| bPVMode==cLineMode || bPVMode==cBypassMode) 	
			{
				...
				if(bPVMode != bSwitchWorkMode)
				{
					...
					{
						wModeSwitchToBatDelay = g_wModeSwitchToBatWaitSec*50; // 10*50*20ms=10s
						if(++wModeSwitchToBatCnt>=wModeSwitchToBatDelay)
						{
							wModeSwitchToBatCnt = 0;
							g_wModeSwitchToBatWaitSec = 0;
					...
					else
					{
						...
						//wModeSwitchToLineDelay = (INT32U)swGetEELineWaitSec() * 10;	//CC test 20201118
...
void	sBatteryMode(void)
{
	...
	if(cHeavyLoadChange == (sdwGetWarningCode() & cHeavyLoadChange))
	{
		sClrWarningCode(cHeavyLoadChange);
	}
		...
		if(event&(1<<eSuperToBat))
		{
			...
			if(fLoad.LoadOnCmd == true)
			{
				...
				if(sbGetEepromModeSelect() == 0)	// APL
				{
					...
					sSetRlyPointer(RLY_ZEROPOINT,cDelay5ms,eSuperSelectPoint,cPrioSuper);
					while(OSMaskEventPend(0x7FFF) == 0);
...
//				sSetInvVoltSoftStepVolt(15);
//				sSetSineVoltPeak((INT16S)((((INT32S)swGetEEOutputVolt() * 181) >> 2) / 10));   // 230*1.414*32=10407 
//				
//				//swInverterVoltSoftCal();
//				//sSetRSinAmp(wVoltSoft);//9954= 220*1.414*32
//				//sSetRNewSinAmp(wVoltSoft);//9954 
//				sSetRSinAmp(4072);//4072= 90*1.414*32
//				sSetRNewSinAmp(4072);//4072 
//				//sSetRSinAmp(0);// 0V
//				//sSetRNewSinAmp(0);//0V
...
void	sFaultMode(void)
{
	...
	while(1)
	{
		...
		if(event&(1<<eSuperTimer))
		{
			...
			/*else if(wFaultCodeTemp == cHeavyLoadChangeFault)
			{
				g_uwOvldRstToLineDelay = 0;
...
INT8U sbInvSoftStart(void)
{
	...
	while(1)
	{
		...
		if(event&(1<<eSuperTimer))   //20ms
	    {
	    	if((sbGetInvVoltSoftFinishSts() != true)&&(++wDelayToLineCnt<500)&&(wBatAvgVoltNew >=swGetBatUnderVolt()))//10S浠ュ悗鏈蒋璧锋垚鍔熶篃浼氬垏鎹㈠競鐢?
			{
...
INT8U sbGetHeavyloadCnt(void)
{
	return(bHeavyloadCnt);
}

void sSetHeavyloadCnt(INT8U bValue)
{
	OS_ENTER_CRITICAL();
	bHeavyloadCnt = bValue;
	OS_EXIT_CRITICAL();	
}
