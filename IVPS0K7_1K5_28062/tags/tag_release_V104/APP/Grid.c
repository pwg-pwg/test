...
void sCalMaxCurAccept(void)
{
	...
	else if(g_wAcChgCurrMax == cIdc18A)//Chandler 20201106 for 1512
	{
		wAcChgCurrMax = cIdc18A;
	}
	else if(g_wAcChgCurrMax == cIdc12A)//Chandler 20201106 for 1512
	{
		wAcChgCurrMax = cIdc12A;
	}
	else
	{
		wAcChgCurrMax = cIdc10A;
	}
	
	// 市电低压降额
	wLineVoltTemp = swGetRLineVoltNew();
	if(wLineVoltTemp >= cVac170v)
	{
	...
	{
		if(wTempDegreeTxt <= cTxtTempDerate10KVA)
		{
			wTxtTempChgCurr = wAcChgCurrMax;
		}
		else if(wTempDegreeTxt >= cTxtTempDerateUp10KVA)
		{
			wTxtTempChgCurr = 0;
		}
		else
		{
			wTxtTempChgCurr = ((INT32S)(cTxtTempDerateUp10KVA - (INT16S)wTempDegreeTxt) * wAcChgCurrMax) / (cTxtTempDerateUp10KVA - cTxtTempDerate10KVA);
		}
	}
	else
	{
		if(wTempDegreeTxt <= cTxtTempDerate)
		{
			wTxtTempChgCurr = wAcChgCurrMax;
		}
		else if(wTempDegreeTxt >= cTxtTempDerateUp)
		{
			wTxtTempChgCurr = 0;
		}
		else
		{
			wTxtTempChgCurr = ((INT32S)(cTxtTempDerateUp - (INT16S)wTempDegreeTxt) * wAcChgCurrMax) / (cTxtTempDerateUp - cTxtTempDerate);
		}
	}
	
	// 散热片高温降额
	wHsTempMax = swGetMaxHsTemp();
	if(wHsTempMax <= cHsTempDerate)
	{
		wInvTempChgCurr = wAcChgCurrMax;
	}
	else if(wHsTempMax >= cHsTempDerateUp)
	{
		wInvTempChgCurr = 0;
	}
	else
	{
		wInvTempChgCurr = ((INT32S)(cHsTempDerateUp - wHsTempMax) * wAcChgCurrMax) / (cHsTempDerateUp - cHsTempDerate);
	}
	
	// 根据SCC充电电流实时调整市电充电电流
	if(wSccChgCurr > 0)
	{
		wSccAdjChgCurr = (INT16S)sbGetEepromMaxChgCur() - (wSccChgCurr / 10);
		if(wSccAdjChgCurr < 0)
		{
			wSccAdjChgCurr = 0;
		}
	...
	{
		wSccAdjChgCurr = sbGetEepromMaxChgCur();
	}
	...
	wMaxVmUpLimit =(INT16S)bMaxACChgCur*80;// 65 鏄祴璇曞�?
}
