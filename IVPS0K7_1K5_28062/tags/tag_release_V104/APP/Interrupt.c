...
extern INT16S IsoReverseChkCnt;
...
INT16U wInvOverCurrentCntSum=0;
INT16U wInvCurrentHeavyFlg = false;
INT16U wInvCurrentHeavyRstCnt = 0;
INT16U wInvOffPoint = 0;
INT16U wInvOffFlg = false;//是否继续打开逆变标志位
...
INT16U wInvOffPointHw =0;//硬件关断时间
...
INT16U wSinPointer_WaveDetCnt =0;
INT16U wLineWavSum =0;
INT16U wRLineWavRealArray[16]={0};
INT16U wLineWavCnt =0;
extern INT16U wRLineLossVoltage;

//extern INT16U TestPICycleSet ;
//extern INT16U TestPICycle ;
extern INT8U ubHardProtectFlag;
extern INT8U ubHardProtectCnt;
extern INT16S wLineVoltCntlQ5; 

void  swGetRLineADSum(INT16U wRLineVoltRealTemp );
void sRLineWavChk(INT16U wRLineAdAbsSum,INT16U wLineWavChkLLevel,INT16U wLineWavChkHLevel,INT8U bLossFilter,INT8U bBackFilter);
...
{	
	...
	{ 
		...
		wInvOverCurrentCntSum = 0;
		if(!wInvCurrentHeavyFlg)
		{
			if(wInvOffPoint > 0)
			{
				wInvOffPoint--;
	 ...
	 if((wRSinPointer % 8) == 0)					//sum value every 8 counts
	{
		...
		dwLineVolt2SumTemp += gi_wRLineVoltRealQuadTemp;//(INT32S)wLineVoltReal * wLineVoltReal;
...
//	}
	...
	wTestModeSample  = 4095;
		...
		#ifdef IVPL1512
		wLineVoltSample1 = ADCRESULT3-2048;   //1V = 4.3977CNT
		wBatVoltSample   = ADCRESULT4;	      //1V = 34.0058CNT
		wOPCurrSample    = ADCRESULT6-2048-wOpCurrSampleBias;	  //1A = 12.784CNT
		wInvLCurrSample1 = ADCRESULT7-2048-wInvCurrSampleBias;	  //1A = 12.784CNT
		wTestModeSample  = ADCRESULT9;
		wInvDCVoltSample = 0;   //1V = 1911.4667CNT
		wInvTempSample   = ADCRESULT10;
		wTxtTempSample   = ADCRESULT11;
		#else
...
//		if(!wInvCurrentHeavyFlg)
//		{
//			if(abs(wInvLCurrSample1) > 468) //78*6
//			{
//				if(++wInvOverCurrentCntSum > 5)
//				{
//					wInvCurrentHeavyFlg = true;
//					wInvOverCurrentCntSum = 0;
//					wInvOffPoint = 76;
//				}
//			}
...
//		else
//		{
//			if(abs(wInvLCurrSample1) < 390) //78*5
//			{
//				if(++wInvCurrentHeavyRstCnt > 1920) //384*5
//				{
//					wInvCurrentHeavyFlg = false;
//					wInvCurrentHeavyRstCnt = 0;
//				}	
//			}
//			else
//			{
...
//		if(wInvOffPoint > 0)
//		{
//			if((wCtrlSinpointer > (192 - wInvOffPoint) && wCtrlSinpointer < 192)
//			|| (wCtrlSinpointer > (384 - wInvOffPoint)))
//			{
//				mDisInvPwmOut();
//				wInvOffFlg = true;
//			}
...
//		wInvOffPoint = 45;
		...
		if(abs(wInvLCurrSample1) > wInvOverCurrentLimit*g_wInvOverCurrentFactor)
		{
			...
			if(wInvOverCurrentLimit != g_wInvOverCurrRated/2)//40->g_wInvOverCurrRated/2
			{
				if(++wInvOverCurrentCnt >= 5)   //20
				{
					...
					wFaultCode = cRInvOverCurrent;//软件过流保护				
	...
	else if(bPVMode == cLineMode )
	{
		if(abs(wInvLCurrSample1) > g_wInvLCurrLineLevel1)  //80*25=1920 //George 150112 2141->g_wInvLCurrLineLevel1
		{
			...
			if(wInvOverCurrentLineModeCnt >= 3)       
			{
				wInvOverCurrentLineModeCnt = 0;
		...
		if((abs(wInvLCurrSample1) > g_wInvLCurrLineLevel2)//40*25=960 
			...
			||(wBatAvgVoltNew > mEEBatVoltOverShut())||(ubBatVoltOveCnt > 10)\
	...
	if(bPVMode == cBatteryMode )
	{
//		if((EPwm1Regs.TZFLG.bit.CBC == 1) || (EPwm2Regs.TZFLG.bit.CBC == 1))//TZ口触发硬件过流保护信号
//		{
//			EALLOW;
...
//			EDIS;
//			
//			wInvOffPointHw = 150;//保护点150个中断周期
//			ProtectFlag2 =1;
//		}
		
		if(LiHardProtectCNT>=3)//过流保护触发
		{
			
			if((wInvOffPointHw ==0)&&(wContinueTrig==0))
			{					
				ubHardProtectFlag =true;
				//wInvOffPointHw =128;
				wInvOffPointHw =150;// 过流保护时间150中断
				wContinueTrig =1;
...
****************************************************************************************/
		...
		wRLoadCurrCntl = (INT16S)(((INT32S)wOPCurrSample * g_wOPCurrCntlFactor) >> 5);	//实时电流，变压器输出的实时电流
		wRInvCurrCntl = -(INT16S)(((INT32S)wInvLCurrSample * g_wInvLCurrCntlFactor) >> 5); //逆变电流,滤波后的电流
...
//		sFBINVController2();
	...
	//swGetRLineADSum(wLineVoltReal);
	//sRLineWavChk(wLineWavSum,cLineWavLowLoss,cLineWavHighLoss,1,10);
	//if(sbGetEepromModeSelect() == 0)//APL
	//{
	//	sRLineWavChk(wLineWavSum,cAPLLineWavLowLoss,cLineWavHighLoss,1,10);//75V loss
	//}
	//else
	//{
	//	sRLineWavChk(wLineWavSum,cUPSLineWavLowLoss,cLineWavHighLoss,1,10);// 150V loss
	//}

	swGetLineQuadraticSum(wLineVoltReal );
	if(sbGetEepromModeSelect() == 0)	// APL
	{
		sRLinePeakVoltChk(gi_wRLineVoltRealQuadratic,cLine70VLowLoss,cLine290VHighLoss,6,120);//
	}
	else
	{
		sRLinePeakVoltChk(gi_wRLineVoltRealQuadratic,cLine150VLowLoss,cLine290VHighLoss,6,120);//
	}
	HardProtectChk();
...
//	}
	#ifdef IVPL1512
		AdcRegs.ADCSOCFRC1.all=0x00A8;//ADC Channel3,5,7
	#else
		AdcRegs.ADCSOCFRC1.all=0x0062;//ADC Channel1,5,6
	#endif
	...
	wInvVoltSample2  = (INT16S)(((INT32S)1610 * pSinTab[wCtrlSinpointer] >> 14));
		...
		#ifdef IVPL1512
			wLineVoltSample2 = ADCRESULT3-2048;   //1V = 4.3977CNT
			wInvLCurrSample2 = ADCRESULT7-2048-wInvCurrSampleBias;	  //1A = 12.784CNT
		#else
...
//		}
		...
		if(LiHardProtectCNT>=3)//过流保护
		{
			if((wInvOffPointHw ==0)&&(wContinueTrig==0))
			{				
				ubHardProtectFlag =true;//硬件触发故障标志位，连续触发达到6次报故障
				//wInvOffPointHw =128;
				wInvOffPointHw =150;
				wContinueTrig =1;
//				wInvOffFlg =1;
...
/*void  swGetRLineADSum(INT16U wRLineVoltRealTemp )
{
	...
	if(++wSinPointer_WaveDetCnt >= 5)   // 9*16*4=384
	{
		wSinPointer_WaveDetCnt=0;
		wLineWavSum =wLineWavSum + abs(wRLineVoltRealTemp) -*(wRLineWavRealArray + wLineWavCnt);
		
		*(wRLineWavRealArray + wLineWavCnt) = abs(wRLineVoltRealTemp);
...
void	sRLineWavChk(INT16U wRLineAdAbsSum,INT16U wLineWavChkLLevel,INT16U wLineWavChkHLevel,INT8U bLossFilter,INT8U bBackFilter)
{
	static INT8U	bRLineWavBackCnt = 0;
	static INT8U	bRLineWavLossCnt = 0;
	
	if(!fLine.RWavLoss)
	{
		if((wRLineAdAbsSum < wLineWavChkLLevel)||(wRLineAdAbsSum > wLineWavChkHLevel))
		{
			bRLineWavLossCnt++;
			if(bRLineWavLossCnt>=bLossFilter)
			{
				bRLineWavLossCnt=0;
				//fLine.RWavLoss = 1;
				wRLineLossVoltage=(INT16U)(((INT32U)wRLineAdAbsSum*73)>>9);//wRLineAdAbsSum/7
				if(mGetLineWavLossSts() == false)
				{
					mSetLineWavLossSts(true);
					OSISREventSend(cPrioSuper, eSuperLineLoss);  
	...
	{
		if((wLineWavChkLLevel < wRLineAdAbsSum)&&(wRLineAdAbsSum < wLineWavChkHLevel))
		{
			bRLineWavBackCnt++;
			if(bRLineWavBackCnt >= bBackFilter)
			{
				bRLineWavBackCnt = 0;
				//fLine.RWavLoss = 0;
				if(mGetLineWavLossSts() == true)
...
extern INT16U sWLiHardProtectCnt;
void HardProtectChk(void)
{
	static INT16S swLiHardProtectDelay = 0;
	if(LiHardProtect==0)//过流保护硬件触发
	{
		sWLiHardProtectCnt++;
		LiHardProtectCNT++;
		if(sWLiHardProtectCnt>100)
		{
...
//		LiHardProtectCNT++;
	...
	{
		if(++swLiHardProtectDelay > 10)//防止间隔性触发没有保护的情况
		{
			swLiHardProtectDelay = 0;
