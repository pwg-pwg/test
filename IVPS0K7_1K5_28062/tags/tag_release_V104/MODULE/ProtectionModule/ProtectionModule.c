...
void sAvrTempChk(INT8U bFilter)
{
	...
	{
		if(sbUnderLevelChk(wTempDegreeTxt, cTxtTempStatusBase, bFilter, &bHighDegreeCntB) == true)
		{
			bTxTempInfo = 1;
...
void sInvTempChk(INT8U bFilter)
{
	...
	wHsTempMax = swGetMaxHsTemp();
	if(sbGetInvTempStatus() < 2)
	{
		if(sbOverLevelChk(wHsTempMax, cHsTempStatusBase + 10, bFilter, &bHighDegreeCntB) == true)
		{
			bHsTempInfo = 2;
	...
	{
		if(sbUnderLevelChk(wHsTempMax, cHsTempStatusBase, bFilter, &bHighDegreeCntB) == true)
		{
			bHsTempInfo = 1;
...
void sAvrTempDeratingChk(INT8U bFilter)
{
	...
	if(bPVMode == cBatteryMode)
	{
		if(g_wVAType == cb10KVAModel)
		{
			...
			{
				if(sbUnderLevelChk(wTempDegreeTxt, cTxtTempDerateBack10KVA, bFilter, &bTxtTempOvDerate) == true)
				{
					fProtection.TxtOverDerate = 0;
				}
			}
			...
			{
				if(sbOverLevelChk(wTempDegreeTxt, cTxtTempDerate10KVA, bFilter, &bTxtTempOvDerate) == true)
				{
		...
		else
		{
			if(1 == fProtection.TxtOverDerate)
			{
				if(sbUnderLevelChk(wTempDegreeTxt, cTxtTempDerateBack, bFilter, &bTxtTempOvDerate) == true)
				{
					fProtection.TxtOverDerate = 0;
				}
			}
			...
			{
				if(sbOverLevelChk(wTempDegreeTxt, cTxtTempDerate, bFilter, &bTxtTempOvDerate) == true)
				{
					fProtection.TxtOverDerate = 1;
				}
			}
		...
		{
			if(sbUnderLevelChk(wHsTempMax, cHsTempDerateBack, bFilter, &bInvTempOvDerate) == true)
			{
				fProtection.InvOverDerate = 0;
			}
		}
		...
		{
			if(sbOverLevelChk(wHsTempMax, cHsTempDerate, bFilter, &bInvTempOvDerate) == true)
			{
